...
Guidance for the automated reviewer (cypher-cli-bot) on PRs in this repo.
...
- `knip` unused exports
- `cypher_change` marker rules — both directions:
  - Missing markers on shared opencode files (`script/check-opencode-annotations.ts`)
  - Markers present in cypher-only paths like `packages/cypher-vscode/`, `packages/cypher-ui/`, `packages/opencode/src/cypher/` (`bun run check-cypher-change`)
- Workflow allowlist drift (`script/check-workflows.ts`)
- Stale `packages/cypher-docs/source-links.md` (`script/extract-source-links.ts`)
...
- Generated artifact freshness (`check-cypher-generated-artifacts.yml`)
...
Cypher CLI is a fork of opencode. Minimizing diff against upstream is a top priority.

- If a change modifies a shared opencode file (anything under `packages/opencode/` not in a path containing `cypher`), ask whether the logic could live in a Cypher-only directory instead (`packages/opencode/src/cypher/`, `packages/cypher-gateway/`, etc.) or be reduced to a smaller hook.
- Refactors or reorganizations of upstream code are a red flag — flag them unless clearly justified.
- See `.cypher/skills/cypher-merge-minimizer/SKILL.md` for the decision rules.

### 4. Cloud config schema mirror

When `Config.Info` in `packages/opencode/src/config/config.ts` gains a new `cypher_change` field, the matching JSON Schema entry must also be added in the cloud repo (`apps/web/src/app/config.json/extras.ts`). CI does **not** check this — flag it as a reminder if you see a new config field added.
...
For changes under `packages/cypher-vscode/webview-ui/`:
