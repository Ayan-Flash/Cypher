...
import { getCypherProfile, getCypherDefaultModel, promptOrganizationSelection } from "../api/profile.js"
...
export async function authenticateWithDeviceAuth(): Promise<DeviceAuthResult> {
  ...
  const profileData = await getCypherProfile(token)
  ...
  const model = await getCypherDefaultModel(token, organizationId)
