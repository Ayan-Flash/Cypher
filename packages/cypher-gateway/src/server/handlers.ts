...
import { fetchCypherPassState } from "../api/cypher-pass.js"
import { fetchCyphercodeNotifications } from "../api/notifications.js"
import { clearModesCache } from "../api/modes.js"
import { HEADER_ORGANIZATIONID, CYPHER_API_BASE, CYPHER_CHAT_URL, CYPHER_EVENT_SERVICE_URL } from "../api/constants.js"
import type { CyphercodeBalance, CyphercodeProfile, CypherPassState } from "../types.js"
import { buildCypherHeaders } from "../headers.js"

export type CypherAuth =
  | { type: "api"; key: string }
  | { type: "oauth"; access: string; refresh: string; expires: number; accountId?: string }
  | { type: "wellknown"; key: string; token: string }

export interface CypherProfileResult {
  profile: CyphercodeProfile
...
export interface AuthStore {
  get(provider: string): Promise<CypherAuth | undefined>
  set(provider: string, auth: Extract<CypherAuth, { type: "oauth" }>): Promise<void>
}
...
export function getToken(auth: CypherAuth | undefined) {
  if (auth?.type === "api") return auth.key
  if (auth?.type === "oauth") return auth.access
  return undefined
}

export function getOrganizationId(auth: CypherAuth | undefined) {
  if (auth?.type === "oauth") return auth.accountId
  return undefined
}

export async function getProfile(auth: AuthStore): Promise<CypherProfileResult> {
  const info = await auth.get("cypher")
  if (!info || info.type !== "oauth") throw new UnauthorizedError("Not authenticated with Cypher Gateway")

  const currentOrgId = info.accountId ?? null
  const [profile, balance, cypherPass] = await Promise.all([
...
export async function getNotifications(auth: AuthStore) {
  ...
  return fetchCyphercodeNotifications({
    cypherToken: token,
...
export async function setOrganization(deps: OrganizationDeps, organizationId: string | null) {
  const info = await deps.auth.get("cypher")
  if (!info || info.type !== "oauth") throw new UnauthorizedError("Not authenticated with Cypher Gateway")

  await deps.auth.set("cypher", {
    type: "oauth",
...
export async function getCloudSessions(token: string, input: CloudSessionsInput) {
  ...
  const response = await fetch(`${CYPHER_API_BASE}/api/trpc/cliSessionsV2.list?${params.toString()}`, {
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json",
      ...buildCypherHeaders(),
  ...
  if (!response.ok) {
    ...
    console.error("[Cypher Gateway] cloud-sessions: tRPC request failed", {
