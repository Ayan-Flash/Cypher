/**
 * Cypher Gateway specific routes
 * Handles profile fetching and organization management for Cypher Gateway provider
 *
 * This factory function accepts OpenCode dependencies to create Cypher-specific routes
 */

import { fetchCyphercodeNotifications, CyphercodeNotificationSchema } from "../api/notifications.js"
import { fetchCypherImageModels } from "../api/models.js"
import { fetchOrganizationModes, clearModesCache } from "../api/modes.js"
import { CYPHER_API_BASE, HEADER_FEATURE, HEADER_ORGANIZATIONID } from "../api/constants.js"
...
interface CypherRoutesDeps extends ImportDeps {
  Hono: new () => Hono
...
/**
 * Create Cypher Gateway routes with OpenCode dependencies injected
 *
 * @example
 * ```typescript
 * import { createCypherRoutes } from "@cypher/cypher-gateway"
 ...
 *
 * export const CypherRoutes = createCypherRoutes({
 *   Hono,
 *   describeRoute,
 *   validator,
 *   resolver,
 ...
 * ```
 */
export function createCypherRoutes(deps: CypherRoutesDeps) {
  const {
    Hono,
    describeRoute,
    validator,
  ...
  const CypherPassState = z.object({
  ...
  const ProfileWithBalance = z.object({
    ...
    cypherPass: CypherPassState.nullable(),
  ...
  const getProxyAuth = async () => {
    const auth = await Auth.get("cypher")
  ...
  return new Hono()
    .get(
      "/profile",
      describeRoute({
        summary: "Get Cypher Gateway profile",
        description: "Fetch user profile and organizations from Cypher Gateway",
        operationId: "cypher.profile",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          return c.json({ error: "Not authenticated with Cypher Gateway" }, 401)
        }
      },
    )
    .post(
      "/organization",
      describeRoute({
        summary: "Update Cypher Gateway organization",
        description: "Switch to a different Cypher Gateway organization",
        operationId: "cypher.organization.set",
        responses: {
          200: {
    ...
    .post(
      ...
      describeRoute({
        ...
        description: "Proxy a Fill-in-the-Middle completion request to the Cypher Gateway",
        operationId: "cypher.fim",
        responses: {
          200: {
    ...
    .post(
      ...
      describeRoute({
        ...
        description: "Proxy an audio transcription request to the Cypher Gateway",
        operationId: "cypher.audio.transcriptions",
        responses: {
          200: {
      ...
      async (c: any) => {
        ...
        if (!proxy.auth) return c.json({ error: "Not authenticated with Cypher Gateway" }, 401)
        ...
        const headers = {
          ...
          ...buildCypherHeaders(undefined, { cypherOrganizationId: proxy.organizationId }),
          [HEADER_FEATURE]: "vscode-extension",
        }

        const response = await fetch(`${CYPHER_API_BASE}/api/gateway/v1/audio/transcriptions`, {
          method: "POST",
          headers,
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "List image-capable models from the Cypher Gateway OpenRouter passthrough",
        operationId: "cypher.models.images",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          const result = await fetchCypherImageModels({
            cypherToken: proxy.token,
            cypherOrganizationId: proxy.organizationId,
          })
          if (result.error) {
    ...
    .post(
      ...
      describeRoute({
        ...
        description:
          "Proxy an image generation request (chat-completions with modalities) to the Cypher Gateway OpenRouter passthrough",
        operationId: "cypher.image.generations",
        responses: {
          200: {
      ...
      async (c: any) => {
        ...
        const response = await fetch(`${CYPHER_API_BASE}/api/openrouter/chat/completions`, {
          method: "POST",
          headers,
          signal: c.req.raw.signal,
    ...
    .get(
      "/notifications",
      describeRoute({
        summary: "Get Cypher notifications",
        description: "Fetch notifications from Cypher Gateway for CLI display",
        operationId: "cypher.notifications",
        responses: {
          200: {
            description: "Notifications list",
            content: {
              "application/json": {
                schema: resolver(z.array(CyphercodeNotificationSchema)),
              },
            },
          },
          ...errors(400, 401),
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "Fetch full session data from the Cypher cloud for preview",
        operationId: "cypher.cloud.session.get",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          const auth = await Auth.get("cypher")
          if (!auth) return c.json({ error: "Not authenticated with Cypher Gateway" }, 401)
          const token = auth.type === "api" ? auth.key : auth.type === "oauth" ? auth.access : undefined
          ...
          console.error("[Cypher Gateway] cloud/session/get: unhandled error", err?.message ?? err)
    ...
    .post(
      ...
      describeRoute({
        ...
        operationId: "cypher.cloud.session.import",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          const auth = await Auth.get("cypher")
          if (!auth) return c.json({ error: "Not authenticated with Cypher" }, 401)
          const token = auth.type === "api" ? auth.key : auth.type === "oauth" ? auth.access : undefined
          if (!token) return c.json({ error: "No valid token found" }, 401)
          ...
          console.error("[Cypher Gateway] cloud/session/import: unhandled error", err?.message ?? err)
          return c.json({ error: "Internal error" }, 500)
        }
      },
    )
    .get(
      "/claw/status",
      describeRoute({
        summary: "Get CypherClaw instance status",
        description: "Fetch the user's CypherClaw instance status via the CypherClaw worker",
        operationId: "cypher.claw.status",
        responses: {
          200: {
              ...
              "application/json": {
                schema: resolver(
                  z.object({
                    ...
                    // cloud `services/cypherclaw/src/index.ts` and the
                    // `PlatformStatusResponse` type in
                    // cloud/apps/web/src/lib/cypherclaw/types.ts. Keeping them in
      ...
      async (c: any) => {
        try {
          ...
          if (err instanceof GatewayError) {
            return c.json({ error: `CypherClaw request failed: ${err.status} ${err.message}` }, err.status as any)
          }
          console.error("[Cypher Gateway] claw/status: error", err?.message ?? err)
          return c.json({ error: "Failed to reach CypherClaw" }, 502)
        }
      },
    )
    .get(
      ...
      describeRoute({
        summary: "Get CypherClaw chat credentials",
        description:
          "Returns the bearer token and endpoint URLs the client uses to talk to the Cypher Chat worker " +
          "and the Event Service. The bearer is the user's existing long-lived Cypher JWT — cypher-chat and " +
          "event-service both verify it directly with NEXTAUTH_SECRET, so no separate token mint is needed.",
        operationId: "cypher.claw.chatCredentials",
        responses: {
          200: {
            description: "Cypher Chat credentials or null",
            content: {
              "application/json": {
                schema: resolver(
                  z
    ...
    .get(
      ...
      describeRoute({
        ...
        description: "Fetch cloud CLI sessions from Cypher API",
        operationId: "cypher.cloudSessions",
        responses: {
          200: {
      ...
      async (c: any) => {
        try {
          ...
          console.error("[Cypher Gateway] cloud-sessions: unhandled error", err?.message ?? err)
          return c.json({ error: "Internal error" }, 500)
        }
      },
    )
