    ...
    return fetch(url, {
      ...
      headers: {
        ...
        ...(target.provider === "cypher"
          ? buildCypherHeaders(undefined, { cypherOrganizationId: input.organizationId })
          : {}),
...
export function createFimHandler(Auth: Auth) {
  return async (c: any) => {
    ...
    const proxy = target.provider === "cypher" ? await getProxyAuth(Auth) : undefined
    const token = target.provider === "cypher" ? proxy?.token : await getProviderKey(Auth, target.provider)

    if (target.provider === "cypher" && !proxy?.auth) {
      return c.json({ error: "Not authenticated with Cypher Gateway" }, 401)
    }

    if (target.provider === "cypher" && !token) {
