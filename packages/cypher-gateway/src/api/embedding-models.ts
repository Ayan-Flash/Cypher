import { z } from "zod"
import { resolveCypherGatewayBaseUrl } from "./url.js"
...
export type CypherEmbeddingModelCatalogIssue = {
...
type Options = {
  ...
  signal?: AbortSignal
  attempts?: number
  onError?: (issue: CypherEmbeddingModelCatalogIssue) => void
}
...
export async function fetchCypherEmbeddingModelCatalog(options: Options = {}): Promise<CypherEmbeddingModelCatalog> {
  const url = new URL("embedding-models", resolveCypherGatewayBaseUrl({ baseURL: options.baseURL, token: options.token }))
  const requested = options.attempts ?? 3
  const attempts = Number.isFinite(requested) ? Math.min(3, Math.max(1, Math.floor(requested))) : 3
  const issue = { current: undefined as CypherEmbeddingModelCatalogIssue | undefined }
  ...
  for (const attempt of Array.from({ length: attempts }, (_, index) => index)) {
    ...
    try {
      ...
      if (!response.ok) {
        issue.current = {
          code: "http",
          message: `Unable to load Cypher embedding models (HTTP ${response.status}).`,
      ...
      issue.current = {
        ...
        message: "Cypher returned an invalid embedding model catalog.",
      }
      break
    } catch (err) {
      if (options.signal?.aborted) throw options.signal.reason
      issue.current = {
        code: "network",
        message: "Unable to connect to Cypher to load embedding models. Check your network connection and try again.",
      }
  ...
  return EMPTY_CYPHER_EMBEDDING_MODEL_CATALOG
}
