...
import type { CyphercodeProfile, Organization, CyphercodeBalance } from "../types.js"
import { CYPHER_API_BASE, DEFAULT_MODEL, DEFAULT_FREE_MODEL } from "./constants.js"

/**
 * Fetch user profile from Cypher API
 */
export async function fetchProfile(token: string): Promise<CyphercodeProfile> {
  const response = await fetch(`${CYPHER_API_BASE}/api/profile`, {
    headers: {
  ...
  // Transform to flat CyphercodeProfile structure
  return {
 ...
 */
export const getCypherProfile = fetchProfile
 ...
 */
export function defaultOrganizationId(profile: CyphercodeProfile): string | undefined {
...
/**
 * Fetch user balance from Cypher API
 * @param token - Authentication token
 * @param organizationId - Optional organization ID for team balance
 */
export async function fetchBalance(token: string, organizationId?: string): Promise<CyphercodeBalance | null> {
  try {
    ...
    if (organizationId) {
      headers["x-cypher-organizationid"] = organizationId
    }

    const response = await fetch(`${CYPHER_API_BASE}/api/profile/balance`, { headers })
 ...
 * Alias for compatibility with existing code
 */
export const getCypherBalance = fetchBalance
...
export async function fetchDefaultModel(token?: string, organizationId?: string): Promise<string> {
  ...
  const url = `${CYPHER_API_BASE}${path}`
 ...
 * Alias for compatibility with existing code
 */
export const getCypherDefaultModel = fetchDefaultModel
