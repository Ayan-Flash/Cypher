...
import { CYPHER_API_BASE, CYPHER_OPENROUTER_BASE, MODELS_FETCH_TIMEOUT_MS, PROMPTS, AI_SDK_PROVIDERS } from "./constants.js"

export type CypherModelsResult = {
...
/**
 * Fetch models from Cypher API (OpenRouter-compatible endpoint)
 *
 * @param options - Configuration options
 * @returns Typed result with models and optional error info
 */
export async function fetchCypherModels(options?: {
  cypherToken?: string
  cypherOrganizationId?: string
  baseURL?: string
}): Promise<CypherModelsResult> {
  const raw = await fetchRawCypherModels(options)
  ...
  for (const model of raw.data) {
    ...
    // Skip models that don't support tools — Cypher requires tool calling
...
export type CypherImageModel = {
  id: string
  name: string
  description?: string
}

export type CypherImageModelsResult = {
  models: CypherImageModel[]
  error?: { kind: "unauthorized" | "network" | "schema" | "http"; status?: number }
}

/**
 * Fetch image-capable models from Cypher API (OpenRouter-compatible endpoint).
 * Uses the same raw fetch as {@link fetchCypherModels} but keeps only models
 * whose `output_modalities` include `"image"`.
 */
export async function fetchCypherImageModels(options?: {
  cypherToken?: string
  cypherOrganizationId?: string
  baseURL?: string
}): Promise<CypherImageModelsResult> {
  const raw = await fetchRawCypherModels(options)
  if (raw.error) return { models: [], error: raw.error }

  const models: CypherImageModel[] = []
...
/**
 * Shared raw fetch + validate used by both {@link fetchCypherModels} and {@link fetchCypherImageModels}.
 */
async function fetchRawCypherModels(options?: {
  cypherToken?: string
  cypherOrganizationId?: string
  baseURL?: string
}): Promise<
  { data: OpenRouterModel[]; error?: undefined } | { data?: undefined; error: NonNullable<CypherModelsResult["error"]> }
> {
  const token = options?.cypherToken
  const organizationId = options?.cypherOrganizationId

  // Construct base URL
  const defaultBaseURL = organizationId ? `${CYPHER_API_BASE}/api/organizations/${organizationId}` : CYPHER_OPENROUTER_BASE

  const baseURL = options?.baseURL ?? defaultBaseURL

  // Transform URL with token if available
  const finalBaseURL = token ? getCypherUrlFromToken(baseURL, token) : baseURL
  ...
  const response = await fetch(modelsURL, {
    headers: {
      ...getDefaultHeaders(),
      ...buildCypherHeaders(undefined, { cypherOrganizationId: organizationId }),
