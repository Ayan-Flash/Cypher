// Verifies fetchCypherModels typed result and 401 fallback behaviour.

import { test, expect } from "bun:test"
import { fetchCypherModels } from "../../src/api/models.js"
...
test("returns empty models and error when both auth and public requests return 401", async () => {
  ...
  const result = await fetchCypherModels({ cypherToken: "bad-token" })
...
test("falls back to public endpoint on 401 and returns models", async () => {
  ...
  const result = await fetchCypherModels({
    cypherToken: "expired-token",
...
test("preserves Auto Efficient routing metadata as a dedicated model field", async () => {
  ...
  expect(result.error).toBeUndefined()
  expect(result.models["cypher-auto/efficient"].autoRouting).toEqual({
    models: ["google/gemini-2.5-flash", "anthropic/claude-sonnet-4.6"],
  })
})
