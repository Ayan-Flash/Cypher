...
import { parseCypherPassState } from "../../src/api/cypher-pass"

describe("parseCypherPassState", () => {
  test("parses batched tRPC subscription data", () => {
    const state = parseCypherPassState([
      {
        result: {
          data: {
            json: {
  ...
  test("returns null without period amounts", () => {
    expect(parseCypherPassState({ status: "none" })).toBeNull()
  })
})
