...
describe("Edit target resolution", () => {
  ...
  test("does NOT route the FIM Mercury model to the edit endpoint", () => {
    // `mercury-edit-2` (kind: fim) must fall through to the cypher placeholder,
    // not the edit endpoint — only `mercury-next-edit` (kind: edit) is NES.
    expect(resolveEditTarget("inception", "mercury-edit-2").provider).toBe("cypher")
  })

  test("routes the Cypher Gateway next-edit model to the gateway proxy", () => {
    const target = resolveEditTarget("cypher", "inception/mercury-next-edit")
    expect(target.provider).toBe("cypher")
    expect(target.model).toBe("inception/mercury-edit-2")
    expect(target.url).toMatch(/\/api\/edit\/completions$/)
  })

  test("falls back to a cypher placeholder (no upstream) for non-edit models", () => {
    expect(resolveEditTarget("cypher", "mistralai/codestral-2508")).toEqual({
      provider: "cypher",
  ...
  test("routes the default model to the gateway proxy", () => {
    expect(resolveEditTarget()).toEqual({
      ...
      url: "https://api.cypher.ai/api/edit/completions",
    })
  })
})
