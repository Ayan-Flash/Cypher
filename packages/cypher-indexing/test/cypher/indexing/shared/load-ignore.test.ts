...
describe("loadIgnore", () => {
  ...
  test("loads root .cypherignore in addition to root .gitignore", async () => {
    await writeFile(path.join(root, ".gitignore"), "dist/\n")
    await writeFile(path.join(root, ".cypherignore"), "secret/\n")
  ...
  test("preserves existing .gitignore-only behavior when .cypherignore is absent", async () => {
  ...
  test("loads nested .cypherignore relative to its directory", async () => {
    await mkdir(path.join(root, "pkg", "sub"), { recursive: true })
    await writeFile(path.join(root, "pkg", ".cypherignore"), "secret.ts\n")
  ...
  test("allows descendants when a parent directory is re-included", async () => {
    await mkdir(path.join(root, "pkg"), { recursive: true })
    await writeFile(path.join(root, ".gitignore"), "pkg/\n")
    await writeFile(path.join(root, ".cypherignore"), "!pkg/\n")
  ...
  test("keeps descendants ignored when only a child directory is re-included", async () => {
    await mkdir(path.join(root, "pkg", "sub"), { recursive: true })
    await writeFile(path.join(root, ".gitignore"), "pkg/\n")
    await writeFile(path.join(root, ".cypherignore"), "!pkg/sub/\n")
  ...
  test("applies .cypherignore after .gitignore in the same directory", async () => {
    await writeFile(path.join(root, ".gitignore"), "*.ts\n")
    await writeFile(path.join(root, ".cypherignore"), "!keep.ts\n")
  ...
  test("ignores the ignore files themselves", async () => {
    ...
    expect(ig.ignores(".cypherignore")).toBe(true)
  })

  test("keeps ignore files ignored after negation rules", async () => {
    await writeFile(path.join(root, ".cypherignore"), "!.gitignore\n")
  ...
  test("ignores ignore file names even when absent during loading", async () => {
    ...
    expect(ig.ignores(".gitignore")).toBe(true)
    expect(ig.ignores("pkg/.cypherignore")).toBe(true)
  })
