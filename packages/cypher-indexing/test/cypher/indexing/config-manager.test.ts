...
describe("CodeIndexConfigManager", () => {
  ...
  test("configures Cypher with hosted auth options and explicit model metadata", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "cypher",
        openAiKey: undefined,
        cypherApiKey: "cypher-token",
        cypherBaseUrl: "https://example.test/api/gateway/",
    ...
    expect(cfg.isFeatureConfigured).toBe(true)
    expect(cfg.getConfig().cypherOptions).toEqual({
      apiKey: "cypher-token",
  ...
  test("requires Cypher model metadata from Cloud config", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "cypher",
        openAiKey: undefined,
  ...
  test("uses configured dimension for Cypher models outside the fallback catalog", () => {
    const cfg = new CodeIndexConfigManager(
      createInput({
        embedderProvider: "cypher",
        openAiKey: undefined,
  ...
  describe("loadConfiguration restart checks", () => {
    ...
    test("requires restart when Cypher auth changes", () => {
      const cfg = new CodeIndexConfigManager(
        createInput({
          embedderProvider: "cypher",
          openAiKey: undefined,
          cypherApiKey: "old-token",
          modelId: "mistralai/mistral-embed-2312",
          modelDimension: 1024,
        }),
      )
