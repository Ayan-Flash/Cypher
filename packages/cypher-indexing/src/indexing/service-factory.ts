...
import { CypherEmbedder } from "./embedders/cypher"
...
export class CodeIndexServiceFactory {
  ...
  public createEmbedder(): IEmbedder {
    ...
    if (provider === "cypher") {
      if (!config.cypherOptions?.apiKey) throw new Error("Cypher API key is required for embedding.")
      if (!config.modelId) throw new Error("Cypher embedding model is required.")
      return new CypherEmbedder({
        apiKey: config.cypherOptions.apiKey,
        baseUrl: config.cypherOptions.baseUrl,
        organizationId: config.cypherOptions.organizationId,
