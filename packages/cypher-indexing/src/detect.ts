export const INDEXING_PLUGIN_NAMES = ["cypher-indexing", "@cypher/cypher-indexing"] as const
...
function isPathSpecifier(value: string): boolean {
  ...
  if (normalized.includes("/.opencode/") || normalized.includes("/.cypher/") || normalized.includes("/.cypher/")) {
    return true
  }
...
function normalizePath(value: string): string {
  ...
  const scoped = parts.findIndex((part, i) => part === "@cypher" && parts[i + 1] === "cypher-indexing")
  if (scoped >= 0) return "@cypher/cypher-indexing"

  const workspace = parts.findIndex((part, i) => part === "packages" && parts[i + 1] === "cypher-indexing")
  if (workspace >= 0) return "@cypher/cypher-indexing"
