...
import * as CypherOAuthCallback from "../cypher/mcp-oauth-callback" // cypher_change
...
<head>
  <!-- cypher_change start -->
  <title>Cypher - Authorization Successful</title>
  <!-- cypher_change end -->
  <style>
...
<head>
  <!-- cypher_change start -->
  <title>Cypher - Authorization Failed</title>
  <!-- cypher_change end -->
  <style>
    body { font-family: system-ui, -apple-system, sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #1a1a2e; color: #eee; }
    .container { text-align: center; padding: 2rem; }
...
export async function ensureRunning(redirectUri?: string): Promise<void> {
  // cypher_change start - delegate Cypher-specific callback binding from here because OAuth state lives in this module
  await CypherOAuthCallback.ensureRunning({
    redirectUri,
  ...
  await new Promise<void>((resolve, reject) => {
    // cypher_change start - EADDRINUSE can still fire when another process
    ...
    server!.listen(currentPort, () => {
      server!.off("error", onError) // cypher_change
