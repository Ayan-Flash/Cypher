  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const filepath = yield* Effect.cached(
        Effect.gen(function* () {
          // cypher_change start - Git for Windows can expose an MSYS rg.exe that fails when spawned natively
      ...
      const tree: Interface["tree"] = Effect.fn("Ripgrep.tree")(function* (input: TreeInput) {
        ...
        for (const file of list) {
          if (file.includes(".cypher") || file.includes(".cypher")) continue // cypher_change
