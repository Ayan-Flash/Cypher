...
export const Action = Schema.NullOr(Schema.Literals(["ask", "allow", "deny"])) // cypher_change - nullable allows null as a delete sentinel
...
const InputObject = Schema.StructWithRest(
  Schema.Struct({
    ...
    agent_manager: Schema.optional(Rule), // cypher_change
    // cypher_change start
...
const normalizeInput = (input: Schema.Schema.Type<typeof InputSchema>): Schema.Schema.Type<typeof InputObject> =>
  input === null || typeof input === "string" ? { "*": input } : input // cypher_change
