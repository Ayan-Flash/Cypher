...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/cypher" // cypher_change
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "cypher") // cypher_change
    default:
      return "/etc/cypher" // cypher_change
  }
}

export function managedConfigDir() {
  return process.env.CYPHER_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
