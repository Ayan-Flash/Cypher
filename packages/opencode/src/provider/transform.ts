...
import { cypherProviderOptions } from "@/cypher/provider-options"
import { isLing } from "@/cypher/model-match" // cypher_change
...
function sdkKey(npm: string): string | undefined {
  switch (npm) {
    ...
    case "@openrouter/ai-sdk-provider":
      return "openrouter"
    case "@cypher/cypher-gateway": // cypher_change
      return "openrouter"
...
function normalizeMessages(
  ...
  // cypher_change start - only filter for Claude models on Bedrock, not all Bedrock models
...
export function temperature(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 0.3 // cypher_change
  return undefined
}
...
export function topP(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 0.95 // cypher_change
  return undefined
}
...
export function topK(model: Provider.Model) {
  ...
  if (isLing(model.api.id)) return 20 // cypher_change
  return undefined
}
...
// cypher_change start - fable and sonnet-5 models are adaptive thinking models like opus-4.7/4.8
...
function anthropicAdaptiveEfforts(apiId: string): string[] | null {
  // cypher_change start - treat opus-4.8 and fable like opus-4.7
...
export function variants(model: Provider.Model): Record<string, Record<string, any>> {
  // cypher_change start
  if (
    ["@cypher/cypher-gateway", "@ai-sdk/openai-compatible"].includes(model.api.npm) &&
    model.variants &&
  ...
  const adaptiveOpus = anthropicOpus47OrLater(model.api.id) || anthropicClaude5(model.api.id) // cypher_change - fable/sonnet-5
  ...
  if (
    ...
    // id.includes("minimax") || // cypher_change
    // id.includes("glm") || // cypher_change
    // id.includes("kimi") || // cypher_change
  ...
  if (id.includes("grok") && id.includes("grok-3-mini")) {
    if (model.api.npm === "@openrouter/ai-sdk-provider" || model.api.npm === "@cypher/cypher-gateway") {
      // cypher_change - add Cypher Gateway support
      return {
  ...
  switch (model.api.npm) {
    ...
    case "@openrouter/ai-sdk-provider":
      ...
      if (
        ...
        !id.includes("claude") &&
        !model.id.includes("mercury") // cypher_change
      )
        return {}
      return Object.fromEntries(
        (model.api.npm === "@cypher/cypher-gateway" || !id.includes("gpt") // cypher_change
          ? OPENAI_EFFORTS
    ...
    case "@ai-sdk/azure":
      ...
      return Object.fromEntries(
        (GPT5_FAMILY_RE.test(id) && gpt5Version(id) === undefined
          ...
          .concat(model.release_date >= OPENAI_XHIGH_EFFORT_RELEASE_DATE ? ["xhigh"] : []) // cypher_change
          .map((effort) => [
            effort,
            {
              reasoningEffort: effort,
    ...
    case "@ai-sdk/google-vertex/anthropic":
      ...
      // cypher_change start - MiniMax M-series toggles thinking on/off rather than exposing effort levels
    ...
    case "@ai-sdk/mistral":
      ...
      // https://docs.mistral.ai/studio-api/conversations/reasoning // cypher_change
      if (!model.capabilities.reasoning) return {}
...
export function options(input: {
  ...
  if (
    ...
    input.model.api.npm === "@cypher/cypher-gateway" // cypher_change
  ) {
    result["usage"] = {
      include: true,
    }
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    if (!input.model.api.id.includes("gpt-5-pro")) {
      ...
      if (
        ...
        input.model.api.npm === "@ai-sdk/github-copilot" || // cypher_change
        input.model.api.npm === "@openrouter/ai-sdk-provider" || // cypher_change
        input.model.api.npm === "@cypher/cypher-gateway" // cypher_change
      ) {
        result["reasoningSummary"] = "auto"
    ...
    if (
      // cypher_change start - gate textVerbosity to Responses-API providers
      (input.model.api.npm === "@ai-sdk/openai" ||
        input.model.api.npm === "@ai-sdk/azure" ||
        input.model.api.npm === "@ai-sdk/github-copilot" ||
        input.model.api.npm === "@openrouter/ai-sdk-provider" ||
        input.model.api.npm === "@cypher/cypher-gateway") &&
      // cypher_change end
...
export function smallOptions(model: Provider.Model) {
  ...
  if (
    ...
    model.api.npm === "@cypher/cypher-gateway" // cypher_change
  ) {
    if (!model.capabilities.reasoning) return {} // cypher_change - omit unsupported reasoning options
    return { reasoning: { enabled: true } } // cypher_change - use the model's supported default effort
  }
...
export function providerOptions(model: Provider.Model, options: { [x: string]: any }) {
  ...
  // cypher_change start
  if (model.api.npm === "@cypher/cypher-gateway") {
    return cypherProviderOptions(options)
  }
  // cypher_change end
