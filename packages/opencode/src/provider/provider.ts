...
import * as ModelsDev from "./models" // cypher_change - assemble dynamic Cypher models around upstream core catalog
...
// cypher_change start
import {
  CYPHER_BUNDLED_PROVIDERS,
  cypherCustomLoaders,
  CYPHER_MODEL_SCHEMA_EXTENSIONS,
  patchModelsDevModel as patchCypherModel,
  patchConfigModel as patchCypherConfigModel,
  patchCustomLoaderResult,
  patchCypherProviderPrivacy,
  cypherSmallModelPriority,
  buildTimeoutSignal,
} from "@/cypher/provider/provider"
import * as ModelsRefresh from "@/cypher/provider/models-refresh"
// cypher_change end
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  ...CYPHER_BUNDLED_PROVIDERS, // cypher_change
}
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    azure: Effect.fnUntraced(function* (provider: Info) {
      ...
      // cypher_change start - prefer explicit Azure endpoint over resource name to avoid conflicting SDK options
      const endpoint = iife(() => {
        return [
      ...
      if (!resource && !endpoint) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              "Azure resource name or endpoint is missing. Set AZURE_RESOURCE_NAME, AZURE_OPENAI_RESOURCE_NAME, AZURE_OPENAI_ENDPOINT, or reconnect the azure provider.", // cypher_change
            )
          },
        }
      }
      ...
      return {
        ...
        options: {
          ...(endpoint ? { baseURL: endpoint } : { resourceName: resource }), // cypher_change
        },
    ...
    llmgateway: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://cypher.ai/", // cypher_change
            "X-Title": "Cypher CLI", // cypher_change
            "X-Source": "cypher", // cypher_change
          },
        },
      }),
    openrouter: () =>
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://cypher.ai/", // cypher_change
            "X-Title": "Cypher CLI", // cypher_change
            "X-BILLING-INVOKE-ORIGIN": "Cypher", // cypher_change
          },
        },
      }),
    vercel: () =>
    ...
    gitlab: Effect.fnUntraced(function* (input: Info) {
      ...
      const aiGatewayHeaders = {
        "User-Agent": `cypher/${InstallationVersion} gitlab-ai-provider/${GITLAB_PROVIDER_VERSION} (${os.platform()} ${os.release()}; ${os.arch()})`, // cypher_change
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `cypher auth cloudflare-ai-gateway`.", // cypher_change
        )
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "Cypher CLI", // cypher_change
          },
        },
      }),
    cypher: () =>
...
export const Model = Schema.Struct({
  ...
  ...CYPHER_MODEL_SCHEMA_EXTENSIONS, // cypher_change
...
export const Info = Schema.Struct({
  id: ProviderID,
  name: Schema.String,
  description: optionalOmitUndefined(Schema.String), // cypher_change
  source: Schema.Literals(["env", "config", "custom", "api"]),
  env: Schema.Array(Schema.String),
  key: optionalOmitUndefined(Schema.String), // cypher_change
  metadata: optionalOmitUndefined(ProviderMetadata), // cypher_change
  options: Schema.Record(Schema.String, Schema.Any),
...
export const ListResult = Schema.Struct({
  ...
  failed: Schema.Array(Schema.String), // cypher_change
})
...
export class ModelNotFoundError extends Schema.TaggedErrorClass<ModelNotFoundError>()("ProviderModelNotFoundError", {
  ...
  modelsEmpty: Schema.optional(Schema.Boolean), // cypher_change
...
function fromModelsDevModel(provider: ModelsDev.Provider, model: ModelsDev.Model): Model {
  ...
  Object.assign(base, patchCypherModel(provider.id, model)) // cypher_change
...
export function fromModelsDevProvider(provider: ModelsDev.Provider): Info {
  ...
  return {
    ...
    source: "custom",
    name: provider.name,
    description: provider.description, // cypher_change
    env: [...(provider.env ?? [])],
    options: {},
    models,
  }
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(() =>
      Effect.gen(function* () {
        ...
        for (const [providerID, provider] of configProviders) {
          if (!provider) continue // cypher_change - null entries are transient delete sentinels
          ...
          for (const [modelID, model] of Object.entries(provider.models ?? {})) {
            if (!model) continue // cypher_change - null entries are transient delete sentinels
            ...
            const parsedModel: Model = {
              ...
              // variants: {}, // cypher_change, moved into patchCypherConfigModel
              ...patchCypherConfigModel(model, existingModel), // cypher_change
            }
            const merged = mergeDeep(ProviderTransform.variants(parsedModel), model.variants ?? {})
            parsedModel.variants = mapValues(
              pickBy(merged, (v): v is NonNullable<typeof v> => !!v && !v.disabled), // cypher_change - drop null delete sentinels
              (v) => omit(v, ["disabled"]),
            )
        ...
        // cypher_change start - load auths before env so OAuth plugins can override inherited credentials
        ...
        for (const [id, provider] of Object.entries(database)) {
          ...
          if (disabled.has(providerID)) continue
          // cypher_change start - prefer explicit OAuth auth over inherited env credentials
          if (
        ...
        // cypher_change start - resolve env once for patchCustomLoaderResult (azure env fallback)
        const cypherEnv = yield* env.all()
        // cypher_change end
        for (const [id, fn] of Object.entries({ ...custom(dep), ...cypherCustomLoaders(dep) })) {
          // cypher_change
          const providerID = ProviderID.make(id)
          if (disabled.has(providerID)) continue
          ...
          if (result) patchCustomLoaderResult(id, result, cypherEnv) // cypher_change
        ...
        for (const [id, provider] of configProviders) {
          if (!provider) continue // cypher_change - null entries are transient delete sentinels
          const providerID = ProviderID.make(id)
          // cypher_change start - keep OAuth plugin source when config and Codex auth coexist
          const oauth =
        ...
        patchCypherProviderPrivacy(providers[ProviderID.make("cypher")], cfg) // cypher_change
    ...
    yield* ModelsRefresh.watch(state) // cypher_change
    ...
    async function resolveSDK(model: Model, s: State, envs: Record<string, string | undefined>) {
      try {
        ...
        options["fetch"] = async (input: any, init?: BunFetchRequestInit) => {
          ...
          const timeout = buildTimeoutSignal(options) // cypher_change - use cancellable timeout for connection phase
          ...
          if (timeout.signal) signals.push(timeout.signal) // cypher_change
          ...
          // cypher_change start - clear connection-phase timeout once headers arrive
          try {
    ...
    const getModel = Effect.fn("Provider.getModel")(function* (providerID: ProviderID, modelID: ModelID) {
      ...
      if (!provider) {
        ...
        const suggestions = catalogProvider
          ...
          : fuzzysort
              ...
              .map((m) => m.target)
        const empty = false // cypher_change
        return yield* new ModelNotFoundError({ providerID, modelID, suggestions, modelsEmpty: empty }) // cypher_change
      }
      ...
      if (!info) {
        ...
        const empty = Object.keys(provider.models).length === 0 // cypher_change
        return yield* new ModelNotFoundError({ providerID, modelID, suggestions, modelsEmpty: empty }) // cypher_change
      }
      return info
    })
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderID) {
      ...
      // cypher_change start
      const cypherPriority = cypherSmallModelPriority(providerID)
      if (cypherPriority) priority = cypherPriority
      // cypher_change end
      ...
      // cypher_change start - fall back to cypher's auto small model
      const cypherFallback = s.providers[ProviderID.make("cypher")]
      if (cypherFallback?.models["cypher-auto/small"]) return cypherFallback.models["cypher-auto/small"]
      // cypher_change end
