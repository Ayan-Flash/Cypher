...
import { fetchCypherModels, type CypherModelsResult } from "@cypher/cypher-gateway"
...
type CypherOptions = NonNullable<Parameters<typeof fetchCypherModels>[0]>
type Options = { -readonly [K in keyof CypherOptions]?: CypherOptions[K] } & { apiKey?: string }
type Failure = NonNullable<CypherModelsResult["error"]>
type Result = { readonly models: Models; readonly error?: Failure }
type View = { models?: Models; timestamp?: number }

export interface CypherModels {
  readonly fetch: (options: CypherOptions) => Effect.Effect<CypherModelsResult, unknown>
}

export class CypherModelsService extends Context.Service<CypherModelsService, CypherModels>()(
  "@cypher/ModelCache/CypherModels",
) {}

export const cypherModelsLayer = Layer.succeed(
  CypherModelsService,
  CypherModelsService.of({ fetch: (options) => Effect.tryPromise(() => fetchCypherModels(options)) }),
)
type Cell = {
...
export class Service extends Context.Service<Service, Interface>()("@cypher/ModelCache") {}
...
export const layer: Layer.Layer<
  Service,
  never,
  Auth.Service | Config.Service | CypherModelsService | HttpClient.HttpClient
> = Layer.effect(
  Service,
  Effect.gen(function* () {
    const auth = yield* Auth.Service
    const cfg = yield* Config.Service
    const cypher = yield* CypherModelsService
    ...
    const authOptions = Effect.fn("ModelCache.authOptions")(function* (providerID: string) {
      if (providerID !== "cypher" && providerID !== "apertis") return {}
      const config = yield* cfg.get()
      const options: Options = {}

      if (providerID === "cypher") {
        const item = config.provider?.[providerID]
        if (item?.options?.apiKey) options.cypherToken = item.options.apiKey
        if (item?.options?.cypherOrganizationId) options.cypherOrganizationId = item.options.cypherOrganizationId

        const info = yield* auth.get(providerID)
        if (info?.type === "api") options.cypherToken = info.key
        if (info?.type === "oauth") {
          options.cypherToken = info.access
          if (info.accountId) options.cypherOrganizationId = info.accountId
        }

        const envToken = process.env.CYPHER_API_KEY ?? process.env.CYPHER_API_KEY
        const envOrg = process.env.CYPHER_ORG_ID ?? process.env.CYPHER_ORG_ID
        if (envToken) options.cypherToken = envToken
        if (envOrg) options.cypherOrganizationId = envOrg
        log.debug("auth options resolved", {
          providerID,
          hasToken: !!options.cypherToken,
          hasOrganizationId: !!options.cypherOrganizationId,
        })
      }
    ...
    const fetchModels = (providerID: string, options: Options): Effect.Effect<Result, unknown> => {
      if (providerID === "cypher") return cypher.fetch(options)
    ...
    const key = (providerID: string, options?: Options) => {
      if (providerID === "cypher") {
        return JSON.stringify([providerID, options?.baseURL, options?.cypherOrganizationId, options?.cypherToken])
      }
...
export const defaultLayer = layer.pipe(
  ...
  Layer.provide(cypherModelsLayer),
)
