...
import { ConfigValidation } from "../cypher/config-validation" // cypher_change
import * as EncodedIO from "../cypher/tool/encoded-io" // cypher_change
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("ApplyPatchTool.execute")(function* (
      ...
      const fileChanges: Array<{
        ...
        deletions: number
        bom: boolean
        encoding: string // cypher_change - preserved per-file encoding
      }> = []
        ...
        switch (hunk.type) {
          case "add": {
            ...
            fileChanges.push({
              ...
              deletions,
              bom: next.bom,
              encoding: "utf-8", // cypher_change - new files default to utf-8
            })
          ...
          case "update": {
            ...
            // cypher_change start - encoding-aware read so non-UTF-8 files decode without
            ...
            let bom = source.bom
            let encoding = read.encoding // cypher_change - overwritten by deriveNewContentsFromChunks below
          ...
          case "delete": {
            // cypher_change start - encoding-aware read so non-UTF-8 files decode without corruption
            ...
            fileChanges.push({
              ...
              deletions,
              bom: source.bom,
              encoding: deleteRead.encoding, // cypher_change
            })
      ...
      for (const change of fileChanges) {
        ...
        switch (change.type) {
          case "add":
            ...
            yield* EncodedIO.write(afs, change.filePath, Bom.join(change.newContent, change.bom), change.encoding) // cypher_change - encoding-aware write (mkdirs) replaces afs.writeWithDirs
            updates.push({ file: change.filePath, event: "add" })
            break

          case "update":
            yield* EncodedIO.write(afs, change.filePath, Bom.join(change.newContent, change.bom), change.encoding) // cypher_change - encoding-aware write replaces afs.writeWithDirs
          ...
          case "move":
            if (change.movePath) {
              // Create parent directories (recursive: true is safe on existing/root dirs)
              yield* EncodedIO.write(afs, change.movePath, Bom.join(change.newContent, change.bom), change.encoding) // cypher_change - encoding-aware write (mkdirs) replaces afs.writeWithDirs
      ...
      // cypher_change start - append Cypher config validation warnings
      ...
      return {
        ...
        metadata: {
          diff: totalDiff,
          files,
          diagnostics: filterDiagnostics(diagnostics, changedPaths), // cypher_change
        },
        output,
      }
    })
