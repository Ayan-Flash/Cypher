...
import { WarpGrepClient } from "@morphllm/morphsdk/tools/warp-grep/client" // cypher_change
import { Telemetry } from "@cypher/cypher-telemetry" // cypher_change
import { Instance } from "../cypher/instance" // cypher_change
import { Bus } from "../bus"
import { TuiEvent } from "../cli/cmd/tui/event"
import DESCRIPTION from "./warpgrep.txt"

// FREE_PERIOD_TODO: Remove CYPHER_WARPGREP_PROXY_URL constant and the proxy
// fallback below. After the free period ends, require MORPH_API_KEY and
// return an error when it is missing.
const CYPHER_WARPGREP_PROXY_URL = "https://api.cypher.ai/api/gateway"

const Parameters = Schema.Struct({
  query: Schema.String.annotate({
    description: "Search query describing what code you are looking for. Be specific and descriptive for best results.", // cypher_change
  }),
})
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          Telemetry.trackToolUsed("codebase_search", ctx.sessionID) // cypher_change
          ...
          const client = new WarpGrepClient({
            morphApiKey: apiKey ?? "cypher-free",
            ...(apiKey ? {} : { morphApiUrl: CYPHER_WARPGREP_PROXY_URL }),
            timeout: 60_000,
          })
          ...
          if (!result.success || !result.contexts?.length) {
            ...
            const isAuthOrRateLimit =
              result.error && /401|402|429|rate.limit|free.period|unauthorized/i.test(result.error) // cypher_change
            const apiKeyMsg =
