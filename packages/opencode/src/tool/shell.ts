...
import { normalizeUrls } from "@/cypher/util/url" // cypher_change
...
const ask = Effect.fn("ShellTool.ask")(function* (
  ...
  scan: Scan,
  command: string,
  description?: string, // cypher_change
) {
  // cypher_change
  if (scan.dirs.size > 0) {
    ...
    yield* ctx.ask({
      ...
      patterns: globs,
      always: globs,
      metadata: scan.access === "read" ? { command, access: "read", ...(description ? { description } : {}) } : {}, // cypher_change
    })
  }
  ...
  yield* ctx.ask({
    ...
    metadata: { command: normalizeUrls(command), ...(description ? { description } : {}) }, // cypher_change
  })
})

// cypher_change start - share bash permission scanning with Cypher interactive terminal
...
function cmd(shell: string, command: string, cwd: string, env: NodeJS.ProcessEnv) {
  if (process.platform === "win32" && Shell.ps(shell)) {
    // cypher_change start - PowerShell args
...
export const ShellTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const permission = yield* ShellPermission // cypher_change
    ...
    const run = Effect.fn("ShellTool.run")(function* (
      ...
      const code: number | null = yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const reader = yield* Effect.forkScoped( // cypher_change - keep the fiber so trailing output can be drained
          ...
          const timeout = Effect.sleep(`${CommandTimeout.duration(input.timeout)} millis`) // cypher_change
          ...
          // cypher_change start - closing the scope interrupts the reader fiber, which can drop
              ...
              return yield* run(
                {
                  ...
                  timeout,
                  description: params.description ?? params.command, // cypher_change
                },
                ctx,
              )
            }),
