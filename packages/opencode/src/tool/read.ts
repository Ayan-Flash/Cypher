...
import { Readable } from "stream" // cypher_change
...
// cypher_change start
import * as Encoding from "../cypher/encoding"
import * as Extract from "../cypher/tool/read-extract"
import * as TextStream from "../cypher/text-stream"
// cypher_change end

const DEFAULT_READ_LIMIT = 2000
const MAX_LINE_LENGTH = 2000
// cypher_change start - report the safe Unicode slice length
const suffix = (length: number) => `... (line truncated to ${length} chars)`
// cypher_change end
const MAX_BYTES = 50 * 1024
const MAX_BYTES_LABEL = `${MAX_BYTES / 1024} KB`
const SAMPLE_BYTES = 4096
const DIRECTORY_CONCURRENCY = 8 // cypher_change
...
export const ReadTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    // cypher_change start - extracted formats use native readers; ordinary text streams through AppFileSystem
    ...
    const isBinaryFile = (filepath: string, bytes: Uint8Array) => {
      ...
      // cypher_change start - UTF-16/32 BOM: NUL bytes are legitimate, skip the NUL/control-char heuristic
    ...
    const run = Effect.fn("ReadTool.execute")(function* (
      ...
      if (stat.type === "Directory") {
        ...
        const limit = Math.max(1, params.limit ?? DEFAULT_READ_LIMIT) // cypher_change - prevent zero-limit loops
      ...
      // cypher_change start - route extractable binary documents through lines()
...
// cypher_change start - extracted formats use native readers; ordinary text is supplied by AppFileSystem above
async function collect(stream: Readable, opts: { limit: number; offset: number }) {
  ...
  try {
    for await (const text of rl) {
      ...
      // cypher_change start - keep truncated output valid Unicode
