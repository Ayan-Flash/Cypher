...
import { Session } from "../session/session" // cypher_change
import { SessionID } from "../session/schema" // cypher_change
import { RecallSearch } from "../cypher/session/recall-search" // cypher_change
...
export const RecallTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    const sessions = yield* Session.Service // cypher_change
    return {
...
async function search(
  ...
  const dirs = await bridge.promise(WorktreeFamily.list().pipe(Effect.provideService(Git.Service, git))) // cypher_change
  const boundary = CypherSessionPromptQueue.active(ctx.sessionID) ?? RecallSearch.active(ctx.messages, ctx.messageID)
