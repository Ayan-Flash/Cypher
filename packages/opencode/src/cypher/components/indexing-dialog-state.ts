import { fetchCypherEmbeddingModelCatalog, resolveCypherGatewayBaseUrl } from "@cypher/cypher-gateway"
import type { Config, IndexingConfig, CypherEmbeddingModelCatalog } from "@cypher/sdk/v2"
...
export async function loadCypherEmbeddingModels(onError?: (message: string) => void) {
  const endpoint = new URL("embedding-models", resolveCypherGatewayBaseUrl()).toString()
  log.info("loading Cypher embedding model catalog", { endpoint })
  const catalog = await fetchCypherEmbeddingModelCatalog({
    onError: (issue) => {
      log.warn("failed to load Cypher embedding model catalog", {
        code: issue.code,
        status: issue.status,
  ...
  log.info("loaded Cypher embedding model catalog", {
...
export function cypherModelOptions(catalog?: CypherEmbeddingModelCatalog) {
...
export function currentCypherModel(catalog: CypherEmbeddingModelCatalog | undefined, model?: string | null) {
