/**
 * Cypher News Component
 *
 * Self-contained component that fetches and displays Cypher news/notifications.
...
import type { CyphercodeNotification } from "@cypher/cypher-gateway"
import { NotificationBanner } from "./notification-banner.js"
import { DialogCypherNotifications } from "./dialog-cypher-notifications.js"
import { News } from "./news.js"

export function CypherNews() {
  const sync = useSync()
  const sdk = useSDK()
  const dialog = useDialog()
  const kv = useKV()

  const [notifications, setNotifications] = createSignal<CyphercodeNotification[]>([])
  const [fetched, setFetched] = createSignal(false)
  const isCypherConnected = createMemo(() => sync.data.provider_next.connected.includes("cypher"))
  const unread = createMemo(() => News.unread(notifications(), kv.get(News.key, [])))

  const openNewsDialog = () => {
    const items = unread()
    if (items.length === 0) return
    dialog.replace(() => <DialogCypherNotifications notifications={items} />)
  ...
  createEffect(
    on(
      ...
      async (status) => {
        ...
        if (!isCypherConnected()) return

        const result = await sdk.client.cypher.notifications()
