...
import type { IndexingConfig, Config } from "@cypher/sdk/v2"
import * as Log from "@opencode-ai/core/util/log"
import { hasCypherIndexingAuth, resolveCypherIndexingAuth, shouldDefaultIndexingToCypher } from "../indexing-auth"
import {
...
function hasCypherAuth(sync: ReturnType<typeof useSync>, scope: IndexingScope, indexing: IndexingConfig): boolean {
  const provider = sync.data.provider_next.all.find((item) => item.id === "cypher")
  const config = indexingScopeConfig(scope, sync.data.config, sync.data.globalConfig, indexing)
  return hasCypherIndexingAuth({ config, provider })
}
...
function defaultIndexing(
  ...
  const provider = sync.data.provider_next.all.find((item) => item.id === "cypher")
  const config = indexingScopeConfig(scope, sync.data.config, sync.data.globalConfig, indexing)
  const auth = resolveCypherIndexingAuth({ config, provider })
  if (!shouldDefaultIndexingToCypher({ ...global, ...indexing }, auth)) return indexing
  return { ...indexing, provider: "cypher", model: null, dimension: null }
}
...
function ProviderSelect(props: SubDialogProps) {
  ...
  const options: DialogSelectOption<EmbeddingProvider>[] = (
    ...
    .filter(([value]) => value !== "cypher" || hasCypherAuth(sync, props.scope, indexing) || indexing.provider === "cypher")
    .map(([value, title]) => ({
      value,
      title,
...
function CypherModelSelect(props: SubDialogProps) {
  const dialog = useDialog()
  const sync = useSync()
  const sdk = props.useSDK()
  const toast = useToast()
  ...
  const [catalog] = createResource(() => loadCypherEmbeddingModels(setError))
  ...
  createEffect(() => {
    ...
    seen.state = state
    log.info("Cypher embedding model resource changed", {
      state: catalog.state,
      models: cfg?.models.length ?? 0,
      current: currentCypherModel(cfg, indexing.model),
      defaultModel: cfg?.defaultModel || undefined,
      scope: props.scope,
    })
  })
  const options = createMemo(() => cypherModelOptions(catalog()))
  const current = createMemo(() => currentCypherModel(catalog(), indexing.model))

  return (
    <DialogSelect
      title="Cypher Embedding Model"
      options={options()}
      current={current()}
      renderFilter={(catalog()?.models.length ?? 0) > 0}
      onSelect={async (option) => {
        if (!option.value || !catalog()?.models.some((model) => model.id === option.value)) return
        log.info("selected Cypher embedding model", { model: option.value, scope: props.scope })
...
function ScopeSelect(props: DialogIndexingProps & { scope: IndexingScope }) {
  ...
  const options: DialogSelectOption<IndexingScope>[] = [
    ...
    { value: "project", title: "Project", description: "Stored in this repo's .cypher config" },
  ]
  ...
  const options = createMemo<DialogSelectOption<string>[]>(() => {
    ...
    const result: DialogSelectOption<string>[] = [
      ...
      },
      {
        ...
        category: "Embedding",
        description: mark(
          indexing.provider === "cypher" ? (indexing.model ?? "Cypher catalog") : (indexing.model ?? "default"),
          [["model"]],
        ),
      },
      {
      ...
      onSelect={async (option) => {
        ...
        switch (option.value) {
          ...
          case "providerSettings":
            ...
            break
          case "model": {
            if (indexing.provider === "cypher") {
              dialog.replace(() => (
                <CypherModelSelect useSDK={props.useSDK} scope={scope()} indexing={indexing} raw={raw} />
              ))
              break
            }
          ...
          case "dimension": {
            if (indexing.provider === "cypher") break
