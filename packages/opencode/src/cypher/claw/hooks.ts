...
/**
 * CypherClaw SolidJS reactive helpers — Cypher Chat protocol.
 *
 * Exposes:
 *   - `createClawStatus`: polled instance status
 *   - `createClawChat`: Cypher Chat connection + active-conversation state
 *
...
/**
 * Poll the CypherClaw instance status every `interval` ms.
 */
export function createClawStatus(sdk: SDK, interval = 10_000) {
  ...
  onMount(() => {
    const poll = async () => {
      const res = await sdk.client.cypher.claw.status().catch(() => null)
...
export function createClawChat(sdk: SDK) {
  ...
  onMount(async () => {
    ...
    const statusRes = await sdk.client.cypher.claw.status().catch(() => null)
    ...
    const credsRes = await sdk.client.cypher.claw.chatCredentials().catch((e: unknown) => {
    ...
    if (!envelope.cypherChatUrl) missing.push("cypherChatUrl")
    ...
    try {
      log.info("connecting cypher-chat")
      chat = await connect({
        envelope,
