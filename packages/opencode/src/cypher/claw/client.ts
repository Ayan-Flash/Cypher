...
/**
 * CypherClaw Cypher Chat client wrapper for the TUI.
 *
 * Hosts the HTTP and WebSocket clients, exposes a multi-conversation API
 * (list/create/select/rename/delete), and translates Cypher Chat events for
...
import { CypherChatClient } from "./cypher-chat-client"
...
export async function connect(input: ConnectInput): Promise<ClawChatClient> {
  ...
  const chat = new CypherChatClient({
    baseUrl: input.envelope.cypherChatUrl,
    getToken: async () => input.envelope.token,
  })
  ...
  const sandboxCtx = `/cypherclaw/${input.sandboxId}`
  ...
  function setActive(id: string | null): void {
    ...
    clearTyping()
    if (id) {
      activeCtx = `/cypherclaw/${input.sandboxId}/${id}`
