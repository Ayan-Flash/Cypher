/**
 * CypherClaw full-screen view
 *
 * Main layout component for the /cypherclaw route.
...
export function CypherClawView() {
  ...
  // Bot display name — sourced from the CypherClaw platform status (set by the
  // user during onboarding via patchBotIdentity). Falls back to the literal
  // "CypherClaw" while loading or for instances that skipped onboarding,
  // matching the web UI's fallback chain.
  const botName = createMemo(() => status()?.botName ?? "CypherClaw")

  // CypherClaw view commands — single source of truth for both the global
  // command palette / keybinds and the in-chat slash autocomplete.
  // The list is reactive on `chat.connected` so the slash menu only
  // exposes `/new` and `/conversations` once we're connected to cypher-chat.
  const cypherCommands = createMemo(() => {
    const ready = chat.connected()
    return [
      {
        name: "cypherclaw.back",
        title: "Back",
      ...
      },
      {
        ...
        desc: "Start a new CypherClaw conversation",
        category: "CypherClaw",
        namespace: "palette",
        slashName: "new",
        slashAliases: [] as string[],
      ...
      {
        name: "cypherclaw.conversations",
        title: "Conversations",
        desc: "Browse, rename, and delete CypherClaw conversations",
        category: "CypherClaw",
        namespace: "palette",
  ...
  useBindings(() => ({
    commands: cypherCommands(),
    bindings: [{ key: "escape", cmd: "cypherclaw.back" }],
  }))
  ...
  const clawSlashes = createMemo<ClawSlashOption[]>(() => {
    const visible = cypherCommands().filter((c) => c.enabled !== false && !c.hidden && c.slashName)
