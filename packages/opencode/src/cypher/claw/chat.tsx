/**
 * CypherClaw chat panel
 *
 * Renders a scrollable message list, a slash-command autocomplete popup,
 * and a textarea for chatting with the CypherClaw bot via Cypher Chat.
...
export function ClawChat(props: {
  ...
  // `bot:cypherclaw:{sandboxId}` — the bot is the only non-self member in 1:1
  // chats, so we resolve bot ids to the user-configured `botName` (which
  // falls back to "CypherClaw" upstream) and render any human collaborators
