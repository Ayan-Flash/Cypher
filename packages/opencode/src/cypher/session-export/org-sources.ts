...
import { resolveCypherIndexingAuth } from "@/cypher/indexing-auth"
...
export async function getAuthOrgId(): Promise<OrgState> {
  try {
    const [cfg, info] = await Promise.all([
      ...
      auth.runPromise((svc) => svc.get("cypher")),
    ])
    const id = resolveCypherIndexingAuth({ config: cfg, auth: info }).organizationId
