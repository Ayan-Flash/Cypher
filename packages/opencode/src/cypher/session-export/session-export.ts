...
declare global {
  const CYPHER_SESSION_EXPORT_WORKER_PATH: string
}
...
function target(): WorkerTarget {
  if (typeof CYPHER_SESSION_EXPORT_WORKER_PATH !== "undefined") return CYPHER_SESSION_EXPORT_WORKER_PATH
...
function currentSurface(): string {
  return process.env.CYPHER_FEATURE?.trim() || "unknown"
}
