...
async function headers(args: HeaderArgs): Promise<Headers> {
  ...
  const out = new Headers({
    ...
    "x-cypher-export-api-version": "1",
    "x-cypher-export-schema-version": "1",
    "x-cypher-export-agent-version": args.agentVersion,
    "x-cypher-export-surface": args.surface,
    "x-cypher-export-root-session-id": first.rootSessionId,
    "x-cypher-export-session-id": first.sessionId,
    "x-cypher-export-batch-id": args.batchId,
    "x-cypher-export-seq-start": String(Math.min(...seqs)),
    "x-cypher-export-seq-end": String(Math.max(...seqs)),
    "x-cypher-export-event-count": String(args.rows.length),
    "x-cypher-export-payload-sha256": await sha256Hex(args.body),
    "x-cypher-export-client-sent-at": new Date().toISOString(),
    "x-cypher-export-content-encoding": "identity",
  })
  const token = process.env.CYPHER_SESSION_EXPORT_AUTH_TOKEN
  if (token) out.set("authorization", `Bearer ${token}`)
  const anon = args.anonId ?? (await anonId(args.anonIdPath))
  if (!token && anon) out.set("x-cypher-anon-id", anon)
  return out
}
