...
//          persist `"snapshot": false` to `.cypher/cypher.json`, and skip. All
...
//     Instead we write the file directly via `CyphercodeConfig.updateProjectConfig`
...
//
// All of this is Cypher-specific — the upstream snapshot module remains a thin
...
import { CypherPartLifecycle } from "@/cypher/session/part-lifecycle"
...
export namespace CypherSnapshotTrack {
  ...
  export const TIMEOUT_MS = duration("CYPHER_SNAPSHOT_TRACK_TIMEOUT_MS", 10_000)
  export const TURN_TIMEOUT_MS = duration("CYPHER_SNAPSHOT_TURN_TIMEOUT_MS", 120_000)
  ...
  export const progressPart = (input: {
    ...
    text: input.text,
    synthetic: true,
    metadata: { [CypherPartLifecycle.key]: "transient" },
  })
  ...
  async function currentDirectory(): Promise<{ directory: string; worktree?: string } | undefined> {
    const { Instance } = await import("@/cypher/instance")
    try {
