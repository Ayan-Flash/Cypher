...
export namespace IgnoreMigrator {
  const log = Log.create({ service: "cypher.ignore-migrator" })

  const CYPHERIGNORE_FILE = ".cypherignore"
  const GLOBAL_CYPHERIGNORE = path.join(os.homedir(), ".cypher", CYPHERIGNORE_FILE)
  ...
  /**
   * Parse .cypherignore content into patterns.
  ...
  /**
   * Load patterns from a .cypherignore file
   */
  ...
  /**
   * Migrate .cypherignore to Opencode permission config
   */
  export async function migrate(options: { projectDir: string; skipGlobalPaths?: boolean }): Promise<MigrationResult> {
    ...
    // 1. Load global .cypherignore (lower priority)
    if (!options.skipGlobalPaths) {
      const globalPatterns = await loadIgnoreFile(GLOBAL_CYPHERIGNORE, "global")
      allPatterns.push(...globalPatterns)

      if (globalPatterns.length > 0) {
        log.debug("loaded global .cypherignore", { count: globalPatterns.length })
      }
    }

    // 2. Load project .cypherignore (higher priority - added last)
    const projectIgnorePath = path.join(options.projectDir, CYPHERIGNORE_FILE)
    const projectPatterns = await loadIgnoreFile(projectIgnorePath, "project")
    allPatterns.push(...projectPatterns)

    if (projectPatterns.length > 0) {
      log.debug("loaded project .cypherignore", { count: projectPatterns.length })
    }
  ...
  /**
   * Load .cypherignore and return permission config.
  ...
  export async function loadIgnoreConfig(
    ...
    try {
      ...
      if (result.patternCount > 0) {
        log.info("loaded .cypherignore patterns", {
      ...
      log.warn("failed to load .cypherignore", { error: err })
      return {}
    }
  }
}
