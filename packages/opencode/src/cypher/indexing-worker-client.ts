...
} from "@cypher/cypher-indexing/engine"
import type { IndexingStatus } from "@cypher/cypher-indexing/status"
...
declare global {
  const CYPHER_INDEXING_WORKER_PATH: string
}
...
export namespace IndexingWorker {
  ...
  const channel = () => {
    ...
    const file =
      typeof CYPHER_INDEXING_WORKER_PATH !== "undefined"
        ? CYPHER_INDEXING_WORKER_PATH
        ...
        return call(
          state,
          {
            ...
            input: {
              ...
              config,
              baselineDirectory,
              lancedbPath: process.env.CYPHER_LANCEDB_PATH,
            },
          },
          (message) => {
