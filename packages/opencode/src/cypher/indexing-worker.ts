import type { CodeIndexManager } from "@cypher/cypher-indexing/engine"
...
async function init(request: Extract<Request, { method: "init" }>) {
  ...
  if (request.input.lancedbPath) process.env.CYPHER_LANCEDB_PATH = request.input.lancedbPath
  const [engine, status] = await Promise.all([
    import("@cypher/cypher-indexing/engine"),
    import("@cypher/cypher-indexing/status"),
  ])
