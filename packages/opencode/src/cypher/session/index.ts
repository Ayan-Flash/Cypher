...
import { ENV_FEATURE } from "@cypher/cypher-gateway"
import { fn } from "@/cypher/fn"
import { existsSync } from "fs"
import path from "path"

export namespace CypherSession {
  const log = Log.create({ service: "session.cypher" })
  ...
  export function attribution(id: string): { rootID: string; feature?: string } {
    ...
    const platform = resolvePlatform(rootID) ?? process.env["CYPHER_PLATFORM"]
  ...
  // ---------------------------------------------------------------------------
  // Provider-reported cost (Cypher / OpenRouter / Vercel AI Gateway)
  // ---------------------------------------------------------------------------
   ...
   *                                      (`costDetails.upstreamInferenceCost` for Cypher)
   ...
   *
   * Cypher does not charge end users a per-request fee, so for the Cypher provider the
  ...
  export function providerCost(input: {
    ...
    const isCypher = (input.provider?.id ?? input.providerID) === "cypher"
    ...
    if (orUsage) {
      ...
      // Cypher doesn't charge a fee on top of the upstream inference cost, so for Cypher
      ...
      const cost = isCypher && upstream !== undefined ? upstream : regular
    ...
    //    upstream inference cost remains available with snake_case preserved. Cypher doesn't
    ...
    //    gateway fee that Cypher would pass through, but Cypher doesn't charge end users a
  ...
  export function shareSession(id: SessionID) {
    return EffectBridge.fromPromise(async () => {
      const { CypherSessions } = await import("@/cypher-sessions/cypher-sessions")
      return CypherSessions.share(id)
  ...
  export function unshareSession(id: SessionID) {
    return EffectBridge.fromPromise(async () => {
      const { CypherSessions } = await import("@/cypher-sessions/cypher-sessions")
      await CypherSessions.unshare(id)
    }).pipe(Effect.catchCause((cause) => Effect.fail(Cause.squash(cause))))
  }

  export async function removeSession(id: string): Promise<void> {
    const { CypherSessions } = await import("@/cypher-sessions/cypher-sessions")
    await CypherSessions.remove(id).catch(() => {})
  }
  ...
  export function* listGlobal<T extends { time: { updated: number }; project?: ProjectInfo | null }>(input: {
    ...
    const worktree = (dir: string) => {
      for (const root of sorted) {
        ...
        if ((parts[0] === ".cypher" || parts[0] === ".cypher") && parts[1] === "worktrees" && parts[2]) {
...
export const cypherSessionFork = fn(
