...
export namespace CypherRoutedModel {
  const ns = "cypher"
  ...
  export function readAuto(
    ...
    if (input.providerID !== ProviderID.cypher) return undefined
    if (!input.modelID.startsWith("cypher-auto/") && !input.modelID.includes("fable")) return undefined
