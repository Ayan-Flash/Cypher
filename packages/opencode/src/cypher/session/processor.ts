...
import { Telemetry, type ReviewCommand } from "@cypher/cypher-telemetry"
import { SessionNetwork } from "@/session/network"
import type { SessionID } from "@/session/schema"
import type { SessionStatus } from "@/session/status"
import { MessageV2 } from "@/session/message-v2"
import { isRecord } from "@/util/record"
import { parseReviewCommand, reviewCommandName } from "@/cypher/review/command"
...
export namespace CypherSessionProcessor {
  const log = Log.create({ service: "session.processor.cypher" })
  ...
  export const PROVIDER_FINISH_ERROR_MESSAGE =
    "The provider ended the response with an error before returning details. Start a new message to retry; Cypher will compact the oversized conversation first if needed."
  ...
  /**
   * Returns the Cypher-specific retry policy options (limit + offline handler).
  ...
  export function retryOpts(input: {
    ...
    return {
      limit: Flag.CYPHER_SESSION_RETRY_LIMIT,
