...
import { PlanFollowup } from "@/cypher/plan-followup"
import { PlanFile } from "@/cypher/plan-file"
import { CypherSession } from "@/cypher/session"
import { CypherSessionMessageOrder } from "@/cypher/session/message-order"
import { Permission } from "@/permission"
import { Question } from "@/question"
import { environmentDetails, type EditorContext } from "@/cypher/editor-context"
import { Identifier } from "@/id/id"
import { Filesystem } from "@/util/filesystem"
import { InstanceState } from "@/effect/instance-state"
import NATIVE_PLAN_PROMPT from "@/cypher/session/native-plan-prompt.txt"
import { MemoryPaths } from "@cypher/cypher-memory/effect/paths"
import { MemoryMarker } from "@/cypher/memory/marker"
import { CyphercodeSystemPrompt } from "@/cypher/system-prompt"
import { CypherToolRegistry } from "@/cypher/tool/registry"
import CODE_SWITCH from "@/session/prompt/code-switch.txt"

export namespace CypherSessionPrompt {
  ...
  function supportsPlanFollowup() {
    return ["cli", "vscode", "jetbrains"].includes(Flag.CYPHER_CLIENT)
  }
  ...
  export const recoverDanglingAssistant = Effect.fn("CypherSessionPrompt.recoverDanglingAssistant")(function* (input: {
    sessionID: SessionID
  ...
  export const recoverProviderFinishError = Effect.fn("CypherSessionPrompt.recoverProviderFinishError")(
    function* (input: {
      sessionID: SessionID
      status: Pick<SessionStatus.Interface, "get">
      sessions: Pick<Session.Interface, "messages" | "removeMessage">
  ...
  export const askPermission = Effect.fn("CypherSessionPrompt.askPermission")(function* (input: {
  ...
  export function memoryToolEnabled(input: { ctx: MemoryPaths.Ctx }) {
    return CypherToolRegistry.memoryToolsEnabled({ ctx: input.ctx })
  }
  ...
  export const memoryInject = Effect.fn("CypherSessionPrompt.memoryInject")(function* (input: {
    ctx: MemoryPaths.Ctx
    sessionID: SessionID
    record: boolean
    ...
    const built =
      cached?.enabled === enabled
        ? cached
        : yield* CyphercodeSystemPrompt.memoryBlocks({
            ctx: input.ctx,
            sessionID: input.sessionID,
            record: input.record,
            enabled,
   ...
   * OneDrive ReparsePoint directories in some Node versions (cypher#9755).
   */
  ...
  export function resolveCloseReason(input: {
    sessionID: string
    closeReasons: Map<string, CypherSession.CloseReason>
    exit: Exit.Exit<any, any>
  }): CypherSession.CloseReason {
  ...
  export function trimBeforeLastSummary(msgs: MessageV2.WithParts[]): MessageV2.WithParts[] {
    const summary = msgs.reduce<{ msg: MessageV2.WithParts; index: number } | undefined>((latest, msg, index) => {
      ...
      if (!latest || CypherSessionMessageOrder.compare(msg, latest.msg, index, latest.index) > 0) return { msg, index }
      return latest
    }, undefined)
