...
import { appendTerminalOutput } from "@/cypher/interactive-terminal/output"
...
export namespace InteractiveTerminal {
  ...
  class StateService extends Context.Service<StateService, { readonly get: () => Effect.Effect<State> }>()(
    "@cypher/InteractiveTerminal.State",
  ) {}
  ...
  function gate(shell: string, command: string) {
    ...
    return `stty -echo; IFS= read -r __cypher_gate; stty echo; ${command}`
  }
  ...
  function environment(input: NodeJS.ProcessEnv) {
    ...
    env.CYPHER_TERMINAL = "1"
    env.CYPHER_INTERACTIVE_TERMINAL = "1"
