...
import {
  ...
  clearModesCache,
  fetchBalance,
  fetchCyphercodeNotifications,
  fetchCypherPassState,
  fetchOrganizationModes,
  fetchProfile,
} from "@cypher/cypher-gateway"
import { DIRECT_FIM_ENV, requestMistralFim, resolveFimTarget } from "@cypher/cypher-gateway/fim"
import { DIRECT_EDIT_ENV, extractFencedBody, resolveEditTarget } from "@cypher/cypher-gateway/edit"
import { buildMercuryEditPrompt } from "@cypher/cypher-gateway/edit-prompt"
import { buildCypherHeaders } from "@cypher/cypher-gateway"
...
import { AudioTranscriptionsBody, ClawStatus, EditBody, FimBody } from "../groups/cypher-gateway"
...
const log = Log.create({ service: "cypher-gateway" })
...
export const cypherGatewayHandlers = HttpApiBuilder.group(InstanceHttpApi, "cypher", (handlers) =>
  Effect.gen(function* () {
    const auth = yield* Auth.Service
    const store = yield* InstanceStore.Service
    const cache = yield* ModelCache.Service

    const profile = Effect.fn("CypherGatewayHttpApi.profile")(function* () {
      const info = yield* auth.get("cypher").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
      if (!info || info.type !== "oauth") return yield* Effect.fail(new HttpApiError.Unauthorized({}))

      const currentOrgId = info.accountId ?? null
      const [profile, balance, cypherPass] = yield* Effect.tryPromise({
        try: () =>
          Promise.all([
            fetchProfile(info.access),
            fetchBalance(info.access, currentOrgId ?? undefined),
            fetchCypherPassState(info.access),
          ]),
        catch: () => new HttpApiError.BadRequest({}),
      })
      return { profile, balance, cypherPass, currentOrgId }
    })

    const authStatus = Effect.fn("CypherGatewayHttpApi.authStatus")(function* () {
      const info = yield* auth.get("cypher").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
    ...
    const proxyAuth = Effect.fn("CypherGatewayHttpApi.proxyAuth")(function* () {
      const info = yield* auth.get("cypher").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      return {
        auth: info,
        token: getToken(info),
    ...
    const modes = Effect.fn("CypherGatewayHttpApi.modes")(function* () {
    ...
    const fim = Effect.fn("CypherGatewayHttpApi.fim")(function* (ctx: { payload: typeof FimBody.Type }) {
      const target = resolveFimTarget(ctx.payload.provider, ctx.payload.model)
      const info = target.provider === "cypher" ? yield* proxyAuth() : undefined
      const token = yield* Effect.gen(function* () {
        if (target.provider === "cypher") return info?.token
      ...
      if (target.provider === "cypher" && !info?.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
          ...
          const run = async (url: string): Promise<Response> => {
            ...
            return fetch(url, {
              ...
              headers: {
                ...
                ...(target.provider === "cypher"
                  ? buildCypherHeaders(undefined, { cypherOrganizationId: info?.organizationId })
                  : {}),
                ...(target.provider === "cypher" ? { [HEADER_FEATURE]: "autocomplete" } : {}),
              },
              signal,
              body: JSON.stringify({
                model: target.model,
    ...
    const edit = Effect.fn("CypherGatewayHttpApi.edit")(function* (ctx: { payload: typeof EditBody.Type }) {
      const target = resolveEditTarget(ctx.payload.provider, ctx.payload.model)
      if (target.provider === "cypher" && !target.url) {
        return yield* Effect.fail(new HttpApiError.BadRequest({}))
      }
      const proxy = target.provider === "cypher" ? yield* proxyAuth() : undefined
      const token = yield* Effect.gen(function* () {
        if (target.provider === "cypher") return proxy?.token
        const item = yield* auth.get(target.provider).pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
        if (item?.type === "api") return item.key
        return DIRECT_EDIT_ENV[target.provider].map((key) => process.env[key]).find(Boolean)
      })
      if (target.provider === "cypher" && !proxy?.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const audioTranscriptions = Effect.fn("CypherGatewayHttpApi.audioTranscriptions")(function* (ctx: {
      ...
      const response = yield* Effect.tryPromise({
        try: () =>
          fetch(`${CYPHER_API_BASE}/api/gateway/v1/audio/transcriptions`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
    ...
    const notifications = Effect.fn("CypherGatewayHttpApi.notifications")(function* () {
      // Locally-detected notice about leftover opencode config; appended so it reuses each client's dismissal path.
      const notice = CyphercodeConfig.opencodeConfigNotification({
        directory: Instance.directory,
        worktree: Instance.worktree,
        scanProject: !Flag.CYPHER_DISABLE_PROJECT_CONFIG,
      })
      ...
      const cloud = yield* Effect.promise(() =>
        fetchCyphercodeNotifications({
          cypherToken: token,
          cypherOrganizationId: getOrganizationId(info),
        }),
      )
      return append(cloud)
    })

    const organization = Effect.fn("CypherGatewayHttpApi.organization")(function* (ctx) {
      const info = yield* auth.get("cypher").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      if (!info || info.type !== "oauth") return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      yield* cache.clear("cypher")
      clearModesCache()
      yield* store.disposeAll().pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      return true
    })

    const clawStatus = Effect.fn("CypherGatewayHttpApi.clawStatus")(function* () {
      const info = yield* auth.get("cypher").pipe(Effect.mapError(() => new HttpApiError.ServiceUnavailable({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      return yield* Effect.tryPromise({
        try: async () => {
          const response = await fetch(`${CYPHER_API_BASE}/api/cypherclaw/status`, { headers })
        ...
        Effect.match({
          onFailure: (err) => {
            if (err instanceof GatewayError)
              return jsonError(`CypherClaw request failed: ${err.status} ${err.message}`, err.status)
            logError("claw/status", err)
            return jsonError("Failed to reach CypherClaw", 502)
          },
    ...
    const clawChatCredentials = Effect.fn("CypherGatewayHttpApi.clawChatCredentials")(function* () {
      const info = yield* auth.get("cypher").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      return {
        ...
        cypherChatUrl: CYPHER_CHAT_URL,
        eventServiceUrl: CYPHER_EVENT_SERVICE_URL,
      }
    })

    const cloudSessions = Effect.fn("CypherGatewayHttpApi.cloudSessions")(function* (ctx) {
      const info = yield* auth.get("cypher").pipe(Effect.mapError(() => new HttpApiError.BadRequest({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSession = Effect.fn("CypherGatewayHttpApi.cloudSession")(function* (ctx) {
      const info = yield* auth.get("cypher").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const cloudSessionImport = Effect.fn("CypherGatewayHttpApi.cloudSessionImport")(function* (ctx) {
      const info = yield* auth.get("cypher").pipe(Effect.mapError(() => new HttpApiError.Unauthorized({})))
      const token = getToken(info)
      if (!token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
    ...
    const imageModels = Effect.fn("CypherGatewayHttpApi.imageModels")(function* () {
      const info = yield* proxyAuth()
      if (!info.auth) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      if (!info.token) return yield* Effect.fail(new HttpApiError.Unauthorized({}))
      ...
      const result = yield* Effect.tryPromise({
        try: () =>
          fetchCypherImageModels({
            cypherToken: info.token,
            cypherOrganizationId: info.organizationId,
          }),
        catch: () => new HttpApiError.BadRequest({}),
      })
