...
import { CyphercodeConfigOverlay } from "@/cypher/config/overlay"
import { CyphercodeConfigSources } from "@/cypher/config/sources"
import { CyphercodeModelState } from "@/cypher/config/model-state"
import { ConfigRules } from "@/cypher/server/routes/config-rules"
import { CyphercodeKeybinds } from "@/cypher/tui/keybinds"
import { CyphercodeTuiConfig } from "@/cypher/tui/config"
  ...
  Effect.gen(function* () {
    ...
    const overlay = Effect.fn("ConfigConsoleHttpApi.overlay")(function* (ctx: {
      ...
      return yield* Effect.promise(() =>
        CyphercodeConfigOverlay.resolve({
          directory: instance.directory,
          worktree: instance.worktree,
    ...
    const overlayUpdate = Effect.fn("ConfigConsoleHttpApi.overlayUpdate")(function* (ctx: {
      ...
      const patch = CyphercodeConfigOverlay.patch(body)
    ...
    const modelState = Effect.fn("ConfigConsoleHttpApi.modelState")(function* () {
      return yield* Effect.promise(() => CyphercodeModelState.get())
    })
    ...
    const modelStateUpdate = Effect.fn("ConfigConsoleHttpApi.modelStateUpdate")(function* (ctx: {
      ...
      return yield* Effect.promise(() =>
        CyphercodeModelState.update({ favorite: ctx.payload.favorite?.map((item) => ({ ...item })) }),
      )
    })

    const tuiConfigGet = Effect.fn("ConfigConsoleHttpApi.tuiConfigGet")(function* () {
      const instance = yield* InstanceState.context
      return yield* Effect.promise(() => CyphercodeTuiConfig.get({ directory: instance.directory }))
    })

    const tuiKeybindList = Effect.fn("ConfigConsoleHttpApi.tuiKeybindList")(function* () {
      return { keybinds: CyphercodeKeybinds.list() }
    })
