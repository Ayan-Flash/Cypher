...
export function matchLegacyCypherOpenApi(input: Record<string, unknown>) {
  rebrand(input)
  ...
  const body = spec.paths?.["/cypher/organization"]?.post?.requestBody?.content?.["application/json"]?.schema
  ...
  const profile = json("/cypher/profile")?.schema?.properties
  const pass = profile?.cypherPass?.properties
  ...
  if (profile?.cypherPass) profile.cypherPass = nullable(profile.cypherPass)
  if (profile?.currentOrgId) profile.currentOrgId = nullable(profile.currentOrgId)

  const sessions = json("/cypher/cloud-sessions")?.schema?.properties
  ...
  const claw = json("/cypher/claw/status")?.schema?.properties
  ...
  const credentials = json("/cypher/claw/chat-credentials")
  ...
  const fim = spec.paths?.["/cypher/fim"]?.post?.responses
  if (!fim) return
  fim["200"] = {
