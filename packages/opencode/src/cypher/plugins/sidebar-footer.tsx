import type { TuiPlugin, TuiPluginApi, TuiPluginModule } from "@cypher/plugin/tui"
...
const id = "internal:cypher-sidebar-footer"
...
export function passLine(pass: CypherPassState) {
...
// A billed turn: a completed Cypher assistant message with a non-zero cost.
export function billable(info: Message) {
  if (info.role !== "assistant") return false
  return info.providerID === "cypher" && info.time.completed !== undefined && info.cost > 0
}
...
function View(props: { api: TuiPluginApi }) {
  ...
  const has = createMemo(() =>
    props.api.state.provider.some(
      (item) =>
        (item.id !== "opencode" && item.id !== "cypher") ||
              ...
              {(pass) => (
                <box gap={0}>
                  <box flexDirection="row" justifyContent="space-between" gap={1}>
                    <text fg={theme().textMuted}>{" └ Cypher Pass"}</text>
      ...
      <Show when={show()}>
        ...
        >
          ...
          <box flexGrow={1} gap={1}>
            ...
            <text fg={theme().textMuted}>Cypher includes free models so you can start immediately.</text>
