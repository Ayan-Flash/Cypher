...
import { Bus } from "@/bus"
import { AgentManagerEvent, type AgentManagerTask } from "@/cypher/agent-manager/event"
...
// Prefer the provider the user already uses for the invoking turn, then the Cypher Gateway,
...
>(
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      execute: (params, ctx) =>
        Effect.gen(function* () {
          const msg = CypherSessionMessageOrder.latest(ctx.messages).user
