...
import { CYPHER_OPENROUTER_BASE } from "@cypher/cypher-gateway"
import DESCRIPTION from "./generate-image.txt"

const log = Log.create({ service: "tool.generate_image" })

const CYPHER_OPENROUTER_URL = `${CYPHER_OPENROUTER_BASE}/chat/completions`
...
function buildRequest(resolved: ResolvedProvider, prompt: string, model: string, inputImage?: string): ResolvedRequest {
  ...
  if (resolved.organizationId) headers["X-CYPHER-ORGANIZATIONID"] = resolved.organizationId
    ...
    return {
      ...
      execute: (params: Schema.Schema.Type<typeof Parameters>, ctx: Tool.Context) =>
        Effect.gen(function* () {
          ...
          const auth = yield* authSvc.get("cypher")
          ...
          if (!resolved) {
            return {
              ...
              output:
                "No image generation provider available. Log in to Cypher or set OPENROUTER_API_KEY, then try again.",
