...
const log = Log.create({ service: "cypher-task-model" })
...
export namespace CypherTask {
  ...
  /** Cypher keeps delegation one level deep to avoid recursive subagent chains. */
   ...
   * Cypher intentionally differs: parent denials remain hard ceilings for Plan Mode
  ...
  const saved = Effect.fn("CypherTask.savedModel")(function* (name: string) {
    if (Flag.CYPHER_CLIENT !== "cli") return undefined
  ...
  export const resolveModel = Effect.fn("CypherTask.resolveModel")(function* (input: {
    name: string
