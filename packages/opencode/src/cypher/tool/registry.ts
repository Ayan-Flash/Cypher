...
const log = Log.create({ service: "cypher-tool-registry" })
type Deps = { agent: Agent.Interface; truncate: Truncate.Interface; indexing?: boolean }
type Loaders = {
  indexing?: () => Promise<{ CypherIndexing: { ready: () => boolean } }>
  semantic?: () => Promise<Pick<typeof import("@/cypher/tool/semantic-search"), "SemanticSearchTool">>
}

export namespace CypherToolRegistry {
  const hint =
  ...
  export function usePatch(input: { modelID: string; family?: string }) {
    if (process.env["CYPHER_E2E_LLM_URL"]) return true
  ...
  /** Resolve Cypher-specific tool Infos outside any InstanceState, so their Truncate/Agent deps are
  ...
  /** Finalize Cypher-specific tools into Tool.Defs. Call this inside the InstanceState state Effect —
  ...
  function semanticTool(deps: Deps, loaders: Loaders) {
    return Effect.gen(function* () {
      const ready = yield* deps.indexing === undefined
        ? (() => {
            const indexing = loaders.indexing ?? (() => import("@/cypher/indexing"))
            return Effect.tryPromise(() => indexing().then((mod) => mod.CypherIndexing.ready())).pipe(
              Effect.catch((err) =>
                Effect.sync(() => {
      ...
      const semantic = loaders.semantic ?? (() => import("@/cypher/tool/semantic-search"))
  ...
  /** Cypher-specific tools to append to the builtin list */
  export function extra(
    ...
    return [
      ...
      tools.save,
      tools.recall,
      ...(Flag.CYPHER_CLIENT === "cli" || Flag.CYPHER_CLIENT === "vscode" ? [tools.process] : []),
      ...(Flag.CYPHER_CLIENT === "cli" && tools.terminal ? [tools.terminal] : []),
      // Agent Manager tools are useful only when the extension can create and display their sessions.
      ...(Flag.CYPHER_CLIENT === "vscode" ? [tools.managerModels, tools.manager] : []),
      ...(Flag.CYPHER_CLIENT === "vscode" &&
  ...
  /** Per-turn cache of `CypherMemory.toolEnabled` keyed by root string, with a short TTL so the
   * step-loop coalesces probes inside a single turn. Cache is invalidated immediately on enable /
   * disable / purge / rebuild via the MemoryEvents bus (subscribed in cypher/bootstrap.ts). */
  export function memoryToolsEnabled(input: { ctx: MemoryPaths.Ctx }) {
    return Effect.gen(function* () {
      ...
      const enabled = yield* Effect.tryPromise({
        try: () => CypherMemory.toolEnabled({ ctx: input.ctx }),
        catch: (err) => err,
      }).pipe(
        Effect.catch((err) =>
          Effect.sync(() => {
  ...
  /** Hide Cypher memory tools from the model when project memory is disabled. */
  export const applyVisibility = Effect.fn("CypherToolRegistry.applyVisibility")(function* (tools: Tool.Def[]) {
    const ctx = yield* InstanceState.context
    const memoryEnabled = yield* memoryToolsEnabled({ ctx })
    return tools.filter((tool) => {
      if (tool.id.startsWith("cypher_memory_")) return memoryEnabled
      return true
    })
  })
