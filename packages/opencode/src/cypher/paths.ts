...
export namespace CyphercodePaths {
  const home = () => process.env.HOME || process.env.USERPROFILE || os.homedir()

  /**
   * Get the platform-specific VSCode global storage path for Cyphercode extension.
   * - macOS: ~/Library/Application Support/Code/User/globalStorage/cypher.cypher-cli
   * - Windows: %APPDATA%/Code/User/globalStorage/cypher.cypher-cli
   * - Linux: ~/.config/Code/User/globalStorage/cypher.cypher-cli
   */
  export function vscodeGlobalStorage(): string {
    const home = os.homedir()
    switch (process.platform) {
      case "darwin":
        return path.join(home, "Library", "Application Support", "Code", "User", "globalStorage", "cypher.cypher-cli")
      case "win32":
        return path.join(
      ...
      default:
        return path.join(home, ".config", "Code", "User", "globalStorage", "cypher.cypher-cli")
    }
  }

  /** Global Cypher directories in user home: ~/.cypher and ~/.cypher (legacy first, .cypher wins later) */
  export function globalDirs(): string[] {
    return [path.join(home(), ".cypher"), path.join(home(), ".cypher")]
  }

  /**
   * Discover Cypher directories containing skills.
   * Returns parent directories (.cypher/ and .cypher/) for glob pattern "skills/[*]/SKILL.md".
   *
   * - Walks up from projectDir to worktreeRoot for .cypher/ and .cypher/
   * - Includes global ~/.cypher/ and ~/.cypher/
   ...
   * by the Cypher VSCode extension.
   */
  export async function skillDirectories(opts: {
    ...
    if (!opts.skipGlobalPaths) {
      // 1. Global ~/.cypher/ and ~/.cypher/ (loaded first so project-level overrides)
    ...
    // 3. Walk up from project dir to worktree root for .cypher/ and .cypher/
    ...
    for (const target of [".cypher", ".cypher"] as const) {
