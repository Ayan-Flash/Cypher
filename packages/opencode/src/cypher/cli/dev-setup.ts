...
const MARKER_START = "# >>> cypherdev launcher >>>"
const MARKER_END = "# <<< cypherdev launcher <<<"

export const DevSetupCommand = cmd({
  command: "dev-setup",
  describe: "install a `cypherdev` shell alias for this checkout",
  builder: (y) =>
    y
      .option("shell", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    UI.empty()
    UI.println(`  ${B}Cypher CLI dev launcher setup${N}`)
    UI.empty()
    ...
    UI.println(
      `  ${action === "install" ? "This will add" : "This will replace"} one line so ${H}cypherdev${N} runs this checkout from any directory:`,
    )
    UI.empty()
    UI.println(`      ${snippet}`)
    UI.empty()
...
// Deprecated: kept so `cypher dev-alias` keeps working for scripts/docs.
...
function aliasLine(shell: Shell, repo: string): string {
  const sh = path.join(repo, "bin", "cypherdev")
  const bat = path.join(repo, "bin", "cypherdev.cmd")
  const q = (s: string) => `'${s.replaceAll("'", `'\\''`)}'`
  if (shell === "fish") return `alias cypherdev ${q(sh)}`
  if (shell === "powershell") return `function cypherdev { & '${bat.replaceAll("'", "''")}' @args }`
  return `alias cypherdev=${q(sh)}`
}
...
async function applyBlock(rc: string, snippet: string): Promise<{ created: boolean; backup?: string }> {
  ...
  const backup = `${rc}.cypherdev.bak.${stamp()}`
...
export async function detectRepo(): Promise<string> {
  const hint = process.env.CYPHER_DEV_REPO
  if (hint) return hint
  ...
  // For a local build it's <repo>/packages/opencode/dist/<target>/bin/cypher, so
  ...
  throw new Error(
    "cannot locate cypher source checkout; set CYPHER_DEV_REPO=/path/to/cypher or run ./bin/cypherdev dev-setup from the repo",
  )
}
