...
import { fetchBalance, fetchProfile, type CyphercodeBalance, type CyphercodeProfile } from "@cypher/cypher-gateway"
...
interface Args {
  ...
  getProfile?: (token: string) => Promise<CyphercodeProfile>
  getBalance?: (token: string, organizationId?: string) => Promise<CyphercodeBalance | null>
...
export const ProfileCommand = cmd({
  command: "profile",
  describe: "show Cypher account profile",
  builder: (yargs: Argv) =>
    yargs.option("json", {
