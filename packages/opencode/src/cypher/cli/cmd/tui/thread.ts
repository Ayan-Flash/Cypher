...
import { importCloudSession } from "@/cypher/cloud-session"
...
export namespace CypherTuiThreadDaemon {
  ...
  export function workerAuth() {
    const password = Flag.CYPHER_SERVER_PASSWORD ?? randomUUID()
    const username = Flag.CYPHER_SERVER_USERNAME ?? "cypher"
    return {
      env: { CYPHER_SERVER_USERNAME: username, CYPHER_SERVER_PASSWORD: password },
