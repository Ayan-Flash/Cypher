/**
 * Cypher-specific TUI app customizations.
 *
...
import { isCypherError, showCypherErrorToast } from "@/cypher/cypher-errors"
import { registerCypherCommands } from "@/cypher/cypher-commands"
import { initializeTUIDependencies } from "@cypher/cypher-gateway/tui"
import { DialogProcessList } from "@/cypher/cli/cmd/tui/component/dialog-process-list"
import { useIndexingWarnings } from "@/cypher/cli/cmd/tui/indexing-warning"
import { CypherTerminalTitle } from "./terminal-title"
import type { CypherTitleIcon } from "./title-icon"
import { Session as SessionApi } from "@/session/session"

// Re-export so upstream can render the route without importing directly
export { CypherClawView } from "@/cypher/claw/view"
export { CypherTerminalTitle } from "./terminal-title"

// Hot reload TUI-local settings (keybinds/theme/ui) when changed from the Cypher Console.
// Called from the App body (below SDKProvider and the TuiConfig provider).
export { useTuiConfigHotReload } from "@/cypher/cli/cmd/tui/context/tui-config-hot-reload"
...
export const APP_TITLE = "Cypher CLI"

/** Public docs URL shown in the command palette. */
export const DOCS_URL = "https://cypher.ai/docs"

/** Human-readable product name used in user-facing messages. */
export const APP_NAME = "Cypher"
...
export function useSessionEffects(deps: {
  ...
  const pty = process.env.CYPHER_PTY_ID
  const state = { prev: "" }
...
export function getTerminalTitle(input: {
  ...
  icon?: CypherTitleIcon.Value
}): CypherTerminalTitle.Result | undefined {
  if (input.route.data.type === "home") {
    return {
      title: CypherTerminalTitle.format({ base: input.base, indicator: "none", icon: input.icon }),
      active: false,
      indicator: "none",
    }
  }
  ...
  if (input.route.data.type === "session") {
    const state = CypherTerminalTitle.session({
      base: input.base,
    ...
    return {
      ...state,
      title: CypherTerminalTitle.format({ base: input.base, title, indicator: state.indicator, icon: input.icon }),
    }
  }

  if (input.route.data.type === "plugin") {
    return {
      title: CypherTerminalTitle.format({
        base: input.base,
  ...
  if (input.route.data.type === "cypherclaw") {
    return {
      title: CypherTerminalTitle.format({ base: input.base, title: "CypherClaw", indicator: "none", icon: input.icon }),
      active: false,
      indicator: "none",
    }
  }
...
/**
 * Intercepts Cypher-specific errors and shows a warning toast.
 * Returns `true` if the error was handled, `false` otherwise.
 */
export function handleSessionError(error: unknown, toast: ReturnType<typeof useToast>): boolean {
  if (error && typeof error === "object" && isCypherError(error as any)) {
    showCypherErrorToast(error as any, toast)
    return true
  }
  return false
}
 ...
 *
 * - Injects TUI dependencies into cypher-gateway
 * - Registers Cypher Gateway commands (profile, teams, cypherclaw, etc.)
...
export function init() {
  ...
  // Inject TUI dependencies for cypher-gateway
  ...
  // Register Cypher Gateway commands (profile, teams, cypherclaw, remote, etc.)
  registerCypherCommands(useSDK)
