...
import type { AssistantMessage, Part, Provider, StepFinishPart } from "@cypher/sdk/v2"
...
export namespace RoutedModelMeta {
  ...
  export function label(list: Providers, model: StepFinishPart["model"]) {
    ...
    const id = CypherRoutedModel.display(model.modelID)
    ...
    return CypherRoutedModel.displayName(text)
  }

  function routed(model: StepFinishPart["model"], message: Message) {
    if (!model) return undefined
    if (message.providerID !== "cypher") return undefined
    if (!message.modelID.startsWith("cypher-auto/")) return undefined
