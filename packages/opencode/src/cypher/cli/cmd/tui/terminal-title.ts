import { CypherTitleIcon } from "./title-icon"
...
export namespace CypherTerminalTitle {
  ...
  } satisfies Record<CypherTitleIcon.Value, Record<Indicator, string>>

  export function format(input: { base: string; title?: string; indicator: Indicator; icon?: CypherTitleIcon.Value }) {
    const text = input.title ? `${input.base} | ${truncate(input.title)}` : input.base
    const prefix = icons[input.icon ?? CypherTitleIcon.Default][input.indicator]
