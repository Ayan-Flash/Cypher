/**
 * Cypher-specific overrides for the provider dialog.
 *
...
import type { ProviderAuthAuthorization } from "@cypher/sdk/v2"
import { CypherAutoMethod } from "@/cypher/components/dialog-cypher-auto-method"
export { selectProvider } from "@/cypher/anaconda-desktop/tui/setup"
...
/**
 * If the provider is Cypher Gateway, renders the custom `CypherAutoMethod`
...
export function renderAutoMethod(opts: {
  ...
  if (opts.providerID !== "cypher") return undefined
  return () => (
    <CypherAutoMethod
 ...
 * provider is Cypher Gateway. Returns `undefined` otherwise.
 */
export function renderApiDescription(
  ...
  if (providerID !== "cypher") return undefined
  return () => (
    <box gap={1}>
      <text fg={theme.textMuted}>
        Cypher Gateway gives you access to all the best coding models at the cheapest prices with a single API key.
      </text>
      <text fg={theme.text}>
        Go to <span style={{ fg: theme.primary }}>https://cypher.ai/gateway</span> to get a key
      </text>
    </box>
  )
}
