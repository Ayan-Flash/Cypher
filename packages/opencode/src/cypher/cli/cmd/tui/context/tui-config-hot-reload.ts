 ...
 * (`sdk.tui.config.get`) and reconcile it into the store via `CypherTuiConfig.useSet`. The
...
import { CypherTuiConfig } from "./tui-config"
...
export function useTuiConfigHotReload() {
  const set = CypherTuiConfig.useSet()
  const sdk = useSDK()
