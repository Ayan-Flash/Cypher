import { createCypherClient, type CypherClient } from "@cypher/sdk/v2"
import { UI } from "@/cli/ui"
import { DaemonClient } from "@/cypher/daemon/client"
import { isBuiltinCommand, type BuiltinCommand } from "@/cypher/session/builtin-commands"
import { Provider } from "@/provider/provider"
import { Filesystem } from "@/util/filesystem"

export namespace CypherRun {
  export async function resolveBuiltin(sdk: CypherClient, command?: string, directory?: string) {
...
export namespace CypherRunDaemon {
  export type Input = {
    directory?: string
    execute: (client: CypherClient) => Promise<void>
  }
  ...
  export async function attach(input: Input) {
    ...
    const client = createCypherClient({ baseUrl: daemon.url, directory: dir, headers: daemon.headers })
