...
import { serverUrls } from "@/cypher/cli/server-urls"
import { AppRuntime } from "@/effect/app-runtime"
import { Daemon } from "@/cypher/daemon/daemon"
import { warnPort } from "@/cypher/cli/port-warning"
import { hasDisplay } from "@/cypher/cli/cmd/tui/util/display"
import { StopCommand } from "@/cypher/cli/cmd/daemon"
...
const OpenCommand = cmd({
  command: "$0",
  describe: "open the local Cypher Console",
  builder: (yargs) =>
  ...
  handler: async (args) => {
    const run = async (signal?: AbortSignal) => {
      ...
      if (!state) throw new Error("Cypher daemon did not provide connection state")
      if (signal?.aborted) return state
      if (daemon.restarted) console.warn("Restarted the Cypher daemon to apply the requested network options")
      ...
      if (hasDisplay()) {
        ...
        console.warn("No display detected; open the Cypher Console URL manually")
      }
      console.log("Cypher Console:")
    ...
    await Daemon.foreground(async (signal) => {
      ...
      if (!signal.aborted) console.log("Press Ctrl+C to stop the Cypher daemon.")
      return state
    })
  },
})

export const CypherConsoleCommand = cmd({
  command: "console",
  describe: "open or stop the local Cypher Console",
