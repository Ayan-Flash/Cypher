...
import { CyphercodeConfig } from "./config"

export namespace CyphercodeConfigSources {
  ...
  const roots = [".cypher", ".cypher"] as const
  const global = ["config.json", "cypher.json", "cypher.jsonc", "opencode.json", "opencode.jsonc"] as const

  export async function list(input: Input): Promise<Result> {
    const project = Flag.CYPHER_DISABLE_PROJECT_CONFIG ? [] : await projectSources(input)
    const dirs = Flag.CYPHER_DISABLE_PROJECT_CONFIG ? [] : await configDirSources(input)
    const sources = [
  ...
  async function envFileSources(): Promise<Pending[]> {
    if (!Flag.CYPHER_CONFIG) return []
    return [
      await fileSource({
        kind: "env-file",
        scope: "env",
        label: "CYPHER_CONFIG",
        file: Flag.CYPHER_CONFIG,
        reason: "Explicit config file from CYPHER_CONFIG.",
      }),
    ]
  }

  async function projectSources(input: Input): Promise<Pending[]> {
    const cypher = await projectFiles("cypher", input)
    const opencode = await projectFiles("opencode", input)
    return Promise.all(
      [...cypher, ...opencode].map((file) =>
  ...
  async function configDirSources(input: Input): Promise<Pending[]> {
    ...
    const env = Flag.CYPHER_CONFIG_DIR ? [Flag.CYPHER_CONFIG_DIR] : []
    const dirs = unique([Global.Path.config, ...project, ...home, ...env]).filter((dir) =>
      CyphercodeConfig.isConfigDir(dir, Flag.CYPHER_CONFIG_DIR),
    )
    ...
    for (const dir of dirs) {
      ...
      for (const name of CyphercodeConfig.ALL_CONFIG_FILES) {
  ...
  function dirScope(dir: string, input: { project: string[]; home: string[] }): Scope {
    if (dir === Flag.CYPHER_CONFIG_DIR) return "env"
  ...
  function envContentSources(): Pending[] {
    ...
    if (process.env.CYPHER_CONFIG_CONTENT) {
      sources.push({
        kind: "env-content",
        scope: "env",
        label: "CYPHER_CONFIG_CONTENT",
        source: "CYPHER_CONFIG_CONTENT",
        exists: true,
        editable: false,
    ...
    if (Flag.CYPHER_CONFIG_DIR) {
      sources.push({
        kind: "runtime-env",
        scope: "env",
        label: "CYPHER_CONFIG_DIR",
        source: "CYPHER_CONFIG_DIR",
        path: Flag.CYPHER_CONFIG_DIR,
        exists: true,
        editable: false,
    ...
    if (Flag.CYPHER_DISABLE_PROJECT_CONFIG) {
      sources.push({
        kind: "runtime-env",
        scope: "env",
        label: "CYPHER_DISABLE_PROJECT_CONFIG",
        source: "CYPHER_DISABLE_PROJECT_CONFIG",
        exists: true,
        editable: false,
    ...
    return [
      {
        kind: "cloud-org",
        scope: "cloud",
        label: "Cypher Cloud organization config",
        source: `${account.url}/api/config`,
        exists: true,
        editable: false,
        reason: "Active organization config is managed by Cypher Cloud; values are not exposed here.",
      },
    ]
  }
  ...
  async function managedSources(): Promise<Pending[]> {
    ...
    const files = await Promise.all(
      CyphercodeConfig.ALL_CONFIG_FILES.map((name) =>
        fileSource({
          kind: "managed-file",
          scope: "managed",
          label: `Managed ${name}`,
  ...
  function runtimeSources(): Pending[] {
    return [
      runtimeSource("CYPHER_PERMISSION", Flag.CYPHER_PERMISSION, "Runtime permission overlay."),
      runtimeSource("CYPHER_DISABLE_AUTOCOMPACT", process.env.CYPHER_DISABLE_AUTOCOMPACT, "Disables automatic compaction."),
      runtimeSource("CYPHER_DISABLE_PRUNE", process.env.CYPHER_DISABLE_PRUNE, "Disables tool-output pruning."),
