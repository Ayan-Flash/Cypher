...
import { CyphercodeConfigSources } from "./sources"

export namespace CyphercodeConfigOverlay {
  const log = Log.create({ service: "cypher.config.overlay" })
  ...
  export const Result = z.object({
    ...
    sources: z.array(CyphercodeConfigSources.Source),
    targets: z.object({
  ...
  export type Input = {
    ...
    global: Config.Info
    sources: CyphercodeConfigSources.Source[]
  }

  const files = ["cypher.jsonc", "cypher.json", "opencode.jsonc", "opencode.json"] as const
  const dirs = [".cypher", ".cypher"] as const
  ...
  const fieldPaths = [
    ...
    ["indexing", "cypher", "apiKey"],
    ["indexing", "cypher", "baseUrl"],
    ["indexing", "cypher", "organizationId"],
  ...
  export async function project(input: { directory: string; worktree?: string }): Promise<Config.Info> {
    ...
    // cypher_change - project config is untrusted; confine {file:} reads to the project root
    ...
    return configs.reduce((result, cfg) => CyphercodeConfig.mergeConfig(result, cfg), {} as Config.Info)
  }
  ...
  export async function projectTarget(input: { directory: string; worktree?: string }) {
    ...
    return candidates.find((file) => existsSync(file)) ?? path.join(input.directory, ".cypher", "cypher.jsonc")
  }
  ...
  export async function resolve(input: Input): Promise<Result> {
    // cypher_change start - project agents untrusted, {file:} confined to the project root; global agents trusted
    const root = input.worktree && input.worktree !== "/" ? input.worktree : input.directory
  ...
  function globalDirs() {
    return [Global.Path.config, path.join(Global.Path.home, ".cypher"), path.join(Global.Path.home, ".cypher")]
  }

  // cypher_change start - root confines untrusted agent {file:} reads
  async function withAgents(input: Config.Info, dirs: string[], trusted: boolean, root?: string): Promise<Config.Info> {
    ...
    const next = CyphercodeConfig.mergeConfig(CyphercodeConfig.mergeConfig(input, { agent }), { agent: mode })
  ...
  async function load(file: string, fileScope?: ConfigVariable.FileScope): Promise<Config.Info> {
    // cypher_change start - a single unsafe/invalid project config file must not break the settings overlay;
  ...
  async function loadUnsafe(file: string, fileScope?: ConfigVariable.FileScope): Promise<Config.Info> {
    ...
    // cypher_change - overlay reads project config files: {env:} rejected, {file:} confined to fileScope.root
