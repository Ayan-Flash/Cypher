...
import { fetchOrganizationModes } from "@cypher/cypher-gateway"
...
export namespace CyphercodeConfig {
  const log = Log.create({ service: "cypher.config" })
  ...
  /** Cypher-specific config file names (highest-to-lowest precedence within cypher). */
  export const CYPHER_CONFIG_FILES = ["cypher.jsonc", "cypher.json"] as const

  /** All config file names in precedence order (cypher + opencode). */
  export const ALL_CONFIG_FILES = ["cypher.jsonc", "cypher.json", "opencode.jsonc", "opencode.json"] as const

  /** Config directory suffixes in update-target preference order. */
  export const CYPHER_DIR_SUFFIXES = [".cypher", ".cypher"] as const

  /** Path patterns for resolving cypher agent names from file paths. */
  export const AGENT_PATTERNS = ["/.cypher/agent/", "/.cypher/agents/", "/.cypher/agent/", "/.cypher/agents/"] as const

  /** Path patterns for resolving cypher command names from file paths. */
   ...
   *
   * This mirrors the Cypher project-config load chain: prefer existing config files
   * in ancestor config directories, then existing root config files, and create
   * `.cypher/cypher.jsonc` when no project config exists yet.
   */
  export const projectConfigUpdateTarget = Effect.fn("CyphercodeConfig.projectConfigUpdateTarget")(function* (input: {
    fs: AppFileSystem.Interface
    directory: string
    worktree?: string
  }) {
    const dirs = yield* input.fs
      .up({ targets: [...CYPHER_DIR_SUFFIXES], start: input.directory, stop: input.worktree })
      .pipe(Effect.orDie)
    ...
    return files.find((file) => existsSync(file)) ?? path.join(input.directory, ".cypher", "cypher.jsonc")
  })

  export const updateProjectConfig = Effect.fn("CyphercodeConfig.updateProjectConfig")(function* (input: {
    fs: AppFileSystem.Interface
    directory: string
    worktree?: string
    config: Config.Info
  ...
  export async function handleInvalid(
    ...
    try {
      const [{ Session }, { capture }] = await Promise.all([import("@/session/session"), import("@/cypher/instance")])
      const ctx = capture()
  ...
  /**
   * Load all Cyphercode legacy configs (modes, workflows, rules, MCP, ignore).
  ...
  export async function loadLegacyConfigs(input: {
    ...
    try {
      ...
      if (Object.keys(migration.agents).length > 0) {
        ...
        log.debug("loaded cypher custom modes", {
          count: Object.keys(migration.agents).length,
          modes: Object.keys(migration.agents),
        })
      }
      for (const skipped of migration.skipped) {
        log.debug("skipped cypher mode", { slug: skipped.slug, reason: skipped.reason })
      }
    } catch (err) {
      log.warn("failed to load cypher modes", { error: err })
    }

    // Load Cyphercode workflows as commands
    try {
      ...
      if (Object.keys(migration.commands).length > 0) {
        ...
        log.debug("loaded cypher workflows as commands", {
          count: Object.keys(migration.commands).length,
          commands: Object.keys(migration.commands),
        })
      }
    } catch (err) {
      log.warn("failed to load cypher workflows", { error: err })
    }

    // Load Cyphercode rules
    try {
      ...
      if (migration.instructions.length > 0) {
        ...
        log.debug("loaded cypher rules", {
          count: migration.instructions.length,
          files: migration.instructions,
        })
      }
      for (const warning of migration.warnings) {
        log.debug("cypher rules warning", { warning })
      }
    } catch (err) {
      log.warn("failed to load cypher rules", { error: err })
    }

    // Load Cyphercode MCP servers (skip global VSCode extension paths unless running in an editor or Console daemon)
    const skipGlobal = process.env["CYPHER_PLATFORM"] !== "vscode" && process.env["CYPHER_FEATURE"] !== "daemon"
    const mcp = await McpMigrator.loadMcpConfig(input.projectDir, skipGlobal)
    if (Object.keys(mcp).length > 0) {
      result = input.merge(result, { mcp })
    }

    // Load .cypherignore patterns
    try {
      const permission = await IgnoreMigrator.loadIgnoreConfig(input.projectDir)
      if (Object.keys(permission).length > 0) {
        result = input.merge(result, { permission })
        log.debug("loaded cypher ignore patterns", {
          hasRead: !!(permission as Record<string, unknown>).read,
          hasEdit: !!(permission as Record<string, unknown>).edit,
        })
      }
    } catch (err) {
      log.warn("failed to load cypher ignore patterns", { error: err })
    }
  ...
  /**
   * Load organization custom modes from the Cypher Cloud API.
  ...
  export async function loadOrganizationModes(
    ...
    const warnings: Config.Warning[] = []
    try {
      const cypher = auth["cypher"]
      if (cypher?.type === "oauth" && cypher.access && cypher.accountId) {
        const modes = await fetchOrganizationModes(cypher.access, cypher.accountId)
        if (modes.length > 0) {
  ...
  const GLOBAL_CONFIG_FILES = ["config.json", "cypher.json", "cypher.jsonc", "opencode.json", "opencode.jsonc"]
  ...
  export function isConfigDir(dir: string, flagDir?: string): boolean {
    return dir.endsWith(".cypher") || dir.endsWith(".cypher") || dir === flagDir
  }

  // ── Opencode config migration notice ─────────────────────────────────

  /** Client-neutral docs page describing where Cypher reads configuration from. */
  export const CONFIG_DOCS_URL = "https://cypher.ai/docs/getting-started/settings"

  /** Stable id for the synthetic "move your opencode config" notification (used for client-side dismissal). */
  export const OPENCODE_NOTIFICATION_ID = "cypher.local.opencode-config-detected"

  /**
   * Detect leftover opencode config directories. Cypher used to fall back to
  ...
  export function detectOpencodeConfig(input: { directory: string; worktree?: string; scanProject: boolean }): string[] {
    ...
    // Global opencode config dir (sibling of the cypher global config dir, e.g. ~/.config/opencode).
  ...
  export function opencodeConfigNotification(input: { directory: string; worktree?: string; scanProject: boolean }) {
    ...
    return {
      ...
      message:
        `Cypher no longer falls back to opencode configuration. ` +
        `Found opencode config at ${found[0]}${suffix}. ` +
        `Move it into a .cypher directory (project) or ${Global.Path.config} (global).`,
