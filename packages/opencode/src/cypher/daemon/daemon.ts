...
export namespace Daemon {
  const username = "cypher"
  const lock = "cypher-daemon"
  ...
  function root() {
    return process.env.CYPHER_TEST_DAEMON_STATE_DIR ?? Global.Path.state
  }

  function logs() {
    return process.env.CYPHER_TEST_DAEMON_LOG_DIR ?? Global.Path.log
  }
  ...
  export function matches(state: State, input: Options, explicit: readonly NetworkOption[]) {
    if (state.password === "cypher") return false
  ...
  async function port(input: Options) {
    ...
    if (input.env?.CYPHER_TEST_DAEMON_EPHEMERAL_PORT) return 0
    ...
    try {
      const child = spawn(cmd[0], [...cmd.slice(1), ...args(input)], {
        ...
        env: {
          ...process.env,
          ...input.env,
          CYPHER_SERVER_USERNAME: username,
          CYPHER_SERVER_PASSWORD: password,
          CYPHER_FEATURE: "daemon",
        },
  ...
  async function line(out: string) {
    ...
    const match = text.match(/cypher server listening on http:\/\/([^:\s]+):(\d+)/)
