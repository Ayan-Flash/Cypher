...
import { MemoryConfig } from "@cypher/cypher-memory/effect/config"
import { MemoryError } from "@cypher/cypher-memory/effect/errors"
import type { MemoryPorts } from "@cypher/cypher-memory/effect/ports"
import { MemoryRedact } from "@cypher/cypher-memory/redact"
import { MemoryShared } from "@cypher/cypher-memory/shared"
...
function recalledMemory(turn: Turn) {
  return [turn.user, ...turn.assistants].flatMap((item) => item.parts).some((part) => {
    if (part.type === "tool") {
      return (
        part.tool === "cypher_memory_recall" &&
    ...
    const marker = (part.metadata as { cypherMemory?: { type?: string; count?: number } } | undefined)?.cypherMemory
