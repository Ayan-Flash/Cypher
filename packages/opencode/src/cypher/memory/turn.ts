...
import { MemoryTurn as TurnCore } from "@cypher/cypher-memory/effect/turn"
...
import { CypherSessionPrompt } from "@/cypher/session/prompt"
...
export namespace MemoryLifecycle {
  export const subscribe = Effect.fn("MemoryLifecycle.subscribe")(function* (input: {
    ...
    yield* input.bus.subscribeCallback(CypherSession.Event.TurnOpen, (evt) =>
      bridge.fork(
    ...
    yield* input.bus.subscribeCallback(CypherSession.Event.TurnClose, (evt) =>
      bridge.fork(
        Effect.gen(function* () {
          const ctx = yield* InstanceState.context
          const enabled = yield* CypherSessionPrompt.memoryToolEnabled({ ctx })
          if (!enabled) return
