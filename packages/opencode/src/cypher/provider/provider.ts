...
//
// Cypher-specific provider logic extracted from packages/opencode/src/provider/provider.ts
...
// calls at well-defined injection points (each marked with cypher_change).

import { createCypher, type CypherProvider, AI_SDK_PROVIDERS, PROMPTS } from "@cypher/cypher-gateway"
import { DEFAULT_HEADERS } from "@/cypher/const"
...
export const CYPHER_BUNDLED_PROVIDERS: Record<string, () => Promise<(options: any) => BundledSDK>> = {
  "@cypher/cypher-gateway": async () => createCypher as unknown as (options: any) => BundledSDK,
}
...
export const CYPHER_MODEL_SCHEMA_EXTENSIONS = {
...
// ---------------------------------------------------------------------------
// fromModelsDevModel patch — returns cypher-specific fields
// ---------------------------------------------------------------------------

export function patchModelsDevModel(providerID: string, source: any) {
  return {
    variants: providerID === "cypher" ? (source.variants ?? {}) : {},
...
// ---------------------------------------------------------------------------
// Config model patch — merges cypher-specific fields from config + existing
// ---------------------------------------------------------------------------
...
export function patchCypherProviderPrivacy(provider: { options?: Record<string, any> } | undefined, config: any) {
  if (!provider || config.hide_prompt_training_models !== true) return
  provider.options = { ...provider.options, dataCollection: "deny" }
}

export function cypherCustomLoaders(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    cypher: Effect.fnUntraced(function* (input: any) {
      ...
      const hasKey = yield* Effect.gen(function* () {
        ...
        if (config.provider?.["cypher"]?.options?.apiKey) return true
        return false
      })

      const options: Record<string, string> = {}
      if (env.CYPHER_ORG_ID) {
        options.cypherOrganizationId = env.CYPHER_ORG_ID
      }
      ...
      return {
        ...
        options,
        async getModel(sdk: CypherProvider, modelID: string) {
  ...
  switch (providerID) {
    ...
    case "cerebras":
      result.options.headers = {
        ...result.options.headers,
        "X-Cerebras-3rd-Party-Integration": "cypher",
      }
      break
    case "azure": {
    ...
    // in provider.ts with single-line cypher_change markers
  }
}
...
export function cypherSmallModelPriority(providerID: string): string[] | undefined {
  if (providerID.startsWith("cypher")) return ["cypher-auto/small"]
  return undefined
}
