# Cypher CLI Configuration Reference

All config lives in `cypher.json` (or `cypher.jsonc`). Precedence low-to-high: remote well-known, global (`~/.config/cypher/cypher.json`), env `CYPHER_CONFIG` (or `CYPHER_CONFIG`), project `./cypher.json`, `.cypher/cypher.json`, `CYPHER_CONFIG_CONTENT` (or `CYPHER_CONFIG_CONTENT`), managed (see Config File Locations). Deep-merged; later wins.

This also covers where Cypher looks for config files, commands, agents, and skills across project, global, and legacy paths such as `.cypher/`, `.cypher/`, and `~/.config/cypher/`, plus Agent Manager setup/run scripts in the VS Code extension.

## Commands (`.cypher/command/*.md`)

Markdown files with YAML frontmatter. The filename (minus `.md`) becomes the command name invoked via `/name`. Commands can live in `.cypher/`, legacy `.cypher/`, and global config roots, with both `command/` and `commands/` directory names supported. See Config File Locations for the full search order.
...
2. `~/.cypher/`
3. `~/.cypher/`
4. The `CYPHER_CONFIG_DIR` or `CYPHER_CONFIG_DIR` directory (if set)
5. project `.cypher/` and `.cypher/` directories from the current working directory up to the worktree root
...
## Agents (`.cypher/agent/*.md`)

Also loaded from legacy `.cypher/` directories and plural `agents/` variants.
...
Markdown files in `.cypher/workflows/` or `.cypher/workflows/` (project-level) and `~/.cypher/workflows/` or `~/.cypher/workflows/` (global). These are automatically converted to commands at startup. The filename (minus `.md`) becomes the command name. Project workflows override global ones with the same name.

## Agent Manager Setup And Run Scripts

For the full product guidance, use the canonical [Agent Manager reference](https://cypher.ai/docs/automate/agent-manager) and [Agent Manager Workflows guide](https://cypher.ai/docs/automate/agent-manager-workflows). Prefer these links instead of guessing documentation paths.

Agent Manager setup/run scripts are project files in the main repository's `.cypher/` directory. They are not `cypher.json` settings and should not be configured inside generated `.cypher/worktrees/<name>/` checkouts.

Agent Manager worktrees usually live under `.cypher/worktrees/`. Think of each worktree as a separate checkout on its own branch: it enables parallel edits, but dependencies, build output, caches, databases, and generated files can consume significant disk space across many worktrees.
...
|---|---|
| macOS / Linux | `.cypher/setup-script`, `.cypher/setup-script.sh` |
| Windows | `.cypher/setup-script.ps1`, `.cypher/setup-script.cmd`, `.cypher/setup-script.bat` |
...
- For port conflicts (`EADDRINUSE`, browser/tests hitting the wrong worktree), inspect the app's dev-server config as well as `.cypher/run-script`. If fixing it requires application changes, ask whether the user wants the app made configurable or only wants a run-script workaround.
...
Tool permissions: `read`, `edit`, `glob`, `grep`, `list`, `bash`, `task`, `webfetch`, `websearch`, `semantic_search`, `cypher_memory_save`, `cypher_memory_recall`, `lsp`, `skill`, `external_directory`, `todowrite`, `todoread`, `question`, `doom_loop`.
...
For example, this configuration will hide all models from the built-in Cypher Gateway as well as any from the OpenAI provider which may be enabled automatically through environment variables.

```jsonc
{
  "$schema": "https://app.cypher.ai/config.json",
  "disabled_providers": ["cypher", "openai"],
}
```

The provider ID is the lowercase name used in the `provider/model` format (e.g., `cypher`, `openai`, `anthropic`, `google`, `groq`).
...
Skills are markdown files at `skills/<name>/SKILL.md` (or `skill/<name>/SKILL.md`) with `name` and `description` in frontmatter. Discovered inside `.cypher/` and legacy `.cypher/` directories.
...
|---|---|---|---|
| Switch theme | `<leader>t` | `/themes` | Pick from 35+ built-in themes (cypher, catppuccin, dracula, github, gruvbox, nord, tokyonight, etc.) |
| Toggle appearance (dark/light) | — | — | Ctrl+P → "Toggle appearance" |

Custom themes: place JSON files in `~/.config/cypher/themes/` or `.cypher/themes/`.
...
Notification settings are managed through `cypher console` under **Settings > CLI > Notifications**, or through `attention` in `tui.json` / `tui.jsonc`. There is no notification slash command or command-palette toggle.
...
### Config files (cypher.json)

| Scope | Path |
|---|---|
| Project | `./cypher.json`, `./cypher.jsonc`, `./opencode.json` (legacy), `./opencode.jsonc` (legacy) |
| Global | `~/.config/cypher/cypher.json`, `~/.config/cypher/cypher.jsonc`, `~/.config/cypher/opencode.json` (legacy), `~/.config/cypher/opencode.jsonc` (legacy), `~/.config/cypher/config.json` (legacy) |
| Managed | Linux: `/etc/cypher/`, macOS: `/Library/Application Support/cypher/`, Windows: `%ProgramData%\cypher\` — loads `cypher.json`, `cypher.jsonc`, `opencode.json`, `opencode.jsonc` (enterprise, highest priority) |

Each config directory (`.cypher/` and legacy `.cypher/`) can also contain `cypher.json`, `cypher.jsonc`, `opencode.json`, or `opencode.jsonc`.

### Config directories

Two directory names are scanned: `.cypher` (canonical) and `.cypher` (legacy fallback). Both are checked at each level, and `.cypher` wins when both define the same entry. `.opencode` directories are not loaded.

- **Project**: walks up from CWD to the git worktree root, checking both directories at each level
- **Home**: `~/.cypher/` and `~/.cypher/`
- **XDG global**: `~/.config/cypher/` (always loaded, lowest file-based precedence)
...
Example: `~/.config/cypher/command/*.md` (global), `~/.cypher/command/*.md` (legacy home), and `.cypher/commands/*.md` (project) all load commands.
...
|---|---|
| `CYPHER_CONFIG` / `CYPHER_CONFIG` | Path to an additional config file (loaded after global) |
| `CYPHER_CONFIG_DIR` / `CYPHER_CONFIG_DIR` | Path to an additional config directory (appended to search list) |
| `CYPHER_CONFIG_CONTENT` / `CYPHER_CONFIG_CONTENT` | Inline JSON config string (high precedence, after project dirs) |
| `CYPHER_DISABLE_PROJECT_CONFIG` / `CYPHER_DISABLE_PROJECT_CONFIG` | Skip all project-level config (files and directories) |
