...
import type { OrganizationMode } from "@cypher/cypher-gateway"

export namespace ModesMigrator {
  // Cyphercode mode structure
  export interface CyphercodeMode {
    slug: string
    name: string
  ...
  export interface CyphercodeModesFile {
    customModes: CyphercodeMode[]
  }
  ...
  export function convertPermissions(groups: CyphercodeMode["groups"]): ConfigPermission.Info {
  ...
  export function convertMode(mode: CyphercodeMode): ConfigAgent.Info {
  ...
  export async function readModesFile(filepath: string): Promise<CyphercodeMode[]> {
    try {
      const content = await fs.readFile(filepath, "utf-8")
      // Wrap YAML content in frontmatter delimiters so gray-matter can parse it
      const wrapped = `---\n${content}\n---`
      const parsed = matter(wrapped).data as CyphercodeModesFile
  ...
  export async function migrate(options: {
    ...
    const allModes: CyphercodeMode[] = []

    if (!options.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global modes)
      const vscodeGlobalPath = path.join(CyphercodePaths.vscodeGlobalStorage(), "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(vscodeGlobalPath)))

      // 2. CLI global settings (fallback/alternative location)
      const cliGlobalPath = path.join(os.homedir(), ".cypher", "cli", "global", "settings", "custom_modes.yaml")
      allModes.push(...(await readModesFile(cliGlobalPath)))

      // 3. Home directory .cyphermodes
      const homeModesPath = path.join(os.homedir(), ".cyphermodes")
    ...
    // 5. Project .cyphermodes
    const projectModesPath = path.join(options.projectDir, ".cyphermodes")
    allModes.push(...(await readModesFile(projectModesPath)))

    // Deduplicate by slug (later entries win)
    const modesBySlug = new Map<string, CyphercodeMode>()
