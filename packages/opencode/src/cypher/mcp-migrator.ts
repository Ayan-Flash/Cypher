...
export namespace McpMigrator {
  const log = Log.create({ service: "cypher.mcp-migrator" })

  // Remote transport types used by the Cyphercode extension
  const REMOTE_TYPES = new Set(["streamable-http", "sse"])

  function isRemote(server: CyphercodeMcpServer): boolean {
    return !!server.type && REMOTE_TYPES.has(server.type)
  }

  // Cyphercode MCP server structure
  export interface CyphercodeMcpServer {
    command?: string
    args?: string[]
  ...
  export interface CyphercodeMcpSettings {
    mcpServers: Record<string, CyphercodeMcpServer>
  }
  ...
  export async function readMcpSettings(filepath: string): Promise<CyphercodeMcpSettings | null> {
    if (!(await Filesystem.exists(filepath))) return null

    try {
      const content = await fs.readFile(filepath, "utf-8")
      return JSON.parse(content) as CyphercodeMcpSettings
    } catch (err) {
  ...
  export function convertServer(name: string, server: CyphercodeMcpServer): ConfigMCP.Info | null {
    if (isRemote(server)) {
      if (!server.url) {
  ...
  export async function migrate(options?: {
    ...
    const allServers: Array<{ name: string; server: CyphercodeMcpServer }> = []

    if (!options?.skipGlobalPaths) {
      // 1. VSCode extension global storage (primary location for global MCP settings)
      const vscodeSettingsPath = path.join(CyphercodePaths.vscodeGlobalStorage(), "settings", "mcp_settings.json")
    ...
    // Check .cypher/mcp.json and .cypher/mcp.json for project-level settings
    // (not "mcp_settings.json" which is only used for global settings)
    // .cypher is loaded first (lower precedence), .cypher second (higher precedence)
    if (options?.projectDir) {
      for (const dir of [".cypher", ".cypher"]) {
    ...
    const serversByName = new Map<string, CyphercodeMcpServer>()
  ...
  /**
   * Load Cyphercode MCP servers and return them as an opencode config partial.
  ...
  export async function loadMcpConfig(
    ...
    try {
      ...
      if (Object.keys(result.mcp).length > 0) {
        log.debug("loaded cypher MCP servers", {
      ...
      for (const skipped of result.skipped) {
        log.debug("skipped cypher MCP server", { name: skipped.name, reason: skipped.reason })
      }

      for (const warning of result.warnings) {
        log.warn("cypher MCP migration warning", { warning })
      }

      return result.mcp
    } catch (err) {
      log.warn("failed to load cypher MCP servers", { error: err })
      return {}
    }
  }
}
