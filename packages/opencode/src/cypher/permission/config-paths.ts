...
import { CyphercodePaths } from "@/cypher/paths"

export namespace ConfigProtection {
  /**
   * Config directory prefixes (relative paths, forward-slash normalized).
   * Matches .cypher/ and legacy .cypher/ at any depth within the project.
   */
  const CONFIG_DIRS = [".cypher/", ".cypher/"]
   ...
   */
  const CONFIG_ROOT_FILES = new Set(["cypher.json", "cypher.jsonc", "opencode.json", "opencode.jsonc", "AGENTS.md"])
  ...
  export function isRelative(pattern: string): boolean {
    ...
    for (const dir of CONFIG_DIRS) {
      const bare = dir.slice(0, -1) // e.g. ".cypher"
      // Match at root (e.g. ".cypher/foo") or nested (e.g. "packages/sub/.cypher/foo")
  ...
  function configs(): string[] {
    return Array.from(
      new Set([Global.Path.config, process.env.XDG_CONFIG_HOME ? path.join(process.env.XDG_CONFIG_HOME, "cypher") : ""]),
    ).filter(Boolean)
  }
  ...
  function fallback(p: string): boolean {
    ...
    return keys(p).some(
      (key) =>
        key.endsWith("/config/cypher") ||
        key.includes("/config/cypher/") ||
        key.endsWith("/.config/cypher") ||
        key.includes("/.config/cypher/"),
    )
  }
  ...
  export function isAbsolute(filepath: string): boolean {
    ...
    // ~/.config/cypher/ (XDG config)
    for (const dir of configs()) {
      if (within(filepath, dir)) return true
    }

    // ~/.cypher/ and ~/.cypher/ (legacy global dirs)
    for (const dir of CyphercodePaths.globalDirs()) {
      if (within(filepath, dir)) return true
    }
