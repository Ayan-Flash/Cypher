...
/**
 * Read project ID from .cypher/config.json, falling back to .cypher/config.json
...
/**
 * Resolve project ID with priority: .cypher/config.json -> .cypher/config.json -> git origin URL
 * @returns Normalized project ID or undefined
 */
async function resolveProjectId(): Promise<string | undefined> {
  const dir = Instance.directory

  // Priority 1: .cypher/config.json (falls back to .cypher/config.json)
...
export namespace CypherProjectID {
  export interface Interface {
    readonly get: () => Effect.Effect<string | undefined>
  }

  export class Service extends Context.Service<Service, Interface>()("@cypher/CypherProjectID") {}

  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const state = yield* InstanceState.make(
        Effect.fn("CypherProjectID.state")(function* () {
...
const { runPromise } = makeRuntime(CypherProjectID.Service, CypherProjectID.defaultLayer)

/**
 * Get the project ID for the current Instance context (cached per-project)
 * @returns Normalized project ID or undefined
 */
export async function getCypherProjectId(): Promise<string | undefined> {
