...
export const PlanFollowupRuntime = {
  ...
  handover(input: LLM.StreamInput, signal: AbortSignal) {
    return llm().runPromise((svc) => CypherLLM.text(svc.stream(input)).pipe(Effect.orDie), { signal })
  },
      ...
      questions: [
        {
          ...
          custom: Flag.CYPHER_CLIENT === "cli" || Flag.CYPHER_CLIENT === "jetbrains",
          options: [
            {
