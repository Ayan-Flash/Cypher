...
import { type IndexingTelemetryEvent, type VectorStoreSearchResult } from "@cypher/cypher-indexing/engine"
import { toIndexingConfigInput, type IndexingConfig } from "@cypher/cypher-indexing/config"
import { hasIndexingPlugin } from "@cypher/cypher-indexing/detect"
import { IndexingStatus, disabledIndexingStatus } from "@cypher/cypher-indexing/status"
...
import { LanceDBRuntime } from "./lancedb" // cypher_change
import { indexingWithCypherDefault, resolveCypherIndexingAuth, type CypherIndexingAuth } from "./indexing-auth" // cypher_change
import { primaryWorktree } from "./primary-worktree"

const log = Log.create({ service: "cypher-indexing" })
...
const baselineDirectory = Effect.fn("CypherIndexing.baselineDirectory")(function* (dir: string) {
...
async function cypherAuth(cfg: Config.Info): Promise<CypherIndexingAuth> {
  const info = await auth.runPromise((svc) => svc.get("cypher"))
  return resolveCypherIndexingAuth({ config: cfg, auth: info })
}

function enrichCypher(input: ReturnType<typeof toIndexingConfigInput>, auth: CypherIndexingAuth) {
  if (input.embedderProvider !== "cypher") return input

  return {
    ...input,
    cypherApiKey: input.cypherApiKey ?? auth.apiKey,
    cypherBaseUrl: input.cypherBaseUrl ?? auth.baseUrl,
    cypherOrganizationId: input.cypherOrganizationId ?? auth.organizationId,
  }
}

async function model(input: ReturnType<typeof toIndexingConfigInput>, auth: CypherIndexingAuth) {
  if (input.embedderProvider !== "cypher") return input

  const catalog = await fetchCypherEmbeddingModelCatalog({ baseURL: auth.baseUrl, token: auth.apiKey })
  ...
  if (!found) {
    if (input.modelId || input.modelDimension) {
      log.warn("ignoring unsupported Cypher embedding model configuration", { model: input.modelId })
    }
    return { ...input, modelId: undefined, modelDimension: undefined }
  }

  if (input.modelId && !chosen) {
    log.warn("using default Cypher embedding model instead of unsupported configuration", {
      model: input.modelId,
      fallback: found.id,
    })
  }
...
export namespace CypherIndexing {
  ...
  const boot = async (hit: Cache): Promise<Entry> => {
    ...
    if (process.env["CYPHER_DISABLE_CODEBASE_INDEXING"] === "vscode-no-workspace") {
    ...
    const auth = await cypherAuth(cfg)
    const globalConfig = await AppRuntime.runPromise(Config.Service.use((svc) => svc.getGlobal()))
    const global = globalConfig.indexing
    const merged = indexingWithCypherDefault({ ...global, ...cfg.indexing }, auth)
    const cfgInput = await model(enrichCypher(input(merged, global), auth), auth)
  ...
  export async function models() {
    try {
      ...
      const fallback = await fetchCypherEmbeddingModelCatalog()
      return fallback.models.length > 0 ? fallback : catalog
    } catch (err) {
      log.warn("falling back to public Cypher embedding model catalog", { err })
      return fetchCypherEmbeddingModelCatalog()
    }
  }
