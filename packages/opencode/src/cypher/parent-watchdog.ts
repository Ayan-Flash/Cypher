 ...
 *
 * Editor clients (VS Code extension, JetBrains plugin) run `cypher serve` as a child
 ...
 * PID via `CYPHER_PARENT_PID`; we poll that PID and re-parenting so the server shuts
 * itself down when the client is gone.
 *
 * No-op unless `CYPHER_PARENT_PID` is set to a valid PID, so a manually launched
 * `cypher serve` (whose parent shell exiting may be intentional) is never affected.
 *
 * Returns a function that stops the watchdog.
 */
export function startParentWatchdog(onOrphan: () => void, intervalMs = 1000): () => void {
  const configured = Number(process.env["CYPHER_PARENT_PID"])
