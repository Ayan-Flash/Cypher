...
import { backendSupport, run as runSandbox, unrestricted, type Profile } from "@cypher/sandbox"
import { Bus } from "@/bus"
...
export function profile(
  ...
  return {
    ...
    environment: {
      deny: ["CYPHER_SERVER_PASSWORD", "CYPHER_SERVER_USERNAME"],
      set: {
...
const snapshot = Effect.fn("SandboxPolicy.snapshot")(function* (sessionID: SessionID) {
  ...
  return yield* locked(
    ...
    Effect.gen(function* () {
      ...
      // A session's create-time cypher.sandbox toggle takes precedence over the config default, so a
