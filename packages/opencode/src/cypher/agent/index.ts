...
function planEditRules(worktree: string) {
  return {
    "*": "deny" as const,
    [path.join(".cypher", "plans", "*.md")]: "allow" as const,
...
// Prepare cypher-specific data derived from config. Call once per state initialization.
export function prepare(cfg: Config.Info): CypherData {
  const mcpRules = getMcpRules(cfg)
  const defaultsPatch = Permission.fromConfig({
    bash,
    recall: "ask",
    ...(Flag.CYPHER_CLIENT === "vscode" && cfg.experimental?.native_notebook_tools === true
      ? { notebook_read: "ask" as const, notebook_edit: "ask" as const, notebook_execute: "ask" as const }
      : {}),
    cypher_memory_recall: "ask",
    cypher_memory_save: "ask",
  })
...
// Lift Cypher-internal metadata onto typed agent fields and remove it from `options`.
...
// Patch the base agents map in-place with all cypher-specific changes:
// - Rename build → code
// - Patch plan with readOnlyBash, mcpRules, .cypher paths
...
export function patchAgents(
  ...
  if (agents.plan) {
    agents.plan = {
      ...
      permission: Permission.merge(
        defaults,
        planGuard(worktree, cypher.mcpRules),
        user,
  ...
  agents.ask = {
    ...
    prompt: PROMPT_ASK,
    options: {},
    permission: Permission.merge(defaults, askGuard(cypher.mcpRules), user, askEditGuard(), denies(user)),
    mode: "primary",
    native: true,
  }
 ...
 * config-backed agent entries, and/or removing it from legacy .cyphermodes YAML files.
 * Scans all config directories for agent/mode .md files matching the name,
 * then also checks the .cyphermodes files the ModesMigrator reads.
 */
export async function remove(input: { name: string; agent?: AgentInfo; dirs: string[]; directory: string }) {
  ...
  // 2. Remove from legacy .cyphermodes YAML files (read by ModesMigrator)
  const { ModesMigrator } = await import("@/cypher/modes-migrator")
  const { CyphercodePaths } = await import("@/cypher/paths")
  const os = await import("os")
  const matter = (await import("gray-matter")).default
  const home = os.default.homedir()
  const modesFiles = [
    path.join(CyphercodePaths.vscodeGlobalStorage(), "settings", "custom_modes.yaml"),
    path.join(home, ".cypher", "cli", "global", "settings", "custom_modes.yaml"),
    path.join(home, ".cyphermodes"),
    path.join(input.directory, ".cyphermodes"),
  ]
...
async function removeConfigAgent(name: string, directory: string) {
  const { CyphercodeConfigOverlay } = await import("@/cypher/config/overlay")
  const files = [
    CyphercodeConfigOverlay.globalTarget(),
    await CyphercodeConfigOverlay.projectTarget({ directory }),
  ]
  let found = false
