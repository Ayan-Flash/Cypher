import { RemoteModelCatalog } from "@/cypher-sessions/remote-model-catalog"
import { RemoteProtocol } from "@/cypher-sessions/remote-protocol"
import type { RemoteWS } from "@/cypher-sessions/remote-ws"
...
type Provide = typeof import("@/cypher/instance").provide
...
// cypher_change start — lazy init to avoid circular dependency
...
function normalizeModel(model: string | RemoteModelCatalog.ModelRef | undefined) {
  ...
  return {
    providerID: ProviderID.make("cypher"),
    modelID: ModelID.make(model.startsWith("cypher/") ? model.slice("cypher/".length) : model),
  }
}
