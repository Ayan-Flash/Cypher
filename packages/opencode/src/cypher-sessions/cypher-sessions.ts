...
import { IngestQueue } from "@/cypher-sessions/ingest-queue"
import { clearInFlightCache, withInFlightCache } from "@/cypher-sessions/inflight-cache"
import type * as SDK from "@cypher/sdk/v2"
import z from "zod"
import { Context, Effect, Layer, Schema, Stream } from "effect"
import { CYPHER_API_BASE } from "@cypher/cypher-gateway"
...
import { RemoteWS } from "@/cypher-sessions/remote-ws"
import { RemoteSender } from "@/cypher-sessions/remote-sender"
...
import { cumulativeSessionDiff } from "@/cypher/session-portability/cumulative-diff"

async function provide<R>(input: { directory: string; fn: () => R }): Promise<R> {
  const { provide } = await import("@/cypher/instance")
  return provide(input)
}

export namespace CypherSessions {
  export const Event = {
    RemoteStatusChanged: BusEvent.define(
      "cypher-sessions.remote-status-changed",
      Schema.Struct({
  ...
  export class Service extends Context.Service<Service, Interface>()("@cypher/CypherSessions") {}

  const log = Log.create({ service: "cypher-sessions" })
  const runtime = makeRuntime(Auth.Service, Auth.defaultLayer)

  const Uuid = z.uuid()
  type Uuid = z.infer<typeof Uuid>

  const tokenValidKeyTemplate = "cypher-sessions:token-valid:"
  let tokenValidKey = tokenValidKeyTemplate + "unknown"

  const tokenKey = "cypher-sessions:token"
  const orgKey = "cypher-sessions:org"
  const clientKey = "cypher-sessions:client"
  const gitUrlKeyPrefix = "cypher-sessions:git-url:"
  ...
  async function authValid(token: string) {
    ...
    return withInFlightCache(tokenValidKey, 15 * 60_000, async () => {
      const response = await fetch(`${CYPHER_API_BASE}/api/user`, {
        headers: {
  ...
  async function cypherToken() {
    return withInFlightCache(tokenKey, ttlMs, async () => {
      const auth = await runtime.runPromise((svc) => svc.get("cypher"))
      ...
      const key = process.env["CYPHER_API_KEY"]?.trim()
      if (key) return key
      return undefined
    })
  }
  ...
  async function getClient(): Promise<Client | undefined> {
    return withInFlightCache(clientKey, ttlMs, async () => {
      const token = await cypherToken()
      if (!token) return undefined

      const valid = await authValid(token)
      if (!valid) return undefined

      const base = process.env["CYPHER_SESSION_INGEST_URL"] ?? "https://ingest.cyphersessions.ai"
  ...
  const shareDisabled = process.env["CYPHER_DISABLE_SHARE"] === "true" || process.env["CYPHER_DISABLE_SHARE"] === "1"
  const ingestDisabled =
    process.env["CYPHER_DISABLE_SESSION_INGEST"] === "true" || process.env["CYPHER_DISABLE_SESSION_INGEST"] === "1"
  const debugIngest =
    process.env["CYPHER_DEBUG_SESSION_INGEST"] === "true" || process.env["CYPHER_DEBUG_SESSION_INGEST"] === "1"
  ...
  const remoteEnabled = process.env["CYPHER_REMOTE"] === "1"
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const state = yield* InstanceState.make(
        Effect.fn("CypherSessions.state")(function* () {
          ...
          yield* watch(Session.Event.Updated, async (evt) => {
            ...
            await ingest.sync(sessionID, [
              { type: "cypher_meta", data: await meta(sessionID) },
      ...
      const init = Effect.fn("CypherSessions.init")(function* () {
  ...
  export async function enableRemote() {
    ...
    enabling = (async () => {
      const token = await cypherToken()
      if (!token) {
        throw new Error("Unable to enable remote: no Cypher credentials found. Run `cypher auth login`.")
      }

      const valid = await authValid(token)
      if (valid === false) {
        throw new Error("Unable to enable remote: invalid or expired Cypher credentials. Run `cypher auth login`.")
      }
      if (valid === undefined) throw new Error("Unable to enable remote: failed to verify Cypher credentials.")

      const url = (process.env["CYPHER_SESSION_INGEST_URL"] ?? "https://ingest.cyphersessions.ai")
        .replace(/^https:\/\//, "wss://")
        .replace(/^http:\/\//, "ws://")
  ...
  export async function share(sessionId: string) {
    if (ingestDisabled) {
      throw new Error("Session ingest is disabled (CYPHER_DISABLE_SESSION_INGEST=1)")
    }

    if (shareDisabled) {
      throw new Error("Sharing is disabled (CYPHER_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to share session: no Cypher credentials found. Run `cypher auth login`.")
    }
    ...
    const url = `https://app.cypher.ai/s/${result.public_id}`
  ...
  export async function unshare(sessionId: string) {
    ...
    if (shareDisabled) {
      throw new Error("Unshare is disabled (CYPHER_DISABLE_SHARE=1)")
    }

    const client = await getClient()
    if (!client) {
      throw new Error("Unable to unshare session: no Cypher credentials found. Run `cypher auth login`.")
    }
  ...
  async function meta(sessionId?: string) {
    const override = sessionId ? CypherSession.resolvePlatform(sessionId) : undefined
    const platform = override || process.env["CYPHER_PLATFORM"] || "cli"
  ...
  async function getOrgId(): Promise<Uuid | undefined> {
    const env = process.env["CYPHER_ORG_ID"]
