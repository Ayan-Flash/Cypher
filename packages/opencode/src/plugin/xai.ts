...
function authHeaders() {
  return {
    ...
    "User-Agent": `cypher/${InstallationVersion}`, // cypher_change
  }
}
...
export function buildAuthorizeUrl(
  ...
  // cypher_change start
  // clients. `referrer=cypher` lets xAI attribute cypher-originated
  // cypher_change end
...
<html>
  <head>
    <title>Cyphercode - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to Cyphercode.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>Cyphercode - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // cypher_change start
      // behavior and crash the entire Cyphercode process. Matches the silent-
      // cypher_change end
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `cypher/${InstallationVersion}`) // cypher_change
