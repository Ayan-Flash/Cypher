import type { Hooks, PluginInput } from "@cypher/plugin"
...
import { refreshCodexAuth } from "@/cypher/provider/codex-refresh" // cypher_change
...
const ALLOWED_MODELS = new Set([
  ...
  "gpt-5.4",
  "gpt-5.4-mini",
  // cypher_change start - additional codex models supported by Cypher
  "gpt-5.1-codex",
  "gpt-5.1-codex-max",
  "gpt-5.1-codex-mini",
  "gpt-5.2-codex",
...
function buildAuthorizeUrl(redirectUri: string, pkce: PkceCodes, state: string): string {
  const params = new URLSearchParams({
    ...
    state,
    originator: "cypher", // cypher_change
  })
...
async function refreshAccessToken(refreshToken: string, issuer = ISSUER, signal?: AbortSignal): Promise<TokenResponse> {
  const response = await fetch(`${issuer}/oauth/token`, {
    method: "POST",
    signal,
    headers: { "Content-Type": "application/x-www-form-urlencoded", "User-Agent": `cypher/${InstallationVersion}` },
    // cypher_change end
    body: new URLSearchParams({
...
<html>
  <head>
    <!-- cypher_change start -->
    <title>Cypher - Codex Authorization Successful</title>
    <!-- cypher_change end -->
    <style>
      body {
        font-family:
  ...
  <body>
    <div class="container">
      ...
      <!-- cypher_change start -->
      <p>You can close this window and return to Cypher.</p>
      <!-- cypher_change end -->
    </div>
    <script>
...
<html>
  <head>
    <!-- cypher_change start -->
    <title>Cypher - Codex Authorization Failed</title>
    <!-- cypher_change end -->
    <style>
      body {
        font-family:
        ...
        return Object.fromEntries(
          Object.entries(provider.models)
            .filter(([, model]) => {
              ...
              if (DISALLOWED_MODELS.has(model.api.id)) return false // cypher_change
