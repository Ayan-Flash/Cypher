...
import { context as instanceContext } from "@/project/instance-context" // cypher_change
...
export function attach<A, E, R>(effect: Effect.Effect<A, E, R>): Effect.Effect<A, E, R> {
  ...
  // cypher_change start - bridge legacy AsyncLocalStorage instance context into Effect runtimes
...
export function makeRuntime<I, S, E>(service: Context.Service<I, S>, layer: Layer.Layer<I, E>) {
  ...
  return {
    ...
    // cypher_change start - allow Cypher-owned service runtimes to release persistent resources
    dispose: async () => {
      const current = rt
      rt = undefined
