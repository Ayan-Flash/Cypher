...
import { Instance, type InstanceContext } from "@/cypher/instance" // cypher_change
...
// cypher_change start - preserve legacy Cypher contexts across Promise callbacks
 ...
 * legacy AsyncLocalStorage contexts for callback code that still reads them. // cypher_change
 *
 * Mirrors `Effect.promise` but restores Cypher compatibility contexts first. // cypher_change
 */
  ...
  return Effect.gen(function* () {
    ...
    return {
      promise: <A, E, R>(effect: Effect.Effect<A, E, R>) =>
        restore(instance, workspace, () => Effect.runPromise(wrap(effect))), // cypher_change
      fork: <A, E, R>(effect: Effect.Effect<A, E, R>) =>
        restore(instance, workspace, () => Effect.runFork(wrap(effect))), // cypher_change
      ...
      bind:
        ...
        (...args: Args) =>
          restore(instance, workspace, () => Effect.runSync(wrap(Effect.sync(() => fn(...args))))), // cypher_change
    } satisfies Shape
  })
}
