...
import { CypherPtySelfCommand } from "@/cypher/pty/self-command" // cypher_change
import type { Proc } from "#pty"
import * as Log from "@opencode-ai/core/util/log"
import { PtyID } from "./schema"
import { Effect, Layer, Context, Schema, Types } from "effect"
import { NonNegativeInt, PositiveInt } from "@opencode-ai/core/schema"
import { SessionID } from "@/session/schema" // cypher_change
...
export const Info = Schema.Struct({
  ...
  pid: NonNegativeInt,
  sessionID: Schema.optional(Schema.NullOr(SessionID)), // cypher_change
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("Pty.create")(function* (input: CreateInput) {
      ...
      const resolved = CypherPtySelfCommand.resolve(input) // cypher_change
      const command = resolved.command || Shell.preferred(cfg.shell) // cypher_change
      const args = resolved.args || [] // cypher_change
      if (Shell.login(command)) {
        args.push("-l")
      }

      const cwd = resolved.cwd || s.dir // cypher_change
      ...
      const env = {
        ...
        TERM: "xterm-256color",
        CYPHER_TERMINAL: "1",
        CYPHER_PTY_ID: id, // cypher_change
      } as Record<string, string>
      // cypher_change start
      // Don't leak the cypher server's auth credential into user shells.
      ...
      // with zero effort. Users who genuinely need `cypher run`/`cypher tui
      // attach` to auto-connect from inside a cypher-spawned terminal can
      // pass `--password` explicitly or re-export the env themselves.
      delete env.CYPHER_SERVER_PASSWORD
      delete env.CYPHER_SERVER_USERNAME
      // cypher_change end
