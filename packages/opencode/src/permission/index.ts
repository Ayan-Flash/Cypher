...
import * as Config from "@/config/config" // cypher_change
...
import os from "os"
import z from "zod" // cypher_change
...
import { CypherHeadless } from "@/cypher/permission/headless"
import { drainCovered } from "@/cypher/permission/drain"
import { ReadPermission } from "@/cypher/permission/read"
import { ExternalDirectoryPermission } from "@/cypher/permission/external-directory"
// cypher_change end
...
export const AskInput = Schema.Struct({
  ...
  ruleset: Ruleset,
  hardRuleset: Schema.optional(Ruleset), // cypher_change
...
interface State {
  ...
  approved: Rule[]
  session: Record<string, Ruleset> // cypher_change
}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const config = yield* Config.Service // cypher_change
    const state = yield* InstanceState.make<State>(
      Effect.fn("Permission.state")(function* (ctx) {
        ...
        const state = {
          ...
          approved: [...(row?.data ?? [])],
          session: {} as Record<string, Ruleset>, // cypher_change
        }
    ...
    const ask = Effect.fn("Permission.ask")(function* (input: AskInput) {
      ...
      // cypher_change start — force "ask" for config file edits
      const isProtected = ConfigProtection.isRequest(request)
      // cypher_change end

      for (const pattern of request.patterns) {
        const rule = resolve(request.permission, pattern, ruleset, approved, local) // cypher_change — include session-scoped rules
        log.info("evaluated", { permission: request.permission, pattern, action: rule })
        // cypher_change start — saved/session approvals cannot override hard Ask/Plan denials
        ...
        if (rule.action === "deny") {
          return yield* new DeniedError({
            ruleset: subset(request.permission, ruleset), // cypher_change
          })
        }
        // cypher_change start — override "allow" to "ask" for config paths
      ...
      // cypher_change start - headless subagent asks fail instead of queuing for a reply that never comes (#11903)
      if (CypherHeadless.denies(request.sessionID)) {
      ...
      const info: Request = {
        ...
        // cypher_change start — inject disableAlways + configProtected metadata for config paths
        metadata: {
          ...request.metadata,
          ...(isProtected
      ...
      pending.set(id, { info, ruleset, hardRuleset, deferred }) // cypher_change
    ...
    const reply = Effect.fn("Permission.reply")(function* (input: ReplyInput) {
      ...
      // cypher_change start — downgrade "always" to "once" for config file edits
      if (ConfigProtection.isRequest(existing.info)) return
      // cypher_change end

      for (const pattern of existing.info.always) {
        // cypher_change start — saveAlwaysRules may have already persisted selected always-rules
        if (!existing.saved) {
          approved.push({
    ...
    return Service.of({ ask, reply, list, saveAlwaysRules, allowEverything, pending }) // cypher_change
  }),
)
...
export function fromConfig(permission: ConfigPermission.Info) {
  ...
  for (const [key, value] of Object.entries(permission)) {
    ...
    if (value === null) continue // cypher_change — null is a delete sentinel
    ruleset.push(
      // cypher_change start — filter out null entries (delete sentinels)
...
export const defaultLayer = layer.pipe(Layer.provide(Bus.layer), Layer.provide(Config.defaultLayer)) // cypher_change

// cypher_change start — inverse of fromConfig: convert rules back to config format
