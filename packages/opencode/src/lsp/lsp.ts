...
import { TsClient } from "../cypher/ts-client" // cypher_change
...
const filterExperimentalServers = (servers: Record<string, LSPServer.Info>, flags: RuntimeFlags.Info) => {
  if (flags.experimentalLspTy) {
    if (servers["pyright"]) {
      log.info("LSP server pyright is disabled because CYPHER_EXPERIMENTAL_LSP_TY is enabled")
    ...
    const getClients = Effect.fnUntraced(function* (file: string) {
      ...
      return yield* Effect.promise(async () => {
        ...
        for (const server of Object.values(s.servers)) {
          ...
          // cypher_change start - use lightweight tsgo-based client when persistent LSP is not enabled
