...
import { TsCheck } from "../cypher/ts-check" // cypher_change
...
// cypher_change start - tsgo native LSP or lightweight diagnostic client
// When CYPHER_EXPERIMENTAL_LSP_TOOL is enabled, spawn tsgo --lsp --stdio as a
...
export const Typescript: Info = {
  ...
  async spawn(root) {
    if (!Flag.CYPHER_EXPERIMENTAL_LSP_TOOL) return undefined
...
export const Clangd: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    // cypher_change start - reject release metadata before it becomes an executable path
    ...
    // cypher_change start - do not use remote metadata as a local path
  ...
  async spawn(root, _ctx, flags) {
    ...
    return {
      process: spawn(
        java,
        [
          "-Djava.import.generatesMetadataFilesAtProjectRoot=false", // cypher_change
          "-jar",
          launcherJar,
          "-configuration",
          configFile,
...
export const LuaLS: Info = {
  ...
  async spawn(root, _ctx, flags) {
    ...
    if (!bin) {
      ...
      // cypher_change start - use a fixed local archive name
