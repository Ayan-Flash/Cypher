...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed.` // cypher_change
  }
  ...
  // ProviderModelNotFoundError: { providerID: string, modelID: string, suggestions?: string[], modelsEmpty?: boolean } // cypher_change
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      ...(providerModelNotFound.modelsEmpty === true ? ["No models are currently available."] : []), // cypher_change
      `Try: \`cypher models\` to list available models`, // cypher_change
      `Or check your cypher.json provider/model names`, // cypher_change
    ].join("\n")
  }
