...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    // cypher_change start - install local themes into supported Cypher config directories
    const base = path.basename(source_dir)
    const local_dir =
      base === ".cypher" || base === ".cypher"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".cypher", "themes")
    // cypher_change end
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".cypher", "tui.json"), // cypher_change
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved; dispose?: () => void; disposeTimeoutMs?: number }) {
  ...
  try {
    ...
    const records = Flag.CYPHER_PURE ? [] : (config.plugin_origins ?? [])
    if (Flag.CYPHER_PURE && config.plugin_origins?.length) {
