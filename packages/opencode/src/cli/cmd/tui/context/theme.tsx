...
import cypher from "./theme/cypher.json" with { type: "json" } // cypher_change
import cypher1 from "./theme/cypher-v1.json" with { type: "json" } // cypher_change
...
import colorblind from "./theme/colorblind.json" with { type: "json" } // cypher_change
...
import type { TuiThemeCurrent } from "@cypher/plugin/tui"
...
export const DEFAULT_THEMES: Record<string, ThemeJson> = {
  ...
  gruvbox,
  kanagawa,
  cypher, // cypher_change
  ["cypher-v1"]: cypher1, // cypher_change
  material,
  matrix,
  mercury,
  monokai,
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "cypher") // cypher_change
        draft.active = typeof active === "string" ? active : "cypher" // cypher_change
        draft.ready = false
      }),
    )
    ...
    function init() {
      void Promise.allSettled([
        ...
        getCustomThemes()
          ...
          .catch(() => {
            setStore("active", "cypher") // cypher_change
          }),
      ]).finally(() => {
        setStore("ready", true)
      })
    ...
    // cypher_change start - safe fallback to cypher import if store lookup fails
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.cypher, store.mode) // cypher_change
    })
    // cypher_change end
    ...
    // cypher_change - use empty object as proxy target; all reads go through the getter
    return {
...
async function getCustomThemes() {
  const directories = [
    ...
    ...(await Array.fromAsync(
      Filesystem.up({
        targets: [".cypher", ".cypher"], // cypher_change
        start: process.cwd(),
      }),
    )),
  ]
  ...
  for (const dir of directories) {
    for (const item of await Glob.scan("themes/*.json", {
      ...
      // cypher_change start - validate custom theme JSON and protect built-in keys
