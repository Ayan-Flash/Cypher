import type {
  ...
  PermissionRequest,
  QuestionRequest,
  SuggestionRequest, // cypher_change
  SessionNetworkWait, // cypher_change
  LspStatus,
  McpStatus,
  McpResource,
  FormatterStatus,
  ...
  ProviderAuthMethod,
  VcsInfo,
  BackgroundProcessInfo, // cypher_change
  InteractiveTerminalSnapshot, // cypher_change
} from "@cypher/sdk/v2"
...
import { batch, createEffect, on, onMount } from "solid-js" // cypher_change - add createEffect/on for workspace re-bootstrap
import { handleSuggestionEvent } from "@/cypher/suggestion/tui/sync" // cypher_change
import { appendTerminalOutput } from "@/cypher/interactive-terminal/output" // cypher_change
import { useToast } from "@tui/ui/toast" // cypher_change
import * as Log from "@opencode-ai/core/util/log"
import { emptyConsoleState, type ConsoleState } from "@/config/console-state"
import type { IndexingStatus } from "@cypher/cypher-indexing/status" // cypher_change
...
export const { use: useSync, provider: SyncProvider } = createSimpleContext({
  ...
  init: () => {
    const [store, setStore] = createStore<{
      ...
      // cypher_change end
      config: Config
      globalConfig: Config // cypher_change
      session: Session[]
      session_status: {
        [sessionID: string]: SessionStatus
      }
      session_diff: {
        [sessionID: string]: Omit<Snapshot.FileDiff, "before" | "after">[] // cypher_change
      }
      todo: {
      ...
      indexing: IndexingStatus // cypher_change
    }>({
      provider_next: {
        all: [],
        default: {},
        connected: [],
        failed: [], // cypher_change
      },
      console_state: emptyConsoleState,
      provider_auth: {},
      config: {},
      globalConfig: {}, // cypher_change
      status: "loading",
      agent: [],
      permission: {},
      question: {},
      ...
      session_diff: {},
      todo: {},
      background_process: {}, // cypher_change
      interactive_terminal: {}, // cypher_change
      message: {},
      part: {},
      lsp: [],
      mcp: {},
      ...
      formatter: [],
      vcs: undefined,
      indexing: { state: "Disabled", message: "Indexing disabled.", processedFiles: 0, totalFiles: 0, percent: 0 }, // cypher_change
    })

    const event = useEvent()
    const project = useProject()
    const sdk = useSDK()
    const toast = useToast() // cypher_change
    const kv = useKV()
    ...
    function evict(sessionID: string) {
      ...
      setStore(
        produce((draft) => {
          ...
          delete draft.interactive_terminal[sessionID] // cypher_change
    ...
    const deleted = new Set<string>() // cypher_change
    const terminalDeleted = new Set<string>() // cypher_change
    let syncedWorkspace = project.workspace.current()
    let vcsVersion = 0 // cypher_change
      ...
      switch (event.type) {
        ...
        case "session.network.restored": {
          ...
          if (match.found) {
            ...
            setStore("network", event.properties.sessionID, match.index, "time", "restored", event.properties.time) // cypher_change
          }
          break
        }
    ...
    async function bootstrap(input: { fatal?: boolean } = {}) {
      ...
      const version = vcsVersion // cypher_change
      ...
      const globalConfigPromise = sdk.client.global.config.get({ throwOnError: true }) // cypher_change
      const blockingRequests: { name: string; promise: Promise<unknown> }[] = [
        ...
        { name: "global.config", promise: globalConfigPromise }, // cypher_change
      ...
      await Promise.allSettled(blockingRequests.map((r) => r.promise))
        ...
        .then(async () => {
          ...
          const globalConfigResponse = globalConfigPromise.then((x) => x.data!) // cypher_change
          ...
          return Promise.all([
            ...
            agentsResponse,
            configResponse,
            globalConfigResponse, // cypher_change
            ...
            const globalConfig = responses[5] // cypher_change
            const sessions = responses[6] // cypher_change
            ...
            batch(() => {
              ...
              setStore("globalConfig", reconcile(globalConfig)) // cypher_change
        ...
        .then(() => {
          ...
          void Promise.all([
            ...
            // cypher_change end
            sdk.client.formatter.status({ workspace }).then((x) => setStore("formatter", reconcile(x.data!))), // cypher_change
            // cypher_change start
            ...
            // cypher_change start - show config warnings as persistent toast
            sdk.client.config
              .warnings()
              .then((x) => {
                const list = x.data ?? []
    ...
    // cypher_change start - re-bootstrap when workspace changes (Agent Manager)
    createEffect(
      on(
    ...
    const result = {
      ...
      get ready() {
        // return true // cypher_change - upstream #23037 left this debug path enabled; keep it commented so future merges do not restore eager ready state.
        if (process.env.CYPHER_FAST_BOOT) return true
          ...
          setStore(
            produce((draft) => {
              ...
              for (const message of messages.data ?? []) {
                infos.push(strip(message.info)) // cypher_change
