...
import { useProject } from "./project" // cypher_change
...
export const { use: useLocal, provider: LocalProvider } = createSimpleContext({
  ...
  init: () => {
    const sync = useSync()
    const sdk = useSDK()
    const project = useProject() // cypher_change
    ...
    const agent = iife(() => {
      ...
      return {
        ...
        current() {
          // cypher_change start - fall back to first agent when current is removed (e.g. org switch)
        ...
        move(direction: 1 | -1) {
          batch(() => {
            ...
            if (!value) return // cypher_change - guard against empty agent list during org switch
    ...
    const model = iife(() => {
      const [modelStore, setModelStore] = createStore<{
        ready: boolean
        // cypher_change start - persisted picks plus process-local overrides
        model: Record<
          string,
          | {
              providerID: string
      ...
      const state = {
        pending: false,
        writer: Promise.resolve() as Promise<unknown>, // cypher_change - serialize writes
      }

      // cypher_change start - keep configured-agent selections process-local
      ...
      function save() {
        ...
        state.pending = false
        // cypher_change start - serialize writes so a slow first write cannot overwrite a later one
        const data = {
      ...
      Filesystem.readJson(filePath)
        .then((x: any) => {
          ...
          if (typeof x.model === "object" && x.model !== null) setModelStore("model", x.model) // cypher_change
      ...
      const currentModel = createMemo(() => {
        ...
        if (!a) return fallbackModel() // cypher_change - guard against empty agent list
        // cypher_change start - configured models beat stale persisted picks
        return (
          getFirstValidModel(
      ...
      return {
        ...
        // cypher_change start - expose persisted per-agent pick separately from overrides
        saved(name: string) {
          return modelStore.model[name]
        },
        // cypher_change end
        // cypher_change start - resolve once all queued writes (atomic write+rename) have settled.
        ...
        cycle(direction: 1 | -1) {
          ...
          const a = agent.current()
          if (!a) return
          apply(a.name, val, !a.model) // cypher_change
          save() // cypher_change
        },
        cycleFavorite(direction: 1 | -1) {
          ...
          const a = agent.current()
          if (!a) return
          apply(a.name, next, !a.model) // cypher_change
        ...
        set(model: { providerID: string; modelID: string }, options?: { recent?: boolean }) {
          batch(() => {
            ...
            const a = agent.current()
            if (!a) return
            apply(a.name, model, !a.model) // cypher_change
            if (options?.recent) {
    ...
    createEffect(() => {
      // cypher_change start - configured models resolve directly without persistence
