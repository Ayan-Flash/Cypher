...
import { CyphercodeDefaultPlugins } from "@/cypher/config/default-plugins" // cypher_change
import type { DeepMutable } from "@opencode-ai/core/schema"
import type { TuiAttentionSoundName } from "@cypher/plugin/tui"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  // cypher_change start - trusted gates {env:}; fileScope confines untrusted {file:} reads
  const load = (
    text: string,
    ...
    // cypher_change end
    Effect.gen(function* () {
      // cypher_change start - only trusted tui config resolves {env:}; untrusted {file:} confined to fileScope
  ...
  // cypher_change start - trusted + fileScope threaded to load
  const loadFile = (filepath: string, trusted: boolean, fileScope?: ConfigVariable.FileScope): Effect.Effect<Info> =>
    ...
    Effect.gen(function* () {
      ...
      return yield* load(text, filepath, trusted, fileScope) // cypher_change
    })

  // cypher_change start - trusted + fileScope threaded to loadFile
  const mergeFile = (acc: Acc, file: string, trusted: boolean, fileScope?: ConfigVariable.FileScope) =>
    // cypher_change end
    Effect.gen(function* () {
      const data = yield* loadFile(file, trusted, fileScope) // cypher_change
  ...
  // cypher_change start - discover canonical and legacy Cypher config directories
  // Every config dir we may read from: global config, .cypher and legacy .cypher
  // folders between cwd and home, and CYPHER_CONFIG_DIR.
  // cypher_change end
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CYPHER_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  for (const file of ConfigPaths.fileInDirectory(Global.Path.config, "tui")) {
    yield* mergeFile(acc, file, true) // cypher_change - global config is trusted
  }

  // 2. Explicit CYPHER_TUI_CONFIG override, if set.
  if (Flag.CYPHER_TUI_CONFIG) {
    const configFile = Flag.CYPHER_TUI_CONFIG
    yield* mergeFile(acc, configFile, true) // cypher_change - explicit env-provided path is trusted
    log.debug("loaded custom tui config", { path: configFile })
  }

  // 3. Project tui files, applied root-first so the closest file wins.
  for (const file of projectFiles) {
    yield* mergeFile(acc, file, false, { root: ctx.directory, source: file }) // cypher_change - untrusted, {file:} confined to project
  }

  // cypher_change start - load tui.json from supported Cypher config directories
  // 4. `.cypher` and legacy `.cypher` directories (and CYPHER_CONFIG_DIR)
  ...
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".cypher") || dir.endsWith(".cypher") || dir === Flag.CYPHER_CONFIG_DIR,
  )
  // cypher_change end

  for (const dir of dirs) {
    // cypher_change start - trust global (home/CYPHER_CONFIG_DIR) dirs like config.ts; in-repo .cypher/.cypher stay untrusted
  ...
  const result: Resolved = {
    ...
    attention: {
      ...
      sound_pack: acc.result.attention?.sound_pack ?? "cypher.default", // cypher_change
  ...
  // cypher_change start - inject Cypher default plugins to keep TUI aligned with server config
