...
import { access, constants } from "fs/promises" // cypher_change
...
const TUI_SCHEMA_URL = "https://app.cypher.ai/tui.json" // cypher_change
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  // cypher_change start: use cypher directory everywhere
  const project = Flag.CYPHER_DISABLE_PROJECT_CONFIG
    ? []
    : await Filesystem.findUp(["cypher.json", "cypher.jsonc"], input.cwd, undefined, { rootFirst: true })
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "cypher")]
  // cypher_change end
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "cypher"))
  }
  if (Flag.CYPHER_CONFIG) files.push(Flag.CYPHER_CONFIG)
