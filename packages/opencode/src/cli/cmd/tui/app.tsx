...
import { NudgeProvider } from "@/cypher/cli/cmd/tui/context/nudge" // cypher_change
...
import { DialogHeadlessLink } from "@/cypher/cli/cmd/tui/component/dialog-headless-link" // cypher_change
...
import * as CypherApp from "@/cypher/cli/cmd/tui/app" // cypher_change
...
import { kitty, resetTerminalState } from "@/cypher/cli/cmd/tui/util/terminal" // cypher_change
import { hasDisplay } from "@/cypher/cli/cmd/tui/util/display" // cypher_change
...
export function tuiRendererConfig(_config: TuiConfig.Resolved): CliRendererConfig {
  const mouseEnabled = !Flag.CYPHER_DISABLE_MOUSE && (_config.mouse ?? true)
  const keyboard = kitty() // cypher_change
  ...
  return {
    ...
    gatherStats: false,
    exitOnCtrlC: false,
    ...(keyboard ? { useKittyKeyboard: {} } : {}), // cypher_change
    autoFocus: false,
...
export function tui(input: TuiInput): TuiHandle {
  ...
  process.on("exit", resetTerminalState) // cypher_change
...
function createTuiLifecycle(input: {
  ...
  const exit = createExit(async (reason, message) => {
    ...
    resetTerminalState() // cypher_change
    if (reason) {
  ...
  return {
    ...
    async fail(error) {
      ...
      win32FlushInputBuffer() // cypher_change
      resetTerminalState() // cypher_change
      completeExit()
      throw error
    },
...
function App(props: { onSnapshot?: () => Promise<string[]> }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.CYPHER_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  const [done, setDone] = createSignal<Record<string, true>>({}) // cypher_change
  const [pasteSummaryEnabled, setPasteSummaryEnabled] = createSignal(
    kv.get("paste_summary_enabled", !sync.data.config.experimental?.disable_paste_summary),
  )

  // cypher_change start
  CypherApp.useSessionEffects({ route, sdk, sync })
  CypherApp.useTuiConfigHotReload()
  // cypher_change end

  // Update terminal window title based on current route and session
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.CYPHER_DISABLE_TERMINAL_TITLE) return

    const titleDefault = CypherApp.APP_TITLE // cypher_change

    // cypher_change start
    const cypherTitle = CypherApp.getTerminalTitle({
      route,
      base: titleDefault,
      sync,
      done: untrack(done),
    ...
    if (cypherTitle) {
      const id = cypherTitle.id
      if (id && cypherTitle.active && untrack(() => done()[id]) !== true) {
        setDone((prev) => ({ ...prev, [id]: true }))
      }
      renderer.setTerminalTitle(cypherTitle.title)
      return
    }
    // cypher_change end
  })
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.CYPHER_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        run: () => {
          // cypher_change start
          if (!hasDisplay()) {
            DialogHeadlessLink.show(dialog, CypherApp.DOCS_URL)
            return
          }
          open(CypherApp.DOCS_URL).catch(() => {})
          dialog.clear()
          // cypher_change end
        },
        category: "System",
  ...
  event.on("session.error", (evt) => {
    ...
    if (CypherApp.handleSessionError(error, toast)) return // cypher_change
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to ${CypherApp.APP_NAME} v${result.data.version}. Please restart the application.`, // cypher_change
    )
  ...
  return (
    <box
      ...
      onMouseUp={Flag.CYPHER_EXPERIMENTAL_DISABLE_COPY_ON_SELECT ? undefined : () => Selection.copy(renderer, toast)}
    >
      <Show when={Flag.CYPHER_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
        <box flexGrow={1} minHeight={0} flexDirection="column">
          <Switch>
            ...
            {/* cypher_change start */}
            <Match when={route.data.type === "cypherclaw"}>
              <CypherApp.CypherClawView />
            </Match>
            {/* cypher_change end */}
          </Switch>
          {plugin()}
