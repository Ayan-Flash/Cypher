...
export const CYPHER_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CYPHER_MODE_KEY = "opencode.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CYPHER_MODE_KEY, CYPHER_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CYPHER_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CYPHER_MODE_KEY, stack.at(-1)?.mode ?? CYPHER_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CYPHER_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CYPHER_MODE_KEY, undefined)
...
export function getOpencodeModeStack(keymap: OpenTuiKeymap) {
  ...
  if (!value) throw new Error("Cypher mode stack is not registered for this keymap") // cypher_change
  return value
}
