...
import type { TuiAttentionSoundName, TuiPlugin, TuiPluginApi } from "@cypher/plugin/tui"
...
const tui: TuiPlugin = async (api) => {
  ...
  // cypher_change start - only completed turns need completion attention
  ...
  api.event.on("session.error", (event) => {
    ...
    if (event.properties.error?.name === "MessageAbortedError") return // cypher_change - manual stops do not need attention
