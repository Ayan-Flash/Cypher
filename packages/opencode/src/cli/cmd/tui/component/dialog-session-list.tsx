...
export function DialogSessionList() {
  ...
  const [global, setGlobal] = createSignal(false) // cypher_change - show current worktree by default
  const deleteHint = useCommandShortcut("session.delete")
  const quickSwitch1 = useCommandShortcut("session.quick_switch.1")
  const quickSwitch9 = useCommandShortcut("session.quick_switch.9")

  // cypher_change start - always fetch from experimental endpoint (returns GlobalSession with worktree info)
  ...
  const [searchResults, searchActions] = createResource(
    () => ({ query: search(), global: global(), directory: project.instance.directory() }), // cypher_change
    async (input) => {
  ...
  const sessions = createMemo(() => searchResults() ?? []) // cypher_change - endpoint applies worktree scope
    ...
    dialog.replace(() => (
      <DialogSessionDeleteFailed
        ...
        onDelete={async () => {
          ...
          if (search()) await searchActions.refetch() // cypher_change - use createResource actions
  ...
  // cypher_change - support local and global sessions
  ...
  const options = createMemo(() => {
    ...
    const all = global() // cypher_change
    ...
    const displayOrder = orderByRecency(sessions()) // cypher_change - respect current scope
    ...
    function buildOption(id: string, category: string) {
      ...
      if (Flag.CYPHER_EXPERIMENTAL_WORKSPACES) {
        if (x.workspaceID) {
          footer = workspace ? (
            <WorkspaceLabel
              type={workspace.type}
      ...
      return {
        ...
        description: all && x.worktreeName ? `(${x.worktreeName})` : undefined, // cypher_change - worktree label
  ...
  return (
    <DialogSelect
      title={global() ? "Sessions (all worktrees)" : "Sessions (current worktree)"} // cypher_change
      options={options()}
      skipFilter={true}
        ...
        },
        {
          ...
          onTrigger: async (option) => {
            if (toDelete() === option.value) {
              ...
              void searchActions.refetch() // cypher_change
              setToDelete(undefined)
              return
            }
      ...
      // cypher_change start - preserve Ctrl+A worktree scope toggle with the upstream keymap engine
