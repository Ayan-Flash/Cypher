...
import { tui } from "../../../../cypher/cli/logo"
...
export function CypherLogo() {
  ...
  const teal = RGBA.fromHex("#14B8A6")
  const logo = tui()

  const renderLine = (line: string): JSX.Element[] => {
    const shadow = tint(theme.background, teal, 0.25)
    ...
    while (i < line.length) {
      ...
      switch (marker) {
        case "_":
          elements.push(
            <text fg={teal} bg={shadow} selectable={false}>
              {" "}
            </text>,
          )
          break
