...
import { useNudge } from "@/cypher/cli/cmd/tui/context/nudge" // cypher_change
...
import type { AssistantMessage, FilePart, UserMessage } from "@cypher/sdk/v2"
...
// cypher_change start
import { CypherSessionTuiSync } from "@/cypher/session/tui-sync"
import { slashMatches } from "@/cypher/cli/cmd/command-display"
import * as AgentRequirements from "@/cypher/cli/agent-requirements"
import { createCostAlertController } from "@/cypher/cli/cmd/tui/cost-alert"
import { MemoryPrompt } from "@/cypher/cli/cmd/tui/component/memory-prompt"
// cypher_change end
import { Flag } from "@opencode-ai/core/flag/flag"
import { type WorkspaceStatus } from "../workspace-label"
import { CYPHER_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, useOpencodeKeymap } from "../../keymap"
import { useTuiConfig } from "../../context/tui-config"
// cypher_change start - vim modal editing for the prompt
import { useVim, VimModeIndicator, vimToggleCommand } from "@/cypher/cli/cmd/tui/component/prompt"
// cypher_change end

export type PromptProps = {
  sessionID?: string
  directory?: string // cypher_change
  visible?: boolean
  disabled?: boolean
  onSubmit?: () => void
...
export function Prompt(props: PromptProps) {
  ...
  const nudge = useNudge() // cypher_change
  ...
  const [store, setStore] = createStore<{
    ...
    interrupt: number
    exitPress: number // cypher_change - track double ctrl+c to exit
    placeholder: number
  }>({
  ...
  // cypher_change start - sync local agent/model whenever newest user message changes
  ...
  createEffect(() => {
    ...
    if (!parts) return
    if (!CypherSessionTuiSync.model({ role: msg.role, parts })) return
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      // cypher_change start - vim modal editing toggle (palette + /vim)
      vimToggleCommand({
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.CYPHER_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
  ...
  useBindings(() => ({
    ...
    bindings: tuiConfig.keybinds.gather("prompt.palette", [
      ...
      "prompt.stash.pop",
      "prompt.stash.list",
      "prompt.vim.toggle", // cypher_change
      "session.interrupt",
      "workspace.set",
    ]),
  }))
  ...
  const ref: PromptRef = {
    ...
    reset() {
      ...
      vim.resetVim() // cypher_change - return to insert mode after the prompt is cleared
    },
    submit() {
      void submit()
    },
  ...
  // cypher_change start - close autocomplete while blocking overlays hide the prompt
  createEffect(() => {
  ...
  // cypher_change start - require a double Ctrl+C to exit from an empty focused prompt
  useBindings(() => ({
    target: inputTarget,
    enabled: inputTarget() !== undefined && !props.disabled && store.prompt.input === "",
    bindings: [
      ...
      commands: [
        {
          ...
          run() {
            ...
            vim.resetVim() // cypher_change - recalled history starts in insert mode
  ...
  async function submitInner() {
    ...
    // cypher_change start - in-memory cost alert command
    ...
    // cypher_change start - gate TUI sends on declared agent requirements
    ...
    if (store.mode === "shell") {
      void sdk.client.session.shell({
        sessionID,
        agent: local.agent.current()?.name ?? "", // cypher_change
        model: {
          providerID: selectedModel.providerID,
      ...
      iife(() => {
        ...
        return sync.data.command.some((x) => slashMatches(x, command)) // cypher_change
      })
    ) {
    ...
    toast.dismiss() // cypher_change - dismiss persistent config warning on first submit
    history.append({
      ...store.prompt,
      mode: currentMode,
    })
    ...
    input.clear()
    vim.resetVim() // cypher_change - drop back to insert mode after sending
    return true
  }
  const exit = useExit()
  ...
  async function pasteInputText(text: string) {
    ...
    if (
      (lineCount >= 5 || pastedContent.length > 800) && // cypher_change #7252 delay paste summary
  ...
  function clearPrompt() {
    ...
    setStore("extmarkToPartIndex", new Map())
    vim.resetVim() // cypher_change - don't leak stale vim mode/selection into an emptied prompt
  }

  // cypher_change start - cost-alert logic lives under cypher/; only prompt mutation stays here
  ...
  const highlight = createMemo(() => {
    ...
    return local.agent.color(agent.name ?? "") // cypher_change
  })
  ...
  const spinnerDef = createMemo(() => {
    const agent =
      status().type !== "idle"
        ...
        : local.agent.current()
    const color = agent ? local.agent.color(agent.name ?? "") : theme.border // cypher_change
    return {
      frames: createFrames({
        color,
        style: "blocks",
        ...
        >
          ...
          >
            <textarea
              ...
              /* cypher_change */ onCursorChange={() => {
                setCursorVersion((value) => value + 1)
                if (store.mode === "normal") auto()?.onCursorChange()
              }}
              /* cypher_change - KeyEvent type for vim key routing */ onKeyDown={(e: KeyEvent) => {
                if (props.disabled) {
                  e.preventDefault()
                  return
                }
                // cypher_change start - route keys through the vim layer when enabled
                if (vim.vimOnKey(e)) {
                  e.preventDefault()
                  e.stopPropagation()
                  return
                ...
                <Show when={local.agent.current()} fallback={<box height={1} />}>
                  {(agent) => (
                    <>
                      ...
                      {/* cypher_change start - vim mode indicator */}
                      <VimModeIndicator
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
          ...
          <Show when={status().type !== "retry"}>
            <box gap={2} flexDirection="row">
              {/* cypher_change start - show "ctrl+c again to exit" hint */}
