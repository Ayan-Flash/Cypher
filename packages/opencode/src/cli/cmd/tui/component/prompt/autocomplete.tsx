...
import { slashDisplay } from "@/cypher/cli/cmd/command-display" // cypher_change
...
export type AutocompleteRef = {
  ...
  // cypher_change start - validate cursor moves and close overlays without mutating draft text
  onCursorChange: () => void
  dismiss: () => void
  // cypher_change end
  visible: false | "@" | "/"
...
export function Autocomplete(props: {
  ...
  const commands = createMemo((): AutocompleteOption[] => {
    ...
    for (const serverCommand of sync.data.command) {
      // cypher_change start - preserve suffixes like :skill when inserting selected slash commands
  ...
  useBindings(() => ({
    ...
    bindings: [
      ...
      // cypher_change start - close stale suggestions while allowing normal cursor movement
      {
        key: "right",
        fallthrough: true,
        cmd: () => {
  ...
  // cypher_change start - keep slash text intact when overlays hide the prompt,
  ...
  onMount(() => {
    ...
    props.ref({
      ...
      // cypher_change start - dismiss stale popup after cursor leaves active filter region
      onCursorChange() {
        if (!store.visible) return
