...
// cypher_change start
import type { BackgroundProcessTool } from "@/cypher/tool/background-process"
import type { InteractiveTerminalTool } from "@/cypher/tool/interactive-terminal"
import type { SemanticSearchTool } from "@/cypher/tool/semantic-search"
// cypher_change end
...
import { CypherErrorBlock } from "@/cypher/components/cypher-error-display" // cypher_change
...
// cypher_change start
import { Suggest } from "@/cypher/suggestion/tui/render"
import { SuggestPrompt } from "@/cypher/suggestion/tui/prompt"
...
// cypher_change start
import { splitDiffHunks } from "@/cypher/tui/diff"
import { session as banner } from "@/cypher/cli/logo"
import { RoutedModelMeta } from "@/cypher/cli/cmd/tui/routes/session/routed-model-meta"

import { formatMarkdownTables } from "../../util/markdown"
import { submitFeedback } from "@/cypher/cli/cmd/tui/feedback"
import { MemoryMessageMeta, MemorySessionTui } from "@/cypher/cli/cmd/tui/routes/session/memory" // cypher_change
// cypher_change end
...
import { CYPHER_BASE_MODE, useBindings, useCommandShortcut, useOpencodeKeymap } from "../../keymap"
...
const sessionBindingCommands = [
  ...
  "messages.copy",
  // cypher_change start - message feedback
...
export function Session() {
  ...
  // cypher_change start - background processes are scoped to the visible session
  ...
  onCleanup(MemorySessionTui.attach({ event, toast, sessionID: route.sessionID })) // cypher_change
  ...
  const sessionCommandList = createMemo(() => [
    ...
    },
    {
      ...
      value: "messages.copy",
      category: "Session",
      // cypher_change start - /copy copies the latest assistant response
      slash: {
        name: "copy",
      },
      // cypher_change end
    ...
    // cypher_change end
    {
      ...
      category: "Session",
      slash: {
        name: "copy-session", // cypher_change - transcript copy moved off /copy
      },
      run: async () => {
        try {
          const sessionData = session()
          if (!sessionData) return
          // cypher_change start - fetch all messages from server instead of truncated sync store
        ...
        <box flexDirection="row" flexGrow={1} minHeight={0}>
          <box flexGrow={1} minHeight={0} paddingBottom={1} paddingLeft={2} paddingRight={2} gap={1}>
            <Show when={session()}>
              ...
              <box flexShrink={0}>
                {/* cypher_change start - the terminal owns the input area while active */}
...
function AssistantMessage(props: { message: AssistantMessage; parts: Part[]; last: boolean }) {
  ...
  const routed = createMemo(() => RoutedModelMeta.info(ctx.providers(), props.parts, ctx.showDetails(), props.message)) // cypher_change
  const route = createMemo(() => routed().footer) // cypher_change
  ...
  return (
    <>
      {/* cypher_change start - provide compact routed-model metadata to part renderers */}
        ...
        <Match when={props.last || final() || props.message.error?.name === "MessageAbortedError"}>
          <box paddingLeft={3}>
            <text marginTop={1}>
              ...
              {/* cypher_change start - show routed model in regular assistant footer */}
              <Show when={route()}>
              ...
              <MemoryMessageMeta parts={props.parts} color={theme.textMuted} /> {/* cypher_change */}
...
// cypher_change start - register rendered step-finish parts
const PART_MAPPING = {
  text: TextPart,
  tool: ToolPart,
...
const INLINE_TOOL_ICON_WIDTH = 2 // cypher_change

// cypher_change start - show concrete routed models reported by gateway/provider responses
function StepFinishPart(props: { last: boolean; part: StepFinishPart; message: AssistantMessage }) {
  ...
  const routed = createMemo(() => {
    if (props.message.providerID !== "cypher") return undefined
    if (!props.message.modelID.startsWith("cypher-auto/")) return undefined
...
function TextPart(props: { last: boolean; part: TextPart; message: AssistantMessage }) {
  ...
  // cypher_change start - format markdown tables with fixed-width columns
...
function Task(props: ToolProps<typeof TaskTool>) {
  ...
  const content = createMemo(() => {
    ...
    } else if (isRunning()) content.push(`↳ Starting...`) // cypher_change
...
function Edit(props: ToolProps<typeof EditTool>) {
  ...
  const hunks = createMemo(() => splitDiffHunks(diffContent() ?? "")) // cypher_change
...
function Question(props: ToolProps<typeof QuestionTool>) {
  ...
  // cypher_change start - show dismissed question content with toggle;
  ...
  return (
    <Switch>
      {/* cypher_change start - toggle between one-liner and full block */}
      <Match when={count() > 0}>
        <Show
          when={expanded()}
          fallback={
