...
import { normalizeUrls } from "@/cypher/util/url"
import { MemoryPermissionRegistry } from "@/cypher/cli/cmd/tui/routes/session/memory-permission"
// cypher_change end
import { CYPHER_BASE_MODE, useBindings, useCommandShortcut } from "../../keymap"
...
function EditBody(props: { request: PermissionRequest }) {
  ...
  const hunks = createMemo(() => splitDiffHunks(diff())) // cypher_change
        ...
        {(() => {
          const info = () => {
            ...
            if (permission === "webfetch") {
              const url = normalizeUrls(typeof data.url === "string" ? data.url : "") // cypher_change
              return {
                icon: "%",
                title: `WebFetch ${url}`,
                body: (
            ...
            const custom = MemoryPermissionRegistry.render(permission, props.request) // cypher_change
            if (custom) return custom // cypher_change
          ...
          const header = () => (
            <box flexDirection="column" gap={0}>
              ...
              {/* cypher_change start - explain config file edits always require approval */}
          ...
          // cypher_change start — hide "Always allow" for config file edits
  ...
  return (
    ...
    >
      <box gap={1} paddingLeft={1} paddingRight={3} paddingTop={1} paddingBottom={1}>
        ...
        <box paddingLeft={1}>
          <text fg={theme.textMuted}>Tell Cypher what to do differently</text>
        </box>
      </box>
      <box
