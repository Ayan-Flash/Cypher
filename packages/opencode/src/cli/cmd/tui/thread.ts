...
import type { GlobalEvent } from "@cypher/sdk/v2"
import type { EventSource } from "./context/sdk"
import { win32DisableProcessedInput, win32InstallCtrlCGuard } from "./win32"
import { importCloudSession, localSessionID, validateCloudFork } from "@/cypher/cloud-session" // cypher_change
import { createCypherClient } from "@cypher/sdk/v2" // cypher_change
import { writeHeapSnapshot } from "v8"
import { TuiConfig } from "./config/tui"
import { CypherTuiThreadDaemon, type StartInput } from "@/cypher/cli/cmd/tui/thread" // cypher_change
import {
  CYPHER_PROCESS_ROLE,
  CYPHER_RUN_ID,
  ensureRunID,
...
declare global {
  const CYPHER_WORKER_PATH: string
}

type RpcClient = ReturnType<typeof Rpc.client<typeof rpc>>

// cypher_change start - lazy-load the TUI app after daemon attach in source mode
...
async function target() {
  if (typeof CYPHER_WORKER_PATH !== "undefined") return CYPHER_WORKER_PATH
...
export function resolveThreadDirectory(project?: string, envPWD = process.env.PWD, cwd = process.cwd()) {
  // cypher_change start - ignore stale PWD from wrappers such as `bun --cwd`, except cypher-dev's caller cwd
  const dev = process.env.CYPHER_DEV_CWD
  ...
  return dev ? root : real // cypher_change
}

export const TuiThreadCommand = cmd({
  command: "$0 [project]",
  describe: "start cypher tui", // cypher_change
  builder: (yargs) =>
    withNetworkOptions(yargs)
      .positional("project", {
        type: "string",
        describe: "path to start cypher in", // cypher_change
      })
      .option("model", {
        type: "string",
        alias: ["m"],
  ...
  handler: async (args) => {
    ...
    try {
      ...
      // cypher_change start - default TUI sessions attach to the daemon unless explicitly disabled
      if (await CypherTuiThreadDaemon.attach({ args, cwd, input: () => input(args.prompt), start })) return
      // cypher_change end
      const auth = CypherTuiThreadDaemon.workerAuth() // cypher_change - protect TUI-owned HTTP routes from unauthenticated local callers
      const env = sanitizedProcessEnv({
        [CYPHER_PROCESS_ROLE]: "worker",
        [CYPHER_RUN_ID]: ensureRunID(),
        ...auth.env, // cypher_change
        CYPHER_BACKGROUND_PROCESS_PORTS: "true", // cypher_change - TUI surfaces inferred background process ports
      })
      ...
      // cypher_change start - graceful shutdown on external signals
      ...
      const transport = external
        ? {
            ...
            fetch: undefined,
            headers: auth.headers, // cypher_change
            events: undefined,
          }
        : {
            url: "http://cypher.internal",
      ...
      try {
        ...
        try {
          await validateSession({
            url: transport.url, // cypher_change
            sessionID: localSessionID(args), // cypher_change
            directory: cwd,
            fetch: transport.fetch,
            headers: transport.headers, // cypher_change
          })
        ...
        // cypher_change start
        await start({
          // cypher_change - shared lazy loader also supports daemon attach
          url: transport.url,
          async onSnapshot() {
