import type { Session as SDKSession, Message, Part } from "@cypher/sdk/v2"
...
const log = Log.create({ service: "import" }) // cypher_change
...
// cypher_change start
/** Extract share ID from a Cypher share URL like https://app.cypher.ai/s/abc123 */
export function parseShareUrl(url: string): string | null {
  const match = url.match(/^https?:\/\/app\.cypher\.ai\/s\/([a-zA-Z0-9_-]+)$/)
...
async function ingestBootstrap(sessionId: string) {
  ...
  return CypherSessions.bootstrap(sessionId)
}
...
const runImport = Effect.fn("Cli.import.body")(function* (file: string, ctx: InstanceContext) {
  ...
  if (isUrl) {
    // cypher_change start - Migrate to upstream ShareNext architecture #10281
    const slug = parseShareUrl(file)
    if (!slug) {
      process.stdout.write(`Invalid URL format. Expected: https://app.cypher.ai/s/<id>`)
