...
import { CyphercodeMcpConfig } from "@/cypher/cli/cmd/mcp" // cypher_change
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: cypher mcp add") // cypher_change
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in cypher.json:") // cypher_change
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // cypher_change start - prefer supported Cypher config directories over root files
  const roots = [
    path.join(baseDir, "cypher.jsonc"),
    path.join(baseDir, "cypher.json"),
  ...
  const candidates = global
    ? roots
    : [
        path.join(baseDir, ".cypher", "cypher.jsonc"),
        path.join(baseDir, ".cypher", "cypher.json"),
        path.join(baseDir, ".cypher", "opencode.jsonc"),
        path.join(baseDir, ".cypher", "opencode.json"),
        path.join(baseDir, ".cypher", "cypher.jsonc"),
        path.join(baseDir, ".cypher", "cypher.json"),
        path.join(baseDir, ".cypher", "opencode.jsonc"),
        path.join(baseDir, ".cypher", "opencode.json"),
  ...
  // Default to cypher.json if none exist
  return path.join(baseDir, "cypher.json")
  // cypher_change end
}
...
async function addMcpToConfig(name: string, mcpConfig: ConfigMCP.Info, configPath: string) {
  ...
  const result = CyphercodeMcpConfig.format(configPath, applyEdits(text, edits)) // cypher_change
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., cypher x @modelcontextprotocol/server-filesystem", // cypher_change
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "cypher-debug", version: InstallationVersion }, // cypher_change
            },
            id: 1,
          }),
        })
        ...
        if (response.status === 401) {
          ...
          try {
            const client = new Client({
              name: "cypher-debug", // cypher_change
