...
import type { Hooks } from "@cypher/plugin"
...
export const ProvidersCommand = cmd({
  // cypher_change start - keep "auth" as primary command name
  command: "auth",
  aliases: ["providers"],
  // cypher_change end
...
export const ProvidersLoginCommand = effectCmd({
  ...
  builder: (yargs) =>
    yargs
      .positional("url", {
        describe: "cypher auth provider", // cypher_change
        type: "string",
      })
      .option("provider", {
        alias: ["p"],
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    const options = [
      ...pipe(
        ...
        map((x) => ({
          ...
          value: x.id,
          hint: {
            cypher: "recommended", // cypher_change
            openai: "ChatGPT login or API key", // cypher_change
          }[x.id],
        })),
      ),
    ...
    if (args.provider) {
      ...
      // cypher_change start - accept codex as an alias for OpenAI ChatGPT auth
    ...
    if (provider === "other") {
      ...
      yield* Prompt.log.warn(
        `This only stores a credential for ${provider} - you will need configure it in cypher.json, check the docs for examples.`, // cypher_change
      )
    }
    ...
    if (provider === "amazon-bedrock") {
      yield* Prompt.log.info(
        "Amazon Bedrock authentication priority:\n" +
          ...
          "Configure via cypher.json options (profile, region, endpoint) or\n" + // cypher_change
    ...
    if (["cloudflare", "cloudflare-ai-gateway"].includes(provider)) {
      yield* Prompt.log.info(
        "Cloudflare AI Gateway can be configured with CLOUDFLARE_GATEWAY_ID, CLOUDFLARE_ACCOUNT_ID, and CLOUDFLARE_API_TOKEN environment variables. Read more: https://cypher.ai/docs/ai-providers/cloudflare", // cypher_change
      )
    }
