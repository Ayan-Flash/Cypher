...
//
// Enable with CYPHER_DIRECT_TRACE=1. Writes one JSON line per event to
// ~/.local/share/cypher/log/direct/<timestamp>-<pid>.jsonl. Also writes // cypher_change
...
export function trace(): Trace | undefined {
  ...
  if (!process.env.CYPHER_DIRECT_TRACE) {
    state = false
    return undefined
  }
