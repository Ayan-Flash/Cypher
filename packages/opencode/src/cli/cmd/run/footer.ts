...
import { RUN_INTERACTIVE_TERMINAL_ROWS } from "@/cypher/cli/cmd/run/interactive-terminal" // cypher_change
...
type RunFooterOptions = {
  ...
  onTerminalWrite: (input: { terminalID: string; data: string }) => Promise<void> // cypher_change
  onTerminalResize: (input: { terminalID: string; cols: number; rows: number }) => Promise<void> // cypher_change
  onTerminalClose: (terminalID: string) => Promise<void> // cypher_change
