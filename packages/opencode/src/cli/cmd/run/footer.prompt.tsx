...
import { slashDisplay, slashMatches } from "@/cypher/cli/cmd/command-display" // cypher_change
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  const slashOptions = createMemo<SlashOption[]>(() => {
    ...
    return [
      ...(input.commands() ?? [])
        .filter((item) => !hidden.has(item.name)) // cypher_change - suggest skills as slash commands
        .map(
          (item) =>
            ({
              kind: "slash",
              name: item.name,
              display: slashDisplay(item), // cypher_change
              description: item.description,
  ...
  const select = (item?: PromptOption) => {
    ...
    if (next.kind === "slash") {
      const text = `${next.display} ` // cypher_change
      const cursor = area.cursorOffset
