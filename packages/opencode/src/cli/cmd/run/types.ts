...
import type { CypherClient, PermissionRequest, QuestionRequest, ToolPart } from "@cypher/sdk/v2"
import type { RunInteractiveTerminalSnapshot } from "@/cypher/cli/cmd/run/types" // cypher_change
...
type PromptModel = Parameters<CypherClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<CypherClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<CypherClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<CypherClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<CypherClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<CypherClient["experimental"]["resource"]["list"]>>["data"]>
...
export type FooterView =
  ...
  | { type: "interactive_terminal"; terminal: RunInteractiveTerminalSnapshot } // cypher_change
...
export type PermissionReply = Parameters<CypherClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<CypherClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<CypherClient["question"]["reject"]>[0]
