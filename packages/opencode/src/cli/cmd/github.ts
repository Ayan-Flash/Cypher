...
import { GitHubSecurity } from "@/cypher/security/github" // cypher_change
...
const AGENT_USERNAME = "cypherconnect[bot]" // cypher_change
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/cypher.yml" // cypher_change
    ...
    yield* Effect.promise(async () => {
      {
        ...
        function printNextSteps() {
          ...
          prompts.outro(
            [
              ...
              step2,
              "",
              "    3. Go to a GitHub issue and comment `/cypher summarize` to see the agent in action", // cypher_change
            ].join("\n"),
          )
        }
        ...
        async function installGitHubApp() {
          ...
          // Open browser
          const url = "https://github.com/apps/cypherconnect" // cypher_change
          const command =
          ...
          async function getInstallation() {
            // cypher_change start - updated to new endpoint
            return await fetch(`https://api.cypher.ai/api/integrations/github/check-installation?owner=${app.owner}`)
              .then((res) => res.json())
        ...
        async function addWorkflowFiles() {
          // cypher_change start - updated workflow template with Cypher branding and gateway secrets
          const providerEnvStr =
            provider === "amazon-bedrock"
              ? ""
              : providers[provider].env.map((e) => `\n          ${e}: \${{ secrets.${e} }}`).join("")

          const cypherGatewayEnv =
            provider === "cypher"
              ? `\n          CYPHER_API_KEY: \${{ secrets.CYPHER_API_KEY }}\n          CYPHER_ORG_ID: \${{ secrets.CYPHER_ORG_ID }}`
              : ""

          const envStr = providerEnvStr || cypherGatewayEnv ? `\n        env:${providerEnvStr}${cypherGatewayEnv}` : ""
...
jobs:
  cypher:
    if: |
      ...
      startsWith(github.event.comment.body, '/cypher')
    ...
    steps:
      ...
      - name: Run Cypher
        uses: Cypher-Org/cypher/github@latest${envStr}
        with:
...
export const GithubRunCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.github.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.cypher.ai" : "https://cypher.ai"
      ...
      try {
        ...
        // Setup cypher session // cypher_change
        ...
        console.log("cypher session", session.id) // cypher_change
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.cypher.ai"
      ...
      async function getUserPrompt() {
        ...
        const mentions = (process.env["MENTIONS"] || "/cypher,/kc") // cypher_change
          .split(",")
        ...
        for (const m of matches) {
          const tag = m[0]
          // cypher_change start - only fetch canonical GitHub attachment routes
      ...
      async function chat(message: string, files: PromptFiles = []) {
        console.log("Sending message to cypher...") // cypher_change
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("cypher-github-action") // cypher_change
        } catch (error) {
      ...
      async function exchangeForAppToken(token: string) {
        // cypher_change start - updated endpoint URLs per new API structure
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `cypher/${type}-${hex}-${timestamp}` // cypher_change
        }
        return `cypher/${type}${issueId}-${timestamp}` // cypher_change
      }
      ...
      function footer(opts?: { image?: boolean }) {
        // cypher_change start - simplified footer with text branding (no image backend yet)
        const share = shareId ? `[cypher session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
        return `\n\n---\n*Powered by [Cypher](https://cypher.ai)*&nbsp;&nbsp;|&nbsp;&nbsp;${share}[github run](${runUrl})`
        // cypher_change end
      }
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the cypher infrastructure after your response", // cypher_change
