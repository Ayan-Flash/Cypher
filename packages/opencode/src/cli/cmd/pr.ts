...
import { existsSync } from "node:fs" // cypher_change

const subcommand = "pr" // cypher_change

// cypher_change start - resolve the currently running CLI instead of hardcoding opencode
export function cliCommand(
  ...
  if (script === subcommand) return [input.execPath] // cypher_change
...
export const PrCommand = effectCmd({
  command: `${subcommand} <number>`, // cypher_change
  describe: "fetch and checkout a GitHub PR branch, then run cypher", // cypher_change
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    const cli = cliCommand() // cypher_change
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        const sessionMatch = prInfo.body.match(/https:\/\/app\.cypher\.ai\/s\/([a-zA-Z0-9_-]+)/) // cypher_change
        if (sessionMatch) {
    ...
    UI.println()
    UI.println("Starting cypher...") // cypher_change
    UI.println()

    const run = sessionId ? [...cli, "-s", sessionId] : cli // cypher_change
    ...
    if (code !== 0) return yield* Effect.die(new Error(`cypher exited with code ${code}`)) // cypher_change
  }),
})
