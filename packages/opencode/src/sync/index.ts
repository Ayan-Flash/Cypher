...
import * as EventWire from "@/cypher/event-wire" // cypher_change
...
> = {
  ...
  wire?: boolean // cypher_change - EventV2 rows cross persistence and bus boundaries as encoded data
}
...
// cypher_change start - serialized rows carry the schema's encoded representation
...
export const layer = Layer.effect(Service)(
  Effect.gen(function* () {
    ...
    const replay: Interface["replay"] = Effect.fn("SyncEvent.replay")(function* (event, options) {
      ...
      // cypher_change start - decode only EventV2 rows
...
function process<Def extends Definition>(
  ...
  Database.transaction((tx) => {
    ...
    const data = def.wire ? EventWire.encode(def.schema, event.data) : event.data // cypher_change
    ...
    if (options.experimentalWorkspaces) {
      ...
      tx.insert(EventTable)
        .values({
          ...
          data: data as Record<string, unknown>, // cypher_change
        })
        .run()
    }
    ...
    Database.effect(() => {
      ...
      const publish = (value: unknown) =>
        ...
        options.bridge.fork(
          Effect.gen(function* () {
            // cypher_change start - encode EventV2 properties before crossing the legacy boundary
            if (def.wire) {
