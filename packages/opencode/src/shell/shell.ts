...
import * as PowerShell from "@/cypher/shell/shell" // cypher_change - PowerShell args
...
export function gitbash() {
  ...
  if (Flag.CYPHER_GIT_BASH_PATH) return Flag.CYPHER_GIT_BASH_PATH
  const git = which("git")
  if (!git) return
...
export function args(file: string, command: string, cwd: string) {
  ...
  if (ps(file)) return PowerShell.args(command) // cypher_change - PowerShell args
  return ["-c", command]
}
