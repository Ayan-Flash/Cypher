...
import { disconnect } from "@/cypher/server/sse" // cypher_change
...
function eventResponse(request: HttpServerRequest.HttpServerRequest) {
  ...
  return HttpServerResponse.stream(
    Stream.make({ payload: { id: Bus.createID(), type: "server.connected", properties: {} } }).pipe(
      ...
      Stream.encodeText,
      // cypher_change start - prevent disconnected SSE clients from retaining full diff payloads
...
export const globalHandlers = HttpApiBuilder.group(RootHttpApi, "global", (handlers) =>
  Effect.gen(function* () {
    ...
    const event = Effect.fn("GlobalHttpApi.event")(function* () {
      const request = yield* HttpServerRequest.HttpServerRequest // cypher_change
      return eventResponse(request) // cypher_change
    })
