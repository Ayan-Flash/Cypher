...
import { EffectBridge } from "@/effect/bridge" // cypher_change
import { InstanceState } from "@/effect/instance-state"
import { MCP } from "@/mcp"
import { Project } from "@/project/project"
import { Provider } from "@/provider/provider" // cypher_change
import { ModelID } from "@/provider/schema" // cypher_change
import { Session } from "@/session/session"
import { ToolJsonSchema } from "@/tool/json-schema"
import { ToolRegistry } from "@/tool/registry"
import { Filesystem } from "@/util/filesystem" // cypher_change
import { Review } from "@/cypher/review/review" // cypher_change
import { WorktreeDiff } from "@/cypher/review/worktree-diff" // cypher_change
import { WorktreeFamily } from "@/cypher/worktree-family" // cypher_change
import { Worktree } from "@/worktree"
import { Effect, Option } from "effect"
import * as HttpServerResponse from "effect/unstable/http/HttpServerResponse"
import { HttpApiBuilder, HttpApiError } from "effect/unstable/httpapi"
import * as Log from "@opencode-ai/core/util/log" // cypher_change
import path from "path" // cypher_change
  ...
  Effect.gen(function* () {
    ...
    const tool = Effect.fn("ExperimentalHttpApi.tool")(function* (ctx: { query: typeof ToolListQuery.Type }) {
      ...
      const list = yield* registry.tools({
        ...
        modelID: model ? ModelID.make(model.api.id) : ctx.query.model, // cypher_change
        family: model?.family, // cypher_change
    ...
    // cypher_change start - discover Agent Manager and external git worktrees
    ...
    // cypher_change start - worktree diff endpoints for agent manager
    ...
    const session = Effect.fn("ExperimentalHttpApi.session")(function* (ctx: { query: typeof SessionListQuery.Type }) {
      ...
      // cypher_change end
      if (roots && directory && !current) return HttpServerResponse.jsonUnsafe([]) // cypher_change
      const sessions = Array.from(
        Session.listGlobal({
          projectID, // cypher_change
          directory: ctx.query.worktrees ? undefined : ctx.query.directory, // cypher_change
          directories: roots, // cypher_change
          currentDirectory: directory, // cypher_change
      ...
      // cypher_change start - resolve worktree folder name for each session
      const result = sorted
      ...
      return HttpServerResponse.jsonUnsafe(list, {
        headers:
          result.length > limit && list.length > 0 // cypher_change
