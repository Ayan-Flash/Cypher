import { Image } from "@/image/image" // cypher_change - classify user image validation defects
import { CypherSessionHttpApi } from "@/cypher/server/httpapi/session-fork" // cypher_change
import { BlockedError as AgentRequirementError } from "@/cypher/agent-requirements" // cypher_change
...
export const sessionHandlers = HttpApiBuilder.group(InstanceHttpApi, "session", (handlers) =>
  Effect.gen(function* () {
    ...
    const forkRaw = CypherSessionHttpApi.forkRaw(fork) // cypher_change - carry upstream bodyless full-session fork support
    ...
    const prompt = Effect.fn("SessionHttpApi.prompt")(function* (ctx: {
      ...
      const message = yield* promptSvc
        .prompt({ ...ctx.payload, sessionID: ctx.params.sessionID } as unknown as SessionPrompt.PromptInput) // cypher_change
        .pipe(
          // cypher_change start - reject only typed user image validation defects as request errors
    ...
    const promptAsync = Effect.fn("SessionHttpApi.promptAsync")(function* (ctx: {
      ...
      yield* requireSession(ctx.params.sessionID)
      // cypher_change start - cast to bridge schema-readonly to PromptInput mutable; matches legacy Hono session.ts
      yield* promptSvc
    ...
    const viewed = Effect.fn("SessionHttpApi.viewed")(function* (ctx: { payload: typeof ViewedPayload.Type }) {
      const { CypherSessions } = yield* Effect.promise(() => import("@/cypher-sessions/cypher-sessions"))
      CypherSessions.setViewedSessions({ focused: ctx.payload.focused ?? [], open: ctx.payload.open ?? [] })
      return true
    })
    // cypher_change end
    ...
    return handlers
      ...
      .handleRaw("fork", forkRaw) // cypher_change - carry upstream bodyless full-session fork support
      .handle("abort", abort)
      .handle("init", init)
      .handle("share", share)
      ...
      .handle("viewed", viewed) // cypher_change
  }),
)
