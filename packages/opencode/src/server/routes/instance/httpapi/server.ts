...
import { Git } from "@/git" // cypher_change
...
import { Storage } from "@/storage/storage" // cypher_change
...
// cypher_change start
import {
  provide as provideCypherHttpApiHandlers,
  provideListener as provideCypherListenerRoutes,
} from "@/cypher/server/httpapi/server"
// cypher_change end
...
const instanceApiRoutes = HttpApiBuilder.layer(InstanceHttpApi).pipe(
  ...
  provideCypherHttpApiHandlers, // cypher_change
)
...
export function createRoutes(
  ...
  return Layer.mergeAll(rootApiRoutes, eventApiRoutes, ptyConnectApiRoutes, instanceRoutes, docRoute, uiRoute).pipe(
    Layer.provide([
      ...
      Format.defaultLayer,
      Git.defaultLayer, // cypher_change
      LSP.defaultLayer,
      MemoryService.layer, // cypher_change
      Installation.defaultLayer,
      MCP.defaultLayer,
      ModelCache.defaultLayer, // cypher_change
      ...
      Notebook.defaultLayer, // cypher_change
      ...
      Storage.defaultLayer, // cypher_change
...
// cypher_change start - keep listener routes local while application services come from AppRuntime
export function createListenerRoutes(corsOptions?: CorsOptions) {
  return Layer.mergeAll(rootApiRoutes, eventApiRoutes, ptyConnectApiRoutes, instanceRoutes, docRoute, uiRoute).pipe(
    provideCypherListenerRoutes(corsOptions),
  )
}
// cypher_change end
