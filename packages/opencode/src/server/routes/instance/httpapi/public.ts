...
// cypher_change start
import { matchLegacyCypherOpenApi } from "@/cypher/server/httpapi/public"
import * as CypherServer from "@/cypher/server/server"
// cypher_change end
...
const QueryParameterSchemas: Record<string, OpenApiSchema> = {
  ...
  "GET /cypher/cloud-sessions cursor": { type: "string" },
  "GET /cypher/cloud-sessions limit": { type: "number" },
  // cypher_change end
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for the cypher serve command", // cypher_change
...
function matchLegacyOpenApi(input: Record<string, unknown>) {
  ...
  matchLegacyCypherOpenApi(input) // cypher_change
  return input
}
...
function applyLegacySchemaOverrides(spec: OpenApiSpec) {
  ...
  // cypher_change start - preserve model reset sentinels in indexing config patches
...
function normalizeParameter(param: OpenApiParameter, route: string) {
  ...
  if (param.in === "path") {
    param.schema = pathParameterSchema(route, param.name) ?? stripOptionalNull(param.schema) // cypher_change
    return
  }
  if (param.in === "query") {
...
export const PublicApi = OpenCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: CypherServer.DOC_TITLE, // cypher_change
    version: "1.0.0",
    description: CypherServer.DOC_DESCRIPTION, // cypher_change
