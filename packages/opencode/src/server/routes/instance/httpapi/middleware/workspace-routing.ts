...
import { forkTargetDirectory } from "@/cypher/server/routes/fork-routing" // cypher_change - fork honors explicit target directory
...
function configuredWorkspaceID(): WorkspaceID | undefined {
  return Flag.CYPHER_WORKSPACE_ID ? WorkspaceID.make(Flag.CYPHER_WORKSPACE_ID) : undefined
}
...
function defaultDirectory(request: HttpServerRequest.HttpServerRequest, url: URL): string {
  return url.searchParams.get("directory") || request.headers["x-cypher-directory"] || process.cwd()
}
...
function planRequest(
  ...
  return Effect.gen(function* () {
    ...
    // cypher_change start - a fork targeting an explicit directory (e.g. a worktree) must not inherit the source session's directory
