...
import { serverUrls } from "@/cypher/cli/server-urls" // cypher_change
...
import * as CypherListener from "@/cypher/server/listener" // cypher_change
...
export async function listen(opts: ListenOptions): Promise<Listener> {
  ...
  return {
    ...
    port: listener.port,
    url: listener.url,
    urls: listener.urls, // cypher_change
...
const listenEffect: (opts: ListenOptions) => Effect.Effect<EffectListener, unknown> = Effect.fn("Server.listen")(
  function* (opts: ListenOptions) {
    ...
    return {
      ...
      port: address.port,
      url: listenerUrl,
      urls: serverUrls(opts.hostname, address.port), // cypher_change
...
function listenerLayer(opts: ListenOptions, port: number) {
  return HttpRouter.serve(HttpApiApp.createListenerRoutes(opts), { // cypher_change
...
function startListener(opts: ListenOptions, port: number) {
  ...
  return CypherListener.build(listenerLayer(opts, port), scope).pipe( // cypher_change
