...
import { appendSessionDiffs, readSessionDiffBase } from "@/cypher/session-portability/cumulative-diff" // cypher_change
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const summarize = Effect.fn("SessionSummary.summarize")(function* (input: {
      ...
      // cypher_change start - preserve imported cumulative diffs when summarizing cloud-forked sessions
      ...
      const msgDiffs = base.length > 0 ? local : yield* computeDiff({ messages }) // cypher_change
    ...
    const diff = Effect.fn("SessionSummary.diff")(function* (input: { sessionID: SessionID; messageID?: MessageID }) {
      ...
      const next = diffs.map((item) => {
        ...
        // cypher_change start — scrub oversized diffs from stored session_diff
