...
import { SessionNetwork } from "./network" // cypher_change
import { CodexAuthExpiredError } from "@/cypher/provider/codex-refresh" // cypher_change
import { CypherSessionMessageOrder } from "@/cypher/session/message-order" // cypher_change
import * as TextStream from "@/cypher/text-stream" // cypher_change
...
function truncateToolOutput(text: string, maxChars?: number) {
  ...
  // cypher_change start - avoid persisting malformed Unicode in compacted tool output
...
// cypher_change start - shared editor context schema (used by MessageV2.User and SessionPrompt.PromptInput)
...
// cypher_change start - strip bloated metadata fields from stored parts to prevent multi-MB payloads
...
export function stripPartMetadata(part: Part): Part {
  // cypher_change - exported for testing
...
// cypher_change - apply stripping inside helpers so all read paths are covered
...
export const toModelMessagesEffect = Effect.fnUntraced(function* (
  ...
  for (const msg of input) {
    ...
    if (msg.info.role === "assistant") {
      ...
      for (const part of msg.parts) {
        // cypher_change - !part.ignored keeps local UI warnings out of future prompts
...
export function parts(message_id: MessageID) {
  ...
  return rows.map(
    (row) =>
      // cypher_change - apply stripping to parts fetched individually as well to cover all read paths
      stripPartMetadata({
        ...row.data,
        id: row.id,
        sessionID: row.session_id,
...
export function filterCompacted(msgs: Iterable<WithParts>) {
  ...
  result.reverse()
  CypherSessionMessageOrder.annotate(result) // cypher_change - preserve chronology before retained-tail projection
...
export function fromError(
  ...
  switch (true) {
    ...
    case e instanceof CodexAuthExpiredError: // cypher_change start
      return new AuthError(
        {
          providerID: "openai",
          message: e.message,
        ...
        { cause: e },
      ).toObject() // cypher_change end
    case SessionNetwork.disconnected(e): // cypher_change start
      return new APIError(
        {
          message: SessionNetwork.message(e), // cypher_change end
          isRetryable: true,
          metadata: {
