...
import os from "os"
import { CypherSessionPrompt } from "@/cypher/session/prompt" // cypher_change
import { CypherSessionMessageOrder } from "@/cypher/session/message-order" // cypher_change
import { CypherSessionPromptQueue } from "@/cypher/session/prompt-queue" // cypher_change
...
import { CypherSessionOverflow } from "@/cypher/session/overflow" // cypher_change
import { CommandTimeout } from "@/cypher/command-timeout" // cypher_change
import { Suggestion } from "@/cypher/suggestion" // cypher_change
import { Question } from "@/question" // cypher_change
import { BUILTIN_COMMANDS } from "@/cypher/session/builtin-commands" // cypher_change
import { legacyReviewMessage } from "@/cypher/review/command" // cypher_change
import { zod } from "@opencode-ai/core/effect-zod" // cypher_change
import { withStatics } from "@opencode-ai/core/schema" // cypher_change
...
// cypher_change
export const shouldAskPlanFollowup = CypherSessionPrompt.shouldAskPlanFollowup

// cypher_change start - persistent tool-output pruning when payload is already large
...
export interface Interface {
  ...
  // cypher_change start - prompt can fail on unmet agent requirements
  readonly prompt: (
    input: PromptInput,
  ...
  // cypher_change start - commands can fail on unmet agent requirements
  readonly command: (
    input: CommandInput,
  ) => Effect.Effect<MessageV2.WithParts, Image.Error | Agent.RequirementBlockedError>
  // cypher_change end
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const question = yield* Question.Service // cypher_change - dismiss superseded pending questions through the shared service
    ...
    const cancel = Effect.fn("SessionPrompt.cancel")(function* (sessionID: SessionID) {
      ...
      yield* CypherSessionPromptQueue.cancel(sessionID) // cypher_change - drop queued follow-up loops on abort
      CypherSessionPrompt.abortPlanFollowup(sessionID) // cypher_change - abort pending plan-followup handover work
    ...
    const title = Effect.fn("SessionPrompt.ensureTitle")(function* (input: {
      ...
      const text = yield* llm
        .stream({
          ...
          tools: {},
          model: mdl,
          sessionID: CypherSessionPrompt.titleID(input.session.id), // cypher_change - isolate title requests from the agent task
          retries: 2,
    ...
    const handleSubtask = Effect.fn("SessionPrompt.handleSubtask")(function* (input: {
      ...
      // cypher_change start - shared reader for the child session id written by task.ts ctx.metadata (#6321)
      const childID = () => {
      ...
      const result = yield* taskTool
        .execute(taskArgs, {
          ...
          // cypher_change start - resolve permissions at ask time so active tools see config edits
          ask: (req: any) =>
            CypherSessionPrompt.askPermission({
              permission,
              agents,
              sessions,
              agent: taskAgent,
        ...
        .pipe(
          ...
          Effect.onInterrupt(() =>
            Effect.gen(function* () {
              ...
              // cypher_change start - propagate partial subagent cost on cancel (#6321)
              const cid = childID()
              if (cid) {
                assistantMessage.cost = yield* CypherCostPropagation.childCost(sessions, SessionID.make(cid))
              }
              // cypher_change end
      ...
      assistantMessage.finish = "tool-calls"
      assistantMessage.time.completed = Date.now()
      // cypher_change start - include subagent total cost on the wrapper message (#6321)
      ...
      const summaryUserMsg: MessageV2.User = {
        ...
        editorContext: lastUser.editorContext, // cypher_change — preserve editor context
      }
      ...
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          const { msg, part, cwd } = yield* Effect.gen(function* () {
            ...
            const callID = ulid() // cypher_change - correlate v2 shell events with the persisted tool part
          ...
          let output = ""
          let aborted = false
          let timeout: string | undefined // cypher_change
    ...
    const getModel = Effect.fn("SessionPrompt.getModel")(function* (
      ...
      if (Provider.ModelNotFoundError.isInstance(err)) {
        ...
        const empty = err.modelsEmpty ? " No models are currently available." : "" // cypher_change
        yield* bus.publish(Session.Event.Error, {
          sessionID,
          error: new NamedError.Unknown({
            message: `Model not found: ${err.providerID}/${err.modelID}.${hint}${empty}`, // cypher_change
          }).toObject(),
        })
      }
    ...
    const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (input: PromptInput) {
      ...
      const ag = agentName ? yield* agents.get(agentName) : yield* agents.defaultInfo() // cypher_change
      if (!ag) {
        const available = (yield* agents.list()).filter((a) => !a.hidden).map((a) => a.name) // cypher_change
        const hint = available.length ? ` Available agents: ${available.join(", ")}` : "" // cypher_change
        const error = new NamedError.Unknown({ message: `Agent not found: "${agentName}".${hint}` }) // cypher_change
        yield* bus.publish(Session.Event.Error, { sessionID: input.sessionID, error: error.toObject() }) // cypher_change
        throw error // cypher_change
      }
      yield* agents.guardRequirements(ag) // cypher_change - enforce requirements before creating a turn
      ...
      const info: MessageV2.User = {
        ...
        system: input.system,
        format: input.format,
        editorContext: input.editorContext, // cypher_change
      }
        ...
        if (part.type === "file") {
          ...
          switch (url.protocol) {
            case "data:":
              ...
              // cypher_change start - normalize user image data before persistence
            ...
            case "file:": {
              ...
              if (mime === "application/x-directory") {
                ...
                const exit = yield* execRead(args, { includeDirectoryFiles: true }).pipe(Effect.exit) // cypher_change inline folder files
              ...
              // cypher_change start - reject oversized user image files before reading and base64 allocation
              ...
              // cypher_change start - apply image limits after resolving user file URLs
      ...
      // cypher_change start - resolve and persist the exact transformed Cypher prompt parts
    ...
    const prompt: (input: PromptInput) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.prompt")(
      function* (input: PromptInput) {
        const session = yield* sessions.get(input.sessionID).pipe(Effect.orDie)
        yield* revert.cleanup(session)
        // cypher_change start - recover interrupted Cypher turns before accepting a follow-up
        yield* CypherSessionPrompt.recoverDanglingAssistant({ sessionID: input.sessionID, status, sessions })
        yield* CypherSessionPrompt.recoverProviderFinishError({ sessionID: input.sessionID, status, sessions })
        // cypher_change end
        ...
        if (permissions.length > 0) {
          // cypher_change start - preserve inherited task restrictions while refreshing prompt tool toggles
          const merged = CypherSessionPrompt.mergeToolPermissions({
        ...
        // cypher_change start — unblock tools waiting on user input so any in-flight
        ...
        return yield* CypherSessionPromptQueue.enqueue(
          input.sessionID,
          message.info.id,
          bridge.run(
    ...
    const lastAssistant = Effect.fnUntraced(function* (sessionID: SessionID) {
      // cypher_change start - retry when cancel races before shellImpl writes messages
    ...
    // cypher_change — mutable close-reason per session, set by runLoop and read by loop
    const closeReasons = new Map<string, CypherSession.CloseReason>()

    // cypher_change start - retain request-scoped snapshot initialization policy
    const runLoop: (input: LoopInput) => Effect.Effect<MessageV2.WithParts, NotFoundError> = Effect.fn(
      "SessionPrompt.run",
    )(function* (input: LoopInput) {
      const sessionID = input.sessionID
      // cypher_change end
      // cypher_change — cache environment details per turn (prompt caching)
      const envCache: CypherSessionPrompt.EnvCache = {}
      const memoryCache = CypherSessionPrompt.memoryCache() // cypher_change
      closeReasons.delete(sessionID) // cypher_change
      let compactionAttempts = 0 // cypher_change - cap compaction attempts per turn to avoid infinite loops
      const ctx = yield* InstanceState.context
      ...
      while (true) {
        ...
        msgs = CypherSessionPromptQueue.scope(sessionID, msgs) // cypher_change - hide later queued prompts
        msgs = CypherSessionPrompt.trimBeforeLastSummary(msgs) // cypher_change - trim on any completed summary (e.g. manual /compact against a text user)

        // cypher_change start - select loop state by chronology after retained-tail projection
        const latest = CypherSessionMessageOrder.latest(msgs)
        ...
        // cypher_change start - compare chronology, not generated IDs
        const userBeforeAssistant =
          latest.userMessage &&
          latest.assistantMessage &&
          CypherSessionMessageOrder.compare(latest.userMessage, latest.assistantMessage) < 0
        // cypher_change end
        // cypher_change start - carry local review command marker into LLM telemetry
        const telemetry =
          CypherSessionProcessor.extractReviewTelemetry(
            msgs.findLast((m) => m.info.role === "user" && m.info.id === lastUser.id)?.parts ?? [],
          ) ?? CypherSessionProcessor.extractSuggestionReviewTelemetry(lastAssistantMsg?.parts ?? [])
        // cypher_change end
        ...
        // cypher_change start - plan_exit is a hard stop before another model call
        if (
          lastAssistant?.finish &&
          hasToolCalls &&
          lastAssistant.parentID === lastUser.id &&
          userBeforeAssistant &&
          CypherSessionPrompt.shouldAskPlanFollowup({ messages: msgs, abort: AbortSignal.any([]) })
        ) {
          const action = yield* Effect.promise((signal) =>
            CypherSessionPrompt.askPlanFollowup({ sessionID, messages: msgs, abort: signal, question }),
          )
        ...
        if (
          ...
          !hasToolCalls &&
          lastAssistant.parentID === lastUser.id && // cypher_change - unrelated later assistants do not answer this turn
          userBeforeAssistant // cypher_change - compare chronology, not generated IDs
        ) {
        ...
        if (task?.type === "compaction") {
          ...
          // cypher_change start - compaction.process only returns "stop" after
        ...
        if (
          ...
          // cypher_change start
          const guard = CypherSessionPrompt.guardCompactionAttempt({
            sessionID,
        ...
        const handle = yield* processor
          .create({
            ...
            sessionID,
            model,
            telemetry, // cypher_change
            snapshotInitialization: input.snapshotInitialization, // cypher_change
          })
        ...
        const outcome: "break" | "continue" = yield* Effect.gen(function* () {
          ...
          const tools = yield* SessionTools.resolve({
            ...
            messages: msgs,
            promptOps,
            memoryCache, // cypher_change
          }).pipe(
            Effect.provideService(Plugin.Service, plugin),
            Effect.provideService(Permission.Service, permission),
            Effect.provideService(Agent.Service, agents), // cypher_change
            Effect.provideService(Session.Service, sessions), // cypher_change
            Effect.provideService(ToolRegistry.Service, registry),
            Effect.provideService(MCP.Service, mcp),
            Effect.provideService(Truncate.Service, truncate),
            // cypher_change start - SWE-Pruner (experimental)
          ...
          if (step > 1 && lastFinished) {
            for (const m of msgs) {
              ...
              const finishedBeforeMessage =
                latest.finishedMessage && CypherSessionMessageOrder.compare(latest.finishedMessage, m) < 0
          ...
          // cypher_change start — ephemeral context injection + post-summary
          ...
          CypherSessionPrompt.injectEditorContext({ msgs, lastUser, sessionID, cache: envCache })
          msgs = CypherSessionPrompt.maybeStripHistoricalMedia(msgs)
          // cypher_change end

          // cypher_change start - persistently prune stale tool outputs when payload is already large
          const [skills, env, mem, instructions] = yield* Effect.all([
            sys.skills(agent),
            sys.environment(model, lastUser.editorContext), // cypher_change
            CypherSessionPrompt.memoryInject({ ctx, sessionID, record: step === 1, cache: memoryCache }), // cypher_change
          ...
          if (size > REQUEST_PRUNE_BYTES) {
            ...
            msgs = CypherSessionPromptQueue.scope(sessionID, msgs)
            msgs = CypherSessionPrompt.trimBeforeLastSummary(msgs)
            yield* plugin.trigger("experimental.chat.messages.transform", {}, { messages: msgs })
            CypherSessionPrompt.injectEditorContext({ msgs, lastUser, sessionID, cache: envCache })
            msgs = CypherSessionPrompt.maybeStripHistoricalMedia(msgs)
          ...
          // cypher_change end
          const system = [...env, ...mem, ...instructions, ...(skills ? [skills] : [])] // cypher_change
          const format = lastUser.format ?? { type: "text" as const }
          if (format.type === "json_schema") system.push(STRUCTURED_OUTPUT_SYSTEM_PROMPT)
          const result = yield* handle.process({
            // cypher_change start - keep Ask/Plan tool filtering hardened against session allows
            user: lastUser,
            agent,
            permission: CypherSessionPrompt.guardPermissions({ agent, session }),
            // cypher_change end
            sessionID,
            ...
            // cypher_change start - feed the provider-reported context size from the last finished
            // turn into the output-token cap, so image/vision input is measured by the provider
            // rather than by encoded payload bytes (see CypherLLM.capOutputTokens). Summary messages
            ...
            reportedContextTokens:
              lastFinished && lastFinished.summary !== true
                ? CypherSessionOverflow.count(lastFinished.tokens)
                : undefined,
            // cypher_change end
          })

          // cypher_change start - persist a lightweight marker when this assistant step had memory context
          const marker = CypherSessionPrompt.memoryPart({ sessionID, message: handle.message, cache: memoryCache })
          ...
          if (finished && !handle.message.error) {
            ...
            if (handle.message.finish === "error") {
              CypherSessionProcessor.providerFinishError(handle.message)
              yield* sessions.updateMessage(handle.message)
              closeReasons.set(sessionID, "error")
              return "break" as const
            }
          ...
          if (result === "compact") {
            ...
            yield* compaction.create({
              ...
              model: lastUser.model,
              auto: true,
              // cypher_change - preflight compaction replays the pending turn without treating media as provider overflow
              overflow: !handle.message.finish && handle.compactError?.() !== undefined, // cypher_change
            })
          }
          // cypher_change start — break out so a newer queued prompt can take over
          ...
          if (CypherSessionPromptQueue.hasFollowup(sessionID)) {
            closeReasons.set(sessionID, "interrupted")
            return "break" as const
          }
          // cypher_change end
          // cypher_change start - guard against providers that end the stream
      ...
      return yield* Effect.onExit(
        ...
        Effect.fnUntraced(function* (exit) {
          yield* bus.publish(CypherSession.Event.TurnClose, {
            ...
            reason: CypherSessionPrompt.resolveCloseReason({
              sessionID: input.sessionID,
              closeReasons,
              exit,
            }),
    ...
    const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
      ...
      if (!cmd) {
        ...
        available.push(...BUILTIN_COMMANDS) // cypher_change - surface built-in session commands in error hint
        available.sort() // cypher_change - alphabetical for stable, easy-to-scan output
      ...
      // cypher_change start - deprecated review aliases should display a static notice without an LLM turn
      ...
      const agent = agentName ? yield* agents.get(agentName) : yield* agents.defaultInfo() // cypher_change
      if (!agent) {
        const available = (yield* agents.list()).filter((a) => !a.hidden).map((a) => a.name) // cypher_change
        const hint = available.length ? ` Available agents: ${available.join(", ")}` : "" // cypher_change
        const error = new NamedError.Unknown({ message: `Agent not found: "${agentName}".${hint}` }) // cypher_change
      ...
      yield* agents.guardRequirements(agent) // cypher_change - command agent overrides must satisfy requirements

      const templateParts = yield* resolvePromptParts(template)
      CypherSessionProcessor.markReviewTelemetry(templateParts, input.command) // cypher_change - mark review commands for completion telemetry
...
export const defaultLayer = Layer.suspend(() =>
  layer
    .pipe(
      ...
      Layer.provide(Question.defaultLayer), // cypher_change - provide pending question dismissal dependency
    ...
    .pipe(
      ...
      Layer.provide(Image.defaultLayer), // cypher_change - provide user image normalization service
      Layer.provide(
        Layer.mergeAll(
...
export const PromptInput = Schema.Struct({
  ...
  // cypher_change start - managed product slow-snapshot policy
  ...
  // cypher_change end
  // cypher_change start - reuse shared editor context schema
...
// cypher_change start - retain precise prompt input types for Cypher callers
...
export class LoopInput extends Schema.Class<LoopInput>("SessionPrompt.LoopInput")({
  sessionID: SessionID,
  snapshotInitialization: Schema.optional(Schema.Literal("wait")), // cypher_change
}) {
