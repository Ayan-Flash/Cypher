...
import { MemoryMarker } from "@/cypher/memory/marker" // cypher_change
...
// cypher_change start
import { SwePruner } from "@/cypher/swe-pruner"
...
export const resolve = Effect.fn("SessionTools.resolve")(function* (input: {
  ...
  processor: Pick<SessionProcessor.Handle, "message" | "metadata" | "completeToolCall"> // cypher_change
  bypassAgentCheck: boolean
  messages: MessageV2.WithParts[]
  promptOps: TaskPromptOps
  memoryCache: MemoryMarker.Cache // cypher_change
}) {
  ...
  for (const item of yield* registry.tools({
    ...
    family: input.model.family, // cypher_change
    agent: input.agent,
  })) {
    // cypher_change start - SWE-Pruner (experimental): advertise the focus parameter on prunable tools
      ...
      execute(args, options) {
        return run.promise(
          Effect.gen(function* () {
            ...
            // cypher_change - mark successful targeted memory recalls for the assistant badge
            if (item.id === "cypher_memory_recall") MemoryMarker.recall({ result: output, cache: input.memoryCache }) // cypher_change
            yield* plugin.trigger(
              "tool.execute.after",
