...
import { CypherRoutedModel } from "@/cypher/session/routed-model" // cypher_change
...
function usage(value: unknown) {
  ...
  const item = value as {
    ...
    raw?: Record<string, unknown> // cypher_change - preserve provider billing details
  }
  const entries = Object.entries({
    ...
    providerMetadata: item.raw ? { aiSdk: item.raw } : undefined, // cypher_change - retain Cypher billing details
    ...
    case "finish-step":
      return Effect.sync(() => [
        LLMEvent.stepFinish({
          ...
          providerMetadata: CypherRoutedModel.write(providerMetadata(event.providerMetadata), event.response?.modelId), // cypher_change
        }),
      ])
