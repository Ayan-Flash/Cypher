...
import { isCypherError } from "@/cypher/cypher-errors" // cypher_change
...
export type RetryReason = string & {} // cypher_change - Cypher does not support OpenCode Go upsell reasons
...
// cypher_change - Cypher does not emit OpenCode Go actions
export function retryable(error: Err, _provider?: string): Retryable | undefined {
  // context overflow errors should not be retried
  if (MessageV2.ContextOverflowError.isInstance(error)) return undefined
  if (MessageV2.APIError.isInstance(error)) {
    const status = error.data.statusCode
    // cypher_change start - Current Cypher errors require user action (login/signup), don't retry
    if (isCypherError(error)) return undefined
    // cypher_change end
    ...
    // cypher_change start - Cypher does not support OpenCode Go upsells. FreeUsageLimitError is not retryable: retrying
...
export function policy(opts: {
  ...
  return Schedule.fromStepWithMetadata(
    Effect.succeed((meta: Schedule.InputMetadata<unknown>) => {
      // cypher_change start — enforce retry limit
      ...
      return Effect.gen(function* () {
        // cypher_change start — handle network disconnect via offline handler
