...
import { capture } from "@/cypher/instance"
...
export namespace SessionNetwork {
  ...
  const urls = ["https://cypher.ai", "https://example.com", "https://cloudflare.com/cdn-cgi/trace"]
  const POLL_MS = 3_000
  ...
  export function disconnected(err: unknown) {
    ...
    // cypher_change - recognize AbortSignal.timeout() errors
    for (const item of chain(err)) {
  ...
  export function message(err: unknown) {
    // cypher_change - check for timeout first
    for (const item of chain(err)) {
  ...
  export const reply = fn(
    ...
    async (input) => {
      ...
      // cypher_change start - reconnect failed remote MCP servers after network recovery
