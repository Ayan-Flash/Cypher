...
// cypher_change - drop unused inArray/lt (listGlobal delegated to CypherSession)
import { eq, and, gte, isNull, desc, like, or } from "drizzle-orm"
import { SyncEvent } from "../sync"
import { PartTable, SessionTable } from "./session.sql"
// cypher_change - ProjectTable removed (unused)
...
// cypher_change start - Cypher session behavior extensions
import { BackgroundProcess } from "@/cypher/background-process"
import { InteractiveTerminal } from "@/cypher/interactive-terminal"
import { CypherSession, cypherSessionFork } from "@/cypher/session"
import { SessionExport } from "@/cypher/session-export"
import * as SandboxPolicy from "@/cypher/sandbox/policy"
import { baseKey, cumulativeSessionDiff } from "@/cypher/session-portability/cumulative-diff" // cypher_change
import { BlockedError as AgentRequirementError } from "@/cypher/agent-requirements"
// cypher_change end
...
const Summary = Schema.Struct({
  ...
  files: Schema.Finite,
  diffs: optionalOmitUndefined(Schema.Array(Snapshot.SummaryFileDiff)), // cypher_change - lightweight diff without patch
})
...
export const GlobalInfo = Schema.Struct({
  ...
  worktreeName: Schema.optional(Schema.String), // cypher_change - basename of the specific worktree directory
...
export const CreateInput = Schema.optional(
  Schema.Struct({
    ...
    platform: Schema.optional(Schema.String), // cypher_change - per-session platform override for telemetry attribution
...
export const Event = {
  ...
  Error: BusEvent.define(
    ...
    Schema.Struct({
      ...
      // cypher_change start - carry pre-message requirement failures over session.error
      error: Schema.optional(Schema.Union([MessageV2.Assistant.fields.error, AgentRequirementError.EffectSchema])),
      // cypher_change end
    }),
  ),
  // cypher_change start
  TurnOpen: CypherSession.Event.TurnOpen,
  TurnClose: CypherSession.Event.TurnClose,
  // cypher_change end
}

export function plan(input: { slug: string; time: { created: number } }, instance: InstanceContext) {
  const base = instance.project.vcs
    ? path.join(instance.worktree, ".cypher", "plans") // cypher_change
...
export const getUsage = (input: {
  ...
  provider?: Provider.Info // cypher_change
}) => {
  ...
  // cypher_change start - Use provider-reported cost when available for OpenRouter/Cypher
  const reported = CypherSession.providerCost({
...
export interface Interface {
  ...
  readonly create: (input?: {
    ...
    permission?: Permission.Ruleset
    platform?: string // cypher_change - per-session platform override for telemetry attribution
    workspaceID?: WorkspaceID
  }) => Effect.Effect<Info>
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createNext = Effect.fn("Session.createNext")(function* (input: {
      ...
      permission?: Permission.Ruleset
      platform?: string // cypher_change - per-session platform override for telemetry attribution
      sourceID?: SessionID // cypher_change - inherited sandbox policy source
      sandboxFallback?: SandboxPolicy.Snapshot // cypher_change - confinement to seed when source state lives in another directory
    }) {
      ...
      // cypher_change start - initialize inherited state before session.created subscribers run
      CypherSession.register({ id: result.id, parentID: result.parentID, platform: input.platform })
    ...
    // cypher_change start - scope children by persisted parent project_id
    ...
    const remove: Interface["remove"] = Effect.fnUntraced(function* (sessionID: SessionID) {
      ...
      try {
        ...
        yield* SandboxPolicy.dispose(
          sessionID,
          Effect.gen(function* () {
            yield* Effect.promise(() => CypherSession.removeSession(sessionID)).pipe(Effect.ignore)
            CypherSession.clearPlatformOverride(sessionID)
            if (hasInstance) {
            ...
            // cypher_change - capture final session-export workspace delta on close/delete
            const workspaceKey = hasInstance ? yield* InstanceState.directory : undefined // cypher_change
            yield* Effect.promise(() => SessionExport.onSessionClose(sessionID, workspaceKey)) // cypher_change
    ...
    const updateMessage = <T extends MessageV2.Info>(msg: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        // cypher_change start - ignore FK errors when session was deleted while processor was still running
        yield* CypherSession.runSyncSafe(sync.run(MessageV2.Event.Updated, { sessionID: msg.sessionID, info: msg }), {
          type: "message update",
          id: msg.id,
    ...
    const updatePart = <T extends MessageV2.Part>(part: T): Effect.Effect<T> =>
      Effect.gen(function* () {
        // cypher_change start - ignore FK errors when session was deleted while processor was still running
        yield* CypherSession.runSyncSafe(
    ...
    const create = Effect.fn("Session.create")(function* (input?: {
      ...
      const session = yield* createNext({
        ...
        platform: input?.platform, // cypher_change
    ...
    const fork = Effect.fn("Session.fork")(function* (input: { sessionID: SessionID; messageID?: MessageID }) {
      ...
      // cypher_change start - forks into another directory cannot read the source confinement from the new dir, so carry it over explicitly
      ...
      const session = yield* createNext({
        ...
        sourceID: input.sessionID, // cypher_change - forks preserve initialized confinement
        sandboxFallback, // cypher_change - seed confinement from the source session's original directory
      })
      const msgs = yield* messages({ sessionID: input.sessionID })
      const idMap = new Map<string, MessageID>()
      const writer = CypherSession.writer(session.id, sync) // cypher_change - commit copied transcript in one transaction
      ...
      for (const msg of msgs) {
        ...
        // cypher_change start - queue copied messages for the atomic transcript commit
        ...
        for (const part of msg.parts) {
          const p: MessageV2.Part = {
            ...
            messageID: cloned.id,
            sessionID: session.id,
            ...(part.type === "step-finish" && { cost: 0 }), // cypher_change - exclude pre-fork spend from model stats
          }
          if (p.type === "compaction" && p.tail_start_id) {
            p.tail_start_id = idMap.get(p.tail_start_id)
          }
          writer.part(p) // cypher_change - queue copied parts for the atomic transcript commit
        }
      }
      yield* writer.commit() // cypher_change - the caller hydrates after commit; copied-row events stay silent
      // cypher_change start - preserve imported/cumulative diffs when forking sessions
...
function* listByProject(
  ...
  // cypher_change start - CypherSession.filters keeps sessions visible across project_id changes
  ...
  const conditions =
    input.path !== undefined
      ...
      : CypherSession.filters({ projectID: input.projectID, directory: input.directory })
  // cypher_change end
...
// cypher_change start - delegate to CypherSession.listGlobal (adds projectID worktree family + directories[])
export function* listGlobal(input?: {
  ...
  yield* CypherSession.listGlobal<GlobalInfo>({ ...input, fromRow })
}
// cypher_change end

// cypher_change - delegate the exported Promise facade to the Cypher session runtime
export const fork = cypherSessionFork
