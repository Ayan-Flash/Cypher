...
import { CypherQuestion } from "@/cypher/question" // cypher_change
...
export const Option = Schema.Struct({
  ...
  // cypher_change start - optional i18n keys so clients can translate while still
  ...
  // cypher_change end
  // cypher_change start - hint to UI clients to switch the active agent/mode picker
...
const base = {
  ...
  // cypher_change start - optional i18n keys for question text and header
...
export interface Interface {
  readonly ask: (input: {
    ...
    blocking?: boolean // cypher_change
    tool?: Tool
  ...
  readonly dismissAll: (sessionID: SessionID) => Effect.Effect<void> // cypher_change
}
  ...
  Effect.gen(function* () {
    ...
    const ask = Effect.fn("Question.ask")(function* (input: {
      ...
      const info: Request = {
        ...
        blocking: input.blocking, // cypher_change
        tool: input.tool,
      }

      // cypher_change start
      yield* CypherQuestion.guardFollowup(input.sessionID, () => new RejectedError())
      // cypher_change end
      ...
      return yield* Effect.ensuring(
        ...
        // cypher_change start - every asked question gets a terminal event when its waiter is interrupted
        CypherQuestion.finalize({
          pending,
          id,
    ...
    // cypher_change start - body lives in @/cypher/question/CypherQuestion.makeDismissAll
    const dismissAll = CypherQuestion.makeDismissAll({
      state,
    ...
    return Service.of({ ask, reply, reject, list, dismissAll }) // cypher_change
  }),
)
