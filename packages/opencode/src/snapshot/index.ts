...
import { Struct } from "effect" // cypher_change
...
import { EffectFlock } from "@opencode-ai/core/util/effect-flock" // cypher_change
import { Config } from "@/config/config"
import { Global } from "@opencode-ai/core/global"
import * as Log from "@opencode-ai/core/util/log"
// cypher_change start
import { Flag } from "@opencode-ai/core/flag/flag"
import { DiffFull } from "../cypher/snapshot/diff-full"
import { CypherSnapshotTrack } from "../cypher/snapshot/track"
import { CypherSnapshotSeed } from "../cypher/snapshot/seed"
import { CypherSnapshotMaterialize } from "../cypher/snapshot/materialize"
...
}).pipe(withStatics((s) => ({ zod: zod(s) }))) // cypher_change
...
// cypher_change start - lightweight FileDiff without patch for session summaries
...
const prune = "7.days"
const retention = 7 * 24 * 60 * 60 * 1000 // cypher_change
...
export const MAX_DIFF_SIZE = 256 * 1024 // cypher_change
...
export interface Interface {
  ...
  // cypher_change start - pass prompt context and managed initialization policy
  readonly track: (opts?: {
    sessionID?: SessionID
    messageID?: MessageID
    snapshotInitialization?: CypherSnapshotTrack.SnapshotInitialization
...
export const layer: Layer.Layer<Service, never, Requirements> =
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const flock = yield* EffectFlock.Service // cypher_change
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Snapshot.state")(function* (ctx) {
          ...
          // cypher_change start - serialize snapshot repositories across CLI and extension processes
          ...
          const enabled = Effect.fnUntraced(function* () {
            ...
            if (Flag.CYPHER_CLIENT === "acp") return false // cypher_change - ACP clients do not support snapshots
          ...
          const add = Effect.fnUntraced(function* (opts?: { env?: Record<string, string>; root?: boolean }) {
            ...
            // cypher_change start - initial seeded writes stay protected by the source pin
            yield* stage(
          ...
          const materialize = Effect.fnUntraced(function* () {
            yield* locked(CypherSnapshotMaterialize.run({ gitdir: state.gitdir, git, fs }).pipe(Effect.orDie)).pipe(
          ...
          const cleanup = Effect.fnUntraced(function* () {
            return yield* locked(
              Effect.gen(function* () {
                ...
                // cypher_change start - retain snapshots for the same seven-day window as object pruning
                yield* CypherSnapshotMaterialize.prune({ gitdir: state.gitdir, git, fs }, Date.now() - retention)
                // cypher_change end
          ...
          const track = Effect.fnUntraced(function* (opts?: Parameters<Interface["track"]>[0]) {
            ...
            return yield* locked(
              Effect.gen(function* () {
                ...
                const seeded: { value?: CypherSnapshotSeed.Output } = {} // cypher_change
                ...
                if (!existed) {
                  ...
                  // cypher_change start - seed all eligible new snapshots from the worktree index
                  seeded.value = yield* CypherSnapshotSeed.seed({
                    dir: state.directory,
                ...
                // cypher_change start - pin every snapshot before background materialization
                ...
                if (
                  seed &&
                  !(yield* CypherSnapshotMaterialize.localize({
                    gitdir: state.gitdir,
                    git,
                    fs,
                ...
                if (
                  seed &&
                  !(yield* CypherSnapshotMaterialize.localizeTrees(
                    { gitdir: state.gitdir, git, fs, staging: seed.staging },
                    hash,
                  ))
                )
                  return
                if (!(yield* CypherSnapshotMaterialize.pin({ gitdir: state.gitdir, git, fs }, hash))) return
          ...
          const diffFull = Effect.fnUntraced(function* (from: string, to: string) {
            return yield* locked(
              Effect.gen(function* () {
                ...
                // cypher_change start - use git patches to avoid blocking the event loop on large diffs
          ...
          yield* materialize() // cypher_change - resume interrupted snapshot object materialization
      ...
      // cypher_change start - service-local state and cache avoid leaking across Snapshot layer instances
      const trackState = CypherSnapshotTrack.makeStates()
      ...
      return Service.of({
        ...
        // cypher_change start - isolate turn-facing snapshot work from poisoned locks
        track: Effect.fn("Snapshot.track")(function* (opts) {
          const ctx = yield* InstanceState.context
          const guard = trackState(ctx.worktree)
          return yield* CypherSnapshotTrack.protect({
            inner: CypherSnapshotTrack.wrap({
        ...
        diffFull: Effect.fn("Snapshot.diffFull")(function* (from: string, to: string) {
          // cypher_change start - cache full diffs at the service boundary
          if (from === to) return []
...
export const defaultLayer = layer.pipe(
  ...
  Layer.provide(EffectFlock.defaultLayer), // cypher_change
)
