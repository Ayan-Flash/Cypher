...
import { Schema } from "effect" // cypher_change - encode EventV2 data at legacy boundaries
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // cypher_change start - legacy bus and SSE consumers require the schema's encoded representation
    ...
    const unsubscribe = yield* events.sync((event) => {
      ...
      const data = EventWire.encode(definition.data, event.data) // cypher_change
      ...
      if (definition.version !== undefined && typeof aggregateID === "string") {
        return provideEventLocation(event, data, sync.run(toSyncDefinition(definition), event.data)) // cypher_change
      }
