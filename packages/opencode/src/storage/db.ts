...
declare const CYPHER_MIGRATIONS: { sql: string; timestamp: number; name: string }[] | undefined
...
export function getChannelPath(flags: Pick<DatabaseFlags, "disableChannelDb"> = readRuntimeFlags()) {
  if (["latest", "beta", "prod"].includes(InstallationChannel) || flags.disableChannelDb)
    return path.join(Global.Path.data, "cypher.db") // cypher_change
  const safe = InstallationChannel.replace(/[^a-zA-Z0-9._-]/g, "-")
  const next = path.join(Global.Path.data, `cypher-${safe}.db`) // cypher_change
  const prev = path.join(Global.Path.data, `cypher-${safe}.db`) // cypher_change
  if (!existsSync(next) && existsSync(prev)) return prev // cypher_change
  return next // cypher_change
}

export const getPath = (flags?: Pick<DatabaseFlags, "disableChannelDb">) => {
  const dbEnv = process.env.CYPHER_DB ?? Flag.CYPHER_DB
  if (dbEnv) {
    if (dbEnv === ":memory:" || path.isAbsolute(dbEnv)) return dbEnv
    return path.join(Global.Path.data, dbEnv)
  }
...
export const Client = Object.assign(
  (flags: DatabaseFlags = readRuntimeFlags()): Client => {
    ...
    const entries =
      typeof CYPHER_MIGRATIONS !== "undefined"
        ? CYPHER_MIGRATIONS
        : migrations(path.join(import.meta.dirname, "../../migration"))
    if (entries.length > 0) {
      log.info("applying migrations", {
        count: entries.length,
        mode: typeof CYPHER_MIGRATIONS !== "undefined" ? "bundled" : "dev",
      })
