...
// cypher_change start - skills can share names with slash commands
  ...
  Effect.gen(function* () {
    ...
    const init = Effect.fn("Command.state")(function* (ctx: InstanceContext) {
      ...
      for (const item of yield* skill.all()) {
        ...
        commands[item.name] = fromSkill(item) // cypher_change
      }
    ...
    const get = Effect.fn("Command.get")(function* (name: string) {
      ...
      const exact = s.commands[name] // cypher_change
      if (exact) return exact // cypher_change
      const alias = legacyReviewCommand(name) // cypher_change
      if (alias) return alias // cypher_change
      ...
      // cypher_change end
      return undefined // cypher_change
    })
