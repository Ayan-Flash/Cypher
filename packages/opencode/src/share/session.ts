...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const share = Effect.fn("SessionShare.share")(function* (sessionID: SessionID) {
      ...
      const result = yield* CypherSession.shareSession(sessionID) // cypher_change - use Cypher public share URLs
      yield* sync.run(Session.Event.Updated, { sessionID, info: { share: { url: result.url } } })
      return result
    })

    const unshare = Effect.fn("SessionShare.unshare")(function* (sessionID: SessionID) {
      yield* CypherSession.unshareSession(sessionID) // cypher_change - use Cypher public share URLs
