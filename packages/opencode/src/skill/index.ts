...
import { BUILTIN_SKILLS } from "../cypher/skills/builtin" // cypher_change
import { primaryPaths } from "../cypher/primary-worktree" // cypher_change
...
const CYPHER_SKILL_PATTERN = "{skill,skills}/**/SKILL.md"
...
const discoverSkills = Effect.fnUntraced(function* (
  ...
  for (const dir of configDirs) {
    yield* scan(state, dir, CYPHER_SKILL_PATTERN)
  }
...
const loadSkills = Effect.fnUntraced(function* (state: State, discovered: DiscoveryState, bus: Bus.Interface) {
  // cypher_change start - seed built-in skills before discovery so user skills can override
  ...
  for (const match of discovered.matches) yield* add(state, match, bus) // cypher_change
    ...
    const discovered = yield* InstanceState.make(
      Effect.fn("Skill.discovery")(function* (ctx) {
        return yield* discoverSkills(
          ...
          ctx.directory,
          ctx.worktree, // cypher_change
        ).pipe(Effect.provideService(Git.Service, git)) // cypher_change
      }),
    )
