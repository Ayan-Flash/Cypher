...
import type { Message, CypherClient, SessionMessageResponse } from "@cypher/sdk/v2"
...
export const AuthMethodID = "cypher-login" // cypher_change
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `cypher auth login` in the terminal",
      name: "Login with Cypher",
      id: AuthMethodID,
    }
    // cypher_change end
...
function makeDirectoryService(sdk: CypherClient) {
...
function makeUsageService(sdk: CypherClient) {
...
async function loadDirectorySnapshot(sdk: CypherClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  const cypherProvider = providers[ProviderID.cypher] // cypher_change
  const cypherModel = cypherProvider ? Provider.sort(Object.values(cypherProvider.models))[0] : undefined // cypher_change
  if (cypherProvider && cypherModel) return { providerID: cypherProvider.id, modelID: cypherModel.id } // cypher_change
