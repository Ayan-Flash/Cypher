...
- Author package entrypoint is `@cypher/plugin/tui`.
...
```json
{
  "$schema": "https://app.cypher.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
...
- Import types from `@cypher/plugin/tui`.
- `@cypher/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running Cypher version.
...
- `cypher plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `cypher plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.cypher` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.cypher`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.cypher`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.cypher` or legacy `.cypher` directory, or `CYPHER_CONFIG_DIR`, Cypher installs `@cypher/plugin` into that directory and writes:
...
That is what makes local config-scoped plugins able to import `@cypher/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Cypher's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Cypher registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Cypher pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@cypher/plugin/tui`.
...
- `message` is required; `title` defaults to `"Cypher"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `cypher.default`.
...
- Local plugins persist installed themes under `.cypher/themes`, or under `themes` beside a config source in `.cypher` or legacy `.cypher`.
...
- `--pure` / `CYPHER_PURE` skips external TUI plugins only.
...
The upstream `.opencode` smoke fixtures remain in the repository for OpenCode compatibility, but Cypher does not auto-load them. Cypher-local examples must live under `.cypher` or legacy `.cypher`.
