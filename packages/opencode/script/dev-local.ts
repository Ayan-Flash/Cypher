...
// Launch the cypher CLI dev build against a locally running cloud dev server.
//   bun dev:local <project-dir> [--cloud <dir>] [--no-ingest] [--print] [-- <cypher args>]
//
// Reads ports from <cloud>/dev/logs/manifest.json (+ .dev-port), probes the web
// server, and points the CLI at it (CYPHER_API_URL / CYPHER_SESSION_INGEST_URL).
// Auth/config/state/cache are isolated under ~/.cypher-dev so it can't clash with
// your main cypher install; real HOME is kept so git/ssh still work.
...
const cypher = path.resolve(import.meta.dir, "../../..")
const home = path.join(os.homedir(), ".cypher-dev")
...
async function main() {
  ...
  env.CYPHER_API_URL = `http://localhost:${webPort}`
  env.CYPHER_DEV_CWD = project
  env.CYPHER_DISABLE_AUTOUPDATE = "1"
  if (ingestPort) env.CYPHER_SESSION_INGEST_URL = `http://localhost:${ingestPort}`
  else env.CYPHER_DISABLE_SESSION_INGEST = "1"
  ...
  process.exit(await Bun.spawn({ cmd: ["bun", "run", "--cwd", "packages/opencode", "--conditions=browser", "src/index.ts", ...pass], cwd: cypher, env, stdio: ["inherit", "inherit", "inherit"] }).exited)
}
