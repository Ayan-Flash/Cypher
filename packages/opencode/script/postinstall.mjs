...
// cypher_change start - variant detection matching bin/cypher logic
const platformMap = {
  darwin: "darwin",
  linux: "linux",
  win32: "windows",
...
const sourceBinary = platform === "windows" ? "cypher.exe" : "cypher"
const targetBinary = path.join(__dirname, "bin", ".cypher")
...
// cypher_change start - copy runtime resources next to cached binary
function copyResources(source) {
  ...
  const worker = path.join(path.dirname(source), "cypher-sandbox-mutation-worker.js")
  if (fs.existsSync(worker)) fs.copyFileSync(worker, path.join(__dirname, "bin", "cypher-sandbox-mutation-worker.js"))
}
...
function main() {
  ...
  for (const name of packageNames()) {
    try {
      ...
      const temp = fs.mkdtempSync(path.join(os.tmpdir(), "cypher-install-"))
      try {
  ...
  throw new Error(
    `It seems your package manager failed to install the right Cypher CLI package. Try manually installing ${packageNames()
...
try {
  ...
  console.error("Failed to setup cypher binary:", error.message)
  process.exit(1)
}
