  ...
  const profiles = {
    darwin: {
      ...
      groups: {
        ...
        cypher: [
          "cypher/{background-process,daemon,diff-full,external-directory-boundary,indexing-worker,indexing-worktree,interactive-terminal,mcp-oauth-callback,primary-worktree,project-id,read-directory,session-diff-restore,snapshot-cache,snapshot-freeze-repro,snapshot-revert-move,snapshot-seed,task-nesting}.test.ts",
          "cypher/cli/cmd/serve.test.ts",
          "cypher/cli/install-artifact.test.ts",
          "cypher/config/config.test.ts",
          "cypher/sandbox/*.test.ts",
          "cypher/server/{config-overlay,listener-runtime,tui-config,worktree-list}.test.ts",
          "cypher/session-export/{e2e,sequence,worker,workspace-provider}.test.ts",
          "cypher/session-export/worker/{storage,zstd}.test.ts",
          "cypher/worktree*.test.ts",
        ],
