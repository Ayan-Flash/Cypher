...
import { TestProfile } from "./cypher/test-profile"
import { TestShard } from "./cypher/test-shard"
...
if (argv.includes("--help") || argv.includes("-h")) {
  console.log(
    [
      ...
      "  --profile <name>     Run a curated test profile (env: CYPHER_TEST_PROFILE)",
      "  --shard <N/M>        Run one balanced file shard (env: CYPHER_TEST_SHARD)",
...
// resources (ports, global filesystem like ~/.local/share/cypher), not CPU.
...
const env = process.env.CYPHER_TEST_PROFILE?.trim() || undefined
if (flag && env && flag !== env) {
  console.error(`Conflicting test profiles: --profile=${flag}, CYPHER_TEST_PROFILE=${env}`)
  process.exit(2)
}
const profile = flag ?? env
const shardFlag = text("shard")
const shardEnv = process.env.CYPHER_TEST_SHARD?.trim() || undefined
if (shardFlag && shardEnv && shardFlag !== shardEnv) {
  console.error(`Conflicting test shards: --shard=${shardFlag}, CYPHER_TEST_SHARD=${shardEnv}`)
  process.exit(2)
}
...
const matched =
  patterns.length > 0
    ...
    : selected
const candidates = patterns.length > 0 && !profile ? matched : matched.filter((file) => !skipped.has(file)) // cypher_change
