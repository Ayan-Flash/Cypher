...
describe("ProviderTransform.variants", () => {
  ...
  test("minimax via cypher gateway returns instant/thinking toggle", () => {
    const model = createMockModel({
      id: "cypher/minimax/minimax-m3",
      providerID: "cypher",
      api: {
        id: "minimax/minimax-m3",
        url: "https://gateway.cypher.ai",
  ...
  // cypher_change start
  describe("@cypher/cypher-gateway", () => {
    test("claude models return empty variants (reasoning disabled)", () => {
      const model = createMockModel({
        id: "cypher/anthropic/claude-sonnet-4",
        providerID: "cypher",
        capabilities: { reasoning: false },
        api: {
          id: "anthropic/claude-sonnet-4",
    ...
    test("anthropic models in api.id return empty variants (reasoning disabled)", () => {
      const model = createMockModel({
        id: "cypher/anthropic/claude-opus-4",
        providerID: "cypher",
        capabilities: { reasoning: false },
        api: {
    ...
    test("codex models return OPENAI_EFFORTS with object-based reasoning format", () => {
      const model = createMockModel({
        id: "cypher/openai/gpt-5.2-codex",
        providerID: "cypher",
        api: {
    ...
    // cypher_change start
    test("mercury-2 uses server-provided variants from cypher gateway", () => {
  ...
  describe("ProviderTransform.smallOptions", () => {
    describe("@cypher/cypher-gateway", () => {
      ...
      test("google models use their default reasoning effort", () => {
        const model = createMockModel({
          id: "cypher/google/gemini-2.0-flash",
          providerID: "cypher",
          api: {
...
// cypher_change start - tests for reasoningSummary guard
describe("ProviderTransform.options - OpenAI Responses API params guard", () => {
  ...
  test("includes reasoningSummary for @cypher/cypher-gateway", () => {
    const result = ProviderTransform.options({
      model: gpt5Model("@cypher/cypher-gateway", "cypher"),
      sessionID,
    })
    expect(result.reasoningSummary).toBe("auto")
  })
