...
it.live("headerTimeout does not abort delayed SSE body after headers arrive", () =>
  Effect.gen(function* () {
    ...
    yield* provideTmpdirInstance(
      ...
      { config: providerConfig(server.url, { headerTimeout: 500 }) }, // cypher_change
    )
  }),
)
...
function withAuthContent<A, E, R>(self: Effect.Effect<A, E, R>, value: Record<string, unknown> = defaultAuthContent()) {
  return Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.CYPHER_AUTH_CONTENT
      process.env.CYPHER_AUTH_CONTENT = JSON.stringify(value)
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous === undefined) delete process.env.CYPHER_AUTH_CONTENT
        else process.env.CYPHER_AUTH_CONTENT = previous
      }),
  )
}
