...
import * as launch from "../../src/lsp/launch" // cypher_change - spy on spawn
...
import { TsCheck } from "../../src/cypher/ts-check" // cypher_change

// cypher_change - Typescript.spawn ignores ctx, so a cast is fine here.
...
describe("lsp.spawn", () => {
  ...
  // cypher_change start - provide the runtime flag so spawn() is reached past the TsClient short-circuit
  ...
  // cypher_change start - Typescript spawn is gated behind CYPHER_EXPERIMENTAL_LSP_TOOL.
  test("spawns tsgo LSP when CYPHER_EXPERIMENTAL_LSP_TOOL is enabled", async () => {
    const saved = Flag.CYPHER_EXPERIMENTAL_LSP_TOOL
    Flag.CYPHER_EXPERIMENTAL_LSP_TOOL = true
    ...
    try {
      ...
      Flag.CYPHER_EXPERIMENTAL_LSP_TOOL = saved
      spawnSpy.mockRestore()
      tsgoSpy.mockRestore()
    }
  })

  test("Typescript.spawn returns undefined when CYPHER_EXPERIMENTAL_LSP_TOOL is off", async () => {
    const saved = Flag.CYPHER_EXPERIMENTAL_LSP_TOOL
    Flag.CYPHER_EXPERIMENTAL_LSP_TOOL = false
    try {
