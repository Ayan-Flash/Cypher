...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CYPHER_TEST ?? "")'), {
      env: {
        CYPHER_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CYPHER_TEST_SHELL"], {
      shell: true,
      env: {
        CYPHER_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CYPHER_TEST_SHELL=ok")
  })
