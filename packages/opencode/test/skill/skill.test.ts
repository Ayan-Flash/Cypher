...
const itWithoutClaudeCodeSkills = testEffect(Layer.mergeAll(skills(false, true), node)) // cypher_change
...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.CYPHER_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.CYPHER_TEST_HOME = prev
      }),
  )

const discovered = <T extends { location: string }>(list: readonly T[]) =>
  list.filter((skill) => ![Skill.BUILTIN_LOCATION, "<built-in>"].includes(skill.location)) // cypher_change

describe("skill", () => {
  // cypher_change start
  it.live("discovers skills from .cypher/skill/ directory", () =>
    // cypher_change end
      ...
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".cypher", "skill", "test-skill", "SKILL.md"),
              `---
name: test-skill
...
`,
          ...
          const list = discovered(yield* skill.all()) // cypher_change
      ...
      (dir) =>
        withHome(
          ...
          Effect.gen(function* () {
            yield* Effect.promise(() =>
              Bun.write(
                path.join(dir, ".cypher", "skill", "dir-skill", "SKILL.md"), // cypher_change: .cypher is primary
                `---
name: dir-skill
...
`,
            ...
            expect(dirs).toContain(path.join(dir, ".cypher", "skill", "dir-skill")) // cypher_change: .cypher is primary
  ...
  // cypher_change start
  it.live("discovers multiple skills from .cypher/skill/ directory", () =>
    // cypher_change end
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".cypher", "skill", "skill-one", "SKILL.md"),
                `---
name: skill-one
...
`,
              ...
              Bun.write(
                path.join(dir, ".cypher", "skill", "skill-two", "SKILL.md"),
                `---
name: skill-two
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".cypher", "skill", "no-frontmatter", "SKILL.md"), // cypher_change: .cypher is primary
              `# No Frontmatter
...
`,
          ...
          const skill = yield* Skill.Service
          expect(discovered(yield* skill.all())).toEqual([]) // cypher_change
        }),
      { git: true },
    ),
  )
  ...
  it.live("discovers skills without descriptions", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          // cypher_change start - load .cypher skills without falling back to .opencode
          yield* Effect.promise(() =>
            Promise.all([
              Bun.write(
                path.join(dir, ".cypher", "skill", "manual-skill", "SKILL.md"),
                `---
name: manual-skill
---
...
`,
              ...
              // cypher_change start
              Bun.write(
                path.join(dir, ".cypher", "skill", "opencode-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .cypher/skill directory.
---
...
`,
              ...
              // cypher_change start
              Bun.write(
                path.join(dir, ".cypher", "skill", "agent-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .cypher/skill directory.
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".cypher", "skills", "agent-skill", "SKILL.md"),
                `---
name: opencode-skill
description: A skill in the .cypher/skills directory.
---
