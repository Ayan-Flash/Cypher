...
import { context as instanceContext, type InstanceContext } from "../../src/project/instance-context" // cypher_change
...
export async function provideTestInstance<R>(input: {
  ...
  try {
    ...
    return await instanceContext.provide(ctx, () => input.fn(ctx)) // cypher_change
  } finally {
    // cypher_change start
...
export async function withTestInstance<R>(input: { directory: string; fn: (ctx: InstanceContext) => R }) {
  const ctx = await runTestInstanceStore((store) => store.load({ directory: input.directory }))
  return instanceContext.provide(ctx, () => input.fn(ctx)) // cypher_change
}
...
function clean(dir: string) {
  return cleanup(dir) // cypher_change
}
    ...
    if (options?.config) {
      ...
      yield* Effect.promise(() =>
        fs.writeFile(
          ...
          JSON.stringify({ $schema: "https://app.cypher.ai/config.json", ...resolved }), // cypher_change
        ),
      )
    }
