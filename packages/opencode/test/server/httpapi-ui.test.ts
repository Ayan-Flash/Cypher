  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        ...
        restoreEnv("CYPHER_SERVER_PASSWORD", original.envPassword)
        restoreEnv("CYPHER_SERVER_USERNAME", original.envUsername)
      }),
    )
  }),
)
      ...
      Layer.provide(
        ConfigProvider.layer(
          ConfigProvider.fromUnknown({
            CYPHER_SERVER_PASSWORD: input?.password,
            CYPHER_SERVER_USERNAME: input?.username,
          }),
        ),
      ),
    ),
...
describe("HttpApi UI fallback", () => {
  // cypher_change start - embedded UI is the only supported fallback; never proxy to app.opencode.ai
  ...
  it.live("accepts auth token for the web UI", () =>
    Effect.gen(function* () {
      let proxied = false // cypher_change
      const response = yield* uiApp({
        password: "secret",
        username: "cypher", // cypher_change
        disableEmbeddedWebUi: true,
        // cypher_change start - authenticated requests still must not proxy when embedded UI is disabled
        client: httpClient(new Response("<html>cypher</html>", { headers: { "content-type": "text/html" } }), () => {
          proxied = true
        }),
        // cypher_change end
      }).request(`/?auth_token=${btoa("cypher:secret")}`)
  ...
  it.live("accepts basic auth for the web UI", () =>
    Effect.gen(function* () {
      ...
      const response = yield* uiApp({
        ...
        headers: { authorization: `Basic ${btoa("cypher:secret")}` },
      })
  ...
  it.live("accepts basic auth passwords containing colons for the web UI", () =>
    Effect.gen(function* () {
      ...
      expect(response.status).toBe(404) // cypher_change - auth succeeds, but Cypher does not proxy a fallback UI
    }),
  )
  ...
  it.live("allows web UI preflight without auth", () =>
    Effect.gen(function* () {
      const response = yield* app({ password: "secret", username: "cypher" }).request("/", {
        // cypher_change
        method: "OPTIONS",
        headers: {
