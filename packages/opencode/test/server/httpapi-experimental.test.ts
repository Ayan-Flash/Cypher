...
import { realpath } from "node:fs/promises" // cypher_change
      ...
      Effect.gen(function* () {
        ...
        yield* withCreatedWorktree(tmp.directory, (info) =>
          Effect.gen(function* () {
            ...
            const directory = yield* Effect.promise(() => realpath(info.directory)) // cypher_change
            expect(yield* json(listed)).toContainEqual({ directory, managed: true }) // cypher_change
