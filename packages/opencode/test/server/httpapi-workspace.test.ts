...
function request(path: string, directory: string, init: RequestInit = {}) {
  return Effect.promise(() => {
    ...
    headers.set("x-cypher-directory", directory)
  ...
  it.live("proxies remote workspace HTTP requests with sanitized forwarding", () =>
    Effect.gen(function* () {
      ...
      try {
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-cypher-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-cypher-workspace")
