...
import { awaitWithTimeout, pollWithTimeout, testEffect } from "../lib/effect" // cypher_change
...
type Sdk = ReturnType<typeof createCypherClient>
...
function app(serverPath: ServerPath, input?: { password?: string; username?: string }) {
  Flag.CYPHER_SERVER_PASSWORD = input?.password
  Flag.CYPHER_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return AppFileSystem.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".cypher", "skills", "project-rest-skill", "SKILL.md"), // cypher_change
      `---
...
describe("HttpApi SDK", () => {
  ...
  httpapiInstance(
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        // cypher_change start - match Hono AuthMiddleware username default ("cypher")
        const bad = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("cypher", "wrong") },
          }).file.read({ path: "hello.txt" }),
        )
        const good = yield* capture(() =>
          client("raw", directory, {
            password: "secret",
            headers: { authorization: authorization("cypher", "secret") },
          }).file.read({ path: "hello.txt" }),
        )
        // cypher_change end
  ...
  // cypher_change start - verify invalid user images fail at the real SDK boundary
  ...
  // cypher_change start - verify provider errors remain in successful assistant messages
