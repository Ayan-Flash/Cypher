...
const original = {
  ...
  envPassword: process.env.CYPHER_SERVER_PASSWORD,
  envUsername: process.env.CYPHER_SERVER_USERNAME,
}
const auth = { username: "opencode", password: "listen-secret" }
const testPty = process.platform === "win32" ? test.skip : test

afterEach(async () => {
  Flag.CYPHER_SERVER_PASSWORD = original.CYPHER_SERVER_PASSWORD
  Flag.CYPHER_SERVER_USERNAME = original.CYPHER_SERVER_USERNAME
  if (original.envPassword === undefined) delete process.env.CYPHER_SERVER_PASSWORD
  else process.env.CYPHER_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.CYPHER_SERVER_USERNAME
  else process.env.CYPHER_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.CYPHER_SERVER_PASSWORD = auth.password
  Flag.CYPHER_SERVER_USERNAME = auth.username
  process.env.CYPHER_SERVER_PASSWORD = auth.password
  process.env.CYPHER_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}

async function startNoAuthListener() {
  Flag.CYPHER_SERVER_PASSWORD = undefined
  Flag.CYPHER_SERVER_USERNAME = auth.username
  delete process.env.CYPHER_SERVER_PASSWORD
  process.env.CYPHER_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      ...(options?.ticketHeader === false ? {} : { "x-cypher-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-cypher-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("default in-process handler does not emit Effect HTTP response logs", async () => {
    ...
    try {
      // cypher_change start - use an authenticated local route instead of proxy-dependent status
      Flag.CYPHER_SERVER_PASSWORD = auth.password
      Flag.CYPHER_SERVER_USERNAME = auth.username
      process.env.CYPHER_SERVER_PASSWORD = auth.password
      process.env.CYPHER_SERVER_USERNAME = auth.username
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-cypher-ticket": "1" },
      })
