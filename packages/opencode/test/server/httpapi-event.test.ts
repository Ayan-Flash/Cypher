...
// cypher_change start - verify transformed EventV2 values at the legacy SSE boundary
...
// cypher_change start - inspect the real global SSE envelope
...
const openEventStream = (directory: string) =>
  Effect.gen(function* () {
    const response = yield* Effect.promise(async () =>
      Server.Default().app.request(EventPaths.event, { headers: { "x-cypher-directory": directory } }),
    )
...
// cypher_change start - read transformed values from the global SSE wire payload
...
describe("event HttpApi", () => {
  ...
  // cypher_change start - transformed EventV2 data is numeric on legacy SSE while domain data stays decoded
