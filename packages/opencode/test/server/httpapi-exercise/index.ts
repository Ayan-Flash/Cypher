 ...
 *
 * The script intentionally isolates `CYPHER_DB` before importing modules that touch
...
import { cypherScenarios } from "../../cypher/server/httpapi-exercise-scenarios" // cypher_change
...
const scenarios: Scenario[] = [
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-cypher-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-cypher-directory")
  }),
  ...
  // cypher_change start
  http.protected.get("/cypher/notebook", "cypher.notebook.list").json(200, array),
  http.protected
    .post("/cypher/notebook/{requestID}/reply", "cypher.notebook.reply")
    .at((ctx) => ({
      path: route("/cypher/notebook/{requestID}/reply", { requestID: "nbr_httpapi_reply" }),
      headers: ctx.headers(),
      body: {
        result: {
          operation: "read",
    ...
    .json(404, object, "status"),
  http.protected
    .post("/cypher/notebook/{requestID}/reject", "cypher.notebook.reject")
    .at((ctx) => ({
      path: route("/cypher/notebook/{requestID}/reject", { requestID: "nbr_httpapi_reject" }),
      headers: ctx.headers(),
  ...
  http.protected
    ...
    .status(500, undefined, "status"), // cypher_change
  http.protected
