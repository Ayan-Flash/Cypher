    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
        ...
        const base: ScenarioContext = {
          ...
          headers: (extra) => ({
            ...(context.dir?.path ? { "x-cypher-directory": context.dir.path } : {}),
            ...extra,
          }),
          file: (name, content) =>
            Effect.promise(() => {
...
const resetState = Effect.promise(async () => {
  ...
  // cypher_change - each exerciser process already owns an isolated DB; unlinking it between scenarios races async Cypher callbacks
  await Bun.sleep(25)
})
