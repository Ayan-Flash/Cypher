...
import { MCP } from "../../src/mcp" // cypher_change
...
const agentLayer = (flags: Partial<RuntimeFlags.Info> = {}) =>
  Agent.layer.pipe(
    ...
    Layer.provide(Layer.mock(MCP.Service)({})), // cypher_change
...
it.instance("build agent has correct default properties", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(build, "interactive_terminal")).toBe("allow") // cypher_change
  }),
)
...
it.instance("plan agent denies edits except .opencode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(plan, "interactive_terminal")).toBe("deny") // cypher_change
...
it.instance("explore agent denies edit and write", () =>
  Effect.gen(function* () {
    ...
    expect(evalPerm(explore, "interactive_terminal")).toBe("deny") // cypher_change
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".cypher", "skill", "perm-skill") // cypher_change
