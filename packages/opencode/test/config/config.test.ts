...
import { Git } from "../../src/git" // cypher_change
import {
...
import { isIndexingPlugin } from "@cypher/cypher-indexing/detect" // cypher_change
import { isAtomicChatPlugin } from "@/cypher/atomic-chat-feature" // cypher_change
...
const schemaConfig = (config: object) => ({ $schema: "https://app.cypher.ai/config.json", ...config }) // cypher_change
...
const managedConfigDir = process.env.CYPHER_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.CYPHER_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.CYPHER_CONSOLE_TOKEN
  else process.env.CYPHER_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})

const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  AppFileSystem.use.writeWithDirs(path.join(managedConfigDir, filename ?? "cypher.json"), JSON.stringify(settings)) // cypher_change
...
const writeConfigEffect = (
  dir: string,
  config: object,
  name = "cypher.json", // cypher_change
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".cypher"), schemaConfig(input.local)) : undefined, // cypher_change
      ].filter(
...
it.effect("creates global jsonc config with schema when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const content = yield* AppFileSystem.use.readFileString(path.join(dir, "cypher.jsonc")) // cypher_change
      expect(content).toContain('"$schema": "https://app.cypher.ai/config.json"') // cypher_change
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
  ),
)

it.effect("does not create global config when CYPHER_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
// cypher_change start
it.instance("preserves Cypher provider free model metadata", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.cypher.ai/config.json",
      model: "cypher/free-e2e",
      provider: {
        cypher: {
          models: {
            "free-e2e": {
    ...
    const config = yield* Config.use.get()
    const model = config.provider?.cypher?.models?.["free-e2e"]
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // cypher_change - upstream hardcodes project config to config.json; Cypher writes to cypher.json
    ...
    const writtenConfig = yield* AppFileSystem.use.readJson(path.join(test.directory, "cypher.json")) // cypher_change
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* AppFileSystem.use.readJson(path.join(dir, "cypher.json")) // cypher_change
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* AppFileSystem.use.writeWithDirs(
      // cypher_change start
      path.join(test.directory, "cypher.jsonc"),
      `{
        // This is a comment
        "$schema": "https://app.cypher.ai/config.json",
        "model": "test/model",
...
it.instance("jsonc overrides json in the same directory", () =>
  Effect.gen(function* () {
    ...
    yield* writeConfigEffect(
      test.directory,
      {
        $schema: "https://app.cypher.ai/config.json", // cypher_change
        model: "base",
        username: "base",
      },
      "cypher.jsonc", // cypher_change
    )
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.cypher.ai/config.json", // cypher_change
      model: "override",
...
// cypher_change start
it.instance("prefers .cypher directory config over legacy .cypher", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(path.join(test.directory, ".cypher"), {
      $schema: "https://app.cypher.ai/config.json",
      model: "legacy/model",
    })
    yield* writeConfigEffect(path.join(test.directory, ".cypher"), {
...
// cypher_change start - project config is untrusted: {env:} rejected; {file:} confined to the project root
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:CYPHER_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.instance("reports warning for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(path.join(test.directory, "cypher.json"), "{ invalid json }")
    yield* Config.use.get()
    const issues = yield* Config.Service.use((svc) => svc.warnings())
    expect(issues.length).toBeGreaterThan(0)
  }),
...
// cypher_change start
it.instance("loads config from .cypher directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".cypher", "agent", "test.md"), // cypher_change
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".cypher", "agent", "ordered.md"), // cypher_change
      `---
permission:
  bash: allow
  "*": deny
...
// cypher_change start
it.instance("loads agents from .cypher/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".cypher", "agents", "helper.md"), // cypher_change
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".cypher", "agents", "nested", "child.md"), // cypher_change
      `---
model: test/model
mode: subagent
---
...
// cypher_change start
it.instance("loads commands from .cypher/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".cypher", "command", "hello.md"), // cypher_change
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".cypher", "command", "nested", "child.md"), // cypher_change
      `---
...
// cypher_change start
it.instance("loads commands from .cypher/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".cypher", "commands", "hello.md"), // cypher_change
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".cypher", "commands", "nested", "child.md"), // cypher_change
      `---
description: Nested command
---
Nested command template`,
...
// cypher_change start
it.instance("prefers .cypher commands over legacy .cypher commands", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".cypher", "command", "hello.md"),
      `---
...
it.instance("updates config and writes to file", () =>
  Effect.gen(function* () {
    ...
    const writtenConfig = yield* AppFileSystem.use.readJson(
      path.join(test.directory, ".cypher", "cypher.jsonc"), // cypher_change
    )
...
it.effect("does not try to install dependencies in read-only CYPHER_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("CYPHER_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(CrossSpawnSpawner.defaultLayer)),
)

it.effect("installs dependencies in writable CYPHER_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* AppFileSystem.use.writeWithDirs(
      path.join(test.directory, ".cypher", "agent", "helper.md"), // cypher_change
      `---
model: test/model
mode: subagent
---
...
it.effect("keeps plugin origins aligned with merged plugin list", () =>
  withConfigTree(
    ...
    Effect.gen(function* () {
      ...
      // cypher_change start - bundled plugins intentionally have no external plugin origins
...
// cypher_change - Note: preload.ts sets CYPHER_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("permission config preserves user key order", () =>
  ...
  Effect.gen(function* () {
    const test = yield* TestInstance
    // cypher_change start — isolate from global config to prevent cross-test contamination
...
// cypher_change start - regression for `env` alias on local MCP entries
...
it.instance("project config can override MCP server enabled status", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // cypher_change - base config in .json, override in .jsonc (jsonc loads second and wins)
...
it.instance("MCP config deep merges preserving base config properties", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    // cypher_change - base config in .json, override in .jsonc (jsonc loads second and wins)
    // cypher_change - Base config with full MCP definition
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.cypher.ai/config.json", // cypher_change
      mcp: {
        myserver: {
    ...
    // cypher_change - Override just enables it, should preserve other properties
    yield* writeConfigEffect(
      test.directory,
      {
        $schema: "https://app.cypher.ai/config.json", // cypher_change
...
// cypher_change start
it.instance("local .cypher config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      $schema: "https://app.cypher.ai/config.json", // cypher_change
    ...
    yield* writeConfigEffect(
      path.join(test.directory, ".cypher"), // cypher_change
      {
        $schema: "https://app.cypher.ai/config.json", // cypher_change
        mcp: {
          docs: {
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "cypher.json") // cypher_change
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.cypher/plugin/oh-my-opencode.js"] // cypher_change
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.cypher/plugin/demo.ts", "file:///project/.cypher/plugin/demo.ts"] // cypher_change

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.cypher/plugin/demo.ts"]) // cypher_change
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* AppFileSystem.use.writeWithDirs(
          path.join(test.directory, ".cypher", "plugin", "my-plugin.js"), // cypher_change
          "export default {}",
        )
...
describe("CYPHER_DISABLE_PROJECT_CONFIG", () => {
  // cypher_change start
  ...
  it.instance("skips project .cypher directory when flag is set", () =>
    withProcessEnv(
      "CYPHER_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* AppFileSystem.use.writeWithDirs(
          path.join(test.directory, ".cypher", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { CYPHER_CONFIG_DIR: undefined, CYPHER_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "CYPHER_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        ...
        yield* withProcessEnvs(
          { CYPHER_DISABLE_PROJECT_CONFIG: "true", CYPHER_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed CYPHER_PERMISSION JSON used to crash
...
describe("CYPHER_PERMISSION env var", () => {
  it.instance("does not crash when CYPHER_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "CYPHER_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("CYPHER_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in CYPHER_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "CYPHER_CONFIG_CONTENT",
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
  ...
  it.instance("substitutes {file:} tokens in CYPHER_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
