...
const globalConfigFiles = ["cypher.json", "cypher.jsonc", "tui.json", "tui.jsonc"].map((file) =>
  path.join(Global.Path.config, file),
)

const cleanState = Effect.gen(function* () {
  const fs = yield* AppFileSystem.Service
  delete process.env.CYPHER_CONFIG
  delete process.env.CYPHER_TUI_CONFIG
...
const withCleanState = <A, E, R>(self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.gen(function* () {
      const disabled = Flag.CYPHER_DISABLE_DEFAULT_PLUGINS
      Flag.CYPHER_DISABLE_DEFAULT_PLUGINS = true
      yield* cleanState
      return disabled
    }),
    () => self,
    (disabled) =>
      Effect.gen(function* () {
        Flag.CYPHER_DISABLE_DEFAULT_PLUGINS = disabled
        yield* cleanState
      }),
  )
...
it.instance("keeps server and tui plugin merge semantics aligned", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const local = path.join(test.directory, ".cypher") // cypher_change
      yield* fs.makeDirectory(local, { recursive: true })

      yield* fs.writeJson(path.join(Global.Path.config, "cypher.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(local, "cypher.json"), {
  ...
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(
        path.join(test.directory, ".cypher", "tui.json"), // cypher_change
  ...
  withCleanState(
    Effect.gen(function* () {
      ...
      expect((yield* getTuiConfig(test.directory)).attention).toEqual({
        ...
        sound: true,
        volume: 0.4,
        sound_pack: "cypher.default", // cypher_change
        sounds: {},
      })
...
it.instance("migrates tui-specific keys from cypher.json when tui.json does not exist", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      const source = path.join(test.directory, "cypher.json")
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "cypher.json.tui-migration.bak"))).toBe(true)
...
it.instance("migrates project legacy tui keys even when global tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), { theme: "global" })
      yield* fs.writeJson(path.join(test.directory, "cypher.json"), {
      ...
      const server = JSON.parse(yield* fs.readFileString(path.join(test.directory, "cypher.json")))
      expect(server.theme).toBeUndefined()
      expect(server.tui).toBeUndefined()
    }),
  ),
...
it.instance("skips migration when cypher.jsonc is syntactically invalid", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
  ...
  "username": "still-broken"
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "cypher.jsonc.tui-migration.bak"))).toBe(false)
      const source = yield* fs.readFileString(path.join(test.directory, "cypher.jsonc"))
...
it.instance("skips migration when tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(test.directory, "cypher.json"), { theme: "legacy" })
      ...
      expect(yield* fs.existsSafe(path.join(test.directory, "cypher.json.tui-migration.bak"))).toBe(false)
    }),
  ),
)
...
it.instance("migration backup preserves JSONC comments", () =>
  ...
  "tui": {
    ...
    "scroll_speed": 1.5
      ...
      const backup = yield* fs.readFileString(path.join(test.directory, "cypher.jsonc.tui-migration.bak"))
...
it.instance("migrates legacy tui keys across multiple cypher.json levels", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(test.directory, "cypher.json"), { theme: "root-theme" })
      yield* fs.writeJson(path.join(nested, "cypher.json"), { theme: "nested-theme" })
...
it.instance("project config takes precedence over CYPHER_TUI_CONFIG (matches CYPHER_CONFIG)", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
...
it.instance("CYPHER_TUI_CONFIG provides settings when no project config exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
...
it.instance("does not derive tui path from CYPHER_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      ...
      yield* fs.writeJson(path.join(customDir, "cypher.json"), { model: "test/model" })
      yield* fs.writeJson(path.join(customDir, "tui.json"), { theme: "should-not-load" })

      yield* withEnv(
        "CYPHER_CONFIG",
        path.join(customDir, "cypher.json"),
        Effect.gen(function* () {
          const config = yield* getTuiConfig(test.directory)
          expect(config.theme).toBeUndefined()
        }),
...
// cypher_change start - trusted global config substitutes; untrusted project config does not
...
it.instance("applies file substitutions when first identical token is in a commented line", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      // cypher_change start - global config is trusted, so the second (uncommented) reference resolves
...
it.instance("loads .cypher/tui.json", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      yield* fs.writeWithDirs(
        path.join(test.directory, ".cypher", "tui.json"),
...
it.instance("silently skips malformed tui.json - load failures degrade to {}", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(path.join(test.directory, ".cypher", "tui.json"), JSON.stringify({ theme: "fallback" })) // cypher_change
