...
import { SessionNetwork } from "../../src/session/network" // cypher_change
...
// cypher_change start - exercise non-default output token ceilings in the processor
...
// cypher_change start - configured output ceiling must reach finish-step overflow accounting
...
it.live("session.processor effect tests reset reasoning state across retries", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
        // cypher_change start — auto-reply to network reconnection prompts triggered by reset()
        ...
        offAsk() // cypher_change — cleanup subscriber
      }),
    { config: (url) => providerCfg(url) },
  ),
)
