...
function fake(
  ...
  return {
    ...
    metadata: Effect.fn("TestSessionProcessor.metadata")(() => Effect.void), // cypher_change
...
type CompactionProcessOptions = {
  ...
  flags?: Partial<RuntimeFlags.Info> // cypher_change
  snapshot?: Layer.Layer<Snapshot.Service> // cypher_change
}
...
function compactionProcessLayer(options?: CompactionProcessOptions) {
  ...
  return Layer.mergeAll(SessionCompaction.layer.pipe(Layer.provide(processor)), processor, bus, status).pipe(
    ...
    Layer.provide(options?.snapshot ?? Snapshot.defaultLayer), // cypher_change
    ...
    Layer.provide(RuntimeFlags.layer({ experimentalEventSystem: true, ...options?.flags })), // cypher_change
    Layer.provide(EventV2Bridge.defaultLayer),
  )
}

// cypher_change start - keep retry-backoff cancellation tests independent of git snapshot cleanup latency
...
describe("session.compaction.process", () => {
  ...
  // cypher_change start - configured output ceiling controls automatic tail budgeting
  itCompaction.instance(
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        // cypher_change start - avoid a scheduler-sensitive inner deadline on loaded CI runners
      ...
      }).pipe(withCompaction({ llm: stub.layer, snapshot: snap })) // cypher_change
    },
    { git: true },
  )
...
describe("SessionNs.getUsage", () => {
  ...
  // cypher_change start - Test for OpenRouter provider cost
  ...
  test("uses upstreamInferenceCost for Cypher provider", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    const provider = { id: "cypher" } as Provider.Info
    const result = SessionNs.getUsage({
      model,
      provider,
      usage: usage({ inputTokens: 1_000_000, outputTokens: 100_000, totalTokens: 1_100_000 }),
    ...
    // Should use upstreamInferenceCost for Cypher provider (BYOK)
  ...
  test("uses regular cost when upstreamInferenceCost is missing for Cypher", () => {
    const model = createModel({
      context: 100_000,
      output: 32_000,
      cost: {
    ...
    // When upstream cost is missing for Cypher, fall back to regular cost field
    expect(result.cost).toBe(0.01)
  })

  // Tests for Anthropic Messages / OpenAI Responses / Vercel AI Gateway cost extraction
  // live in test/cypher/provider-cost.test.ts (cypher_change).
  // cypher_change end
