...
import { USER_AGENT } from "../../src/installation" // cypher_change

type ConfigModel = NonNullable<NonNullable<NonNullable<Config.Info["provider"]>[string]>["models"]>[string] // cypher_change
...
describe("session.llm.ai-sdk adapter", () => {
  ...
  // cypher_change start - preserve AI SDK raw usage for Cypher provider billing
  ...
  test("preserves providerMetadata on step-finish so Anthropic cache writes survive getUsage", async () => {
    ...
    expect(stepFinish.providerMetadata).toEqual({
      ...
      cypher: { routedModelID: "claude-3-5-sonnet" },
    })
    // cypher_change end
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(headers.get("User-Agent") ?? "").toMatch(/^Cypher-Code\//) // cypher_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const capture = yield* Effect.promise(() => request)
        const body = capture.body
        const headers = capture.headers // cypher_change

        expect(capture.url.pathname.endsWith("/messages")).toBe(true)
        expect(headers.get("User-Agent")?.split(" ")[0]).toBe(USER_AGENT) // cypher_change
