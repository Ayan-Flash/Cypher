...
import { legacyReviewMessage } from "../../src/cypher/review/command"
// cypher_change end
...
import { Auth } from "../../src/auth" // cypher_change
...
import { Suggestion } from "../../src/cypher/suggestion" // cypher_change - accept suggestion in telemetry test
...
import { MemoryService } from "@cypher/cypher-memory/effect/service" // cypher_change
...
function makePrompt(input?: { processor?: "blocking" }) {
  const deps = Layer.mergeAll(
    ...
    LLM.defaultLayer,
    Env.defaultLayer,
    input?.processor === "blocking" ? fastAgents : AgentSvc.defaultLayer, // cypher_change
    ...
    MemoryService.layer, // cypher_change
  ...
  const registry = ToolRegistry.layer.pipe(
    ...
    Layer.provide(Auth.defaultLayer), // cypher_change
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<Config.Info>) {
  yield* writeText(
    ...
    JSON.stringify({ $schema: "https://app.cypher.ai/config.json", ...config }), // cypher_change
  )
})
...
// cypher_change start - wait for the runner state that cancel observes instead of session status
...
// cypher_change start - replacement prompts unblock pending Question service requests
noLLMServer.instance(
...
// cypher_change start - cover user image normalization before persistence
noLLMServer.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const tool = yield* pollWithTimeout(
        Effect.gen(function* () {
          const msgs = yield* MessageV2.filterCompactedEffect(chat.id)
          const assistant = msgs.findLast((item) => item.info.role === "assistant" && item.info.agent === "code") // cypher_change
...
// cypher_change start - child task failures stay tool errors so the parent can recover
it.instance(
...
it.instance(
  ...
  () =>
    // cypher_change start - hold the model response instead of cancelling an infinite stream
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
...
it.instance(
  ...
  10_000, // cypher_change - Windows CI can take longer to cancel the live loop
)
...
// cypher_change start - handleSubtask propagates child session cost to wrapper (#6321)
it.instance(
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.yieldNow // cypher_change - let the queued caller join without a wall-clock race
  ...
  { git: true },
  10_000, // cypher_change - Windows CI can take longer to cancel queued live loops
)
...
it.instance(
  ...
  () =>
    // cypher_change start - gate the failing stream so both callers join the same run
    Effect.gen(function* () {
      const { llm } = yield* useServerConfig(providerCfg)
      const gate = yield* Deferred.make<void>()
      const prompt = yield* SessionPrompt.Service
...
it.instance(
  ...
  10_000, // cypher_change - loaded CI runners can exceed 3s for two prompt turns
)
...
it.instance(
  ...
  10_000, // cypher_change - Windows CI can take longer to enter and cancel the live loop
)
...
// cypher_change start - verify shell v2 events correlate with the persisted tool part
unixNoLLMServer(
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* Effect.yieldNow // cypher_change - give the queued loop a scheduler turn instead of a wall-clock window
  ...
  { git: true },
  30_000, // cypher_change - Windows CI process startup can exceed 3s
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const a = yield* prompt.loop({ sessionID: chat.id }).pipe(Effect.forkChild)
      const b = yield* prompt.loop({ sessionID: chat.id }).pipe(Effect.forkChild)
      yield* Effect.yieldNow // cypher_change - give the queued loops a scheduler turn instead of a wall-clock window
...
unixNoLLMServer(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const loop = yield* prompt.loop({ sessionID: chat.id }).pipe(Effect.forkChild)
      yield* Effect.yieldNow // cypher_change - give the queued loop a scheduler turn before cancelling
...
noLLMServer.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(agents.map((agent) => agent.name)).toEqual(["code"]) // cypher_change
    }),
  {
    config: {
      ...cfg,
...
// cypher_change start - Cypher review command behavior
noLLMServer.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit)) {
        ...
        expect(NamedError.Unknown.isInstance(err)).toBe(true)
        if (NamedError.Unknown.isInstance(err)) {
          expect(err.data.message).toContain("code") // cypher_change - "build" renamed to "code"
        }
      }
    }),
  30_000,
