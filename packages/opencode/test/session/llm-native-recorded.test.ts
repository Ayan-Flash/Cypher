...
const CYPHER_FIXTURES_DIR = path.join(FIXTURES_DIR, "cypher") // cypher_change

const zenURL = (connection: string) => `https://console.opencode.ai/proxy/connections/${connection}/v1` // cypher_change
...
const cloneModel = (model: ModelsDev.Provider["models"][string]) => {
  ...
  if (typeof experimental === "boolean")
    return cloned as NonNullable<NonNullable<NonNullable<Config.Info["provider"]>[string]>["models"]>[string] // cypher_change
  ...
  return rest as NonNullable<NonNullable<NonNullable<Config.Info["provider"]>[string]>["models"]>[string] // cypher_change
}
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.CYPHER_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    canRecord: () => Boolean(envValue("CYPHER_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("CYPHER_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  {
    id: "opencode-proxy",
    name: "OpenCode proxy", // cypher_change
    providerID: ProviderID.opencode,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["opencode", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.CYPHER_RECORD_CONSOLE_TOKEN && process.env.CYPHER_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.opencode,
        name: "OpenCode Zen",
        env: ["CYPHER_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.CYPHER_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.CYPHER_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.CYPHER_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["opencode", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CYPHER_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("CYPHER_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("CYPHER_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const canRun = (scenario: RecordedScenario) =>
  ...
  shouldRecord
    ? scenario.canRecord()
    : HttpRecorder.hasCassetteSync(scenario.cassette, { directory: CYPHER_FIXTURES_DIR })
  // cypher_change end

const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set CYPHER_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  return Layer.mergeAll(
    ...
    LLM.layer.pipe(
      ...
      Layer.provide(
        HttpRecorder.Cassette.fileSystem({ directory: CYPHER_FIXTURES_DIR }).pipe(Layer.provide(NodeFileSystem.layer)), // cypher_change
      ),
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://app.cypher.ai/config.json", ...scenario.config(model) }), // cypher_change
    ),
  )
