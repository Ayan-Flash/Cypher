...
const originalEnv = {
  ...
  CYPHER_EXPERIMENTAL_WORKSPACES: process.env.CYPHER_EXPERIMENTAL_WORKSPACES,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CYPHER_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CYPHER_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CYPHER_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CYPHER_EXPERIMENTAL_WORKSPACES).toBe("true")
