...
import { CypherClient, type GlobalEvent } from "@cypher/sdk/v2"
...
type GlobalEventStream = Awaited<ReturnType<CypherClient["global"]["event"]>>["stream"]
type SessionMessage = NonNullable<Awaited<ReturnType<CypherClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<CypherClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<CypherClient["session"]["status"]>>["data"]>
...
function sdk(
  input: {
    ...
    globalEvent?: CypherClient["global"]["event"]
    promptAsync?: CypherClient["session"]["promptAsync"]
    status?: CypherClient["session"]["status"]
    messages?: CypherClient["session"]["messages"]
    children?: CypherClient["session"]["children"]
    permissions?: CypherClient["permission"]["list"]
    questions?: CypherClient["question"]["list"]
  } = {},
) {
  const client = new CypherClient()

  const globalEvent: CypherClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: CypherClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: CypherClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: CypherClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: CypherClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: CypherClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: CypherClient["question"]["list"] = input.questions ?? (() => ok([]))
