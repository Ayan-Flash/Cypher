...
// cypher_change start: rebrand fixtures off upstream repo path
test("parses https URL with .git suffix", () => {
  expect(parseGitHubRemote("https://github.com/Cypher-Org/cypher.git")).toEqual({
    owner: "Cypher-Org",
    repo: "cypher",
  })
})
...
test("parses https URL without .git suffix", () => {
  expect(parseGitHubRemote("https://github.com/Cypher-Org/cypher")).toEqual({ owner: "Cypher-Org", repo: "cypher" })
})

test("parses git@ URL with .git suffix", () => {
  expect(parseGitHubRemote("git@github.com:Cypher-Org/cypher.git")).toEqual({ owner: "Cypher-Org", repo: "cypher" })
})

test("parses git@ URL without .git suffix", () => {
  expect(parseGitHubRemote("git@github.com:Cypher-Org/cypher")).toEqual({ owner: "Cypher-Org", repo: "cypher" })
})

test("parses ssh:// URL with .git suffix", () => {
  expect(parseGitHubRemote("ssh://git@github.com/Cypher-Org/cypher.git")).toEqual({
    owner: "Cypher-Org",
    repo: "cypher",
  })
})
...
test("parses ssh:// URL without .git suffix", () => {
  expect(parseGitHubRemote("ssh://git@github.com/Cypher-Org/cypher")).toEqual({ owner: "Cypher-Org", repo: "cypher" })
})
// cypher_change end
