...
import type { TuiAttentionSoundPack } from "@cypher/plugin/tui"
...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const localDest = path.join(dir, ".cypher", "themes", localThemeFile) // cypher_change
      const globalDest = path.join(Global.Path.config, "themes", globalThemeFile)
      const preloadedDest = path.join(dir, ".cypher", "themes", preloadedThemeFile) // cypher_change
...
`,
  ...
  try {
    ...
    const leaked_global_to_local = await fs
      .stat(path.join(tmp.path, ".cypher", "themes", tmp.extra.globalThemeFile)) // cypher_change
      .then(() => true)
      .catch(() => false)
...
`,
  ...
  process.env.CYPHER_PLUGIN_META_FILE = path.join(tmp.path, "plugin-meta.json")
  const config = createTuiResolvedConfig({
    plugin: [
  ...
  try {
    ...
    cwd.mockRestore()
    wait.mockRestore()
    delete process.env.CYPHER_PLUGIN_META_FILE
  }
})
...
test("updates installed theme when plugin metadata changes", async () => {
  await using tmp = await tmpdir<{
    ...
    init: async (dir) => {
      ...
      const dest = path.join(dir, ".cypher", "themes", themeFile) // cypher_change
...
`,
    ...
    const list = await Filesystem.readJson<Record<string, { themes?: Record<string, { dest: string }> }>>(
      process.env.CYPHER_PLUGIN_META_FILE!,
    )
