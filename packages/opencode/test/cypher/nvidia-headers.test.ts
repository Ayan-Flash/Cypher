...
it.live("nvidia provider includes Cypher billing origin header", () =>
    ...
    withNvidiaKey(
      Provider.Service.use((provider) =>
        Effect.gen(function* () {
          ...
          expect(headers["HTTP-Referer"]).toBe("https://cypher.ai/")
          expect(headers["X-Title"]).toBe("Cypher CLI")
          expect(headers["X-BILLING-INVOKE-ORIGIN"]).toBe("Cypher")
        }),
      ),
    ),
  ),
