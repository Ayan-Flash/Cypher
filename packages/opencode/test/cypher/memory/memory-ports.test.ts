...
import { MemoryModel, MemorySession } from "../../../src/cypher/memory/ports"
...
describe("memory ports", () => {
  test("session port extracts the latest turn, recall markers, and all assistant steps", async () => {
    ...
    const diffs = [
      {
        file: "packages/opencode/src/cypher/memory/ports.ts",
        additions: 4,
        deletions: 1,
    ...
    expect(view?.recent).toContain("Tool cypher_memory_recall completed")
