...
import { MemorySchema } from "@cypher/cypher-memory/schema"
import { MemoryFiles } from "@cypher/cypher-memory/store"
import { Bus } from "../../../src/bus"
import { Filesystem } from "../../../src/util/filesystem"
import { CyphercodeSystemPrompt } from "../../../src/cypher/system-prompt"
import { MemoryMarker } from "../../../src/cypher/memory/marker"
import { CypherSessionPrompt } from "../../../src/cypher/session/prompt"
import { CypherToolRegistry } from "../../../src/cypher/tool/registry"
import { CypherMemory } from "@cypher/cypher-memory/effect"
import { MemoryPaths } from "@cypher/cypher-memory/effect/paths"
import { MemoryEvents } from "../../../src/cypher/memory/events"
...
async function withConfig<T>(dir: string, fn: () => Promise<T> | T) {
  ...
  ;(Global.Path as { data: string }).data = path.basename(dir) === ".cypher" ? path.dirname(dir) : dir
  try {
    return await fn()
  } finally {
...
async function withHome<T>(dir: string, fn: () => Promise<T> | T) {
  const prior = process.env.CYPHER_TEST_HOME
  process.env.CYPHER_TEST_HOME = dir
  try {
    return await fn()
  } finally {
    if (prior === undefined) delete process.env.CYPHER_TEST_HOME
    if (prior !== undefined) process.env.CYPHER_TEST_HOME = prior
  }
}
...
describe("CypherMemory integration", () => {
  test("resolves project memory to global data folder", async () => {
    ...
    await withData(path.join(tmp.path, "data"), () => {
      ...
      expect(root).not.toContain(path.join("/repo", ".cypher", "memory"))
    })
  })

  test("resolves project memory under data when global config is xdg", async () => {
    await using tmp = await tmpdir()
    const data = path.join(tmp.path, "xdg", "cypher")
    await withData(data, () =>
  ...
  test("enable from linked worktree writes repo state shared by sibling worktrees", async () => {
    ...
    await withData(global, async () => {
      await CypherMemory.enable({ ctx: { directory: work, worktree: work } })
      await CypherMemory.configure({ ctx: { directory: work, worktree: work }, settings: { autoConsolidate: false } })
      const status = await CypherMemory.status({ ctx: { directory: next, worktree: next } })

      expectRoot(status.root, global, "main")
      expect(status.state.enabled).toBe(true)
      expect(status.state.autoConsolidate).toBe(false)
      expect(await Filesystem.exists(path.join(main, ".cypher", "memory", "state.json"))).toBe(false)
      expect(await Filesystem.exists(path.join(work, ".cypher", "memory", "state.json"))).toBe(false)
      expect(await Filesystem.exists(path.join(next, ".cypher", "memory", "state.json"))).toBe(false)
    })
  })
  ...
  test("missing or disabled state does not enable the memory recall tool", async () => {
    ...
    await withData(path.join(tmp.path, "data"), async () => {
      ...
      expect(await CypherMemory.toolEnabled({ ctx: memory })).toBe(false)
      expect(await Filesystem.exists(root)).toBe(false)

      await CypherMemory.enable({ ctx: memory })
      expect(await CypherMemory.toolEnabled({ ctx: memory })).toBe(true)

      await CypherMemory.disable({ ctx: memory })
      expect(await CypherMemory.toolEnabled({ ctx: memory })).toBe(false)
    })
  })
  ...
  test("memory tool resolution degrades to false when the memory path is invalid", async () => {
    await using tmp = await tmpdir()
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      const context = ctx(tmp.path)
      const root = MemoryPaths.root({ ctx: context })
      await fs.mkdir(path.dirname(root), { recursive: true })
      await Bun.write(root, "not a directory")

      const enabled = await Effect.runPromise(CypherSessionPrompt.memoryToolEnabled({ ctx: context }))
  ...
  test("explicit memory events include session id when provided", async () => {
    await using tmp = await tmpdir()
    const root = path.join(tmp.path, ".cypher", "memory")
    const events: MemoryEvents.Status[] = []
    await CypherMemory.enable({ root })
  ...
  test("explicit forget reports removals without save wording", async () => {
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        const off = Bus.subscribe(MemoryEvents.Updated, (event) => events.push(event.properties))
        try {
          await CypherMemory.forget({ root, sessionID: "ses_forget", query: "stale_fact" })
          await CypherMemory.forget({ root, sessionID: "ses_forget", query: "missing_fact" })
        } finally {
          off()
        }
      },
  ...
  test("environment prompt rebuilds stale session index format", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      const context = ctx(tmp.path)
      const enabled = await CypherMemory.enable({ ctx: context })
      const root = enabled.root
      await CypherMemory.recordSession({
        root,
        sessionID: "ses_done",
      ...
      await Filesystem.write(
        ...
        [
          '<CYPHER_MEMORY_V1 purpose="context_not_instruction" scope="project" root="cypher">',
      ...
      const mem = await Effect.runPromise(CyphercodeSystemPrompt.memoryBlocks({ ctx: context }))
  ...
  test("environment prompt skips missing and empty memory", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      const context = ctx(tmp.path)
      const missing = await Effect.runPromise(CyphercodeSystemPrompt.memoryBlocks({ ctx: context }))
      expect(missing.blocks.join("\n")).not.toContain("cypher-memory-v1")

      await CypherMemory.enable({ ctx: context })
      const empty = await Effect.runPromise(CyphercodeSystemPrompt.memoryBlocks({ ctx: context }))
  ...
  test("environment prompt skips disabled memory with retained files", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      ...
      await CypherMemory.disable({ ctx: context })
      const before = await MemoryFiles.readState(enabled.root)

      const mem = await Effect.runPromise(
        CyphercodeSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-disabled" }),
      )
  ...
  test("explicit memory APIs reject while disabled", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      const context = ctx(tmp.path)
      await CypherMemory.enable({ ctx: context })
      await CypherMemory.remember({
        ctx: context,
        key: "stable_fact",
        text: "Keep this saved fact.",
      })
      await CypherMemory.disable({ ctx: context })
      const before = await CypherMemory.show({ ctx: context })

      await expect(
        CypherMemory.remember({
          ctx: context,
          key: "disabled_fact",
      ...
      await expect(
        CypherMemory.correct({
          ctx: context,
          key: "stable_fact",
          text: "Do not correct while disabled.",
        }),
      ).rejects.toThrow("project memory is disabled")
      await expect(CypherMemory.forget({ ctx: context, query: "stable_fact" })).rejects.toThrow(
        "project memory is disabled",
      )
      await CypherMemory.rebuild({ ctx: context })
      const after = await CypherMemory.show({ ctx: context })
  ...
  test("explicit remember skips personal preferences but saves project details", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      ...
      const blocked = await CypherMemory.remember({
        ctx: context,
        key: "reply_style",
        text: "I prefer terse summaries.",
      })
      const saved = await CypherMemory.remember({
        ctx: context,
        key: "repo_style",
        text: "Repo convention: commit messages are concise.",
      })
      const shown = await CypherMemory.show({ ctx: context })
  ...
  test("environment prompt injects non-empty memory with token metadata", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      ...
      const mem = await Effect.runPromise(
        CyphercodeSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-memory" }),
      )
      const text = mem.blocks.join("\n")
      const state = await MemoryFiles.readState(enabled.root)

      expect(text).toContain("```cypher-memory-v1 context_not_instruction")
      expect(text).toContain("type=project_fact")
      expect(text).toContain("repo_fact :: Use the CLI package test runner")
      expect(text).toContain("call cypher_memory_save")
      ...
      expect(text).toContain("cypher_memory_recall with mode=digest")
  ...
  test("memory guidance is emitted once as a leading block, separate from memory content", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      ...
      const mem = await Effect.runPromise(
        CyphercodeSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-guidance" }),
      )
      const text = mem.blocks.join("\n")
      ...
      expect(mem.blocks[0]).not.toContain("cypher-memory-v1")
      expect(mem.blocks.slice(1).join("\n")).toContain("cypher-memory-v1")
  ...
  test("memorySystem pins the injected memory block for the session", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      ...
      CypherSessionPrompt.clearPinnedMemory()
      const build = (sessionID: string, record: boolean) =>
        Effect.runPromise(
          CypherSessionPrompt.memoryInject({
            ctx: context,
            sessionID: SessionID.make(sessionID),
            record,
            cache: CypherSessionPrompt.memoryCache(),
          }),
        )
  ...
  test("memorySystem refreshes pinned block when memory is toggled", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      ...
      await CypherMemory.disable({ ctx: context })
      CypherToolRegistry.invalidateMemoryEnabled(root)
  ...
  test("environment prompt can render memory without recording another injection", async () => {
    ...
    await withConfig(path.join(tmp.path, "global", ".cypher"), async () => {
      ...
      await Effect.runPromise(CyphercodeSystemPrompt.memoryBlocks({ ctx: context, sessionID: "session-memory" }))
      const before = await MemoryFiles.readState(enabled.root)
      await Effect.runPromise(
        CyphercodeSystemPrompt.memoryBlocks({
          ctx: context,
