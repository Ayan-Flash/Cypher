...
import { remove } from "../../src/cypher/agent"
import type { Info as AgentInfo } from "../../src/agent/agent"
import { tmpdir } from "../fixture/fixture"

describe("Cypher agent remove", () => {
  test("removes config-backed imported agents", async () => {
    await using tmp = await tmpdir()
    const dir = path.join(tmp.path, ".cypher")
    const file = path.join(dir, "cypher.jsonc")
  ...
  "agent": {
    ...
    expect(cfg.agent.code.model).toBe("cypher/gpt-5")
  })
})
