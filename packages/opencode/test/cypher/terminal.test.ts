...
import { kitty, sequences } from "../../src/cypher/cli/cmd/tui/util/terminal"

const keys = ["TERM_PROGRAM", "MSYSTEM", "CYPHER_DISABLE_KITTY_KEYBOARD", "CYPHER_ENABLE_KITTY_KEYBOARD"] as const
...
test("allows explicitly enabling Kitty keyboard", () => {
  env({ CYPHER_ENABLE_KITTY_KEYBOARD: "1", MSYSTEM: "MINGW64" })
...
test("allows explicitly disabling Kitty keyboard", () => {
  env({ CYPHER_DISABLE_KITTY_KEYBOARD: "1", CYPHER_ENABLE_KITTY_KEYBOARD: "1" })
...
test("resets common terminal input modes", () => {
  env({ CYPHER_DISABLE_KITTY_KEYBOARD: "1" })
