...
import { plain, session, supports, tui } from "../../src/cypher/cli/logo"

describe("cypher logo", () => {
  ...
  test("allows an override", () => {
    expect(supports({ CYPHER_UNICODE_LOGO: "1", SSH_TTY: "/dev/pts/0" }, "linux")).toBe(true)
    expect(supports({ CYPHER_UNICODE_LOGO: "0" }, "linux")).toBe(false)
  })

  test("uses modern and fallback logo variants", () => {
    expect(tui({ CYPHER_UNICODE_LOGO: "1" }, "linux").join("\n")).toContain("🬺🬏")
