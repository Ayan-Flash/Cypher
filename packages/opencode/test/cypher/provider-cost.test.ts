...
const cypher = { id: "cypher" } as Provider.Info

// Calculated cost for the `model()` + `baseUsage` pair: 1M input * $3 + 100k output * $15 = 3 + 1.5
const fallback = 3 + 1.5

describe("CypherSession.providerCost — Anthropic Messages / OpenAI Responses", () => {
...
describe("CypherSession.providerCost — Vercel AI Gateway", () => {
    ...
    const result = SessionNs.getUsage({
      ...
      metadata: {
        gateway: {
          ...
          // which Cypher doesn't pass on to end users — must be ignored.
          cost: "0",
...
describe("CypherSession.providerCost — fallback", () => {
