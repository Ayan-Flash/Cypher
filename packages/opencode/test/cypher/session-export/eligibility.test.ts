...
import { isEligible, setKillSwitch, resetEligibility, type OrgState } from "@/cypher/session-export/eligibility"

const base = {
  model: {
    api: { npm: "@cypher/cypher-gateway" },
    isFree: true,
  },
...
describe("isEligible", () => {
  ...
  test("free Cypher Gateway personal context is eligible", () => {
    expect(isEligible(base)).toBe(true)
  })

  test("paid Cypher Gateway is ineligible", () => {
  ...
  test("non-Cypher provider with isFree=true is ineligible", () => {
