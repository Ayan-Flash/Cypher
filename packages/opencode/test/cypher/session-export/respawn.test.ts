...
import { getKillSwitchReason, resetEligibility } from "@/cypher/session-export/eligibility"
...
describe("SessionExport worker respawn", () => {
  ...
  beforeEach(async () => {
    ...
    feature = process.env.CYPHER_FEATURE
    resetEligibility()
  })
  ...
  afterEach(async () => {
    await SessionExport.shutdown()
    resetEligibility()
    if (feature === undefined) delete process.env.CYPHER_FEATURE
    else process.env.CYPHER_FEATURE = feature
  })

  test("passes surface to worker init", () => {
    const workers: FakeWorker[] = []
    process.env.CYPHER_FEATURE = "cli"
    SessionExport.init({
      agentVersion: "v0",
      dbPath: ":memory:",
      subscribeAll: () => () => {},
