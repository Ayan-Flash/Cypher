...
import { getActiveOrg, resetOrgSource, setOrgSource } from "@/cypher/session-export/eligibility"

const env = process.env.CYPHER_ORG_ID

describe("getActiveOrg", () => {
  beforeEach(() => {
    delete process.env.CYPHER_ORG_ID
    resetOrgSource()
  })

  afterEach(() => {
    if (env === undefined) delete process.env.CYPHER_ORG_ID
    else process.env.CYPHER_ORG_ID = env
    resetOrgSource()
  })
  ...
  test("returns env value when CYPHER_ORG_ID is set", async () => {
    setOrgSource(async () => ({ type: "org", id: "org_auth" }))
    process.env.CYPHER_ORG_ID = "org_envvar"
