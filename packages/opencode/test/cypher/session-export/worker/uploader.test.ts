...
import { Storage } from "@/cypher/session-export/worker/storage"
import { Uploader, backoffFor } from "@/cypher/session-export/worker/uploader"
import { Config } from "@/cypher/session-export/config"
...
describe("Uploader", () => {
  ...
  beforeEach(() => {
    token = process.env.CYPHER_SESSION_EXPORT_AUTH_TOKEN
    delete process.env.CYPHER_SESSION_EXPORT_AUTH_TOKEN
  ...
  afterEach(() => {
    ...
    if (token === undefined) delete process.env.CYPHER_SESSION_EXPORT_AUTH_TOKEN
    else process.env.CYPHER_SESSION_EXPORT_AUTH_TOKEN = token
  })

  test("2xx response marks rows uploaded and deletes them", async () => {
    const telemetry: unknown[] = []
    const calls: Array<{ input: string; init: RequestInit }> = []
    process.env.CYPHER_SESSION_EXPORT_AUTH_TOKEN = "local-token"
    ...
    expect(headers.get("x-cypher-export-api-version")).toBe("1")
    expect(headers.get("x-cypher-export-schema-version")).toBe("1")
    expect(headers.get("x-cypher-export-agent-version")).toBe("v0")
    expect(headers.get("x-cypher-export-root-session-id")).toBe("s1")
    expect(headers.get("x-cypher-export-session-id")).toBe("s1")
    expect(headers.get("x-cypher-export-seq-start")).toBe("0")
    expect(headers.get("x-cypher-export-seq-end")).toBe("0")
    expect(headers.get("x-cypher-export-event-count")).toBe("1")
    expect(headers.get("x-cypher-export-content-encoding")).toBe("identity")
    expect(headers.get("x-cypher-export-client-sent-at")).toMatch(/^\d{4}-\d{2}-\d{2}T/)
    expect(headers.get("x-cypher-export-payload-sha256")).toBe(await sha256(body))
  ...
  test("sends anonymous id with all export headers when auth token is absent", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    const names = [
      "x-cypher-export-api-version",
      "x-cypher-export-schema-version",
      "x-cypher-export-agent-version",
      "x-cypher-export-surface",
      "x-cypher-export-root-session-id",
      "x-cypher-export-session-id",
      "x-cypher-export-batch-id",
      "x-cypher-export-seq-start",
      "x-cypher-export-seq-end",
      "x-cypher-export-event-count",
      "x-cypher-export-payload-sha256",
      "x-cypher-export-client-sent-at",
      "x-cypher-export-content-encoding",
    ]

    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-cypher-anon-id")).toBe("install_abc")
  ...
  test("falls back to telemetry id when anon id is not provided", async () => {
    ...
    const headers = new Headers(calls[0].init.headers)
    expect(headers.get("authorization")).toBeNull()
    expect(headers.get("x-cypher-anon-id")).toBe("install_from_disk")
  })
  ...
  test("includes surface in batch metadata and upload headers", async () => {
    ...
    expect(headers.get("x-cypher-export-surface")).toBe("vscode-extension")
