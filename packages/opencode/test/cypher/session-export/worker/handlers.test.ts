...
import { Chunker } from "@/cypher/session-export/worker/chunks"
import { Scrubber } from "@/cypher/session-export/worker/scrub"
import { handleEvent } from "@/cypher/session-export/worker/handlers"
import type {
  CompactionCaptured,
  LlmRequestCompleted,
  LlmRequestStarted,
...
} from "@/cypher/session-export/events"
...
function started(id: string, input: Partial<LlmRequestStarted["input"]>): LlmRequestStarted {
  return {
    ...
    agent: "claude",
    modeId: "build",
    model: { providerId: "cypher", modelId: "free-1", isFree: true },
