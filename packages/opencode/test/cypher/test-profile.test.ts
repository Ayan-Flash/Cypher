...
import { TestProfile } from "../../script/cypher/test-profile"
...
describe("test profiles", () => {
  test("darwin profile contains valid test files", () => {
    ...
    expect(result.files).toContain("cypher/cli/install-artifact.test.ts")
    expect(result.files).toContain("cypher/sandbox/macos-confinement.test.ts")
    expect(result.files).toContain("file/watcher.test.ts")
    expect(result.files).toContain("cypher/interactive-terminal.test.ts")
    const sandbox = all.filter((file) => file.startsWith("cypher/sandbox/"))
    expect(result.files.filter((file) => file.startsWith("cypher/sandbox/"))).toEqual(sandbox)
    ...
    expect(result.files).not.toContain("cypher/sessions/remote-ws.test.ts")
