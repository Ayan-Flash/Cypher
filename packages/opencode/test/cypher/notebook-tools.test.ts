...
import { NotebookEditTool, NotebookExecuteTool, NotebookReadTool } from "@/cypher/tool/notebook-host"
...
test("uses dedicated VS Code notebook permission defaults only when enabled", () => {
  ...
  try {
    ...
    const disabled = CypherAgent.prepare({}).defaultsPatch
    expect(disabled.some((rule) => rule.permission.startsWith("notebook_"))).toBe(false)

    const rules = CypherAgent.prepare({ experimental: { native_notebook_tools: true } }).defaultsPatch
