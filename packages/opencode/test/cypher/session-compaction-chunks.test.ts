...
import { CypherCompactionChunks } from "../../src/cypher/session/compaction-chunks"
import { CypherSessionCompaction } from "../../src/cypher/session/compaction"
...
describe("CypherCompactionChunks", () => {
  test("splits oversized history into chronological chunks", async () => {
    ...
    const chunks = await Effect.runPromise(CypherCompactionChunks.split({ messages, model, size: 2_000 }))
  ...
  test("uses runtime output cap for fallback selection and chunk budget", () => {
    ...
    expect(CypherCompactionChunks.needed({ cfg, model, tokens: 5_000, outputTokenMax })).toBe(false)
    expect(CypherCompactionChunks.budget({ cfg, model, outputTokenMax })).toBe(5_692)
  })
