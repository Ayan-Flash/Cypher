...
test("extracts ZIP paths with PowerShell metacharacters literally", async () => {
  ...
  const cmd = "Compress-Archive -LiteralPath $env:CYPHER_TEST_SOURCE -DestinationPath $env:CYPHER_TEST_ARCHIVE -Force"

  await fs.writeFile(source, "safe")
  await Process.run(["powershell", "-NoProfile", "-NonInteractive", "-Command", cmd], {
    env: {
      CYPHER_TEST_SOURCE: source,
      CYPHER_TEST_ARCHIVE: archive,
    },
  })
