...
} from "../../src/cypher/indexing-feature"
...
describe("indexing plugin helpers", () => {
  test("detects plugin-enabled configs", () => {
    ...
    expect(indexingEnabled({ plugin: ["@cypher/cypher-indexing@1.0.0"] })).toBe(true)
  })
  ...
  test("does not add duplicate indexing plugin", () => {
    const list = ensureIndexingPlugin(["@cypher/cypher-indexing@1.0.0"], INDEXING_PLUGIN)
    expect(list).toEqual(["@cypher/cypher-indexing@1.0.0"])
  })
