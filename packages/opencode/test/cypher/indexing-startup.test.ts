...
import { CodeIndexManager } from "@cypher/cypher-indexing/engine"
import { normalizeIndexingStatus } from "@cypher/cypher-indexing/status"
import type { Config } from "../../src/config/config"
import { GlobalBus } from "../../src/bus/global"
import { WorkspaceID } from "../../src/control-plane/schema"
import { WorkspaceContext } from "../../src/control-plane/workspace-context"
import { CypherIndexing } from "../../src/cypher/indexing"
import { indexingWarningKey } from "../../src/cypher/indexing-warning"
import { IndexingWorker } from "../../src/cypher/indexing-worker-client"
...
const cypher: Partial<Config.Info> = {
  plugin: ["@cypher/cypher-indexing"],
  indexing: {
    enabled: true,
    vectorStore: "qdrant",
...
const staleCypher: Partial<Config.Info> = {
  plugin: ["@cypher/cypher-indexing"],
  indexing: {
    enabled: true,
    provider: "cypher",
...
const configDir = process.env["CYPHER_CONFIG_DIR"]
const disabled = process.env["CYPHER_DISABLE_CODEBASE_INDEXING"]
...
async function wait(read: () => Promise<CypherIndexing.Status>, state: CypherIndexing.Status["state"]) {
...
afterEach(async () => {
  ...
  if (configDir === undefined) delete process.env["CYPHER_CONFIG_DIR"]
  else process.env["CYPHER_CONFIG_DIR"] = configDir
  if (disabled === undefined) delete process.env["CYPHER_DISABLE_CODEBASE_INDEXING"]
  else process.env["CYPHER_DISABLE_CODEBASE_INDEXING"] = disabled
  global.fetch = fetch
  await disposeAllInstances()
})

describe("indexing model catalog", () => {
  test("ignores a project-scoped Cypher origin", async () => {
    await using tmp = await tmpdir({
      git: true,
      init: async (dir) => {
        const global = path.join(dir, "global")
        const project = path.join(dir, "project")
        await fs.mkdir(path.join(project, ".cypher"), { recursive: true })
        await fs.mkdir(global, { recursive: true })
        await Bun.write(path.join(global, "cypher.jsonc"), "{}")
        await Bun.write(
          path.join(project, ".cypher", "cypher.jsonc"),
          JSON.stringify({ indexing: { cypher: { baseUrl: "http://127.0.0.1:4567" } } }),
        )
        return { global, project }
      },
    })
    process.env["CYPHER_CONFIG_DIR"] = tmp.extra.global
    const calls: string[] = []
    ...
    const response = await Server.Default().app.request("/indexing/models", {
      headers: { "x-cypher-directory": tmp.extra.project },
    })
...
describe("indexing startup degradation", () => {
  test("keeps server routes alive when indexing initialization fails", async () => {
    ...
    process.env["CYPHER_CONFIG_DIR"] = tmp.path
  ...
  test("retains and deduplicates indexing warnings for TUI replay", async () => {
    ...
    const on = (data: {
      ...
      if (data.payload?.type !== CypherIndexing.Warning.type) return
  ...
  test("coalesces burst indexing progress publications", async () => {
    const complete: CypherIndexing.Status = {
      state: "Complete",
    ...
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["CYPHER_CONFIG_DIR"] = tmp.path
    const events: CypherIndexing.Status[] = []
    const on = (data: {
      directory?: string
      payload?: { type?: string; properties?: { status?: CypherIndexing.Status } }
    }) => {
      if (data.directory !== tmp.path) return
      if (data.payload?.type !== CypherIndexing.Event.type) return
    ...
    try {
      await withTestInstance({
        directory: tmp.path,
        fn: async () => expect(await wait(() => CypherIndexing.current(), "Complete")).toEqual(complete),
      })
  ...
  test("does not publish initialized status after in-flight startup is disposed", async () => {
    ...
    try {
      await provideTestInstance({
        directory: tmp.path,
        init: Effect.promise(() => CypherIndexing.init()),
        fn: async () => {
          await called(init)
          expect((await CypherIndexing.current()).state).toBe("In Progress")
        },
      })
  ...
  test("keeps degraded indexing queryable but releases its failed engine", async () => {
    ...
    try {
      await provideTestInstance({
        ...
        init: Effect.promise(() => CypherIndexing.init()),
        fn: async () => {
          const status = await wait(() => CypherIndexing.current(), "Error")

          expect(status.state).toBe("Error")
          expect(status.message).toContain("Failed to initialize: test indexing initialization failed")
          expect(await CypherIndexing.available()).toBe(false)
          expect(CypherIndexing.ready()).toBe(false)
          expect(await CypherIndexing.search("boot failure")).toEqual([])
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(CypherIndexing.ready()).toBe(false)
          expect(await CypherIndexing.available()).toBe(false)
          const search = CypherIndexing.search("boot failure")
  ...
  test("stays disabled when indexing enablement is unset", async () => {
    ...
    await provideTestInstance({
      directory: tmp.path,
      fn: async () => {
        const status = await wait(() => CypherIndexing.current(), "Disabled")
        ...
        expect(await CypherIndexing.available()).toBe(false)
        expect(CypherIndexing.ready()).toBe(false)
        expect(await CypherIndexing.search("disabled")).toEqual([])
  ...
  test("enriches Cypher provider config from env auth", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    const key = process.env.CYPHER_API_KEY
    const org = process.env.CYPHER_ORG_ID

    await using tmp = await tmpdir({ git: true, config: cypher })
    process.env["CYPHER_CONFIG_DIR"] = tmp.path
    process.env.CYPHER_API_KEY = "cypher-token"
    process.env.CYPHER_ORG_ID = "org_123"
    ...
    try {
      await provideTestInstance({
        ...
        fn: async () => {
          ...
          expect(init.mock.calls[0]?.[0]).toMatchObject({
            embedderProvider: "cypher",
            cypherApiKey: "cypher-token",
            cypherOrganizationId: "org_123",
      ...
      if (key === undefined) delete process.env.CYPHER_API_KEY
      else process.env.CYPHER_API_KEY = key
      if (org === undefined) delete process.env.CYPHER_ORG_ID
      else process.env.CYPHER_ORG_ID = org
      init.mockRestore()
    }
  })

  test("falls back from unsupported stored Cypher models to the hosted default", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
    ...
    await using tmp = await tmpdir({ git: true, config: staleCypher })
    process.env["CYPHER_CONFIG_DIR"] = tmp.path
    process.env.CYPHER_API_KEY = "cypher-token"
  ...
  test("uses hosted dimensions for supported Cypher models", async () => {
    global.fetch = (() =>
      Promise.resolve(
        new Response(
          JSON.stringify({
  ...
  test("leaves Cypher model metadata unset when the hosted catalog is unavailable", async () => {
  ...
  test("does not default to Cypher when an existing provider config is present", async () => {
    const init = spyOn(CodeIndexManager.prototype, "initialize").mockResolvedValue({ requiresRestart: false })
    const key = process.env.CYPHER_API_KEY
  ...
  test("stays disabled when VS Code starts without a workspace folder", async () => {
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["CYPHER_CONFIG_DIR"] = tmp.path
    process.env["CYPHER_DISABLE_CODEBASE_INDEXING"] = "vscode-no-workspace"
    const init = spyOn(CodeIndexManager.prototype, "initialize")
    ...
    try {
      await provideTestInstance({
        ...
        init: Effect.promise(() => CypherIndexing.init()),
        fn: async () => {
          const status = await CypherIndexing.current()
          ...
          expect(await CypherIndexing.available()).toBe(false)
          expect(CypherIndexing.ready()).toBe(false)
          expect(await CypherIndexing.search("no workspace")).toEqual([])
          expect(init).not.toHaveBeenCalled()
        },
      })
    } finally {
