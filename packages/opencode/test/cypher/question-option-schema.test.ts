/**
 * Contract tests for Cypher-specific fields on QuestionOption / QuestionInfo.
 *
 * packages/opencode/src/question/index.ts is a shared upstream file.
 * Two Cypher additions to the Option schema have been silently dropped by
...
describe("QuestionOption schema — Cypher-specific field contract", () => {
  ...
  // Static source checks — guard the cypher_change markers so a conflict
  // resolution that drops the fields is caught immediately.
  test("source declares mode as an optional field inside a cypher_change block", () => {
    const src = fs.readFileSync(SOURCE, "utf-8")
    expect(src).toMatch(/cypher_change start[^\n]*hint to UI clients/)
    expect(src).toMatch(/mode:\s*Schema\.optional\(Schema\.String\)/)
    expect(src).toMatch(/cypher_change end/)
  })

  test("source declares labelKey and descriptionKey inside a cypher_change block", () => {
    const src = fs.readFileSync(SOURCE, "utf-8")
    expect(src).toMatch(/cypher_change start[^\n]*i18n keys/)
