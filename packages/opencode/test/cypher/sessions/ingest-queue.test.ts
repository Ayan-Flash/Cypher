...
import { IngestQueue } from "../../../src/cypher-sessions/ingest-queue"
...
describe("share ingest queue", () => {
  ...
  test("cypher_meta uses stable key and coalesces", async () => {
    const sent: unknown[] = []
    const sched = scheduler(() => clock.now)
    ...
    await q.sync("s7", [
      { type: "cypher_meta", data: { platform: "cli", gitUrl: "https://github.com/old/repo.git", gitBranch: "main" } },
    ])
    clock.now = 100
    await q.sync("s7", [
      {
    ...
    expect(sent.length).toBe(1)
    expect((sent[0] as any).data.length).toBe(1)
    expect((sent[0] as any).data[0].type).toBe("cypher_meta")
