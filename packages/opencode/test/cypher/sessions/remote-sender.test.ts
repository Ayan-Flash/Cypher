...
import { RemoteModelCatalog } from "../../../src/cypher-sessions/remote-model-catalog"
import { RemoteSender } from "../../../src/cypher-sessions/remote-sender"
import type { RemoteWS } from "../../../src/cypher-sessions/remote-ws"
import type { RemoteProtocol } from "../../../src/cypher-sessions/remote-protocol"
...
import { Suggestion } from "../../../src/cypher/suggestion" // cypher_change
...
describe("RemoteSender", () => {
    ...
    expect(calls).toEqual([
      {
        ...
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderID.make("cypher"), modelID: ModelID.make("anthropic/claude-sonnet-4-20250514") },
      },
    ])
  })

  test("send_message keeps cypher-prefixed model unchanged before internal conversion", async () => {
    const { conn } = fakeConn()
    const calls: SessionPrompt.PromptInput[] = []
    const sender = RemoteSender.create({
      conn,
    ...
    expect(calls).toEqual([
      {
        sessionID: SessionID.make("ses_x"),
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderID.make("cypher"), modelID: ModelID.make("gpt-5-mini") },
      },
    ])
  })
  ...
  test("send_message does not special-case cypher-prefixed model", async () => {
    const { conn, sent } = fakeConn()
    const calls: SessionPrompt.PromptInput[] = []
    const sender = RemoteSender.create({
      conn,
    ...
    expect(sent[0]).toEqual({ type: "response", id: "req_model_cypher", result: {} })
    expect(calls).toEqual([
      {
        sessionID: SessionID.make("ses_x"),
        parts: [{ type: "text", text: "hello" }],
        model: { providerID: ProviderID.make("cypher"), modelID: ModelID.make("cypher/gpt-5-mini") },
      },
    ])
  })
  // cypher_change end
