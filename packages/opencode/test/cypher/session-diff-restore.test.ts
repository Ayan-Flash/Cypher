...
import { appendSessionDiffs, mergeSessionDiffs } from "../../src/cypher/session-portability/cumulative-diff"
import { extractSessionDiffs, restoreSessionDiffs } from "../../src/cypher/session-portability/session-diff-restore"

const dirs: string[] = []

function tmp() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "cypher-session-diff-"))
  dirs.push(dir)
  return dir
}
