...
import { CypherHeadless } from "../../src/cypher/permission/headless"
...
it.live("headless run: subagent permission asks fail instead of waiting forever", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* () {
      ...
      expect(CypherHeadless.denies(child.id)).toBe(true)
      expect(CypherHeadless.denies(root.id)).toBe(false)

      CypherHeadless.clear(root.id)
    }),
    { git: true },
  ),
)
...
it.live("interactive run: subagent permission asks still queue for a human reply", () =>
  provideTmpdirServer(
    Effect.fnUntraced(function* () {
      ...
      const fiber = yield* CypherSessionPrompt.askPermission({
        permission,
        agents: { get: () => Effect.succeed(agent) },
        sessions,
        agent,
