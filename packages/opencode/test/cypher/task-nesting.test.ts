...
import { BackgroundProcess } from "../../src/cypher/background-process"
...
import * as SandboxPolicy from "../../src/cypher/sandbox/policy"
...
describe("Cypher task nesting", () => {
  ...
  test("preserves inherited restrictions while refreshing prompt tool toggles", () => {
    const permission = CypherSessionPrompt.mergeToolPermissions({
      existing: [
      ...
      () =>
        Effect.gen(function* () {
          ...
          const effective = Permission.merge(
            ...
            CypherSessionPrompt.guardPermissions({ agent: validator, session: child }),
          )
