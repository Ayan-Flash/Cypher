...
`,
      ...
      const client = process.env["CYPHER_CLIENT"]
      ...
      try {
        ...
        yield* Effect.promise(() => BackgroundProcess.stopSession(sessionID))
        if (client === undefined) delete process.env["CYPHER_CLIENT"]
        else process.env["CYPHER_CLIENT"] = client
      }
    }),
  )
...
`,
      ...
      const client = process.env["CYPHER_CLIENT"]
      const scans = process.env["CYPHER_BACKGROUND_PROCESS_PORTS"]
      process.env["CYPHER_CLIENT"] = "cli"
      process.env["CYPHER_BACKGROUND_PROCESS_PORTS"] = "true"
      ...
      try {
        ...
        if (scans === undefined) delete process.env["CYPHER_BACKGROUND_PROCESS_PORTS"]
        else process.env["CYPHER_BACKGROUND_PROCESS_PORTS"] = scans
      }
    }),
  )
...
`,
      ...
      const scans = process.env["CYPHER_BACKGROUND_PROCESS_PORTS"]
      process.env["CYPHER_CLIENT"] = "cli"
      delete process.env["CYPHER_BACKGROUND_PROCESS_PORTS"]
