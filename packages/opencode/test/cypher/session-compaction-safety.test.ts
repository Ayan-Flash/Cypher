// Unit tests for CypherSessionPrompt history-trim / media-strip helpers.
...
import { CypherSessionMessageOrder } from "../../src/cypher/session/message-order"
...
describe("CypherSessionPrompt.hasCompletedSummary", () => {
  test("returns false for empty array", () => {
    expect(CypherSessionPrompt.hasCompletedSummary([])).toBe(false)
  })
  ...
  test("returns false when only summary has an error", () => {
    ...
    expect(CypherSessionPrompt.hasCompletedSummary(msgs)).toBe(false)
  })
  ...
  test("returns true when summary has finish and no error", () => {
    ...
    expect(CypherSessionPrompt.hasCompletedSummary(msgs)).toBe(true)
  })
})
...
describe("MessageV2.latest", () => {
  test("selects chronological state after retained pre-compaction tail", () => {
    ...
    const state = CypherSessionMessageOrder.latest(msgs)
  ...
  test("keeps queued subtasks moved after a chronologically later assistant", () => {
    ...
    CypherSessionMessageOrder.annotate([active, queued, done])

    const state = CypherSessionMessageOrder.latest([active, done, queued])
...
describe("CypherSessionPrompt.trimBeforeLastSummary", () => {
  ...
  test("trims to summary's text-user parent (reference-session bug)", () => {
    ...
    const result = CypherSessionPrompt.trimBeforeLastSummary(filtered)
  ...
  test("keeps the newest summary when retained history contains an older summary", () => {
    ...
    expect(CypherSessionPrompt.trimBeforeLastSummary(filtered)).toBe(filtered)
  })
  ...
  test("is idempotent", () => {
    ...
    const first = CypherSessionPrompt.trimBeforeLastSummary(msgs)
    const second = CypherSessionPrompt.trimBeforeLastSummary(first)
...
describe("CypherSessionPrompt.stripHistoricalMedia", () => {
  test("replaces image file part in historical user message with placeholder text", () => {
    ...
    const result = CypherSessionPrompt.stripHistoricalMedia(msgs)
  ...
  test("does not mutate the input", () => {
    ...
    const result = CypherSessionPrompt.stripHistoricalMedia(input)
...
describe("CypherSessionPrompt.maybeStripHistoricalMedia", () => {
  test("returns input unchanged when no completed summary exists", () => {
    const msgs = [
      user("msg_hist", [filePart("msg_hist", "image/png", "hist.png")]),
      user("msg_last", [textPart("msg_last", "follow-up")]),
    ]
    const result = CypherSessionPrompt.maybeStripHistoricalMedia(msgs)
    expect(result).toBe(msgs)
  })
