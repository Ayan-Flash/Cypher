    ...
    () =>
      provideTmpdirServer(
        Effect.fnUntraced(function* ({ llm }) {
          ...
          const turnClose = yield* Deferred.make<CypherSession.CloseReason>()
          const unsub = yield* bus.subscribeCallback(CypherSession.Event.TurnClose, (evt) => {
          ...
          expect(yield* llm.calls).toBe(CypherSessionPrompt.MAX_COMPACTION_ATTEMPTS * 2 + 1)
...
describe("CypherSessionPrompt.guardCompactionAttempt", () => {
  it.effect("returns { exhausted: false } and does not mutate state below the cap", () =>
    Effect.sync(() => {
      ...
      const result = CypherSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_under",
        attempts: CypherSessionPrompt.MAX_COMPACTION_ATTEMPTS - 1,
        closeReasons,
        message: msg,
      })
  ...
  it.effect("sets close reason and attaches error once attempts reach the cap", () =>
    Effect.sync(() => {
      ...
      const result = CypherSessionPrompt.guardCompactionAttempt({
        sessionID: "ses_cap",
        attempts: CypherSessionPrompt.MAX_COMPACTION_ATTEMPTS,
        closeReasons,
        message: msg,
      })
