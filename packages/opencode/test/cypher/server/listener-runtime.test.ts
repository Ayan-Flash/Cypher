...
const previous = {
  flag: Flag.CYPHER_SERVER_PASSWORD,
  env: process.env.CYPHER_SERVER_PASSWORD,
}

afterEach(async () => {
  Flag.CYPHER_SERVER_PASSWORD = previous.flag
  if (previous.env === undefined) delete process.env.CYPHER_SERVER_PASSWORD
  else process.env.CYPHER_SERVER_PASSWORD = previous.env
  ...
  try {
    ...
    try {
      const response = await fetch(new URL(SessionPaths.abort.replace(":sessionID", sessionID), listener.url), {
        method: "POST",
        headers: { "x-cypher-directory": tmp.path },
      })
