...
const state = Layer.effectDiscard(
  Effect.gen(function* () {
    const original = Flag.CYPHER_EXPERIMENTAL_WORKSPACES
    Flag.CYPHER_EXPERIMENTAL_WORKSPACES = true
    yield* Effect.addFinalizer(() =>
      Effect.promise(async () => {
        Flag.CYPHER_EXPERIMENTAL_WORKSPACES = original
        await resetDatabase()
      }),
    )
  }),
...
describe("Cypher Console worktree listing", () => {
  run(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const directory = path.join(test.directory, ".cypher", "worktrees", "console-list")
