...
import { CypherGatewayApi, CypherGatewayPaths } from "../../../src/cypher/server/httpapi/groups/cypher-gateway"
import { cypherGatewayHandlers } from "../../../src/cypher/server/httpapi/handlers/cypher-gateway"
...
const TestHttpApi = HttpApi.make("opencode-instance").addHttpApi(CypherGatewayApi)
...
const layer = HttpRouter.serve(
  HttpApiBuilder.layer(TestHttpApi).pipe(
    Layer.provide(cypherGatewayHandlers),
...
describe("Cypher gateway HttpApi statuses", () => {
  it.live("reports locally stored API authentication without a Gateway request", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(CypherGatewayPaths.authStatus)
  ...
  it.live("preserves cloud session list rate limits", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(CypherGatewayPaths.cloudSessions)
  ...
  it.live("preserves missing cloud session previews", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(CypherGatewayPaths.cloudSession.replace(":id", "missing"))
  ...
  it.live("preserves cloud session preview server failures", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClient.get(CypherGatewayPaths.cloudSession.replace(":id", "failed"))
  ...
  it.live("preserves cloud session import authentication failures", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(CypherGatewayPaths.cloudSessionImport, { sessionId: "unauthorized" })
  ...
  it.live("maps cloud session import transport failures to internal errors", () =>
    Effect.gen(function* () {
      ...
      const response = yield* post(CypherGatewayPaths.cloudSessionImport, { sessionId: "failed" })
  ...
  it.live("preserves CypherClaw worker failures", () =>
    Effect.gen(function* () {
      yield* stub(() => new Response("worker failed", { status: 500 }))

      const response = yield* HttpClient.get(CypherGatewayPaths.clawStatus)

      expect(response.status).toBe(500)
      expect(yield* response.json).toEqual({ error: "CypherClaw request failed: 500 worker failed" })
    }),
  )

  it.live("normalizes numeric CypherClaw timestamps", () =>
    Effect.gen(function* () {
  ...
  it.live("maps CypherClaw transport failures to bad gateway", () =>
    Effect.gen(function* () {
      yield* stub(() => Promise.reject(new TypeError("network error")))
      ...
      expect(yield* response.json).toEqual({ error: "Failed to reach CypherClaw" })
    }),
  )
})
