...
import { CyphercodeConfigOverlay } from "../../../src/cypher/config/overlay"
...
async function config(dir: string, value: unknown) {
  await Bun.write(path.join(dir, "cypher.json"), JSON.stringify(value, null, 2))
}
...
describe("config overlay routes", () => {
  test("ignores unsafe patch paths", () => {
    const patched = CyphercodeConfigOverlay.patch({
      scope: "project",
      unset: [
        ["__proto__", "polluted"],
  ...
  test("prefers .cypher over legacy .cypher and ignores .opencode in project overlays", async () => {
    ...
    const entries = [
      ...
      },
      {
        root: ".cypher",
        source: "cypher",
        value: { username: "cypher", model: "test/cypher" },
      },
      {
        root: ".cypher",
        source: "cypher",
        value: { username: "cypher" },
      },
    ] as const

    for (const item of entries) {
      const dir = path.join(project.path, item.root)
      await Filesystem.write(path.join(dir, "cypher.json"), JSON.stringify(item.value))
    ...
    const body = await CyphercodeConfigOverlay.resolve({
    ...
    expect(body.project.username).toBe("cypher")
    expect(body.project.model).toBe("test/cypher")
    ...
    expect(body.targets.project).toBe(path.join(project.path, ".cypher", "cypher.json"))
  })
  ...
  test.serial("tolerates unsafe project config instead of failing the overlay", async () => {
    ...
    await Filesystem.write(
      path.join(project.path, ".cypher", "cypher.json"),
  ...
  test.serial("writes project indexing overrides to .cypher/cypher.jsonc", async () => {
    await using global = await tmpdir()
    await using project = await tmpdir()
    ...
    const file = path.join(project.path, ".cypher", "cypher.jsonc")
    const saved = (await Bun.file(file).json()) as { indexing: Record<string, unknown> }
    const body = await json<Overlay>(await req(project.path, "/config/overlay?scope=project"))

    expect(await Bun.file(path.join(project.path, ".cypher", "cypher.json")).exists()).toBe(false)
  ...
  test.serial("removes local scalar override and falls back to global", async () => {
    await using global = await tmpdir()
    await using project = await tmpdir({ config: { model: "cypher/project-model", username: "alice" } })
    await setGlobal(global.path, { model: "cypher/global-model" })
  ...
  test.serial("writes project mcp overrides without copying inherited servers", async () => {
    ...
    const saved = (await Bun.file(path.join(project.path, ".cypher", "cypher.jsonc")).json()) as {
  ...
  terminal("preserves active terminals after updating global console preferences", async () => {
    ...
    const headers = { "x-cypher-directory": project.path }
