...
import { AgentBuilderPaths } from "../../../src/cypher/server/httpapi/groups/agent-builder"
import { BackgroundProcessPaths } from "../../../src/cypher/server/httpapi/groups/background-process"
import { BranchNamePaths } from "../../../src/cypher/server/httpapi/groups/branch-name"
import { ConfigConsolePaths } from "../../../src/cypher/server/httpapi/groups/config-console"
import { IndexingPaths, CypherEmbeddingModel } from "../../../src/cypher/server/httpapi/groups/indexing"
import { CypherGatewayPaths } from "../../../src/cypher/server/httpapi/groups/cypher-gateway"
import { CyphercodePaths } from "../../../src/cypher/server/httpapi/groups/cypher"
import { MemoryPaths } from "../../../src/cypher/server/httpapi/groups/memory"
import { NetworkPaths } from "../../../src/cypher/server/httpapi/groups/network"
import { TelemetryPaths } from "../../../src/cypher/server/httpapi/groups/telemetry"
...
describe("Cypher PublicApi OpenAPI contract", () => {
  test("uses Cypher branding", () => {
    const spec = OpenApi.fromApi(PublicApi)
    expect(spec.info.title).toBe("cypher")
    expect(spec.info.description).toBe("cypher api")
  })

  test("constrains embedding model metadata", () => {
    const accepts = (dimension: number, scoreThreshold: number) =>
      Result.isSuccess(
        EffectSchema.decodeUnknownResult(CypherEmbeddingModel)({
          id: "provider/model",
          name: "Model",
          dimension,
          scoreThreshold,
  ...
  test("keeps directory routing queries on Cypher Console routes", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const routes = [
  ...
  test("keeps workspace routing queries on all Cypher-owned routed endpoints", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const routes = [
      ...
      { method: "get", path: CyphercodePaths.sessionModelUsage },
  ...
  test("keeps personal organization resets nullable", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const body = spec.paths[CypherGatewayPaths.organization]?.post?.requestBody as Body | undefined
  ...
  test("keeps Cypher gateway responses nullable", () => {
    const spec = OpenApi.fromApi(PublicApi)
    ...
    const profile = response(CypherGatewayPaths.profile)?.properties
    expect(profile?.balance).toEqual({ anyOf: [expect.objectContaining({ type: "object" }), { type: "null" }] })
    expect(profile?.cypherPass).toEqual({ anyOf: [expect.objectContaining({ type: "object" }), { type: "null" }] })
    ...
    const pass = profile?.cypherPass?.anyOf?.find((item) => item.type === "object")?.properties
    ...
    const auth = response(CypherGatewayPaths.authStatus)?.properties
    expect(auth).toEqual({
      authenticated: { type: "boolean" },
      type: { type: "string", enum: ["api", "oauth"] },
    })

    const sessions = response(CypherGatewayPaths.cloudSessions)?.properties
    ...
    const claw = response(CypherGatewayPaths.clawStatus)?.properties
    ...
    expect(response(CypherGatewayPaths.clawChatCredentials)).toEqual({
  ...
  test("keeps transcription prompts in the public contract", () => {
    const spec = OpenApi.fromApi(PublicApi)
    const body = spec.paths[CypherGatewayPaths.audioTranscriptions]?.post?.requestBody as Body | undefined
    const schema = body?.content?.["application/json"]?.schema
