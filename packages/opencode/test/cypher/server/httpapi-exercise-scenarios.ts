...
function enable(ctx: ScenarioContext) {
  const dir = directory(ctx)
  return Effect.promise(() => CypherMemory.enable({ ctx: { directory: dir, worktree: dir } }))
}
...
export const cypherScenarios: Scenario[] = [
  ...
  http.protected.get("/cypher/profile", "cypher.profile").probe({ path: "/path" }).status(401),
  http.protected.get("/cypher/auth-status", "cypher.authStatus").json(200, (body) => {
    object(body)
    check(body.authenticated === false, "Cypher auth status should report signed out")
    check(body.type === undefined, "Cypher auth status should not expose a credential type while signed out")
  }),
  http.protected.get("/cypher/modes", "cypher.modes").json(200, (body) => {
    object(body)
    array(body.modes)
  }),
  http.protected
    .post("/cypher/fim", "cypher.fim")
    .at((ctx) => ({ path: "/cypher/fim", headers: ctx.headers(), body: { prefix: "const value = ", suffix: "\n" } }))
    .status(401),
  http.protected
    .post("/cypher/edit", "cypher.edit")
    .at((ctx) => ({ path: "/cypher/edit", headers: ctx.headers(), body: edit }))
    .status(401),
  http.protected
    .post("/cypher/audio/transcriptions", "cypher.audio.transcriptions")
    .at((ctx) => ({
      path: "/cypher/audio/transcriptions",
      headers: ctx.headers(),
      body: { model: "whisper-1", input_audio: { data: "", format: "wav" } },
    }))
    .status(401),
  http.protected.get("/cypher/notifications", "cypher.notifications").json(200, array),
  http.protected.get("/cypher/models/images", "cypher.models.images").probe({ path: "/path" }).status(401),
  http.protected
    .post("/cypher/organization", "cypher.organization.set")
    .at((ctx) => ({ path: "/cypher/organization", headers: ctx.headers(), body: { organizationId: null } }))
    .status(401),
  http.protected.get("/cypher/claw/status", "cypher.claw.status").probe({ path: "/path" }).status(401),
  http.protected.get("/cypher/claw/chat-credentials", "cypher.claw.chatCredentials").probe({ path: "/path" }).status(401),
  http.protected.get("/cypher/cloud-sessions", "cypher.cloudSessions").probe({ path: "/path" }).status(401),
  http.protected
    .get("/cypher/cloud/session/{id}", "cypher.cloud.session.get")
    .probe({ path: "/path" })
    .at((ctx) => ({ path: route("/cypher/cloud/session/{id}", { id: "httpapi-missing" }), headers: ctx.headers() }))
    .status(401),
  http.protected
    .post("/cypher/cloud/session/import", "cypher.cloud.session.import")
    .at((ctx) => ({ path: "/cypher/cloud/session/import", headers: ctx.headers(), body: { sessionId: "missing" } }))
    .status(401),
  ...
  http.protected
    ...
    .status(400),
  http.protected
    .get("/session/{sessionID}/model-usage", "cypher.sessionModelUsage")
  ...
  http.protected
    .post("/cypher/heap/snapshot", "cypher.heap.snapshot")
    .mutating()
    .jsonEffect(200, (body) =>
      Effect.gen(function* () {
  ...
  http.protected
    .get("/cypher/agent/requirements", "cypher.agentRequirements")
    .at((ctx) => ({ path: "/cypher/agent/requirements?agent=httpapi-agent", headers: ctx.headers() }))
    .json(200, (body, ctx) => {
      object(body)
  ...
  http.protected
    .post("/cypher/skill/remove", "cypher.removeSkill")
    .mutating()
    .preserveDatabase()
    .seeded((ctx) =>
      Effect.gen(function* () {
        const location = yield* file(
          ctx,
          ".cypher/skill/httpapi-remove/SKILL.md",
          "---\nname: httpapi-remove\ndescription: HTTP API removal fixture.\n---\n# HTTP API remove\n",
        )
        const sentinel = yield* file(ctx, ".cypher/skill/httpapi-remove/KEEP.txt", "synthetic sentinel\n")
        return { location, sentinel }
      }),
    )
    .at((ctx) => ({
      path: "/cypher/skill/remove",
      headers: ctx.headers(),
  ...
  http.protected
    .post("/cypher/agent/remove", "cypher.removeAgent")
    .mutating()
    .seeded((ctx) => file(ctx, ".cypher/agent/httpapi-remove.md", "---\ndescription: HTTP API remove\n---\nRemove me.\n"))
    .at((ctx) => ({ path: "/cypher/agent/remove", headers: ctx.headers(), body: { name: "httpapi-remove" } }))
    .jsonEffect(200, (body, ctx) =>
      Effect.gen(function* () {
  ...
  http.protected
    .post("/cypher/agent/remove", "cypher.removeAgent")
    .at((ctx) => ({ path: "/cypher/agent/remove", headers: ctx.headers(), body: { name: "httpapi-missing" } }))
    .status(400),
  http.protected
    .post("/cypher/session-import/project", "cypher.sessionImport.project")
    .mutating()
    .at((ctx) => ({
      path: "/cypher/session-import/project",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/cypher/session-import/session", "cypher.sessionImport.session")
    .mutating()
    .seeded((ctx) => ctx.project())
    .at((ctx) => ({
      path: "/cypher/session-import/session",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/cypher/session-import/message", "cypher.sessionImport.message")
    .mutating()
    .seeded((ctx) => ctx.session({ title: "Import message" }))
    .at((ctx) => ({
      path: "/cypher/session-import/message",
      headers: ctx.headers(),
      body: {
  ...
  http.protected
    .post("/cypher/session-import/part", "cypher.sessionImport.part")
    .mutating()
    .seeded((ctx) =>
      Effect.gen(function* () {
    ...
    .at((ctx) => ({
      path: "/cypher/session-import/part",
      headers: ctx.headers(),
      body: {
