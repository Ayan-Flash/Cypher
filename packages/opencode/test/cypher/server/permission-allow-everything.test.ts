...
import { AllowEverythingPermission } from "../../../src/cypher/permission/allow-everything"
...
const original = {
  password: Flag.CYPHER_SERVER_PASSWORD,
  username: Flag.CYPHER_SERVER_USERNAME,
  envPassword: process.env.CYPHER_SERVER_PASSWORD,
  envUsername: process.env.CYPHER_SERVER_USERNAME,
}

afterEach(() => {
  Flag.CYPHER_SERVER_PASSWORD = original.password
  Flag.CYPHER_SERVER_USERNAME = original.username
...
const auth = () => `Basic ${Buffer.from("cypher:secret").toString("base64")}`

const requireAuth = () => {
  Flag.CYPHER_SERVER_PASSWORD = "secret"
  Flag.CYPHER_SERVER_USERNAME = undefined
  process.env.CYPHER_SERVER_PASSWORD = "secret"
  delete process.env.CYPHER_SERVER_USERNAME
}
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        const blocked = await Server.Default().app.request("/permission/allow-everything", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-cypher-directory": tmp.path },
        ...
        const enable = await Server.Default().app.request("/permission/allow-everything", {
          method: "POST",
          headers: { "Content-Type": "application/json", "x-cypher-directory": tmp.path, authorization: auth() },
          body: JSON.stringify({ enable: true }),
        })
