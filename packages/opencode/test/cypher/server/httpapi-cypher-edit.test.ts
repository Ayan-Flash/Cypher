...
const env = {
  CYPHER_AUTH_CONTENT: process.env.CYPHER_AUTH_CONTENT,
...
async function send(body: Record<string, unknown> = edit, signal?: AbortSignal) {
  ...
  return app().request(CypherGatewayPaths.edit, {
    method: "POST",
...
function authenticate() {
  process.env.CYPHER_AUTH_CONTENT = JSON.stringify({
    cypher: {
      type: "oauth",
...
describe("HttpApi Cypher next edit", () => {
  test("requires Cypher Gateway authentication for the Cypher-backed model", async () => {
    process.env.CYPHER_AUTH_CONTENT = "{}"
  ...
  test("proxies Cypher-backed edits with gateway auth and autocomplete headers", async () => {
    authenticate()
