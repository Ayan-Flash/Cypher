...
import { invalidateAfterProviderAuthChange } from "../../../src/cypher/server/provider-auth-lifecycle"
...
it.effect("clears provider models before disposing instances after auth changes", () =>
  Effect.gen(function* () {
    ...
    yield* invalidateAfterProviderAuthChange("cypher").pipe(Effect.provide(layer(events)))

    expect(yield* Ref.get(events)).toEqual(["clear:cypher", "dispose"])
  }),
)
