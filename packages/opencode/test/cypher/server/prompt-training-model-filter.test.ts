...
function models(input: unknown, key: "all" | "providers") {
  ...
  const cypher = input[key].find((provider) => record(provider) && provider.id === "cypher")
  if (!record(cypher) || !record(cypher.models)) return []
  return Object.keys(cypher.models)
}

function request(path: string, dir: string) {
  return Effect.promise(async () => {
    const result = await Server.Default().app.request(path, { headers: { "x-cypher-directory": dir } })
...
it.live(
  ...
  Effect.gen(function* () {
      ...
      Effect.promise(() =>
        tmpdir({
          config: {
            formatter: false,
            lsp: false,
            enabled_providers: ["cypher"],
            hide_prompt_training_models: true,
            provider: { cypher: { options: { baseURL } } },
          },
        }),
      ),
    ...
    yield* Effect.acquireRelease(
      Effect.sync(() => {
        process.env.CYPHER_API_KEY = "test-key"
      }),
      () =>
        Effect.sync(() => {
