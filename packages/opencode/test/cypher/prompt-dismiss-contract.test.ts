/**
 * Contract test for prompt.ts Cypher-specific invariants.
 *
 * prompt.ts is a shared upstream file. The Cypher-specific "new prompt unblocks
 * pending suggestions/questions then enqueues without cancelling the in-flight
 * stream" behaviour lives inside a cypher_change block. An upstream merge
...
describe("prompt.ts Cypher-specific invariants", () => {
  test("imports Suggestion from cypher/suggestion", () => {
    const content = fs.readFileSync(PROMPT_FILE, "utf-8")
    expect(content).toMatch(/import\s*\{[^}]*Suggestion[^}]*\}\s*from\s*["']@\/cypher\/suggestion["']/)
  })
  ...
  test("dismissAll for suggestions and questions runs before enqueue, without cancelling the in-flight fiber", () => {
    ...
    // Critically, the block must NOT call state.cancel or CypherSessionPromptQueue.reserve —
    ...
    const block = content.match(
      /cypher_change start[^\n]*unblock tools[\s\S]*?Suggestion\.dismissAll[\s\S]*?question\.dismissAll[\s\S]*?CypherSessionPromptQueue\.enqueue/,
    )
    expect(block).not.toBeNull()
    expect(content).not.toMatch(/state\.cancel\(input\.sessionID\)/)
    expect(content).not.toMatch(/CypherSessionPromptQueue\.reserve/)
  })
  ...
  test("runLoop breaks out between LLM steps when a newer prompt was enqueued", () => {
    ...
    expect(content).toContain("CypherSessionPromptQueue.hasFollowup(sessionID)")
  })
})
