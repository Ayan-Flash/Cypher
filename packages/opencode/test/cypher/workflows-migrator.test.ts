...
import { WorkflowsMigrator } from "../../src/cypher/workflows-migrator"
...
Actual description here.`
  ...
  describe("discoverWorkflows", () => {
    ...
    test("discovers workflows from legacy .cypher/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".cypher", "workflows")
    ...
    test("discovers global workflows from ~/.cypher/workflows/", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".cypher", "workflows", "global.md"), "# Global\n\nGlobal workflow")
      ...
      expect(
        workflows.some((w) => w.source === "global" && w.path.includes(path.join(".cypher", "workflows", "global.md"))),
      ).toBe(true)
    })

    test("applies markdown substitutions to workflow content", async () => {
      process.env.CYPHER_WORKFLOW_TEST = "env content"
      await using tmp = await tmpdir({
        init: async (dir) => {
          const workflowsDir = path.join(dir, ".cypher", "workflows")
          ...
          await Bun.write(
            ...
            ["# Workflow", "", "{file:../../guide.md}", "{env:CYPHER_WORKFLOW_TEST}"].join("\n"),
          )
        },
      })
      ...
      try {
        ...
        delete process.env.CYPHER_WORKFLOW_TEST
      }
    })
  })
  ...
  describe("convertToCommand", () => {
    test("converts workflow to command format", () => {
      const workflow: WorkflowsMigrator.CyphercodeWorkflow = {
        name: "code-review",
    ...
    test("project workflows override global workflows with same name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const projectDir = path.join(dir, ".cypher", "workflows")
