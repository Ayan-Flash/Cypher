...
import { MemorySaveTool } from "@/cypher/tool/memory-save"
...
const watch = process.env.CYPHER_EXPERIMENTAL_DISABLE_FILEWATCHER
...
beforeEach(() => {
  process.env.CYPHER_EXPERIMENTAL_DISABLE_FILEWATCHER = "true"
  spyOn(RemoteSender, "create").mockReturnValue({ handle() {}, dispose() {} })
})

afterEach(async () => {
  mock.restore()
  if (watch === undefined) delete process.env.CYPHER_EXPERIMENTAL_DISABLE_FILEWATCHER
  if (watch !== undefined) process.env.CYPHER_EXPERIMENTAL_DISABLE_FILEWATCHER = watch
  await resetDatabase()
})
...
describe("cypher_memory_save", () => {
  ...
  test("defaults mutating memory tool permission to ask", () => {
    const cypher = CypherAgent.prepare({})

    expect(Permission.evaluate("cypher_memory_recall", "typed", cypher.defaultsPatch).action).toBe("ask")
    expect(Permission.evaluate("cypher_memory_save", "remember", cypher.defaultsPatch).action).toBe("ask")
  })

  test("remembers, corrects, and forgets explicit project memory", async () => {
    await using dir = await tmpdir({ git: true })
    await withConfig(path.join(dir.path, "global", ".cypher"), async () => {
      ...
      const corrected = await execute(
        dir.path,
        {
          action: "correct",
          key: "cypher_cli_tui",
          text: "Cypher CLI should be treated as a terminal UI.",
        },
        gate,
      )
      const shown = await CypherMemory.show({ ctx: memory })

      expect(saved.title).toBe("Cypher memory saved: 1 op")
      expect(corrected.title).toBe("Cypher memory correction saved: 1 op")
      expect(shown.sources.project).toContain("- cypher_cli_tui :: Cypher CLI is a TUI.")
      expect(shown.sources.corrections).toContain("- cypher_cli_tui :: Cypher CLI should be treated as a terminal UI.")

      const forgotten = await execute(dir.path, { action: "forget", query: "cypher_cli_tui" }, gate)
      const next = await CypherMemory.show({ ctx: memory })

      expect(forgotten.title).toBe("Cypher memory updated: 2 removed")
      expect(next.sources.project).not.toContain("cypher_cli_tui")
      expect(next.sources.corrections).not.toContain("cypher_cli_tui")
      expect(asks.map((req) => req.permission)).toEqual(["cypher_memory_save", "cypher_memory_save", "cypher_memory_save"])
  ...
  test("does not save when project memory is disabled", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".cypher"), async () => {
      ...
      expect(result.title).toBe("Cypher memory: disabled")
  ...
  test("declines user-level memory but saves project conventions", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".cypher"), async () => {
      ...
      expect(skipped.title).toBe("Cypher memory skipped: out of scope")
      expect(skipped.output).toContain("user-level memory is not supported yet")
      expect(blocked.title).toBe("Cypher memory unchanged")
  ...
  test("handles malformed save inputs without corrupting memory files", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".cypher"), async () => {
      ...
      expect(missing.title).toBe("Cypher memory remember: no text")
      expect(forget.title).toBe("Cypher memory forget: no query")
      expect(secret.title).toBe("Cypher memory: error")
      expect(secret.output).toContain("rejected secret-like content")
      expect(noisy.title).toBe("Cypher memory saved: 1 op")
