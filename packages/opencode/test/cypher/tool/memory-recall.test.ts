...
import { MemoryRecallTool } from "@/cypher/tool/memory-recall"
...
describe("cypher_memory_recall description", () => {
...
describe("cypher_memory_recall", () => {
  ...
  test("shows typed memory hits separately from session digests", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".cypher"), async () => {
      const memory = { directory: dir.path, worktree: dir.path }
      const enabled = await CypherMemory.enable({ ctx: memory })
        ...
        ops: [
          {
            ...
            section: "Commands",
            key: "vscode_tests",
            text: "Run VS Code unit tests from packages/cypher-vscode with bun run test:unit.",
          },
          {
            action: "add",
            file: "project.md",
      ...
      expect(typed.title).toContain("Cypher memory typed")
      ...
      expect(digest.title).toContain("Cypher memory digest")
  ...
  test("catalog mode lists all stored keys with optional filter", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".cypher"), async () => {
        ...
        ops: [
          {
            ...
            file: "project.md",
            section: "Facts",
            key: "cypher_was_originally_a_fork",
            text: "cypher was originally a fork of roo and has a cypher-legacy repo",
          },
          {
            action: "add",
            file: "environment.md",
      ...
      expect(all.output).toContain("cypher_was_originally_a_fork")
      expect(all.output).toContain("vscode_tests")
      expect(filtered.output).toContain("cypher_was_originally_a_fork")
  ...
  test("search mode renders typed and digest memory without catalog mode", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".cypher"), async () => {
      ...
      expect(result.title).toContain("Cypher memory search")
  ...
  test("handles large unmatched recall queries without returning stored memory", async () => {
    ...
    await withConfig(path.join(dir.path, "global", ".cypher"), async () => {
      ...
      expect(result.title).toBe("Cypher memory search: no results")
