...
import {
  ...
  IMAGE_MODELS,
  DEFAULT_MODEL,
} from "../../../src/cypher/tool/generate-image"
...
describe("generate-image provider resolver", () => {
  test("uses Cypher cloud when Cypher auth is present", () => {
    const result = resolveProvider({ type: "oauth", access: "cypher-token", accountId: "org-123" }, undefined)
    expect(result).not.toBeNull()
    expect(result!.token).toBe("cypher-token")
    expect(result!.organizationId).toBe("org-123")
    expect(result!.provider).toBe("cypher")
    expect(result!.url).toContain("openrouter")
  })

  test("uses Cypher cloud with API key auth", () => {
    const result = resolveProvider({ type: "api", key: "cypher-api-key" }, undefined)
    expect(result!.token).toBe("cypher-api-key")
    expect(result!.provider).toBe("cypher")
  })

  test("falls back to OpenRouter with BYO key when no Cypher auth", () => {
  ...
  test("prefers Cypher auth over OpenRouter key", () => {
    const result = resolveProvider({ type: "oauth", access: "cypher-token" }, "or-key")
    expect(result!.provider).toBe("cypher")
    expect(result!.token).toBe("cypher-token")
  })
})
