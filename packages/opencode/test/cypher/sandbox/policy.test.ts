...
import { assertWrite, run as runSandbox } from "@cypher/sandbox"
import { Effect, Exit } from "effect"
import { profile } from "@/cypher/sandbox/policy"
import { SandboxPreference } from "@/cypher/sandbox/preference"
...
function fixture() {
  return tmpdir<Dirs>({
    init: async (root) => {
      ...
      const a = path.join(main, ".cypher", "worktrees", "a")
      const b = path.join(main, ".cypher", "worktrees", "b")
...
function expected(...directories: string[]) {
  return new Set([...directories, ...cypher])
}
...
describe("sandbox policy", () => {
  ...
  test("keeps Cypher state writable without exposing sandbox policy state", async () => {
    await using tmp = await fixture()
    const dirs = tmp.extra
    ...
    expect(policy.environment.deny).toEqual(["CYPHER_SERVER_PASSWORD", "CYPHER_SERVER_USERNAME"])
