...
import * as ToolNetwork from "@/cypher/sandbox/network"
import { MCP } from "@/mcp"
...
const sessions = Layer.mock(Session.Service)({
  get: () => Effect.succeed(session("/workspace/project/.cypher/worktrees/a")),
})
...
mac("confines model-originated file mutations to the active worktree", () =>
  Effect.gen(function* () {
    ...
    const cypher = yield* call(write, { filePath: state, content: "state" }, "call-state").pipe(Effect.exit)
    ...
    expect(Exit.isSuccess(cypher)).toBe(true)
