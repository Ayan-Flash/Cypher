...
import { Instance } from "../../src/cypher/instance"
import { provideTestInstance } from "../fixture/fixture"
import { PlanFollowup } from "../../src/cypher/plan-followup"
...
describe("plan_exit detection", () => {
        ...
        tools: [
          {
            tool: "plan_exit",
            input: {},
            output: "Plan is ready at .cypher/plans/plan.md. Ending planning turn.",
          },
        ],
      })
  ...
  test("CypherSessionPrompt resolves plan follow-up through the supplied question service", () =>
    withInstance(async () => {
      const seeded = await seed({
        text: "Here is the plan",
        tools: [
      ...
      const result = await Effect.runPromise(
        Effect.gen(function* () {
          ...
          const pending = CypherSessionPrompt.askPlanFollowup({
            sessionID: seeded.sessionID,
            messages: seeded.messages,
            abort: AbortSignal.any([]),
            question,
  ...
  test("CypherSessionPrompt cleans listener-local plan follow-up when aborted outside instance context", () => {
  ...
  test("JetBrains client enables plan follow-up with custom answer", () =>
    withInstance(async () => {
      ...
      try {
        process.env.CYPHER_CLIENT = "jetbrains"
        const seeded = await seed({
          text: "Here is the plan",
          tools: [
            {
        ...
        else process.env.CYPHER_CLIENT = prev
      }
    }))
  ...
  test("plan reminder reuses custom plan_exit path when refining", () =>
    withInstance(async () => {
      ...
      await CypherSessionPrompt.insertPlanReminders({
