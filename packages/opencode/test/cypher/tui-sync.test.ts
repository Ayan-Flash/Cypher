...
import { CypherSessionTuiSync } from "../../src/cypher/session/tui-sync"
...
function syncVariant(input: { current: string | undefined; message: Message; parts?: readonly { type: string }[] }) {
  if (!CypherSessionTuiSync.model({ role: input.message.role, parts: input.parts })) return input.current
  return input.message.model?.variant ?? "default"
}

describe("CypherSessionTuiSync.model", () => {
  test("syncs normal user messages", () => {
    expect(CypherSessionTuiSync.model({ role: "user", parts: [{ type: "text" }] })).toBe(true)
  })

  test("skips compaction marker user messages", () => {
    expect(CypherSessionTuiSync.model({ role: "user", parts: [{ type: "compaction" }] })).toBe(false)
  })

  test("skips messages before parts load", () => {
    expect(CypherSessionTuiSync.model({ role: "user" })).toBe(false)
  })
  ...
  test("skips messages checked with stored parts", () => {
    ...
    expect(CypherSessionTuiSync.model({ role: msg.role, parts })).toBe(false)
  })

  test("skips non-user messages", () => {
    expect(CypherSessionTuiSync.model({ role: "assistant", parts: [{ type: "text" }] })).toBe(false)
  })
