...
import { generateHelp, generateCommandTable } from "../../src/cypher/help"
...
import { ProvidersCommand } from "../../src/cli/cmd/providers" // cypher_change — upstream renamed auth → providers
...
import { HelpCommand } from "../../src/cypher/help-command"
import { ProfileCommand } from "../../src/cypher/cli/cmd/profile"
import { DaemonCommand } from "../../src/cypher/cli/cmd/daemon"
import { CypherConsoleCommand } from "../../src/cypher/cli/cmd/console"
...
const AttachStub = {
  command: "attach <url>",
  describe: "attach to a running cypher server",
  handler() {},
}
...
describe("cypher help --all (markdown)", () => {
  test("contains ## heading for each known top-level command", async () => {
    ...
    for (const cmd of ["run", "auth", "debug", "mcp", "session", "agent", "profile"]) {
      expect(output).toContain(`## cypher ${cmd}`)
    }
  })

  test("contains headings for nested subcommands", async () => {
    const output = await generateHelp({ all: true, format: "md", commands })
    expect(output).toContain("cypher auth login")
    expect(output).toContain("cypher auth logout")
    expect(output).toContain("cypher debug config")
  })
})

describe("cypher help --all (text)", () => {
...
describe("cypher help <command>", () => {
  test("cypher help auth contains auth subcommand headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).toContain("cypher auth login")
    expect(output).toContain("cypher auth logout")
    expect(output).toContain("cypher auth list")
  })

  test("cypher help auth does NOT contain run or debug headings", async () => {
    const output = await generateHelp({ command: "auth", format: "md", commands })
    expect(output).not.toContain("## cypher run")
    expect(output).not.toContain("## cypher debug")
  })

  test("documents console stop and foreground mode", async () => {
    const output = await generateHelp({ command: "console", format: "md", commands })
    expect(output).toContain("cypher console stop")
  ...
  test("documents daemon foreground mode", async () => {
    ...
    expect(output).toContain("cypher daemon start")
    expect(output).toContain("--foreground")
    expect(output).toContain("-f")
  })
})
...
describe("edge cases", () => {
  ...
  test("cypher help nonexistent throws unknown command error", async () => {
...
describe("generateCommandTable", () => {
  ...
  test("contains rows for known commands", async () => {
    ...
    for (const name of ["run", "auth", "debug", "mcp"]) {
      expect(output).toContain(`cypher ${name}`)
    }
  })

  test("default command appears as cypher [project], not $0", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`cypher [project]`")
  ...
  test("skips commands with no describe", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).not.toContain("`cypher generate`")
  })

  test("contains cypher completion row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`cypher completion`")
  })

  test("contains cypher help row", async () => {
    const output = await generateCommandTable({ commands })
    expect(output).toContain("`cypher help")
  })
})

describe("Cypher CLI customizations are wired into index.ts", () => {
  const file = (rel: string) => Bun.file(path.resolve(import.meta.dir, rel)).text()
  const INDEX = "../../src/index.ts"
  const SETUP = "../../src/cypher/cli/setup.ts"
  const BARREL = "../../src/cypher/commands.ts"

  test("CLI is branded `cypher`, not `opencode`", async () => {
    const index = await file(INDEX)
    expect(index).toContain('.scriptName("cypher")')
    expect(index).not.toContain('.scriptName("opencode")')
  })

  test("index.ts invokes the CypherCli integration points", async () => {
    // These thin call-sites are the only wiring between upstream index.ts and the Cypher
    // customizations in setup.ts. If a future upstream merge drops them, every Cypher command
    // and the telemetry/lifecycle hooks silently disappear, exactly the regression this guards.
    const index = await file(INDEX)
    expect(index).toContain("CypherCli.register(")
    expect(index).toContain("CypherCli.bootstrap(")
    expect(index).toContain("CypherCli.shutdown(")
  })

  test("registers the local Cypher Console instead of the upstream account console", async () => {
    const index = await file(INDEX)
    const setup = await file(SETUP)
    const barrel = await file(BARREL)
    expect(setup).toContain("CypherConsoleCommand")
