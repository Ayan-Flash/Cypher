...
import { CypherSessionCompaction } from "@/cypher/session/compaction"
import { CypherSessionPromptQueue } from "@/cypher/session/prompt-queue"
import { Suggestion } from "../../src/cypher/suggestion"
...
describe("session prompt queue", () => {
  test("scopes queued turns without moving prior assistant history", async () => {
    ...
    const ids = await Effect.runPromise(
      CypherSessionPromptQueue.enqueue(
        sessionID,
        two,
        Effect.sync(() => CypherSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id)),
        Effect.succeed([]),
      ),
    )
  ...
  test("retarget keeps older queued prompts hidden", async () => {
    ...
    const result = await Effect.runPromise(
      CypherSessionPromptQueue.enqueue(
        ...
        base,
        Effect.sync(() => {
          CypherSessionPromptQueue.retarget(sessionID, injected)
          return {
            active: CypherSessionPromptQueue.active(sessionID),
            ids: CypherSessionPromptQueue.scope(sessionID, messages).map((item) => item.info.id),
          }
        }),
          ...
          CypherSessionPromptQueue.enqueue(
            ...
            Effect.promise(async () => {
              await Effect.runPromise(
                CypherSessionCompaction.create({
                  session: store,
              ...
              return { compact, ids: CypherSessionPromptQueue.scope(session.id, messages).map((item) => item.info.id) }
            }),
  ...
  test("hasFollowup reports true only for prompts enqueued after the active slot started", async () => {
    ...
    const first = Effect.runPromise(
      CypherSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "first:start", value: CypherSessionPromptQueue.hasFollowup(sessionID) })
          firstStarted.resolve()
          yield* Effect.promise(() => firstReleased.promise)
          observed.push({ where: "first:end", value: CypherSessionPromptQueue.hasFollowup(sessionID) })
          return "first"
        }),
    ...
    const second = Effect.runPromise(
      CypherSessionPromptQueue.enqueue(
        ...
        Effect.gen(function* () {
          observed.push({ where: "second:start", value: CypherSessionPromptQueue.hasFollowup(sessionID) })
    ...
    expect(CypherSessionPromptQueue.hasFollowup(sessionID)).toBe(true)
    ...
    const third = Effect.runPromise(
      CypherSessionPromptQueue.enqueue(
        ...
        Effect.sync(() => {
          observed.push({ where: "third:start", value: CypherSessionPromptQueue.hasFollowup(sessionID) })
          return "third"
        }),
    ...
    expect(CypherSessionPromptQueue.hasFollowup(sessionID)).toBe(false)
  ...
  test("cancel on a session with no active tail is a no-op and does not leak state", async () => {
    ...
    await Effect.runPromise(CypherSessionPromptQueue.cancel(sessionID))

    expect(CypherSessionPromptQueue._hasInternalState(sessionID)).toBe(false)
          ...
          scoped(tmp.path, async (prompt) => {
            ...
            const ids = await Effect.runPromise(
              CypherSessionPromptQueue.enqueue(
                session.id,
                MessageID.make("msg_probe"),
                Effect.succeed(CypherSessionPromptQueue.scope(session.id, []).map((item) => item.info.id)),
                Effect.succeed([]),
              ),
            )
            expect(ids).toEqual([])
            expect(CypherSessionPromptQueue.hasFollowup(session.id)).toBe(false)
          }),
      })
    } finally {
      server.stop(true)
