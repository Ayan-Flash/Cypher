...
describe("indexing worktrees", () => {
  test("shares the primary checkout index with a linked worktree", async () => {
    ...
    const worktree = path.join(tmp.path, ".cypher", "worktrees", "feature")
    ...
    await provideTestInstance({
      directory: worktree,
      fn: async () => {
        await CypherIndexing.search("worktree")
        expect((await CypherIndexing.current()).state).toBe("Complete")
        expect(await CypherIndexing.available()).toBe(true)
      },
    })
  ...
  test("does not classify an ordinary directory from its pathname", async () => {
    await using tmp = await tmpdir({ git: true, config: cfg })
    process.env["CYPHER_CONFIG_DIR"] = tmp.path
    const directory = path.join(tmp.path, ".cypher", "worktrees", "feature")
    ...
    await provideTestInstance({
      directory,
      fn: async () => {
        await CypherIndexing.search("ordinary directory")
        expect((await CypherIndexing.current()).state).toBe("Complete")
        expect(await CypherIndexing.available()).toBe(true)
      },
    })
