...
import { filterPromptTrainingModels, nonEmptyProviders } from "../../../src/cypher/provider/model-filter"

function model(id: string, training?: boolean): Provider.Model {
  return {
    id: ModelID.make(id),
    providerID: ProviderID.cypher,
    api: { id: "cypher", url: "https://api.cypher.ai", npm: "@cypher/cypher-gateway" },
    name: id,
    capabilities: {
      temperature: true,
      reasoning: false,
...
describe("prompt-training model filter", () => {
  test("hides only explicitly marked Cypher Gateway models", () => {
    const providers = {
      cypher: provider("cypher", {
    ...
    expect(Object.keys(result.cypher.models)).toEqual(["private", "unknown"])
    expect(Object.keys(result.other.models)).toEqual(["training"])
    expect(Object.keys(providers.cypher.models)).toEqual(["training", "private", "unknown"])
  })

  test("preserves the catalog when disabled", () => {
    const providers = { cypher: provider("cypher", { training: model("training", true) }) }
