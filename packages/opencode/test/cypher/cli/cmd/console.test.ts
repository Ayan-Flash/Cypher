...
import { getNetworkIPs, serverUrls } from "../../../../src/cypher/cli/server-urls"
...
describe("console daemon startup", () => {
  test("detects every explicit network option form", () => {
    ...
    expect(explicitNetworkOptions(["cypher", "console", "--", "--port=4321"])).toStrictEqual([])
  })
  ...
  test("rejects legacy fixed-password daemon state", () => {
    expect(Daemon.matches({ ...state(), password: "cypher" }, opts(), [])).toBe(false)
  })
