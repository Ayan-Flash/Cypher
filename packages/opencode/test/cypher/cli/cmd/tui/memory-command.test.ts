...
import type { CypherClient } from "@cypher/sdk/v2"
import { runMemoryCommand } from "@/cypher/cli/cmd/tui/memory-command"
import { MemoryTuiEvents } from "@/cypher/cli/cmd/tui/memory-events"
...
describe("memory TUI command parser", () => {
  ...
  test("status opens overview dialog", async () => {
    const shown: string[] = []
    const opened: string[] = []
    const client = { memory: {} } as unknown as CypherClient
  ...
  test("on and off call enable and disable endpoints", async () => {
    ...
    const client = {
      memory: {
        enable: async () => {
          calls.push("enable")
          return { data: { root: "/tmp/cypher-data/memory/repo-abc123", index: { tokens: 42 } } }
        },
        disable: async () => {
          calls.push("disable")
