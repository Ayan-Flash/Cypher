...
import { CypherTerminalTitle } from "@/cypher/cli/cmd/tui/terminal-title"
...
function resolve(input: TuiConfig.Info): TuiConfig.Resolved {
  ...
  return {
    ...
    attention: {
      ...
      sound_pack: input.attention?.sound_pack ?? "cypher.default",
...
describe("CypherTuiConfig.makeStore", () => {
  test("reactive reads update when set() reconciles a new config", () => {
    const store = CypherTuiConfig.makeStore(resolve(cfg({ keybinds: { app_exit: "ctrl+c" }, theme: "cypher" })))
    ...
    createRoot((d) => {
      ...
      createEffect(() =>
        titles.push(
          CypherTerminalTitle.format({ base: "Cypher CLI", indicator: "working", icon: store.config.title_icon }),
        ),
      )
    })
    ...
    expect(themes).toEqual(["cypher"])
    expect(icons).toEqual(["none"])
    expect(titles).toEqual(["Cypher CLI"])
    ...
    expect(themes).toEqual(["cypher", "nord"])
    expect(icons).toEqual(["none", "emojis"])
    expect(titles).toEqual(["Cypher CLI", "💭 Cypher CLI"])

    dispose()
  })

  test("set() restores the default title icon when the setting is removed", () => {
    const store = CypherTuiConfig.makeStore(resolve(cfg({ title_icon: "unicode" })))
