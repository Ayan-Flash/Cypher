...
import { CypherRun } from "../../../../src/cypher/cli/cmd/run"
import { buildRunMessage } from "../../../../src/cypher/cli/cmd/run-message"
...
describe("CypherRun", () => {
  test("prefers a configured command over an endpoint-backed built-in", async () => {
    ...
    expect(await CypherRun.resolveBuiltin(sdk as never, "compact", "/tmp/project")).toBeUndefined()
  })
  ...
  test("resolves an endpoint-backed built-in when no configured command matches", async () => {
    ...
    expect(await CypherRun.resolveBuiltin(sdk as never, "compact", "/tmp/project")).toBe("compact")
  })
...
describe("buildRunMessage", () => {
  ...
  test("passes args['--'] through verbatim without wrap-quote (#9622)", () => {
    // `cypher run -- "- Who are you?"` - yargs+populate-- captures the leading-dash
