...
import { CypherTuiThreadDaemon } from "../../../../src/cypher/cli/cmd/tui/thread"
...
describe("cypher tui thread", () => {
  ...
  test("uses cypher-dev caller directory when running through package cwd", async () => {
    await using root = await tmpdir()
    const pkg = path.join(root.path, "packages", "opencode")
    await fs.mkdir(pkg, { recursive: true })

    const prev = process.env.CYPHER_DEV_CWD
    process.env.CYPHER_DEV_CWD = root.path
    try {
      ...
      if (prev === undefined) delete process.env.CYPHER_DEV_CWD
      else process.env.CYPHER_DEV_CWD = prev
    }
  })
  ...
  test("validates imported daemon session over HTTP after importing from cloud", async () => {
    ...
    using server = Bun.serve({
      ...
      fetch(request) {
        ...
        calls.push(route)
        if (route === "POST /cypher/cloud/session/import") return Response.json({ id: local })
    ...
    const args = { port: 0, hostname: "127.0.0.1", mdns: false, "mdns-domain": "cypher.local", cors: [] }
    const start: Parameters<typeof CypherTuiThreadDaemon.attach>[0]["start"] = async (input) => {
      opened.push(input.args.sessionID)
    }

    try {
      await CypherTuiThreadDaemon.attach({
      ...
      expect(calls).toEqual(["POST /cypher/cloud/session/import", `GET /session/${local}`])
  ...
  test("imports cloud fork before validating daemon session", async () => {
    ...
    mock.module("@/cypher/daemon/client", () => ({
      DaemonClient: {
    ...
    const mod = await import(`../../../../src/cypher/cli/cmd/tui/thread?${key}`)

    const handled = await mod.CypherTuiThreadDaemon.attach({
