...
import { detectRepo, findRepoFrom } from "../../../src/cypher/cli/dev-setup"
...
describe("findRepoFrom", () => {
  test("returns the repo root when started from a nested path", async () => {
    ...
    const nested = path.join(tmp.path, "packages", "opencode", "src", "cypher", "cli")
  ...
  test("returns undefined when no sentinel is found up to filesystem root", async () => {
    ...
    // above tmp does not contain a cypher repo at its own root.
    const found = await findRepoFrom(tmp.path)
...
describe("detectRepo", () => {
  test("honours CYPHER_DEV_REPO without touching the filesystem", async () => {
    // Pointing at a path that does not exist — the hint must be returned as-is
    // (this proves no walk happens when CYPHER_DEV_REPO is set).
    const hint = "/definitely/does/not/exist/cypher-repo"
    const got = await withEnv("CYPHER_DEV_REPO", hint, () => detectRepo())
    expect(got).toBe(hint)
  })

  test("CYPHER_DEV_REPO wins over the filesystem walk", async () => {
    // Build a synthetic repo, point CYPHER_DEV_REPO at it, and run from an
    // unrelated cwd. Even though the walk from cwd (inside the real cypher
    // checkout) would succeed, the hint must take priority.
    await using tmp = await tmpdir()
    await makeSynthRepo(tmp.path)

    const got = await withEnv("CYPHER_DEV_REPO", tmp.path, () => detectRepo())
