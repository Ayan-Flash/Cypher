...
const wrapper = path.join(root, "bin", "cypher")
...
describe("npm install artifact behavior", () => {
  test("keeps the CLI wrapper contract", async () => {
    ...
    expect(text).toContain("const envPath = process.env.CYPHER_BIN_PATH")
    expect(text).toContain('const base = "@cypher/cli-" + platform + "-" + arch')
  ...
  test("copies cached binary runtime resources during postinstall", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cypher-postinstall-artifact-"))
    try {
      const pkg = path.join(tmp, "node_modules", "@cypher", "cli")
      const native = path.join(tmp, "node_modules", "@cypher", `cli-${process.platform}-${process.arch}`)
      ...
      await Bun.write(
        ...
        JSON.stringify({
          optionalDependencies: {
            [`@cypher/cli-${process.platform}-${process.arch}`]: "1.0.0",
          },
        }),
      )
      await Bun.write(
        path.join(native, "package.json"),
        JSON.stringify({ name: `@cypher/cli-${process.platform}-${process.arch}` }),
      )
      const binary = "#!/bin/sh\n# binary\nexit 0\n"
      await Bun.write(path.join(bin, "cypher"), binary)
      await Bun.write(path.join(bin, "cypher-sandbox-mutation-worker.js"), "worker")
      ...
      expect(await Bun.file(path.join(pkg, "bin", ".cypher")).text()).toBe(binary)
      expect(await Bun.file(path.join(pkg, "bin", "cypher-sandbox-mutation-worker.js")).text()).toBe("worker")
  ...
  test("links npm bin commands to the wrapper during local install", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "cypher-install-artifact-"))
    try {
      ...
      await fs.copyFile(wrapper, path.join(bin, "cypher"))
      await Bun.write(
        path.join(pkg, "package.json"),
        JSON.stringify(
          {
            name: "cypher-install-artifact-repro",
            version: "1.0.0",
            bin: {
              cypher: "./bin/cypher",
      ...
      const hidden = path.join(prefix, "node_modules", ".bin", ".cypher")
