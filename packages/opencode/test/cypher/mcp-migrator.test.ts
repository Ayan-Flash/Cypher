...
import { McpMigrator } from "../../src/cypher/mcp-migrator"
...
describe("McpMigrator", () => {
  describe("convertServer", () => {
    test("converts local server with command and args", () => {
      const server: McpMigrator.CyphercodeMcpServer = {
        command: "npx",
  ...
  describe("migrate", () => {
    ...
    test("migrates servers from project .cypher/mcp.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".cypher")
          await Bun.write(
    ...
    test("reads from legacy .cypher/mcp.json when .cypher/mcp.json is absent", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".cypher")
          await Bun.write(
    ...
    // Regression: malformed .cypher/mcp.json must not prevent .cypher/mcp.json from loading
    test("loads .cypher/mcp.json even when .cypher/mcp.json is malformed", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(path.join(dir, ".cypher", "mcp.json"), "{ corrupt json !!!")
          await Bun.write(
            path.join(dir, ".cypher", "mcp.json"),
            JSON.stringify({
              mcpServers: {
    ...
    test(".cypher/mcp.json overrides .cypher/mcp.json for same server name", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          await Bun.write(
            path.join(dir, ".cypher", "mcp.json"),
    ...
    test("does not read project-level .cypher/mcp_settings.json", async () => {
      await using tmp = await tmpdir({
        init: async (dir) => {
          const settingsDir = path.join(dir, ".cypher")
          await Bun.write(
