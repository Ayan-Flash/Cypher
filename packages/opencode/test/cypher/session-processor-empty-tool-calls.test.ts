        ...
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === CypherSessionProcessor.REASONING_LENGTH_WARNING,
          )

          expect(warning?.ignored).toBe(true)

          const modelMsgs = yield* MessageV2.toModelMessagesEffect([{ info: handle.message, parts }], mdl)
          expect(JSON.stringify(modelMsgs)).not.toContain(CypherSessionProcessor.REASONING_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          ...
          const warning = parts.find(
            (part): part is MessageV2.TextPart =>
              part.type === "text" && part.text === CypherSessionProcessor.OUTPUT_LENGTH_WARNING,
          )
          ...
          expect(json).not.toContain(CypherSessionProcessor.OUTPUT_LENGTH_WARNING)
        }),
      { git: true },
    ),
  )
      ...
      (dir) =>
        Effect.gen(function* () {
          ...
          const selection = {
            ...
            modelID: ModelID.make("cypher-auto/efficient"),
          }
          ...
          yield* test.reply(
            ...
            LLMEvent.stepFinish({
              ...
              reason: "stop",
              usage: usage(),
              providerMetadata: { cypher: { routedModelID: "openai/gpt-5.5-20260423" } },
            }),
            LLMEvent.finish({ reason: "stop", usage: usage() }),
          )
