...
afterAll(async () => {
  ...
  for (const file of ["cypher.jsonc", "cypher.json", "config.json", "opencode.json", "opencode.jsonc"]) {
    ...
    withDir({ git: true }, () =>
      Effect.gen(function* () {
        const fiber = yield* ask({
          ...
          permission: "bash",
          patterns: ["cypher-permission-8353 test"],
          metadata: { rules: ["cypher-permission-8353 *", "cypher-permission-8353 test"] },
          always: ["cypher-permission-8353 *", "cypher-permission-8353 test"],
          ruleset: [],
        }).pipe(Effect.forkScoped)
        ...
        yield* saveAlwaysRules({
          ...
          approvedAlways: ["cypher-permission-8353 test"],
        })
        ...
        expect(cfg.permission?.bash).toMatchObject({ "cypher-permission-8353 test": "allow" })
        expect(cfg.permission?.bash).not.toMatchObject({ "cypher-permission-8353 *": "allow" })

        const broad = yield* ask({
          id: PermissionID.make("permission_saved_always_broad"),
          sessionID: SessionID.make("session_saved_always"),
          permission: "bash",
          patterns: ["cypher-permission-8353 install"],
          metadata: {},
          always: [],
          ruleset: [],
        }).pipe(Effect.forkScoped)
