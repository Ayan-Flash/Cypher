...
import { patchModelsDevModel } from "../../src/cypher/provider/provider"

describe("Cypher provider model metadata", () => {
  test("preserves Auto Efficient routing models from Models.dev data", () => {
    const patch = patchModelsDevModel("cypher", {
  ...
  test("Provider.Model schema accepts Auto Efficient routing models", () => {
    const model = Schema.decodeUnknownSync(Provider.Model)({
      ...
      providerID: "cypher",
      api: { id: "cypher", url: "https://cypher.ai", npm: "@cypher/cypher-gateway" },
