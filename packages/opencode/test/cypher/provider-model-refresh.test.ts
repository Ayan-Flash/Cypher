...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    const flags = {
      source: Flag.CYPHER_MODELS_URL,
      path: Flag.CYPHER_MODELS_PATH,
      disabled: Flag.CYPHER_DISABLE_MODELS_FETCH,
      key: process.env.ACME_API_KEY,
    }

    yield* Effect.acquireUseRelease(
      Effect.promise(async () => {
        Flag.CYPHER_MODELS_URL = source
      ...
      () =>
        Effect.promise(async () => {
          Flag.CYPHER_MODELS_URL = flags.source
          Flag.CYPHER_MODELS_PATH = flags.path
          Flag.CYPHER_DISABLE_MODELS_FETCH = flags.disabled
  ...
  { config: { disabled_providers: ["cypher", "apertis"] } },
)
