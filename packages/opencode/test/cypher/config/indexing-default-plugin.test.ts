...
import { CyphercodeDefaultPlugins } from "../../../src/cypher/config/default-plugins"
import { INDEXING_PLUGIN } from "../../../src/cypher/indexing-feature"
...
describe("cypher default indexing plugin", () => {
  afterEach(async () => {
  ...
  test("injects indexing without registering an external plugin origin", () => {
    ...
    CyphercodeDefaultPlugins.apply(config, { disabled: false })
  ...
  test("removes a persisted indexing marker from external plugin origins", () => {
    ...
    CyphercodeDefaultPlugins.apply(config, { disabled: true })
  ...
  test("does not hard-enable indexing plugin when default plugins are disabled", async () => {
    const original = Flag.CYPHER_DISABLE_DEFAULT_PLUGINS
    ...
    try {
      ...
      Flag.CYPHER_DISABLE_DEFAULT_PLUGINS = original
    }
  })
})
