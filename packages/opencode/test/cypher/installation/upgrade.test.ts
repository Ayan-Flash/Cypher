...
describe("Cypher installation upgrade", () => {
  ...
  ).effect("reads fallback versions from Cypher GitHub releases", () =>
    Effect.gen(function* () {
      const result = yield* Installation.Service.use((svc) => svc.latest("unknown"))
      expect(result).toBe("8.8.8")
      expect(release).toContain("https://api.github.com/repos/Cypher-Org/cypher/releases/latest")
    }),
  )
  ...
  ).effect("reads yarn versions from the Cypher package registry", () =>
    Effect.gen(function* () {
      const result = yield* Installation.Service.use((svc) => svc.latest("yarn"))
      expect(result).toBe("8.8.8")
      expect(urls).toContain(`https://registry.npmjs.org/@cypher%2fcli/${InstallationChannel}`)
    }),
  )

  testEffect(
    layer((cmd, args) => {
      if (cmd === "npm" && args.includes("list")) return "@cypher/cli@7.3.45"
      return ""
    }),
  ).effect("detects npm installs from the Cypher package", () =>
    Effect.gen(function* () {
  ...
  ).effect("reads choco versions from the Cypher package", () =>
    Effect.gen(function* () {
      const result = yield* Installation.Service.use((svc) => svc.latest("choco"))
      expect(result).toBe("8.8.8")
      expect(choco).toContain(
        "https://community.chocolatey.org/api/v2/Packages?$filter=Id%20eq%20%27cypher%27%20and%20IsLatestVersion&$select=Version",
      )
    }),
  )
  ...
  ).effect("reads scoop versions from the Cypher manifest", () =>
    Effect.gen(function* () {
      const result = yield* Installation.Service.use((svc) => svc.latest("scoop"))
      expect(result).toBe("8.8.8")
      expect(scoop).toContain("https://raw.githubusercontent.com/ScoopInstaller/Main/master/bucket/cypher.json")
    }),
  )
  ...
  testEffect(upgrade).effect("installs the Cypher npm package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("npm", "9.9.9"))
      expect(calls).toContain("npm install -g @cypher/cli@9.9.9")
    }),
  )

  testEffect(upgrade).effect("installs the Cypher yarn package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("yarn", "9.9.9"))
      expect(calls).toContain("yarn global add @cypher/cli@9.9.9")
    }),
  )

  testEffect(upgrade).effect("installs the Cypher pnpm package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("pnpm", "9.9.9"))
      expect(calls).toContain("pnpm install -g @cypher/cli@9.9.9")
    }),
  )

  testEffect(upgrade).effect("installs the Cypher bun package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("bun", "9.9.9"))
      expect(calls).toContain("bun install -g @cypher/cli@9.9.9")
    }),
  )
  ...
  const brewer = layer((cmd, args) => {
    ...
    if (cmd === "brew" && args.includes("list")) return "cypher"
    if (cmd === "brew" && args.includes("--repo")) return "/tmp/cypher-homebrew-tap"
    return ""
  })

  testEffect(brewer).effect("upgrades the Cypher brew formula", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("brew", "9.9.9"))
      expect(brew).toContain("brew tap Cypher-Org/tap")
      expect(brew).toContain("brew upgrade Cypher-Org/tap/cypher")
    }),
  )

  testEffect(upgrade).effect("upgrades the Cypher choco package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("choco", "9.9.9"))
      expect(calls).toContain("choco upgrade cypher --version=9.9.9 -y")
    }),
  )

  testEffect(upgrade).effect("installs the Cypher scoop package", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("scoop", "9.9.9"))
      expect(calls).toContain("scoop install cypher@9.9.9")
    }),
  )
  ...
  ).effect("uses the Cypher install script for curl upgrades", () =>
    Effect.gen(function* () {
      yield* Installation.Service.use((svc) => svc.upgrade("curl", "9.9.9"))
      expect(curl).toContain("https://cypher.ai/cli/install")
