...
// Tests that unauthenticated Cypher models are assembled with paid models and autoloaded anonymously.
...
import { cypherCustomLoaders, patchCypherProviderPrivacy } from "../../src/cypher/provider/provider"
...
it.live("assembles paid Cypher models without auth", () =>
  Effect.gen(function* () {
    const providers = yield* ModelsDev.Service.use((models) => models.get()).pipe(
      Effect.provide(layer()),
      provideInstance(process.cwd()),
    )
    const cypher = Provider.fromModelsDevProvider(providers.cypher)

    expect(cypher.models["paid-model"]).toMatchObject({
      id: "paid-model",
      providerID: "cypher",
      cost: { input: 1, output: 2 },
      isFree: false,
...
it.live("does not infer free status from zero catalog prices", () =>
  Effect.gen(function* () {
    ...
    expect(cypher.models["free-model"].isFree).toBeUndefined()
  }),
)
...
it.effect("enables a paid catalog when config apiKey is present", () =>
  Effect.gen(function* () {
    const result = yield* load({ config: { provider: { cypher: { options: { apiKey: "test-key" } } } } })
    expect(result.autoload).toBe(true)
...
it.effect("keeps data collection denied after configured options are applied", () =>
  Effect.sync(() => {
    const provider = { options: { dataCollection: "allow", baseURL: "https://api.cypher.ai" } }
    patchCypherProviderPrivacy(provider, { hide_prompt_training_models: true })
    expect(provider.options).toEqual({ dataCollection: "deny", baseURL: "https://api.cypher.ai" })
  }),
)
