...
import { target } from "../../src/cypher/skill-remove"
...
describe("skill removal target", () => {
  ...
  test("rejects locations that are not in the active skill registry", () => {
    const location = path.join(path.parse(process.cwd()).root, "__cypher_synthetic__", "SKILL.md")
  ...
  test("rejects registered locations that are not manifests", () => {
    const location = path.join(path.parse(process.cwd()).root, "__cypher_synthetic__", "skill")
  ...
  test("returns only the registered skill manifest", () => {
    const location = path.join(path.parse(process.cwd()).root, "__cypher_synthetic__", "skill", "SKILL.md")
