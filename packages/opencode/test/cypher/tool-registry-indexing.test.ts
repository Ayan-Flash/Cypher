...
import { CyphercodeBootstrap } from "../../src/cypher/bootstrap"
import { CypherSessions } from "../../src/cypher-sessions/cypher-sessions"
...
describe("cypher tool registry indexing", () => {
  const logger = Log.create({ service: "cypher-tool-registry" })

  it.live("omits semantic_search without waiting for slow indexing startup", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const avail = spyOn(CypherIndexing, "available").mockImplementation(() => new Promise<boolean>(() => {}))
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          const ready = spyOn(CypherIndexing, "ready").mockImplementation(() => {
            throw err
          })
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          const ready = spyOn(CypherIndexing, "ready").mockImplementation(() => Promise.reject(err) as unknown as boolean)
          const warn = spyOn(logger, "warn").mockImplementation(() => {})
  ...
  it.live("registers semantic_search when indexing is enabled", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const ready = spyOn(CypherIndexing, "ready").mockReturnValue(true)
  ...
  it.live("omits semantic_search hint from glob and grep descriptions when indexing is not ready", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const ready = spyOn(CypherIndexing, "ready").mockReturnValue(false)
  ...
  it.live("omits interactive_terminal from subagent definitions", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const prev = process.env["CYPHER_CLIENT"]
        process.env["CYPHER_CLIENT"] = "cli"
        return prev
      }),
      () =>
        provideTmpdirInstance(
      ...
      (prev) =>
        Effect.sync(() => {
          if (prev === undefined) delete process.env["CYPHER_CLIENT"]
          if (prev !== undefined) process.env["CYPHER_CLIENT"] = prev
        }),
    ),
  )
  ...
  test("enables semantic search from indexing configuration before the index is ready", () => {
    ...
    expect(CypherToolRegistry.indexing({}, { indexing: { enabled: true } })).toBe(true)
  })

  it.live("omits memory tools when project memory is disabled but keeps cypher_local_recall", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const agent = yield* Agent.Service
          ...
          expect(ids).not.toContain("cypher_memory_recall")
          expect(ids).not.toContain("cypher_memory_save")
          // cypher_local_recall is a transcript-recall tool gated by `recall: "ask"` in agent
          // permissions; it must NOT be coupled to project-memory enablement.
          expect(ids).toContain("cypher_local_recall")
        }),
      { git: true },
    ),
  )
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          const probe = spyOn(CypherMemory, "toolEnabled")

          try {
            const a = yield* CypherToolRegistry.memoryToolsEnabled({ ctx })
            const b = yield* CypherToolRegistry.memoryToolsEnabled({ ctx })
            const c = yield* CypherToolRegistry.memoryToolsEnabled({ ctx })

            expect([a, b, c]).toEqual([false, false, false])
            // Cache hit: only the first call should reach CypherMemory.toolEnabled.
  ...
  it.live("memoryToolsEnabled reflects enable/disable immediately after invalidate", () =>
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          const ctx = yield* InstanceState.context
          const root = (yield* Effect.promise(() => CypherMemory.prepare({ ctx }))).toString()

          const first = yield* CypherToolRegistry.memoryToolsEnabled({ ctx })
          expect(first).toBe(false)

          yield* Effect.promise(() => CypherMemory.enable({ ctx }))
          ...
          const afterEnable = yield* CypherToolRegistry.memoryToolsEnabled({ ctx })
          expect(afterEnable).toBe(true)

          yield* Effect.promise(() => CypherMemory.disable({ ctx }))

          CypherToolRegistry.invalidateMemoryEnabled(root)
          const afterDisable = yield* CypherToolRegistry.memoryToolsEnabled({ ctx })
    ...
    provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          expect(ids).toContain("cypher_memory_recall")
          expect(ids).toContain("cypher_memory_save")
          expect(ids).toContain("cypher_local_recall")
        }),
      { git: true },
    ),
  ...
  test("conditionally includes Cypher registry extras", () => {
    const prev = process.env["CYPHER_CLIENT"]
    ...
    const tools = {
      ...
      memory: def("cypher_memory_recall"),
      save: def("cypher_memory_save"),
    ...
    try {
      process.env["CYPHER_CLIENT"] = "cli"
      expect(CypherToolRegistry.extra(tools, {}).map((tool) => tool.id)).toEqual([
        "semantic_search",
      ...
      expect(CypherToolRegistry.extra(tools, { experimental: { codebase_search: true } }).map((tool) => tool.id)).toEqual(
        [
          "codebase_search",
          "semantic_search",
          "cypher_memory_recall",
      ...
      expect(
        CypherToolRegistry.extra(tools, { experimental: { codebase_search: true, image_generation: true } }).map(
          (tool) => tool.id,
        ),
      ).toEqual([
        "codebase_search",
      ...
      process.env["CYPHER_CLIENT"] = "vscode"
      expect(CypherToolRegistry.extra(tools, { experimental: { codebase_search: true } }).map((tool) => tool.id)).toEqual(
        [
          "codebase_search",
          "semantic_search",
      ...
      expect(
        CypherToolRegistry.extra(tools, {
      ...
      expect(CypherToolRegistry.extra({ ...tools, semantic: undefined }, {}).map((tool) => tool.id)).toEqual([
        "cypher_memory_recall",
        "cypher_memory_save",
        "recall",
        "background_process",
      ...
      process.env["CYPHER_CLIENT"] = "desktop"
      expect(CypherToolRegistry.extra(tools, {}).map((tool) => tool.id)).toEqual([
        "semantic_search",
        "cypher_memory_recall",
        "cypher_memory_save",
  ...
  test("logs indexing bootstrap failures without blocking session bootstrap", async () => {
    const logger = Log.create({ service: "cypher-bootstrap" })
    const err = new Error("indexing init failed")
    const calls: string[] = []
    const sessions = Layer.succeed(
      CypherSessions.Service,
      CypherSessions.Service.of({ init: () => Effect.sync(() => calls.push("sessions")) }),
    )
    ...
    const indexing = spyOn(CypherIndexing, "init").mockRejectedValue(err)
    const warn = spyOn(logger, "warn").mockImplementation(() => {})

    try {
      await Effect.runPromise(
        CyphercodeBootstrap.Service.use((svc) => svc.init()).pipe(
          Effect.provide(
            CyphercodeBootstrap.layer.pipe(Layer.provide([sessions, bus, memory, session, summary, provider])),
          ),
          Effect.scoped,
        ),
      )
