...
import { AgentManagerTool } from "../../src/cypher/tool/agent-manager"
import { AgentManagerEvent, type AgentManagerStart } from "../../src/cypher/agent-manager/event"
...
const providers = {
  test: {
    ...
    models: {
      ...
      // "Shared" is also offered by the cypher provider, to exercise provider resolution.
  ...
  cypher: {
    ...
    name: "Cypher Gateway",
    models: {
      "cypher/shared": { id: "cypher/shared", providerID: "cypher", name: "Shared", variants: { low: {} } },
      "cypher/only": { id: "cypher/only", providerID: "cypher", name: "Gateway Only", variants: { low: {} } },
    },
  } as unknown as Provider.Info,
  zeta: {
    id: "zeta",
...
// Default provider is `test`, so resolution should prefer test, then cypher, then others.
...
describe("agent_manager tool", () => {
  ...
  test("inherits the latest invoking model and variant when omitted", async () => {
    const task = await publish(runtime, { prompt: "Fix" }, [
      message("msg_current", "cypher", "cypher/shared", "low", 2),
      message("msg_old", "test", "reasoning/model", "high", 1),
    ])

    expect(String(task?.model?.providerID)).toBe("cypher")
    expect(String(task?.model?.modelID)).toBe("cypher/shared")
  ...
  test("explicit model and variant override the invoking selection", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "test/reasoning/model", variant: "high" }, [
      message("msg_current", "cypher", "cypher/shared", "low"),
    ])
  ...
  test("does not inherit a variant when only the model is overridden", async () => {
    ...
    expect(String(task?.model?.providerID)).toBe("cypher")
    expect(String(task?.model?.modelID)).toBe("cypher/only")
    expect(task?.variant).toBeUndefined()
  })
  ...
  test("prefers the invoking provider for an explicit model override", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "Shared", variant: "low" }, [
      message("msg_current", "cypher", "cypher/only", "low"),
    ])
    expect(String(task?.model?.providerID)).toBe("cypher")
    expect(String(task?.model?.modelID)).toBe("cypher/shared")
  })
  ...
  test("falls back to the cypher gateway when the preferred provider lacks the model", async () => {
    const task = await publish(runtime, { prompt: "Fix", model: "Gateway Only" })
    // Default provider `test` does not offer it; cypher is preferred over zeta.
    expect(String(task?.model?.providerID)).toBe("cypher")
  })

  test("narrows to a provider that supports the requested variant", async () => {
    const rt = makeRuntime("cypher")
    // cypher is preferred, but only `test`'s Shared has the `high` variant.
