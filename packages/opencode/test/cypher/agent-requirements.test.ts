...
import * as AgentRequirements from "@/cypher/agent-requirements"
...
async function client(value: string | undefined, run: () => Promise<void>) {
  ...
  try {
    if (value === undefined) delete process.env.CYPHER_CLIENT
    if (value !== undefined) process.env.CYPHER_CLIENT = value
    await run()
  } finally {
