...
describe("cypher tool registry semantic tool import failure", () => {
  test("omits semantic_search when the semantic search tool cannot load", async () => {
    ...
    try {
      const result = await provideTestInstance({
        ...
        fn: () =>
          Effect.runPromise(
            CypherToolRegistry.build(infos(), deps, {
              indexing: async () => ({
                CypherIndexing: {
                  ready: () => true,
                },
...
function infos() {
  return {
    ...
    memory: info("cypher_memory_recall"),
    save: info("cypher_memory_save"),
