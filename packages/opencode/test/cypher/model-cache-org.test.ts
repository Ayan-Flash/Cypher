// Regression test: OAuth accountId must flow into model fetch as cypherOrganizationId
...
type Options = Parameters<ModelCache.CypherModels["fetch"]>[0]

function layer(info: Auth.Info | undefined, captured: Ref.Ref<Options | undefined>) {
  const auth = Layer.mock(Auth.Service)({
    get: (id) => Effect.succeed(id === "cypher" ? info : undefined),
  })
...
it.live("model fetch uses accountId from OAuth auth as cypherOrganizationId", () =>
  Effect.gen(function* () {
    ...
    yield* ModelCache.Service.use((cache) => cache.fetch("cypher")).pipe(Effect.provide(layer(info, captured)))
    expect(yield* Ref.get(captured)).toMatchObject({
      cypherToken: "test-oauth-token",
      cypherOrganizationId: "org-enterprise-123",
    })
  }),
)

it.live("model fetch without OAuth accountId does not set cypherOrganizationId", () =>
  Effect.gen(function* () {
    const captured = yield* Ref.make<Options | undefined>(undefined)
    const info = new Auth.Oauth({
      type: "oauth",
    ...
    yield* ModelCache.Service.use((cache) => cache.fetch("cypher")).pipe(Effect.provide(layer(info, captured)))
    expect(yield* Ref.get(captured)).toMatchObject({ cypherToken: "test-personal-token" })
    expect((yield* Ref.get(captured))?.cypherOrganizationId).toBeUndefined()
  }),
)
  ...
  Effect.gen(function* () {
    ...
    yield* ModelCache.Service.use((cache) =>
      Effect.gen(function* () {
        ...
        expect(yield* cache.get("cypher")).toBeDefined()

        yield* cache.clear("cypher")
        expect(yield* cache.get("cypher")).toBeUndefined()
