...
import type { Part, StepFinishPart } from "@cypher/sdk/v2"
import { RoutedModelMeta } from "../../src/cypher/cli/cmd/tui/routes/session/routed-model-meta"
import { CypherRoutedModel } from "../../src/cypher/session/routed-model"
...
describe("session routed model", () => {
  ...
  test("preserves finish-step response model in provider metadata", async () => {
    const events = await adapt([
      unchecked({
        ...
        usage: {},
        providerMetadata: { openrouter: { routed: true }, cypher: { existing: true } },
      }),
    ])
    ...
    expect(event.providerMetadata).toEqual({
      ...
      cypher: { existing: true, routedModelID: "openai/gpt-5.5-20260423" },
    })
  })
  ...
  test("shortens date-suffixed routed model ids for display", () => {
    expect(CypherRoutedModel.display("moonshotai/kimi-k2.7-code-20260612")).toBe("moonshotai/kimi-k2.7-code")
    expect(CypherRoutedModel.display("openai/gpt-5.5-2026-04-23")).toBe("openai/gpt-5.5")
    expect(CypherRoutedModel.display("openai/gpt-5.5")).toBe("openai/gpt-5.5")
  })

  test("formats routed model names for compact display", () => {
    expect(CypherRoutedModel.displayName("Qwen: Qwen3.7 Plus (20% off)")).toBe("Qwen 3.7 Plus")
    expect(CypherRoutedModel.displayName("anthropic.claude-opus-4-5-20251101-v1:0")).toBe(
      "anthropic.claude-opus-4-5-20251101-v1:0",
    )
    expect(CypherRoutedModel.displayName("moonshotai/kimi-k2.7-code")).toBe("kimi-k2.7-code")
    expect(CypherRoutedModel.displayName("o3")).toBe("o3")
  })

  test("shows compact labels only for Cypher auto selections", () => {
    ...
    const same = RoutedModelMeta.info(
      undefined,
      [reason, finish({ providerID: "cypher", modelID: "cypher-auto/efficient" })],
      false,
      {
        providerID: "cypher",
        modelID: "cypher-auto/efficient",
  ...
  test("reads routed model only for selected Cypher auto models", () => {
    const meta = { cypher: { routedModelID: "openai/gpt-5.5-20260423" } }

    expect(
      CypherRoutedModel.readAuto(meta, {
        providerID: ProviderID.cypher,
        modelID: "cypher-auto/efficient",
      }),
    ).toEqual({
      providerID: ProviderID.cypher,
