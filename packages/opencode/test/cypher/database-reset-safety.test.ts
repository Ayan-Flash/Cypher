...
const paths = [/\bDatabase\.getPath\s*\(/, /\bDatabase\.Path\b/, /cypher\.db(?:-wal|-shm)?/]
...
//    so CYPHER_DB was not set to :memory:.
// 2. The test process inherited CYPHER_DISABLE_CHANNEL_DB=true from the VS Code
//    extension. Database.getPath() therefore returned the real shared database
//    at ~/.local/share/cypher/cypher.db.
// 3. The upstream resetDatabase() helper deleted cypher.db, cypher.db-wal, and
//    cypher.db-shm. The test then recreated the database and inserted its
//    identifiable "parent" and "child" sessions.
// 4. Conversations stored only in the deleted database were lost. Git
//    worktrees, branches, and .cypher/agent-manager.json survived because they are
//    stored separately.
//
...
describe("test database safety", () => {
  ...
  test("preserves the resolved database when the in-memory preload is missing", async () => {
    ...
    // user database. This simulates a disk-backed CYPHER_DB and verifies that the
    // reset helper rejects it before cleanup can run.
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "sessions.db")
    const previous = Flag.CYPHER_DB
