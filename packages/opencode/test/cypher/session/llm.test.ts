...
describe("cypher.session.llm.timeout", () => {
  test("uses prepared options before the provider fallback", () => {
    const result = CypherLLM.timeout({
  ...
  test("omits the timeout when it is not configured", () => {
    expect(CypherLLM.timeout({ options: {} })).toEqual({})
  })
})

describe("cypher.session.llm.text", () => {
  ...
  test("fails on stream interruption", async () => {
    const text = CypherLLM.text(Stream.fail(new DOMException("Aborted", "AbortError")))
