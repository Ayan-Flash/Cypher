    ...
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".cypher", "agent", "skip.md"),
          `---
mode: "banana"
---
Broken agent prompt`,
        ...
        await Filesystem.write(
          path.join(dir, ".cypher", "agent", "keep.md"),
          `---
model: test/model
---
Valid agent prompt`,
...
Broken agent prompt`,
  ...
  test("skips invalid command markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".cypher", "command", "skip.md"),
          `---
subtask: "banana"
---
Broken command template`,
        ...
        await Filesystem.write(
          path.join(dir, ".cypher", "command", "keep.md"),
          `---
...
Broken command template`,
  ...
  test("collects warnings for invalid agent markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".cypher", "agent", "broken.md"),
          `---
mode: "banana"
---
Broken agent`,
  ...
  test("collects warnings for invalid command markdown configs", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(
          path.join(dir, ".cypher", "command", "broken.md"),
          `---
subtask: "banana"
---
Broken command`,
  ...
  test("collects warnings for invalid JSON in .cypher directory config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, ".cypher", "cypher.json"), "{ not valid json !!!")
      },
    })
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(warns.some((w) => w.path.includes("cypher.json") && w.message.includes("not valid JSON"))).toBe(true)
      },
    })
  })

  test("collects warnings for invalid schema in .cypher directory config", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        await Filesystem.write(path.join(dir, ".cypher", "cypher.json"), JSON.stringify({ unknownField: true }))
      },
    })
    ...
    await provideTestInstance({
      ...
      fn: async () => {
        ...
        expect(cfg).toBeDefined()
        expect(warns.some((w) => w.path.includes("cypher.json") && w.message.includes("invalid"))).toBe(true)
      },
    })
  })
