...
import { Daemon } from "../../src/cypher/daemon/daemon"
import { DaemonClient } from "../../src/cypher/daemon/client"
import { tmpdir } from "../fixture/fixture"

const original = {
  state: process.env.CYPHER_TEST_DAEMON_STATE_DIR,
  log: process.env.CYPHER_TEST_DAEMON_LOG_DIR,
  disabled: process.env.CYPHER_NO_DAEMON,
}

afterEach(async () => {
  if (process.env.CYPHER_TEST_DAEMON_STATE_DIR !== original.state) await Daemon.stop().catch(() => undefined)
  restore()
})

function restore() {
  if (original.state === undefined) delete process.env.CYPHER_TEST_DAEMON_STATE_DIR
  else process.env.CYPHER_TEST_DAEMON_STATE_DIR = original.state
  if (original.log === undefined) delete process.env.CYPHER_TEST_DAEMON_LOG_DIR
  else process.env.CYPHER_TEST_DAEMON_LOG_DIR = original.log
  if (original.disabled === undefined) delete process.env.CYPHER_NO_DAEMON
  else process.env.CYPHER_NO_DAEMON = original.disabled
}

function dirs(root: string) {
  process.env.CYPHER_TEST_DAEMON_STATE_DIR = path.join(root, "state")
  process.env.CYPHER_TEST_DAEMON_LOG_DIR = path.join(root, "log")
  return {
    XDG_DATA_HOME: path.join(root, "xdg-data"),
    XDG_CONFIG_HOME: path.join(root, "xdg-config"),
    XDG_STATE_HOME: path.join(root, "xdg-state"),
    XDG_CACHE_HOME: path.join(root, "xdg-cache"),
    CYPHER_TEST_DAEMON_EPHEMERAL_PORT: "1",
  }
}
...
function cli(args: string[], env: NodeJS.ProcessEnv = {}) {
  return Bun.spawn([process.execPath, "--conditions=browser", path.join(process.cwd(), "src/index.ts"), ...args], {
    ...
    env: {
      ...process.env,
      ...env,
      CYPHER_CONFIG_CONTENT: '{"experimental":{"openTelemetry":false}}',
      CYPHER_DISABLE_PROJECT_CONFIG: "1",
      CYPHER_DISABLE_AUTOUPDATE: "1",
...
describe("daemon manager", () => {
  ...
  test("does not forward bundled bun entrypoints to the daemon child", () => {
    const proc = {
      argv: ["/tmp/cypher", "/$bunfs/root/src/index.js", "daemon", "start"],
      execArgv: ["--user-agent=cypher/test", "--use-system-ca", "--"],
      execPath: "/tmp/cypher",
    }
    expect(Daemon.command(undefined, proc)).toStrictEqual(["/tmp/cypher"])
    expect(
      Daemon.command(undefined, {
        ...proc,
        argv: ["C:/tmp/cypher.exe", "B:/~BUN/root/src/index.js", "daemon", "start"],
        execPath: "C:/tmp/cypher.exe",
      }),
    ).toStrictEqual(["C:/tmp/cypher.exe"])
    expect(
      Daemon.command(undefined, {
        ...proc,
        argv: ["C:/tmp/cypher.exe", "b:\\~BUN\\root\\src\\index.js", "daemon", "start"],
  ...
  test("forwards source entrypoints to the daemon child", () => {
    expect(
      Daemon.command(undefined, {
        argv: ["/tmp/bun", "/tmp/cypher/src/index.ts", "daemon", "start"],
        execArgv: ["--conditions=browser"],
        execPath: "/tmp/bun",
      }),
    ).toStrictEqual(["/tmp/bun", "--conditions=browser", "/tmp/cypher/src/index.ts"])
  })
  ...
  test("does not reuse legacy fixed-password daemons", () => {
    ...
    const state: Daemon.State = {
      ...
      username: "cypher",
      password: "cypher",
      token: Buffer.from("cypher:cypher").toString("base64"),
      version: "test",
  ...
  test("starts, reuses, authenticates, and stops a daemon", async () => {
    ...
    expect(started.state?.password).not.toBe("cypher")
    expect(started.state?.token).not.toBe(Buffer.from("cypher:cypher").toString("base64"))
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const source = path.join(process.cwd(), "src/cypher/daemon/daemon.ts")
      const script = `
  ...
  test("supports console stop as a daemon stop alias", async () => {
    ...
    expect(code).toBe(0)
    expect(stdout).toContain("cypher daemon stopped")
  ...
  test.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const stdout = capture(proc.stdout, "Press Ctrl+C to stop the Cypher daemon.")
      const stderr = new Response(proc.stderr).text()
      ...
      try {
        ...
        expect(await deadline(proc.exited, 10_000)).toBe(0)
        expect((await Daemon.status()).running).toBe(false)
        expect(await stdout.text).toContain("cypher daemon started")
        await stderr
      } finally {
        if (proc.exitCode === null) proc.kill("SIGKILL")
        await proc.exited
  ...
  test("daemon client honors the escape hatch", async () => {
    await using tmp = await tmpdir()
    const started = await Daemon.start(opts(tmp.path))
    process.env.CYPHER_NO_DAEMON = "1"
