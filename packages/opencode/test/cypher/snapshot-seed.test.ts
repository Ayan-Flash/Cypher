...
import { CypherSnapshotMaterialize } from "../../src/cypher/snapshot/materialize"
import { CypherSnapshotSeed } from "../../src/cypher/snapshot/seed"
...
test(
  ...
  async () => {
    ...
    expect(
      (
        await $`git --git-dir=${common} rev-parse --verify --quiet ${CypherSnapshotMaterialize.ref(fast.gitdir)}`
          .nothrow()
          .text()
      ).trim(),
    ).toBe("")
...
test(
  ...
  async () => {
    ...
    const sourceRef = CypherSnapshotMaterialize.ref(result.gitdir)
    ...
    const expired = `refs/cypher/snapshots/1/${result.value!}`
    ...
    expect((await $`git --git-dir=${result.gitdir} for-each-ref refs/cypher/snapshots`.text()).trim()).toContain(
      result.value!,
    )

    const locked = `refs/cypher/snapshots/2/${result.value!}`
...
test("interrupted seed removes borrowed state after source gc", async () => {
  ...
  const original = await fs.readFile(index)
  const ref = CypherSnapshotMaterialize.ref(gitdir)
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      const fiber = yield* CypherSnapshotSeed.seed({
        dir: source.path,
