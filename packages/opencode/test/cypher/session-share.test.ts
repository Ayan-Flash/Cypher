...
it.instance("shares and unshares sessions through Cypher public URLs", () => {
  const urls: string[] = []
  ...
  return Effect.gen(function* () {
    ...
    expect(result.url).toBe("https://app.cypher.ai/s/public-1")
    expect((yield* session.get(info.id)).share?.url).toBe("https://app.cypher.ai/s/public-1")
    ...
    Effect.ensuring(
      Effect.gen(function* () {
        const auth = yield* Auth.Service
        yield* auth.remove("cypher").pipe(Effect.ignore)
        request.mockRestore()
      }),
    ),
