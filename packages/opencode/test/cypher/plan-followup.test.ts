...
import { formatTodos, generateHandover, PlanFollowup, PlanFollowupRuntime } from "../../src/cypher/plan-followup"
...
describe("plan follow-up", () => {
  ...
  test("ask - hides custom answer row on VS Code where the main prompt input handles typed replies", () =>
    withInstance(async () => {
      ...
      try {
        ...
        process.env.CYPHER_CLIENT = prev
      }
    }))
  ...
  test("ask - retargets prompt queue so injected message is visible in scope", () =>
    withInstance(async () => {
      const { CypherSessionPromptQueue } = await import("../../src/cypher/session/prompt-queue")
      ...
      CypherSessionPromptQueue.retarget(seeded.sessionID, original)
      ...
      const scoped = CypherSessionPromptQueue.scope(seeded.sessionID, all)
