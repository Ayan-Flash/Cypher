...
import { CypherCompactionPayloadRecovery } from "../../src/cypher/session/compaction-payload-recovery"
...
describe("CypherCompactionPayloadRecovery", () => {
  test("detects Cypher gateway payload-size errors", () => {
    const error = new MessageV2.ContextOverflowError({
      message: "Request Entity Too Large",
      responseBody: "Request Entity Too Large\n\nFUNCTION_PAYLOAD_TOO_LARGE",
    }).toObject()

    expect(CypherCompactionPayloadRecovery.matches(error)).toBe(true)
  })
  ...
  test("strips media and marks completed tool outputs compacted", async () => {
    ...
    await Effect.runPromise(
      CypherCompactionPayloadRecovery.strip({
  ...
  test("retries compaction without media and tool outputs after payload-size failure", async () => {
    ...
    stub.push((input) => {
      ...
      return Stream.fail(
        new APICallError({
          message: "Request Entity Too Large",
          url: "https://api.cypher.ai/api/openrouter/responses",
          requestBodyValues: {},
          statusCode: 413,
