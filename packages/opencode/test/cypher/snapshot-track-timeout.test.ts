...
//
// Unit tests for CypherSnapshotTrack.wrap — the slow-repo guard that sits
...
import { CypherSnapshotTrack } from "../../src/cypher/snapshot/track"
import { CypherPartLifecycle } from "../../src/cypher/session/part-lifecycle"
...
const makeHooks = (
  answer: CypherSnapshotTrack.Answer | Promise<CypherSnapshotTrack.Answer>,
): { hooks: CypherSnapshotTrack.Hooks; calls: Calls } => {
  const calls: Calls = { ask: 0, persist: 0, progress: [] }
  const hooks: CypherSnapshotTrack.Hooks = {
    async ask() {
      calls.ask += 1
      return answer
    },
...
describe("CypherSnapshotTrack.protect", () => {
  it.effect("returns at the availability deadline without waiting for cancellation", () =>
    Effect.gen(function* () {
      ...
      const fiber = yield* CypherSnapshotTrack.protect({
        inner,
        state,
        fallback,
        operation: "patch",
  ...
  it.effect("bypasses later operations after a deadline opens the directory circuit", () =>
    Effect.gen(function* () {
      ...
      const started = yield* Deferred.make<void>()
      let calls = 0
      const first = yield* CypherSnapshotTrack.protect({
        inner: Effect.sync(() => {
          calls += 1
      ...
      const second = yield* CypherSnapshotTrack.protect({
        inner: Effect.sync(() => {
          calls += 1
          return "unexpected"
        }),
  ...
  test("keeps circuit state isolated by directory", () => {
    const states = CypherSnapshotTrack.makeStates()
...
describe("CypherSnapshotTrack.wrap", () => {
  ...
  test("concurrent timeout does not override an active continue choice", async () => {
    const state = CypherSnapshotTrack.makeState()
    const answer = Promise.withResolvers<CypherSnapshotTrack.Answer>()
    const asked = Promise.withResolvers<void>()
    const { hooks, calls } = makeHooks(answer.promise)
    const firstHooks: CypherSnapshotTrack.Hooks = {
      ...hooks,
      async ask(input) {
        asked.resolve()
...
describe("CypherSnapshotTrack progress indicator", () => {
  test("classifies persisted progress as transient", () => {
    const part = CypherSnapshotTrack.progressPart({
      sessionID: SESSION,
      messageID: MESSAGE,
    ...
    expect(CypherPartLifecycle.transient(part)).toBe(true)
  })
  ...
  test("slow-but-succeeding path (under timeout) starts then ends the indicator", async () => {
    ...
    expect(withoutFrame(firstText.text)).toBe(
      CypherSnapshotTrack.formatProgress(CypherSnapshotTrack.PROGRESS_INITIALIZING, "").trim(),
    )
  ...
  test("timed-out path keeps the initializing label (no text escalation)", async () => {
    ...
    const base = CypherSnapshotTrack.formatProgress(CypherSnapshotTrack.PROGRESS_INITIALIZING, "").trim()
    for (const evt of events) {
...
describe("CypherSnapshotTrack constants", () => {
  test("TIMEOUT_MS defaults to 10s and respects env override", () => {
    ...
    // with CYPHER_SNAPSHOT_TRACK_TIMEOUT_MS, which this test suite does not set.
    expect(CypherSnapshotTrack.TIMEOUT_MS).toBe(10_000)
  })

  test("exposes stable answer labels", () => {
    expect(CypherSnapshotTrack.ANSWER_CONTINUE).toBe("Continue with snapshots")
    expect(CypherSnapshotTrack.ANSWER_DISABLE).toBe("Disable for this project")
  })
})
