...
import type { Config, IndexingConfig } from "@cypher/sdk/v2"
import {
  ...
  mergeIndexingConfig,
  type IndexingScope,
} from "../../src/cypher/components/indexing-dialog-state"

describe("indexing dialog state", () => {
  test.serial("loads Cypher models directly from the public catalog", async () => {
    ...
    global.fetch = (async (input) => {
      ...
      return new Response(
        JSON.stringify({
          defaultModel: "cypher/default",
          models: [{ id: "cypher/default", name: "Default", dimension: 1024, scoreThreshold: 0.35 }],
          aliases: {},
        }),
    ...
    try {
      const catalog = await loadCypherEmbeddingModels()

      expect(catalog.models).toHaveLength(1)
      expect(catalog.defaultModel).toBe("cypher/default")
  ...
  test("builds stable loading, empty, and loaded model options", () => {
    expect(cypherModelOptions()).toEqual([{ value: "", title: "Loading supported models..." }])
    expect(cypherModelOptions({ defaultModel: "", models: [], aliases: {} })).toEqual([
    ...
    expect(cypherModelOptions(catalog)).toEqual([
      { value: "provider/default", title: "Default (1024d)" },
      { value: "provider/code", title: "Code (code, 1536d)" },
    ])
    expect(currentCypherModel(catalog, "code")).toBe("provider/code")
    expect(currentCypherModel(catalog, "missing")).toBe("provider/default")
  })
  ...
  test("isolates global auth config from project indexing values", () => {
    const project: IndexingConfig = { cypher: { apiKey: "project-key", baseUrl: "https://project.test" } }
    const inherited: IndexingConfig = { enabled: true }
    const effective: Config = {
      provider: { cypher: { options: { apiKey: "provider-key" } } },
      indexing: project,
    }
