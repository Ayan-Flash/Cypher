...
import { capture } from "../../src/cypher/instance" // cypher_change
...
describe("InstanceStore", () => {
  it.live("loads instance context", () =>
    Effect.gen(function* () {
      ...
      // cypher_change start - capture prefers legacy ALS, then falls back to the Effect fiber reference
  ...
  it.live("runs bootstrap with InstanceRef provided", () =>
    Effect.gen(function* () {
      ...
      expect(capture()).toBeUndefined() // cypher_change - bootstrap legacy ALS does not leak into the caller
    }),
  )
  ...
  // cypher_change start - reload disposers retain legacy instance context
  ...
  // cypher_change - InstanceStore.boot provides InstanceRef to bootstrap.run so
  // CyphercodeBootstrap (and anything it forkDetaches, e.g. CypherIndexing.init) can read
  // the current directory. This regression test pins the Cypher contract.
  it.live("provides InstanceRef during bootstrap for Cypher bootstrap compatibility", () =>
    Effect.gen(function* () {
      const dir = yield* tmpdirScoped({ git: true })
      const store = yield* InstanceStore.Service
