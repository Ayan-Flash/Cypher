...
// cypher_change start - skip on windows: address windows ci failures #9496
...
const githubBase = <A, E, R>(url: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const previous = process.env.CYPHER_REPO_CLONE_GITHUB_BASE_URL
      process.env.CYPHER_REPO_CLONE_GITHUB_BASE_URL = url
      return previous
    }),
    () => self,
    (previous) =>
      Effect.sync(() => {
        if (previous) process.env.CYPHER_REPO_CLONE_GITHUB_BASE_URL = previous
        else delete process.env.CYPHER_REPO_CLONE_GITHUB_BASE_URL
      }),
  )
...
describe("tool.glob", () => {
  ...
  // cypher_change start - absolute glob patterns outside the project
  unixInstance(
