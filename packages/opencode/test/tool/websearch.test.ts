...
describe("websearch provider", () => {
  ...
  test("supports an operational override", () => {
    const original = process.env.CYPHER_WEBSEARCH_PROVIDER

    try {
      process.env.CYPHER_WEBSEARCH_PROVIDER = "parallel"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("parallel")

      process.env.CYPHER_WEBSEARCH_PROVIDER = "exa"
      expect(selectWebSearchProvider(SESSION_ID)).toBe("exa")
    } finally {
      if (original === undefined) delete process.env.CYPHER_WEBSEARCH_PROVIDER
      else process.env.CYPHER_WEBSEARCH_PROVIDER = original
    }
  })
  ...
  test("is only enabled for cypher or explicit websearch provider flags", () => {
    // cypher_change
    expect(webSearchEnabled(ProviderID.cypher, { exa: false, parallel: false })).toBe(true) // cypher_change
    expect(webSearchEnabled(ProviderID.opencode, { exa: false, parallel: false })).toBe(false) // cypher_change
