...
// cypher_change - skip on windows: address windows ci failures #9496
...
describe("tool.skill", () => {
  unix("execute returns skill content block with files", () =>
    provideTmpdirInstance((dir) =>
      Effect.gen(function* () {
        const skill = path.join(dir, ".cypher", "skill", "tool-skill") // cypher_change
...
`,
        ...
        const home = process.env.CYPHER_TEST_HOME
  ...
  // cypher_change start
  it.live("built-in cypher-config includes named command lookup guidance", () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          ...
          const result = yield* tool.execute({ name: "cypher-config" }, ctx)

          expect(result.metadata.dir).toBe("builtin")
          expect(result.output).toContain("Finding a named command")
          expect(result.output).toContain("~/.config/cypher/")
          expect(result.output).toContain("~/.cypher/")
