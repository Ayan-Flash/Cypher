...
import { Cause, Deferred, Effect, Exit, Fiber, Layer } from "effect" // cypher_change - Cause/Deferred for resume-hint coverage
...
import { MessageID, PartID, SessionID } from "../../src/session/schema" // cypher_change - SessionID used by cost propagation tests
import { SessionRunState } from "@/session/run-state"
import { SessionStatus } from "@/session/status"
import { ModelID, ProviderID } from "../../src/provider/schema"
import { Provider } from "../../src/provider/provider" // cypher_change
import { CypherSession } from "../../src/cypher/session" // cypher_change
...
const layer = (flags: Partial<RuntimeFlags.Info> = {}) =>
  Layer.mergeAll(
    ...
    Provider.defaultLayer, // cypher_change
...
// cypher_change start - stub signature + prompt body extended to persist assistant cost for propagation tests
...
describe("tool.task", () => {
  ...
  // cypher_change start - resumed children rebuild parent platform attribution after restart
  it.instance("execute preserves platform attribution when resuming a task", () =>
    Effect.gen(function* () {
      const sessions = yield* Session.Service
      const { chat, assistant } = yield* seed()
      CypherSession.setPlatformOverride(chat.id, "agent-manager")
      const child = yield* sessions.create({ parentID: chat.id, title: "Existing child" })
      ...
      expect(CypherSession.resolveRoot(child.id)).toBe(chat.id)
    }),
  )
  // cypher_change end
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // cypher_change start — use arrayContaining: Cypher appends inherited caller restrictions
        ...
        expect(seen?.tools).toEqual({
          ...
          todowrite: false,
          task: false, // cypher_change - Cypher disallows nested subagents
          bash: false,
          read: false,
        })
      }),
    {
      config: {
        ...
        experimental: {
          ...
          openTelemetry: true, // cypher_change
        },
      },
    },
  )

  // cypher_change start - terminal child assistant errors fail the task tool boundary
  ...
  // cypher_change start - background subagent failures also surface the resumable task_id (#11620)
  ...
  // cypher_change start - completed background tasks propagate their invocation cost delta
...
// cypher_change start - subagent cost propagation coverage (#6321)
