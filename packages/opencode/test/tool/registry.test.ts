...
import { Effect, Exit, Layer, Result, Schema } from "effect" // cypher_change
...
import { Command } from "@/command" // cypher_change
import { Auth } from "@/auth" // cypher_change
import * as SandboxNetwork from "@/cypher/sandbox/network" // cypher_change
import { run as runSandbox, type Profile } from "@cypher/sandbox" // cypher_change
import { MemoryService } from "@cypher/cypher-memory/effect/service" // cypher_change

const node = CrossSpawnSpawner.defaultLayer
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".cypher")])), // cypher_change
})
...
const registryLayer = (opts: RegistryLayerOptions = {}) =>
  ToolRegistry.layer
    ...
    .pipe(
      ...
      Layer.provide(Command.defaultLayer), // cypher_change
      Layer.provide(Auth.defaultLayer), // cypher_change
      Layer.provide(MemoryService.layer), // cypher_change
    )
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .cypher/tool (singular)" /* cypher_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const opencode = path.join(test.directory, ".cypher") // cypher_change
  ...
  it.instance("ignores non-tool exports in .cypher/tool files" /* cypher_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".cypher", "tool") // cypher_change
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools from .cypher/tools (plural)" /* cypher_change */, () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const opencode = path.join(test.directory, ".cypher") // cypher_change
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".cypher", "tools") // cypher_change
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const plugin = path.join(opencode, "node_modules", "@cypher", "plugin") // cypher_change
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@cypher/plugin", type: "module", exports: { ".": "./dist/index.js" } }), // cypher_change
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@cypher/plugin"', // cypher_change
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".cypher", "tools") // cypher_change
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
