...
The MCP `StdioClientTransport` (third-party SDK) is handled separately via a process shim in `src/mcp/index.ts` that sets `process.type = "browser"` when running inside the VS Code extension (`CYPHER_PLATFORM=vscode`), which causes the SDK's internal `isElectron()` check to return `true` and enable `windowsHide`.

## Storage

Filesystem-based JSON, not a database. Data lives in `~/.local/share/cypher/storage/`. Keys are path arrays: `Storage.write(["session", projectID, sessionID], data)`.

## TUI

Built with **SolidJS + OpenTUI** (`@opentui/solid`) -- a terminal UI framework. JSX renders to the terminal using elements like `<box>`, `<text>`, `<scrollbox>`. The TUI communicates with the server via `@cypher/sdk`.
...
`opencode/` is a fork of upstream opencode. When a change must touch a shared upstream file, extract the Cypher-specific logic into a mirror file under `src/cypher/<same/path>.ts` (tests under `test/cypher/<same/path>.test.ts`) and call into it from the upstream file behind a single `cypher_change` marker. Example: a Cypher override for `src/cli/cmd/tui/component/dialog-provider.tsx` lives at `src/cypher/cli/cmd/tui/component/dialog-provider.tsx`. Avoid inlining Cypher-specific logic directly into shared upstream files. Files and directories whose path contains `cypher` never need `cypher_change` markers.
