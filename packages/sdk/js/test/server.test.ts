// cypher_change start - Tests for CYPHER_CONFIG_CONTENT merging
import { describe, test, expect, beforeEach, afterEach } from "bun:test"
import { buildConfigEnv } from "../src/server"

describe("buildConfigEnv", () => {
  const originalEnv = process.env.CYPHER_CONFIG_CONTENT

  beforeEach(() => {
    delete process.env.CYPHER_CONFIG_CONTENT
  })
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      ...
      process.env.CYPHER_CONFIG_CONTENT = originalEnv
    }
  })
  ...
  test("preserves existing CYPHER_CONFIG_CONTENT when spawning with new config", () => {
    // Simulate Cyphercode having injected modes via CYPHER_CONFIG_CONTENT
    process.env.CYPHER_CONFIG_CONTENT = JSON.stringify({
      agent: {
        translate: { mode: "primary", prompt: "You are a translator" },
      },
      instructions: [".cypher/rules/main.md"],
    })
    ...
    expect(parsed.instructions).toContain(".cypher/rules/main.md")
  ...
  test("handles invalid JSON in existing CYPHER_CONFIG_CONTENT gracefully", () => {
    process.env.CYPHER_CONFIG_CONTENT = "invalid json {"
