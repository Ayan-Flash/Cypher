...
import { CypherClient } from "./gen/sdk.gen.js"
import { wrapClientError } from "../error-interceptor.js"
export { type Config as CypherClientConfig, CypherClient }
...
function rewrite(request: Request, values: { directory?: string; workspace?: string }) {
  ...
  for (const [name, key] of [
    ["x-cypher-directory", "directory"],
    ["x-cypher-workspace", "workspace"],
  ] as const) {
    const value = pick(
  ...
  next.headers.delete("x-cypher-directory")
  next.headers.delete("x-cypher-workspace")
  return next
}

export function createCypherClient(config?: Config & { directory?: string; experimental_workspaceID?: string }) {
  if (!config?.fetch) {
  ...
  if (config?.directory) {
    config.headers = {
      ...config.headers,
      "x-cypher-directory": encodeURIComponent(config.directory),
    }
  }
  ...
  if (config?.experimental_workspaceID) {
    config.headers = {
      ...config.headers,
      "x-cypher-workspace": config.experimental_workspaceID,
    }
  }
  ...
  return new CypherClient({ client })
}
