...
function rewrite(request: Request, directory?: string) {
  ...
  const value = pick(request.headers.get("x-cypher-directory"), directory)
  ...
  const next = new Request(url.href, request) // cypher_change
  next.headers.delete("x-cypher-directory")
  return next
}

export function createCypherClient(config?: Config & { directory?: string }) {
  if (!config?.fetch) {
