 ...
 * then copies it into packages/cypher-vscode/bin/cypher.
 *
...
const targetBinPath = join(targetBinDir, "cypher")
...
function sourceBinaryPath(): string {
  return join(opencodeDir, "dist", `cypher-cli-${process.platform}-${process.arch}`, "bin", "cypher")
}
...
async function rebuild() {
  ...
  try {
    ...
    log(`Binary updated (${elapsed}s): ${relative(packagesDir, source)} -> bin/cypher`)
  } catch (err) {
...
// Watch for changes
log(`Watching ${relative(cypherVscodeDir, opencodeSrcDir)}/ for changes...`)
