...
Cypher CLI is an open source AI coding agent platform. It ships as a CLI and editor clients that all build on the same backend. This package (`packages/cypher-vscode/`) is the **VS Code extension**.

### Products and How They Relate

All products are thin clients over the **CLI** (`packages/opencode/`, published as `@cypher/cli`). The CLI is a fork of upstream [OpenCode](https://github.com/anomalyco/opencode) with Cypher-specific additions (gateway auth, telemetry, migration, code review, branding). It contains the full AI agent runtime, tool execution, session management, provider integrations (500+ models), and an HTTP API server.

Every client spawns or connects to a `cypher serve` process and communicates via HTTP REST + SSE using the auto-generated `@cypher/sdk`.

```
                        @cypher/cli  (packages/opencode/)
                     ┌────────────────────────────────┐
...
|---|---|---|---|
| Cypher CLI (TUI) | `packages/opencode/` | Interactive terminal UI (SolidJS + OpenTUI) | In-process — TUI and server run together |
| Cypher CLI (`cypher run`) | `packages/opencode/` | Non-interactive headless mode for scripting | In-process — no network socket |
| **Cypher VS Code Extension** | **`packages/cypher-vscode/`** | VS Code extension with sidebar chat + Agent Manager | Bundles CLI binary, spawns `cypher serve --port 0` as child process |

### Cypher-Domain Packages

| Package | Name | Role |
|---|---|---|
| `packages/cypher-vscode/` | `cypher-cli` | **This package.** VS Code extension. |
| `packages/cypher-gateway/` | `@cypher/cypher-gateway` | Auth (device flow), AI provider routing (OpenRouter), Cypher API integration (profile, balance, teams) |
| `packages/cypher-ui/` | `@cypher/cypher-ui` | SolidJS component library (40+ components, built on `@kobalte/core`). Shared by this extension's webview and docs screenshot stories |
| `packages/cypher-telemetry/` | `@cypher/cypher-telemetry` | PostHog analytics + OpenTelemetry tracing for the CLI |
| `packages/cypher-i18n/` | `@cypher/cypher-i18n` | Translation strings (16 languages) |
| `packages/cypher-docs/` | `@cypher/cypher-docs` | Documentation site (Next.js + Markdoc) |

### Upstream OpenCode Packages (not Cypher-specific)

| Package | Name | Role |
|---|---|---|
| `packages/opencode/` | `@cypher/cli` | Core CLI — forked from upstream OpenCode. AI agents, tools, sessions, server. |
...
The extension bundles its own CLI binary at `bin/cypher` — it does NOT use a system-installed CLI. To build it:
...
The script checks for a prebuilt binary in `packages/opencode/dist/`, builds the CLI if needed, and copies it to `bin/cypher`.

## Architecture

### Extension ↔ CLI Backend

The extension is a client of the CLI. Activation creates one shared `CypherConnectionService`; on its first connection, which autocomplete may prewarm, `ServerManager` spawns `bin/cypher serve --port 0`, captures the dynamically assigned port from stdout, and communicates over HTTP + SSE. The current child process is reused unless it exits. A random password is generated and passed via `CYPHER_SERVER_PASSWORD` env var for basic auth.

```
Extension (Node.js)                          CLI Backend (child process)
┌──────────────────────────┐                ┌──────────────────────┐
│ CypherConnectionService    │── HTTP/SSE ──> │ cypher serve --port 0  │
│   ├── ServerManager      │                │   Hono REST API      │
│   ├── HttpClient         │                │   SSE event stream   │
│   └── SSEClient          │                │   Session management │
│                          │                │   AI agent runtime   │
│ CypherProvider (sidebar)   │                └──────────────────────┘
│ CypherProvider (agent mgr) │
│ CypherProvider (open tabs) │
└──────────────────────────┘
```

- **`CypherConnectionService`** (`src/services/cli-backend/connection-service.ts`) is created once during extension activation and shared across the sidebar, Cypher editor tabs, and Agent Manager. It owns the current server process, HTTP client, and SSE connection.
...
- The sidebar, every **Open in Tab** Cypher panel, and the Agent Manager chat provider reuse this connection. Multiple **`CypherProvider`** instances subscribe to it, with SSE events filtered per-webview via a `trackedSessionIds` Set. Agent Manager terminals may use additional PTY/WebSocket channels to the same backend, not separate `cypher serve` processes.
...
- CSP requires nonce for scripts and `font-src` for bundled fonts — see [`CypherProvider.ts`](src/CypherProvider.ts:777)
- HTML root has `data-theme="cypher-vscode"` to activate cypher-ui's VS Code theme bridge
...
3. **CypherProvider** (`src/CypherProvider.ts`): Add a `fetchAndSend*()` method using the cached message pattern, and handle the corresponding `request*` message from the webview in `handleWebviewMessage()`
...
- **Cached messages** (e.g. `cachedProvidersMessage`, `cachedAgentsMessage` in CypherProvider): Ensures webview refreshes get data immediately without waiting for a new HTTP round-trip
...
| State | No dedicated state file | `.cypher/agent-manager.json` |
| Terminals | None | Dedicated VS Code terminal per session |
| Setup scripts | None | Configurable `.cypher/setup-script` runs per worktree |
| Multi-version | Not supported | Up to 4 parallel worktrees with the same prompt |

### Architecture

Agent Manager local worktree sessions use the current shared `cypher serve` process owned by `CypherConnectionService`; no session starts its own backend. Their CLI requests pass the worktree path as `directory`, which resolves directory-scoped backend state. Setup scripts, terminal PTYs, git subprocesses, and a separately opened VS Code window are separate process or extension-host boundaries, not per-worktree `cypher serve` instances.
...
## Webview UI (cypher-ui)

New webview features must use **`@cypher/cypher-ui`** components instead of raw HTML elements with inline styles. This is a Solid.js component library built on `@kobalte/core`.

- Import via deep subpaths: `import { Button } from "@cypher/cypher-ui/button"`
...
- Global styles imported via `import "@cypher/cypher-ui/styles"` in [`index.tsx`](webview-ui/src/index.tsx:2)
- [`chat.css`](webview-ui/src/styles/chat.css) is being progressively migrated — when replacing a component with cypher-ui, remove the corresponding CSS rules from it
- New CSS for components not yet in cypher-ui goes into `chat.css` grouped by comment-delimited sections (`/* Component Name */`). Once a cypher-ui equivalent exists, remove the section.
- **Check existing webview usages first**: `webview-ui/src/` and `packages/cypher-ui/src/stories/` show how cypher-ui components are composed. Do not rely only on the component API in isolation.
- **`data-component` and `data-slot` attributes carry CSS styling** — cypher-ui uses `[data-component]` and `[data-slot]` attribute selectors, not class names. Reuse existing component slots where available so shared styles apply consistently.
- **Prefer cypher-ui styles**: Always reuse existing cypher-ui CSS variables, tokens, and component styles instead of writing custom CSS. If a style doesn't exist in cypher-ui yet, add it there and reuse it rather than inlining or duplicating styles in the webview.
- **Icons**: cypher-ui has 75+ custom SVG icons in [`packages/ui/src/components/icon.tsx`](../../packages/ui/src/components/icon.tsx). To list all available icon names: `node -e "const c=require('fs').readFileSync('../../packages/ui/src/components/icon.tsx','utf8');[...c.matchAll(/^\\s{2}[\"']?([\\w-]+)[\"']?:\\s*\x60/gm)].map(m=>m[1]).sort().forEach(n=>console.log(n))"`. Icon names use both hyphenated (`arrow-left`) and bare-word (`brain`, `console`, `providers`) keys.
...
Generated screenshot baselines live under `packages/cypher-docs/public/img/screenshot-tests/` and are referenced from docs as `/docs/img/screenshot-tests/...`. If a generated VS Code visual-regression screenshot is used in docs, add the docs usage to the `DOCS` map in `tests/visual-regression.spec.ts` and keep `tests/visual-regression.spec.mts` in sync while that file exists.
...
- All debug output must be prepended with `[Cypher New]` for easy filtering

## Naming Conventions

- All VSCode commands must use `cypher-cli.new.` prefix (not `cypher-cli.`)
- All view IDs must use `cypher-cli.new.` prefix, **except** the sidebar view which uses `cypher-cli.SidebarProvider` to preserve user sidebar position when upgrading from the legacy extension

## Cyphercode Change Markers

This package is entirely Cypher-specific — `cypher_change` markers are NOT needed in any files under `packages/cypher-vscode/`. The markers are only necessary when modifying shared upstream opencode files.
...
The `spawn` wrapper covers long-lived processes (e.g. `cypher serve`). The `exec` wrapper covers short commands (e.g. `git`, `tar`). If you need the raw callback form of `execFile` for some reason, pass `windowsHide: true` explicitly in the options object.
