...
1. **`bun run watch`** is already running for the cypher extension.
2. In the cypher VSCode window, press **F5** → opens the **Extension Development Host**.
3. In the Dev Host: `File → Open Folder…` → `packages/cypher-vscode/docs/nes-examples/` inside this repo.
4. Open Settings (`Cmd+,`), confirm:
   - `cypher-cli.new.autocomplete.enableAutoTrigger` → ✓ (default true)
   - `cypher-cli.new.autocomplete.model` → **Mercury Next Edit (Inception)** ← *NOT* "Mercury Edit 2", which is the classic FIM option
   - `cypher-cli.new.autocomplete.nextEdit.apiKey` → your `sk_...` key
   - VSCode global `editor.inlineSuggest.enabled` → ✓
5. To watch the pipeline live: `View → Output` → pick the **"Cypher CLI · Next Edit"** channel.
...
- **No log lines appearing**: confirm the output channel is "Cypher CLI · Next Edit". Also confirm you reloaded the Dev Host after rebuilding.
