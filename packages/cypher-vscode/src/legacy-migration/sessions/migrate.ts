...
export async function migrate(
  ...
  try {
    ...
    const project = await client.cypherCode.sessionImport.project(payload.project, { throwOnError: true })
    const projectID = project.data?.id ?? payload.project.id
    const session = await client.cypherCode.sessionImport.session(
      {
        ...payload.session,
        projectID,
    ...
    for (const msg of payload.messages) {
      await client.cypherCode.sessionImport.message(msg, { throwOnError: true })
    }

    for (const part of payload.parts) {
      await client.cypherCode.sessionImport.part(part, { throwOnError: true })
    }
