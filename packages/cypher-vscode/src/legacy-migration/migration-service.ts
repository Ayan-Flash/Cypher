 ...
 *
 * Reads legacy Cypher CLI v5.x data from VS Code SecretStorage and the extension's
...
const MIGRATION_STATUS_KEY = "cypher.legacyMigrationStatus"
...
export async function clearLegacyData(context: vscode.ExtensionContext): Promise<void> {
  ...
  const legacyStateKeys = [
    "cypher-cli.allowedCommands",
    "cypher-cli.deniedCommands",
    "cypher-cli.autoApprovalEnabled",
    "cypher-cli.fuzzyMatchThreshold",
    "cypher-cli.diffEnabled",
    "cypher-cli.language",
    "cypher-cli.customModes",
    "cypher-cli.firstInstallCompleted",
    "cypher-cli.telemetrySetting",
  ...
  // Clear legacy VS Code settings registered under the "cypher-cli" configuration scope.
  ...
  const cfg = vscode.workspace.getConfiguration("cypher-cli")
...
async function migrateProvider(
  ...
  // For providers that support an organization ID (e.g. Cypher Gateway), migrate using OAuth
...
async function migrateDefaultModel(settings: LegacyProviderSettings, client: CypherClient): Promise<MigrationResultItem> {
  const provider = settings.apiProvider
  if (!provider) {
...
async function migrateAutocomplete(settings: LegacyAutocompleteSettings): Promise<MigrationResultItem> {
  try {
    const config = vscode.workspace.getConfiguration("cypher-cli.new.autocomplete")
...
function readLegacySettings(context: vscode.ExtensionContext): LegacySettings {
  ...
  return {
    autoApprovalEnabled: context.globalState.get<boolean>("cypher-cli.autoApprovalEnabled"),
    allowedCommands: context.globalState.get<string[]>("cypher-cli.allowedCommands"),
    deniedCommands: context.globalState.get<string[]>("cypher-cli.deniedCommands"),
    ...
    language: context.globalState.get<string>("cypher-cli.language"),
