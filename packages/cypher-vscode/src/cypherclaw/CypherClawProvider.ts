/**
 * CypherClaw panel provider for the VS Code extension.
 *
 * Owns the Cypher Chat HTTP client + event-service WebSocket connection
...
import { CypherChatApiError, CypherChatClient } from "./cypher-chat-client"
...
export class CypherClawProvider implements vscode.Disposable {
  static readonly viewType = "cypher-cli.new.CypherClawPanel"
  ...
  private chat: CypherChatClient | null = null
  ...
  constructor(
    ...
    private readonly connection: CypherConnectionService,
  ) {}
  ...
  openPanel(): void {
    ...
    const panel = vscode.window.createWebviewPanel(CypherClawProvider.viewType, "CypherClaw", vscode.ViewColumn.One, {
      enableScripts: true,
  ...
  private attach(panel: vscode.WebviewPanel): void {
    ...
    panel.iconPath = {
      light: vscode.Uri.joinPath(this.uri, "assets", "icons", "cypher-light.svg"),
      dark: vscode.Uri.joinPath(this.uri, "assets", "icons", "cypher-dark.svg"),
    }
    ...
    panel.webview.html = buildWebviewHtml(panel.webview, {
      scriptUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "cypherclaw.js")),
      styleUri: panel.webview.asWebviewUri(vscode.Uri.joinPath(this.uri, "dist", "cypherclaw.css")),
    ...
    const msgSub = panel.webview.onDidReceiveMessage((msg: CypherClawInMessage) => this.onMessage(msg))
    ...
    const unsub = this.connection.onLanguageChanged((locale) => {
      this.post({ type: "cypherclaw.locale", locale })
    })
  ...
  private post(msg: CypherClawOutMessage): void {
    this.panel?.webview.postMessage(msg)
  }

  private async onMessage(msg: CypherClawInMessage): Promise<void> {
    switch (msg.type) {
      case "cypherclaw.ready":
        await this.init()
        return
      case "cypherclaw.openExternal": {
  ...
  private get locale(): string {
    const override = vscode.workspace.getConfiguration("cypher-cli.new").get<string>("language")
  ...
  private async init(): Promise<void> {
    ...
    try {
      this.post({ type: "cypherclaw.state", state: { phase: "loading", locale: this.locale } })
      ...
      this.post({ type: "cypherclaw.state", state })
      this.startPolling()
      this.startBotNudge()
    } finally {
   ...
   */
  private async bootstrap(client: CypherClient, gen: number): Promise<boolean> {
    ...
    if (!this.sandboxId) {
      this.post({ type: "cypherclaw.state", state: { phase: "noInstance", locale: this.locale } })
      return false
    }
  ...
  private async resolveStatus(client: CypherClient, gen: number): Promise<boolean> {
    const statusRes = await client.cypher.claw.status().catch(() => null)
    if (this.stale(gen)) return false
  ...
  private async fetchChatToken(gen: number): Promise<ChatToken | null> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherClaw chat token fetch failed:", message)
      // Token fetch typically fails when the instance hasn't been upgraded
      // to support cypher-chat — surface that as the upgrade prompt.
      this.post({ type: "cypherclaw.state", state: { phase: "needsUpgrade", locale: this.locale } })
      return null
    }
  }
  ...
  private async openChatStream(envelope: ChatToken, gen: number): Promise<boolean> {
    ...
    const events = new EventServiceClient({
      ...
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "cypherclaw.error", error: "Authentication expired" })
      },
    })
    this.events = events

    const chat = new CypherChatClient({
      baseUrl: envelope.cypherChatUrl,
      getToken: () => tokens.get(),
      onUnauthorized: () => {
        tokens.clear()
        this.post({ type: "cypherclaw.error", error: "Authentication expired" })
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.error("[Cypher New] CypherClaw event-service connect failed:", message)
      this.post({
        type: "cypherclaw.state",
  ...
  private async loadInitialSnapshots(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Cypher New] CypherClaw listConversations failed:", message)
    }
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Cypher New] CypherClaw getBotStatus failed:", message)
    }
    ...
    if (!this.activeConversationId && this.conversations.length > 0) {
      ...
      try {
        ...
        const message = err instanceof Error ? err.message : String(err)
        console.warn("[Cypher New] CypherClaw getConversationStatus failed:", message)
      }
  ...
  private async resolveClient(): Promise<CypherClient | null> {
    if (this.connection.getConnectionState() !== "connected") {
      try {
        ...
        console.debug("[Cypher New] CypherClaw connect deferred:", (err as Error)?.message ?? err)
        return null
      }
    }
    try {
      ...
      console.debug("[Cypher New] CypherClaw getClient deferred:", (err as Error)?.message ?? err)
      return null
    }
  }
  ...
  private subscribeSandboxContext(): void {
    ...
    const ctx = `/cypherclaw/${this.sandboxId}`
  ...
  private subscribeConversationContext(conversationId: string): void {
    ...
    const ctx = `/cypherclaw/${this.sandboxId}/${conversationId}`
    this.events.subscribe([ctx])
  ...
  private attachEventHandlers(events: EventServiceClient): void {
    ...
    this.chatSubs.push(
      events.on("conversation.created", (ctx, e: ConversationCreatedEvent) => {
        if (!this.sandboxId || ctx !== `/cypherclaw/${this.sandboxId}`) return
    ...
    this.chatSubs.push(
      events.on("conversation.left", (ctx, e: ConversationLeftEvent) => {
        ...
        if (this.activeConversationId === e.conversationId) {
          ...
          this.post({ type: "cypherclaw.activeConversation", conversationId: null })
          this.post({
            type: "cypherclaw.messages",
            conversationId: e.conversationId,
            messages: [],
            hasMore: false,
            replace: true,
          })
          this.post({ type: "cypherclaw.conversationStatus", status: null })
        }
        this.broadcastConversations({ replace: true })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("bot.status", (ctx, e: BotStatusEvent) => {
        ...
        this.post({ type: "cypherclaw.botStatus", status: this.botStatus })
      }),
    )
      ...
      events.on("message.created", (ctx, e: MessageCreatedEvent) => {
        ...
        if (e.clientId) {
          ...
          if (idx !== -1) {
            ...
            this.post({
              type: "cypherclaw.messageReplaced",
    ...
    this.chatSubs.push(
      events.on("action.delivery_failed", (ctx, e: ActionDeliveryFailedEvent) => {
        ...
        this.broadcastMessages({ replace: true })
        this.post({ type: "cypherclaw.error", error: "Couldn't reach the bot — please try again" })
      }),
    )
    ...
    this.chatSubs.push(
      events.on("conversation.status", (ctx, e: ConversationStatusEvent) => {
        ...
        this.post({ type: "cypherclaw.conversationStatus", status: this.conversationStatus })
      }),
    )
  }
  ...
  private async selectConversation(conversationId: string): Promise<void> {
    ...
    this.post({ type: "cypherclaw.activeConversation", conversationId })
    this.typingMembers = []
  ...
  private async createConversation(title?: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to create conversation") })
    }
  }
  ...
  private async renameConversation(conversationId: string, title: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to rename conversation") })
      void this.refreshConversations()
    }
  }
  ...
  private async leaveConversation(conversationId: string): Promise<void> {
    ...
    try {
      ...
      if (this.activeConversationId === conversationId) {
        ...
        this.messages = []
        this.post({ type: "cypherclaw.activeConversation", conversationId: null })
        this.post({ type: "cypherclaw.messages", conversationId, messages: [], hasMore: false, replace: true })
      }
      this.broadcastConversations({ replace: true })
    } catch (err) {
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to leave conversation") })
    }
  }
  ...
  private async loadMoreConversations(): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to load conversations") })
    }
  }
  ...
  private async sendMessage(
    ...
    // cypher-chat validates clientId as a ULID (Crockford Base32); generate
    ...
    if (conversationId === this.activeConversationId) {
      ...
      this.post({ type: "cypherclaw.messageOptimistic", conversationId, message: optimistic })
    }
    ...
    try {
      ...
      console.error("[Cypher New] CypherClaw sendMessage failed:", err instanceof Error ? err.message : err)
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to send message") })
      if (conversationId === this.activeConversationId) {
        this.messages = this.messages.filter((m) => m.id !== pendingId)
        this.post({ type: "cypherclaw.messageRemoved", conversationId, messageId: pendingId })
      }
    }
  }
  ...
  private async editMessage(conversationId: string, messageId: string, content: ContentBlock[]): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to edit message") })
      if (snapshot && conversationId === this.activeConversationId) {
  ...
  private async deleteMessage(conversationId: string, messageId: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to delete message") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async loadMoreMessages(conversationId: string, before: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to load messages") })
    }
  }
  ...
  private async addReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to add reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async removeReaction(conversationId: string, messageId: string, emoji: string): Promise<void> {
    ...
    try {
      ...
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to remove reaction") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async executeAction(
    ...
    try {
      ...
      this.post({ type: "cypherclaw.error", error: this.formatError(err, "Failed to execute action") })
      if (snapshot && conversationId === this.activeConversationId) {
        this.messages = this.messages.map((m) => (m.id === messageId ? snapshot : m))
        this.broadcastMessages({ replace: true })
      }
  ...
  private async refreshConversations(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Cypher New] CypherClaw refreshConversations failed:", message)
    }
  }
  ...
  private async refreshActiveMessages(): Promise<void> {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : String(err)
      console.warn("[Cypher New] CypherClaw refreshActiveMessages failed:", message)
    }
  }
  ...
  private upsertTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "cypherclaw.typing", conversationId: this.activeConversationId, memberId })
    }
  ...
  private removeTypingMember(memberId: string): void {
    ...
    if (this.activeConversationId) {
      this.post({ type: "cypherclaw.typingStop", conversationId: this.activeConversationId, memberId })
    }
  }
  ...
  private formatError(err: unknown, fallback: string): string {
    if (err instanceof CypherChatApiError) {
  ...
  private startBotNudge(): void {
    ...
    this.botNudge = setInterval(() => {
      ...
      this.chat.requestBotStatus(this.sandboxId).catch((err) => {
        console.debug("[Cypher New] CypherClaw requestBotStatus failed:", (err as Error)?.message ?? err)
      })
    }, BOT_STATUS_NUDGE_MS)
  }
  ...
  private async poll(): Promise<void> {
    try {
      ...
      const res = await client.cypher.claw.status()
      if (res?.data) {
        this.status = res.data as ClawStatus
        this.post({ type: "cypherclaw.status", data: this.status })
      }
    } catch (err) {
      console.debug("[Cypher New] CypherClaw poll failed:", (err as Error)?.message ?? err)
    }
  }
