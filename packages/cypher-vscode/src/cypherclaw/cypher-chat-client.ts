/**
 * HTTP client for the cypher-chat Cloudflare Worker.
 *
 * Minimal inline port of `@cypher/cypher-chat/client` (cloud monorepo) tailored
 ...
 * the cypher-chat worker is the source of truth and validates at its edge.
 */
...
export type CypherChatClientConfig = {
  baseUrl: string
  getToken: () => Promise<string>
  onUnauthorized?: () => void
}

export class CypherChatApiError extends Error {
  constructor(
    public readonly status: number,
    public readonly body: unknown,
  ) {
    super(`CypherChat request failed: ${status}${formatBodyDetail(body)}`)
    this.name = "CypherChatApiError"
  }
}
...
export class CypherChatClient {
  private readonly baseUrl: string
  private readonly getToken: () => Promise<string>
  private readonly onUnauthorized: (() => void) | undefined
  private readonly sendQueues: SendQueue = new Map()

  constructor(config: CypherChatClientConfig) {
  ...
  private async request<T>(path: string, opts: HttpOpts = {}): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new CypherChatApiError(res.status, body)
    }
