 ...
 *
 * Minimal inline port of `@cypher/event-service` (cloud monorepo). Connects
 * to the cypher events Cloudflare Worker using a two-step ticket flow:
...
import type { CypherChatEventMap, CypherChatEventName } from "./types"

const WS_SUBPROTOCOL = "cypher.events.v1"
...
export class EventServiceClient {
  ...
  on<N extends CypherChatEventName>(event: N, handler: (ctx: string, payload: CypherChatEventMap[N]) => void): () => void {
    ...
    // cypher-chat worker edge before broadcast.
    const wrapped: EventHandler = (ctx, payload) => handler(ctx, payload as CypherChatEventMap[N])
    set.add(wrapped)
  ...
  private handleMessage(data: string): void {
    ...
    if (m.type === "error") {
      console.warn("[Cypher New] event-service server error", m)
    }
  }
