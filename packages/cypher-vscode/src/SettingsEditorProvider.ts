...
const PANEL_TITLES: Record<PanelView, string> = {
  settings: "Cypher Settings",
  profile: "Cypher Profile",
 ...
 *
 * Uses a full CypherProvider under the hood so each panel has
...
export class SettingsEditorProvider implements vscode.Disposable {
  ...
  private providers = new Map<PanelView, CypherProvider>()
  ...
  /** Extract the PanelView from a viewType string like "cypher-cli.new.settingsPanel". */
  static viewFromType(type: string): PanelView | undefined {
    const match = type.match(/^cypher-cli\.new\.(\w+)Panel$/)
  ...
  private wirePanel(panel: vscode.WebviewPanel, view: PanelView, projectDirectory: string | null): void {
    ...
    // Create a dedicated CypherProvider for this panel so it has full
    ...
    const readyDisposable = panel.webview.onDidReceiveMessage((msg) => {
      if (msg.type === "webviewReady") {
        // Small delay to let CypherProvider's own webviewReady handler finish first
        setTimeout(() => {
    ...
    panel.onDidDispose(() => {
      console.log(`[Cypher New] ${title} panel disposed`)
