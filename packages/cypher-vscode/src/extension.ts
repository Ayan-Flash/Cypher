...
import { CypherClawProvider } from "./cypherclaw/CypherClawProvider"
...
import { CypherConnectionService } from "./services/cli-backend"
...
import { registerCodeActions, registerTerminalActions, CypherActionProvider } from "./services/code-actions"
...
const RESTORE_KEY = "cypher.workbench.restore"
...
// without requiring the user to open a Cypher sidebar or panel first. The CLI backend is NOT spawned here;
// it starts lazily when a webview connects or when ensureBackendForAutocomplete() triggers it.
export function activate(context: vscode.ExtensionContext) {
  console.log("Cypher CLI extension is now active")
  shuttingDown = false

  const telemetry = TelemetryProxy.getInstance()

  // Create shared connection service (one server for all webviews)
  const connectionService = new CypherConnectionService(context)
  ...
  const unsubscribeStateChange = connectionService.onStateChange((state) => {
    if (state === "connected") {
      ...
      if (config) {
        ...
        // CLI reads CYPHER_TELEMETRY_LEVEL once at spawn, so without this call
      ...
      try {
        ...
        console.log("[Cypher New] CLI connected, calling remoteService.refresh()")
        remoteService.refresh().catch((err) => console.warn("[Cypher New] initial remote refresh failed:", err))
      } catch {
  ...
  for (const folder of vscode.workspace.workspaceFolders ?? []) {
    void markWorkspace(folder.uri.fsPath, (msg) => console.warn(`[Cypher New] ${msg}`))
  }
  ...
  const tabPanels = new Map<vscode.WebviewPanel, CypherProvider>()
  ...
  const provider = new CypherProvider(context.extensionUri, connectionService, context)
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(CypherProvider.viewType, provider, {
  ...
  // commands by default.
  const skip = ["cypher-cli.new.agentManagerOpen", "cypher-cli.new.agentManager.showTerminal"]
  if (process.platform === "darwin") skip.push("cypher-cli.new.agentManager.runScript")
  ensureCommandsSkipShell(skip)

  // Create CypherClaw chat provider for editor panel
  const cypherClawProvider = new CypherClawProvider(context.extensionUri, connectionService)
  context.subscriptions.push(cypherClawProvider)
  ...
  // Register serializer so CypherClaw panel restores when VS Code restarts
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer(CypherClawProvider.viewType, {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
        cypherClawProvider.restorePanel(panel)
        return Promise.resolve()
      },
    }),
  )
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("cypher-cli.new.TabPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
        const tabProvider = new CypherProvider(context.extensionUri, connectionService, context, {
        ...
        panel.onDidDispose(
          () => {
            console.log("[Cypher New] Tab panel restored from restart disposed")
  ...
  for (const suffix of settingsViews) {
    context.subscriptions.push(
      vscode.window.registerWebviewPanelSerializer(`cypher-cli.new.${suffix}`, {
        deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewPanelSerializer("cypher-cli.new.SubAgentViewerPanel", {
      deserializeWebviewPanel(panel: vscode.WebviewPanel) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("cypher-cli.new.sidebarTitle.plusButtonClicked", () => {
      track("new_task", "cypher-cli.new.plusButtonClicked")
    }),
    vscode.commands.registerCommand("cypher-cli.new.sidebarTitle.historyButtonClicked", () => {
      track("history", "cypher-cli.new.historyButtonClicked")
    }),
    vscode.commands.registerCommand("cypher-cli.new.sidebarTitle.agentManagerOpen", () => {
      track("agent_manager", "cypher-cli.new.agentManagerOpen")
    }),
    vscode.commands.registerCommand("cypher-cli.new.sidebarTitle.cypherClawOpen", () => {
      track("cypherclaw", "cypher-cli.new.cypherClawOpen")
    }),
    vscode.commands.registerCommand("cypher-cli.new.sidebarTitle.marketplaceButtonClicked", () => {
      track("marketplace", "cypher-cli.new.marketplaceButtonClicked")
    }),
    vscode.commands.registerCommand("cypher-cli.new.sidebarTitle.profileButtonClicked", () => {
      track("profile", "cypher-cli.new.profileButtonClicked")
    }),
    vscode.commands.registerCommand("cypher-cli.new.sidebarTitle.settingsButtonClicked", () => {
      track("settings", "cypher-cli.new.settingsButtonClicked")
    }),
    vscode.commands.registerCommand("cypher-cli.new.plusButtonClicked", () => {
    ...
    vscode.commands.registerCommand("cypher-cli.new.agentManagerOpen", () => {
      agentManagerProvider.openPanel()
    }),
    vscode.commands.registerCommand("cypher-cli.new.marketplaceButtonClicked", (directory?: string | null) => {
      marketplacePanelProvider.openPanel(directory)
    }),
    vscode.commands.registerCommand("cypher-cli.new.cypherClawOpen", () => {
      cypherClawProvider.openPanel()
    }),
    vscode.commands.registerCommand("cypher-cli.new.historyButtonClicked", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("cypher-cli.new.cycleAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("cypher-cli.new.cyclePreviousAgentMode", () => {
      const tab = activeTabProvider()
    ...
    vscode.commands.registerCommand("cypher-cli.new.profileButtonClicked", () => {
      settingsEditorProvider.openPanel("profile")
    }),
    vscode.commands.registerCommand("cypher-cli.new.settingsButtonClicked", (tab?: string) => {
      settingsEditorProvider.openPanel("settings", tab)
    }),
    vscode.commands.registerCommand("cypher-cli.new.openIndexingSettings", () => {
      settingsEditorProvider.openPanel("settings", "indexing")
    }),
    vscode.commands.registerCommand("cypher-cli.new.showMemory", async () => {
      if (agentManagerProvider.isActive()) {
        await agentManagerProvider.showMemory()
        return
      }
      const target = activeTabProvider() ?? provider
      if (target === provider) await vscode.commands.executeCommand("cypher-cli.SidebarProvider.focus")
      await target.waitForReady()
      await target.showMemory()
    }),
    vscode.commands.registerCommand("cypher-cli.new.toggleMemory", async () => {
      if (agentManagerProvider.isActive()) {
    ...
    vscode.commands.registerCommand("cypher-cli.new.openMigrationWizard", () => {
      provider.postMessage({ type: "migrationState", needed: true, source: "legacy" })
    }),
    // legacy-migration end
    vscode.commands.registerCommand("cypher-cli.new.generateTerminalCommand", async () => {
      ...
      if (!input) return
      await vscode.commands.executeCommand("cypher-cli.SidebarProvider.focus")
      await provider.waitForReady()
      provider.postMessage({ type: "triggerTask", text: `Generate a terminal command: ${input}` })
    }),
    vscode.commands.registerCommand("cypher-cli.new.toggleRemote", () => {
      remoteService.toggle().catch((err) => console.error("[Cypher New] toggleRemote command failed:", err))
    }),
    vscode.commands.registerCommand("cypher-cli.new.openInTab", () => {
      return openCypherInNewTab(
        context,
        connectionService,
    ...
    vscode.commands.registerCommand(
      "cypher-cli.new.showChanges",
      (arg?: { sessionId?: string; turnId?: string; initialSourceId?: string }) => {
        diffViewerProvider.openFromCommand(arg)
      },
    ),
    vscode.commands.registerCommand("cypher-cli.new.openSubAgentViewer", (sessionID: string, title?: string) => {
      subAgentViewerProvider.openPanel(sessionID, title)
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.previousSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionPrevious" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.nextSession", () => {
      agentManagerProvider.postMessage({ type: "action", action: "sessionNext" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.previousTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabPrevious" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.nextTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "tabNext" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.search", () => {
      agentManagerProvider.postMessage({ type: "action", action: "search" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.showTerminal", () => {
    ...
    vscode.commands.registerCommand("cypher-cli.new.agentManager.runScript", () => {
      agentManagerProvider.postMessage({ type: "action", action: "runScript" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.toggleDiff", () => {
      agentManagerProvider.postMessage({ type: "action", action: "toggleDiff" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.showShortcuts", () => {
      agentManagerProvider.postMessage({ type: "action", action: "showShortcuts" })
    }),

    vscode.commands.registerCommand("cypher-cli.new.agentManager.newTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTab" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.newTerminal", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newTerminal" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.closeTab", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeTab" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.newWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "newWorktree" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.openWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "openWorktree" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.openPR", () => {
      agentManagerProvider.postMessage({ type: "action", action: "openPR" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.closeWorktree", () => {
      agentManagerProvider.postMessage({ type: "action", action: "closeWorktree" })
    }),
    vscode.commands.registerCommand("cypher-cli.new.agentManager.advancedWorktree", () =>
      agentManagerProvider.openAdvancedWorktree(),
    ),
    ...Array.from({ length: 9 }, (_, i) =>
      vscode.commands.registerCommand(`cypher-cli.new.agentManager.jumpTo${i + 1}`, () => {
        agentManagerProvider.postMessage({ type: "action", action: `jumpTo${i + 1}` })
      }),
    ),
  )

  // Register URI handler for extension deep links (vscode://cypher.cypher-cli/cypher/...)
  context.subscriptions.push(
    vscode.window.registerUriHandler({
      async handleUri(uri: vscode.Uri) {
        const sessionMatch = uri.path.match(/^\/cypher\/s\/([a-zA-Z0-9_-]+)$/)
        const sessionId = sessionMatch?.[1]
        if (sessionId) {
          console.log("[Cypher New] URI handler: opening cloud session:", sessionId)
          await vscode.commands.executeCommand(`${CypherProvider.viewType}.focus`)
          provider.openCloudSession(sessionId)
          return
        }

        if (uri.path !== "/cypher/switch" && uri.path !== "/cypher/model") return
        const params = new URLSearchParams(uri.query)
        const modelID = params.get("model") || undefined
        const agent = params.get("agent") || undefined
        if (!modelID && !agent) return
        console.log("[Cypher New] URI handler: applying linked Cypher selection:", { modelID, agent })
        await vscode.commands.executeCommand(`${CypherProvider.viewType}.focus`)
        provider.selectCypherModel(modelID, agent)
      },
    }),
  )
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("cypher-cli.new.reload", () => {
      provider.reload().catch((e) => console.error("[Cypher New] reload command failed:", e))
    }),
  )
  ...
  context.subscriptions.push(
    vscode.languages.registerCodeActionsProvider(
      { scheme: "file" },
      new CypherActionProvider(),
      CypherActionProvider.metadata,
    ),
  )
...
async function openCypherInNewTab(
  context: vscode.ExtensionContext,
  connectionService: CypherConnectionService,
  agentManagerProvider: AgentManagerProvider,
  tabPanels: Map<vscode.WebviewPanel, CypherProvider>,
  ...
  const panel = vscode.window.createWebviewPanel("cypher-cli.new.TabPanel", EXTENSION_DISPLAY_NAME, targetCol, {
    enableScripts: true,
  ...
  panel.iconPath = {
    light: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "cypher-light.svg"),
    dark: vscode.Uri.joinPath(context.extensionUri, "assets", "icons", "cypher-dark.svg"),
  }
  ...
  panel.onDidDispose(
    () => {
      console.log("[Cypher New] Tab panel disposed")
      tabPanels.delete(panel)
      tabProvider.dispose()
    },
    null,
