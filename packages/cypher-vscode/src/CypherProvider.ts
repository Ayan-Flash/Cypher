...
import type {
  ...
  FilePartInput,
  Config,
} from "@cypher/sdk/v2/client"
import { MaxCostNudge, type MaxCostChoice } from "@opencode-ai/core/cypher/cost/max-cost-nudge"
import { type CypherConnectionService, ServerStartupError } from "./services/cli-backend"
...
import { saveImage } from "./cypher-provider/save-image"
import { handleEditorAction } from "./cypher-provider/editor-actions"
import { exportTranscript } from "./cypher-provider/export-transcript"
import {
  TelemetryProxy,
...
import { createMarketplaceRemover, removeMcp } from "./cypher-provider/remove-config-item"
import { AgentRequirementsController } from "./cypher-provider/agent-requirements-controller"
...
import { slimInfo, slimPart, slimParts } from "./cypher-provider/slim-metadata"
import { handleSidebarWorktreeMessage } from "./cypher-provider/sidebar-worktree"
import { parseMessageFiles, type MessageFile } from "./cypher-provider/message-files"
import { renameSession } from "./cypher-provider/rename-session"
import { handleFileSearch } from "./cypher-provider/file-search"
import { watchFontSizeConfig } from "./cypher-provider/font-size"
import { getTerminalContents } from "./services/terminal/context"
import { disposeGitChangesTarget } from "./cypher-provider/git-changes-target"
import { interceptMessage } from "./cypher-provider/git-changes-request"
import { matchFollowup, recordFollowup, type Followup } from "./cypher-provider/followup-session"
import { clearCommandsCache, loadCommands } from "./cypher-provider/commands"
import { fetchMessagePage, MESSAGE_PAGE_LIMIT } from "./cypher-provider/message-page"
import {
  dismissNotification,
...
} from "./cypher-provider/notifications"
import { childID } from "./cypher-provider/task-session"
import { VisibleTaskStreams } from "./cypher-provider/visible-task-streams"
import { handleNetworkEvent, clearNetworkWaits } from "./cypher-provider/network"
import { SessionAbort } from "./cypher-provider/abort"
import {
  buildAutocompleteSettingsMessage,
  validAutocompleteSetting,
  watchAutocompleteConfig,
} from "./services/autocomplete/settings"
import { routeEarlyMessage } from "./cypher-provider/early-message"
import * as ModelState from "./cypher-provider/model-state"
import { handleForkSession } from "./cypher-provider/fork-session"
import { openConfig } from "./cypher-provider/open-config"
import {
  getWorkStylePayload,
  handleWorkStyleMessage,
  isWorkStyleSetting,
  watchWorkStyleConfig,
} from "./cypher-provider/work-style"
import * as McpOAuth from "./cypher-provider/mcp-oauth"
import { retryable, backoff, MAX_RETRIES } from "./util/retry"
import { hasGit } from "./cypher-provider/git-status"
...
} from "./cypher-provider/handlers/migration"
...
import {
  ...
  type AuthContext,
} from "./cypher-provider/handlers/auth"
import {
  handleRequestCloudSessions,
  handleRequestCloudSessionData,
  handleImportAndSend,
  type CloudSessionContext,
} from "./cypher-provider/handlers/cloud-session"
import {
  handlePermissionResponse,
  fetchAndSendPendingPermissions,
  type PermissionContext,
} from "./cypher-provider/handlers/permission-handler"
import {
  handleQuestionReply,
  handleQuestionReject,
  fetchAndSendPendingQuestions,
} from "./cypher-provider/handlers/question"
import { fetchAndSendPendingSuggestions } from "./cypher-provider/handlers/suggestion"
import { nativeTitle } from "./cypher-provider/native-tab-title"
import { parseReview, reviewMetadata, type ReviewMessageData } from "./shared/review-comments"
import { CypherProviderMemory } from "./cypher-provider/memory"
...
import type { Agent } from "@cypher/sdk/v2/client"
import { configFeatures } from "./features"
import { createAutoApproveBridge } from "./cypher-provider/auto-approve"
import type { CypherProviderOptions } from "./cypher-provider/options"
import { fetchCypherEmbeddingModelCatalog } from "@cypher/cypher-gateway"
import { fetchImageModels } from "./image-generation/models"
import { stopSessionProcesses } from "./cypher-provider/background-process"
import { sandboxDefault, sandboxSessionMetadata } from "./shared/sandbox-session"
import {
  buildIndexingSettingsMessage,
  validIndexingSetting,
  watchIndexingConfig,
} from "./cypher-provider/indexing-settings"
...
function sandboxClient(client: CypherClient | null) {
...
export class CypherProvider implements vscode.WebviewViewProvider, TelemetryPropertiesProvider {
  public static readonly viewType = "cypher-cli.SidebarProvider"
  ...
  private readonly extensionVersion =
    vscode.extensions.getExtension("cypher.cypher-cli")?.packageJSON?.version ?? "unknown"
  ...
  /** Cached cypherEmbeddingModelsLoaded payload so requestCypherEmbeddingModels is resilient offline. */
  private cachedCypherEmbeddingModelsMessage: unknown = null
  ...
  private pendingCypherModel: { modelID?: string; agent?: string } | null = null
  ...
  private readonly memory = new CypherProviderMemory({
  ...
  constructor(
    ...
    private readonly opts: CypherProviderOptions = {},
  ) {
  ...
  private checkpoint(sid: string, run: () => Promise<void>): void {
    ...
    void pending.then(cleanup, (error) => {
      console.error("[Cypher New] checkpoint mutation failed:", error)
      cleanup()
    })
  }
  ...
  /**
   * Convenience getter that returns the shared SDK CypherClient or null if not yet connected.
   * Preserves the existing null-check pattern used throughout handler methods.
   */
  private get client(): CypherClient | null {
    try {
  ...
  private openMarketplacePanel(directory: unknown): void {
    if (typeof directory === "string" && directory) {
      vscode.commands.executeCommand("cypher-cli.new.marketplaceButtonClicked", directory)
      return
    }
    vscode.commands.executeCommand("cypher-cli.new.marketplaceButtonClicked", this.projectDirectory)
  }

  // Strip metadata unused by the webview to keep session switches fast.
  // Logic in cypher-provider/slim-metadata.ts.
  ...
  private async syncWebviewState(reason: string): Promise<void> {
    ...
    console.log("[Cypher New] CypherProvider: 🔄 syncWebviewState()", {
      reason,
    ...
    if (!this.isWebviewReady) {
      console.log("[Cypher New] CypherProvider: ⏭️ syncWebviewState skipped (webview not ready)")
      return
    }
    ...
    if (serverInfo) {
      const langConfig = vscode.workspace.getConfiguration("cypher-cli.new")
      this.postMessage({
        type: "ready",
        serverInfo,
    ...
    // Profile returns 401 when user isn't logged into Cypher Gateway — that's expected.
    ...
    if (this.connectionState === "connected" && this.client) {
      console.log("[Cypher New] CypherProvider: 👤 syncWebviewState fetching profile...")
      const profileResult = await retry(() => this.client!.cypher.profile())
      const profileData = profileResult.data ?? null
      console.log("[Cypher New] CypherProvider: 👤 syncWebviewState profile:", profileData ? "received" : "null")
      this.postMessage({
        type: "profileData",
        data: profileData,
      })
  ...
  private setSidebarVisible(visible: boolean): void {
    ...
    vscode.commands.executeCommand("setContext", "cypher-cli.new.sidebarVisible", visible)
  }

  /** Resolve a WebviewPanel for displaying Cypher in an editor tab. */
  ...
  public selectCypherModel(modelID?: string, agent?: string): void {
    if (!modelID && !agent) return
    this.pendingCypherModel = { ...(modelID && { modelID }), ...(agent && { agent }) }
    this.flushPendingCypherModel()
  }
    ...
    this.webviewMessageDisposable = webview.onDidReceiveMessage(async (message) => {
      ...
      if (
        await handleSidebarWorktreeMessage(message, {
          post: (msg) => this.postMessage(msg),
          openAgentManager: () => vscode.commands.executeCommand("cypher-cli.new.agentManagerOpen"),
          openAdvancedWorktree: () => vscode.commands.executeCommand("cypher-cli.new.agentManager.advancedWorktree"),
          openChanges: (sessionId?: string, turnId?: string) =>
            vscode.commands.executeCommand("cypher-cli.new.showChanges", { sessionId, turnId }),
      ...
      switch (message.type) {
        case "webviewReady":
          console.log("[Cypher New] CypherProvider: ✅ webviewReady received")
        ...
        case "syncSession":
          this.handleSyncSession(message.sessionID, message.parentSessionID).catch((e) =>
            console.error("[Cypher New] handleSyncSession failed:", e),
          )
          break
        case "loadSessions":
          this.handleLoadSessions().catch((e) => console.error("[Cypher New] handleLoadSessions failed:", e))
          break
        ...
        case "openSettingsPanel":
          vscode.commands.executeCommand("cypher-cli.new.settingsButtonClicked", message.tab)
          break
        case "openCypherClaw":
          vscode.commands.executeCommand("cypher-cli.new.cypherClawOpen")
          break
        ...
        case "forkSession":
          handleForkSession(this.forkCtx, message.sessionId, message.messageId).catch((e) =>
            console.error("[Cypher New] handleForkSession failed:", e),
          )
          break
        case "retryConnection":
          console.log("[Cypher New] CypherProvider: 🔄 Retrying connection...")
          this.initializeConnection().catch((e) =>
            console.error("[Cypher New] CypherProvider: ❌ Retry connection failed:", e),
          )
          break
        case "reload":
          this.handleReload().catch((e) => console.error("[Cypher New] CypherProvider: Reload failed:", e))
          break
        case "openSubAgentViewer":
          vscode.commands.executeCommand("cypher-cli.new.openSubAgentViewer", message.sessionID, message.title)
          break
        case "saveImage":
          return saveImage(this.getWorkspaceDirectory(this.currentSession?.id), message)
        case "requestProviders":
          this.fetchAndSendProviders().catch((e) => console.error("[Cypher New] fetchAndSendProviders failed:", e))
          break
        ...
        case "fetchCustomProviderModels":
          this.handleFetchCustomProviderModels(message).catch((e) =>
            console.error("[Cypher New] fetchCustomProviderModels failed:", e),
          )
          break
        case "compact":
          ...
          break
        case "requestAgents":
          this.fetchAndSendAgents().catch((e) => console.error("[Cypher New] fetchAndSendAgents failed:", e))
          break
        case "requestSkills":
          this.fetchAndSendSkills().catch((e) => console.error("[Cypher New] fetchAndSendSkills failed:", e))
          break
        case "requestAgentRequirements":
          this.requirements
            ...
            .catch((e) => console.error("[Cypher New] fetchAndSendAgentRequirements failed:", e))
          break
        case "requestCommands":
          this.fetchAndSendCommands().catch((e) => console.error("[Cypher New] fetchAndSendCommands failed:", e))
          break
        case "removeSkill":
          this.removeSkillViaCli(message.location).catch((e: unknown) =>
            console.error("[Cypher New] removeSkill failed:", e),
          )
          break
        case "removeAgent":
          this.handleRemoveAgent(message.name).catch((e) => console.error("[Cypher New] handleRemoveAgent failed:", e))
          break
        case "removeMcp":
          this.handleRemoveMcp(message.name).catch((e) => console.error("[Cypher New] handleRemoveMcp failed:", e))
          break
        case "requestMcpStatus":
          this.fetchAndSendMcpStatus().catch((e) => console.error("[Cypher New] fetchAndSendMcpStatus failed:", e))
          break
        case "connectMcp": {
          ...
          if (c1) {
            ...
            ).catch((e) => console.error("[Cypher New] connectMcpServer failed:", e))
          }
          break
        }
        case "disconnectMcp": {
          ...
          if (c2) {
            void McpOAuth.disconnectMcpServer(c2, message.name, this.getWorkspaceDirectory(), () =>
              this.refreshMcpStatus(),
            ).catch((e) => console.error("[Cypher New] disconnectMcpServer failed:", e))
          }
          break
        }
        case "authenticateMcp": {
          ...
          if (c) {
            void McpOAuth.authenticateMcpServer(c, message.name, this.getWorkspaceDirectory(), () =>
              this.refreshMcpStatus(),
            ).catch((e) => console.error("[Cypher New] authenticateMcpServer failed:", e))
          }
          break
        }
        ...
        case "toggleSandbox":
          ...
          break
        case "requestConfig":
          this.fetchAndSendConfig().catch((e) => console.error("[Cypher New] fetchAndSendConfig failed:", e))
          break
        case "requestGlobalConfig":
          this.fetchAndSendGlobalConfig().catch((e) => console.error("[Cypher New] fetchAndSendGlobalConfig failed:", e))
          break
        case "requestIndexingStatus":
          this.fetchAndSendIndexingStatus().catch((e) =>
            console.error("[Cypher New] fetchAndSendIndexingStatus failed:", e),
          )
          break
        case "requestIndexingSettings":
          this.postMessage(buildIndexingSettingsMessage())
          break
        case "requestCypherEmbeddingModels":
          this.fetchAndSendCypherEmbeddingModels().catch((e) =>
            console.error("[Cypher New] fetchAndSendCypherEmbeddingModels failed:", e),
          )
          break
        case "requestImageModels":
          this.fetchAndSendImageModels().catch((e) => console.error("[Cypher New] fetchAndSendImageModels failed:", e))
          break
        case "updateConfig":
        ...
        case "openSettingsTab":
          if (message.tab === "indexing") {
            await vscode.commands.executeCommand("cypher-cli.new.openIndexingSettings")
          }
          break
        case "setLanguage":
          await vscode.workspace
            .getConfiguration("cypher-cli.new")
        ...
        case "requestRemoteStatus":
          this.remoteService
            ...
            .catch((err) => console.error("[Cypher New] remote message failed:", err))
          break
        case "deleteSession":
        ...
        case "requestNotifications":
          this.fetchAndSendNotifications().catch((e) =>
            console.error("[Cypher New] fetchAndSendNotifications failed:", e),
          )
          break
        ...
        case "enhancePrompt": {
          ...
          void sdkClient.enhancePrompt
            ...
            .catch((err: unknown) => {
              ...
              console.error("[Cypher New] CypherProvider: Failed to enhance prompt:", err)
   ...
   * Subscribes to the shared CypherConnectionService.
   */
  ...
  private async doInitializeConnection(): Promise<void> {
    console.log("[Cypher New] CypherProvider: 🔧 Starting initializeConnection...")
    ...
    try {
      ...
      this.unsubscribeEvent = this.connectionService.onEventFiltered(
        (payload, directory) => {
          ...
          if (event.type === "cypher-sessions.remote-status-changed") return true
          ...
          // CypherProvider instance. The Settings panel is a separate provider with no tracked
      ...
      this.unsubscribeState = this.connectionService.onStateChange(async (state, error) => {
        ...
        if (state === "connected") {
          ...
          try {
            ...
            const sdkClient = this.client
            if (sdkClient) {
              const profileResult = await sdkClient.cypher.profile()
            ...
            console.error("[Cypher New] CypherProvider: ❌ Failed during connected state handling:", error)
            this.postMessage({
              type: "error",
      ...
      // Subscribe to notification dismiss broadcast from other CypherProvider instances
      this.unsubscribeNotificationDismiss = this.connectionService.onNotificationDismissed(() => {
        this.fetchAndSendNotifications()
      })

      // Subscribe to language change broadcast from other CypherProvider instances
      ...
      // Subscribe to profile change broadcast from other CypherProvider instances
      this.unsubscribeProfileChange = this.connectionService.onProfileChanged((data) => {
        this.postMessage({ type: "profileData", data })
      })

      // Subscribe to favorites change broadcast from other CypherProvider instances
      this.unsubscribeFavoritesChange = this.connectionService.onFavoritesChanged((favorites) => {
        this.postMessage({ type: "favoritesLoaded", favorites })
      })

      // Subscribe to model-selector expand/collapse broadcast from other CypherProvider instances
      this.unsubscribeModelSelectorExpanded = this.connectionService.onModelSelectorExpandedChanged((value) => {
        this.postMessage({ type: "modelSelectorExpandedLoaded", value })
      })

      // legacy-migration start
      // Subscribe to migration-complete broadcast from any CypherProvider instance
      ...
      console.log("[Cypher New] CypherProvider: ✅ initializeConnection completed successfully")
    } catch (error) {
      console.error("[Cypher New] CypherProvider: ❌ Failed to initialize connection:", error)
      this.connectionState = "error"
      this.postMessage({
        type: "connectionState",
        state: "error",
  ...
  private async handleCreateSession(): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to create session:", error)
      this.postMessage({
        type: "error",
  ...
  private refreshSessionDetails(sessionID: string, dir: string, signal?: AbortSignal): void {
    ...
    this.client.session
      ...
      .catch((e: unknown) => console.warn("[Cypher New] CypherProvider: getSession failed (non-critical):", e))
    ...
    this.client.session
      ...
      .catch((e: unknown) => console.error("[Cypher New] CypherProvider: Failed to fetch session statuses:", e))
  }
  ...
  private fetchAndSendSessionModelUsage(sessionID: string, requestID: string): Promise<void> {
    ...
    return this.connectionService
      ...
      .then((client) => client.cypherCode.sessionModelUsage({ sessionID, directory }, { throwOnError: true }))
      .then((response) => {
      ...
      .catch((error: unknown) => {
        console.warn("[Cypher New] CypherProvider: Failed to load session model usage:", error)
  ...
  private async handleLoadMessages(
    ...
    try {
      ...
      if (abort?.signal.aborted) return
      console.error("[Cypher New] CypherProvider: Failed to load messages:", error)
  ...
  private async handleSyncSession(sessionID: string, parentSessionID?: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to sync child session:", err)
    }
  }
  ...
  private async flushPendingSessionRefresh(reason: string): Promise<void> {
    ...
    console.log("[Cypher New] CypherProvider: 🔄 Flushing deferred sessions refresh", { reason })
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to flush session refresh:", error)
    }
  ...
  private async handleLoadSessions(): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to load sessions:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleTerminalContext(requestId: string): Promise<void> {
    try {
      ...
      console.error("[Cypher New] Failed to capture terminal context:", error)
      this.postMessage({
  ...
  private async handleDeleteSession(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to delete session:", error)
      this.postMessage({
        type: "error",
  ...
  private async handleRenameSession(sessionID: string, title: string): Promise<void> {
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to rename session:", error)
  ...
  private async handleExportSessionTranscript(sessionID: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to export session transcript:", error)
      this.postMessage({
        type: "error",
    ...
    const task = (async () => {
      ...
      while (true) {
        ...
        try {
          ...
          const settings = vscode.workspace.getConfiguration("cypher-cli.new.model")
          const message = {
          ...
          console.error("[Cypher New] CypherProvider: Failed to fetch providers:", error)
        }
        if (!this.providersQueued) return
        generation = this.providersGeneration
      }
  ...
  private async fetchAndSendAgents(): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to fetch agents:", error)
    }
  }
  ...
  private async fetchAndSendSkills(): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to fetch skills:", error)
    }
  }
  ...
  private async fetchAndSendCommands(): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to fetch commands:", error)
    }
  }
  ...
  private async removeSkillViaCli(location: string): Promise<boolean> {
    ...
    try {
      const dir = this.getWorkspaceDirectory()
      const result = await this.client.cypherCode.removeSkill({ location, directory: dir })
      if (result.error) {
        console.error("[Cypher New] removeSkill returned error:", result.error)
      ...
      console.error("[Cypher New] Failed to remove skill:", error)
      this.cachedSkillsMessage = null
      this.cachedCommandsMessage = null
      await Promise.all([this.fetchAndSendSkills(), this.fetchAndSendCommands()])
      return false
  ...
  private async handleRemoveAgent(name: string): Promise<void> {
    if (!this.client) return
    try {
      const result = await this.client.cypherCode.removeAgent({ name, directory: this.getWorkspaceDirectory() })
      if (result.error) {
        console.error("[Cypher New] removeAgent returned error:", result.error)
      }
    } catch (err) {
      console.error("[Cypher New] Failed to remove agent:", err)
    }
    this.cachedAgentsMessage = null
  ...
  private async handleRemoveMcp(name: string): Promise<void> {
    ...
    if (!removed) {
      console.error("[Cypher New] CypherProvider: Failed to remove MCP server:", name)
    }
  }
  ...
  private async fetchAndSendMcpStatus(): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to fetch MCP status:", error)
    }
  }
  ...
  private async fetchAndSendConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to fetch config:", error)
    }
  }
  ...
  private async fetchAndSendGlobalConfig(): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to fetch global config:", error)
    }
  }
  ...
  private async fetchAndSendIndexingStatus(): Promise<void> {
    ...
    try {
      ...
      const auth = Buffer.from(`cypher:${config.password}`).toString("base64")
      const res = await fetch(`${config.baseUrl}/indexing/status`, {
        headers: {
          Authorization: `Basic ${auth}`,
          ...(dir ? { "x-cypher-directory": dir } : {}),
        },
      })
      ...
      console.error("[Cypher New] CypherProvider: Failed to fetch indexing status:", error)
    }
  }

  private async fetchAndSendCypherEmbeddingModels(): Promise<void> {
    const catalog = await fetchCypherEmbeddingModelCatalog()
    const message = { type: "cypherEmbeddingModelsLoaded", catalog }
    this.cachedCypherEmbeddingModelsMessage = message
    this.postMessage(message)
  }
  ...
  private async fetchAndSendConfigUpdated(): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to fetch config after update:", error)
    }
  }
  ...
  private async checkConfigWarnings(from: string): Promise<void> {
    if (this.configWarningsShown) {
      console.log("[Cypher New] CypherProvider: config warnings already shown", { from })
      return
    }
    if (!this.client) {
      console.log("[Cypher New] CypherProvider: config warnings skipped (no client)", { from })
      return
    }
    try {
      const dir = this.getWorkspaceDirectory()
      console.log("[Cypher New] CypherProvider: checking config warnings", { from, dir })
      const result = await this.client.config.warnings({ directory: dir })
      const list = result?.data ?? []
      console.log("[Cypher New] CypherProvider: config warnings fetched", { from, count: list.length })
      if (list.length === 0) return
      this.configWarningsShown = true

      const first = list[0]!
      const summary = list.length === 1 ? first.message : `${first.message} (and ${list.length - 1} more)`
      console.warn("[Cypher New] CypherProvider: showing config warnings", { from, count: list.length, path: first.path })
      ...
      if (action === "Show Details") {
        ...
        const channel = vscode.window.createOutputChannel("Cypher Config Warnings")
        channel.clear()
        channel.appendLine(lines.join("\n\n"))
        channel.show()
      }
    } catch (err) {
      console.warn("[Cypher New] CypherProvider: checkConfigWarnings failed:", { from, err })
    }
  }
  ...
  // Cloud session methods extracted to cypher-provider/handlers/cloud-session.ts
  ...
  private sendNotificationSettings(): void {
    const attention = vscode.workspace.getConfiguration("cypher-cli.new.attention")
    this.postMessage({
  ...
  private sendTimelineSetting(): void {
    const config = vscode.workspace.getConfiguration("cypher-cli.new")
    this.postMessage({
  ...
  private async runToggleSandbox(
    ...
    try {
      ...
      const remembered = await this.connectionService.sandboxPreference
        ...
        .then(() => true)
        .catch((error) => {
          console.error("[Cypher New] Failed to persist sandbox default:", error)
          return false
        })
      this.postMessage({
        type: "sandboxStatus",
  ...
  private async handleUpdateConfig(
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Config write succeeded but post-write refresh failed:", error)
      const patch =
  ...
  private postConfigFailure(error: unknown): void {
    console.error("[Cypher New] CypherProvider: Failed to update config:", error)
    this.postMessage({
  ...
  private async withRetry(
    ...
    try {
      for (let attempt = 1; ; attempt++) {
        ...
        console.log(`[Cypher New] CypherProvider: Retry on ${status}, attempt ${attempt}/${MAX_RETRIES}, delay ${delay}ms`)
  ...
  private maxCostSetting(): number {
    return this.setMaxCost(vscode.workspace.getConfiguration("cypher-cli.new").get<number>("maxCost", 0))
  }

  private commitMessageLanguageSetting(): string {
    return vscode.workspace.getConfiguration("cypher-cli.new").get<string>("languageCommitMessage", "sync")
  }
  ...
  private async handleSendMessage(
    ...
    try {
      ...
      await runWithMessageConfirmation(this.confirmations, messageID, "CypherProvider: Message request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[Cypher New] CypherProvider: Failed to send message:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleSendCommand(
    ...
    try {
      ...
      await this.requirements.assertAgentRequirements(agent, dir)
      await this.checkpoints.get(sid)
      await runWithMessageConfirmation(this.confirmations, messageID, "CypherProvider: Command request", () =>
        this.withRetry(
          () =>
      ...
      console.error("[Cypher New] CypherProvider: Failed to send command:", error)
      this.postMessage({
        type: "sendMessageFailed",
  ...
  private async handleRevertSession(sessionID: string, messageID: string, partID?: string): Promise<void> {
    ...
    if (error) {
      console.error("[Cypher New] CypherProvider: Failed to revert session:", error)
  ...
  private async handleUnrevertSession(sessionID: string): Promise<void> {
    ...
    if (error) {
      console.error("[Cypher New] CypherProvider: Failed to unrevert session:", error)
  ...
  private async handleCompact(sessionID?: string, providerID?: string, modelID?: string): Promise<void> {
    ...
    if (!target) {
      console.error("[Cypher New] CypherProvider: No sessionID for compact")
      return
    }

    if (!providerID || !modelID) {
      console.error("[Cypher New] CypherProvider: No model selected for compact")
      this.postMessage({
        type: "error",
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to compact session:", error)
      this.postMessage({
        type: "error",
  ...
  // Permission + question handlers extracted to cypher-provider/handlers/permission.ts and question.ts
  ...
  // Cloud session handlers extracted to cypher-provider/handlers/cloud-session.ts
  ...
  // Auth handlers extracted to cypher-provider/handlers/auth.ts
  ...
  private async disposeGlobal(): Promise<void> {
    ...
    await this.client.global
      .dispose()
      .catch((e: unknown) => console.warn("[Cypher New] CypherProvider: global.dispose() after org switch failed:", e))

    // Org switch succeeded — refresh profile and providers independently (best-effort)
    try {
      const profileResult = await this.client!.cypher.profile()
   ...
   * The key uses dot notation relative to `cypher-cli.new` (e.g. "browserAutomation.enabled").
   */
  private async handleUpdateSetting(key: string, value: unknown): Promise<void> {
    ...
    const config = vscode.workspace.getConfiguration(`cypher-cli.new${section ? `.${section}` : ""}`)
  ...
  /**
   * Reset all "cypher-cli.new.*" extension settings to their defaults by reading
   * contributes.configuration from the extension's package.json at runtime.
   * Only resets settings under the "cypher-cli.new." namespace to avoid touching
   * settings from the previous version of the extension which shares the same
   * extension ID and "cypher-cli.*" namespace.
   */
  private async handleResetAllSettings(): Promise<void> {
    const confirmed = await vscode.window.showWarningMessage(
      "Reset all Cypher CLI extension settings to defaults?",
      { modal: true },
      "Reset",
    )
    if (confirmed !== "Reset") return

    const prefix = "cypher-cli.new."
    const ext = vscode.extensions.getExtension("cypher.cypher-cli")
    ...
    await this.extensionContext?.globalState.update("cypher.dismissedNotificationIds", undefined)
    await this.extensionContext?.globalState.update("cypher.agentMigrationBannerDismissed", undefined)
    await this.extensionContext?.globalState.update("cypher.marketplace.dismissedSuggestions", undefined)
    ...
    vscode.window.showInformationMessage("Cypher CLI settings have been reset to defaults.")
  }
  ...
  private sendBrowserSettings(): void {
    const config = vscode.workspace.getConfiguration("cypher-cli.new.browserAutomation")
    this.postMessage({
  ...
  private sendClaudeCompatSetting(): void {
    const enabled = vscode.workspace.getConfiguration("cypher-cli.new").get<boolean>("claudeCodeCompat", false)
    this.postMessage({
  ...
  private async handleReload(): Promise<void> {
    if (!this.client) {
      console.warn("[Cypher New] handleReload: no client connection")
      return
    }
    const dir = this.getWorkspaceDirectory(this.currentSession?.id)
    try {
      ...
      if (status === 409) {
        ...
        console.error("[Cypher New] handleReload: reload endpoint failed:", err)
  ...
  private handleEvent(event: ProviderEvent, directory?: string): void {
    if (event.type === "cypher-sessions.remote-status-changed") {
    ...
    // so this runs on every CypherProvider instance — including the Settings panel which has no
    ...
    if (event.type === "message.part.updated") {
      ...
      if (childId && !this.trackedSessionIds.has(childId)) {
        console.log("[Cypher New] CypherProvider: 🔗 Auto-adopting child session from task tool", { childId })
    ...
    if (!this.webview) {
      const type =
        ...
        typeof (message as { type?: unknown }).type === "string"
          ...
          : "<unknown>"
      console.warn("[Cypher New] CypherProvider: ⚠️ postMessage dropped (no webview)", { type })
      return
    }

    void this.webview.postMessage(message).then(undefined, (error) => {
      console.error("[Cypher New] CypherProvider: ❌ postMessage failed", error)
    })
  }

  private flushPendingCypherModel(): void {
    if (!this.webview || !this.isWebviewReady || !this.client || !this.pendingCypherModel) return

    const pending = this.pendingCypherModel
    this.pendingCypherModel = null
    this.postMessage({ type: "selectCypherModel", ...pending })
  }
  ...
  public async appendReviewComments(comments: unknown[], autoSend = false): Promise<void> {
    ...
    if (!this.webview) {
      await vscode.commands.executeCommand(`${CypherProvider.viewType}.focus`)
    }
  ...
  public async toggleMemory(sessionID?: string): Promise<void> {
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to toggle memory:", error)
  ...
  private async getGitRemoteUrl(): Promise<string | undefined> {
    try {
      ...
      console.warn("[Cypher New] CypherProvider: Failed to get git remote URL:", error)
      return undefined
    }
  }
   ...
   * given directory, filtered through .cypherignore.
   */
  ...
  private async gatherEditorContext(dir?: string): Promise<EditorContext> {
    ...
    // Visible files (capped to avoid bloating context, filtered through .cypherignore)
    const visibleFiles = [
      ...new Set(
        [
    ...
    // Active file (also filtered through .cypherignore)
  ...
  // legacy-migration start -------------------------------------------------------
  // Migration handlers extracted to cypher-provider/handlers/migration.ts
