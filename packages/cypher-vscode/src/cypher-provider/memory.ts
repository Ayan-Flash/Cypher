...
} from "@cypher/cypher-memory/commands"
...
export type CypherProviderMemoryInput = {
  client(): CypherClient | undefined
...
function memory(client: CypherClient | undefined): MemoryApi | undefined {
  return (client as { memory?: MemoryApi } | undefined)?.memory
}

function request(input: Record<string, unknown>): { value: CypherProviderMemoryMessage } | { error: string } {
...
export class CypherProviderMemory {
  private readonly cached = new Map<string, unknown>()
  private tail = Promise.resolve()

  constructor(private readonly input: CypherProviderMemoryInput) {}
  ...
  async handle(message: Record<string, unknown>): Promise<boolean> {
    if (message.type === "requestMemory") {
      ...
      ).catch((err: unknown) => console.error("[Cypher New] fetchAndSendMemory failed:", err))
      return true
    }
  ...
  private async load(sessionID?: string, includeSources = false): Promise<void> {
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to fetch memory:", err)
      this.input.post({
        type: "memoryLoaded",
        sessionID,
  ...
  private async doShow(sessionID?: string): Promise<void> {
    ...
    try {
      ...
      console.error("[Cypher New] CypherProvider: Failed to show memory:", err)
      this.input.post({
        type: "memoryLoaded",
        sessionID,
  ...
  run(message: CypherProviderMemoryMessage): Promise<boolean> {
  ...
  private async execute(message: CypherProviderMemoryMessage): Promise<boolean> {
    const client = this.input.client()
    if (!client) {
      this.input.post({
        type: "memoryOperationResult",
    ...
    try {
      ...
      const refreshed = await Promise.all([
        ...
        console.warn("[Cypher New] Memory changed but refresh failed:", err)
        return undefined
      })
      ...
      console.error("[Cypher New] CypherProvider: Failed memory operation:", err)
      this.input.post({
        type: "memoryOperationResult",
        operation: message.operation,
        sessionID: message.sessionID,
  ...
  private async action(api: MemoryApi, directory: string, message: CypherProviderMemoryMessage) {
  ...
  private async remember(api: MemoryApi, directory: string, message: CypherProviderMemoryMessage) {
  ...
  private async correct(api: MemoryApi, directory: string, message: CypherProviderMemoryMessage) {
    const text = message.text?.trim()
  ...
  private async forget(api: MemoryApi, directory: string, message: CypherProviderMemoryMessage) {
  ...
  private async purge(api: MemoryApi, directory: string, message: CypherProviderMemoryMessage) {
  ...
  private async auto(api: MemoryApi, directory: string, message: CypherProviderMemoryMessage) {
