...
async function apply(connection: CypherConnectionService, directory: string, style: WorkStyle) {
  const settings = vscode.workspace.getConfiguration("cypher-cli.new")
...
export async function handleWorkStyleApplyMessage(input: {
  ...
  if (input.message.style !== "human-in-the-loop" && input.message.style !== "autonomous") {
    console.error("[Cypher New] Invalid style in applyWorkStyle message")
