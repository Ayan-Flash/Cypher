...
const KEY = "cypher.dismissedNotificationIds"
...
export async function fetchAndSendNotifications(ctx: NotificationsContext): Promise<void> {
  ...
  try {
    const { data: all } = await retry(() => ctx.client!.cypher.notifications(undefined, { throwOnError: true }))
    ...
    console.error("[Cypher New] CypherProvider: Failed to fetch notifications:", error)
  }
}
