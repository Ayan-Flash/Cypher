import type { Session, SessionStatus } from "@cypher/sdk/v2/client"
...
export async function handleForkSession(ctx: ForkContext, sessionId: string, messageId?: string): Promise<void> {
  const status =
    ...
    (await Promise.resolve()
      ...
      .catch((e) => {
        console.error("[Cypher New] refreshForkStatus failed:", e)
  ...
  await forkSession(
    {
      ...
      registerSession: () => {},
      log: (...args) => console.log("[Cypher New] CypherProvider:", ...args),
    },
    sessionId,
    undefined,
    messageId,
