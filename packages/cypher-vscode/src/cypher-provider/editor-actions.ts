...
function previewImage(dir: vscode.Uri | undefined, dataUrl: string, filename: string): void {
        ...
        return Promise.all(
          stale.map((name) =>
            Promise.resolve(vscode.workspace.fs.delete(vscode.Uri.joinPath(root, name), { recursive: true })).then(
              undefined,
              (err: unknown) => {
                console.warn("[Cypher New] CypherProvider: Failed to delete stale preview:", err)
              },
            ),
          ),
        )
  ...
  void vscode.workspace.fs
    ...
    .then(() => clean())
    .then(open, (err) => console.error("[Cypher New] CypherProvider: Failed to preview image:", err))
}
  ...
  if (message.type === "validateFiles") {
    ...
    if (id && paths && opts.post) {
      ...
      validateFiles(opts.dir(), paths).then(
        ...
        (err) => console.error("[Cypher New] CypherProvider: validateFiles failed:", err),
      )
    }
    return true
  }
...
function openContent(content: string, language?: string): void {
  vscode.workspace.openTextDocument({ content, language: language || "log" }).then(
    ...
    (err) => console.error("[Cypher New] CypherProvider: Failed to open content:", err),
  )
}
  ...
  vscode.workspace.openTextDocument(uri).then(
    (doc) => {
      ...
      vscode.window
        ...
        .then(undefined, (err) => console.error("[Cypher New] CypherProvider: Failed to show document:", uri.fsPath, err))
    },
    (err) => console.error("[Cypher New] CypherProvider: Failed to open file:", uri.fsPath, err),
  )
}
    ...
    (matches) => {
      ...
      if (matches.length > 1) {
        ...
        vscode.window.showQuickPick(items, { placeHolder: `Multiple matches for "${name}"` }).then(
          ...
          (err) => console.error("[Cypher New] CypherProvider: showQuickPick failed:", err),
        )
        return
      }
      vscode.window.showWarningMessage(`File not found: ${filePath}`)
    },
    (err: unknown) => console.error("[Cypher New] CypherProvider: findFiles failed:", err),
  )
}
