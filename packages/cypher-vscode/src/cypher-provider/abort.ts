import type { CypherClient, SessionStatus } from "@cypher/sdk/v2/client"
import { sameDirectory } from "../cypher-provider-utils"
...
export class SessionAbort {
  ...
  async stop(client: CypherClient, sessionID: string, fallback: string) {
    ...
    if (failures.length > 0) {
      console.error("[Cypher New] CypherProvider: Failed to abort session in one or more directories:", failures)
      return false
    }
...
export async function abortSession(input: { client: CypherClient; sessionID: string; dir: string }) {
