/**
 * Cloud session handlers — extracted from CypherProvider.
 *
...
import type { CypherClient, Session, TextPartInput, FilePartInput } from "@cypher/sdk/v2/client"
import type { CloudSessionData, EditorContext } from "../../services/cli-backend/types"
import { getErrorMessage, sessionToWebview, mapCloudSessionMessageToWebviewMessage } from "../../cypher-provider-utils"
...
export async function handleRequestCloudSessions(
  ...
  try {
    const result = await ctx.client.cypher.cloudSessions({
    ...
    console.error("[Cypher New] CypherProvider: Failed to fetch cloud sessions:", error)
    ctx.postMessage({
      type: "error",
...
export async function handleRequestCloudSessionData(ctx: CloudSessionContext, sessionId: string): Promise<void> {
  ...
  try {
    const result = await ctx.client.cypher.cloud.session.get({ id: sessionId }, { signal: AbortSignal.timeout(TIMEOUT) })
    ...
    console.error("[Cypher New] Failed to load cloud session data:", err)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId: sessionId,
...
export async function handleImportAndSend(
  ...
  try {
    const result = await ctx.client.cypher.cloud.session.import(
      {
    ...
    console.error("[Cypher New] CypherProvider: ❌ Cloud session import failed:", error)
    ctx.postMessage({
      type: "cloudSessionImportFailed",
      cloudSessionId,
  ...
  try {
    ...
    console.error("[Cypher New] Failed to send message after cloud import:", err)
    ctx.postMessage({
