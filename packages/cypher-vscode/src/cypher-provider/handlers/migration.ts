/**
 * Legacy migration handlers — extracted from CypherProvider.
 *
 * Manages the migration wizard for users upgrading from Cypher CLI v5.x.
...
export async function checkAndShowMigrationWizard(ctx: MigrationContext): Promise<void> {
  ...
  console.log("[Cypher New] CypherProvider: 🔄 Legacy data detected, showing migration wizard")
...
async function startLegacyMigration(
  ...
  try {
    ...
    console.error("[Cypher New] CypherProvider: ❌ Migration failed", error)
    ctx.postMessage({
      type: "migrationComplete",
      source: "legacy",
      operationId,
