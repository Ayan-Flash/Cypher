...
function getConfig() {
  return vscode.workspace.getConfiguration("cypher-cli.new")
}
...
export function watchWorkStyleConfig(post: (message: unknown) => void, next?: vscode.Disposable) {
  ...
  const watcher = vscode.workspace.onDidChangeConfiguration((event) => {
    if (keys.some((key) => event.affectsConfiguration(`cypher-cli.new.${key}`))) post(getWorkStylePayload())
  })
...
async function hasAnySession(connection: CypherConnectionService, directory: string): Promise<boolean> {
...
async function initializeWorkStyle(connection: CypherConnectionService, directory: string): Promise<void> {
...
export async function handleWorkStyleMessage(input: {
  ...
  if (input.message.type === "requestWorkStyle") {
    const initialized = await initializeWorkStyle(input.connection, input.directory)
      .then(() => true)
      .catch((err: unknown) => {
        console.error("[Cypher New] Failed to initialize work style:", err)
        return false
      })
  ...
  if (!input.message.style) {
    console.error("[Cypher New] Missing style in setWorkStyle message")
    return true
  }
