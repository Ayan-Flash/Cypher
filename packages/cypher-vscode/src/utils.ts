...
export function getWebviewFontSize(): number {
  const raw = vscode.workspace.getConfiguration("cypher-cli.new").get<number>("fontSize", 13)
  return clamp(raw)
}

function fontStyle(): string {
  const base = getWebviewFontSize()
  const vars = SIZES.map((size) => `--cypher-font-size-${size}: ${(base * size) / 13}px;`).join("\n      ")
  return `:root {
      ${vars}
      --cypher-font-scale: ${base / 13};
      --font-size-x-small: var(--cypher-font-size-10);
      --font-size-small: var(--cypher-font-size-11);
      --font-size-base: var(--cypher-font-size-13);
      --font-size-large: var(--cypher-font-size-16);
    }`
}
...
export function buildWebviewHtml(
  ...
  return `<!DOCTYPE html>
<html lang="en" data-theme="cypher-vscode">
<head>
  <meta charset="UTF-8">
...
<body>
  <div id="root"></div>
  <script nonce="${nonce}">window.ICONS_BASE_URI = "${opts.iconsBaseUri}"; window.CYPHER_SHIKI_WORKER_URI = "${opts.workerUri}";</script>
