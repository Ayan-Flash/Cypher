...
import type { CypherProvider } from "../../CypherProvider"
...
export function registerCodeActions(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("cypher-cli.new.explainCode", async () => {
    ...
    vscode.commands.registerCommand("cypher-cli.new.fixCode", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("cypher-cli.new.improveCode", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("cypher-cli.new.addToContext", async () => {
      const ctx = getEditorContext()
      if (!ctx) return
    ...
    vscode.commands.registerCommand("cypher-cli.new.focusChatInput", async () => {
      const view = target()
      if (view === provider) {
        await reveal()
      }
