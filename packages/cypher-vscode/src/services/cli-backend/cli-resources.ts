...
const cypherSandboxWorker = "cypher-sandbox-mutation-worker.js"
const bwrap = "bwrap"
...
export function resolveTreeSitterEnv(root: string): Record<string, string> {
  return { CYPHER_TREE_SITTER_WASM_DIR: treeSitterDirForExtension(root) }
}
...
export function cypherSandboxWorkerForBinary(file: string): string {
  const p = paths(file)
  return p.join(p.dirname(file), cypherSandboxWorker)
}

export function hasCypherSandboxWorker(file: string): boolean {
  return fs.existsSync(cypherSandboxWorkerForBinary(file))
}
...
export async function copyCypherSandboxWorker(source: string, target: string): Promise<void> {
  const from = cypherSandboxWorkerForBinary(source)
  const to = cypherSandboxWorkerForBinary(target)
  if (!fs.existsSync(from)) throw new Error(`Cypher sandbox mutation worker not found at ${from}`)
  await fs.promises.copyFile(from, to)
}

function cacheRoot() {
  const root = process.env.XDG_CACHE_HOME ?? path.join(os.homedir(), ".cache")
  return path.join(root, "cypher-vscode", "bwrap")
}
...
export function resolveLocalBwrapEnv(
  ...
  return { CYPHER_BWRAP_PATH: executable }
}
