...
export function resolveIndexingEnv(folders: readonly WorkspaceFolderLike[] | undefined): Record<string, string> {
  ...
  return { CYPHER_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" }
}

export function resolveManagedServerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  return { ...env, CYPHER_DISABLE_CHANNEL_DB: "true" }
}
...
export class ServerManager {
  ...
  async getServer(): Promise<ServerInstance> {
    console.log("[Cypher New] ServerManager: 🔍 getServer called")
    if (this.instance) {
      console.log("[Cypher New] ServerManager: ♻️ Returning existing instance:", { port: this.instance.port })
      return this.instance
    }

    if (this.startupPromise) {
      console.log("[Cypher New] ServerManager: ⏳ Startup already in progress, waiting...")
      return this.startupPromise
    }

    console.log("[Cypher New] ServerManager: 🚀 Starting new server instance...")
    this.startupPromise = this.startServer()
    try {
      this.instance = await this.startupPromise
      console.log("[Cypher New] ServerManager: ✅ Server started successfully:", { port: this.instance.port })
      return this.instance
    } finally {
  ...
  private async startServer(): Promise<ServerInstance> {
    ...
    console.log("[Cypher New] ServerManager: 📍 CLI path:", cliPath)
    console.log("[Cypher New] ServerManager: 🔐 Generated password (length):", password.length)
    ...
    console.log("[Cypher New] ServerManager: 📄 CLI isFile:", stat.isFile())
    console.log("[Cypher New] ServerManager: 📄 CLI mode (octal):", (stat.mode & 0o777).toString(8))

    return new Promise((resolve, reject) => {
      console.log("[Cypher New] ServerManager: 🎬 Spawning CLI process:", cliPath, ["serve", "--port", "0"])
      const cfg = vscode.workspace.getConfiguration("cypher-cli.new")
      ...
      const bwrapEnv = process.env.CYPHER_BWRAP_PATH ? {} : resolveLocalBwrapEnv(this.context.extensionPath, localCli)
      ...
      //     `cypher-cli.new.extraCaCerts` (NODE_EXTRA_CA_CERTS).
      ...
      const serverProcess = spawn(cliPath, ["serve", "--port", "0"], {
        ...
        env: {
          ...
          CYPHER_SERVER_PASSWORD: password,
          ...
          CYPHER_PARENT_PID: String(process.pid),
          CYPHER_CLIENT: "vscode",
          CYPHER_ENABLE_QUESTION_TOOL: "true",
          CYPHER_FEATURE: "vscode-extension",
          ...indexingEnv,
          CYPHER_TELEMETRY_LEVEL: vscode.env.isTelemetryEnabled ? "all" : "off",
          CYPHER_APP_NAME: "cypher-cli",
          CYPHER_PLATFORM: "vscode",
          CYPHER_MACHINE_ID: vscode.env.machineId,
          CYPHER_APP_VERSION: this.context.extension.packageJSON.version,
          CYPHER_VSCODE_VERSION: vscode.version,
          CYPHER_VERSION: this.context.extension.packageJSON.version,
          CYPHER_EDITOR_NAME: `${vscode.env.appName} ${vscode.version}`,
          ...(!claudeCompat && { CYPHER_DISABLE_CLAUDE_CODE: "true" }),
      ...
      console.log("[Cypher New] ServerManager: 📦 Process spawned with PID:", serverProcess.pid)
      ...
      serverProcess.stdout?.on("data", (data: Buffer) => {
        ...
        console.log("[Cypher New] ServerManager: 📥 CLI Server stdout:", output)

        const port = parseServerPort(output)
        if (port !== null && !resolved) {
          resolved = true
          console.log("[Cypher New] ServerManager: 🎯 Port detected:", port)
      ...
      serverProcess.stderr?.on("data", (data: Buffer) => {
        ...
        console.error("[Cypher New] ServerManager: ⚠️ CLI Server stderr:", errorOutput)
        stderrLines.push(errorOutput)
      })

      serverProcess.on("error", (error) => {
        console.error("[Cypher New] ServerManager: ❌ Process error:", error)
        if (!resolved) {
          reject(error)
        }
      })
      ...
      serverProcess.on("exit", (code) => {
        console.log("[Cypher New] ServerManager: 🛑 Process exited with code:", code)
      ...
      setTimeout(() => {
        if (!resolved) {
          console.error(`[Cypher New] ServerManager: ⏰ Server startup timeout (${STARTUP_TIMEOUT_SECONDS}s)`)
  ...
  private getCliPath(): string {
    ...
    console.log("[Cypher New] ServerManager: 📦 Using CLI path:", cliPath)
    return cliPath
  }
  ...
  dispose(): void {
    ...
    console.log("[Cypher New] ServerManager: 🔴 Disposing — sending SIGTERM to process group, PID:", proc.pid)
    ...
    const timer = setTimeout(() => {
      if (proc.exitCode === null) {
        console.warn("[Cypher New] ServerManager: ⚠️ Process did not exit after SIGTERM, sending SIGKILL")
