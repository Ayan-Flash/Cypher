...
// equivalents in @cypher/sdk. All API types (Session, Event, Agent,
// McpStatus, Config, etc.) should be imported from "@cypher/sdk/v2/client".

import type { IndexingStatus as SdkIndexingStatus } from "@cypher/sdk/v2/client"
...
export interface CyphercodeOrganization {
  id: string
  name: string
  role: string
}
...
export interface CyphercodeProfile {
  email: string
  name?: string
  organizations?: CyphercodeOrganization[]
...
// Cloud session from the Cypher cloud API (cli_sessions_v2)
...
// Full cloud session data for preview (from /cypher/cloud/session/:id)
