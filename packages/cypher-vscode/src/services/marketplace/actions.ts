...
export async function removeMarketplaceItemFromAllScopes(
  ...
  try {
    ...
    console.warn("[Cypher New] Marketplace removal failed:", err)
    return false
  }
}
...
async function fetchSkills(ctx: MarketplaceActionContext, dir: string) {
  try {
    ...
    console.warn("[Cypher New] Failed to fetch CLI skills for marketplace:", err)
    return undefined
  }
}

async function invalidate(
  ctx: { connection: CypherConnectionService },
  scope: "project" | "global",
  dir: string,
): Promise<void> {
  const client = await ctx.connection.getClientAsync(dir).catch((err: unknown) => {
    console.warn("[Cypher New] Marketplace CLI invalidation deferred:", err)
    return null
  })
  if (!client) return
  ...
  if (scope === "global") {
    await client.global.config.update({ config: {} }).catch((err: unknown) => {
      console.warn("[Cypher New] global.config.update after marketplace change failed:", err)
    })
  }
  await client.instance.dispose({ directory: dir }).catch((err: unknown) => {
    console.warn("[Cypher New] instance.dispose() after marketplace change failed:", err)
  })
}
...
async function removeLegacyMcp(
  ...
  if (project && scope !== "global") {
    files.push(vscode.Uri.file(path.join(project, ".cypher", "mcp.json")))
    files.push(vscode.Uri.file(path.join(project, ".cypher", "mcp.json")))
  }
  ...
  for (const uri of files) {
    ...
    try {
      ...
      console.warn("[Cypher New] Failed to remove legacy MCP from", uri.fsPath, err)
    }
  }
  return removed
}
