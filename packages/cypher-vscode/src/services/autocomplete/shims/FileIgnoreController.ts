...
const CYPHERIGNORE = ".cypherignore"
const GITIGNORE = ".gitignore"

/**
 * Patterns for sensitive environment files, applied only when no .cypherignore exists.
 */
...
export class FileIgnoreController {
  ...
  async initialize(): Promise<void> {
    ...
    // Try .cypherignore first — if it exists, use only that.
    ...
    const cypherignorePath = path.join(this.workspacePath, CYPHERIGNORE)
    if (fs.existsSync(cypherignorePath)) {
      const cypherignoreContent = fs.readFileSync(cypherignorePath, "utf-8")
      if (cypherignoreContent.trim()) {
        this.ignoreInstance.add(cypherignoreContent)
        this.ignoreInstance.add(CYPHERIGNORE)
        return
      }
    }
