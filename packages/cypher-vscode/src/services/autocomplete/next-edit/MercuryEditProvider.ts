...
export interface MercuryEditProviderOptions {
  ...
  /** Provider id to send to the gateway (e.g. `"cypher"` or `"inception"`). */
  providerId?: string
...
/**
 * Thin wrapper around the SDK's `client.cypher.edit(...)` endpoint (non-streaming).
 * The gateway (`packages/cypher-gateway/src/server/edit.ts`) handles auth, routing
...
export class MercuryEditProvider {
  ...
  async suggest(ctx: MercuryEditRequestContext): Promise<MercuryEditSuggestion | null> {
    ...
    nesLog(
      `-> /cypher/edit provider=${provider} model=${model} region=[${ctx.editableRegionStartLine},${ctx.editableRegionEndLine}] diffs=${ctx.editDiffHistory.length} snippets=${ctx.recentlyViewedSnippets.length}`,
    )
    ...
    try {
      ...
      const { data, error, response } = await client.cypher.edit(
        {
          provider,
          model,
