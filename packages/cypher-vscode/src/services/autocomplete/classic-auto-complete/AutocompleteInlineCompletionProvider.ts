 ...
 */
export const INLINE_COMPLETION_ACCEPTED_COMMAND = "cypher.autocomplete.inline-completion.accepted"
...
export class AutocompleteInlineCompletionProvider implements vscode.InlineCompletionItemProvider {
  ...
  private connectionService: CypherConnectionService
  ...
  public async provideInlineCompletionItems_Internal(
    ...
    try {
      ...
      if (matchingResult !== null) {
        ...
        vscode.commands.executeCommand("setContext", "cypher-cli.new.autocomplete.hasSuggestions", true)
  ...
  private async hasBalance(): Promise<boolean> {
    try {
      ...
      const result = await client.cypher.profile().catch(() => null)
