 ...
 * Uses the SDK's cypher.fim() SSE endpoint which handles auth and streaming.
 *
...
export async function generateFim(
  ...
  console.info(`[FIM] request provider=${info.providerID} model=${info.requestModel} url=/cypher/fim`)

  const { stream } = await client.cypher.fim(
    {
      prefix,
      suffix,
 ...
 */
export function hasValidCredentials(connectionService: CypherConnectionService): boolean {
