...
const CONFIG_SECTION = "cypher-cli.new.autocomplete"
...
export class AutocompleteServiceManager {
  ...
  constructor(context: vscode.ExtensionContext, connectionService: CypherConnectionService) {
    ...
    this.config = vscode.workspace.onDidChangeConfiguration((event) => {
      if (event.affectsConfiguration("cypher-cli.new.language")) {
  ...
  private async updateGlobalContext() {
    await vscode.commands.executeCommand(
      "setContext",
      "cypher.autocomplete.enableSmartInlineTaskKeybinding",
  ...
  private handleFatalAutocompleteError(status: number | null): void {
    const msg =
      status === 402
        ? t("cypher:autocomplete.creditsExhausted.message")
        : t("cypher:autocomplete.authError.message")

    if (status === 402) {
      vscode.window.showWarningMessage(msg, t("cypher:autocomplete.creditsExhausted.addCredits")).then((choice) => {
        if (choice === t("cypher:autocomplete.creditsExhausted.addCredits")) {
          vscode.env.openExternal(vscode.Uri.parse("https://app.cypher.ai/credits"))
        }
      })
    } else {
  ...
  public async showIncompatibilityExtensionPopup() {
    const message = t("cypher:autocomplete.incompatibilityExtensionPopup.message")
    const disableCopilot = t("cypher:autocomplete.incompatibilityExtensionPopup.disableCopilot")
    const disableInlineAssist = t("cypher:autocomplete.incompatibilityExtensionPopup.disableInlineAssist")
    ...
    if (response === disableCopilot) {
      ...
      await vscode.commands.executeCommand("cypher-cli.new.autocomplete.disable")
    }
  }
