 ...
 * .cypherignore / .gitignore files are modified) and shares a single
...
export class ChatTextAreaAutocomplete {
  private connection: CypherConnectionService
  readonly telemetry: AutocompleteTelemetry
  private ignore: FileIgnoreController | null = null
  private dir = ""
  private watcher: vscode.FileSystemWatcher | undefined

  constructor(connectionService: CypherConnectionService, telemetry?: AutocompleteTelemetry) {
    this.connection = connectionService
    this.telemetry = telemetry ?? new AutocompleteTelemetry("chat-textarea")
    this.watcher = vscode.workspace.createFileSystemWatcher("**/{.cypherignore,.gitignore}")
    const invalidate = () => {
  ...
  async getCompletion(userText: string, visibleCodeContext?: VisibleCodeContext): Promise<{ suggestion: string }> {
    const cfg = vscode.workspace.getConfiguration("cypher-cli.new.autocomplete")
