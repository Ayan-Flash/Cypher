...
const CONFIG = "cypher-cli.new.autoApprove"
const KEY = "enabled"
...
export function registerToggleAutoApprove(
      ...
      try {
        ...
        for (const req of pending) {
          ...
          await client.permission
            ...
            .catch((err) => {
              console.error("[Cypher New] toggleAutoApprove: failed to drain pending:", err)
            })
        }
      } catch (err) {
        console.error("[Cypher New] toggleAutoApprove: failed to list pending permissions:", err)
      }
    }
  ...
  const approve = async (event: Asked, directory?: string) => {
    ...
    return client.permission
      ...
      .then(
        () => true,
        (err) => {
          console.error("[Cypher New] toggleAutoApprove: failed to auto-reply:", err)
          return false
        },
      )
  }
  ...
  context.subscriptions.push(vscode.commands.registerCommand("cypher-cli.new.toggleAutoApprove", toggle))
...
function tryGetClient(connectionService: CypherConnectionService): CypherClient | undefined {
  try {
