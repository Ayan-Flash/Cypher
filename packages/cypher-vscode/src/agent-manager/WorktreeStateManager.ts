 ...
 *
 * Persists to `.cypher/agent-manager.json`. Decouples worktrees from sessions
...
import { CYPHER_DIR, migrateAgentManagerData, type MigrationResult } from "./constants"
...
export class WorktreeStateManager {
  ...
  constructor(root: string, log: (msg: string) => void) {
    this.root = root
    this.file = path.join(root, CYPHER_DIR, STATE_FILE)
    this.log = log
  }
  ...
  async load(): Promise<StateLoadResult> {
    // Migrate Agent Manager data from .cypher → .cypher before first read
  ...
  private apply(content: string): void {
    ...
    for (const [id, wt] of Object.entries(data.worktrees ?? {})) {
      // Rewrite stale .cypher paths while preserving the separator style already stored.
      const fixed =
        wt.path?.replace(/([/\\])\.cypher([/\\])/g, (_match, leadingSep, trailingSep) => {
          return `${leadingSep}.cypher${trailingSep}`
        }) ?? wt.path
