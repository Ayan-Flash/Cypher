...
// TODO: Remove the legacy .cypher -> .cypher migration helpers below after the
// GA release cleanup tracked in https://github.com/Cypher-Org/cypher/issues/6986.
...
/** Cypher config directory name (project-level and inside worktrees). */
export const CYPHER_DIR = ".cypher"

/** Legacy config directory name for backward compatibility reads. */
export const LEGACY_DIR = ".cypher"

/** Agent Manager files that should be migrated from .cypher/ to .cypher/. */
...
export interface MigrationResult {
  /** Number of git worktree refs that were rewritten from .cypher → .cypher. */
  refsFixed: number
}

/**
 * Migrate Agent Manager data from .cypher/ to .cypher/.
 *
 * Moves individual Agent Manager files/directories (worktrees, state,
 * setup scripts) from the legacy .cypher/ into .cypher/. Skips items
 * that already exist in .cypher/ (the new location wins). This is safe
 * because Agent Manager exclusively owns these files.
 *
 * Fixes git worktree internal references (.git/worktrees/{name}/gitdir)
 * whenever .cypher/worktrees/ exists so partially migrated repos recover too.
 *
...
export async function migrateAgentManagerData(root: string, log: (msg: string) => void): Promise<MigrationResult> {
  ...
  const target = path.join(root, CYPHER_DIR)

  if (await isDirectory(legacy)) {
    // Ensure .cypher/ exists
    try {
    ...
    for (const item of AGENT_MANAGER_ITEMS) {
      ...
      if (await exists(dst)) {
        log(`Skipping ${item}: already exists in ${CYPHER_DIR}`)
        continue
      }

      try {
        await fs.promises.rename(src, dst)
        log(`Migrated ${item} from ${LEGACY_DIR} to ${CYPHER_DIR}`)
      } catch (err) {
...
/**
 * After moving worktrees from .cypher/ to .cypher/, fix git internal refs.
 *
 ...
 * gitdir files that reference the old .cypher path.
 *
...
async function fixGitWorktreeRefs(root: string, log: (msg: string) => void): Promise<number> {
  ...
  const newSegment = path.join(root, CYPHER_DIR) + path.sep
  let fixed = 0
