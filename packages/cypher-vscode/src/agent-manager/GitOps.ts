 ...
 * `GIT_SSH_COMMAND` originated from Cypher (safe) or was inherited from the
 * parent process (untrusted).
 */
export const CYPHER_NON_INTERACTIVE_SSH_COMMAND = "ssh -o BatchMode=yes"
...
export function nonInteractiveEnv(): NodeJS.ProcessEnv {
  ...
  if (!process.env.GIT_SSH_COMMAND) {
    env.GIT_SSH_COMMAND = CYPHER_NON_INTERACTIVE_SSH_COMMAND
  }
  return env
}

/**
 * True when `env.GIT_SSH_COMMAND` is the fixed value Cypher sets, rather than
 ...
 */
export function isCypherOwnedSshCommand(env: NodeJS.ProcessEnv): boolean {
  return env.GIT_SSH_COMMAND === CYPHER_NON_INTERACTIVE_SSH_COMMAND
}
...
export class GitOps {
  ...
  async buildWorktreePatch(sourcePath: string, baseBranch: string, selectedFiles?: string[]): Promise<string> {
    const tmp = await fs.mkdtemp(nodePath.join(os.tmpdir(), "cypher-apply-"))
