 ...
 * sessions are stored in `.cypher/agent-manager.json`. The UI shows two
...
export class AgentManagerProvider implements Disposable {
  public static readonly viewType = "cypher-cli.new.AgentManagerPanel"
  ...
  constructor(
    ...
    this.outputChannel = host.createOutput("Cypher Agent Manager")
    ...
    this.unsubTool = this.connectionService.onEventFiltered(
      (event) => (event as { type?: string }).type === "cypher.agent_manager.start",
  ...
  private async initializeState(): Promise<void> {
    ...
    // When the .cypher → .cypher migration rewrote git worktree refs, nudge
  ...
  private openWorktreeFile(sessionId: string, filePath: string, line?: number, column?: number): void {
    ...
    try {
      ...
      console.error("[Cypher New] AgentManagerProvider: Cannot resolve file path:", err)
      return
    }
   ...
   * Called from CypherProvider when the sidebar sends "continueInWorktree".
   */
