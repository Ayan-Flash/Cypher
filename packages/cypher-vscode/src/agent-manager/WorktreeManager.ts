 ...
 *
 * Ported from cypher/src/core/cypher/agent-manager/WorktreeManager.ts.
 * Handles creation, discovery, and cleanup of worktrees stored in
 * {projectRoot}/.cypher/worktrees/
 */
...
import { type GitOps, isCypherOwnedSshCommand, nonInteractiveEnv } from "./GitOps"
...
const TEMP_PREFIX = ".cypher-delete-"
...
import { CYPHER_DIR, LEGACY_DIR, migrateAgentManagerData } from "./constants"

const SESSION_ID_FILE = "session-id"
const METADATA_FILE = "metadata.json"
const GIT_METADATA_FILE = "cypher-agent-manager-metadata.json"
...
export class WorktreeManager {
  ...
  constructor(root: string, log: (msg: string) => void, ops?: GitOps) {
    this.root = root
    this.dir = path.join(root, CYPHER_DIR, "worktrees")
    this.git = simpleGit(root)
    this.ops = ops
    this.log = log
  }

  /** Run once before first read/write to migrate Agent Manager data from .cypher → .cypher. */
  ...
  private async removeWorktreeImpl(worktreePath: string, branch?: string): Promise<void> {
    ...
    const temp = path.join(path.dirname(worktreePath), `.cypher-delete-${randomUUID()}`)
    try {
  ...
  /** Remove orphaned .cypher-delete-* temp dirs left by interrupted deletions. */
  ...
  async readMetadata(
    ...
    // Check .cypher/ first, then legacy .cypher/
    for (const dirName of [CYPHER_DIR, LEGACY_DIR]) {
  ...
  async ensureGitExclude(): Promise<void> {
    ...
    const items = [
      [".cypher/worktrees/", "Cypher CLI agent worktrees"],
      [".cypher/agent-manager.json", "Cypher Agent Manager state"],
      [".cypher/setup-script", "Cypher CLI worktree setup script"],
      [".cypher/setup-script.sh", "Cypher CLI worktree setup script"],
      [".cypher/setup-script.ps1", "Cypher CLI worktree setup script"],
      [".cypher/setup-script.cmd", "Cypher CLI worktree setup script"],
      [".cypher/setup-script.bat", "Cypher CLI worktree setup script"],
      [".cypher/worktrees/", "Cypher CLI legacy agent worktrees"],
      [".cypher/agent-manager.json", "Cypher Agent Manager legacy state"],
      [".cypher/setup-script", "Cypher CLI legacy worktree setup script"],
      [".cypher/setup-script.sh", "Cypher CLI legacy worktree setup script"],
      [".cypher/setup-script.ps1", "Cypher CLI legacy worktree setup script"],
      [".cypher/setup-script.cmd", "Cypher CLI legacy worktree setup script"],
      [".cypher/setup-script.bat", "Cypher CLI legacy worktree setup script"],
    ] as const
  ...
  async resolveStartPoint(
    ...
    if (remote) {
      ...
      try {
        ...
        // is the fixed value Cypher injects — never for an inherited one, which
        // could be attacker-controlled.
        const env = nonInteractiveEnv()
        await simpleGit(this.root, { unsafe: { allowUnsafeSshCommand: isCypherOwnedSshCommand(env) } })
          .env(env)
