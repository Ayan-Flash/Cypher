/**
 * Provider action handlers extracted from CypherProvider to stay under max-lines.
 * These are pure async functions that operate on the SDK client — no vscode dependency.
 */
import type { Config, CypherClient } from "@cypher/sdk/v2"
...
import { isCustomProviderPackage, CYPHER_AUTO, CYPHER_PROVIDER_ID, parseModelString } from "./shared/provider-model"
...
export async function fetchProviderData(client: CypherClient, dir: string) {
  const authRequest =
    typeof client.provider.auth === "function"
      ? client.provider
          ...
          .catch(() => ({}))
      : Promise.resolve({})
  const cypherRequest = client.cypher
    .authStatus({ directory: dir }, { throwOnError: true })
    .then((r) => (r.data?.authenticated ? (r.data.type ?? null) : null))
    .catch(() => null)

  const [{ data: response }, authMethods, cypherAuth] = await Promise.all([
  ...
  delete authStates[CYPHER_PROVIDER_ID]
  if (cypherAuth) authStates[CYPHER_PROVIDER_ID] = cypherAuth
  ...
  return {
    ...
    disposeGlobal: async (reason: string) => {
      ...
      await client.global.dispose().catch((error: unknown) => {
        console.warn(`[Cypher New] CypherProvider: global.dispose() after ${reason} failed:`, error)
      })
    },
...
async function removeAuth(ctx: ActionContext, id: string, configured: boolean) {
  try {
    ...
    console.warn(`[Cypher New] auth.remove failed for configured provider ${id} (non-fatal):`, err)
  }
}
