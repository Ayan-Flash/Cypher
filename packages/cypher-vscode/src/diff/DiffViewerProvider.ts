...
export class DiffViewerProvider implements vscode.Disposable {
  public static readonly viewType = "cypher-cli.new.DiffViewerPanel"
  ...
  constructor(
    ...
    this.output = vscode.window.createOutputChannel("Cypher Diff Panel")
  }
  ...
  /**
   * Entry point for the `cypher-cli.new.showChanges` command. Composes the
  ...
  private onWebviewReady(): void {
    ...
    void this.panel.webview.postMessage({
      ...
      languageOverride: vscode.workspace.getConfiguration("cypher-cli.new").get<string>("language"),
