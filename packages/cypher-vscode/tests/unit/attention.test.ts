...
describe("attention defaults", () => {
  it("keeps attention sounds opt-in", async () => {
    ...
    expect(properties["cypher-cli.new.attention.enabled"]?.default).toBe(false)
    expect(properties["cypher-cli.new.attention.sound"]?.default).toBe("default")
    expect(properties["cypher-cli.new.attention.sound"]?.enum).toEqual(["default", "system", ...CustomSoundIDs])
    expect(properties["cypher-cli.new.sounds.agentEnabled"]).toBeUndefined()
    expect(properties["cypher-cli.new.sounds.permissionsEnabled"]).toBeUndefined()
    expect(properties["cypher-cli.new.sounds.errorsEnabled"]).toBeUndefined()
  })
