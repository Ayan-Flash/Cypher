...
import { createAutoApproveBridge } from "../../src/cypher-provider/auto-approve"
import type { Event, CypherClient } from "@cypher/sdk/v2/client"
import type { CypherConnectionService } from "../../src/services/cli-backend/connection-service"
...
function config(initial: boolean, info: Record<string, unknown> = {}) {
  ...
  return {
    ...
    emit(key = "cypher-cli.new.autoApprove.enabled") {
...
function connection(client: CypherClient | null, dirs = new Map<string, string>()) {
  const svc = {
    getClient: () => {
  ...
  } as unknown as CypherConnectionService
...
describe("registerToggleAutoApprove", () => {
  ...
  it("uses the SSE directory for worktree permissions before session mappings are available", async () => {
    ...
    await ctrl.approve(asked("perm_worktree", "ses_worktree"), "/workspace/.cypher/worktrees/feature")
    await ctrl.approve(asked("perm_child", "ses_child"), "/workspace/.cypher/worktrees/feature")

    expect(replies).toEqual([
      { requestID: "perm_worktree", directory: "/workspace/.cypher/worktrees/feature", reply: "once" },
      { requestID: "perm_child", directory: "/workspace/.cypher/worktrees/feature", reply: "once" },
    ])
  })
  ...
  it("uses the shared permission directory before falling back to session mappings", async () => {
    ...
    const conn = connection(
      ...
      new Map([["perm_shared", "/workspace/.cypher/worktrees/shared"]]),
    )
    const ctrl = registerToggleAutoApprove(
      context(),
      conn.svc,
    ...
    expect(replies).toEqual([
      { requestID: "perm_shared", directory: "/workspace/.cypher/worktrees/shared", reply: "once" },
    ])
  })
