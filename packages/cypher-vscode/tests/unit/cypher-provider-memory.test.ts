...
import { CypherProviderMemory } from "../../src/cypher-provider/memory"

function subject(client: CypherClient | undefined) {
  const posts: unknown[] = []
  const memory = new CypherProviderMemory({
    client: () => client,
    session: () => undefined,
    dir: () => "/repo",
...
describe("CypherProviderMemory", () => {
  it("handles clients without memory endpoints gracefully", async () => {
    const item = subject({} as CypherClient)
  ...
  it("evicts older cached memory payloads", async () => {
    let client: CypherClient | undefined
    const posts: unknown[] = []
    const item = new CypherProviderMemory({
      client: () => client,
      session: () => undefined,
    ...
    expect(posts[1]).toMatchObject({
      type: "memoryLoaded",
      sessionID: "ses_8",
      show: { root: "/repo/ses_8/.cypher/memory" },
    })
  })
    ...
    const item = subject({
      memory: {
        configure: async (input: unknown) => {
          ...
          return { data: { root: "/repo/.cypher/memory", state: state.state } }
        },
        purge: async (input: unknown) => {
          calls.push(["purge", input])
          return { data: { root: "/repo/.cypher/memory", purged: true } }
        },
        status: async () => ({ data: state }),
        show: async () => ({ data: view }),
      },
