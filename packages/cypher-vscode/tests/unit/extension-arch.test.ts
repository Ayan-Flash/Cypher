...
const CYPHER_PROVIDER_FILE = path.join(ROOT, "src/CypherProvider.ts")
...
describe("Extension — package.json command sync", () => {
  ...
  it("every contributes.commands entry has a registerCommand() call", () => {
    ...
    const dynamic: Record<string, string> = {
      "cypher-cli.new.agentManager.jumpTo": "registerCommand(`cypher-cli.new.agentManager.jumpTo${",
    }
  ...
  /**
   * All declared commands must use the cypher-cli.new. prefix.
   * The legacy cypher-cli.* namespace (without .new.) belongs to the old
   * extension and must not be reintroduced.
   */
  it("all declared commands use the cypher-cli.new. prefix", () => {
    const bad = declared.filter((cmd) => !cmd.startsWith("cypher-cli.new."))
    expect(
      bad,
      `Commands without "cypher-cli.new." prefix — use the namespaced form:\n` + bad.map((b) => `  - ${b}`).join("\n"),
    ).toEqual([])
  })
  ...
  it("scopes Agent Manager search to the panel and leaves the integrated terminal alone", () => {
    const binding = pkg.contributes?.keybindings?.find(
      (item: { command: string }) => item.command === "cypher-cli.new.agentManager.search",
    )
    expect(binding).toMatchObject({
      key: "ctrl+f",
      mac: "cmd+f",
      when: "activeWebviewPanelId == 'cypher-cli.new.AgentManagerPanel' && !terminalFocus",
    })
  })

  it("scopes the open PR shortcut to Agent Manager", () => {
    const binding = pkg.contributes?.keybindings?.find(
      (item: { command: string }) => item.command === "cypher-cli.new.agentManager.openPR",
    )
    expect(binding).toMatchObject({
      key: "ctrl+shift+r",
      mac: "cmd+shift+r",
      when: "activeWebviewPanelId == 'cypher-cli.new.AgentManagerPanel'",
    })
  })
})

// ---------------------------------------------------------------------------
// CypherProvider handler wiring — every new CypherProvider() must get
// setContinueInWorktreeHandler() called before resolving its webview.
//
// Regression: tab panels created via openCypherInNewTab() and the TabPanel
...
describe("Extension — CypherProvider handler wiring", () => {
  const ext = fs.readFileSync(EXTENSION_FILE, "utf-8")

  /**
   * Every `new CypherProvider(` in extension.ts must be followed (before the
   * next `new CypherProvider(`) by a `setContinueInWorktreeHandler` call.
   * This prevents future tab/panel additions from silently missing the handler.
   */
  it("every CypherProvider instance gets setContinueInWorktreeHandler wired", () => {
    const pattern = /new CypherProvider\(/g
    ...
    expect(
      instances.length,
      "expected at least 3 CypherProvider instances (sidebar, tab, deserializer)",
    ...
    for (let i = 0; i < instances.length; i++) {
      ...
      if (!region.includes("setContinueInWorktreeHandler")) {
        ...
        missing.push(`CypherProvider at line ${line}`)
      }
    }

    expect(
      missing,
      `These CypherProvider instances are missing setContinueInWorktreeHandler.\n` +
  ...
  it("openCypherInNewTab wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const fn = ext.indexOf("function openCypherInNewTab")
    expect(fn, "openCypherInNewTab must exist").toBeGreaterThan(-1)
  ...
  it("TabPanel deserializer wires setContinueInWorktreeHandler before resolveWebviewPanel", () => {
    const serializer = ext.indexOf('"cypher-cli.new.TabPanel"')
...
// ---------------------------------------------------------------------------
// CypherProvider — continueInWorktree error fallback
//
...
describe("CypherProvider — remote focus lifecycle", () => {
  const provider = fs.readFileSync(CYPHER_PROVIDER_FILE, "utf-8")
...
describe("CypherProvider — continueInWorktree error fallback", () => {
  const helper = fs.readFileSync(path.join(ROOT, "src/cypher-provider/continue-worktree.ts"), "utf-8")
