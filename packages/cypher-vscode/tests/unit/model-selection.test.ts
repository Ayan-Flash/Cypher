...
import { CYPHER_AUTO, parseModelString } from "../../src/shared/provider-model"
...
const providers = {
  cypher: makeProvider("cypher", "Cypher Gateway", ["cypher-auto/free"]),
...
describe("parseModelString", () => {
  ...
  it("keeps slashes inside cypher model ids", () => {
    expect(parseModelString("cypher/cypher-auto/free")).toEqual({
      providerID: "cypher",
      modelID: "cypher-auto/free",
    })
  })
...
describe("resolveModelSelection", () => {
  ...
  it("uses cypher auto as the explicit final fallback", () => {
    const result = resolveModelSelection({
      providers,
      connected: [],
      fallback: CYPHER_AUTO,
    })
    expect(result).toEqual(CYPHER_AUTO)
  })

  it("keeps the explicit fallback even when cypher is missing from the loaded catalog", () => {
    const result = resolveModelSelection({
