...
const { CypherProvider, unwrapSyncEvent } = await import("../../src/CypherProvider")
...
function makeProvider(client: ReturnType<typeof createClient>) {
  ...
  const provider = new CypherProvider({} as never, connection as never)
...
describe("CypherProvider.handleAbort", () => {
...
describe("CypherProvider sandbox status", () => {
...
describe("CypherProvider sandbox toggle", () => {
  ...
  it("resolves a blank worktree default against the routed directory", async () => {
    ...
    await internal.fetchAndSendSandboxDefault("/repo/.cypher/worktrees/wt-1")

    expect(client.configReads).toEqual([{ directory: "/repo/.cypher/worktrees/wt-1" }])
    expect(client.sandboxSupport).toEqual([{ directory: "/repo/.cypher/worktrees/wt-1" }])
  })
  ...
  it("waits for a blank toggle before creating the first prompt session", async () => {
    ...
    expect(client.created).toEqual([
      expect.objectContaining({ metadata: { "cypher.sandbox": { enabled: true, version: 0 } } }),
    ])
  ...
  it("snapshots the remembered default before sending the first prompt", async () => {
    ...
    expect(client.created).toEqual([
      expect.objectContaining({
        directory: "/repo",
        metadata: { "cypher.sandbox": { enabled: true, version: 0 } },
      }),
    ])
    expect(client.sandboxed).toHaveLength(0)
    expect(client.prompted).toHaveLength(1)
...
describe("CypherProvider revert ordering", () => {
...
describe("CypherProvider.handleLoadMessages / focus mode freshness", () => {
...
describe("CypherProvider.handleDeleteSession / background processes", () => {
...
describe("CypherProvider.handleLoadMessages / slim payload", () => {
...
describe("CypherProvider.loadMessages / sub-agent viewer", () => {
...
describe("CypherProvider.handleLoadMessages / prepend into deleted session", () => {
