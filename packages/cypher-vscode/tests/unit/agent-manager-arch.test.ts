...
describe("Agent Manager CSS Prefix", () => {
  it("all class selectors should use am- prefix", () => {
    ...
    // - `cypher-diff-theme` is the shared Pierre diff theme utility defined
    //   in webview-ui/src/styles/diff.css and reused across webviews.
    // - `css` is matched from `@import "./diff.css"` file extension, not a
    //   class selector.
    const host = new Set(["vscode-high-contrast", "vscode-high-contrast-light", "cypher-diff-theme", "css"])
  ...
  it("all CSS custom properties should use am- prefix", () => {
    ...
    // Allow cypher-ui design tokens, vscode theme variables, and third-party
    // library tokens (@pierre/diffs, cypher-ui sticky-accordion) used as fallbacks
...
describe("Agent Manager Webview — non-git sessionsLoaded fix", () => {
  ...
  /**
   * Regression: when isGitRepo is false, the Cypher server never sends a
...
// ---------------------------------------------------------------------------
// CypherProvider — pendingSessionRefresh race condition fix
// ---------------------------------------------------------------------------

describe("CypherProvider — pending session refresh on reconnect", () => {
  const provider = fs.readFileSync(CYPHER_PROVIDER_FILE, "utf-8")
  const utils = fs.readFileSync(path.join(ROOT, "src/cypher-provider-utils.ts"), "utf-8")
   ...
   * cypher-provider-utils) sets it when httpClient is unavailable, and
  ...
  it("loadSessions sets pendingSessionRefresh when client is null", () => {
    ...
    expect(start, "loadSessions must exist in cypher-provider-utils").toBeGreaterThan(-1)
...
//
// Regression: PR #7473 moved CypherNotifications into MessageList. It calls
