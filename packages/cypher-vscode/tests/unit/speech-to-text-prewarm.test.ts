...
const SCRIPT = `
  ...
  const [config, setConfig] = createSignal({ disabled_providers: ["cypher"] })
  ...
  if (sent.length !== 0) fail("prewarmed without Cypher access")
  setAuth({ cypher: "api" })
  if (sent.length !== 0) fail("prewarmed while Cypher was disabled")
  setConfig({})
  if (sent.length !== 1 || sent[0]?.type !== "speechToTextPrewarm") {
    fail("did not prewarm after Cypher access became available")
  }
  setAuth({ cypher: "oauth" })
...
describe("speech-to-text prewarm", () => {
  it("starts only after Cypher speech access becomes available", () => {
    const attempts = 3
