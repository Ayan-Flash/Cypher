...
const { CypherActionProvider } = await import("../../src/services/code-actions/code-action-provider")

const provider = new CypherActionProvider()
  ...
  describe("provideCodeActions", () => {
    ...
    describe("non-empty range, no diagnostics", () => {
      it("returns Add, Explain, Improve actions", () => {
        ...
        expect(titles).toContain("Add to Cypher CLI")
        expect(titles).toContain("Explain with Cypher CLI")
        expect(titles).toContain("Improve with Cypher CLI")
      })

      it("does not include Fix action", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(0) as never)
        expect(result.map((a) => a.title)).not.toContain("Fix with Cypher CLI")
      })
      ...
      it("uses correct command IDs", () => {
        ...
        expect(commands).toContain("cypher-cli.new.addToContext")
        expect(commands).toContain("cypher-cli.new.explainCode")
        expect(commands).toContain("cypher-cli.new.improveCode")
      })
    ...
    describe("non-empty range, with diagnostics", () => {
      it("returns Add and Fix actions", () => {
        ...
        const titles = result.map((a) => a.title)
        expect(titles).toContain("Add to Cypher CLI")
        expect(titles).toContain("Fix with Cypher CLI")
      })
      ...
      it("does not include Explain or Improve actions", () => {
        ...
        const titles = result.map((a) => a.title)
        expect(titles).not.toContain("Explain with Cypher CLI")
        expect(titles).not.toContain("Improve with Cypher CLI")
      })
      ...
      it("Fix action is preferred", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(1) as never)
        const fix = result.find((a) => a.title === "Fix with Cypher CLI")
      ...
      it("uses correct Fix command ID", () => {
        const result = provider.provideCodeActions({} as never, makeRange(false) as never, makeContext(1) as never)
        const fix = result.find((a) => a.title === "Fix with Cypher CLI")
        expect(fix?.command?.command).toBe("cypher-cli.new.fixCode")
      })
    })
  })
})
