...
import type { Config } from "@cypher/sdk/v2/client"

// vscode mock is provided by the shared preload (tests/setup/vscode-mock.ts)
const { CypherProvider } = await import("../../src/CypherProvider")
...
describe("CypherProvider indexing refresh", () => {
  it("reloadAfterAuthChange fetches config first, then indexing status", async () => {
    const provider = new CypherProvider({} as never, {} as never)
  ...
  it("handleUpdateConfig no longer eagerly fetches indexing status", async () => {
    ...
    const provider = new CypherProvider({} as never, conn.service as never)
    const internal = provider as unknown as Internals
  ...
  it("fetchAndSendIndexingStatus uses current session directory header", async () => {
    const worktree = "/repo/.cypher/.cypher/worktrees/feature"
    ...
    try {
      const provider = new CypherProvider(
        {} as never,
        {
          getClient: () => ({}) as never,
      ...
      const auth = Buffer.from("cypher:secret").toString("base64")
      expect(headers.get("Authorization")).toBe(`Basic ${auth}`)
      expect(headers.get("x-cypher-directory")).toBe(worktree)
    } finally {
