...
} from "../../src/cypher-provider/agent-requirements"
import { AgentRequirementsController } from "../../src/cypher-provider/agent-requirements-controller"
...
describe("agent requirement helpers", () => {
  ...
  it("scopes requests to active workspace, session, and worktree directories", () => {
    const sessions = new Map([["session-1", "/repo/.cypher/worktrees/one"]])
    const base = {
      workspaceDirectory: root,
      workspaceDirectories: ["/workspace"],
      projectDirectory: root,
      sessionDirectories: sessions,
      worktreeDirectories: () => ["/repo/.cypher/worktrees/two"],
    }

    expect(requirementDirectory({ ...base, requested: "/repo" })).toBe(root)
    expect(requirementDirectory({ ...base, requested: "/repo/.cypher/worktrees/one", sessionID: "session-1" })).toBe(
      "/repo/.cypher/worktrees/one",
    )
    expect(requirementDirectory({ ...base, requested: "/repo/.cypher/worktrees/two" })).toBe("/repo/.cypher/worktrees/two")
