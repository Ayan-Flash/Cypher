...
import { mergeFileSearchResults } from "../../src/cypher-provider/file-search-results"
...
describe("mergeFileSearchResults", () => {
  ...
  it("preserves camel-case scoring for acronym-style queries", () => {
    const result = mergeFileSearchResults({
      query: "amprov",
      backend: [
        "packages/cypher-docs/pages/contributing/architecture/onboarding-improvements.md",
        "packages/cypher-vscode/src/agent-manager/AgentManagerProvider.ts",
      ],
      open: new Set(),
    })
    expect(result[0]).toBe("packages/cypher-vscode/src/agent-manager/AgentManagerProvider.ts")
  })
})
