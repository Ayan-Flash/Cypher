...
describe("parseServerPort", () => {
  it("parses port from standard CLI startup message", () => {
    expect(parseServerPort("cypher server listening on http://127.0.0.1:12345")).toBe(12345)
  })
  ...
  it("parses port when embedded in longer output", () => {
    const output = "[INFO] 2024-01-01 cypher server listening on http://127.0.0.1:54321\n[INFO] ready"
...
describe("cli tree-sitter resources", () => {
  it("resolves resources next to the VS Code bundled CLI", () => {
    const root = "/Users/test/.vscode/extensions/cypher.cypher-cli-7.2.50-darwin-arm64"
    const bin = `${root}/bin/cypher`
    ...
    expect(cypherSandboxWorkerForBinary(bin)).toBe(`${root}/bin/cypher-sandbox-mutation-worker.js`)
    expect(resolveTreeSitterEnv(root)).toEqual({ CYPHER_TREE_SITTER_WASM_DIR: `${root}/bin/tree-sitter` })
  })

  it("resolves resources next to a Windows packaged CLI", () => {
    const root = String.raw`C:\Users\test\.vscode\extensions\cypher.cypher-cli-7.2.50-win32-x64`
    const bin = String.raw`${root}\bin\cypher.exe`
    ...
    expect(cypherSandboxWorkerForBinary(bin)).toBe(String.raw`${root}\bin\cypher-sandbox-mutation-worker.js`)
    expect(resolveTreeSitterEnv(root)).toEqual({
      CYPHER_TREE_SITTER_WASM_DIR: String.raw`${root}\bin\tree-sitter`,
    })
  })

  it("copies the Cypher sandbox worker with the packaged CLI binary", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "cypher-vscode-sandbox-worker-"))
    try {
      const source = path.join(root, "dist", "@cypher", "cli-darwin-arm64", "bin", "cypher")
      const target = path.join(root, "extension", "bin", "cypher")
      await fs.mkdir(path.dirname(source), { recursive: true })
      await fs.mkdir(path.dirname(target), { recursive: true })
      await fs.writeFile(cypherSandboxWorkerForBinary(source), "worker")

      await copyCypherSandboxWorker(source, target)

      expect(await fs.readFile(cypherSandboxWorkerForBinary(target), "utf8")).toBe("worker")
    } finally {
  ...
  it("copies runtime and language WASMs with the packaged CLI binary", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "cypher-vscode-tree-sitter-"))
    try {
      const source = path.join(root, "dist", "@cypher", "cli-darwin-arm64", "bin", "cypher")
      const target = path.join(root, "extension", "bin", "cypher")
  ...
  it("copies the Linux sandbox helper and license resources", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "cypher-vscode-sandbox-"))
    try {
      const source = path.join(root, "dist", "bin", "cypher")
      const target = path.join(root, "extension", "bin", "cypher")
  ...
  it("removes stale sandbox resources when the source has none", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "cypher-vscode-sandbox-stale-"))
    try {
      const source = path.join(root, "dist", "bin", "cypher")
      const target = path.join(root, "extension", "bin", "cypher")
...
describe("toErrorMessage", () => {
  ...
  it("strips ANSI codes before matching Error:", () => {
    const ansiError = "\x1b[91m\x1b[1mError: \x1b[0mConfig file at /path/cypher.json is not valid JSON(C):"
    const result = toErrorMessage("startup failed", [ansiError])
    expect(result.userMessage).toBe("Config file at /path/cypher.json is not valid JSON(C):")
  })
  ...
  it("includes CLI path in userDetails when provided", () => {
    const result = toErrorMessage("startup failed", [], "/usr/local/bin/cypher")
    expect(result.userDetails).toContain("CLI path: /usr/local/bin/cypher")
  })
...
describe("server workspace helpers", () => {
  ...
  it("disables codebase indexing only when no workspace folder is open", () => {
    expect(resolveIndexingEnv(undefined)).toEqual({ CYPHER_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
    expect(resolveIndexingEnv([])).toEqual({ CYPHER_DISABLE_CODEBASE_INDEXING: "vscode-no-workspace" })
  ...
  it("uses the shared database for the managed backend while preserving the environment", () => {
    expect(resolveManagedServerEnv({ PATH: "/usr/bin", CYPHER_DISABLE_CHANNEL_DB: "false" })).toEqual({
      PATH: "/usr/bin",
