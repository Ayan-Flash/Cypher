 ...
 * 2. Every WebviewMessage type handled in CypherProvider has a corresponding member in the WebviewMessage union
...
const CYPHER_PROVIDER_UTILS_FILE = path.join(ROOT, "src/cypher-provider-utils.ts")
...
describe("CypherProvider message handler coverage", () => {
  it("all WebviewMessage switch cases in CypherProvider exist in WebviewMessage union", () => {
    const providerContent = readFile(CYPHER_PROVIDER_FILE)
    ...
    expect(
      unrecognized,
      `CypherProvider switch cases not found in any message type definition: ${unrecognized.join(", ")}`,
    ).toEqual([])
  })
})
...
describe("mapSSEEventToWebviewMessage output types", () => {
  it("all output types from mapSSEEventToWebviewMessage exist in ExtensionMessage", () => {
    const utilsContent = readFile(CYPHER_PROVIDER_UTILS_FILE)
    const messagesContent = readMessageTypeSources()
