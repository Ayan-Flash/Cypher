...
import { globalFiles, localFiles } from "../../src/cypher-provider/config-file"
import { openConfig } from "../../src/cypher-provider/open-config"
...
const labels = {
  ...
  sourceHomeCypher: "Home Cypher",
  sourceHomeCyphercode: "Home Cyphercode",
  sourceHomeOpencode: "Home Opencode",
  sourceProjectCypher: "Project Cypher",
  sourceProjectCyphercode: "Project Cyphercode",
...
async function temp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "cypher-config-"))
  dirs.push(dir)
  return dir
}
...
describe("config file discovery", () => {
  it("discovers global, env, legacy, and virtual config sources", async () => {
    ...
    process.env.CYPHER_CONFIG_DIR = extra
    process.env.CYPHER_CONFIG_CONTENT = "{}"
    await file(path.join(xdg, "cypher", "cypher.json"))
    await file(path.join(home, ".cypher", "opencode.json"))
    await file(path.join(home, ".opencode", "cypher.jsonc"))
    await file(envfile)
    ...
    expect(sources).toContain("sourceHomeCyphercode")
  ...
  it("marks project files unloaded when project config is disabled", async () => {
    ...
    await file(path.join(root, "cypher.json"))
    ...
    expect(list.find((item) => item.recommended)?.file).toBe(path.join(root, ".cypher", "cypher.jsonc"))
  })
})
...
describe("openConfig", () => {
  ...
  it("opens the only editable config without showing the picker", async () => {
    reset()
    const root = await temp()
    const cfg = path.join(root, ".cypher", "cypher.jsonc")
    await file(cfg)
