  ...
  it("keeps previous file results visible while the next search is pending", async () => {
    ...
    for (const handler of handlers) {
      handler({
        ...
        dir: "/repo",
        paths: ["packages/cypher-vscode/src/extension.ts"],
        items: [{ path: "packages/cypher-vscode/src/extension.ts", type: "opened-file" }],
      })
    }

    expect(mention.mentionResults()).toEqual([{ type: "opened-file", value: "packages/cypher-vscode/src/extension.ts" }])
