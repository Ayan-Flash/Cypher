...
describe("WorktreeStateManager", () => {
  ...
  beforeEach(() => {
    ...
    // Pre-create .cypher dir so fire-and-forget saves don't race on mkdir
    fs.mkdirSync(path.join(root, ".cypher"), { recursive: true })
    logs.length = 0
  ...
  describe("session CRUD", () => {
    ...
    it("drops obsolete session prefs while loading state", async () => {
      const file = path.join(root, ".cypher", "agent-manager.json")
      fs.writeFileSync(
        file,
        JSON.stringify({
          worktrees: {},
  ...
  describe("persistence", () => {
    ...
    it("creates .cypher directory if missing", async () => {
      ...
      expect(fs.existsSync(path.join(fresh, ".cypher", "agent-manager.json"))).toBe(true)
    })
    ...
    it("backs up a corrupt state file before recovery saves", async () => {
      ...
      const files = fs.readdirSync(path.join(root, ".cypher"))
      expect(result.status).toBe("failed")
  ...
  describe("tab order", () => {
    ...
    it("does not persist empty tab order", async () => {
      ...
      const content = fs.readFileSync(path.join(root, ".cypher", "agent-manager.json"), "utf-8")
  ...
  describe("validate", () => {
    ...
    it("resolves relative paths against root", async () => {
      const relative = ".cypher/worktrees/test-branch"
  ...
  describe("legacy .cypher migration", () => {
    it("migrates and loads state from .cypher when .cypher is absent", async () => {
      // Remove the .cypher dir created in beforeEach
      fs.rmSync(path.join(root, ".cypher"), { recursive: true, force: true })

      // Write state to legacy .cypher dir (migration will move it to .cypher)
      const legacyDir = path.join(root, ".cypher")
    ...
    it("skips migration when .cypher state already exists", async () => {
      // .cypher state already present — migration should skip agent-manager.json
      fs.writeFileSync(
        path.join(root, ".cypher", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-new": {
              branch: "new-branch",
    ...
    it("rewrites stale .cypher paths in worktree entries (unix)", async () => {
      fs.writeFileSync(
        path.join(root, ".cypher", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
            "wt-stale": {
              branch: "fix",
              path: "/repo/.cypher/worktrees/fix",
              parentBranch: "main",
              createdAt: new Date().toISOString(),
            },
          },
      ...
      expect(manager.getWorktrees()[0].path).toBe(`/repo/.cypher/worktrees/fix`)
    })

    it("rewrites stale .cypher paths with backslashes (windows)", async () => {
      fs.writeFileSync(
        path.join(root, ".cypher", "agent-manager.json"),
        JSON.stringify({
          worktrees: {
      ...
      expect(manager.getWorktrees()[0].path).toBe("C:\\.cypher\\worktrees\\fix")
    })
  })
})
