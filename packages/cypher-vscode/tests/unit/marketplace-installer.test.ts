...
const tmpDir = path.join(os.tmpdir(), `cypher-test-${Date.now()}`)

class TestPaths extends MarketplacePaths {
  override configPath(scope: "project" | "global", workspace?: string): string {
    if (scope === "global") return path.join(tmpDir, "global", "cypher.json")
    return path.join(tmpDir, "project", ".cypher", "cypher.json")
  }
