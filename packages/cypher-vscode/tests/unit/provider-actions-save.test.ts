...
describe("saveCustomProvider", () => {
  ...
  // Regression tests for https://github.com/Cypher-Org/cypher/issues/9186
  //
...
describe("fetchProviderData", () => {
  ...
  it("uses local Cypher auth status instead of profile availability", async () => {
    const client = {
      provider: {
        list: async () => ({
          data: {
            all: [{ id: "cypher", name: "Cypher Gateway", source: "custom", env: [], models: {} }],
            connected: ["cypher"],
            default: { cypher: "cypher-auto/frontier" },
          },
        }),
        auth: async () => ({ data: {} }),
      },
    ...
    expect(result.authStates).toEqual({ cypher: "oauth" })
  })

  it("does not infer Cypher speech access without stored Gateway auth", async () => {
    const client = {
      provider: {
        list: async () => ({
          data: {
            all: [{ id: "cypher", name: "Cypher Gateway", source: "config", key: "configured", env: [], models: {} }],
            connected: ["cypher"],
            default: { cypher: "cypher-auto/frontier" },
          },
        }),
