...
import type { Event, Session } from "@cypher/sdk/v2/client"
...
describe("CypherProvider follow-up sessions", () => {
  it("adopts pending follow-up sessions for single-session views", async () => {
    const service = connection()
    const provider = new CypherProvider({} as never, service as never)
  ...
  it("calls onFollowupAdopted listeners with session and directory", async () => {
    ...
    internal.pendingFollowup = { dir: "/repo/.cypher/worktrees/feat", time: Date.now() }
    service.emit(created({ id: "ses-wt", directory: "/repo/.cypher/worktrees/feat" }))
    await Promise.resolve()

    expect(adopted).toEqual([{ id: "ses-wt", dir: "/repo/.cypher/worktrees/feat" }])
  })
})
