...
import { mapSSEEventToWebviewMessage } from "../../src/cypher-provider-utils"
import { configFeatures } from "../../src/features"
import type { EventIndexingStatus, IndexingStatus } from "@cypher/sdk/v2/client"
...
describe("indexing feature detection", () => {
  it("enables indexing settings when the indexing plugin is present", () => {
    expect(configFeatures({ plugin: ["cypher-indexing"] }).indexing).toBe(true)
  })

  it("detects supported indexing plugin specifiers", () => {
    expect(configFeatures({ plugin: ["cypher-indexing"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["cypher-indexing@1.2.3"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["@cypher/cypher-indexing"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["@cypher/cypher-indexing@1.2.3"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["file:///tmp/.opencode/plugin/cypher-indexing.js"] }).indexing).toBe(true)
    expect(configFeatures({ plugin: ["file:///tmp/node_modules/@cypher/cypher-indexing/index.js"] }).indexing).toBe(
      true,
    )
  })

  it("ignores unrelated plugin lists", () => {
    expect(configFeatures({ plugin: ["@cypher/cypher-gateway"] }).indexing).toBe(false)
