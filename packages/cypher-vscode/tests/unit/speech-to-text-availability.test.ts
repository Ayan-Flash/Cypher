...
describe("speech-to-text availability", () => {
  it("shows speech input for stored Cypher credentials", () => {
    expect(canUseSpeechToText({}, { cypher: "oauth" })).toBe(true)
    expect(canUseSpeechToText({}, { cypher: "api" })).toBe(true)
  })

  it("hides speech input without usable Cypher credentials", () => {
    expect(canUseSpeechToText({}, {})).toBe(false)
    expect(canUseSpeechToText({}, { cypher: "wellknown" })).toBe(false)
  })

  it("honors enabled and disabled provider configuration", () => {
    expect(canUseSpeechToText({ disabled_providers: ["cypher"] }, { cypher: "oauth" })).toBe(false)
    expect(canUseSpeechToText({ enabled_providers: ["openai"] }, { cypher: "oauth" })).toBe(false)
    expect(canUseSpeechToText({ enabled_providers: ["cypher"] }, { cypher: "oauth" })).toBe(true)
  })
