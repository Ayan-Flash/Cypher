...
describe("buildFeedbackProperties — non-Cypher providers", () => {
...
describe("buildFeedbackProperties — Cypher Gateway", () => {
  it("includes sessionID, messageID, parentMessageID", () => {
    const props = buildFeedbackProperties({ ...baseInput, providerID: "cypher", next: "up" })
    expect(props).toEqual({
      providerID: "cypher",
      modelID: "claude-sonnet-4-5",
      rating: "up",
