...
import type { NotebookRequest } from "@cypher/sdk/v2/client"
import * as vscode from "vscode"
import { CypherConnectionService } from "../../src/services/cli-backend/connection-service"
...
function harness(context: NotebookBridgeContext, dirs = ["/repo"]) {
  ...
  const request = (value: NotebookRequest = read, directory = "/repo") =>
    handlers.event?.(
      { id: `event-${value.id}`, type: "cypher.notebook.requested", properties: value } as SSEPayload,
      directory,
    )
  const cancel = (id = read.id, directory = "/repo") =>
    handlers.event?.(
      {
        id: `cancel-${id}`,
        type: "cypher.notebook.cancelled",
...
describe("CypherConnectionService notebook directories", () => {
  it("tracks the workspace root, current request directory, and provider directories", async () => {
    ...
    const service = new CypherConnectionService({} as vscode.ExtensionContext)
