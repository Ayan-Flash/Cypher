...
describe("isModelValid", () => {
  ...
  it("rejects a disconnected non-cypher provider", () => {
    expect(isModelValid(providers, [], { providerID: "openai", modelID: "gpt-4o" })).toBe(false)
  })

  it("accepts cypher models when present in the catalog", () => {
    expect(isModelValid(providers, [], { providerID: "cypher", modelID: "cypher-auto/free" })).toBe(true)
  })
