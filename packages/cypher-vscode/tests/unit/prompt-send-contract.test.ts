 ...
 *
 * Protects against accidental removal during Cypher development.
 */
...
const CYPHERPROVIDER_FILE = path.join(ROOT, "src/CypherProvider.ts")
...
describe("CypherProvider pruneDeletedSession contract", () => {
  const source = readFile(CYPHERPROVIDER_FILE)
...
describe("CypherConnectionService pruneSession contract", () => {
  const source = readFile(CONNECTION_SERVICE_FILE)

  it("drops the deleted session from focused and opened Maps", () => {
    // CypherProvider's pruneDeletedSession calls connectionService.pruneSession.
