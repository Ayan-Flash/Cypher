...
const GLOBALS = "colorScheme:dark;theme:cypher-vscode;vscodeTheme:dark-modern"
const STORY_ID = "settings--indexing-provider-blur-race"
const CYPHER_STORY_ID = "settings--indexing-cypher-model-preset"
const CYPHER_LOADING_STORY_ID = "settings--indexing-cypher-catalog-loading"
...
test("Cypher exposes only supported embedding model presets", async ({ page }) => {
  await page.setViewportSize({ width: 420, height: 720 })
  await page.goto(storyUrl(CYPHER_STORY_ID), { waitUntil: "load" })
  await disableAnimations(page)
  await page.waitForSelector("#storybook-root *", { state: "attached" })

  await expect(page.getByText("Cypher model preset", { exact: true })).toBeVisible()
...
test("enabling Cypher before its catalog loads does not store an empty model", async ({ page }) => {
  const saved = page.getByTestId("indexing-cypher-loading-save")
  const cfg = async () => JSON.parse(((await saved.textContent()) ?? "{}").trim()) as Saved
  const verify = async () => {
    await expect.poll(async () => (await cfg()).provider).toBe("cypher")
    expect((await cfg()).model).toBeNull()
    expect((await cfg()).dimension).toBeNull()
  }

  await page.setViewportSize({ width: 420, height: 720 })
  await page.goto(storyUrl(CYPHER_LOADING_STORY_ID), { waitUntil: "load" })
  await disableAnimations(page)
  await page.waitForSelector("#storybook-root *", { state: "attached" })
  await page.getByRole("button", { name: "Local", exact: true }).click()
  await page
