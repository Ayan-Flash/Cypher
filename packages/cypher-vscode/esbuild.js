...
/**
 * Force all solid-js imports (from cypher-ui and the webview) to resolve to
...
const solidDedupePlugin = {
  name: "solid-dedupe",
  setup(build) {
    // Resolve these bare specifiers to the cypher-vscode-local copy
 ...
 * variants) to the Cypher implementation in `webview-ui/pierre-worker.ts`.
 *
 * The upstream module loads Pierre's Shiki worker via a Vite-only
 * `?worker&url` import that esbuild can't resolve. The Cypher replacement loads
...
/**
 * Resolve the synthetic `cypher-shiki-worker` entry point to Pierre's Shiki worker
...
const shikiWorkerEntryPlugin = {
  ...
  setup(build) {
    build.onResolve({ filter: /^cypher-shiki-worker$/ }, async () => {
    ...
    build.onLoad({ filter: /sprite\.svg$/ }, (args) => {
      ...
      return {
        contents: `
          ...
          const inject = () => {
            if (!document.getElementById("cypher-sprite")) {
...
function createShikiWorkerContext() {
  return esbuild.context({
    entryPoints: ["cypher-shiki-worker"],
    bundle: true,
    format: "iife",
    minify: production,
    sourcemap: !production,
...
async function main() {
  ...
  // Build CypherClaw webview (SolidJS, standalone chat panel)
  const cypherClawCtx = await createBrowserWebviewContext("webview-ui/cypherclaw/index.tsx", "dist/cypherclaw.js")
