...
export const ServerProvider: ParentComponent = (props) => {
  ...
  onMount(() => {
    const unsubscribe = vscode.onMessage((message: ExtensionMessage) => {
      switch (message.type) {
        case "ready":
          console.log("[Cypher New] Server ready:", message.serverInfo)
        ...
        case "connectionState":
          console.log("[Cypher New] Connection state changed:", message.state)
        ...
        case "error":
          console.error("[Cypher New] Server error:", message.message)
          setErrorMessage(message.message)
          setErrorDetails(message.message)
          break

        case "profileData":
          console.log("[Cypher New] Profile data:", message.data ? "received" : "null")
          setProfileData(message.data)
          break

        case "deviceAuthStarted":
          console.log("[Cypher New] Device auth started")
          setDeviceAuth({
            status: "pending",
            code: message.code,
        ...
        case "deviceAuthComplete":
          console.log("[Cypher New] Device auth complete")
        ...
        case "deviceAuthFailed":
          console.log("[Cypher New] Device auth failed:", message.error)
          setDeviceAuth({ status: "error", error: message.error })
          break

        case "deviceAuthCancelled":
          console.log("[Cypher New] Device auth cancelled")
    ...
    console.log("[Cypher New] Webview ready")
   ...
   * outside the Profile page — e.g. the Cypher Gateway card in the Providers
