...
import { CYPHER_AUTO, CYPHER_PROVIDER_ID, parseModelString } from "../../../src/shared/provider-model"
...
export const SessionProvider: ParentComponent = (props) => {
  ...
  const [pendingCypherModel, setPendingCypherModel] = createSignal<{
    modelID?: string
    agent?: string
    after: number
  } | null>(null)
  ...
  function selectCypherModel(modelID?: string, agent?: string) {
    if (!modelID && !agent) return
    setPendingCypherModel({ ...(modelID && { modelID }), ...(agent && { agent }), after: catalog() })
    if (modelID) vscode.postMessage({ type: "requestProviders" })
  }

  const unsubCypherModel = vscode.onMessage((message: ExtensionMessage) => {
    if (message.type === "providersLoaded") {
      setCatalog((value) => value + 1)
      return
    }
    if (message.type === "selectCypherModel") selectCypherModel(message.modelID, message.agent)
  })
  onCleanup(unsubCypherModel)

  createEffect(() => {
    const pending = pendingCypherModel()
    if (!pending || agents().length === 0 || (pending.modelID && catalog() <= pending.after)) return
    setPendingCypherModel(null)
    if (pending.modelID && !provider.providers()[CYPHER_PROVIDER_ID]?.models[pending.modelID]) {
      console.warn("[Cypher New] Ignoring unavailable Cypher catalog model:", pending.modelID)
      return
    }
    if (pending.agent && !agentNames().has(pending.agent)) {
      console.warn("[Cypher New] Ignoring unavailable Cypher agent:", pending.agent)
      return
    }
    if (pending.agent) selectAgent(pending.agent)
    if (pending.modelID) selectModel(CYPHER_PROVIDER_ID, pending.modelID)
  })
  ...
  function handleExtensionMessage(message: ExtensionMessage): void {
    ...
    switch (message.type) {
      ...
      case "cloudSessionImportFailed": {
        ...
        console.error("[Cypher New] Cloud session import failed:", message.error)
        break
      }
  ...
  function handlePartUpdated(
    ...
    if (!effectiveMessageID) {
      console.warn("[Cypher New] Part updated without messageID:", part.id, part.type)
      return
    }
  ...
  function sendMessage(
    ...
    if (!server.isConnected()) {
      console.warn("[Cypher New] Cannot send message: not connected")
      return
    }
  ...
  function sendCommand(
    ...
    if (!server.isConnected()) {
      console.warn("[Cypher New] Cannot send command: not connected")
      return
    }
  ...
  function abort() {
    ...
    if (!scope) {
      console.warn("[Cypher New] Cannot abort: no current or pending session")
      return
    }
  ...
  function compact() {
    if (!server.isConnected()) {
      console.warn("[Cypher New] Cannot compact: not connected")
      return
    }

    const sessionID = currentSessionID()
    if (!sessionID) {
      console.warn("[Cypher New] Cannot compact: no current session")
      return
    }
  ...
  function createSession() {
    if (!server.isConnected()) {
      console.warn("[Cypher New] Cannot create session: not connected")
      return
    }
  ...
  function loadSessions() {
    if (!server.isConnected()) {
      console.warn("[Cypher New] Cannot load sessions: not connected")
      return
    }
  ...
  function selectSession(id: string) {
    ...
    if (id.startsWith("cloud:")) {
      console.warn("[Cypher New] Cannot select cloud preview session via selectSession")
      return
    }
  ...
  function selectCloudSession(cloudSessionId: string) {
    if (!server.isConnected()) {
      console.warn("[Cypher New] Cannot select cloud session: not connected")
      return
    }
    const key = `cloud:${cloudSessionId}`
    agentDrafts.prune(draftSessionID())
  ...
  function deleteSession(id: string) {
    if (!server.isConnected()) {
      console.warn("[Cypher New] Cannot delete session: not connected")
      return
    }
  ...
  function renameSession(id: string, title: string) {
    if (!server.isConnected()) {
      console.warn("[Cypher New] Cannot rename session: not connected")
      return
    }
  ...
  function exportSessionTranscript(id: string) {
    if (!server.isConnected()) {
      console.warn("[Cypher New] Cannot export session transcript: not connected")
      return
    }
    if (id.startsWith("cloud:")) {
      console.warn("[Cypher New] Cannot export cloud session transcript")
      return
    }
