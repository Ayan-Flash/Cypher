...
export const CypherLogo = () => {
  const icons = (window as { ICONS_BASE_URI?: string }).ICONS_BASE_URI || ""
  const light =
    document.body.classList.contains("vscode-light") || document.body.classList.contains("vscode-high-contrast-light")
  const file = light ? "cypher-light.svg" : "cypher-dark.svg"

  return (
    <div class="cypher-logo">
      <img src={`${icons}/${file}`} alt="Cypher CLI" />
    </div>
  )
}
...
export const WelcomeEmptyState: Component<WelcomeEmptyStateProps> = (props) => {
  ...
  return (
    <div class="message-list-empty">
      <CypherLogo />
      <p class="cypher-about-text">{language.t("session.messages.welcome")}</p>
