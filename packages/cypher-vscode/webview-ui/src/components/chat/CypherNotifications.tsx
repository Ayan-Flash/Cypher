...
export const CypherNotifications: Component = () => {
  ...
  /**
   * Resolve suggestModelId to a cypher-provider model selection.
   * Only the cypher provider is supported — the model must be present in the
  ...
  const suggestedModel = createMemo(() => {
    ...
    const sel = { providerID: CYPHER_PROVIDER_ID, modelID: id }
  ...
  return (
    <Show when={total() > 0}>
      <div class="cypher-notifications">
        <div class="cypher-notifications-card">
          <div class="cypher-notifications-header">
            <span class="cypher-notifications-title">{current()?.title}</span>
            <Show when={total() > 1}>
              <span class="cypher-notifications-nav-count">
                {safeIndex() + 1} / {total()}
              </span>
            </Show>
          </div>
          <p class="cypher-notifications-message">{current()?.message}</p>
          <div class="cypher-notifications-footer">
            <div class="cypher-notifications-cta-group">
              <Show when={canSwitchModel()}>
                <button class="cypher-notifications-action-btn" onClick={handleTryModel}>
                  {suggestedName()
              ...
              <Show when={current()?.action}>
                {(action) => (
                  <button class="cypher-notifications-action-btn" onClick={() => handleAction(action().actionURL)}>
                    {action().actionText}
                  </button>
                )}
              </Show>
            </div>
            <div class="cypher-notifications-next-group">
              <Show when={safeIndex() > 0}>
                <button class="cypher-notifications-back-link" onClick={() => setIndex(safeIndex() - 1)}>
                  {language.t("common.goBack")}
                </button>
              </Show>
              <button class="cypher-notifications-primary-btn" onClick={handleNext}>
