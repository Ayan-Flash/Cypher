...
import { Collapsible } from "@cypher/cypher-ui/collapsible"
...
import { disabledProviderOptions, providersWithCypherFallback, visibleConnectedIds } from "./provider-visibility"
import { isCustomProviderPackage, CYPHER_PROVIDER_ID } from "../../../../src/shared/provider-model"
...
const ProvidersTab: Component = () => {
  ...
  const popularProviders = createMemo(() => {
    ...
    return sortProviders(
      all.filter(
        (item) =>
          item.id !== CYPHER_PROVIDER_ID && isPopularProvider(item) && !connected.has(item.id) && !disabled.has(item.id),
      ),
    )
  })
  ...
  const providers = createMemo(() => providersWithCypherFallback(provider.providers()))
  ...
  function connectProvider(item: Provider) {
    if (item.id === CYPHER_PROVIDER_ID) {
      // Route Cypher Gateway sign-in through the Profile view so the user sees
  ...
  return (
    <div>
      <Show when={!disabledIds().has(CYPHER_PROVIDER_ID)}>
        {/* Cypher Gateway — always at the top, not editable */}
        <Card>
          <div
            style={{
              display: "flex",
          ...
          >
            <ProviderIcon id={providerIcon(CYPHER_PROVIDER_ID)} width={20} height={20} />
            <span
              style={{
      ...
      {/* Connected providers (excluding Cypher) */}
