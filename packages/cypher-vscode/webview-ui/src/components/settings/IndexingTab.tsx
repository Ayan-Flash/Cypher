...
import { Card } from "@cypher/cypher-ui/card"
import { DEFAULT_VECTOR_STORE } from "@cypher/cypher-indexing/config"
import { formatCypherEmbeddingModelLabel, getCypherEmbeddingModel } from "@cypher/cypher-indexing/embedding-models"
import { Select } from "@cypher/cypher-ui/select"
import { Switch } from "@cypher/cypher-ui/switch"
import { TextField } from "@cypher/cypher-ui/text-field"
import { useConfig } from "../../context/config"
import { formatIndexingLabel, useIndexing } from "../../context/indexing"
import { useCypherEmbeddingModels } from "../../context/cypher-embedding-models"
...
const IndexingTab: Component = () => {
  ...
  const embeds = useCypherEmbeddingModels()
  ...
  const cypherDefault = () =>
    getCypherEmbeddingModel(embeds.catalog().defaultModel, embeds.catalog())?.id ?? embeds.catalog().defaultModel
  const cypherModels = createMemo(() =>
    embeds.catalog().models.map((model) => ({
      value: model.id,
      label: formatCypherEmbeddingModelLabel(model),
    })),
  )
  const knownCypherModel = (model: string | null | undefined) =>
    getCypherEmbeddingModel(model ?? undefined, embeds.catalog())?.id
  const cypherValue = () => knownCypherModel(cfg().model) ?? cypherDefault()
  const cypherAvailable = () => !!server.profileData() || provider.authStates()[CYPHER_PROVIDER_ID] !== undefined
  const selectedProvider = () => cfg().provider ?? (cypherAvailable() ? "cypher" : undefined)
  const staleCypherModel = () => selectedProvider() === "cypher" && !!cfg().model && !knownCypherModel(cfg().model)
  const providers = createMemo(() =>
    allProviders.filter((item) => item.value !== "cypher" || cypherAvailable() || selectedProvider() === "cypher"),
  )
  const fields = createMemo(() => providerFields(selectedProvider()))

  const saveProvider = (next: ProviderId | undefined) => {
    if (next === "cypher") {
      const model = knownCypherModel(cfg().model) ?? (cypherDefault() || null)
      updateIndexing({
        provider: next,
        model,
        dimension: null,
  ...
  const saveEnabled = (enabled: boolean) => {
    if (enabled && !cfg().provider && cypherAvailable()) {
      updateIndexing({
        enabled,
        provider: "cypher",
        model: knownCypherModel(cfg().model) ?? (cypherDefault() || null),
        dimension: null,
      })
      return
    }
  ...
  const saveModel = (value: string) => {
    if (selectedProvider() === "cypher") return
  ...
  const content = (_scope: IndexingScope) => (
    <div style={{ display: "flex", "flex-direction": "column", gap: "16px" }}>
      ...
      <Card>
        ...
        <Show when={selectedProvider() === "cypher"}>
          <Show when={cypherModels().length > 0}>
            <SettingsRow
              title={language.t("settings.indexing.cypherModel.title")}
              description={description(language.t("settings.indexing.cypherModel.description"), [["model"]])}
              tag={() => tag(scope(), [["model"]])}
            >
              <Select
                options={cypherModels()}
                current={cypherModels().find((item) => item.value === cypherValue())}
                value={(item) => item.value}
                label={(item) => item.label}
                onSelect={(item) => updateIndexing({ model: item?.value ?? cypherDefault(), dimension: null })}
                variant="secondary"
                size="small"
                triggerVariant="settings"
        ...
        <SettingsRow
          ...
          tag={() => (selectedProvider() === "cypher" ? undefined : tag(scope(), [["dimension"]]))}
          last={!selectedProvider() || (fields().length === 0 && !(selectedProvider() === "cypher" && !cypherAvailable()))}
        >
          <TextField
            value={
              staleCypherModel() || cfg().dimension === undefined || cfg().dimension === null
                ? ""
                : String(cfg().dimension)
            }
            placeholder={
              selectedProvider() === "cypher" ? "Provided by Cypher" : language.t("settings.indexing.dimension.placeholder")
            }
            disabled={selectedProvider() === "cypher"}
            onChange={(value) => saveNumber("dimension", value, { integer: true, min: 1 })}
          />
        </SettingsRow>
        <Show when={selectedProvider() === "cypher" && !cypherAvailable()}>
          <SettingsRow
            title={language.t("settings.indexing.cypherSignIn.title")}
            description={language.t("settings.indexing.cypherSignIn.description")}
            last
          >
            <span />
          </SettingsRow>
