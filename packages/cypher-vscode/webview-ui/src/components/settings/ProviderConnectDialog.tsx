...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@cypher/sdk/v2/client"
    ...
    return (
      ...
      >
              ...
              <Match when={prompt.type === "select"}>
                <div style={{ display: "flex", "flex-direction": "column", gap: "4px" }}>
                  ...
                  <Show when={state.field === prompt.key && state.error}>
                    <span style={{ "font-size": "var(--cypher-font-size-12)", color: "var(--vscode-errorForeground)" }}>
                      {state.error}
                    </span>
                  </Show>
                </div>
        ...
        <Show when={state.error && !state.field}>
          <div style={{ color: "var(--vscode-errorForeground)", "font-size": "var(--cypher-font-size-13)" }}>
            {state.error}
          </div>
        </Show>
        <div class="dialog-confirm-actions provider-connect-actions">
          <div class="provider-connect-byok">
            {language.t("provider.connect.cypherGateway.byok.prefix")}
            <a
              href="https://blog.cypher.ai/p/cypher-gateway-now-supports-byok-20-providers"
              onClick={(e) => {
                e.preventDefault()
                openExternal("https://blog.cypher.ai/p/cypher-gateway-now-supports-byok-20-providers")
              }}
              class="provider-connect-byok-link"
            >
              {language.t("provider.connect.cypherGateway.byok.link")}
            </a>
            {language.t("provider.connect.cypherGateway.byok.suffix")}
          </div>
