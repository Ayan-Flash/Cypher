...
const MARKETPLACE_DOCS = "https://cypher.ai/docs/customize/marketplace"
const MCP_DOCS = "https://cypher.ai/docs/automate/mcp/what-is-mcp"
...
export const InstallModal = (props: Props) => {
  ...
  const destination = (target = scope().value) => {
    const base = target === "project" ? ".cypher" : "~/.config/cypher"
    if (props.item.type === "mcp") return `${base}/cypher.json`
    if (props.item.type === "agent") return `${base}/agents/${props.item.id}.md`
    if (target === "project") return `.cypher/skills/${props.item.id}/`
    return `~/.cypher/skills/${props.item.id}/`
  }
