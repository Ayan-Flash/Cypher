...
import {
  CYPHER_PROVIDER_ID as CYPHER_GATEWAY_ID,
  PROVIDER_PRIORITY as PROVIDER_ORDER,
  providerOrderIndex,
} from "../../../../src/shared/provider-model"

export { CYPHER_GATEWAY_ID, PROVIDER_ORDER }

export const CYPHER_AUTO_SMALL_IDS = new Set(["cypher-auto/small", "auto-small"])
export const CYPHER_AUTO_EFFICIENT_ID = "cypher-auto/efficient"
...
export function isAuto(model: Pick<EnrichedModel, "providerID" | "id">): boolean {
  return (
    model.providerID === CYPHER_GATEWAY_ID && (model.id.startsWith("cypher-auto/") || CYPHER_AUTO_SMALL_IDS.has(model.id))
  )
}

export function isAutoEfficient(model: Pick<EnrichedModel, "providerID" | "id">): boolean {
  return model.providerID === CYPHER_GATEWAY_ID && model.id === CYPHER_AUTO_EFFICIENT_ID
}
...
export function isSmall(model: Pick<EnrichedModel, "providerID" | "id">): boolean {
  return model.providerID === CYPHER_GATEWAY_ID && CYPHER_AUTO_SMALL_IDS.has(model.id)
}
...
// elsewhere, so preserve bare trailing "Free" words (e.g. "Cypher Auto Free").
...
export function stripSubProviderPrefix(name: string): string {
  ...
  if (prefix.toLowerCase() === CYPHER_GATEWAY_ID) return name
...
export function buildTriggerLabel(
  ...
  if (resolvedName) {
    if (providerID === CYPHER_GATEWAY_ID) return stripSubProviderPrefix(resolvedName)
    if (providerName) return `${providerName} / ${resolvedName}`
    return resolvedName
  }
  if (raw?.providerID && raw?.modelID) {
    return raw.providerID === CYPHER_GATEWAY_ID ? raw.modelID : `${raw.providerID} / ${raw.modelID}`
  }
