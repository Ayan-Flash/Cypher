...
const STORAGE_KEY = "cypher.prompt-history.v1"
...
function load(): string[] {
  try {
    ...
    console.warn("[Cypher New] prompt history load failed", err)
    return []
  }
}
...
function save(items: string[]) {
  try {
    ...
    console.warn("[Cypher New] prompt history save failed", err)
  }
}
