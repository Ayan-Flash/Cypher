...
export function convertToMentionPath(path: string, cwd: string): string {
  ...
  try {
    ...
    console.error("[Cypher New] Failed to decode dropped URI:", err, cleaned)
  }
 ...
 */
export const CYPHER_FILE_PATH_MIME = "application/x-cypher-file-path"

/**
 * Extract file paths from a drop's DataTransfer.
 * Checks (in order):
 * 1. Internal relative-path drag (application/x-cypher-file-path)
...
export function extractDropPaths(dt: DataTransfer): string[] | null {
  ...
  const cypher = dt.getData(CYPHER_FILE_PATH_MIME)
  if (cypher) {
    const paths = cypher.split(/\r?\n/).filter((line) => line.trim() !== "")
