...
import { DataProvider } from "@cypher/cypher-ui/context/data"
...
import type { Message as SDKMessage, Part as SDKPart } from "@cypher/sdk/v2"
...
export const DataBridge: Component<{ children: any }> = (props) => {
  ...
  return (
    <DataProvider
      data={data}
      directory={directory()}
      // @ts-expect-error — onPermissionRespond/onQuestion* are extension-specific props not yet in cypher-ui's DataProvider types
...
export const MermaidDownloadBridge: Component = () => {
  ...
  onMount(() => {
    ...
    window.addEventListener("cypher:save-image", save)
    onCleanup(() => {
      window.removeEventListener("cypher:save-image", save)
    })
  })
...
const AppContent: Component = () => {
  ...
  const handleCypherModel = (message: { type?: string }) => {
    if (message.type === "selectCypherModel") setCurrentView("newTask")
  }
  ...
  onMount(() => {
    const handler = (event: MessageEvent) => {
      ...
      if (message?.type === "action" && message.action) {
        console.log("[Cypher New] App: 🎬 action:", message.action)
        handleViewAction(message.action)
      }
      if (message?.type === "navigate" && message.view && VALID_VIEWS.has(message.view)) {
        console.log("[Cypher New] App: 🧭 navigate:", message.view, message.tab ? `tab=${message.tab}` : "")
      ...
      if (message?.type === "openCloudSession" && message.sessionId) {
        console.log("[Cypher New] App: ☁️ openCloudSession:", message.sessionId)
        session.selectCloudSession(message.sessionId)
        setCurrentView("newTask")
      }
      handleCypherModel(message)
      handleForked(message)
      if (message?.type === "viewSubAgentSession" && message.sessionID) {
        console.log("[Cypher New] App: 🔍 viewSubAgentSession:", message.sessionID)
      ...
      if (message?.type === "migrationState") {
        console.log("[Cypher New] App: 🔄 migrationState:", message.needed)
