...
import { CypherEmbeddingModelsContext } from "../context/cypher-embedding-models"
...
export const ModelsSpeechToText: Story = {
  ...
  render: () => (
    <StoryProviders cypherAuth config={{ experimental: { speech_to_text_model: "google/chirp-3" } } as any}>
      <div style={{ "max-height": "700px", overflow: "auto" }}>
        <ModelsTab />
      </div>
    </StoryProviders>
...
export const IndexingCypherModelPreset: Story = {
  name: "IndexingTab - Cypher stale custom model fallback",
  render: () => {
    const cfg: Config = {
      indexing: {
        provider: "cypher",
    ...
    return (
      <StoryProviders config={cfg}>
        <CypherEmbeddingModelsContext.Provider value={{ catalog: () => catalog }}>
          <div style={{ "max-height": "700px", overflow: "auto" }}>
            <IndexingTab />
          </div>
        </CypherEmbeddingModelsContext.Provider>
      </StoryProviders>
    )
  },
}

export const IndexingCypherCatalogLoading: Story = {
  name: "IndexingTab - Cypher catalog loading",
  render: () => {
    const [saved, setSaved] = createSignal<Record<string, unknown>>({})
    const cfg: Config = {
      indexing: {},
    ...
    return (
      <>
        ...
        <pre data-testid="indexing-cypher-loading-save">{JSON.stringify(saved(), null, 2)}</pre>
      </>
    )
  },
}
