...
import { DataProvider, type OpenDiffFn, type OpenFileFn } from "@cypher/cypher-ui/context/data"
import { DiffComponentProvider } from "@cypher/cypher-ui/context/diff"
import { CodeComponentProvider } from "@cypher/cypher-ui/context/code"
import { FileComponentProvider } from "@cypher/cypher-ui/context/file"
import { DialogProvider } from "@cypher/cypher-ui/context/dialog"
import { MarkedProvider } from "@cypher/cypher-ui/context/marked"
import { I18nProvider } from "@cypher/cypher-ui/context"
import { Diff } from "@cypher/cypher-ui/diff"
import { Code } from "@cypher/cypher-ui/code"
import { File } from "@cypher/cypher-ui/file"
...
import { CypherEmbeddingModelsProvider } from "../context/cypher-embedding-models"
...
const dict: Record<string, string> = { ...appEn, ...amEn, ...uiEn, ...cypherEn }
...
// ---------------------------------------------------------------------------
// Mock providers — pre-loaded Cypher Gateway model for stories
// ---------------------------------------------------------------------------
...
const MockProviderProvider: ParentComponent<{ cypherAuth?: boolean }> = (props) => {
  const value = {
    providers: () => MOCK_PROVIDERS as any,
    connected: () => ["cypher"],
    defaults: () => ({}),
    defaultSelection: () => ({ providerID: "cypher", modelID: "anthropic/claude-sonnet-4-6" }),
    models: () => MOCK_MODELS,
    findModel: (sel: any) => _findModel(MOCK_MODELS, sel),
    authMethods: () => ({}),
    authStates: () => (props.cypherAuth ? { cypher: "oauth" } : {}) as Record<string, ProviderAuthState>,
    isModelValid: () => true,
  }
...
function mockNotificationsValue(items: CyphercodeNotification[] = []) {
  return {
    notifications: () => items,
...
export function mockSessionValue(overrides?: {
  ...
  return {
    ...
    selected: () => ({ providerID: "cypher", modelID: "anthropic/claude-sonnet-4-6" }),
    selectModel: noop,
    ...
    getSessionModel: () => ({ providerID: "cypher", modelID: "anthropic/claude-sonnet-4-6" }),
...
interface StoryProvidersProps {
  ...
  questions?: QuestionRequest[]
  suggestions?: SuggestionRequest[]
  notifications?: CyphercodeNotification[]
    ...
    <VSCodeProvider>
      <ServerProvider>
        <FeedbackProvider>
          ...
          >
            <DisplayProvider>
              <MockProviderProvider cypherAuth={props.cypherAuth}>
                <DialogProvider>
