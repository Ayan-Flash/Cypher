...
export function ClawProvider(props: { children: JSX.Element }) {
  ...
  const applyState = (s: CypherClawState) => {
    setPhase(s.phase)
    setLocale(s.locale)
    if (s.phase === "ready") {
      setStatus(s.status)
  ...
  const handleConversationMessage = (msg: CypherClawOutMessage, active: string | null): boolean => {
    switch (msg.type) {
      case "cypherclaw.messages":
        if (msg.conversationId === active) {
          setMessages(msg.messages)
          setHasMoreMessages(msg.hasMore)
        }
        return true
      case "cypherclaw.messageOptimistic":
        if (msg.conversationId === active) {
          setMessages((prev) => (prev.some((m) => m.id === msg.message.id) ? prev : [...prev, msg.message]))
        }
        return true
      case "cypherclaw.messageReplaced":
        if (msg.conversationId === active) {
  ...
  onMount(() => {
    vscode.postMessage({ type: "cypherclaw.ready" })
  })
  ...
  const ctx: ClawCtx = {
    ...
    locale,
    error,
    retry: () => vscode.postMessage({ type: "cypherclaw.ready" }),
    ...
    selectConversation: (conversationId) => vscode.postMessage({ type: "cypherclaw.selectConversation", conversationId }),
    createConversation: (title) => vscode.postMessage({ type: "cypherclaw.createConversation", title }),
    renameConversation: (conversationId, title) =>
      vscode.postMessage({ type: "cypherclaw.renameConversation", conversationId, title }),
    leaveConversation: (conversationId) => vscode.postMessage({ type: "cypherclaw.leaveConversation", conversationId }),
    loadMoreConversations: () => vscode.postMessage({ type: "cypherclaw.loadMoreConversations" }),
    ...
    editMessage: (conversationId, messageId, content) =>
      vscode.postMessage({ type: "cypherclaw.editMessage", conversationId, messageId, content }),
    deleteMessage: (conversationId, messageId) =>
      vscode.postMessage({ type: "cypherclaw.deleteMessage", conversationId, messageId }),
    loadMoreMessages: (conversationId, before) =>
      vscode.postMessage({ type: "cypherclaw.loadMoreMessages", conversationId, before }),

    addReaction: (conversationId, messageId, emoji) =>
      vscode.postMessage({ type: "cypherclaw.addReaction", conversationId, messageId, emoji }),
    removeReaction: (conversationId, messageId, emoji) =>
      vscode.postMessage({ type: "cypherclaw.removeReaction", conversationId, messageId, emoji }),

    executeAction: (conversationId, messageId, groupId, value) =>
      vscode.postMessage({ type: "cypherclaw.executeAction", conversationId, messageId, groupId, value }),

    sendTyping: (conversationId) => vscode.postMessage({ type: "cypherclaw.sendTyping", conversationId }),
    sendTypingStop: (conversationId) => vscode.postMessage({ type: "cypherclaw.sendTypingStop", conversationId }),
    ...
    botStatus,
    conversationStatus,
    markConversationRead: (conversationId) => vscode.postMessage({ type: "cypherclaw.markRead", conversationId }),

    openExternal: (url) => vscode.postMessage({ type: "cypherclaw.openExternal", url }),
  }
