// CypherClaw active-conversation view — message list + composer.
// Mirrors cloud/apps/web/src/app/(app)/claw/cypher-chat/components/MessageArea.tsx

import { For, Show, createEffect, createMemo, createSignal, on, onCleanup } from "solid-js"
import { Button } from "@cypher/cypher-ui/button"
import { Spinner } from "@cypher/cypher-ui/spinner"
import { createAutoScroll } from "@cypher/cypher-ui/hooks"
...
export function MessageArea() {
  ...
  //
  // Uses @cypher/cypher-ui's createAutoScroll (the same one MessageList.tsx
  ...
  const sendDisabledReason = createMemo(() => {
    ...
    const st = botDisplay().state
    if (st === "unknown") return t("cypherClaw.chat.waitingBotStatus")
    return t("cypherClaw.chat.botOffline")
  })

  // Render the typing banner with friendly names. Bot members come in as
  // `bot:cypherclaw:{sandboxId}` — the bot is the only non-self member in
  // 1:1 chats, so we resolve bot ids to the user-configured `botName`
  // (falls back to the default "CypherClaw" label) and render any human
  // collaborators by their raw memberId.
  const typingNames = createMemo(() => {
    const activeId = claw.activeConversationId()
    if (!activeId) return []
    const assistant = claw.assistantName() ?? t("cypherClaw.message.bot")
  ...
  return (
    <Show
      ...
      fallback={
        <div class="cypherclaw-empty-area">
          <p>{t("cypherClaw.conversations.selectOne")}</p>
        </div>
      }
    >
      <div class="cypherclaw-area">
        {/* Messages */}
        <div class="cypherclaw-area-messages-wrap">
          <div
            class="cypherclaw-area-messages"
            ref={(el) => {
              scrollEl = el
              auto.scrollRef(el)
            }}
          ...
          >
            <div ref={auto.contentRef}>
              <Show when={claw.messages().length === 0}>
                <div class="cypherclaw-empty">
                  {claw.assistantName()
                    ? t("cypherClaw.chat.emptyWithBot").replace("{bot}", claw.assistantName()!)
                    : t("cypherClaw.chat.empty")}
                </div>
              </Show>
        ...
        <Show when={typingNames().length > 0}>
          <div class="cypherclaw-typing">
            <Spinner />
            <span>
              {typingNames().length === 1
                ? t("cypherClaw.typing.one").replace("{name}", typingNames()[0])
                : t("cypherClaw.typing.many").replace("{count}", String(typingNames().length))}
            </span>
          </div>
        </Show>
        ...
        <Show when={replyingTo()}>
          {(r) => (
            <div class="cypherclaw-reply-preview">
              <span class="cypherclaw-reply-preview-label">{t("cypherClaw.message.replyTo")}</span>
              <span class="cypherclaw-reply-preview-text">{replyText(r())}</span>
              <button
                type="button"
                class="cypherclaw-iconbtn-sm"
                onClick={() => setReplyingTo(null)}
                aria-label={t("cypherClaw.message.cancelReply")}
              >
                ×
              </button>
            </div>
        ...
        <div class="cypherclaw-input-wrap">
          <textarea
            ref={input}
            class="cypherclaw-input"
            placeholder={sendDisabledReason() ?? t("cypherClaw.chat.placeholder")}
            disabled={!canSend()}
            value={text()}
            onInput={onInput}
            onKeyDown={onKeyDown}
            rows={1}
            aria-label={t("cypherClaw.chat.placeholder")}
          />
          <Button variant="primary" disabled={!canSend() || !text().trim()} onClick={submit}>
            {t("cypherClaw.chat.send")}
          </Button>
        </div>
      </div>
    </Show>
