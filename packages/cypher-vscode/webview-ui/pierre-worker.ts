// Cypher-specific replacement for the shared `@opencode-ai/ui/pierre/worker`
...
import { LINE_DIFF_TYPE } from "@cypher/cypher-ui/pierre"
import { WorkerPoolManager } from "@pierre/diffs/worker"
import { ensureCypherDiffTheme, CYPHER_DIFF_THEME } from "@opencode-ai/ui/pierre/cypher-diff-theme"

// Register the "Cypher" theme before any pool initializes. resolveThemes([theme])
// runs on the main thread during initialize() and throws "resolveTheme: No valid
// loader for Cypher" if the theme name was never registered. Registering here makes
...
function uri(): string | undefined {
  ...
  return (window as { CYPHER_SHIKI_WORKER_URI?: string }).CYPHER_SHIKI_WORKER_URI
}

export function workerFactory(): Worker {
  const url = uri()
  if (!url) throw new Error("CYPHER_SHIKI_WORKER_URI is not set")
...
function createPool() {
  const pool = new WorkerPoolManager(
    ...
    { theme: CYPHER_DIFF_THEME, lineDiffType: LINE_DIFF_TYPE, preferredHighlighter: ENGINE },
  )
  void pool.initialize().catch((err) => console.warn("[Cypher New] Failed to initialize Pierre worker pool", err))
  return pool
}
