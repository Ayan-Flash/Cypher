      ...
      <Match when={props.message.role === "user" && props.message}>
        {(userMessage) => (
          <UserMessageDisplay
            ...
            queued={props.queued} // cypher_change
          />
        )}
      </Match>
            ...
            {(file) => {
              ...
              return (
                <div
                  ...
                  data-queued={props.queued ? "" : undefined} // cypher_change
...
PART_MAPPING["text"] = function TextPartDisplay(props) {
  ...
  const isLastTextPart = createMemo(() => {
    const last = (data.store.part?.[props.message.id] ?? [])
      .filter((item): item is TextPart => item?.type === "text" && !!item.text?.trim() && !item.synthetic) // cypher_change
      .at(-1)
    return last?.id === part().id
  })
  const showCopy = createMemo(() => {
    if (part().synthetic) return false // cypher_change
  ...
  return (
    <Show when={text() && showSyntheticPart() /* cypher_change */}>
