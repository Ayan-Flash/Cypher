...
import { Icon, type IconProps } from "./icon" // cypher_change: added Icon
...
export interface BasicToolProps {
  ...
  forceOpen?: boolean
  defer?: boolean
  retainDetails?: boolean // cypher_change
  hasDetails?: boolean // cypher_change
  locked?: boolean
  animated?: boolean
  allowPendingToggle?: boolean // cypher_change
...
export function BasicTool(props: BasicToolProps) {
  ...
  const hasDetails = () => props.hasDetails ?? !!hasChildren() // cypher_change
  ...
  createEffect(
    on(
      ...
      (value) => {
        ...
        if (!value) {
          cancel()
          if (!props.retainDetails) setState("ready", false) // cypher_change
          return
        }
  ...
  const handleOpenChange = (value: boolean) => {
    if (pending() && !props.allowPendingToggle) return // cypher_change
    if (props.hideDetails) return // cypher_change
    if (props.locked && !value) return
    setOpen(value)
    props.onOpenChange?.(value) // cypher_change
  }
