...
import { deferredHighlight, fnv1a } from "../context/marked" // cypher_change
...
import { tryFastRender } from "../cypher/markdown-fast-path" // cypher_change
import { hasMermaid, preserveMermaid, renderMermaid, type MermaidLabels } from "../cypher/markdown-mermaid" // cypher_change
import { preserveStreamingHighlight } from "../cypher/markdown-stream-highlight" // cypher_change
import { createIncrementalMarkdown, type MarkdownBlock } from "../cypher/markdown-incremental-dom" // cypher_change
...
type Rendered = { content: string; blocks: MarkdownBlock[] } // cypher_change
...
export function Markdown(
  ...
  const [html] = createResource(
    ...
    async (src): Promise<Rendered> => {
      // cypher_change end
      if (isServer) return { content: fallback(src.text), blocks: [] } // cypher_change
      if (!src.text) return { content: "", blocks: [] } // cypher_change

      const base = src.key ?? checksum(src.text)
      return Promise.all(
        stream(src.text, src.streaming).map(async (block, index) => {
          const hash = checksum(block.raw) ?? "" // cypher_change
          ...
          if (key && hash) {
            ...
            if (cached && cached.hash === hash) {
              touch(key, cached)
              return { key: `${base}:${index}`, hash, html: cached.html, mode: block.mode } // cypher_change
            }
          }
          ...
          return { key: `${base}:${index}`, hash, html: safe, mode: block.mode } // cypher_change
        }),
      )
        .then((blocks) => ({ content: blocks.map((block) => block.html).join(""), blocks })) // cypher_change
        .catch(() => ({ content: fallback(src.text), blocks: [] })) // cypher_change
    },
    { initialValue: { content: fallback(local.text), blocks: [] } }, // cypher_change
  )

  let copyCleanup: (() => void) | undefined
  // cypher_change start: generation counter prevents stale deferredHighlight
  ...
  // cypher_change start: Mermaid diagram rendering
  const mermaidState = { gen: 0, signal: { aborted: false } }
  // cypher_change end

  // cypher_change start: rAF-coalesced morphdom render.
  ...
  createEffect(() => {
    ...
    const rendered = html.latest ?? html() ?? { content: "", blocks: [] } // cypher_change
    const content = local.text ? rendered.content : "" // cypher_change
    if (!container) return
    if (isServer) return

    if (!content) {
      // cypher_change start: cancel any in-flight coalesced render so a
      ...
      // cypher_change end
      incremental.reset() // cypher_change
      container.innerHTML = ""
      // cypher_change start: Mermaid diagram rendering
    ...
    if (incremental.render(local.streaming ?? false, container, rendered.blocks, labels, mermaid)) return // cypher_change
    incremental.reset() // cypher_change

    // cypher_change start: queue the latest content for a single rAF tick.
    ...
    pendingFrame = requestAnimationFrame(() => {
      ...
      // cypher_change start: morphdom guard for highlighted blocks (issue #6221)
      ...
      morphdom(container, temp, {
        ...
        onBeforeElUpdated: (fromEl, toEl) => {
          ...
          // cypher_change start: preserve rendered Mermaid diagrams across
          ...
          if (
            ...
            if (preserveStreamingHighlight(fromEl, toEl, local.streaming ?? false)) return false // cypher_change
            // Source changed during streaming — fall through so morphdom replaces // cypher_change
      ...
      kickMermaid(container, local.streaming ?? false, mermaid) // cypher_change
      kickHighlight(container, nextLabels)
    })
    // cypher_change end
  })

  // cypher_change start: progressive Shiki highlighting (issue #6221, PR #7102).
  ...
  onCleanup(() => {
    // cypher_change: cancel any in-flight deferredHighlight pass so its
    ...
    mermaidState.gen++
    // cypher_change end
    // cypher_change: cancel any queued rAF parse so it doesn't touch the
