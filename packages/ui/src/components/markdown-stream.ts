...
import { stableBlocks } from "../cypher/markdown-stable-blocks" // cypher_change
...
export function stream(text: string, live: boolean) {
  ...
  const candidate = tokens.findLast((token) => token.type !== "space") // cypher_change
  const blocks = candidate && !open(candidate.raw) ? stableBlocks(tokens, heal) : undefined // cypher_change
  if (blocks) return blocks // cypher_change
