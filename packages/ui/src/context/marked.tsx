...
// cypher_change: marked-shiki highlighted code blocks synchronously during
...
// cypher_change start: import types for double-dollar math extension
import type { MarkedExtension, TokenizerAndRendererExtension } from "marked"
// cypher_change end
import { bundledLanguages, type BundledLanguage } from "shiki"
import { parseFilePath } from "../file-path" // cypher_change
import { createSimpleContext } from "./helper"
import { getSharedHighlighter } from "@pierre/diffs" // cypher_change
import { ensureCypherDiffTheme } from "../pierre/cypher-diff-theme" // cypher_change

// cypher_change start: the "Cypher" diff/highlight theme registration moved to
// ../pierre/cypher-diff-theme so the diff worker pool can register it without
...
// route registration through ensureCypherDiffTheme() instead.
ensureCypherDiffTheme()
// cypher_change end

// cypher_change start: double-dollar-only math rules for marked.
const BLOCK = /^\$\$\n((?:\\[^]|[^\\])+?)\n\$\$(?:\n|$)/
const INLINE = /^\$\$(?!\$)((?:\\.|[^\\\n])*?(?:\\.|[^\\\n$]))\$\$/
// cypher_change end
...
function renderMathInText(text: string): string {
  ...
  // cypher_change: removed single-dollar inline math ($...$) rendering.
...
// cypher_change: parseFilePath imported from ../file-path

// cypher_change start: highlight cache for deferred highlighting
...
export async function deferredHighlight(
  ...
  const highlighter = await getSharedHighlighter({ themes: ["Cypher"], langs: [] })
      ...
      setTimeout(() => {
        ...
        try {
          ...
          const highlight = () => {
            ...
            const html = highlighter.codeToHtml(code, { lang: language, theme: "Cypher", tabindex: false })
...
export const { use: useMarked, provider: MarkedProvider } = createSimpleContext({
  ...
  init: (props: { nativeParser?: NativeMarkdownParser }) => {
    // cypher_change start: two-pass parser — first pass skips Shiki highlighting
    ...
    const parser = marked.use(
      ...
      // cypher_change start: enable only double-dollar math.
      // Single $ is far more common as a currency symbol in agent responses
      ...
      // cypher_change end
      // cypher_change: markedShiki removed — the custom `code` renderer
