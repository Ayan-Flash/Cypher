...
// The "Cypher" Pierre/Shiki theme used by every diff review surface (Code / Diff /
// File / SessionReview) and by markdown code highlighting. Pierre resolves the
// theme by name when a worker pool initializes (resolveThemes(["Cypher"])) or when
// getSharedHighlighter() attaches it; if the name was never registered it throws
// "resolveTheme: No valid loader for Cypher".
//
...
// diff component imports the worker factory, which calls ensureCypherDiffTheme()
...
// without markdown (e.g. cypher-console) to fire a racy `void import(...)` purely to
...
// the registration through ensureCypherDiffTheme() instead.

export const CYPHER_DIFF_THEME = "Cypher"
...
export function ensureCypherDiffTheme(): void {
  if (RegisteredCustomThemes.has(CYPHER_DIFF_THEME)) return

  registerCustomTheme(CYPHER_DIFF_THEME, () => {
    return Promise.resolve({
      name: CYPHER_DIFF_THEME,
      colors: {
