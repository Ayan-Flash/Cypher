...
// cypher_change start - make worker finalization await the process close event without a Windows race
  ...
  it.live(
    ...
    () =>
      Effect.promise(async () => {
        ...
        try {
          await waitForFile(ready, 20_000) // cypher_change - hosted macOS can start this worker slowly after stress tests
          await stopWorker(proc) // cypher_change - stopWorker now awaits close before returning
    ...
    60_000, // cypher_change - match the wider worker readiness window
  )
})
