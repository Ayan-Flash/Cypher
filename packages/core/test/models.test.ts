...
// test/preload.ts pins CYPHER_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.CYPHER_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.CYPHER_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.CYPHER_MODELS_PATH = undefined
  Flag.CYPHER_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.CYPHER_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.CYPHER_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
...
describe("ModelsDev Service", () => {
  ...
  it.live("refresh(true) fetches via HttpClient and updates the cache", () =>
    Effect.gen(function* () {
      ...
      expect(final.calls[0].userAgent).toEndWith(`/${Flag.CYPHER_CLIENT}`)
    }),
  )
