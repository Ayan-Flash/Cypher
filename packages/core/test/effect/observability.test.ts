...
const opencodeClient = process.env.CYPHER_CLIENT

afterEach(() => {
  if (otelResourceAttributes === undefined) delete process.env.OTEL_RESOURCE_ATTRIBUTES
  else process.env.OTEL_RESOURCE_ATTRIBUTES = otelResourceAttributes

  if (opencodeClient === undefined) delete process.env.CYPHER_CLIENT
  else process.env.CYPHER_CLIENT = opencodeClient
})
