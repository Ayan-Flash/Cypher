...
const auth = Effect.acquireRelease(
  Effect.sync(() => {
    const value = process.env.CYPHER_AUTH_CONTENT
    delete process.env.CYPHER_AUTH_CONTENT
    return value
  }),
  (value) =>
    Effect.sync(() => {
      if (value === undefined) delete process.env.CYPHER_AUTH_CONTENT
      else process.env.CYPHER_AUTH_CONTENT = value
    }),
)

describe("AccountV2 auth-v2 migration", () => {
  it.live("preserves multiple accounts, active selection, and Cypher organization", () =>
            ...
            Effect.gen(function* () {
              ...
              const result = yield* Effect.gen(function* () {
                ...
                return {
                  ...
                  active: yield* accounts.active(AccountV2.ServiceID.make("cypher")),
                }
