    ...
    return {
      "catalog.transform": Effect.fn(function* (evt) {
        for (const item of evt.provider.list()) {
          ...
          if (item.provider.id !== ProviderV2.ID.make("zenmux")) continue // cypher_change
          evt.provider.update(item.provider.id, (provider) => {
            provider.options.headers["HTTP-Referer"] ??= "https://cypher.ai/" // cypher_change
            provider.options.headers["X-Title"] ??= "Cypher CLI" // cypher_change
          })
        }
      }),
    }
