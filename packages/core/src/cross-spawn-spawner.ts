...
import { prepareCommand as prepareSandbox } from "@cypher/sandbox" // cypher_change
import { tap as tapStdio, tapped } from "./cypher/stdio-tap" // cypher_change - Bun drops buffered stdio on close
...
export const make = Effect.gen(function* () {
  ...
  const setupOutput = (
    ...
    let stdout = proc.stdout
      ? NodeStream.fromReadable({
          evaluate: () => tapped(proc, "stdout"), // cypher_change - read the spawn-time tap
          onError: (cause) => toPlatformError("fromReadable(stdout)", toError(cause), command),
        })
      : Stream.empty
    let stderr = proc.stderr
      ? NodeStream.fromReadable({
          evaluate: () => tapped(proc, "stderr"), // cypher_change - read the spawn-time tap
  ...
  const spawn = (command: ChildProcess.StandardCommand, opts: NodeChildProcess.SpawnOptions) =>
    Effect.callback<readonly [NodeChildProcess.ChildProcess, ExitSignal], PlatformError.PlatformError>((resume) => {
      ...
      tapStdio(proc) // cypher_change - must run in the same tick as spawn
      let end = false
    ...
    function* (command) {
      switch (command._tag) {
        case "StandardCommand": {
          ...
          // cypher_change start - prepare agent-scoped commands through the selected sandbox backend
          ...
          const [proc, signal] = yield* Effect.acquireRelease(
            // cypher_change start - spawn the prepared command and options
            spawn(target, {
              cwd: dir,
            ...
            Effect.fnUntraced(function* ([proc, signal]) {
              ...
              const kill = timeout(proc, command, target.options) // cypher_change
              if (done) {
              ...
              // cypher_change start - preserve kill options from the prepared command
