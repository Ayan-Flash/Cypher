export function base64Encode(value: string) {
  const normalized = value.replace(/\\/g, "/") // cypher_change - normalize Windows paths
  const bytes = new TextEncoder().encode(normalized) // cypher_change - encode as UTF-8 bytes
