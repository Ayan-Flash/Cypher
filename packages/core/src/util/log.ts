...
import { existsSync, writeFileSync } from "fs" // cypher_change
import fs from "fs/promises"
import * as Global from "../global"
import { Schema } from "effect"
import { Glob } from "./glob"
import { createStream } from "rotating-file-stream" // cypher_change
import { CYPHER_RUN_ID } from "./opencode-process" // cypher_change
...
const keep = 10
const initializedRunID = "CYPHER_LOG_INITIALIZED_RUN_ID"
...
let write = stderr
let stream: ReturnType<typeof createStream> | undefined // cypher_change

export async function init(options: Options) {
  if (options.level) level = options.level
  void cleanup(Global.Path.log)
  // cypher_change start - initialize one rotating stream and truncate dev.log once per Cypher run
  if (stream) {
    const active = stream
    stream = undefined
  ...
  const run = process.env[CYPHER_RUN_ID]
