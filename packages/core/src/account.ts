...
export class OAuthCredential extends Schema.Class<OAuthCredential>("AccountV2.OAuthCredential")({
  ...
  accountId: Schema.optional(Schema.String), // cypher_change - preserve Cypher organization during v1 migration
}) {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const prior = path.join(global.data, "auth-v2.json") // cypher_change
    ...
    const parseAuthContent = () => {
      try {
        return JSON.parse(process.env.CYPHER_AUTH_CONTENT ?? "")
      } catch {}
    }

    const load: () => Effect.Effect<Writable, Error> = Effect.fnUntraced(function* () {
      if (process.env.CYPHER_AUTH_CONTENT) {
      ...
      // cypher_change start - migrate the previous Cypher multi-account store after the current store
