...
import { decorateFileSystem, ensureDirectory } from "@cypher/sandbox" // cypher_change
import { dirname, isAbsolute, join, relative, resolve as pathResolve, sep } from "path" // cypher_change - harden containment checks
...
export namespace AppFileSystem {
  ...
  export const layer = Layer.effect(
    Service,
    Effect.gen(function* () {
      const fs = decorateFileSystem(yield* FileSystem.FileSystem) // cypher_change
      ...
      const ensureDir = Effect.fn("FileSystem.ensureDir")(function* (path: string) {
        yield* ensureDirectory(fs, path) // cypher_change - mutate through the sandbox-confined filesystem
      })
      ...
      const writeWithDirs = Effect.fn("FileSystem.writeWithDirs")(function* (
        ...
        yield* write.pipe(
          Effect.catchIf(
            ...
            () =>
              Effect.gen(function* () {
                yield* ensureDirectory(fs, dirname(path)) // cypher_change - sandbox-confined mkdir
                yield* write
              }),
          ),
        )
  ...
  export function contains(parent: string, child: string) {
    // cypher_change start - reject cross-drive and escaped relative paths
