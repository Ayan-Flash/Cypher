...
import { fetchProfile } from "@cypher/cypher-gateway"
...
export namespace Identity {
  ...
  export async function getMachineId(): Promise<string | undefined> {
    ...
    const override = process.env.CYPHER_MACHINE_ID
    if (override) {
      machineId = override
      return machineId
    }
  ...
  export async function updateFromCypherAuth(token: string | null, accountId?: string): Promise<void> {
