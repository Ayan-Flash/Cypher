...
class AgentEditDialogTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = CypherAppService(scope, FakeAppRpcApi())
    }
    ...
    fun `test loads agent into form`() {
        ...
        edt {
            ...
            assertEquals(CypherCliParser.MODE_ALL, field<ComboBox<*>>(root, title("mode")).selectedItem)
            ...
            val result = d.result()
            assertEquals("cypher/gpt-5", result.model)
    ...
    fun `test reads edits back into draft`() {
        ...
        val result = edt {
            ...
            field<ComboBox<*>>(root, title("mode")).selectedItem = CypherCliParser.MODE_SUBAGENT
        ...
        assertEquals(CypherCliParser.MODE_SUBAGENT, result.mode)
    ...
    fun `test native agent omits restricted editing`() {
        ...
        edt {
            ...
            assertEquals(CypherCliParser.MODE_PRIMARY, result.mode)
    ...
    private fun draft() = AgentEditDraft(name = "code", mode = CypherCliParser.MODE_PRIMARY, native = false)
    ...
    private fun title(field: String) = CypherBundle.message("settings.agentBehavior.agents.edit.$field")
