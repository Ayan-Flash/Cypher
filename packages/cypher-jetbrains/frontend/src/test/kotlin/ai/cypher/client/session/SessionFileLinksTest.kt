...
class SessionFileLinksTest : BasePlatformTestCase() {
    ...
    fun `test isFileHref separates file refs from urls`() {
        ...
        assertFalse(SessionFileLinks.isFileHref("https://cypher.ai/docs"))
    ...
    fun `test managed worktree storage filter only rejects worktree subtree`() {
        ...
        assertFalse(isManagedWorktreeStorage(".cypher/plans/x.md"))
        assertTrue(isManagedWorktreeStorage(".cypher/worktrees"))
        assertTrue(isManagedWorktreeStorage(".cypher/worktrees/foo/backend/src/Main.java"))
    }
