...
class LoadingPanelTest : BasePlatformTestCase() {
    ...
    fun `test loading retry and offline update retained label`() {
        ...
        assertEquals(CypherBundle.message("session.empty.loading"), panel.labelText())
    ...
    fun `test blank retry and offline use fallback text`() {
        ...
        assertEquals(CypherBundle.message("session.status.retry"), panel.labelText())

        panel.setState(SessionState.Offline("", requestId = "req1"))
        assertEquals(CypherBundle.message("session.status.offline"), panel.labelText())
    }
