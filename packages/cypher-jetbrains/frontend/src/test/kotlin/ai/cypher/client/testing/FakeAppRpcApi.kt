...
/**
 * Fake [CypherAppRpcApi] for testing.
 *
 * Push state changes via [state]. Health check returns [health].
 *
 * Every `suspend` method asserts it is NOT called on the EDT.
 */
class FakeAppRpcApi : CypherAppRpcApi {

    val state = MutableStateFlow(CypherAppStateDto(CypherAppStatusDto.DISCONNECTED))
    ...
    var fakeDeviceAuth = DeviceAuthDto(code = "TEST-1234", verificationUrl = "https://auth.cypher.ai/device")
