...
class SessionModelTest : BasePlatformTestCase() {
    ...
    fun `test initial app and workspace state`() {
        assertEquals(CypherAppStatusDto.DISCONNECTED, model.app.status)
        assertEquals(CypherWorkspaceStatusDto.PENDING, model.workspace.status)
    ...
    fun `test isReady requires app and workspace readiness`() {
        model.app = CypherAppStateDto(CypherAppStatusDto.READY)
        assertFalse(model.isReady())

        model.workspace = CypherWorkspaceStateDto(CypherWorkspaceStatusDto.READY)
