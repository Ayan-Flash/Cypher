...
class PromptLifecycleTest : SessionControllerTestBase() {
    ...
    override fun setUp() {
        super.setUp()
        edt { CypherPluginSettings.unsetAutoApprove() }
    }
    ...
    fun `test prompt records aggregate mention telemetry`() {
        appRpc.state.value = ai.cypher.rpc.dto.CypherAppStateDto(ai.cypher.rpc.dto.CypherAppStatusDto.READY, config = ai.cypher.rpc.dto.ConfigDto(model = "cypher/gpt-5"))
    ...
    fun `test PermissionAsked moves state to AwaitingPermission`() {
        ...
        assertSession(
            ...
            [code] [cypher/gpt-5] [awaiting-permission]
            """,
            m,
        )
    }
    ...
    fun `test PermissionReplied resumes Busy state`() {
        ...
        assertSession(
            """
            [code] [cypher/gpt-5] [busy] [considering next steps]
            """,
            m,
        )
    }
    ...
    fun `test QuestionAsked moves state to AwaitingQuestion`() {
        ...
        assertSession(
            ...
            [code] [cypher/gpt-5] [awaiting-question]
            """,
            m,
        )
    }
    ...
    fun `test auto approve drains pending permissions during recovery`() {
        ...
        projectRpc.state.value = workspaceReady()
        rpc.pendingPermissionList.add(permission("perm_pending"))
        edt { CypherPluginSettings.setAutoApprove(true) }
    ...
    fun `test auto approve persists in properties`() {
        ...
        assertFalse(CypherPluginSettings.getAutoApprove())
        edt { m.setAutoApprove(true) }

        assertTrue(CypherPluginSettings.getAutoApprove())
    ...
    fun `test child non-permission events do not change root state`() {
        ...
        emit(ChatEventDto.SessionStatusChanged("ses_child", ai.cypher.rpc.dto.SessionStatusDto("busy")), flush = false)
        ...
        val stateEvents = modelEvents.filterIsInstance<ai.cypher.client.session.model.SessionModelEvent.StateChanged>()
