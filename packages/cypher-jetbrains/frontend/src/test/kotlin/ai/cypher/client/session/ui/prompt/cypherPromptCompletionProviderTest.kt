...
class CypherPromptCompletionProviderTest : BasePlatformTestCase() {
    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeWorkspaceRpcApi
    private lateinit var provider: CypherPromptCompletionProvider

    override fun setUp() {
        super.setUp()
        scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
        rpc = FakeWorkspaceRpcApi()
        val workspaces = CypherWorkspaceService(scope, rpc)
        provider = CypherPromptCompletionProvider(
    ...
    fun `test accepting mention mid token replaces glued suffix`() {
        ...
        assertEquals(
            CypherPromptCompletionProvider.MentionHit(0, 22, "backend/deploy-dev.sh", true),
    ...
    fun `test highlights known slash command at start`() {
        assertEquals(
            listOf(CypherPromptCompletionProvider.Highlight(0, 4, CypherPromptCompletionProvider.HighlightKind.COMMAND)),
    ...
    fun `test highlights client slash command aliases at start`() {
        assertEquals(
            listOf(CypherPromptCompletionProvider.Highlight(0, 6, CypherPromptCompletionProvider.HighlightKind.COMMAND)),
            provider.highlights("/clear"),
        )
    }

    fun `test highlights server slash command at start`() {
        rpc.state.value = CypherWorkspaceStateDto(CypherWorkspaceStatusDto.READY, commands = listOf(CommandDto("deploy")))

        waitFor { provider.highlights("/deploy prod").isNotEmpty() }

        assertEquals(
            listOf(CypherPromptCompletionProvider.Highlight(0, 7, CypherPromptCompletionProvider.HighlightKind.COMMAND)),
    ...
    fun `test highlights special mentions without tracked paths`() {
        assertEquals(
            listOf(
                CypherPromptCompletionProvider.Highlight(4, 16, CypherPromptCompletionProvider.HighlightKind.MENTION),
            ),
    ...
    fun `test serverCommand does not route client aliases`() {
        rpc.state.value = CypherWorkspaceStateDto(CypherWorkspaceStatusDto.READY, commands = listOf(CommandDto("clear")))
    ...
    fun `test highlights tracked mentions longest first`() {
        ...
        assertEquals(
            listOf(CypherPromptCompletionProvider.Highlight(4, 14, CypherPromptCompletionProvider.HighlightKind.MENTION)),
    ...
    fun `test highlights unresolved mention after validation`() {
        ...
        assertEquals(
            listOf(CypherPromptCompletionProvider.Highlight(4, 16, CypherPromptCompletionProvider.HighlightKind.INVALID)),
    ...
    fun `test highlights existing hand typed mention after validation`() {
        ...
        assertEquals(
            listOf(CypherPromptCompletionProvider.Highlight(4, 13, CypherPromptCompletionProvider.HighlightKind.MENTION)),
    ...
    fun `test mentionAt resolves tracked file mention`() {
        ...
        assertEquals(
            CypherPromptCompletionProvider.MentionHit(4, 13, "src/a.ts", true),
    ...
    fun `test mentionAt marks unresolved file mention after validation`() {
        ...
        assertEquals(
            CypherPromptCompletionProvider.MentionHit(4, 15, "missing.ts", false),
    ...
    private fun noMatches() = CypherBundle.message("prompt.completion.noMatches")
}
