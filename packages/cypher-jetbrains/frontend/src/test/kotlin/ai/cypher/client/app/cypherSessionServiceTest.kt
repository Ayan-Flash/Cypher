...
class CypherSessionServiceTest : BasePlatformTestCase() {
    private lateinit var scope: CoroutineScope
    private lateinit var rpc: FakeSessionRpcApi
    private lateinit var service: CypherSessionService

    override fun setUp() {
        super.setUp()
        scope = CoroutineScope(SupervisorJob())
        rpc = FakeSessionRpcApi()
        service = CypherSessionService(project, scope, rpc)
    }
    ...
    fun `test events logs normal completion`() = runBlocking(Dispatchers.Default) {
        val log = TestLog()
        service = CypherSessionService(project, scope, rpc, log)
