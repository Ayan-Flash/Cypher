...
import ai.cypher.client.session.ui.prompt.SendPromptContext
...
class SendPromptActionTest : BasePlatformTestCase() {
    fun `test action invokes prompt context`() {
        ...
        assertEquals("Send the current Cypher prompt", action.templatePresentation.description)
    }
    ...
    fun `test frontend descriptor registers send prompt shortcuts`() {
        ...
        assertTrue(xml.contains("id=\"Cypher.SendPrompt\""))
