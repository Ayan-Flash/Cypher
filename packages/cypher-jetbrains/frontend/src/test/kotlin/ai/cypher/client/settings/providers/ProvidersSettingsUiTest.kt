...
import ai.cypher.client.settings.base.SettingsListRenderer
import ai.cypher.client.settings.base.SettingsListActionCell
import ai.cypher.client.settings.base.settingsListCellAt
import ai.cypher.client.settings.base.settingsListCellBounds
import ai.cypher.client.settings.base.settingsListSectionTitle
import ai.cypher.client.settings.base.settingsListVisibleCells
import ai.cypher.client.testing.FakeProviderRpcApi
...
import ai.cypher.rpc.dto.ProviderMetadataDto
...
class ProvidersSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test popular rows use vscode order including cypher`() {
        val rows = providerListRows(
            ProviderSettingsDto(
                providers = listOf(
                    provider("openrouter", "OpenRouter", priority = 5),
                    provider("cypher", "Cypher", priority = 0),
        ...
        assertEquals(listOf("cypher", "anthropic", "deepseek", "openai", "google", "openrouter", "vercel"), rows.map { it.key })
    ...
    fun `test connected cypher gateway has no provider settings actions`() {
        val rows = providerListRows(
            ProviderSettingsDto(
                providers = listOf(provider("cypher", "Cypher Gateway")),
                connected = listOf("cypher"),
            ),
            "",
        )

        assertEquals(listOf("cypher"), rows.map { it.key })
        assertEquals("Connected providers", settingsListSectionTitle(rows, 0))
    ...
    fun `test provider oauth waiting countdown can be cancelled without refresh`() {
        val callback = CompletableDeferred<ai.cypher.rpc.dto.ProviderActionResultDto>()
        val rpc = installProvider(
            ProviderSettingsDto(
                providers = listOf(provider("github-copilot", "GitHub Copilot")),
                auth = mapOf("github-copilot" to listOf(ProviderAuthMethodDto("oauth", "OAuth"))),
        ...
        callback.complete(ai.cypher.rpc.dto.ProviderActionResultDto(providerState(provider("stale", "Stale"))))
    ...
    fun `test provider oauth auto response shows device auth panel`() {
        ...
        edt {
            ...
            assertEquals("https://auth.openai.com/device", fieldsByName(panel, "cypher.provider.oauth.url").single().text)
            val qr = components(panel).filterIsInstance<JBLabel>().single { it.name == "cypher.provider.oauth.qr" }
    ...
    fun `test provider action failure returns error state`() = runBlocking {
        ...
        rpc.disconnectError = IllegalStateException("Cypher backend is not ready")
        val service = CypherProviderService(cs, rpc)

        val result = withContext(kotlinx.coroutines.Dispatchers.Default) {
            service.disconnect(ProviderDisconnectDto("/test", "openai"))
        }

        assertEquals("Cypher backend is not ready", result.error)
    ...
    private fun installProvider(state: ProviderSettingsDto): FakeProviderRpcApi {
        ...
        ApplicationManager.getApplication().replaceService(
            CypherProviderService::class.java,
            CypherProviderService(cs, rpc),
            testRootDisposable,
        )
        return rpc
    }
