...
class CypherReadyConfigurableTest : BasePlatformTestCase() {
    ...
    fun `test disconnected shows unavailable and does not create ready component`() {
        ...
        edt {
            val text = text(root)
            assertTrue(text, text.contains(CypherBundle.message("settings.cli.unavailable.title")))
            assertTrue(text, text.contains(CypherBundle.message("settings.cli.unavailable.message")))
    ...
    fun `test ready transition creates ready component once`() {
        ...
        rpc.state.value = CypherAppStateDto(CypherAppStatusDto.READY)
        flushUntil { edt { cfg?.created == 1 } }
        rpc.state.value = CypherAppStateDto(CypherAppStatusDto.DISCONNECTED)
        flush()

        edt {
            assertEquals(1, cfg?.created)
            assertTrue(text(root).contains("Ready content"))
            assertFalse(text(root).contains(CypherBundle.message("settings.cli.unavailable.title")))
        }
    }
