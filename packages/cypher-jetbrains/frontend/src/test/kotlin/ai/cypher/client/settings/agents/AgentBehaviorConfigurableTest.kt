...
class AgentBehaviorConfigurableTest : BasePlatformTestCase() {
    ...
    fun `test id matches xml registration`() {
        ...
        assertEquals("ai.cypher.jetbrains.settings.agentBehavior", cfg.id)
    }

    fun `test child ids match xml registration`() {
        assertEquals("ai.cypher.jetbrains.settings.agentBehavior.agents", AgentsConfigurable.ID)
        assertEquals("ai.cypher.jetbrains.settings.agentBehavior.mcp", McpConfigurable.ID)
    }
