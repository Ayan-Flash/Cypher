...
import ai.cypher.client.session.model.Message
...
class TurnViewTest : BasePlatformTestCase() {
    ...
    fun `test upsertPart adds a new TextView for Text content`() {
        val mv = MessageView(msg("a1", "assistant"), openFile)
        val text = ai.cypher.client.session.model.Text("p1")
    ...
    fun `test upsertPart updates existing part rather than adding duplicate`() {
        val mv = MessageView(msg("a1", "assistant"), openFile)
        val t1 = ai.cypher.client.session.model.Text("p1").also { it.content.append("v1") }
        mv.upsertPart(t1)

        val t2 = ai.cypher.client.session.model.Text("p1").also { it.content.append("v2") }
        mv.upsertPart(t2)
    ...
    fun `test removePart removes the renderer`() {
        val mv = MessageView(msg("a1", "assistant"), openFile)
        mv.upsertPart(ai.cypher.client.session.model.Text("p1").also { it.content.append("x") })
        mv.removePart("p1")
    ...
    fun `test appendDelta reaches TextView`() {
        val mv = MessageView(msg("a1", "assistant"), openFile)
        mv.upsertPart(ai.cypher.client.session.model.Text("p1").also { it.content.append("hello ") })
    ...
    fun `test MessageView pre-populates parts from Message on creation`() {
        val message = msg("a1", "assistant")
        val text = ai.cypher.client.session.model.Text("p1").also { it.content.append("preloaded") }
