...
import ai.cypher.client.session.history.CloudHistoryItem
import ai.cypher.client.session.history.HistoryController
import ai.cypher.client.session.history.HistoryDataKeys
import ai.cypher.client.session.history.HistorySelection
import ai.cypher.client.session.history.HistorySource
...
class HistorySessionActionsTest : BasePlatformTestCase() {
    ...
    fun `test rename action passes untitled fallback to input`() {
        ...
        assertEquals(listOf(CypherBundle.message("history.untitled")), prompts)
        assertTrue(rpc.renames.isEmpty())
    }
    ...
    fun `test frontend descriptor registers history actions`() {
        val xml = javaClass.classLoader.getResourceAsStream("cypher.jetbrains.frontend.xml")
            ?.bufferedReader()
            ?.use { it.readText() }
            ?: error("missing frontend descriptor")

        assertTrue(xml.contains("id=\"Cypher.Session.Open\""))
        assertTrue(xml.contains("id=\"Cypher.Session.Rename\""))
        assertTrue(xml.contains("id=\"Cypher.Session.Delete\""))
        assertTrue(xml.contains("id=\"Cypher.History.ContextMenu\""))
        assertTrue(xml.contains("id=\"Cypher.Session.ContextMenu\""))
        assertTrue(xml.contains("ref=\"Cypher.Session.Open\""))
        assertTrue(xml.contains("ref=\"Cypher.Session.Rename\""))
        assertTrue(xml.contains("ref=\"Cypher.Session.Delete\""))
