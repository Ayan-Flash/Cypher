...
import ai.cypher.client.testing.fire
...
import ai.cypher.client.settings.base.settingsListCellBounds
import ai.cypher.client.testing.FakeAgentBehaviorRpcApi
...
class AgentsSettingsUiTest : BasePlatformTestCase() {
    ...
    fun `test changing default agent saves patch`() {
        ...
        assertEquals("ask", appRpc.configPatches.single().values[CypherCliParser.CONFIG_DEFAULT_AGENT])
    ...
    fun `test apply waits for backend ready before refetching`() {
        ...
        flushUntil { rows(panel).size == 6 }
        appRpc.afterConfig = {
            app._state.value = app._state.value.copy(status = CypherAppStatusDto.LOADING)
        ...
        assertTrue(edt { text(panel).contains(CypherBundle.message("settings.agentBehavior.saving")) })

        app._state.value = app._state.value.copy(status = CypherAppStatusDto.READY)
    ...
    fun `test importing an agent stages patch until apply`() {
        ...
        appRpc.afterConfig = { patch ->
            if (patch.agents.containsKey("reviewer")) {
                agentRpc.agents = agentRpc.agents + AgentDetailDto(
                    "reviewer",
                    description = "Reviews code",
                    mode = CypherCliParser.MODE_SUBAGENT,
                    native = false,
                    removable = true,
                )
            }
        ...
        edt {
            ...
            assertEquals(CypherCliParser.MODE_SUBAGENT, patch.mode)
    ...
    fun `test importing invalid JSON shows settings error message`() {
        ...
        val message = CypherBundle.message("settings.agentBehavior.agents.import.invalidJson")
    ...
    fun `test apply failure reloads and keeps remaining staged changes`() {
        ...
        flushUntil { text(panel).contains(CypherBundle.message("settings.agentBehavior.agents.create.failed")) }
    ...
    private fun install() {
        ...
        agentRpc = FakeAgentBehaviorRpcApi().apply {
            agents = listOf(
                AgentDetailDto("ask", displayName = "Ask", description = "Ask questions", mode = CypherCliParser.MODE_PRIMARY, native = true),
                AgentDetailDto("code", displayName = "Code", description = "Code things", mode = CypherCliParser.MODE_PRIMARY, native = true),
                AgentDetailDto("generated", description = "Generated", mode = CypherCliParser.MODE_SUBAGENT, native = false, removable = false),
                AgentDetailDto("hidden", description = "Hidden custom", mode = CypherCliParser.MODE_PRIMARY, native = false, removable = true, hidden = true),
                AgentDetailDto("old", description = "Old custom", mode = CypherCliParser.MODE_PRIMARY, native = false, removable = true, deprecated = true),
                AgentDetailDto("worker", description = "Worker", mode = CypherCliParser.MODE_SUBAGENT, native = false, removable = true),
            )
        }
        val workspaceRpc = FakeWorkspaceRpcApi().apply { models = ModelsWorkspaceDto(providers()) }
        app = CypherAppService(cs, appRpc)
        val ready = CypherAppStateDto(
        ...
        ApplicationManager.getApplication().replaceService(CypherAppService::class.java, app, testRootDisposable)
        ApplicationManager.getApplication().replaceService(CypherAgentBehaviorService::class.java, CypherAgentBehaviorService(cs, agentRpc), testRootDisposable)
        ApplicationManager.getApplication().replaceService(CypherWorkspaceService::class.java, CypherWorkspaceService(cs, workspaceRpc), testRootDisposable)
    }

    private fun providers() = ProvidersDto(
        providers = listOf(ProviderDto("cypher", "Cypher", models = mapOf("gpt-5" to ModelDto("gpt-5", "GPT-5")))),
