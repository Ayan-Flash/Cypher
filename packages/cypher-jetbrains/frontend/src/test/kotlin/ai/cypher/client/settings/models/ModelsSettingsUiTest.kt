...
class ModelsSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = CypherAppService(appScope, rpc)
        workspaces = CypherWorkspaceService(appScope, workspaceRpc)
        val state = CypherAppStateDto(
            CypherAppStatusDto.READY,
            config = ConfigDto(model = "cypher/old"),
    ...
    fun `test matching app state before save callback keeps pending target`() {
        ...
        rpc.state.value = state("cypher/new")
    ...
    fun `test matching app state after save callback preserves dirty draft`() {
        ...
        edt {
            assertSelected(panel, "cypher/old")
            assertTrue(panel.modified())
        }
    }
    ...
    fun `test stale app state during pending save is ignored`() {
        ...
        rpc.state.value = state("cypher/old")
    ...
    fun `test failed save after dispose shows notification`() {
        ...
        flushUntil {
            notes.any { it.groupId == "Cypher CLI" && it.type == NotificationType.ERROR }
        }
    ...
    fun `test logged out save keeps login banner stable`() {
        ...
        edt { ui = ModelsSettingsUi(uiScope, app, workspaces, directory = "/test") }
        val panel = requireUi()
        flushUntil { text(panel).contains("Old") && text(panel).contains("Sign in to Cypher CLI") }
        ...
        edt {
            select(panel, "new")
            panel.applyDraft()
            assertTrue(text(panel).contains("Sign in to Cypher CLI"))
    ...
    private fun state(model: String): CypherAppStateDto = CypherAppStateDto(
        CypherAppStatusDto.READY,
    ...
    private fun select(panel: ModelsSettingsUi, id: String) {
        ...
        picker.onSelect(ModelPicker.Item(id, id.replaceFirstChar { it.titlecase() }, "cypher", "Cypher"))
    }
