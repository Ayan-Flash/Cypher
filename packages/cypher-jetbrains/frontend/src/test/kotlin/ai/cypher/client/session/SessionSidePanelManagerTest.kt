...
import ai.cypher.client.session.history.HistoryPanel
...
import ai.cypher.client.testing.TestUiTimers
...
class SessionSidePanelManagerTest : BasePlatformTestCase() {
    ...
    fun `test inactive sessions keep queued style updates`() {
        ...
        val style = ai.cypher.client.session.ui.style.SessionEditorStyle.create(family = "Courier New", size = 24)
    ...
    fun `test history overlays update before hidden timeout`() {
        ...
        assertEquals(CypherBundle.message("history.badge.question"), history.badgeText(0))
    }
    ...
    private fun setInactiveDisposeTimeout(ms: Int) {
        val key = "cypher.session.inactive.disposeTimeoutMs"
    ...
    private fun JPanel.controller(): ai.cypher.client.session.controller.SessionController {
        val field = SessionUi::class.java.getDeclaredField("controller")
        field.isAccessible = true
        return field.get(this) as ai.cypher.client.session.controller.SessionController
    }
