...
import ai.cypher.client.settings.profile.ProfileUi
import ai.cypher.client.settings.profile.formatResetDate
import ai.cypher.client.settings.profile.formatShortBalance
...
class UserProfileConfigurableTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        rpc = FakeAppRpcApi()
        app = CypherAppService(scope, rpc)
        app._state.value = CypherAppStateDto(CypherAppStatusDto.READY)
        edt {
            panel = ProfileUi(
                profile = null,
    ...
    fun `test login updates profile UI`() {
        ...
        edt {
            ...
            buttons(panel).first { it.text == "Login with Cypher CLI" }.doClick()
        }
        flush()
        ...
        assertEquals(listOf("https://auth.cypher.ai/device"), urls)
    }

    fun `test logout updates profile UI`() {
        val profile = ProfileDto(email = "alice@test.com", name = "Alice")
        rpc.fakeProfile = profile
        app._state.value = CypherAppStateDto(CypherAppStatusDto.READY, profile = profile)
        edt { panel.update(profile, CypherAppStatusDto.READY) }
        ...
        edt {
            ...
            assertTrue(buttons(panel).any { it.text == "Login with Cypher CLI" })
        }
    }
    ...
    fun `test organization switch updates balance UI`() {
        ...
        app._state.value = CypherAppStateDto(CypherAppStatusDto.READY, profile = personal)
        edt { panel.update(personal, CypherAppStatusDto.READY) }
    ...
    fun `test logged in profile uses compact stack and copyable email`() {
        ...
        edt {
            ...
            layout(panel)
            val logo = labelsByName(panel, "cypher.profile.logo.loggedIn").single()
    ...
    fun `test personal profile shows cypher pass usage`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
            cypherPass = ProfileCypherPassDto(
        ...
        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Cypher Pass"))
            ...
            assertTrue("pass meter should be visible", panelsByName(panel, "cypher.profile.passPanel").single().isVisible)
            assertFalse(t, t.contains("Get Cypher Pass"))
        }
    }

    fun `test cypher pass short amounts use half up rounding`() {
        assertEquals("$3", formatShortBalance(2.5))
        assertEquals("$2", formatShortBalance(2.49))
    }

    fun `test cypher pass hides empty bonus and invalid renewal rows`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
        ...
        val invalid = profile.copy(
            cypherPass = profile.cypherPass?.copy(
                currentPeriodBonusCreditsUsd = 0.0,
                nextBillingAt = "not-a-date",
            ),
        )
        edt { panel.update(invalid, CypherAppStatusDto.READY) }
        ...
        val none = invalid.copy(cypherPass = invalid.cypherPass?.copy(nextBillingAt = null))
        edt { panel.update(none, CypherAppStatusDto.READY) }
    ...
    fun `test cypher pass meter clamps base zero and avoids noop repaint`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(267.59),
        ...
        val meter = edt { componentsByName(panel, "cypher.profile.passMeter").single() }
        ...
        edt { panel.update(full, CypherAppStatusDto.READY) }
    ...
    fun `test personal profile shows cypher pass without balance`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            cypherPass = ProfileCypherPassDto(
        ...
        val updated = profile.copy(
            cypherPass = profile.cypherPass?.copy(currentPeriodUsageUsd = 88.0),
        )
        ...
        edt {
            ...
            assertTrue("pass panel should be visible", panelsByName(panel, "cypher.profile.passPanel").single().isVisible)
    ...
    fun `test personal profile without cypher pass shows subscribe link`() {
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
            balance = ProfileBalanceDto(10.0),
        ...
        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Get Cypher Pass to add credits and earn bonuses"))
            buttons(panel).first { it.text == "Dashboard" }.doClick()
            buttons(panel).first { it.text == "Top up" }.doClick()
            buttons(panel).first { it.text == "Get Cypher Pass to add credits and earn bonuses" }.doClick()
        }

        assertEquals(listOf("https://app.cypher.ai/profile", "https://app.cypher.ai/credits", "https://cypher.ai/pricing/cypher-pass"), urls)
    }

    fun `test cypher pass renewal date uses utc`() {
    ...
    fun `test org profile hides cypher pass`() {
        val orgs = listOf(ProfileOrganizationDto(id = "org_1", name = "Acme", role = "ADMIN"))
        val profile = ProfileDto(
            email = "alice@test.com",
            name = "Alice",
        ...
        edt {
            val t = text(panel)
            assertFalse(t, t.contains("Cypher Pass"))
            assertFalse(t, t.contains("Get Cypher Pass"))
        }
    }
    ...
    fun `test logged out update retains login button`() {
        edt {
            val btn = buttons(panel).first { it.text == "Login with Cypher CLI" }
            panel.update(null, CypherAppStatusDto.READY)
            val btn2 = buttons(panel).first { it.text == "Login with Cypher CLI" }
            assertSame(btn, btn2)
        }
    }

    fun `test logged out profile shows cypher icon above login content`() {
        edt {
            panel.update(null, CypherAppStatusDto.READY)
            panel.setSize(800, 600)
            layout(panel)

            val logo = labelsByName(panel, "cypher.profile.logo.loggedOut").single()
    ...
    fun `test account update retains name label`() {
        ...
        edt {
            panel.update(alice, CypherAppStatusDto.READY)
            val lbl = labels(panel).first { it.text == "Alice" }
            panel.update(bob, CypherAppStatusDto.READY)
    ...
    fun `test organization switch keeps account visible during transient null profile`() {
        ...
        val transientState = CypherAppStateDto(
            status = CypherAppStatusDto.READY,
            profile = null,
        ...
        edt {
            panel.update(personal, CypherAppStatusDto.READY)
    ...
    fun `test login shows device auth card before completion`() {
        ...
        flushUntil { text(panel).contains("Sign in to Cypher CLI") }

        edt {
            val t = text(panel)
            assertTrue(t, t.contains("Sign in to Cypher CLI"))
            assertTrue(t, t.contains("Step 1:"))
            assertTrue(t, t.contains("Open this URL"))
            assertTrue(t, t.contains("https://auth.cypher.ai/device"))
        ...
        edt {
            val qr = labelsByName(panel, "cypher.login.qr").firstOrNull()
            assertNotNull(qr)
    ...
    fun `test cancel login invalidates stale completion`() {
        ...
        edt { buttons(panel).first { it.text == "Login with Cypher CLI" }.doClick() }
        flushUntil { text(panel).contains("Sign in to Cypher CLI") }
    ...
    fun `test auth card retains qr label across sync`() {
        ...
        val qrBefore = edt { labelsByName(panel, "cypher.login.qr").firstOrNull() }
        assertNotNull(qrBefore)

        // Force another sync call while still pending
        edt { panel.update(null, CypherAppStatusDto.READY) }
        flush()

        val qrAfter = edt { labelsByName(panel, "cypher.login.qr").firstOrNull() }
    ...
    fun `test url field selects all on click`() {
        ...
        edt {
            val field = fieldsByName(panel, "cypher.login.url").firstOrNull()
    ...
    fun `test balance card has card background`() {
        ...
        edt {
            val card = panelsByName(panel, "cypher.profile.balanceCard").firstOrNull()
    ...
    fun `test code panel has card background`() {
        ...
        edt {
            val codePanel = panelsByName(panel, "cypher.login.codePanel").firstOrNull()
    ...
    fun `test combo model not rebuilt on balance change with same org list`() {
        ...
        edt { panel.update(updated, CypherAppStatusDto.READY) }
    ...
    fun `test combo model updated in place when org list changes`() {
        ...
        app._state.value = CypherAppStateDto(CypherAppStatusDto.READY, profile = profile1)
        edt { panel.update(profile1, CypherAppStatusDto.READY) }
        ...
        edt { panel.update(profile2, CypherAppStatusDto.READY) }
    ...
    fun `test profile update does not trigger organization rpc`() {
        ...
        app._state.value = CypherAppStateDto(CypherAppStatusDto.READY, profile = profile)
        edt { panel.update(profile, CypherAppStatusDto.READY) }
        edt { panel.update(profile.copy(currentOrgId = "org_1"), CypherAppStatusDto.READY) }
        flush()
    ...
    fun `test connecting while logged in keeps logged-in card visible`() {
        ...
        edt { panel.update(null, CypherAppStatusDto.CONNECTING) }
    ...
    fun `test loading while logged in keeps logged-in card visible`() {
        ...
        edt { panel.update(null, CypherAppStatusDto.LOADING) }
    ...
    fun `test loading with null profile while logged in does not crash`() {
        ...
        edt { panel.update(CypherAppStateDto(CypherAppStatusDto.LOADING)) }
        ...
        edt { panel.update(switched, CypherAppStatusDto.READY) }
    ...
    fun `test connecting while logged in with org selected keeps combo selection`() {
        ...
        app._state.value = CypherAppStateDto(CypherAppStatusDto.READY, profile = profile)
        edt {
            panel.update(profile, CypherAppStatusDto.READY)
            combos(panel).single().selectedIndex = 1
        }
    ...
    fun `test preferred focus for logged-out is login button`() {
        edt {
            val focus = panel.preferredFocus()
            val loginBtn = buttons(panel).firstOrNull { it.text == "Login with Cypher CLI" }
    ...
    fun `test device auth without code hides code panel and step2 label`() {
        ...
        rpc.fakeDeviceAuth = DeviceAuthDto(code = null, verificationUrl = "https://auth.cypher.ai/device")
