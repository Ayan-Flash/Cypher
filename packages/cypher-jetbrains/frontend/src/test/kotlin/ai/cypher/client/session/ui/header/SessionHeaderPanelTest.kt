...
import ai.cypher.client.session.model.StepFinish
...
import ai.cypher.client.session.controller.SessionControllerTestBase
...
class SessionHeaderPanelTest : SessionControllerTestBase() {
    ...
    fun `test starts hidden for empty header`() {
        appRpc.state.value = ai.cypher.rpc.dto.CypherAppStateDto(ai.cypher.rpc.dto.CypherAppStatusDto.READY)
    ...
    fun `test timeline colors honor semantic named color keys`() {
        val old = UIManager.getColor("Cypher.Session.Timeline.Read")
        val color = Color(12, 34, 56)

        try {
            UIManager.put("Cypher.Session.Timeline.Read", color)

            assertEquals(color.rgb, SessionUiStyle.Timeline.READ.rgb)
        } finally {
            UIManager.put("Cypher.Session.Timeline.Read", old)
        }
    }
    ...
    private fun promptedHeader(): ai.cypher.client.session.controller.SessionController {
        appRpc.state.value = ai.cypher.rpc.dto.CypherAppStateDto(
            ai.cypher.rpc.dto.CypherAppStatusDto.READY,
            config = ai.cypher.rpc.dto.ConfigDto(model = "cypher/gpt-5"),
        )
        projectRpc.state.value = workspaceReady(
            providers = listOf(
                ProviderDto(
                    ...
                    models = mapOf(
                        "gpt-5" to ModelDto(
                            id = "gpt-5",
                            name = "GPT-5",
                            limit = ai.cypher.rpc.dto.ModelLimitDto(context = 2_000_000, output = 200_000),
                        ),
                    ),
                ),
            ),
