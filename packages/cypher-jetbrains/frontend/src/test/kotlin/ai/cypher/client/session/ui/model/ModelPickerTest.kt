...
class ModelPickerTest : BasePlatformTestCase() {
    ...
    fun `test section order starts with favorites and recommended`() {
        val rows = modelPickerRows(listOf(
            ...
            item("auto", "Cypher Auto", "cypher", "Cypher", 0.0),
        ...
        assertEquals("cypher/auto", rows[1].key)
    ...
    fun `test cypher provider group is first and other providers keep source order`() {
        val rows = modelPickerRows(listOf(
            item("gpt", "GPT", "openai", "OpenAI"),
            item("auto", "Auto", "cypher", "Cypher"),
            item("claude", "Claude", "anthropic", "Anthropic"),
        ), emptyList(), "")

        assertEquals(listOf("Cypher", "OpenAI", "Anthropic"), rows.indices.mapNotNull { modelPickerSectionTitle(rows, it) })
    }
    ...
    fun `test small rows are included only when requested`() {
        val items = listOf(
            item("auto-small", "Small", "cypher", "Cypher"),
            item("auto", "Auto", "cypher", "Cypher"),
        )
        ...
        assertEquals(listOf("cypher/auto"), default.mapNotNull { it.key })
        assertEquals(listOf("cypher/auto", "cypher/auto-small"), small.mapNotNull { it.key }.sorted())
    }
    ...
    fun `test non-cypher selected model uses provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test non-cypher selected model strips duplicate provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test non-cypher selected model strips vscode provider prefix`() {
        val picker = ModelPicker()
    ...
    fun `test cypher selected model remains unprefixed`() {
        val picker = ModelPicker()

        picker.setItems(listOf(item("auto", "Cypher Auto", "cypher", "Cypher")))
    ...
    fun `test details panel displays preview metadata and updates favorite star`() {
        ...
        val item = ModelPicker.Item(
            ...
            options = ModelOptionsDto("Fast routed model https://cypher.ai"),
        ...
        assertTrue(panel.html().any { it.contains("href=\"https://cypher.ai\"") })
    ...
    fun `test details panel updates retained description component`() {
        ...
        val item = ModelPicker.Item(
            ...
            options = ModelOptionsDto("First https://cypher.ai"),
        )

        panel.update(item)
        val count = panel.treeSize()
        val html = panel.htmlPanes().single()
        panel.update(item.copy(id = "second", display = "Second", options = ModelOptionsDto("Second https://cypher.ai/docs")))
    ...
    fun `test selected paid model with training flag indicates data collection`() {
        ...
        picker.setItems(listOf(item("paid", "Paid", "cypher", "Cypher", training = true)))
    ...
    fun `test selected free model without training flag does not indicate data collection`() {
        ...
        picker.setItems(listOf(item("free", "Free", "cypher", "Cypher", free = true)))
    ...
    fun `test display parts sanitize free suffix`() {
        val parts = ModelText.parts(item("auto", "Auto Free (free)", "cypher", "Cypher"))
    ...
    fun `test renderer shows empty favorite star only for selected row`() {
        val row = ModelPickerRow(item("auto", "Auto", "cypher", "Cypher"), "Cypher", false)
    ...
    fun `test renderer keeps favorite star visible`() {
        val row = ModelPickerRow(item("auto", "Auto", "cypher", "Cypher"), "Cypher", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { setOf("cypher/auto") })
        val list = JBList(model)
    ...
    fun `test renderer shows free badge for free model`() {
        val row = ModelPickerRow(ModelPicker.Item("auto", "Auto", "cypher", "Cypher", free = true), "Cypher", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { emptySet() })
        val list = JBList(model)
    ...
    fun `test renderer shows data collection warning for paid training model`() {
        val row = ModelPickerRow(item("paid", "Paid", "cypher", "Cypher", training = true), "Cypher", false)
        val model = CollectionListModel(listOf(row))
        val renderer = ModelPickerRenderer(model, { null }, { emptySet() })
        val list = JBList(model)
    ...
    fun `test renderer shows BYOK instead of free when both are available`() {
        val row = ModelPickerRow(
            ModelPicker.Item("claude", "Claude", "cypher", "Cypher", free = true, byok = true),
            "Cypher",
            false,
        )
        val model = CollectionListModel(listOf(row))
