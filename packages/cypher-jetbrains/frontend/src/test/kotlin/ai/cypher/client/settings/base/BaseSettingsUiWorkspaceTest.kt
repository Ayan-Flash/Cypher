...
class BaseSettingsUiWorkspaceTest : BasePlatformTestCase() {
    ...
    fun `test model state updates are delivered on edt`() {
        rpc.models = ModelStateDto(favorite = listOf(ModelSelectionDto("cypher", "new")))
        rpc.state.value = state("ready")
        val view = create("/test")
    ...
    private fun state(model: String) = CypherAppStateDto(
    ...
    private class FakePanel(
        ...
        workspaces: CypherWorkspaceService,
        hint: String,
        ...
        override fun draft(state: CypherAppStateDto): Draft = Draft(state.config?.model ?: "none")
