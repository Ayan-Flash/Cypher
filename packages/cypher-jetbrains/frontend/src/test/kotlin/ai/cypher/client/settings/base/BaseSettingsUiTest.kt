...
class BaseSettingsUiTest : BasePlatformTestCase() {
    ...
    override fun setUp() {
        ...
        app = CypherAppService(appScope, FakeAppRpcApi())
        workspaces = CypherWorkspaceService(appScope, FakeWorkspaceRpcApi())
    }
    ...
    fun `test login banner can be shown`() {
        ...
        edt { assertTrue(text(view).contains("Sign in to Cypher CLI")) }
    }
    ...
    fun `test login banner can be disabled`() {
        ...
        edt { assertFalse(text(view).contains("Sign in to Cypher CLI")) }
    }
    ...
    private class FakePanel(
        ...
        override fun draft(state: CypherAppStateDto): Draft = draft
