...
import ai.cypher.client.session.SessionFileOpener
...
import ai.cypher.client.session.views.PlanExitView
...
import ai.cypher.client.session.views.question.QuestionResultView
import ai.cypher.client.session.views.question.QuestionView
import ai.cypher.client.session.ui.selection.SessionCopyTarget
import ai.cypher.client.session.views.MessageToolbar
import ai.cypher.client.session.views.MessageView
import ai.cypher.client.session.views.TextView
import ai.cypher.client.session.views.TurnView
import ai.cypher.client.session.views.base.PartView
import ai.cypher.client.session.views.tool.TaskToolView
import ai.cypher.client.session.views.tool.ToolView
import ai.cypher.client.session.views.todo.TodoWriteView
import ai.cypher.client.ui.DiffStatBadge
import ai.cypher.client.ui.layout.Stack
import ai.cypher.rpc.dto.DiffFileDto
...
import ai.cypher.rpc.dto.SessionRevertDto
import ai.cypher.rpc.dto.TodoDto
...
class SessionMessageListPanelTest : BasePlatformTestCase() {
    ...
    fun `test user prompt toolbar omits rollback when revert handler is absent`() {
        ...
        assertFalse(components(toolbar).filterIsInstance<JButton>().any { it.toolTipText == CypherBundle.message("revert.message.rollback") })
    }
    ...
    fun `test user prompt toolbar shows rollback when revert handler is present`() {
        ...
        val rollback = components(toolbar)
            ...
            .first { it.toolTipText == CypherBundle.message("revert.message.rollback") }
        rollback.doClick()
    ...
    fun `test text markdown link uses panel url opener`() {
        ...
        model.upsertMessage(msg("a1", "assistant"))
        model.updateContent("a1", part("p1", "a1", "text", text = "[docs](https://cypher.ai/docs)"))

        val view = item.findMessage("a1")!!.part("p1") as TextView
        view.md.simulateLink("https://cypher.ai/docs")

        assertEquals(listOf("https://cypher.ai/docs"), urls)
    }
    ...
    fun `test rollback banner uses session dialog card with standard actions`() {
        ...
        assertEquals(
            listOf(CypherBundle.message("revert.banner.redo"), CypherBundle.message("revert.banner.redo.all")),
            buttons.map { it.text },
        )
        assertEquals(listOf(CypherBundle.message("revert.banner.redo")), buttons.filter { it.isVisible }.map { it.text })
        ...
        val hint = components(banner)
            ...
            .first { it.text == CypherBundle.message("revert.banner.hint") }
    ...
    fun `test rollback banner shows redo all only for multiple reverted messages`() {
        ...
        assertTrue(components(banner).filterIsInstance<JButton>().first { it.text == CypherBundle.message("revert.banner.redo.all") }.isVisible)

        model.setRevert(SessionRevertDto("u2"))
        banner.update()

        assertFalse(components(banner).filterIsInstance<JButton>().first { it.text == CypherBundle.message("revert.banner.redo.all") }.isVisible)
    }
    ...
    fun `test rollback banner buttons invoke actions`() {
        ...
        components(banner).filterIsInstance<JButton>().first { it.text == CypherBundle.message("revert.banner.redo") }.doClick()
        components(banner).filterIsInstance<JButton>().first { it.text == CypherBundle.message("revert.banner.redo.all") }.doClick()
    ...
    fun `test rollback state disables banner actions and shows inline progress`() {
        ...
        val buttons = components(banner).filterIsInstance<JButton>()
        assertTrue(buttons.filter { it.text == CypherBundle.message("revert.banner.redo") }.all { !it.isEnabled })
        assertTrue(buttons.filter { it.text == CypherBundle.message("revert.banner.redo.all") }.all { !it.isEnabled })
    ...
    fun `test rollback banner explains snapshotless history only revert`() {
        ...
        val notice = components(banner).filterIsInstance<JBLabel>()
            .first { it.text == CypherBundle.message("revert.banner.filesNotRestored") }
    ...
    fun `test completed plan update replaces tool view and keeps open file action`() {
        ...
        view.simulateLink(".cypher/plans/x.md")

        assertEquals(listOf(".cypher/plans/x.md"), opened)
    }
