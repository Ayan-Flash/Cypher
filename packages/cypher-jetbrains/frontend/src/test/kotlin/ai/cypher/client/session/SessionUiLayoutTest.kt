...
import ai.cypher.client.session.ui.RevertProgress
import ai.cypher.client.session.ui.SessionDropOverlay
import ai.cypher.client.session.ui.SessionLayoutPanel
...
class SessionUiLayoutTest : SessionUiTestBase() {
    ...
    fun `test account overlay shows after recents complete`() {
        appRpc.state.value = CypherAppStateDto(CypherAppStatusDto.READY, profile = ProfileDto(email = "user@example.com"))
        rpc.recent.add(session("ses_1"))
        ui = newUi(displayMs = 1_000)
