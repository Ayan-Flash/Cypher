...
class DeleteSessionAction : AnAction() {
    ...
    override fun actionPerformed(e: AnActionEvent) {
        ...
        val msg = if (items.size == 1)
            CypherBundle.message("history.delete.confirm.message", ai.cypher.client.session.history.title(items[0]))
        else
            CypherBundle.message("history.delete.confirm.message.multiple", items.size)
