...
import ai.cypher.client.session.model.AgentItem
import ai.cypher.client.session.model.ModelLimitItem
import ai.cypher.client.session.model.ModelItem
...
import ai.cypher.client.session.model.PermissionMeta
import ai.cypher.client.session.model.PermissionRequestState
import ai.cypher.client.session.model.Question
import ai.cypher.client.session.model.QuestionItem
import ai.cypher.client.session.model.QuestionOption
import ai.cypher.client.session.model.Reasoning
import ai.cypher.client.session.model.ToolCallRef
import ai.cypher.client.session.model.Text
import ai.cypher.client.plugin.CypherPluginSettings
import ai.cypher.client.session.SessionRef
import ai.cypher.client.telemetry.Telemetry
import ai.cypher.client.util.UiTimer
...
class SessionController(
  parent: Disposable,
  ref: SessionRef? = null,
  private val sessions: CypherSessionService,
  private val workspace: Workspace,
  private val app: CypherAppService,
  ...
  private val log: CypherLog = LOG,
    ...
    companion object {
        private val LOG = CypherLog.create(SessionController::class.java)
    ...
    val autoApprove: Boolean get() = CypherPluginSettings.getAutoApprove()
        ...
        cs.launch {
            try {
                ...
                edt {
                    ...
                    val msg = e.message ?: CypherBundle.message("session.error.prompt")
                    updateModel {
    ...
    fun setAutoApprove(value: Boolean) {
        assertEdt()
        CypherPluginSettings.setAutoApprove(value)
    ...
    fun compact() {
        ...
        cs.launch {
            try {
                ...
                edt {
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: CypherBundle.message("session.error.compact")))
                    }
                }
            }
        }
    ...
    fun revert(message: String, part: String? = null) {
        ...
        val op = beginReverting(
            CypherBundle.message("session.status.rollingback"),
    ...
    fun unrevert() {
        ...
        if (revertOp != null) return
        val op = beginReverting(
            CypherBundle.message("session.status.redoing"),
    ...
    fun cancelRevert() {
        ...
        if (state is SessionState.Reverting) {
            model.setState(state.copy(text = CypherBundle.message("session.status.operation.finishing")))
        }
        revertJob?.cancel()
    }
    ...
    fun retryConnection() {
        ...
        if (model.app.status != CypherAppStatusDto.READY || model.app.status == CypherAppStatusDto.ERROR) {
            app.retryAsync()
            return
        }
        ...
        if (model.workspace.status == CypherWorkspaceStatusDto.ERROR) {
            workspace.reload()
        }
    }
            ...
            try {
                ...
                edt {
                    updatePermission(
                        ...
                        e.message ?: CypherBundle.message("session.permission.error"),
                    )
                }
            }
        }
    ...
    private fun approve(id: String, restore: () -> Permission) {
        ...
        cs.launch {
            try {
                ...
                edt {
                    if (disposed) return@edt
                    model.setState(SessionState.Busy(CypherBundle.message("session.status.considering")))
                }
                ...
                edt {
                    ...
                    model.setState(SessionState.AwaitingPermission(restore().copy(
                        ...
                        message = e.message ?: CypherBundle.message("session.permission.error"),
                    )))
                }
            }
        }
    ...
    init {
        ...
        cs.launch {
            app.state.collect { state ->
                if (state.status == CypherAppStatusDto.READY) app.fetchVersionAsync()
        ...
        cs.launch {
            workspace.state.collect { state ->
                fire(SessionControllerEvent.WorkspaceChanged) {
                    ...
                    if (state.status != CypherWorkspaceStatusDto.READY) return@fire
                    ...
                    model.models = state.providers?.let { providers ->
                        providers.providers
                            .filter { it.id == CYPHER_PROVIDER || it.id in providers.connected }
                            .flatMap { provider ->
                ...
                if (state.status == CypherWorkspaceStatusDto.READY) {
                    fire(SessionControllerEvent.WorkspaceReady)
                    edt {
        ...
        cs.launch {
            try {
                ...
                edt {
                    ...
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: CypherBundle.message("history.error.local")))
                    }
                    showSession()
                    loaded(false)
                }
        ...
        cs.launch {
            try {
                ...
                edt {
                    if (disposed) return@edt
                    updateModel {
                        model.setState(SessionState.Error(e.message ?: CypherBundle.message("history.error.cloud")))
                    }
                    showSession()
                    loaded(false)
                }
    ...
    private fun seedStatus(dto: SessionStatusDto) {
        ...
        val state = when (dto.type) {
            "busy" -> SessionState.Busy(CypherBundle.message("session.status.considering"))
    ...
    private fun handle(event: ChatEventDto) {
        ...
        when (event) {
            ...
            is ChatEventDto.TurnOpen -> {
                ...
                tool = null
                if (revertOp != null) {
                    revertDeferred = SessionState.Busy(CypherBundle.message("session.status.considering"))
                    return
                }
                model.setState(SessionState.Busy(CypherBundle.message("session.status.considering")))
            }
    ...
    private fun error(event: ChatEventDto.Error, reveal: Boolean) {
        ...
        if (isPaidModelAuthRequired(event.error)) {
            ...
            model.setState(SessionState.LoginRequired(CypherBundle.message("session.login.required.description")))
            return
        }
        val msg = event.error?.message ?: event.error?.type ?: CypherBundle.message("session.error.unknown")
        ...
        val state = when (dto.type) {
            ...
            "busy" -> {
                ...
                if (current is SessionState.Idle || current is SessionState.Error)
                    SessionState.Busy(CypherBundle.message("session.status.considering"))
    ...
    private fun onRevertTimeout(op: RevertOp) {
        ...
        revertJob?.cancel()
        failReverting(op, RuntimeException(CypherBundle.message("session.error.revert.timeout")))
    }
    ...
    private fun failReverting(op: RevertOp, e: Exception) {
        ...
        revertDeferred = null
        model.setState(SessionState.Error(e.message ?: CypherBundle.message("session.error.unknown")))
    }
    ...
    private fun configModel(agent: String): String? {
        if (model.app.status != CypherAppStatusDto.READY) return null
        val cfg = model.app.config
    ...
    private fun status(): String = when (partType) {
        "reasoning" -> CypherBundle.message("session.status.thinking")
        "text" -> CypherBundle.message("session.status.writing")
        "tool" -> when (tool) {
            "task" -> CypherBundle.message("session.status.delegating")
            "todowrite", "todoread" -> CypherBundle.message("session.status.planning")
            "read" -> CypherBundle.message("session.status.gathering")
            "glob", "grep", "list" -> CypherBundle.message("session.status.searching.codebase")
            "webfetch", "websearch", "codesearch" -> CypherBundle.message("session.status.searching.web")
            "edit", "write" -> CypherBundle.message("session.status.editing")
            "bash" -> CypherBundle.message("session.status.commands")
            else -> CypherBundle.message("session.status.considering")
        }
        else -> CypherBundle.message("session.status.considering")
    }
    ...
    private fun resolveConnectionState(): SessionControllerEvent.ConnectionChanged {
        ...
        if (app.status == CypherAppStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                CypherBundle.message("session.connection.error.app"),
                app.errors.toErrorText() ?: app.error,
            )
        }

        if (app.status == CypherAppStatusDto.DOWNLOADING) {
        ...
        if (workspace.status == CypherWorkspaceStatusDto.ERROR) {
            return SessionControllerEvent.ConnectionChanged.ShowError(
                CypherBundle.message("session.connection.error.workspace"),
                workspace.errors.toErrorText() ?: workspace.error,
                "workspace",
            )
        }

        if (app.status == CypherAppStatusDto.READY && workspace.status == CypherWorkspaceStatusDto.READY && app.warnings.isNotEmpty()) {
        ...
        if (app.status == CypherAppStatusDto.READY && workspace.status == CypherWorkspaceStatusDto.READY) {
...
private fun summary(count: Int): String {
    val base = CypherBundle.message("session.connection.warning.config")
...
private const val CYPHER_AUTO_MODEL = "cypher-auto/free"
...
private fun resolveModelSelection(
    ...
    fallback: ModelSelectionDto? = ModelSelectionDto(CYPHER_PROVIDER, CYPHER_AUTO_MODEL),
): ModelSelectionDto? {
...
private fun valid(providers: ProvidersDto?, item: ModelSelectionDto?): ModelSelectionDto? {
    ...
    if (item.providerID != CYPHER_PROVIDER && item.providerID !in providers.connected) return null
