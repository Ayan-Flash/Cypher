...
import ai.cypher.client.actions.CypherActionPlaces
...
import ai.cypher.client.session.controller.SessionControllerListener
...
class ConnectionPanel(
    ...
    companion object {
        internal const val CLI_GROUP_ID = "Cypher.CliGroup"
    ...
    private val retry = ActionLink(CypherBundle.message("session.connection.retry")) {
        showRecoveryPopup()
    }.apply {
        isVisible = false
    ...
    private fun showConnecting() {
        ...
        label.text = CypherBundle.message("session.connection.connecting")
        detail = null
        expanded = false
    ...
    private fun showDownloading(percent: Int, version: String?, platform: String?) {
        ...
        label.text = if (version != null && platform != null) {
            CypherBundle.message("session.connection.downloading.version", version, platform, pct)
        } else {
            CypherBundle.message("session.connection.downloading", pct)
        }
        detail = null
        expanded = false
        toggle.isVisible = false
    ...
    private fun showRecoveryPopup() {
        JBPopupFactory.getInstance()
            .createActionGroupPopup(
                ...
                true,
                CypherActionPlaces.connectionRetryPopup(),
            )
    ...
    private fun recoveryGroup(): ActionGroup {
        ...
        group.add(object : DumbAwareAction(CypherBundle.message("session.connection.retry")) {
            override fun actionPerformed(e: AnActionEvent) {
                controller.retryConnection()
            }
        })
        group.addSeparator()
        ActionManager.getInstance().getAction("Cypher.Restart")?.let { group.add(it) }
        ActionManager.getInstance().getAction("Cypher.Reinstall")?.let { group.add(it) }
        return group
    }
