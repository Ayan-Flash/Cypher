package ai.cypher.client.session.ui.model
...
import ai.cypher.client.ui.md.MdViewFactory
...
internal class ModelDetailsPanel(
    ...
    private val empty = JBLabel(CypherBundle.message("model.picker.details.empty")).apply {
    ...
    private val data = badge(CypherBundle.message("model.picker.dataCollected"))
    private val latest = badge(CypherBundle.message("model.picker.details.latest"))
    private val badges = Stack.horizontal(UiStyle.Gap.xs()).next(free).next(byok).next(data).next(latest)
    private val props = RowsSection(CypherBundle.message("model.picker.details.properties"))
    private val bench = RowsSection(CypherBundle.message("model.picker.details.terminalBench"))
    private val caps = TagsSection(CypherBundle.message("model.picker.details.capabilities"))
    private val desc = MarkdownSection(CypherBundle.message("model.picker.details.description"))
    private val routeText = JBLabel().apply {
        foreground = UIUtil.getLabelForeground()
        setAllowAutoWrapping(true)
    }
    private val route = Stack.vertical(UiStyle.Gap.xs())
        .next(heading(CypherBundle.message("model.picker.details.autoRouting")))
        .next(routeText)
    private val ids = RowsSection(CypherBundle.message("model.picker.details.ids"))
    ...
    fun update(value: ModelPicker.Item?) {
        ...
        syncRouting(value)
        ids.update(listOf(
            CypherBundle.message("model.picker.details.providerId") to value.provider,
            CypherBundle.message("model.picker.details.modelId") to value.id,
        ))
        refresh()
    }
    ...
    private fun syncStar(value: ModelPicker.Item) {
        ...
        star.toolTipText = if (selected) {
            CypherBundle.message("model.picker.favorite.remove")
        } else {
            CypherBundle.message("model.picker.favorite.add")
        }
    }
    ...
    private fun properties(item: ModelPicker.Item): List<Pair<String, String>> {
        ...
        return buildList {
            item.releaseDate?.let { add(CypherBundle.message("model.picker.details.released") to date(it)) }
            if (!item.free) {
                item.cost?.let { cost ->
                    add(CypherBundle.message("model.picker.details.input") to price(cost.input))
                    add(CypherBundle.message("model.picker.details.output") to price(cost.output))
                    add(CypherBundle.message("model.picker.details.cached") to cached(cost.input, cost.cache?.read))
                    add(CypherBundle.message("model.picker.details.average") to price(average(cost.input, cost.output, cost.cache?.read)))
                } ?: run {
                    item.inputPrice?.let { add(CypherBundle.message("model.picker.details.input") to price(it)) }
                    item.outputPrice?.let { add(CypherBundle.message("model.picker.details.output") to price(it)) }
                }
            }
            ctx?.let { add(CypherBundle.message("model.picker.details.context") to context(it)) }
        }
    }
    ...
    private fun bench(item: ModelPicker.Item): List<Pair<String, String>> {
        ...
        return listOf(
            CypherBundle.message("model.picker.details.completion") to percent(bench.overallScore),
            CypherBundle.message("model.picker.details.costAttempt") to attempt(bench.avgAttemptCostUsd),
        )
    }
    ...
    private fun capabilities(item: ModelPicker.Item): List<String> {
        ...
        return buildList {
            if (cap?.reasoning == true || item.reasoning) add(CypherBundle.message("model.picker.details.reasoning"))
            val input = cap?.input
            if (input?.text == true) add(CypherBundle.message("model.picker.details.modality.text"))
            if (input?.image == true) add(CypherBundle.message("model.picker.details.modality.image"))
            if (input?.audio == true) add(CypherBundle.message("model.picker.details.modality.audio"))
            if (input?.video == true) add(CypherBundle.message("model.picker.details.modality.video"))
            if (input?.pdf == true) add(CypherBundle.message("model.picker.details.modality.pdf"))
            if (item.attachment) add(CypherBundle.message("model.picker.details.attachments"))
        }
    }
...
private fun price(value: Double): String {
    if (value == 0.0) return CypherBundle.message("model.picker.free")
...
private fun cached(input: Double, read: Double?): String {
    ...
    return CypherBundle.message("model.picker.details.notSupported")
}
