...
import ai.cypher.client.ui.PickerButton
...
class ModelPicker : PickerButton() {
    ...
    var emptyText: String = CypherBundle.message("settings.models.notSet")
    ...
    init {
        isEnabled = false
        text = " "
        toolTipText = CypherBundle.message("model.picker.tooltip")
    ...
    private fun refresh() {
        ...
        toolTipText = if (item?.let(ModelText::collectsData) == true) ModelText.dataCollectedTooltip() else CypherBundle.message("model.picker.tooltip")
        isEnabled = true
    ...
    private fun showPopup() {
        ...
        val list = JBList(model).apply {
            ...
            isFocusable = false
            emptyText.text = CypherBundle.message("model.picker.no.matches")
        ...
        val search = SearchTextField(false).apply {
            textEditor.emptyText.text = CypherBundle.message("model.picker.search")
        }
        ...
        fun syncExpand() {
            ...
            expand.toolTipText = if (expanded) {
                CypherBundle.message("model.picker.details.minimize")
            } else {
                CypherBundle.message("model.picker.details.maximize")
            }
...
internal const val MODEL_PICKER_EXPANDED_KEY = "cypher.model.picker.expanded"
...
internal object ModelText {
    private val small = setOf("cypher-auto/small", "auto-small")
    ...
    fun buttonLabel(item: ModelPicker.Item): String {
        ...
        if (item.provider == "cypher") return part
        val provider = item.providerName.trim()
        if (provider.isEmpty()) return part
        return "$provider / $part"
    }

    fun small(item: ModelPicker.Item): Boolean = item.provider == "cypher" && item.id in small

    fun providerSort(id: String): Int = if (id == "cypher") 0 else 1

    fun dataCollected(): String = CypherBundle.message("model.picker.dataCollected")

    fun dataCollectedTooltip(): String = XmlStringUtil.wrapInHtmlLines(
        CypherBundle.message("model.picker.tooltip"),
        CypherBundle.message("model.picker.dataCollected.current"),
    )

    fun freeLabel(): String = CypherBundle.message("model.picker.free")
