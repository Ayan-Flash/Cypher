...
class AgentBehaviorConfigurable : SearchableConfigurable {
    ...
    override fun getDisplayName(): String = CypherBundle.message("settings.agentBehavior.displayName")

    override fun createComponent(): JComponent {
        val panel = Stack.vertical()
        panel.border = JBUI.Borders.empty(UiStyle.Gap.lg(), 0, 0, 0)
        val desc = JBLabel(CypherBundle.message("settings.agentBehavior.description"))
        desc.border = JBUI.Borders.emptyBottom(UiStyle.Gap.pad())
        panel.next(desc)
        listOf(
            CypherBundle.message("settings.agentBehavior.agents.displayName") to AgentsConfigurable.ID,
            CypherBundle.message("settings.agentBehavior.mcp.displayName") to McpConfigurable.ID,
        ).forEach { (label, id) ->
    ...
    companion object {
        const val ID = "ai.cypher.jetbrains.settings.agentBehavior"
    }
}
