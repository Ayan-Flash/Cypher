...
import ai.cypher.client.settings.profile.formatBalance
...
internal class SessionAccountOverlay(
    ...
    private val profileBtn = HoverIcon().apply {
        ...
        toolTipText = CypherBundle.message("action.Cypher.ShowProfile.description")
        accessibleContext.accessibleName = CypherBundle.message("action.Cypher.ShowProfile.text")
    ...
    @RequiresEdt
    private fun updateLoggedIn(prof: ai.cypher.rpc.dto.ProfileDto, switching: Boolean, target: String?): Boolean {
        var layout = false

        val orgs = prof.organizations
        val personal = prof.hasPersonalAccount
        val next = (if (personal) listOf(AccountChoice(null, CypherBundle.message("profile.personalAccount"))) else emptyList()) +
        ...
        val tip = if (switching) {
            CypherBundle.message("profile.switchingAccount")
        } else {
            CypherBundle.message("session.account.switcher")
        }
    ...
    @RequiresEdt
    private fun syncBalance(prof: ai.cypher.rpc.dto.ProfileDto): Boolean {
        var layout = false
        ...
        if (next == null) {
            ...
            val tip = CypherBundle.message("session.account.balance", next)
