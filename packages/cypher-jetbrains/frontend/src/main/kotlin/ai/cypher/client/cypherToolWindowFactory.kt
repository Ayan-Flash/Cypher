...
import ai.cypher.client.session.SessionSidePanelManager
...
/**
 * Creates the Cypher CLI tool window and delegates session content management.
 *
 ...
 * completes.
 */
class CypherToolWindowFactory : ToolWindowFactory, DumbAware {
    override fun createToolWindowContent(project: Project, toolWindow: ToolWindow) {
        project.service<CypherToolWindowSetupService>().create(toolWindow)
    }
}

private val LOG = CypherLog.create(CypherToolWindowFactory::class.java)

@Service(Service.Level.PROJECT)
internal class CypherToolWindowSetupService(
    ...
    fun create(toolWindow: ToolWindow) {
        ...
        try {
            val workspaces = service<CypherWorkspaceService>()
            ...
            LOG.error("Failed to create Cypher tool window content", e)
        }
    }
    ...
    private fun setup(
        ...
        try {
            ...
            val actions = listOfNotNull(
                ActionManager.getInstance().getAction("Cypher.NewSession"),
                ActionManager.getInstance().getAction("Cypher.History"),
                ActionManager.getInstance().getAction("Cypher.Settings"),
            )
            toolWindow.setTitleActions(actions)
        } catch (e: Exception) {
            Telemetry.send("Tool Window Setup Failed", mapOf("stage" to "setup", "errorClass" to e::class.java.name))
            LOG.error("Failed to set up Cypher tool window content", e)
        }
    }
}
