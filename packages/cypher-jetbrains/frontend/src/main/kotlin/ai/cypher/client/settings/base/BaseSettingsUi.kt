...
internal abstract class BaseSettingsUi<C : BaseContentPanel, D, P, R, W>(
    ...
    protected var appState: CypherAppStateDto = app.state.value
        private set
    ...
    @RequiresEdt
    private fun updateApp(state: CypherAppStateDto) {
        appState = state
        if (state.status != CypherAppStatusDto.READY) {
    ...
    private fun load() {
        ...
        if (appState.status != CypherAppStatusDto.READY || workspaceLoading || workspaceLoaded) return
    ...
    @RequiresEdt
    protected abstract fun draft(state: CypherAppStateDto): D
    ...
    @RequiresEdt
    protected open fun onSaveFailedAfterDispose(change: P) = CypherNotifications.error(failedText())
    ...
    @RequiresEdt
    protected open fun unavailable(state: CypherAppStateDto) = Unit
