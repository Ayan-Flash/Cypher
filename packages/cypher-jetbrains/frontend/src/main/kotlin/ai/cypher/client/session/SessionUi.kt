...
import ai.cypher.client.migration.CypherMigrationService
import ai.cypher.client.migration.MigrationUiController
import ai.cypher.client.migration.MigrationUiState
import ai.cypher.client.migration.ui.MigrationOverlayPanel
...
import ai.cypher.client.session.ui.ConnectionPanel
import ai.cypher.client.session.ui.empty.EmptySessionPanel
import ai.cypher.client.session.ui.LoadingPanel
import ai.cypher.client.session.ui.ReasoningPicker
import ai.cypher.client.session.ui.RevertBanner
import ai.cypher.client.session.ui.mode.ModePicker
import ai.cypher.client.session.ui.model.ModelPicker
import ai.cypher.client.session.ui.prompt.CypherPromptCompletionProvider
import ai.cypher.client.session.ui.prompt.MentionAction
import ai.cypher.client.session.ui.prompt.PromptPanel
import ai.cypher.client.session.ui.prompt.SlashAction
import ai.cypher.client.session.ui.prompt.mentionParts as promptMentionParts
import ai.cypher.client.session.ui.account.SessionAccountOverlay
import ai.cypher.client.session.ui.popup.HeaderPopupController
import ai.cypher.client.session.ui.SessionDropOverlay
import ai.cypher.client.session.ui.SessionRootPanel
import ai.cypher.client.session.ui.SessionMessageListPanel
import ai.cypher.client.session.ui.attachment.AttachmentEditorKind
import ai.cypher.client.session.ui.attachment.attachmentParams
import ai.cypher.client.session.ui.attachment.ensureAttachmentEditorKind
import ai.cypher.client.session.ui.attachment.isEmbeddedAttachment
import ai.cypher.client.session.ui.header.SessionHeaderPanel
import ai.cypher.client.session.ui.selection.SessionContextMenu
import ai.cypher.client.session.ui.selection.SessionHoverCopyOverlay
...
import ai.cypher.client.session.controller.EVENT_FLUSH_MS
import ai.cypher.client.session.controller.SessionController
import ai.cypher.client.session.controller.SessionControllerEvent
import ai.cypher.client.session.ui.style.SessionUiStyle
import ai.cypher.client.session.views.LoginRequiredView
import ai.cypher.client.session.views.permission.PermissionView
import ai.cypher.client.session.views.question.QuestionView
...
import ai.cypher.client.vfs.CypherVfsManager
...
class SessionUi(
    ...
    private val migration: MigrationUiController = service<CypherMigrationService>(),
    private val timers: UiTimerSource = UiTimers,
) : JPanel(BorderLayout()), Disposable, SessionEditorStyleTarget, UiDataProvider {

    companion object {
        private val LOG = CypherLog.create(SessionUi::class.java)
    ...
    private val flushMs =
        Registry.intValue("cypher.session.flushMs", EVENT_FLUSH_MS.toInt())
            .takeIf { it > 0 }
            ?.toLong()
            ?: EVENT_FLUSH_MS
    ...
    private val controller = SessionController(
        ...
        comp = this,
        flushMs = flushMs,
        condense = Registry.`is`("cypher.session.condense", true),
        displayMs = displayMs,
    ...
    private lateinit var completion: CypherPromptCompletionProvider
    ...
    private fun buildUi() {
        ...
        completion = CypherPromptCompletionProvider(
            workspace = workspace,
            service = workspaces,
    ...
    private fun syncPromptRevert() {
        ...
        val text = msg.parts.values.filterIsInstance<ai.cypher.client.session.model.Text>().firstOrNull()?.content?.toString() ?: return
        prompt.setText(text)
    ...
    private fun slashActions(): List<SlashAction> {
        val fns: Map<SlashAction.Spec, () -> Unit> = mapOf(
            ...
            SlashAction.SETTINGS to { openCypherSettings() },
            SlashAction.HELP to { BrowserUtil.browse("https://cypher.ai/docs") },
        )
    ...
    private fun bind(spec: SlashAction.Spec, action: () -> Unit) = SlashAction(
        spec.name,
        CypherBundle.message(spec.descriptionKey),
        spec.hints,
        {
    ...
    private fun openAttachment(messageId: String, item: FileAttachment) {
        ...
        if (isEmbeddedAttachment(url)) {
            ...
            LOG.info("kind=attachment-open route=cypher-vfs session=$id message=$messageId part=${item.id} name=${attachmentName(item)}")
            ensureAttachmentEditorKind()
            project.service<CypherVfsManager>().open(
    ...
    private fun openCypherSettings() {
        ShowSettingsUtil.getInstance().showSettingsDialog(
            project,
            Predicate { cfg: Configurable ->
                cfg is ConfigurableWithId && cfg.getId() == CypherSettingsConfigurable.ID
            },
            { _: Configurable -> },
        )
    }
...
private fun ai.cypher.client.session.model.QuestionItem.planFollowup() =
