package ai.cypher.client.session.views.question
...
class QuestionView(
    ...
    private val back = HoverIcon().apply {
        ...
        toolTipText = CypherBundle.message("session.question.back")
    ...
    private val fwd = HoverIcon().apply {
        ...
        icon = ico
        disabledIcon = IconLoader.getDisabledIcon(ico)
        toolTipText = CypherBundle.message("session.question.next")
    ...
    private fun syncPage() {
        ...
        body.removeAll()
        if (review(q)) {
            card.setHeader(CypherBundle.message("session.question.review.title"))
            addReview(q)
        } else {
            val item = q.items[idx]
            val hint = CypherBundle.message(
    ...
    private fun syncHeader(q: Question) {
        ...
        summary.text = CypherBundle.message("session.question.summary", shown, total)
    ...
    private fun syncFooter(q: Question) {
        ...
        actions.add(BaseQuestionView.Action(ID_DISMISS, CypherBundle.message("session.question.dismiss"), primary = false) { doReject() })

        if (review(q)) {
            actions.add(BaseQuestionView.Action(ID_BACK, CypherBundle.message("session.question.back"), primary = false) { goBack() })
            actions.add(BaseQuestionView.Action(ID_MAIN, CypherBundle.message("session.question.submit"), primary = true) { doReply() })
        } else {
            val label = when {
                direct(q) -> CypherBundle.message("session.question.submit")
                lastItem(q) -> CypherBundle.message("session.question.review")
                else -> CypherBundle.message("session.question.next")
            }
    ...
    private fun reviewRow(item: QuestionItem, i: Int): JPanel {
        ...
        val answer = text(
            joined.ifBlank { CypherBundle.message("session.question.review.notAnswered") },
            UiStyle.Colors.fg(),
            true,
        )
    ...
    private fun customRow(item: QuestionItem, set: MutableSet<String>): JPanel {
        ...
        val label = text(CypherBundle.message("session.question.custom.label"), UiStyle.Colors.fg(), true)
    ...
    private fun buildCustomEditor(): SessionEditorTextField {
        ...
        ed.setPlaceholder(CypherBundle.message("session.question.custom.placeholder"))
