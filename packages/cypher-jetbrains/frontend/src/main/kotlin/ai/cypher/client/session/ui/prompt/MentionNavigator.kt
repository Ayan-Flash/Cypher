...
internal class MentionNavigator(
    ...
    private val provider: CypherPromptCompletionProvider,
    ...
    fun install() {
        ...
        object : DumbAwareAction(CypherBundle.message("prompt.mention.goto")) {
    ...
    private fun syncTooltip(hit: CypherPromptCompletionProvider.MentionHit?) {
        val text = hit?.takeIf { !it.resolved }?.let { CypherBundle.message("prompt.mention.unresolved", it.value) }
        if (editor.contentComponent.toolTipText != text) editor.contentComponent.toolTipText = text
    }

    private fun syncLink(hit: CypherPromptCompletionProvider.MentionHit?, mouse: MouseEvent) {
