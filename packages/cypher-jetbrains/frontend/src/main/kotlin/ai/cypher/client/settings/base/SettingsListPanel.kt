...
internal abstract class SettingsListPanel(
    ...
    protected val view = SettingsListView(CypherBundle.message("settings.agentBehavior.empty"), cfg) { key, id ->
        onCell(key, id)
    }
    ...
    protected open fun emptyText(): String = CypherBundle.message("settings.agentBehavior.empty")

    protected open fun loadingText(): String = CypherBundle.message("settings.agentBehavior.loading")
    ...
    private fun toolbar(): JComponent {
        ...
        if (showRefresh()) {
            ...
            actions += SettingsToolbarAction(
                CypherBundle.message("settings.agentBehavior.refresh"),
                CypherBundle.message("settings.agentBehavior.refresh.description"),
    ...
    protected suspend fun waitForReady() {
        val flow = service<CypherAppService>().state
        val saw = withTimeoutOrNull(RELOAD_START_TIMEOUT_MS) {
            flow.first { it.status != CypherAppStatusDto.READY }
        } != null
        if (!saw) return
    ...
    private companion object {
        val LOG = CypherLog.create(SettingsListPanel::class.java)
