...
internal const val DASHBOARD_URL = "https://app.cypher.ai/profile"
internal const val TOP_UP_URL = "https://app.cypher.ai/credits"
internal const val PASS_URL = "https://cypher.ai/pricing/cypher-pass"
...
internal class ProfileUi(
    ...
    private val account = LoggedInProfileUi(
        ...
        pass = {
            telemetry("Cypher Pass Opened", mapOf("surface" to "settings"))
            browse(PASS_URL)
        },
        logout = ::logout,
     ...
     * account content) when [CypherAppStateDto.progress]`.profile` is [ProfileStatusDto.PENDING],
     ...
     */
    @RequiresEdt
    fun update(state: CypherAppStateDto) {
        checkEdt()
     ...
     */
    @RequiresEdt
    fun update(profile: ProfileDto?, status: CypherAppStatusDto) {
        checkEdt()
        this.status = status
    ...
    private fun targetCard(): Card {
        ...
        val transientLoad = s == CypherAppStatusDto.DOWNLOADING || s == CypherAppStatusDto.CONNECTING || s == CypherAppStatusDto.LOADING || s == CypherAppStatusDto.MIGRATION_REQUIRED
        if (transientLoad && shown == Card.LOGGED_IN) return Card.LOGGED_IN
        return when {
            s == CypherAppStatusDto.DISCONNECTED || transientLoad -> Card.LOGGED_OUT
            s == CypherAppStatusDto.ERROR -> Card.LOGGED_OUT
     ...
     * Called from [ai.cypher.client.settings.profile.UserProfileConfigurable.disposeUIResources].
     */
    @RequiresEdt
    fun dispose() {
        attempt++
...
internal fun compactLoginError(e: Exception): String {
    val msg = e.message?.trim() ?: return CypherBundle.message("profile.login.failed")
    val lower = msg.lowercase()
    if (HTML_MARKERS.any { lower.contains(it) }) {
        val status = HTTP_STATUS_RE.find(msg)?.groupValues?.getOrNull(1)
        return if (status != null) "${CypherBundle.message("profile.login.failed")} ($status)"
        else CypherBundle.message("profile.login.failed")
    }
    val norm = msg.replace(Regex("\\s+"), " ")
    val summary = norm.take(180)
    return if (summary.isNotBlank()) summary else CypherBundle.message("profile.login.failed")
}
