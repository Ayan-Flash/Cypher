...
class CypherPromptCompletionProvider(
    private val workspace: Workspace,
    private val service: CypherWorkspaceService,
    ...
    private fun slash(prefix: String, result: CompletionResultSet) {
        ...
        result.withPrefixMatcher(PlainPrefixMatcher.ALWAYS_TRUE)
            .addElement(info(prefix, CypherBundle.message("prompt.completion.noMatches")))
    }
    ...
    private fun mention(prefix: String, result: CompletionResultSet) {
        ...
        if (search.indexing) {
            val msg = CypherBundle.message("prompt.mention.indexing")
        ...
        if (known.isEmpty() && search.files.isEmpty()) {
            val msg = CypherBundle.message("prompt.completion.noMatches")
            out.addElement(info(prefix, msg))
        }
    }
