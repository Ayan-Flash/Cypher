...
class SessionSidePanelManager(
    ...
    private val resolve: (String) -> Workspace = { dir -> service<CypherWorkspaceService>().workspace(dir) },
    private val status: () -> Map<String, SessionActivityKind> = { project.service<CypherSessionService>().activity() },
    ...
    private fun schedule(ui: SessionUi) {
        cancel(ui)
        val delay = Registry.intValue("cypher.session.inactive.disposeTimeoutMs").coerceAtLeast(0)
