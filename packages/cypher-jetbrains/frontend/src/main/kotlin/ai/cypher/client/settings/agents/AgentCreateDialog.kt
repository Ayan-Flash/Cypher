...
internal class AgentCreateDialog(private val names: Collection<String>) : DialogWrapper(true), AgentCreateDialogHandle {
    ...
    private val mode = ComboBox(arrayOf(CypherCliParser.MODE_PRIMARY, CypherCliParser.MODE_SUBAGENT, CypherCliParser.MODE_ALL)).apply {
        selectedItem = CypherCliParser.MODE_PRIMARY
    }
    ...
    init {
        title = CypherBundle.message("settings.agentBehavior.agents.create.title")
        init()
        initValidation()
    }
    ...
    override fun result(): AgentCreateDto = AgentCreateDto(
        ...
        mode = mode.selectedItem?.toString() ?: CypherCliParser.MODE_PRIMARY,
    ...
    override fun createCenterPanel(): JComponent {
        ...
        SettingsRows().apply {
            row(SettingsRow(
                CypherBundle.message("settings.agentBehavior.agents.create.name"),
                description = CypherBundle.message("settings.agentBehavior.agents.create.name.description"),
                value = id,
            ))
            row(SettingsRow(
                CypherBundle.message("settings.agentBehavior.agents.create.mode"),
                description = CypherBundle.message("settings.agentBehavior.agents.create.mode.description"),
                value = mode,
            ))
            row(SettingsRow(
                CypherBundle.message("settings.agentBehavior.agents.create.scope"),
                description = CypherBundle.message("settings.agentBehavior.agents.create.scope.description"),
                value = scope,
            ))
            row(SettingsStackedRow(
                CypherBundle.message("settings.agentBehavior.agents.create.description"),
                description = CypherBundle.message("settings.agentBehavior.agents.create.description.description"),
                value = box(description),
            ))
            row(SettingsStackedRow(
                CypherBundle.message("settings.agentBehavior.agents.create.prompt"),
                description = CypherBundle.message("settings.agentBehavior.agents.create.prompt.description"),
                value = box(prompt),
            ))
            panel.next(this)
        }
    ...
    override fun getDimensionServiceKey(): String = "Cypher.AgentCreateDialog"

    override fun doValidateAll(): List<ValidationInfo> = validateAgentCreate(result(), names).map { err ->
        ValidationInfo(CypherBundle.message(err.key), component(err.field))
    }
    ...
    private class PromptField : EditorTextField(
        ...
        init {
            ...
            setPlaceholder(CypherBundle.message("settings.agentBehavior.agents.create.prompt.placeholder"))
...
private fun projectLabel() = CypherBundle.message("settings.agentBehavior.agents.create.scope.project")

private fun globalLabel() = CypherBundle.message("settings.agentBehavior.agents.create.scope.global")
