...
class RevertBanner(
    ...
    private val hint = JBLabel(CypherBundle.message("revert.banner.hint")).apply {
        font = JBFont.small()
    }

    private val notice = JBLabel(CypherBundle.message("revert.banner.filesNotRestored")).apply {
        font = JBFont.small()
    }

    init {
        isOpaque = false
        card.setHeaderIcon(AllIcons.Actions.Back, CypherBundle.message("revert.message.rollback"))
        body.next(files).next(hint).next(notice)
        card.setContent(body)
        card.setActions(listOf(
            BaseQuestionView.Action("redo", CypherBundle.message("revert.banner.redo"), primary = false) { redoAction() },
            BaseQuestionView.Action("all", CypherBundle.message("revert.banner.redo.all"), primary = false) { redoAllAction() },
        ))
    ...
    fun update() {
        ...
        card.setHeader(CypherBundle.message(if (total == 1) "revert.banner.count.one" else "revert.banner.count.other", total))
