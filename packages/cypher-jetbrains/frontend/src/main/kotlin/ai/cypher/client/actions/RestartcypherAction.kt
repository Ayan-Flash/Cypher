...
class RestartCypherAction : AnAction(), DumbAware {
    override fun actionPerformed(e: AnActionEvent) {
        Telemetry.send("CLI Restart Clicked", mapOf("surface" to "settings"))
        service<CypherAppService>().restartAsync()
    }

    override fun update(e: AnActionEvent) {
        e.presentation.isEnabled = true
        if (e.place == CypherActionPlaces.connectionRetryPopup()) {
            e.presentation.text = CypherBundle.message("action.Cypher.Restart.cli.text")
        }
    }
}
