...
import ai.cypher.client.settings.base.SettingsDraftPage
import ai.cypher.client.settings.base.SettingsDraftState
...
class AgentsConfigurable : AgentBehaviorConfigurableBase<JComponent>() {
    ...
    override fun getDisplayName(): String = CypherBundle.message("settings.agentBehavior.agents.displayName")
    ...
    companion object { const val ID = "ai.cypher.jetbrains.settings.agentBehavior.agents" }
}
...
internal class AgentsSettingsUi(
    ...
    private val app get() = service<CypherAppService>()
    ...
    override suspend fun fetch(): List<SettingsListItem> {
        val agents = service<CypherAgentBehaviorService>().agents(dir)
        models = items(service<CypherWorkspaceService>().models(dir).providers)
        val next = agentsDraft(service<CypherAppService>().state.value.config, agents)
    ...
    override fun toolbarRight(): JComponent = Stack.horizontal(UiStyle.Gap.sm())
        .next(JBLabel(CypherBundle.message("settings.agentBehavior.agents.default")))
        .next(makePicker())
    ...
    override fun searchPlaceholder() = CypherBundle.message("settings.agentBehavior.agents.search")
    ...
    override fun applyDraft() {
        ...
        if (!launch("apply") { id ->
            ...
            for (name in target.deleted) {
                if (!behavior.removeAgent(dir, name)) {
                    failed = CypherBundle.message("settings.agentBehavior.agents.delete.failed")
                    break
                }
            ...
            if (failed == null) {
                for ((name, input) in target.created) {
                    if (!behavior.createAgent(dir, input)) {
                        failed = CypherBundle.message("settings.agentBehavior.agents.create.failed")
                        break
                    }
            ...
            if (failed == null) {
                for ((name, item) in target.imported) {
                    if (app.updateConfig(item) == null) {
                        failed = CypherBundle.message("settings.agentBehavior.agents.import.failed")
                        break
                    }
            ...
            if (failed == null) {
                ...
                if (change != null && app.updateConfig(change) == null) {
                    failed = CypherBundle.message("settings.agentBehavior.save.failed")
                }
            ...
            withContext(edt) {
                if (!active(id)) {
                    if (failed != null) CypherNotifications.error(failed)
                    return@withContext
                }
                details = agents
                if (failed != null) {
        ...
        showProgress(CypherBundle.message("settings.agentBehavior.saving"))
    }
    ...
    private fun rows(): List<SettingsListItem> = displayRows(base, draft).map { row ->
        ...
        object : SettingsListItem {
            ...
            override val badges = listOfNotNull(
                SettingsBadge(
                    CypherBundle.message("settings.agentBehavior.badge.notApplied"),
                    UiStyle.Badge.Secondary,
                ).takeIf { row.intent == AgentIntent.Modified || row.intent == AgentIntent.New },
                SettingsBadge(
                    CypherBundle.message("settings.agentBehavior.badge.willRemove"),
                    UiStyle.Badge.Alert,
                ...
                ).takeIf { CypherCliParser.isSubagent(item.mode) },
                SettingsBadge(
                    CypherBundle.message("settings.agentBehavior.badge.custom"),
                    UiStyle.Badge.Primary,
                ).takeIf { canDelete(item) },
                SettingsBadge(CypherBundle.message("settings.agentBehavior.badge.hidden")).takeIf { item.hidden },
                SettingsBadge(CypherBundle.message("settings.agentBehavior.badge.disabled")).takeIf { item.disable },
                SettingsBadge(
            ...
            override val cells = when (row.intent) {
                ...
                -> listOf(SettingsListCell(UNDO_CELL, CypherBundle.message("settings.agentBehavior.undo"), primary = true))
                AgentIntent.Unchanged,
                AgentIntent.Modified,
                -> listOfNotNull(
                    SettingsListCell(EDIT_CELL, CypherBundle.message("settings.agentBehavior.edit")),
                    SettingsListCell(
                    DELETE_CELL,
    ...
    private fun syncNames() {
        names = listOf("") + displayRows(base, draft)
            ...
            .map { it.agent }
            .filter { CypherCliParser.defaultAgentCandidate(it.mode, it.hidden) && !it.disable }
            .map { it.name }
    }
    ...
    private fun addAction(): DefaultActionGroup = DefaultActionGroup(
        CypherBundle.message("settings.agentBehavior.agents.add"),
        true,
    ).apply {
    ...
    internal inner class CreateAction : DumbAwareAction(CypherBundle.message("settings.agentBehavior.agents.create")) {
    ...
    internal inner class ImportAction : DumbAwareAction(CypherBundle.message("settings.agentBehavior.agents.import")) {
        override fun getActionUpdateThread() = ActionUpdateThread.EDT
        ...
        internal fun perform(file: VirtualFile? = choose(this@AgentsSettingsUi)) {
            ...
            showProgress(CypherBundle.message("settings.agentBehavior.agents.import.progress"))
        }

        private fun load(file: VirtualFile, names: Collection<String>): AgentImport {
            if (file.length > MAX_AGENT_IMPORT_SIZE) {
                throw SettingsMessageException(CypherBundle.message(AgentImportError.TOO_LARGE.key))
            }
            val text = String(file.contentsToByteArray(), StandardCharsets.UTF_8)
            return try {
                parseAgentImport(text, names)
            } catch (e: AgentImportException) {
                throw SettingsMessageException(CypherBundle.message(e.error.key))
            }
        }
    }
    ...
    private companion object {
        ...
        const val CYPHER_PROVIDER = "cypher"
    }
...
private fun chooseImportFile(_parent: JComponent): VirtualFile? {
    val descriptor = FileChooserDescriptor(true, false, false, false, false, false).apply {
        title = CypherBundle.message("settings.agentBehavior.agents.import.title")
        description = CypherBundle.message("settings.agentBehavior.agents.import.description")
