...
/**
 * Gear icon action placed in the Cypher tool window title bar.
 *
 * Looks up [Cypher.SettingsGroup] from [ActionManager] and shows it
 * as a popup. The group composition is declared in
 * `cypher.jetbrains.frontend.xml`.
 */
class CypherSettingsAction : AnAction() {

    companion object {
        const val GROUP_ID = "Cypher.SettingsGroup"

        internal fun popupGroup(group: ActionGroup): ActionGroup {
            return ActionGroupUtil.forceRecursiveUpdateInBackground(group)
        }

        internal fun refreshConfigTargets(e: AnActionEvent, service: CypherWorkspaceService): List<Job> {
