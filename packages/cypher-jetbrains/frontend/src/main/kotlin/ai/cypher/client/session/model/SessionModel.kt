package ai.cypher.client.session.model
 ...
 *
 * **EDT-only access** — no synchronization. [ai.cypher.client.session.controller.SessionController] guarantees all
...
class SessionModel {
    ...
    var app: CypherAppStateDto = CypherAppStateDto(CypherAppStatusDto.DISCONNECTED)
    var version: String? = null

    var workspace: CypherWorkspaceStateDto = CypherWorkspaceStateDto(CypherWorkspaceStatusDto.PENDING)
    ...
    @RequiresEdt
    fun isReady(): Boolean = app.status == CypherAppStatusDto.READY && workspace.status == CypherWorkspaceStatusDto.READY
...
private fun ai.cypher.rpc.dto.PartTimeDto.durationMs(): Long? {
