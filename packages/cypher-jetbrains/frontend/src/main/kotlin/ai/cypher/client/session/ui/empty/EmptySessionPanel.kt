...
class EmptySessionPanel(
    ...
    init {
        ...
        val logo = JBLabel(
            IconLoader.getIcon("/icons/cypher-content.svg", EmptySessionPanel::class.java),
        ).apply {
            horizontalAlignment = JBLabel.CENTER
        }
    ...
    internal fun feedbackText() = CypherBundle.message("feedback.button")
    ...
    internal fun explanationText() = CypherBundle.message("session.empty.welcome")
    ...
    internal open class ShowHistoryButton(
        text: String = CypherBundle.message("session.showHistory"),
    ...
    private fun welcomeHtml() = XmlStringUtil.wrapInHtml(
        "<div style='text-align:center'>${XmlStringUtil.escapeString(CypherBundle.message("session.empty.welcome"))}</div>"
    )
