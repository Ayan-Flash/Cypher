...
class CypherVfsManager(private val project: Project) {
    @RequiresEdt
    fun open(kind: String, params: Map<String, String> = emptyMap(), focus: Boolean = true): Boolean {
        ...
        if (ApplicationManager.getApplication().isUnitTestMode) {
            file.putUserData(FileEditorProvider.KEY, CypherFileEditorProvider())
        }
    ...
    fun close(kind: String, params: Map<String, String> = emptyMap()) {
        ...
        CypherVirtualFileSystem.getInstance().release(file.path)
    }
    ...
    private fun file(kind: String, params: Map<String, String>): CypherVirtualFile? {
        val path = CypherPath(kind, params)
        val fs = CypherVirtualFileSystem.getInstance()
        return fs.refreshAndFindFileByPath(fs.getPath(path)) as? CypherVirtualFile
    }
}
