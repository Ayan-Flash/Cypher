...
class HistoryPanel(
    ...
    private val repoOnly = JBCheckBox(CypherBundle.message("history.cloud.repo.only"), true).apply {
        isVisible = false
    ...
    private val localInfo = TabInfo(localPanel)
        .setText(CypherBundle.message("history.tab.local"))
        .setForeSideComponent(back())
    private val cloudInfo = TabInfo(cloudPanel)
        .setText(CypherBundle.message("history.tab.cloud"))
        .setForeSideComponent(back())
    ...
    private fun search(model: HistoryModel<out HistoryItem>) = SearchTextField(false).apply {
        textEditor.emptyText.text = CypherBundle.message("history.search.placeholder")
    ...
    private fun back(): JComponent {
        val label = CypherBundle.message("history.back")
    ...
    private fun localList() = HistoryList(controller.local).apply {
        ...
        cursor = Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)
        emptyText.text = CypherBundle.message("history.empty")
    ...
    private fun <T : HistoryItem> syncList(list: JBList<T>, model: HistoryModel<T>) {
        ...
        list.emptyText.text = when {
            model.loading -> CypherBundle.message("history.loading")
            model.error != null -> model.error.orEmpty()
            else -> CypherBundle.message("history.empty")
        }
    }
    ...
    private fun installContextMenu(list: JBList<out HistoryItem>) {
        val group = ActionManager.getInstance().getAction("Cypher.History.ContextMenu")
    ...
    private fun confirm(item: LocalHistoryItem) {
        ...
        val result = com.intellij.openapi.ui.Messages.showYesNoDialog(
            this,
            CypherBundle.message("history.delete.confirm.message", title(item)),
            CypherBundle.message("history.delete.confirm.title"),
    ...
    internal fun confirmDelete(items: List<LocalHistoryItem>) {
        ...
        val msg = if (active.size == 1)
            CypherBundle.message("history.delete.confirm.message", title(active[0]))
        else
            CypherBundle.message("history.delete.confirm.message.multiple", active.size)
        val result = com.intellij.openapi.ui.Messages.showYesNoDialog(
            this,
            msg,
            CypherBundle.message("history.delete.confirm.title"),
    ...
    private class LoadMoreButton : JButton(CypherBundle.message("history.cloud.load.more")) {
