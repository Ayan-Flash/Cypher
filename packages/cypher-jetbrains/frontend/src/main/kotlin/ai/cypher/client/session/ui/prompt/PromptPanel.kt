...
import ai.cypher.client.actions.SendPromptAction
import ai.cypher.client.actions.StopSessionAction
...
import ai.cypher.client.session.model.PromptAttachment
import ai.cypher.client.session.model.PromptAttachmentExtractor
...
import ai.cypher.client.ui.iconButton
...
class PromptPanel(
    ...
    private val completion: CypherPromptCompletionProvider? = null,
    ...
    companion object {
        private val LOG = CypherLog.create(PromptPanel::class.java)
    ...
    private val reset = HoverIcon().apply {
        ...
        toolTipText = CypherBundle.message("model.picker.reset")
        accessibleContext.accessibleName = CypherBundle.message("model.picker.reset")
        isVisible = false
    ...
    private val enhance = HoverIcon().apply {
        icon = WAND_ICON
        toolTipText = CypherBundle.message("prompt.action.enhance")
        accessibleContext.accessibleName = CypherBundle.message("prompt.action.enhance")
    ...
    private fun key(kind: CypherPromptCompletionProvider.HighlightKind): TextAttributesKey = when (kind) {
        CypherPromptCompletionProvider.HighlightKind.MENTION -> MENTION_KEY
        CypherPromptCompletionProvider.HighlightKind.COMMAND -> COMMAND_KEY
        CypherPromptCompletionProvider.HighlightKind.INVALID -> INVALID_KEY
    }
    ...
    private fun enhance() {
        ...
        if (source.isBlank()) {
            editor.text = CypherBundle.message("prompt.action.enhance.description")
            syncEditorHeight()
            focus()
            return
        }
    ...
    private fun completeEnhancement(id: Long, source: String, result: Result<String>) {
        ...
        result.onSuccess {
            ...
            CypherNotifications.error(
                project,
                CypherBundle.message("prompt.action.enhance.failed"),
                CypherBundle.message("prompt.action.enhance.failed.description"),
            )
        }
    }
    ...
    private fun syncEnhance() {
        ...
        enhance.toolTipText = if (enhancing) {
            CypherBundle.message("prompt.action.enhance.loading")
        } else {
            CypherBundle.message("prompt.action.enhance")
        }
    }
        ...
        cs.launch {
            try {
                ...
                withContext(Dispatchers.Main) {
                    ...
                    notify(CypherBundle.message("prompt.attachment.send.failed", e.message ?: e.javaClass.simpleName))
                }
            }
        }
    }
    ...
    private fun installCompletionShortcut(ed: com.intellij.openapi.editor.Editor) {
        ...
        val action = completionAction ?: object : DumbAwareAction(CypherBundle.message("prompt.completion.action")) {
    ...
    private fun addAttachment(item: PromptAttachment) {
        if (!attachment && PromptAttachmentExtractor.media(item.mime)) {
            ...
            notify(CypherBundle.message("prompt.attachment.unsupported.model"))
            return
        }
    ...
    private fun promptDto(text: String, files: List<PromptPartDto>) = ai.cypher.rpc.dto.PromptDto(
        parts = buildList {
    ...
    private fun notify(text: String) {
        com.intellij.notification.Notification("Cypher CLI", text, com.intellij.notification.NotificationType.WARNING).notify(project)
    }
    ...
    private fun syncAutoApprove() {
        ...
        auto.toolTipText = if (autoApprove) {
            CypherBundle.message("prompt.action.autoApprove.enabled.tooltip")
        } else {
            CypherBundle.message("prompt.action.autoApprove.disabled.tooltip")
        }
        auto.accessibleContext.accessibleName = if (autoApprove) {
            CypherBundle.message("prompt.action.autoApprove.disable")
        } else {
            CypherBundle.message("prompt.action.autoApprove.enable")
        }
        auto.repaint()
    }
    ...
    private fun tooltip(): String {
        val id = if (busy) StopSessionAction.ID else SendPromptAction.ID
        val text = if (busy) {
            CypherBundle.message("prompt.button.stop")
        } else {
            CypherBundle.message("prompt.button.send")
        }
        ...
        return XmlStringUtil.wrapInHtml(
            XmlStringUtil.escapeString(tip) + "<br>" +
                XmlStringUtil.escapeString(CypherBundle.message("prompt.button.send.tooltip.stop", stop))
        )
    }
    ...
    private fun placeholder(): String {
        ...
        if (send.isNotEmpty() && line.isNotEmpty()) {
            return CypherBundle.message("prompt.placeholder.with.shortcuts", send, line)
        }
        if (send.isNotEmpty()) return CypherBundle.message("prompt.placeholder.with.send", send)
        if (line.isNotEmpty()) return CypherBundle.message("prompt.placeholder.with.newline", line)
        return CypherBundle.message("prompt.placeholder")
    }
