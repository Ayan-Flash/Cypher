 ...
 * enters [ai.cypher.client.session.model.SessionState.LoginRequired].
 *
 * Mirrors the anchored placement of [PermissionView] and [question.QuestionView]:
 * it stays as a stable child inside [ai.cypher.client.session.ui.SessionMessageListPanel]
...
class LoginRequiredView(
    ...
    init {
        ...
        card.setHeader(CypherBundle.message("session.login.required.title"))
        card.setActions(listOf(
            BaseQuestionView.Action(ID_DISMISS, CypherBundle.message("session.login.required.dismiss"), primary = false) { dismiss() },
            BaseQuestionView.Action(ID_OPEN, CypherBundle.message("session.login.required.button"), primary = true) { openProfile() },
        ))
    ...
    internal fun openProfileButton() = button(CypherBundle.message("session.login.required.button"))
    internal fun dismissButton() = button(CypherBundle.message("session.login.required.dismiss"))
