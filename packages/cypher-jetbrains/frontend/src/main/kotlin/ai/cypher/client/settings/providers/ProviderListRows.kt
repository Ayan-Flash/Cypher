...
import ai.cypher.client.session.ui.model.ModelSearch
...
internal data class ProviderListRow(
    ...
    override val badges: List<SettingsBadge>
        get() = when (provider.source) {
            "env" -> listOf(SettingsBadge(CypherBundle.message("settings.providers.badge.env")))
            else -> emptyList()
        }
...
internal fun providerListActionText(action: ProviderListAction) = when (action) {
    ProviderListAction.CONNECT -> CypherBundle.message("settings.providers.connect")
    ProviderListAction.OAUTH -> CypherBundle.message("settings.providers.oauth")
    ProviderListAction.DISCONNECT -> CypherBundle.message("settings.providers.disconnect")
    ProviderListAction.ENABLE -> CypherBundle.message("settings.providers.enable")
}
...
internal fun providerListRows(state: ProviderSettingsDto, query: String, disabledRows: Boolean = false): List<ProviderListRow> {
    ...
    rows += connected.map { ProviderListRow(it, CypherBundle.message("settings.providers.connected"), providerActions(it, state, disabled), connected = true, disabled = disabledRows) }
    rows += popular.map { ProviderListRow(it, CypherBundle.message("settings.providers.popular"), providerActions(it, state, disabled), disabled = disabledRows) }
    rows += all.map { ProviderListRow(it, CypherBundle.message("settings.providers.all"), providerActions(it, state, disabled), disabled = disabledRows) }
    return rows
}
...
internal fun providerActions(
    ...
    if (provider.id == CYPHER_PROVIDER_ID && configured(provider, state, state.connected.toSet())) return emptyList()
