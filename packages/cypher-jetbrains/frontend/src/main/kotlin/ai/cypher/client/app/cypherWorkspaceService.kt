...
class CypherWorkspaceService internal constructor(
    private val cs: CoroutineScope,
    private val rpc: CypherWorkspaceRpcApi?,
) {
    /** Platform constructor — resolves RPC from the service container. */
    constructor(cs: CoroutineScope) : this(cs, null)

    companion object {
        private val LOG = CypherLog.create(CypherWorkspaceService::class.java)
        private val INIT = CypherWorkspaceStateDto(CypherWorkspaceStatusDto.PENDING)
    }
    ...
    private suspend fun <T> call(block: suspend CypherWorkspaceRpcApi.() -> T): T {
        val api = rpc
        return if (api != null) block(api) else durable { block(CypherWorkspaceRpcApi.getInstance()) }
    }

    private fun <T> stream(block: suspend CypherWorkspaceRpcApi.() -> Flow<T>): Flow<T> = flow {
        val api = rpc
        if (api != null) block(api).collect { emit(it) }
        else durable { block(CypherWorkspaceRpcApi.getInstance()).collect { emit(it) } }
    }
