...
internal object HistoryTime {
    ...
    fun title(section: HistorySection): String = when (section) {
        HistorySection.TODAY -> CypherBundle.message("history.group.today")
        HistorySection.YESTERDAY -> CypherBundle.message("history.group.yesterday")
        HistorySection.WEEK -> CypherBundle.message("history.group.week")
        HistorySection.MONTH -> CypherBundle.message("history.group.month")
        HistorySection.OLDER -> CypherBundle.message("history.group.older")
    }
    ...
    fun relative(item: HistoryItem, now: Long = System.currentTimeMillis()): String {
        ...
        if (diff < MINUTE) return CypherBundle.message("history.time.moments")
        if (diff < HOUR) return CypherBundle.message("history.time.minutes", (diff / MINUTE).coerceAtLeast(1))
        if (diff < DAY) return CypherBundle.message("history.time.hours", (diff / HOUR).coerceAtLeast(1))
        if (diff < 7 * DAY) return CypherBundle.message("history.time.days", (diff / DAY).coerceAtLeast(1))
        ...
        if (months < 1) return CypherBundle.message("history.time.days", (diff / DAY).coerceAtLeast(1))
        if (months < 12) return CypherBundle.message("history.time.months", months)
        return CypherBundle.message("history.time.years", months / 12)
    }
