...
import ai.cypher.client.ui.RoundedContentPanel
...
internal class LoggedInProfileUi(
    ...
    companion object {
        private val LOG = CypherLog.create(LoggedInProfileUi::class.java)
    }
    ...
    private val logoLabel = JBLabel(IconLoader.getIcon("/icons/cypher-profile.svg", LoggedInProfileUi::class.java)).apply {
        name = "cypher.profile.logo.loggedIn"
        accessibleContext.accessibleName = CypherBundle.message("settings.cypher.displayName")
    }

    private val titleLabel = JBLabel(CypherBundle.message("profile.balance.title")).apply {
        foreground = UiStyle.Colors.weak()
    }
    ...
    private val refreshBtn = JButton(CypherBundle.message("profile.action.refresh"), AllIcons.Actions.Refresh)
        .also {
            it.isOpaque = false
    ...
    private val passTitleLabel = JBLabel(CypherBundle.message("profile.pass.title")).apply {
    ...
    private val passBonusLabel = JBLabel(CypherBundle.message("profile.pass.bonusLabel")).apply {
        foreground = UiStyle.Colors.weak()
        font = UiStyle.Fonts.small()
    }
    ...
    private val passRenewLabel = JBLabel(CypherBundle.message("profile.pass.renewsLabel")).apply {
        foreground = UiStyle.Colors.weak()
        font = UiStyle.Fonts.small()
    }
    ...
    private val passRenewRow = Stack.horizontal(UiStyle.Gap.md())
        ...
        .fill(UiStyle.Gap.md())
        .next(passRenewValue)
    private val passLink = ActionLink(CypherBundle.message("profile.pass.subscribe")) {
        pass()
    }.apply {
        name = "cypher.profile.passSubscribe"
    }
    ...
    private val balanceCard = RoundedContentPanel(UiStyle.Gap.pad(), UiStyle.Gap.xl()).apply {
        name = "cypher.profile.balanceCard"
        addToTop(titleLabel)
    ...
    val dashboardBtn = JButton(CypherBundle.message("profile.action.dashboard"))
        .also { it.addActionListener { dashboard() } }
    val topUpBtn = JButton(CypherBundle.message("profile.action.topUp"))
        .also { it.addActionListener { topUp() } }
    val logoutBtn = JButton(CypherBundle.message("profile.action.logout"))
    ...
    fun update(profile: ProfileDto) {
        ...
        val item = if (personal) profile.cypherPass else null
    ...
    @RequiresEdt
    private fun syncPass(item: ProfileCypherPassDto?, show: Boolean): Boolean {
        var changed = false
        ...
        if (item != null) {
            val usage = CypherBundle.message(
                "profile.pass.used",
    ...
    fun setRefreshing(refreshing: Boolean) {
        ...
        val text = if (refreshing) CypherBundle.message("profile.action.refreshing")
        else CypherBundle.message("profile.action.refresh")
    ...
    private fun applyOrganizations(profile: ProfileDto) {
        ...
        val keys: List<Pair<String?, String>> = (if (personal) listOf(null to CypherBundle.message("profile.personalAccount")) else emptyList()) +
