...
import ai.cypher.client.app.CypherProviderService
import ai.cypher.client.plugin.CypherBundle
import ai.cypher.client.settings.base.BaseContentPanel
import ai.cypher.client.settings.base.SettingsPanel
import ai.cypher.client.settings.base.SettingsListConfig
import ai.cypher.client.settings.base.SettingsToolbarAction
import ai.cypher.client.settings.base.SettingsListView
...
import ai.cypher.rpc.dto.CustomModelDto
import ai.cypher.rpc.dto.CustomProviderSaveDto
import ai.cypher.rpc.dto.ProviderAuthMethodDto
import ai.cypher.rpc.dto.ProviderAuthOptionDto
...
import ai.cypher.rpc.dto.ProviderSettingsProviderDto
...
internal class ProvidersSettingsUi(
    ...
    companion object {
        val LOG = CypherLog.create(ProvidersSettingsUi::class.java)
    }

    private val add = SettingsToolbarAction(
        CypherBundle.message("settings.providers.addCustom"),
        CypherBundle.message("settings.providers.addCustom.description"),
        AllIcons.General.Add,
        { !busy },
    ) { custom() }
    private val refresh = SettingsToolbarAction(
        CypherBundle.message("settings.providers.refresh"),
        CypherBundle.message("settings.providers.refresh.description"),
        AllIcons.Actions.Refresh,
        { !busy },
    ) { reload() }
    private val view = ProvidersContent(::connect, ::oauth, ::disconnect, ::enable)
    private val search = SearchTextField(false).apply {
        textEditor.emptyText.text = CypherBundle.message("settings.providers.search")
    }
    ...
    fun reload() {
        ...
        if (!launch("reload") { id ->
            val next = service<CypherProviderService>().state(directory)
    ...
    private fun syncLoading() {
        checkEdt()
        showProgress(CypherBundle.message("settings.providers.loading"))
    }
    ...
    private fun connect(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("connect provider=${provider.id}") { id ->
            val result = service<CypherProviderService>().connect(ProviderConnectDto(directory, provider.id, key, metadata))
    ...
    private fun oauth(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("authorize provider=${provider.id}") { id ->
            val ready = service<CypherProviderService>().authorize(ProviderOAuthAuthorizeDto(directory, provider.id, method))
                    ...
                    if (ready.method == "auto" && url != null) {
                        showOAuthDevice(
                            ...
                            DeviceOAuthInfo(
                                ...
                                expiresIn = (CypherProviderService.OAUTH_RPC_TIMEOUT_MS / 1000).toInt(),
            ...
            val result = service<CypherProviderService>().callback(ProviderOAuthCallbackDto(directory, provider.id, method, code))
            apply(id, result.state, result.error)
        }) return
        showProgress(
            CypherBundle.message("settings.providers.oauth.starting", provider.name),
            CypherBundle.message("settings.providers.oauth.cancel"),
        ) { cancelOAuth(request) }
    }
    ...
    private fun disconnect(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("disconnect provider=${provider.id}") { id ->
            val result = service<CypherProviderService>().disconnect(ProviderDisconnectDto(directory, provider.id))
            apply(id, result.state, result.error)
        }) return
        syncLoading()
    }
    ...
    private fun enable(provider: ProviderSettingsProviderDto) {
        ...
        if (!launch("enable provider=${provider.id}") { id ->
            val result = service<CypherProviderService>().enable(ProviderEnableDto(directory, provider.id))
            apply(id, result.state, result.error)
        }) return
        syncLoading()
    }
    ...
    private fun custom() {
        ...
        if (!launch("save custom provider") { id ->
            val result = service<CypherProviderService>().saveCustom(input)
            apply(id, result.state, result.error)
        }) return
        syncLoading()
    }
    ...
    private fun syncOAuthWaiting(id: Int) {
        checkEdt()
        if (!active(id)) return
        val expiry = System.currentTimeMillis() + CypherProviderService.OAUTH_RPC_TIMEOUT_MS
        fun text(): String {
            val ms = (expiry - System.currentTimeMillis()).coerceAtLeast(0)
            val remain = ((ms + 999) / 1000).toInt()
            val min = remain / 60
            val sec = remain % 60
            return CypherBundle.message("settings.providers.oauth.waitingTimed", "$min:${sec.toString().padStart(2, '0')}")
        }
        stopTimer()
        showProgress(text(), CypherBundle.message("settings.providers.oauth.cancel")) { cancelOAuth(id) }
        timer = Timer(1000) {
            if (!active(id)) {
                stopTimer()
                return@Timer
...
internal class ProvidersContent(
    ...
    private val view = SettingsListView(CypherBundle.message("settings.providers.noMatches"), SettingsListConfig.Preferred) { key, id ->
        activate(key, id)
    }
    private var state = ProviderSettingsDto()
    private var busy = false
...
private class ApiKeyDialog(title: String, method: ProviderAuthMethodDto?) : DialogWrapper(true) {
    ...
    override fun createCenterPanel(): JComponent {
        ...
        panel.next(JBLabel(CypherBundle.message("settings.providers.apiKey")))
        panel.next(key)
    ...
    override fun doValidate(): ValidationInfo? {
        if (key().isBlank()) return ValidationInfo(CypherBundle.message("settings.providers.apiKeyRequired"), key)
        return null
    }
...
private class CustomProviderDialog : DialogWrapper(true) {
    ...
    init {
        title = CypherBundle.message("settings.providers.customTitle")
        init()
        initValidation()
    }
    ...
    override fun createCenterPanel(): JComponent {
        val panel = Stack.vertical(UiStyle.Gap.sm())
        listOf(
            CypherBundle.message("settings.providers.customId") to id,
            CypherBundle.message("settings.providers.customName") to name,
            CypherBundle.message("settings.providers.customUrl") to url,
            CypherBundle.message("settings.providers.apiKey") to key,
            CypherBundle.message("settings.providers.customEnv") to env,
            CypherBundle.message("settings.providers.customModels") to models,
        ).forEach { (label, field) ->
    ...
    override fun doValidate(): ValidationInfo? {
        if (id.text.isBlank()) return ValidationInfo(CypherBundle.message("settings.providers.customIdRequired"), id)
        if (url.text.isBlank()) return ValidationInfo(CypherBundle.message("settings.providers.customUrlRequired"), url)
        return null
    }
}
