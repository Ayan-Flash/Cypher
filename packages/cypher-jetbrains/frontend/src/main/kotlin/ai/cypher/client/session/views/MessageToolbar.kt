...
import ai.cypher.client.session.ui.selection.SessionCopyButton
import ai.cypher.client.ui.ToolbarButtonAction
import ai.cypher.client.ui.UiStyle
import ai.cypher.client.ui.layout.Stack
import ai.cypher.client.ui.toolbarButton
...
internal class MessageToolbar(
    ...
    tooltip: String = CypherBundle.message("session.copy.hover"),
) : JPanel(FlowLayout(FlowLayout.LEFT, 0, 0)) {
    constructor(text: () -> String?, revert: (() -> Unit)?) : this(
        text,
        revert?.let {
            listOf(ToolbarButtonAction(AllIcons.Actions.Rollback, CypherBundle.message("revert.message.rollback"), it))
        }.orEmpty(),
        CypherBundle.message("session.copy.prompt"),
    )
