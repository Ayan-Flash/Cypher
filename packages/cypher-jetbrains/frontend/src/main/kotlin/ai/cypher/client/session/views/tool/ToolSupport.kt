...
private val LOG = CypherLog.create(ToolParts::class.java)
...
internal fun title(tool: Tool) = when (tool.name) {
    "read" -> CypherBundle.message("session.part.tool.read")
    "bash" -> CypherBundle.message("session.part.tool.shell")
    else -> toolTitle(tool)
}
...
internal fun stateText(tool: Tool) = when (tool.state) {
    ToolExecState.PENDING -> CypherBundle.message("session.part.tool.pending")
    ToolExecState.RUNNING -> CypherBundle.message("session.part.tool.running")
    ToolExecState.COMPLETED -> ""
    ToolExecState.ERROR -> CypherBundle.message("session.part.tool.error")
}
...
internal fun target(tool: Tool): Target? {
    ...
    val path = CypherCliParser.tag(out, "path") ?: return null
    val type = CypherCliParser.tag(out, "type") ?: return null
...
private class Preview {
    ...
    fun build(): String {
        ...
        text.append(CypherBundle.message("session.part.tool.truncated"))
