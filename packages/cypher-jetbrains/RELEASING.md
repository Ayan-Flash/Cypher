...
Maintainers can use the Cypher `release-jetbrains` skill to drive this process from a version request such as `next rc` or an explicit version. The skill resolves and confirms the version, dispatches and watches the prepare workflow, helps produce a filtered human-readable JetBrains/CLI changelog draft, commits the reviewed changelog to the release PR, and watches publishing after the PR is merged.

JetBrains plugin builds and runtime downloads use the Cypher Core version pinned in `packages/cypher-jetbrains/package.json`, so verify that pin points at a published `v<version>` release before creating the release tag.

`cypher.cli.pinned=false` in `packages/cypher-jetbrains/gradle.properties` is local development mode only. It generates the client from `packages/opencode/` and bundles a locally built CLI into the plugin; production Gradle builds and release scripts fail until the property is restored to `true`.

The skill lives at `.cypher/skills/release-jetbrains/SKILL.md`. It does not move or recreate release tags, and merge permission is only required if the user explicitly asks the skill to merge the release PR automatically.
...
| Field | Meaning |
|---|---|
| `packages/cypher-jetbrains/package.json` `version` | The pinned Cypher CLI release used for OpenAPI generation and runtime downloads. |
| `packages/cypher-jetbrains/gradle.properties` `cypher.jetbrains.version` | The JetBrains Marketplace plugin version. |
...
```bash
bun .cypher/skills/release-jetbrains/script/check-pin.ts
```

The script reports the CLI that `origin/main` will lock, the latest published stable CLI release, the CLI shipped by the latest `jetbrains/v*` tag, whether `cypher.cli.pinned=true`, and whether all runtime assets exist. Stop before tagging if the pin is behind the latest CLI and you want to test the newer CLI first.

To test a different CLI pin locally:

```bash
bun .cypher/skills/release-jetbrains/script/set-pin.ts --latest
cd packages/cypher-jetbrains
./gradlew typecheck
./gradlew test
```
...
```bash
bun .cypher/skills/release-jetbrains/script/set-pin.ts --latest --pr
```
...
[https://github.com/Cypher-Org/cypher/actions/workflows/prepare-jetbrains-release.yml](https://github.com/Cypher-Org/cypher/actions/workflows/prepare-jetbrains-release.yml)
...
| File | Purpose |
|---|---|
| `packages/cypher-jetbrains/gradle.properties` | JetBrains plugin version in `cypher.jetbrains.version`. |
| `packages/cypher-jetbrains/CHANGELOG.md` | Release notes packaged into the plugin. |

Review `packages/cypher-jetbrains/gradle.properties` and edit `packages/cypher-jetbrains/CHANGELOG.md` before merging. This changelog entry is rendered into JetBrains `<change-notes>`, so it appears on the Marketplace and inside IntelliJ plugin UI.

The PR can change release metadata such as `packages/cypher-jetbrains/gradle.properties` and `packages/cypher-jetbrains/CHANGELOG.md`, but it does not change the tagged source code that will be built.
...
[https://github.com/Cypher-Org/cypher/actions/workflows/publish-jetbrains.yml](https://github.com/Cypher-Org/cypher/actions/workflows/publish-jetbrains.yml)
...
4. Search for **Cypher CLI** in the Marketplace tab.
...
If Marketplace publishing succeeded but GitHub Release upload failed, manually create or edit the GitHub Release for the existing tag. Use the reviewed release notes from `packages/cypher-jetbrains/CHANGELOG.md` in the merged release PR.
...
| Secret | Purpose |
|---|---|
| `CYPHER_MAINTAINER_APP_ID` | GitHub App ID used to create/update release PRs and immediate release tags. |
| `CYPHER_MAINTAINER_APP_SECRET` | GitHub App private key used to create/update release PRs and immediate release tags. |
