...
group = "ai.cypher.jetbrains"

fun port(value: String): Int {
    val text = value.trim()
    require(text.isNotEmpty()) {
        "cypher.splitModeServerPort must be an integer from 0 to 65535; use 0 or omit it for a random port"
    }
    val n = text.toIntOrNull()
        ?: error("cypher.splitModeServerPort must be an integer from 0 to 65535; use 0 or omit it for a random port")
...
val pinned = providers.gradleProperty("cypher.cli.pinned").map { it.trim().toBoolean() }.orElse(true).get()
val override = providers.gradleProperty("cypher.version").orNull?.trim()?.takeIf { it.isNotEmpty() }
val prop = providers.gradleProperty("cypher.jetbrains.version").orNull?.trim()?.takeIf { it.isNotEmpty() }
val tag = gitTag()?.removePrefix("jetbrains/v")
val ver = override?.let(::checked) ?: prop?.let(::checked) ?: if (release) checked(
    tag ?: error("Missing JetBrains plugin version. Publish builds must set cypher.jetbrains.version or run from a jetbrains/v<version> tag."),
) else checked(tag ?: "0.0.0-dev")

if (release && !pinned) error(
    "cypher.cli.pinned=false is a dev-only mode and cannot be released. Set cypher.cli.pinned=true before a production/publish build."
)

val channel = providers.gradleProperty("cypher.channel").map { it.trim() }.orElse("default")
val splitPort = providers.gradleProperty("cypher.splitModeServerPort").map(::port).orElse(0)
val isolated = providers.gradleProperty("cypher.dev.storage.isolated").map { it.toBoolean() }.orElse(false)
val worktreeRoot = providers.gradleProperty("cypher.dev.worktree.root").orElse(
...
changelog {
    ...
    repositoryUrl = "https://github.com/Cypher-Org/cypher"
    groups = listOf("Added", "Changed", "Fixed", "Removed", "Security")
    combinePreReleases = false
}

val notes = providers.gradleProperty("cypher.changeNotes").orElse(
    provider {
...
tasks.withType<RunIdeTask> {
    val level = providers.gradleProperty("cypher.dev.log.level").orNull ?: "DEBUG"
    val content = providers.gradleProperty("cypher.dev.log.chat.content").orNull ?: "off"
    val preview = providers.gradleProperty("cypher.dev.log.chat.preview.max").orNull ?: "160"
    systemProperty("cypher.dev.log.level", level)
    systemProperty("cypher.dev.log.chat.content", content)
    systemProperty("cypher.dev.log.chat.preview.max", preview)
    systemProperty("cypher.dev.storage.isolated", isolated.get().toString())
    systemProperty("cypher.dev.worktree.root", worktreeRoot.get())
}
