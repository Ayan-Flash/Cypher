...
/**
 * Logging interface for the Cypher JetBrains plugin.
 *
 * In normal (non-sandbox) mode, output goes through IntelliJ's own [com.intellij.openapi.diagnostic.Logger],
 * which writes to the standard IDE log file, and to rotated `cypher-dev.log.*` files inside the IDE log directory.
 *
 * In sandbox mode (i.e. when running via `./gradlew runIde`, detected via the `idea.plugin.in.sandbox.mode`
 * system property), output is written only to `cypher-dev.log.*`.
 *
 * Usage:
 * ```kotlin
 * private val log = CypherLog.create(this::class.java)
 *
 ...
 * ```
 *
 * The log level for the sandbox file can be controlled via the `cypher.dev.log.level` system property
...
interface CypherLog {
    ...
    companion object {
        fun create(cls: Class<*>): CypherLog {
            return create(cls, sandbox())
        }

        internal fun create(cls: Class<*>, sandbox: Boolean): CypherLog = logger(
            sandbox = sandbox,
            intellij = { IntellijLog(cls) },
            file = { FileLog(cls) },
        )

        internal fun logger(sandbox: Boolean, intellij: () -> CypherLog, file: () -> CypherLog): CypherLog {
        ...
        fun payload(log: CypherLog? = null): Map<String, String> = buildMap {
...
internal class IntellijLog(cls: Class<*>) : CypherLog {
...
internal class FileLog(cls: Class<*>) : CypherLog {
    ...
    companion object {
        ...
        private val root: java.util.logging.Logger by lazy {
            val logger = java.util.logging.Logger.getLogger("ai.cypher")
            val payload = CypherLog.payload().entries.joinToString(" ") { "${it.key}=${it.value}" }
        ...
        private val handler: FileHandler by lazy {
            ...
            val path = dir.resolve("cypher-dev.log.%g")
            IntellijLog(FileLog::class.java).info("Cypher diagnostic log directory: $dir")
            val h = FileHandler(path.toString(), LIMIT, COUNT, true)
            h.formatter = CypherFormatter()
            h
        }
        ...
        private fun resolveLogDir(): Path {
            ...
            while (current.parent != null) {
                ...
                if (name.startsWith("log_run")) {
                    side = if (name.lowercase().contains("frontend")) "cypher-frontend" else "cypher-backend"
                }
                if (name == "cypher.jetbrains" && side != null) {
        ...
        private fun resolveLevel(): Level {
            val prop = System.getProperty("cypher.dev.log.level") ?: return Level.INFO
...
internal class CypherFormatter : Formatter() {
    ...
    override fun format(record: LogRecord): String {
        ...
        val category = record.sourceClassName ?: record.loggerName ?: "cypher.dev"
...
internal class CompositeLog(vararg val delegates: CypherLog) : CypherLog {
    override val isDebugEnabled: Boolean
