 ...
 * or worktree). Each call routes to a [CypherBackendWorkspace]
 * via the workspace manager.
 */
@Rpc
interface CypherWorkspaceRpcApi : RemoteApi<Unit> {
    companion object {
        suspend fun getInstance(): CypherWorkspaceRpcApi {
            return RemoteApiProviderService.resolve(remoteApiDescriptor<CypherWorkspaceRpcApi>())
        }
    }
    ...
    suspend fun state(directory: String): Flow<CypherWorkspaceStateDto>
