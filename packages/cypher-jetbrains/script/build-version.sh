...
if grep -Eq '^[[:space:]]*cypher\.cli\.pinned[[:space:]]*=[[:space:]]*false[[:space:]]*$' "$plugin/gradle.properties"; then
  echo "cypher.cli.pinned=false is a dev-only mode and cannot be released. Set cypher.cli.pinned=true before building a version." >&2
  exit 1
fi
...
./gradlew buildPlugin -Pproduction=true -Pcypher.version="$version" -Pcypher.channel=eap

if [[ "$sign" == "1" ]]; then
  ./gradlew signPlugin -Pproduction=true -Pcypher.version="$version" -Pcypher.channel=eap
fi
...
if [[ "$skip_verification" == "1" ]]; then
  ...
  if [[ "$sign" == "1" ]]; then
    ./gradlew verifyPluginSignature -Pproduction=true -Pcypher.version="$version" -Pcypher.channel=eap
  fi
  ./gradlew verifyPlugin -Pproduction=true -Pcypher.version="$version" -Pcypher.channel=eap
fi
