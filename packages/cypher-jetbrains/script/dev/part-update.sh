...
usage() {
  ...
  printf '       %s <cypher-log> <session-id>\n' "$0" >&2
}
...
resolve() {
  case "$target" in
    client|frontend)
      for file in \
        "$plugin/.intellijPlatform/sandbox/cypher.jetbrains/cypher-frontend/cypher-dev.log" \
    ...
    backend)
      for file in \
        "$plugin/.intellijPlatform/sandbox/cypher.jetbrains/cypher-backend/cypher-dev.log" \
...

{
  sub(/\r$/, "")
  cls = logger($0)
  if ((mode == "client" || mode == "frontend") && cls !~ /CypherSessionService$/) next
  if (mode == "backend" && cls !~ /CypherBackendChatManager$/) next
