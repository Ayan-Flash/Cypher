...
- Stop orphaned Cypher Core processes on Windows so closing the IDE no longer leaves a lingering `cypher serve` process or blocks the next IDE launch.
...
- Add Cypher Core support for localized commit-message generation, AI image generation, large bash-output pruning, and improved model-usage display.

### Fixed

- Harden Cypher Core startup and shutdown so startup failures show clearer diagnostics, app close stops the CLI process, and lingering child processes are cleaned up more reliably.
...
- Fix Cypher Core Bedrock SSO credential resolution and commit-message error handling when no changes are available.

### Changed

- Update the JetBrains plugin to download Cypher Core 7.4.5.
...
- First GA release of the native Cypher extension for JetBrains IDEs.
- Download the pinned Cypher Core release at runtime instead of bundling CLI binaries, keeping the JetBrains plugin smaller while verifying downloaded archives before use.
- Show Cypher Core runtime details from the JetBrains plugin so users can see which Core release is active.
...
- Add clearer Cypher Core runtime information and diagnostics for release download failures.
...
- Download the required Cypher Core release at runtime and prune stale cached runtime binaries automatically.
...
- Download the pinned Cypher Core release at runtime instead of bundling every CLI binary in the JetBrains plugin, keeping the Marketplace package smaller while still verifying downloaded artifacts.

## [7.0.1] - 2026-07-06

### Added

- Launch the first public Cypher JetBrains release with native JetBrains sessions and remote development support.
...
- Show Cypher Pass usage, bonus credits, renewal dates, and top-up actions in the JetBrains user profile.
...
- Improve CLI recovery, config paths, and `.cypher` config directory handling.
...
- Update the bundled Cypher CLI runtime with the latest fixes used by the JetBrains plugin.
...
- Initial JetBrains plugin release with a native Cypher CLI tool window.
...
- Bundled Cypher CLI runtime for macOS, Linux, and Windows.
