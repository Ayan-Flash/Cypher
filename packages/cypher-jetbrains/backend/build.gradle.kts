...
val generatedProps = layout.buildDirectory.dir("generated/cypher-props")
val generatedCli = layout.buildDirectory.dir("generated/cypher-cli-res")
val pinned = providers.gradleProperty("cypher.cli.pinned").map { it.trim().toBoolean() }.orElse(true)
...
val writeCypherProperties by tasks.registering(WriteProperties::class) {
    description = "Write pinned Cypher CLI properties"
    val out = generatedProps.map { it.file("cypher.properties") }
...
val stageRepoCli by tasks.registering(StageRepoCliTask::class) {
    ...
    val bin = repoRootDir.dir("dist/@cypher/cli-${platform()}/bin")
    this.bin.set(bin)
    archive.set(generatedCli.map { it.file("cypher-cli.zip") })
...
openApiGenerate {
    ...
    packageName.set("ai.cypher.jetbrains.api")
    apiPackage.set("ai.cypher.jetbrains.api.client")
    modelPackage.set("ai.cypher.jetbrains.api.model")
...
tasks.named("compileKotlin") {
    dependsOn(fixGeneratedApi, writeCypherProperties)
    if (repoCli.get()) dependsOn(stageRepoCli)
    inputs.dir(generatedApi)
}

tasks.named("processResources") {
    dependsOn(writeCypherProperties)
    if (repoCli.get()) dependsOn(stageRepoCli)
}
