...
import ai.cypher.backend.workspace.CypherBackendWorkspace
...
class CypherBackendWorkspaceTest {

    private val mock = MockCliServer()
    private val log = TestLog()
    private val scope = CoroutineScope(SupervisorJob() + Dispatchers.Default)
    private val apps = mutableListOf<CypherBackendAppService>()
    ...
    private fun setup(): CypherBackendAppService =
        CypherBackendAppService.create(scope, FakeCliServer(mock), log).also { apps.add(it) }

    private suspend fun connect(app: CypherBackendAppService) {
        app.connect()
        ...
        assertIs<CypherAppState.Ready>(state, "App startup failed; logs=${log.messages}")
    }

    private suspend fun ready(app: CypherBackendAppService): CypherBackendWorkspace {
        connect(app)
        return app.workspaces.get("/test/project")
    }

    private suspend fun loaded(ws: CypherBackendWorkspace) {
        withTimeout(15_000) {
            ws.state.first { it is CypherWorkspaceState.Ready }
        }
    }
    ...
    fun `get creates workspace on demand after Ready`() = runBlocking {
        ...
        val state = ws.state.value as CypherWorkspaceState.Ready
    ...
    fun `workspaces stopped on app disconnect`() = runBlocking {
        ...
        assertIs<CypherWorkspaceState.Pending>(ws.state.value)
    ...
    fun `workspace reaches Ready after creation`() = runBlocking {
        ...
        assertIs<CypherWorkspaceState.Ready>(ws.state.value)
    }
    ...
    fun `providers failure retries then transitions to Error`() = runBlocking {
        ...
        withTimeout(15_000) {
            ws.state.first { it is CypherWorkspaceState.Error }
        }

        val err = ws.state.value as CypherWorkspaceState.Error
    ...
    // ------ Data mapping ------
    // Detailed provider/command/path parsing correctness is covered in CypherCliDataParserTest.
    ...
    fun `concurrent get for same directory returns same instance`() = runBlocking {
        val port = mock.start()
        val http = CypherBackendHttpClients.api(mock.password)
        val api = DefaultApi(basePath = "http://127.0.0.1:$port", client = http)
        val events = MutableSharedFlow<SseEvent>()
        val sessions = CypherBackendSessionManager(scope, log)
        val manager = CypherBackendWorkspaceManager(scope, sessions, log)
    ...
    fun `concurrent load calls on workspace produce valid final state`() = runBlocking {
        ...
        val state = withTimeout(15_000) {
            ws.state.first {
                it is CypherWorkspaceState.Ready &&
                    it.providers.providers.firstOrNull()?.id == "openai"
            }
        } as CypherWorkspaceState.Ready
    ...
    fun `SSE global disposed triggers full app reload with new data`() = runBlocking {
        ...
        val prev = (app.appState.value as CypherAppState.Ready).rev
        val before = mock.requestCount("/global/config")
        val reload = async(start = CoroutineStart.UNDISPATCHED) {
            app.appState.first { it is CypherAppState.Ready && it.rev > prev }
        }
