...
class WorkspacePathScopingTest {
    ...
    private val base: Path = Path.of(System.getProperty("user.home")).resolve("cypher-scope-test").normalize()
    ...
    @Test
    fun `workspace scope keeps normal project and cypher plan files`() {
        assertEquals("backend/src/Main.java", relativeWithinWorkspace(base, at("backend", "src", "Main.java")))
        assertEquals(".cypher/plans/x.md", relativeWithinWorkspace(base, at(".cypher", "plans", "x.md")))
    }

    @Test
    fun `workspace scope rejects managed worktree storage from main checkout`() {
        assertNull(relativeWithinWorkspace(base, at(".cypher", "worktrees")))
        assertNull(relativeWithinWorkspace(base, at(".cypher", "worktrees", "foo", "backend", "src", "Main.java")))
    }

    @Test
    fun `workspace scope allows files inside the active worktree`() {
        val root = at(".cypher", "worktrees", "foo")
    ...
    fun `workspace scope rejects sibling worktrees from active worktree`() {
        val root = at(".cypher", "worktrees", "foo")
        val sibling = at(".cypher", "worktrees", "bar", "backend", "src", "Main.java")
    ...
    fun `workspace scope rejects nested managed worktree storage`() {
        ...
        assertNull(relativeWithinWorkspace(root, root.resolve(".cypher/worktrees/bar/backend/src/Main.java")))
    }
    ...
    fun `git changes returns capped large diff without blocking`() = runBlocking {
        ...
        try {
            ...
            val diff = assertNotNull(CypherWorkspaceRpcApiImpl().gitChanges(dir.toString()))
    ...
    fun `git changes returns null outside git repositories`() = runBlocking {
        val dir = Files.createTempDirectory("cypher-non-git")
        try {
            assertNull(CypherWorkspaceRpcApiImpl().gitChanges(dir.toString()))
        } finally {
            delete(dir)
        }
    }
    ...
    private fun repo(): Path? {
        ...
        val dir = Files.createTempDirectory("cypher-git")
        git(dir, "init")
