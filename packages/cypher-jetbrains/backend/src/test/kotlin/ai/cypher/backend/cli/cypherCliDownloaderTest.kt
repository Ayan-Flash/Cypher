...
class CypherCliDownloaderTest {
    ...
    fun `downloads extracts and caches pinned cli`() = runBlocking {
        MockWebServer().use { server ->
            ...
            assertEquals(File(File(dir, "1.2.3"), CypherCliPlatform.current()).absolutePath, cli.parentFile.parentFile.absolutePath)
            assertEquals("#!/bin/sh\n", cli.readText())
            assertTrue(File(cli.parentFile, "cypher-sandbox-mutation-worker.js").isFile)
            assertEquals("/api/v1.2.3", server.takeRequest().path)
            assertEquals("/release/v1.2.3/cypher-${CypherCliPlatform.current()}.${CypherCliPlatform.archive()}", server.takeRequest().path)
            assertEquals(CliDownload(0, "1.2.3", CypherCliPlatform.current()), seen.first())
            assertTrue(seen.any { it.percent == 100 && it.version == "1.2.3" && it.platform == CypherCliPlatform.current() })
            assertTrue(
                log.messages.any {
                    it.startsWith("INFO: Cypher CLI 1.2.3 for ${CypherCliPlatform.current()} is not cached; downloading new release into ") &&
                        it.contains("/.tmp/")
                }
            )
            assertTrue(log.messages.any { it.contains("Cypher CLI path diagnostics:") && it.contains("cacheRoot=${dir.absolutePath}") })
            assertTrue(log.messages.any { it.contains("Cypher CLI cache target:") && it.contains("exe=${cli.absolutePath}") })
            assertTrue(log.messages.any { it.contains("Cypher CLI cache lock path:") && it.contains(File(dir, ".lock").canonicalPath) })

            val cachedProgress = mutableListOf<CliDownload>()
            val cached = CypherCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
            ...
            assertContains(log.messages, "INFO: Using cached Cypher CLI 1.2.3 for ${CypherCliPlatform.current()} at ${cli.absolutePath}")

            File(cli.parentFile.parentFile, ".complete").writeText("ok\n")
            server.enqueue(metadata(bytes))
            server.enqueue(MockResponse().setResponseCode(200).setBody(Buffer().write(bytes)))
            val stale = CypherCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
            ...
            server.enqueue(metadata(bytes))
            server.enqueue(MockResponse().setResponseCode(200).setBody(Buffer().write(bytes)))
            val forced = CypherCliDownloader(
                log = log,
                root = dir,
                baseUrl = server.url("/release").toString(),
                api = server.url("/api").toString(),
    ...
    fun `prunes stale versions and removes the extracted archive`() = runBlocking {
        MockWebServer().use { server ->
            val stale = File(File(dir, "0.0.1"), CypherCliPlatform.current())
            ...
            val archived = File(cli.parentFile.parentFile, "cypher-${CypherCliPlatform.current()}.${CypherCliPlatform.archive()}")
    ...
    fun `rejects cli archive with mismatched digest`() = runBlocking {
        MockWebServer().use { server ->
            ...
            assertFalse(File(File(File(dir, "1.2.3"), CypherCliPlatform.current()), ".complete").exists())
        }
    }
    ...
    fun `fails clearly and logs when the release has no matching asset`() = runBlocking {
        MockWebServer().use { server ->
            ...
            val name = "cypher-${CypherCliPlatform.current()}.${CypherCliPlatform.archive()}"
    ...
    fun `fails clearly and logs when the asset has no digest`() = runBlocking {
        MockWebServer().use { server ->
            server.enqueue(
                MockResponse().setResponseCode(200).setBody(
                    """{"assets":[{"name":"cypher-${CypherCliPlatform.current()}.${CypherCliPlatform.archive()}"}]}"""
                )
            )
            val log = TestLog()
            val ex = assertFailsWith<IllegalStateException> {
    ...
    fun `cache lock times out clearly when already held in this process`() = runBlocking {
        ...
        RandomAccessFile(file, "rw").channel.use { channel ->
            channel.lock().use {
                ...
                assertContains(ex.message.orEmpty(), "Timed out waiting for Cypher CLI cache lock")
                assertContains(ex.message.orEmpty(), file.canonicalPath)
                assertTrue(log.messages.any { it.contains("Waiting for Cypher CLI cache lock") && it.contains(file.canonicalPath) })
                assertTrue(log.messages.any { it.contains("Timed out waiting for Cypher CLI cache lock") && it.contains(file.canonicalPath) })
            }
        }
    }
    ...
    private fun archive(script: String = "#!/bin/sh\n"): ByteArray {
        val files = mapOf(
            "bin/${CypherCliPlatform.exe()}" to script.toByteArray(),
            "bin/cypher-sandbox-mutation-worker.js" to "worker\n".toByteArray(),
        )
        if (CypherCliPlatform.archive() == "zip") return zip(files)
        return tar(files)
    }
    ...
    private fun metadata(digest: String) = MockResponse().setResponseCode(200).setBody(
        """{"assets":[{"name":"cypher-${CypherCliPlatform.current()}.${CypherCliPlatform.archive()}","digest":"$digest"}]}"""
    )
