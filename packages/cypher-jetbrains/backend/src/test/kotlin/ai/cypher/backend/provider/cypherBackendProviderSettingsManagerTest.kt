...
class CypherBackendProviderSettingsManagerTest {
    ...
    @Test
    fun `disconnecting cypher gateway returns error without logout`() = runBlocking {
        mock.providers = """{
            "all":[{"id":"cypher","name":"Cypher Gateway","source":"custom","models":{}}],
            "default":{},
            "connected":["cypher"],
            "failed":[]
        }""".trimIndent()
        ...
        mock.resetCounts()
        val result = manager.disconnect(ProviderDisconnectDto("/test", "cypher"))

        assertEquals("Cypher Gateway cannot be disconnected from provider settings.", result.error)
        assertFalse(result.profileCleared)
        assertNull(mock.lastAuthDeletePath)
        assertEquals(0, mock.requestCount("/auth/cypher"))
        assertEquals(0, mock.requestCount("/global/dispose"))
    }
    ...
    fun `state waits through dispose triggered reload`() = runBlocking {
        ...
        val app = app()
        val manager = CypherBackendProviderSettingsManager(app)
        ...
        try {
            ...
            withTimeout(5_000) {
                app.appState.first { it is CypherAppState.Loading }
            }
    ...
    fun `awaitReady fails fast when disconnected`() = runBlocking {
        val app = CypherBackendAppService.create(scope, FakeCliServer(mock), TestLog())
    ...
    private suspend fun manager(): CypherBackendProviderSettingsManager {
        return CypherBackendProviderSettingsManager(app())
    }

    private suspend fun app(): CypherBackendAppService {
        val app = CypherBackendAppService.create(scope, FakeCliServer(mock), TestLog())
        app.connect()
        withTimeout(10_000) {
            app.appState.first { it is CypherAppState.Ready }
        }
        return app
    }
}
