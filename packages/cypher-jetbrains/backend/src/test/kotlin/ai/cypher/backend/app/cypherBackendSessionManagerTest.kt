...
class CypherBackendSessionManagerTest {
    ...
    private fun setup(): CypherBackendAppService {
        return CypherBackendAppService.create(scope, FakeCliServer(mock), log).also { apps.add(it) }
    }

    private suspend fun ready(app: CypherBackendAppService) {
        app.connect()
        withTimeout(10_000) {
    ...
    fun `cloudSessions passes filters and cursor`() = runBlocking {
        ...
        app.sessions.cloudSessions("/repo path", "cur 1", 25, "git@github.com:Cypher-Org/cypher.git")

        val path = mock.lastCloudSessionsPath ?: error("missing cloud sessions request")
        assertTrue(path.startsWith("/cypher/cloud-sessions?"))
        ...
        assertTrue(decoded.contains("gitUrl=git@github.com:Cypher-Org/cypher.git"), path)
    }
    ...
    fun `importCloudSession posts cloud id and returns local session`() = runBlocking {
        ...
        assertTrue(path.startsWith("/cypher/cloud/session/import?"))
        assertTrue(URLDecoder.decode(path, "UTF-8").contains("directory=/repo path"), path)
