...
import ai.cypher.backend.app.ConnectionState
import ai.cypher.backend.app.CypherConnectionService
...
class CypherConnectionServiceTest {
    ...
    fun `connect transitions to Connected`() = runBlocking {
        ...
        val svc = CypherConnectionService(
          scope,
          fake,
    ...
    fun `connect provides API client`() = runBlocking {
        val svc = CypherConnectionService(scope, fake, {}, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    fun `connect reports downloading then connecting before ready`() = runBlocking {
        ...
        val svc = CypherConnectionService(scope, server, {}, log)
    ...
    fun `SSE concurrent callbacks preserve callback order`() = runBlocking {
        ...
        val svc = CypherConnectionService(scope, fake, {}, blocked)
        val field = CypherConnectionService::class.java.getDeclaredField("listener")
        ...
        val sourceField = CypherConnectionService::class.java.getDeclaredField("source")
    ...
    fun `init error transitions to Error state`() = runBlocking {
        ...
        val svc = CypherConnectionService(scope, failing, {}, log)
        svc.connect()
    ...
    // extractType tests moved to CypherCliDataParserTest
    ...
    fun `dispose prevents reconnect on late SSE callbacks`() = runBlocking {
        val reconnects = AtomicInteger(0)
        val svc = CypherConnectionService(scope, fake, { reconnects.incrementAndGet() }, log)
        svc.connect()
        mock.awaitSseConnection()
    ...
    private class BlockingLog : CypherLog {
