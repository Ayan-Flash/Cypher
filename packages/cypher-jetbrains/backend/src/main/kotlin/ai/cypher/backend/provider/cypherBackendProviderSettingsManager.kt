package ai.cypher.backend.provider
...
import ai.cypher.backend.rpc.CypherWorkspaceDtoMapper
...
import ai.cypher.rpc.dto.CustomProviderConfigDto
...
internal class CypherBackendProviderSettingsManager(
    private val app: CypherBackendAppService,
) {
    companion object {
        private val LOG = CypherLog.create(CypherBackendProviderSettingsManager::class.java)
    ...
    suspend fun state(directory: String): ProviderSettingsDto {
        ...
        val providers = load("providers", errors) {
            CypherCliDataParser.parseProviderSettingsProviders(get("/provider?directory=${enc(directory)}"))
        }
        val auth = load("provider_auth", errors) {
            CypherCliDataParser.parseProviderAuth(get("/provider/auth?directory=${enc(directory)}"))
        } ?: emptyMap()
    ...
    suspend fun connect(input: ProviderConnectDto): ProviderActionResultDto {
        val body = CypherCliDataParser.buildProviderAuthJson(input.key, input.metadata)
    ...
    suspend fun authorize(input: ProviderOAuthAuthorizeDto): ProviderOAuthReadyDto {
        val body = CypherCliDataParser.buildProviderOAuthJson(input.method, input.inputs)
        val raw = post("/provider/${enc(input.providerId)}/oauth/authorize?directory=${enc(input.directory)}", body, OAUTH_CALL_TIMEOUT_SECONDS)
        val parsed = CypherCliDataParser.parseOAuthReady(raw)
    ...
    suspend fun callback(input: ProviderOAuthCallbackDto): ProviderActionResultDto {
        val body = CypherCliDataParser.buildProviderOAuthJson(input.method, code = input.code)
    ...
    suspend fun disconnect(input: ProviderDisconnectDto): ProviderActionResultDto {
        ...
        if (input.providerId == "cypher") {
            return ProviderActionResultDto(current, error = "Cypher Gateway cannot be disconnected from provider settings.")
        }
        ...
        if (cfg?.npm == "@ai-sdk/openai-compatible") {
            patch(input.directory, cfg.scope, CypherCliDataParser.buildCustomProviderDeletePatch(input.providerId))
        ...
        if (provider?.source == "config") {
            ...
            patch(input.directory, scope, CypherCliDataParser.buildDisabledProviderPatch(ids))
            dispose()
            return ProviderActionResultDto(state(input.directory))
        }
        deleteAuth(input.providerId)
    ...
    suspend fun enable(input: ProviderEnableDto): ProviderActionResultDto {
        ...
        scopes.distinct().forEach { scope ->
            patch(input.directory, scope, CypherCliDataParser.buildDisabledProviderPatch(disabledFor(current, scope).filter { it != input.providerId }))
        }
        dispose()
        return ProviderActionResultDto(state(input.directory))
    }
    ...
    suspend fun saveCustom(input: CustomProviderSaveDto): ProviderActionResultDto {
        ...
        patch(CypherCliDataParser.buildCustomProviderPatch(input))
        if (input.envVar.isNullOrBlank()) {
            val key = input.apiKey?.takeIf { it.isNotBlank() }
            if (key != null) put("/auth/${enc(input.id)}", CypherCliDataParser.buildProviderAuthJson(key, emptyMap()))
        } else {
            deleteAuth(input.id)
        }
        dispose()
    ...
    suspend fun fetch(input: CustomModelFetchDto): CustomModelFetchResultDto {
        ...
        return try {
            ...
            CustomModelFetchResultDto(CypherCliDataParser.parseModelIds(raw))
        } catch (e: Exception) {
    ...
    private suspend fun <T> load(resource: String, errors: MutableList<LoadErrorDto>, block: suspend () -> T): T? {
        ...
        return try {
            ...
            errors.add(CypherWorkspaceDtoMapper.error(LoadError(resource = resource, detail = e.message)))
            null
        }
    }
    ...
    private fun parsed(raw: String): ParsedConfig {
        val result = CypherCliDataParser.parseProviderConfig(raw)
    ...
    private suspend fun request(request: Request, timeoutSeconds: Long = CALL_TIMEOUT_SECONDS): String {
        ...
        val http = app.http?.newBuilder()
            ...
            ?.build() ?: throw IllegalStateException("Cypher HTTP client is unavailable")
