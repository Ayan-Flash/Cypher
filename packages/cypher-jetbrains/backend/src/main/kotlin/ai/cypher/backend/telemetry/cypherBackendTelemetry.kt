...
class CypherBackendTelemetry(
    private val log: CypherLog = CypherLog.create(CypherBackendTelemetry::class.java),
) {
    companion object {
    ...
    private fun base(): Map<String, JsonPrimitive> =
        CypherLog.payload(log).mapValues { JsonPrimitive(it.value) }
}
