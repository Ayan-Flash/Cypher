...
import ai.cypher.backend.dev.CypherDevMode
...
/**
 * Manages the Cypher CLI binary lifecycle.
 *
 * Downloads the pinned CLI into IntelliJ's system directory,
 * spawns `cypher serve --port 0`, and exposes the result as [State].
 *
 * Concurrency is handled by the owning [CypherBackendAppService] — all public
 * methods except [exited] are called under its mutex. [exited] is called from
 * [CypherConnectionService]'s IO dispatcher and is thread-safe via the stale-ref
 * guard and volatile [process] field.
 */
class CypherBackendCliManager(
    private val log: CypherLog = CypherLog.create(CypherBackendCliManager::class.java),
    ...
    @Volatile
    private var job: CypherProcessJob? = null
    ...
    private suspend fun resolveCli(onProgress: (CliDownload) -> Unit): File {
        ...
        forceExtract = false
        if (!CypherProps.pinned()) {
            if (force) log.info("Force re-extracting local repo CLI ${CypherProps.cliVersion()}")
            val cli = CypherRepoCli.extract(force)
            onProgress(CliDownload(100, CypherProps.cliVersion(), CypherCliPlatform.current()))
            return cli
        }
        if (force) log.info("Force re-downloading CLI ${CypherProps.cliVersion()}")
        return CypherCliDownloader(log = log).resolve(CypherProps.cliVersion(), force, onProgress)
    }
    ...
    internal fun buildEnv(pwd: String, base: Map<String, String> = EnvironmentUtil.getEnvironmentMap()): Map<String, String> =
        buildCypherCliEnv(pwd, base, log)
    ...
    private suspend fun spawn(cli: File, start: Long): CliServer.State =
        withContext(Dispatchers.IO) {
            ...
            log.info("CLI env: CYPHER_CLIENT=jetbrains CYPHER_PLATFORM=jetbrains CYPHER_APP_NAME=cypher-cli")
            val proc = try {
                builder.start()
            } catch (e: Exception) {
            ...
            // can never be orphaned. Null on other platforms / when unavailable — see CypherProcessJob.
            val jobHandle = CypherProcessJob.assign(proc.pid(), log)
            ...
            }, "cypher-cli-stderr").apply { isDaemon = true; start() }
            this@CypherBackendCliManager.stderr = err
    ...
    private fun clearJob(): CypherProcessJob? = synchronized(lock) {
        val current = job
        job = null
        current
    }
 ...
 */
private fun confirmKilled(proc: Process, kids: List<ProcessHandle>, log: CypherLog, timeoutSeconds: Long) {
    val parentExited = proc.waitFor(timeoutSeconds, TimeUnit.SECONDS)
 ...
 * order deadlocked IDE shutdown; the order is locked by CypherBackendCliShutdownTest. Never waits, so
...
internal fun closeStreams(proc: Process, log: CypherLog) {
...
internal fun startupDiagnostics(cli: File, env: Map<String, String>, log: CypherLog): String {
    val home = System.getProperty("user.home").orEmpty()
    val profile = EnvironmentUtil.getValue("USERPROFILE").orEmpty()
    val data = env["XDG_DATA_HOME"] ?: home.takeIf { it.isNotBlank() }?.let { File(it, ".local/share/cypher").absolutePath }.orEmpty()
    ...
    if (data.isNotBlank() && remote(data)) {
        lines += "warning: Cypher CLI data dir appears to be on a non-local drive (${root(data)}); SQLite WAL may hang. Set XDG_DATA_HOME/XDG_STATE_HOME/XDG_CONFIG_HOME/XDG_CACHE_HOME to a local disk."
    }
    val text = lines.joinToString("\n")
    log.info("CLI startup diagnostics:\n$text")
    if (data.isNotBlank() && remote(data)) {
        log.warn("Cypher CLI data dir appears to be on a non-local drive (${root(data)}); SQLite WAL may hang. Set XDG_DATA_HOME/XDG_STATE_HOME/XDG_CONFIG_HOME/XDG_CACHE_HOME to a local disk.")
    }
    return text
}
...
internal suspend fun awaitReady(
    ...
    onTimeout: () -> Unit,
    diagnostics: () -> String,
    log: CypherLog = CypherLog.create(CypherBackendCliManager::class.java),
    ...
    }, "cypher-cli-stdout").apply { isDaemon = true; start() }
    onThread(thread)
...
internal fun buildCypherCliEnv(
    ...
    put("CYPHER_PARENT_PID", ProcessHandle.current().pid().toString())
    put("CYPHER_CLIENT", "jetbrains")
    put("CYPHER_ENABLE_QUESTION_TOOL", "true")
    put("CYPHER_PLATFORM", "jetbrains")
    put("CYPHER_APP_NAME", "cypher-cli")
    put("CYPHER_TELEMETRY_LEVEL", if (CypherDevMode.enabled()) "off" else "all")
    if (!CypherClaudeCompatSettings.get()) put("CYPHER_DISABLE_CLAUDE_CODE", "true")
    put("CYPHER_FEATURE", "jetbrains-plugin")
    putIfAbsent("CYPHER_CONFIG_CONTENT", DEFAULT_CONFIG)
    ideEnv(log).forEach { entry -> put(entry.key, entry.value) }
    devStorageEnv(log)?.forEach { entry -> put(entry.key, entry.value) }
}

private fun ideEnv(log: CypherLog): Map<String, String> = buildMap {
    runCatching {
        val info = ApplicationInfo.getInstance()
        val name = info.fullApplicationName
        val build = info.build.asString()
        put("CYPHER_EDITOR_NAME", "$name $build")
    }.onFailure { log.info("Could not read ApplicationInfo: ${it.message}") }

    runCatching {
        val version = CypherPlugin.version()
        if (version != null) put("CYPHER_APP_VERSION", version)
    }.onFailure { log.info("Could not read plugin version: ${it.message}") }

    runCatching {
        put("CYPHER_MACHINE_ID", machineId())
    }.onFailure { log.info("Could not read machine ID: ${it.message}") }
}

private fun machineId(): String {
    val file = File(PathManager.getSystemPath(), "cypher/machine-id")
...
private fun devStorageEnv(log: CypherLog): Map<String, String>? {
    val enabled = System.getProperty("cypher.dev.storage.isolated", "false").toBoolean()
    if (!enabled) return null
    val root = System.getProperty("cypher.dev.worktree.root") ?: run {
        log.warn("cypher.dev.storage.isolated=true but cypher.dev.worktree.root is not set; skipping dev storage isolation")
        return null
    }
    val dev = File(root, ".cypher-dev")
