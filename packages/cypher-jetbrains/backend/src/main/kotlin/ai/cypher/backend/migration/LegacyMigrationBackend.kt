...
/**
 * Backend adapter for writing migrated data to the CLI (cypher serve).
 *
...
interface LegacyMigrationBackend {
    ...
    /** POST /cypher/session-import/project — returns the project ID */
    fun importProject(project: JsonObject): String

    /** POST /cypher/session-import/session */
    fun importSession(session: JsonObject): LegacyImportResult

    /** POST /cypher/session-import/message */
    fun importMessage(message: JsonObject)

    /** POST /cypher/session-import/part */
