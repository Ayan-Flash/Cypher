...
/**
 * OkHttp implementation of [LegacyMigrationBackend] against `cypher serve`.
 *
 ...
 * constraints getting in the way. Mirrors the pattern in CypherBackendChatManager.
 *
...
class LegacyMigrationHttpBackend(
    ...
    override fun importProject(project: JsonObject): String {
        ...
        val url = "$base/cypher/session-import/project?directory=${encode(dir)}"
    ...
    override fun importSession(session: JsonObject): LegacyImportResult {
        ...
        val url = "$base/cypher/session-import/session?directory=${encode(dir)}"
    ...
    override fun importMessage(message: JsonObject) {
        ...
        val url = "$base/cypher/session-import/message?sessionID=${encode(sessionId)}"
    ...
    override fun importPart(part: JsonObject) {
        ...
        val url = "$base/cypher/session-import/part?sessionID=${encode(sessionId)}"
