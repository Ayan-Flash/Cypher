...
object CypherRepoCli {
    suspend fun extract(force: Boolean): File = extract(
        force = force,
        root = File(PathManager.getSystemPath(), "cypher/repo-cli"),
        source = {
            CypherRepoCli::class.java.classLoader.getResourceAsStream("cypher-cli.zip")
                ?: throw IllegalStateException("cypher-cli.zip resource not found; rebuild with cypher.cli.pinned=false")
        },
    )

    internal suspend fun extract(force: Boolean, root: File, source: () -> InputStream): File = withContext(Dispatchers.IO) {
        val exe = File(root, "bin/${CypherCliPlatform.exe()}")
        ...
        if (!exe.isFile) throw IllegalStateException("Local repo CLI archive did not contain bin/${CypherCliPlatform.exe()}")
        if (!SystemInfo.isWindows) exe.setExecutable(true)
        done.writeText("ok\n")
        return@withContext exe
    }
