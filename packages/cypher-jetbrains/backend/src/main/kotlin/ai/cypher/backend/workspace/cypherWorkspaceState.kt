 ...
 *
 * Only populated after [CypherAppState.Ready][ai.cypher.backend.app.CypherAppState.Ready]
 * — the CLI server must be connected and global data loaded before
 * workspace data can be fetched.
 */
sealed class CypherWorkspaceState {
    data object Pending : CypherWorkspaceState()
    data class Loading(val progress: CypherWorkspaceLoadProgress) : CypherWorkspaceState()
    data class Ready(
        val providers: ProviderData,
        val agents: AgentData,
        val commands: List<CommandInfo>,
        val skills: List<SkillInfo>,
    ) : CypherWorkspaceState()
    data class Error(val message: String, val errors: List<LoadError> = emptyList()) : CypherWorkspaceState()
}

/**
 * Tracks which workspace data fetches have completed during
 * the [CypherWorkspaceState.Loading] phase.
 */
data class CypherWorkspaceLoadProgress(
