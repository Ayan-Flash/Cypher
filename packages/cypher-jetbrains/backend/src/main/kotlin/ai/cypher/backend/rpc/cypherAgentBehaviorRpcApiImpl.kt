...
import ai.cypher.backend.cli.CypherClaudeCompatSettings
...
class CypherAgentBehaviorRpcApiImpl(private val backend: CypherBackendAppService? = null) : CypherAgentBehaviorRpcApi {
    companion object {
        private val LOG = CypherLog.create(CypherAgentBehaviorRpcApiImpl::class.java)
        private val JSON = "application/json".toMediaType()
        private val saved = ConcurrentHashMap<String, SavedMcp>()
        private val port = AtomicInteger(-1)
    }

    private val app: CypherBackendAppService get() = backend ?: service()
    ...
    override suspend fun agents(directory: String): List<AgentDetailDto> {
        ...
        val removable = CypherCliDataParser.parseAgentRemovable(request(directory, "/agent", null))
    ...
    override suspend fun skills(directory: String) = CypherCliDataParser.parseAgentBehaviorSkills(request(directory, "/skill", null))

    override suspend fun removeSkill(directory: String, location: String): Boolean =
        post(directory, "/cypher/skill/remove", JsonObject(mapOf("location" to JsonPrimitive(location))))

    override suspend fun removeAgent(directory: String, name: String): Boolean =
        post(directory, "/cypher/agent/remove", JsonObject(mapOf("name" to JsonPrimitive(name))))
    ...
    override suspend fun commands(directory: String) = CypherCliDataParser.parseAgentBehaviorCommands(request(directory, "/command", null))

    override suspend fun mcpStatus(directory: String) = CypherCliDataParser.parseMcpStatus(request(directory, "/mcp", null)).also { items ->
    ...
    override suspend fun mcpConfig(directory: String): Map<String, McpServerConfigDto> {
        ...
        val workspace = if (directory.isBlank()) emptyMap() else try {
            CypherCliDataParser.parseConfig(request(directory, "/config", null)).mcp
        } catch (e: Exception) {
    ...
    override suspend fun saveMcp(directory: String, name: String, scope: String, config: McpConfigDto?): Boolean {
        ...
        if (scope == "workspace") {
            patchConfig("/config?directory=${encode(directory)}", CypherCliDataParser.buildConfigPatch(patch))
        } else {
    ...
    override suspend fun claudeCodeCompat(): Boolean = CypherClaudeCompatSettings.get()

    override suspend fun setClaudeCodeCompat(value: Boolean): Boolean {
        CypherClaudeCompatSettings.set(value)
        app.restart()
        return value
    }
