...
import ai.cypher.backend.cli.CliServer
import ai.cypher.backend.cli.CypherBackendCliManager
...
import ai.cypher.jetbrains.api.infrastructure.ClientError
import ai.cypher.jetbrains.api.infrastructure.ClientException
import ai.cypher.jetbrains.api.infrastructure.ServerError
import ai.cypher.jetbrains.api.infrastructure.ServerException
import ai.cypher.jetbrains.api.model.ConfigWarnings200ResponseInner
...
import ai.cypher.jetbrains.api.model.ProviderOauthAuthorizeRequest
import ai.cypher.jetbrains.api.model.ProviderOauthCallbackRequest
...
class CypherBackendAppService private constructor(
  ...
  private val loadTimeoutMs: Long,
    ...
    constructor(cs: CoroutineScope) : this(
        cs,
        CypherBackendCliManager(),
      CypherLog.create(CypherBackendAppService::class.java),
        APP_LOAD_TIMEOUT_MS,
    )
    ...
    companion object {
        ...
        ) = CypherBackendAppService(cs, server, log, loadTimeoutMs)
    }

    private val mutex = Mutex()
    private val connection = CypherConnectionService(cs, server, onReconnect = {
        cs.launch { reconnect() }
    ...
    private val _appState = MutableStateFlow<CypherAppState>(CypherAppState.Disconnected)
    val appState: StateFlow<CypherAppState> = _appState.asStateFlow()
    ...
    val sessions = CypherBackendSessionManager(cs, log)
    val chat = CypherBackendChatManager(cs, log)
    val models = CypherBackendModelStateManager(log)
    val workspaces = CypherBackendWorkspaceManager(cs, sessions, log)
    @Volatile var profile: CypherProfile200Response? = null
        private set

    @Volatile var config: ConfigDto? = null
        private set

    @Volatile var notifications: List<CypherNotifications200ResponseInner> = emptyList()
        private set
    ...
    suspend fun connect() {
        mutex.withLock {
            ...
            if (current is CypherAppState.Ready || current is CypherAppState.Downloading || current is CypherAppState.Connecting || current is CypherAppState.Loading || current is CypherAppState.MigrationRequired) return
            ensureWatcher()
            connection.connect()
        }
    }
    ...
    suspend fun retry() {
        mutex.withLock {
            when (val current = _appState.value) {
                CypherAppState.Disconnected -> {
                    ensureWatcher()
                    connection.connect()
                }
                is CypherAppState.Downloading,
                CypherAppState.Connecting,
                is CypherAppState.Loading -> Unit
                is CypherAppState.MigrationRequired -> {
                    log.info("retry: rerunning migration detection")
                    load()
                }
                is CypherAppState.Ready -> {
                    if (current.data.warnings.isEmpty()) return
                    log.info("retry: refreshing config warnings")
                    refreshConfigState()
                    val next = _appState.value
                    val warns = (next as? CypherAppState.Ready)?.data?.warnings
                    if (next is CypherAppState.Ready && warns.isNullOrEmpty()) return
                    restartConnection("warnings remained after refresh")
                }
                is CypherAppState.Error -> {
    ...
    fun requireReady() {
        when (_appState.value) {
            is CypherAppState.Ready -> return
            is CypherAppState.MigrationRequired -> throw IllegalStateException("Migration required")
            else -> throw IllegalStateException("Cypher backend is not ready")
        }
    }
    ...
    suspend fun awaitReady(timeoutMs: Long = READY_TIMEOUT_MS) {
        when (_appState.value) {
            ...
            is CypherAppState.MigrationRequired -> throw IllegalStateException("Migration required")
            is CypherAppState.Downloading,
            is CypherAppState.Loading,
            CypherAppState.Connecting -> {
                val state = withTimeoutOrNull(timeoutMs) {
                    appState.first { it !is CypherAppState.Downloading && it !is CypherAppState.Loading && it !is CypherAppState.Connecting }
                }
                when (state) {
                    is CypherAppState.Ready -> return
                    is CypherAppState.MigrationRequired -> throw IllegalStateException("Migration required")
    ...
    suspend fun updateConfig(patch: ConfigPatchDto): CypherAppState {
        val http = connection.apiClient ?: throw IllegalStateException("Not connected")
        val current = _appState.value as? CypherAppState.Ready ?: throw IllegalStateException("Cypher backend is not ready")
        val connected = connection.state.value as? ConnectionState.Connected
        val body = CypherCliDataParser.buildConfigPatch(patch)
        ...
        if (cfg == null) {
            ...
            return (_appState.value as? CypherAppState.Ready) ?: current
        }
        val warns = fetchWarnings()
        val state = _appState.value
        if (state is CypherAppState.Ready && state.data === current.data && connection.state.value == connected) {
            config = cfg
    ...
    internal suspend fun resumeAfterMigration() {
        mutex.withLock {
            if (_appState.value !is CypherAppState.MigrationRequired) return
            load()
        }
    }
    ...
    private suspend fun reconnect() {
        mutex.withLock {
            val current = _appState.value
            if (current is CypherAppState.Ready || current is CypherAppState.Downloading || current is CypherAppState.Loading || current is CypherAppState.MigrationRequired) {
    ...
    private fun ensureWatcher() {
        ...
        watcher = cs.launch {
            connection.state.collect { next ->
                when (next) {
                    ConnectionState.Disconnected -> _appState.value = CypherAppState.Disconnected
                    is ConnectionState.Downloading -> _appState.value = CypherAppState.Downloading(next.percent, next.version, next.platform)
                    ConnectionState.Connecting -> _appState.value = CypherAppState.Connecting
     ...
     *
     * Progress is tracked via [LoadProgress] and emitted as [CypherAppState.Loading].
     * On success, transitions to [CypherAppState.Ready].
     * On failure of required data, transitions to [CypherAppState.Error].
     */
    private fun load() {
        synchronized(loadLock) {
            ...
            loader = cs.launch {
                ...
                _appState.value = CypherAppState.Loading(progress.get())
                ...
                if (migration != null) {
                    ...
                    warnings = emptyList()
                    _appState.value = CypherAppState.MigrationRequired(migration)
                ...
                var prof: CypherProfile200Response? = null
                var notifs: List<CypherNotifications200ResponseInner> = emptyList()
                    ...
                    withTimeout(loadTimeoutMs) {
                        ...
                        launch {
                            ...
                            progress.updateAndGet { it.copy(profile = status) }
                                .also { _appState.value = CypherAppState.Loading(it) }
                        }
                        launch {
    ...
    private fun captureLoad(event: String, start: Long, props: Map<String, String>) {
        ...
        cs.launch {
            runCatching {
                service<CypherBackendTelemetry>().capture(
                    http,
                    port,
                    event,
    ...
    private fun setTelemetry(enabled: Boolean) {
        ...
        cs.launch {
            runCatching {
                service<CypherBackendTelemetry>().setEnabled(http, port, enabled)
    ...
    private fun captureBackend(event: String, props: Map<String, String>) {
        ...
        cs.launch {
            runCatching {
                service<CypherBackendTelemetry>().capture(http, port, event, props)
    ...
    private suspend fun detectMigration(): LegacyMigrationDetection? = withContext(Dispatchers.IO) {
        ...
        val store = CypherBackendLegacyMigrationStoreService.store(log)
        val status = store.status()
        if (status != null) {
            log.info("Migration check: skipped because status=$status")
            return@withContext null
        }
        val detection = CypherBackendMigrationManager(http, connection.port).detect(store)
     ...
     * as failures.
     */
    private suspend fun fetchProfile(): FetchResult<CypherProfile200Response?> {
        val client = connection.appLoadApi
            ?: return FetchResult.ok(null)
        return try {
            val response = client.cypherProfile()
    ...
    private suspend fun fetchConfig(): FetchResult<ConfigDto> {
        ...
        return try {
            ...
            FetchResult.ok(CypherCliDataParser.parseConfig(body))
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
    ...
    private suspend fun fetchNotifications(): FetchResult<List<CypherNotifications200ResponseInner>> {
        val client = connection.appLoadApi
            ?: return FetchResult.fail("notifications", detail = "Not connected")
        return try {
            FetchResult.ok(client.cypherNotifications())
        } catch (e: Exception) {
    ...
    private suspend fun refreshConfigState() {
        val current = _appState.value as? CypherAppState.Ready ?: return
        val connection = connection.state.value as? ConnectionState.Connected ?: return
        val cfg = fetchConfig().value ?: return
        val warns = fetchWarnings()
        val state = _appState.value
        if (state !is CypherAppState.Ready || state.data !== current.data) return
    ...
    private fun setAppReady(data: AppData) {
        ...
        _appState.value = CypherAppState.Ready(data, rev.incrementAndGet())
    }

    private fun setAppError(message: String, errors: List<LoadError>) {
        val state = CypherAppState.Error(message, errors)
        warnAppError(state)
        _appState.value = state
    }

    private fun warnAppError(state: CypherAppState.Error) {
    ...
    private suspend fun awaitLoadResult(prev: CypherAppState): CypherAppState {
        val next = appState.first { it !== prev }
        if (next !is CypherAppState.Loading) return next
        return appState.first { it !is CypherAppState.Loading }
    }
     ...
     *   Re-fetches config and updates [CypherAppState.Ready] data in-place.
     *
                ...
                connection.events.collect { event ->
                    when (event.type) {
                        ...
                        "global.disposed" -> {
                            ...
                            val current = _appState.value
                            if (current is CypherAppState.Ready) load()
                        }
    ...
    private fun reset() {
        ...
        notifications = emptyList()
        warnings = emptyList()
        _appState.value = CypherAppState.Disconnected
    }
     ...
     * Updates the current [CypherAppState.Ready] profile in-place if the app is ready.
     */
    suspend fun refreshProfile(): CypherProfile200Response? {
        val result = fetchProfile()
        val fresh = result.value
        val current = _appState.value
        if (current is CypherAppState.Ready) {
    ...
    /**
     * Start the Cypher device auth login flow.
    ...
    suspend fun startLogin(directory: String?): DeviceAuthDto {
        ...
        val response = client.providerOauthAuthorize(providerID = "cypher", directory = directory, providerOauthAuthorizeRequest = body)
    ...
    /**
     * Complete the Cypher device auth login flow.
     ...
     */
    suspend fun completeLogin(directory: String?): CypherProfile200Response? {
        val client = connection.api ?: throw IllegalStateException("Not connected")
        client.providerOauthCallback(providerID = "cypher", directory = directory, providerOauthCallbackRequest = ProviderOauthCallbackRequest(method = 0.0))
    ...
    suspend fun logout(): Boolean {
        val client = connection.api ?: throw IllegalStateException("Not connected")
        val result = client.authRemove(providerID = "cypher")
        val current = _appState.value
        if (current is CypherAppState.Ready) {
            profile = null
     ...
     */
    suspend fun setOrganization(organizationId: String?): CypherProfile200Response? {
        val http = connection.apiClient ?: throw IllegalStateException("Not connected")
        val body = JsonObject(
            mapOf("organizationId" to (organizationId?.let { JsonPrimitive(it) } ?: JsonNull)),
        ).toString()
        val request = Request.Builder()
            .url("http://127.0.0.1:$port/cypher/organization")
            .header("Accept", "application/json")
