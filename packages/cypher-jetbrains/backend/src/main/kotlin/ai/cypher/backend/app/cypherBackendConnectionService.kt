 ...
 *
 * Concurrency is handled by the owning [CypherBackendAppService] — `connect`,
 * `restart`, and `reinstall` are called under its mutex. Internal reconnect
 * attempts delegate back to the owner via [onReconnect].
 *
 * Not a service — owned and instantiated by [CypherBackendAppService].
 */
class CypherConnectionService(
  ...
  private val appLoadTimeoutMs: Long,
    ...
    constructor(
      ...
      server: CliServer,
      onReconnect: () -> Unit,
    ) : this(cs, server, onReconnect, CypherLog.create(CypherConnectionService::class.java), 30_000L)
     ...
     *
     * Called under [CypherBackendAppService]'s mutex — no internal guard needed.
     */
    suspend fun connect() {
        open()
    }
     ...
     *
     * Called under [CypherBackendAppService]'s mutex.
     */
    suspend fun restart() {
    ...
    private suspend fun open() {
        ...
        val ac = CypherBackendHttpClients.api(password)
        val lc = CypherBackendHttpClients.appLoad(password, appLoadTimeoutMs)
        val hc = CypherBackendHttpClients.health(password)
        apiClient = ac
        appLoadClient = lc
        healthClient = hc
    ...
    private val listener = object : EventSourceListener() {
        ...
        override fun onEvent(src: EventSource, id: String?, type: String?, data: String) {
            ...
            synchronized(lock) {
                ...
                lastEvent.set(System.currentTimeMillis())
                val kind = type ?: CypherCliDataParser.extractEventType(data)
     ...
     * goes through [CypherBackendAppService]'s mutex for a full restart.
     */
    ...
    private fun close() {
        api = null
        appLoadApi = null
        apiClient?.let { CypherBackendHttpClients.shutdown(it) }
        apiClient = null
        appLoadClient?.let { CypherBackendHttpClients.shutdown(it) }
        appLoadClient = null
        healthClient?.let { CypherBackendHttpClients.shutdown(it) }
        healthClient = null
    }
    ...
    fun dispose() {
        ...
        log.info("CypherConnectionService disposed")
    }
}
