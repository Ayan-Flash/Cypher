...
- Confirm `ai.cypher.jetbrains` is the final permanent Marketplace plugin ID before the first upload.
...
- Confirm the plugin is owned by the correct Cypher vendor or organization.
...
- Confirm `CYPHER_MAINTAINER_APP_ID` and `CYPHER_MAINTAINER_APP_SECRET` are available to create release PRs and immediate release tags.
...
- Review and edit `packages/cypher-jetbrains/CHANGELOG.md` in the generated release PR.
