...
This is the Cypher CLI documentation site. Cypher CLI is the leading open source agentic engineering platform.
...
Note that this site is served under cypher.ai/docs so the `/docs` prefix **must** be present in every image path.

#### Generated screenshots

When updating screenshots for active docs pages, prefer generated screenshot-test assets from `packages/cypher-docs/public/img/screenshot-tests/` and reference them as `/docs/img/screenshot-tests/...`. Only replace a hand-captured image when the generated screenshot matches the docs content closely. Do not replace screenshots in VSCode Legacy docs tabs or sections.

If a docs page references a generated VS Code visual-regression screenshot, record that usage in `packages/cypher-vscode/tests/visual-regression.spec.ts` by adding the story ID to the `DOCS` map. Keep `packages/cypher-vscode/tests/visual-regression.spec.mts` in sync while that file exists. If no matching generated screenshot exists, add or update a Storybook story in `packages/cypher-vscode/webview-ui/src/stories/` and let visual-regression CI generate the baseline.
...
Before writing documentation, review `packages/cypher-docs/STYLE_GUIDE.md` for voice, tone, and formatting conventions.
