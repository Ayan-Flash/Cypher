---
title: "AI Providers"
description: "Configure and connect different AI model providers to Cypher CLI"
---

# AI Providers

Cypher CLI supports a wide variety of AI providers, giving you flexibility in how you power your AI-assisted development workflow. Choose from cloud providers, local models, or AI gateways based on your needs.

## Getting Started

The fastest way to get started is with **Cypher CLI's built-in provider**, which requires no configuration. Simply sign in and start coding.
...
- **[Atomic Chat](/docs/ai-providers/atomic-chat)** - Local models with TurboQuant inference and auto-discovery in Cypher CLI
...
|---|---|
| Ease of use | [Cypher CLI (built-in)](/docs/ai-providers/cypher) |
...
You can prevent specific providers from loading using `disabled_providers` in your `cypher.json` (or `cypher.jsonc`). This is useful to hide models from built-in or detected providers that you don't intend to use.
...
Both fields accept provider IDs — the lowercase identifier used in the `provider/model` format (e.g. `cypher`, `anthropic`, `openai`, `google`, `groq`).

## Next Steps

- **New to Cypher CLI?** Start with the [Cypher CLI provider](/docs/ai-providers/cypher) - no setup required
