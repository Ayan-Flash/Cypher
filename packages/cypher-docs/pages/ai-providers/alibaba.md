...
# Using Alibaba Cloud With Cypher CLI

Cypher CLI supports Alibaba Cloud Model Studio (DashScope) through the native Alibaba AI SDK provider. Use it to run Qwen and other DashScope-hosted models directly from Cypher.
...
Set the DashScope API key as an environment variable or configure it in your `cypher.json` config file:
...
- **Native provider:** Cypher uses the native Alibaba AI SDK provider, not a generic compatibility shim, when you select the built-in Alibaba provider.
- **Cache controls:** Cypher forwards provider cache controls for compatible Alibaba/DashScope models, which can reduce repeated context cost where the model supports caching.
...
- **Authentication errors:** Verify `DASHSCOPE_API_KEY` is set in the same shell that launches Cypher, or stored in your Cypher provider config.
- **Model not found:** Confirm the model ID is available in your DashScope account and try the model picker to see Cypher's current catalog.
