---
title: "Using xAI Grok with Cypher CLI"
description: "Connect xAI's Grok models to Cypher CLI. Use a SuperGrok or X Premium subscription via OAuth or a paid API key. Guide to setup in VS Code and the CLI."
sidebar_label: xAI (Grok)
---

# Using xAI (Grok) With Cypher CLI
...
Cypher CLI supports two ways to connect xAI:
...
- **OAuth login — no API keys:** Sign in through your browser and Cypher CLI handles token management automatically.
- **Automatic token refresh:** Cypher CLI refreshes your access token in the background so long-running sessions stay authenticated.

{% callout type="note" %}
SuperGrok and X Premium subscription access works with Cypher CLI's core functionality (VS Code extension and CLI). For cloud features such as Cloud Agents or CypherClaw, use the [Cypher Gateway](/docs/gateway) — the Gateway supports xAI via [BYOK](/docs/getting-started/byok) with an API key (OAuth and subscription-based access are not supported through the Gateway).
{% /callout %}
...
```bash
cypher auth login --provider xai
```

Cypher CLI offers three methods at the prompt:
...
- **Sign out:** To disconnect in VS Code, use the "Disconnect" button in the provider settings. In the CLI, run `cypher auth logout` and choose xAI.
...
- **Token rotation:** xAI rotates refresh tokens on each use. Cypher CLI persists the latest tokens automatically. If you run Cypher CLI from multiple processes simultaneously, the first refresh can invalidate the other process's token — re-run `cypher auth login --provider xai` to restore the session.
...
3. **Create a Key:** Click to create a new API key. Give your key a descriptive name (e.g., "Cypher CLI").
