...
The Agent Manager is a control panel for running and orchestrating multiple Cypher CLI agents, with support for parallel worktree-isolated sessions.

The Agent Manager is a **full-panel editor tab** built directly into the extension. It uses the extension's embedded runtime, so no separate Cypher CLI installation or CLI authentication setup is required. It supports:
...
- Command Palette: "Cypher CLI: Open Agent Manager"
...
Managed worktrees are created under `.cypher/worktrees/` in your project. Cypher also stores Agent Manager UI state in `.cypher/agent-manager.json`.
...
2. Enter a branch name (or let Cypher generate one)
...
3. Cypher creates one worktree + session per version and runs them in parallel
...
Cypher can start Agent Manager sessions from chat with the `agent_manager` tool. It is available by default only in the VS Code extension because Agent Manager is an extension feature.
...
Create a script file in `.cypher/` using the appropriate filename for your platform:
...
Cypher runs the script automatically whenever a new worktree is created. It uses `sh` for POSIX scripts, PowerShell for `.ps1`, and `cmd.exe` for `.cmd` / `.bat`, so executable permissions are not required.
...
# Copy a nested env file that Cypher does not auto-copy.
...
Before the setup script runs, Cypher automatically copies root-level `.env` files from the main repo into the new worktree.
...
- Files that already exist in the worktree, because Cypher never overwrites them

Use `.cypher/setup-script` for anything outside the automatic copy rules, including nested env files, ignored local config, local certificates, local database files, generated config directories, or tool-specific files required to run the project.
...
For example, on macOS / Linux create `.cypher/run-script`:
...
Agent Manager state is persisted in `.cypher/agent-manager.json`. It stores worktrees, sections, session tabs, ordering, collapsed state, diff preferences, and cached PR metadata. Git branches and worktree directories remain on disk separately.

Closing a managed worktree removes it from Agent Manager, deletes its `.cypher/worktrees/` directory, and deletes the local branch. Closing an imported external worktree removes the Agent Manager entry but leaves the external directory and branch untouched.
