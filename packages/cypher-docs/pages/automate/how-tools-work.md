...
Cypher CLI uses tools to interact with your code and environment. These specialized helpers perform specific actions like reading files, making edits, running commands, or searching your codebase. Tools provide automation for common development tasks without requiring manual execution.

## Tool Workflow

Describe what you want to accomplish in natural language, and Cypher CLI will:
...
**Cypher CLI:** (Proposes the `write` tool)
...
Every tool use is subject to a permission check. The default action for any tool with no matching rule in your config is **`ask`** — meaning Cypher will pause and prompt you before executing it.
...
| `websearch` | Searches the web (Cypher/OpenRouter users) | Web |
