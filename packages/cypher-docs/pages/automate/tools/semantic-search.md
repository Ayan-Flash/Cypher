---
title: Semantic Search Tool | Cypher CLI Docs
...
- When Cypher CLI needs to find code related to specific functionality across your project
...
- When implementing a new feature, Cypher CLI searches for "authentication middleware" to understand existing patterns before writing new code.
- When debugging an issue, Cypher CLI searches for "error handling in API calls" to find related error patterns across the codebase.
- When refactoring code, Cypher CLI searches for "database transaction patterns" to ensure consistency across all database operations.
- When onboarding to a new codebase, Cypher CLI searches for "configuration loading" to understand how the application bootstraps.
