...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between Cypher CLI and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (Cypher CLI) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as Cypher CLI
...
- Single-client scenarios (one Cypher CLI instance per server)
...
1. The client (Cypher CLI) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from Cypher CLI
...
STDIO servers run locally on the same machine as Cypher CLI, which has several important implications:
...
- **Execution**: Starts and stops with Cypher CLI (child process lifecycle)
...
- Start when needed by Cypher CLI
- Not require network configuration
- Need to be installed alongside Cypher CLI or via a package manager
...
## Configuring Transports in Cypher CLI

For detailed information on configuring STDIO and SSE transports in Cypher CLI, including example configurations, see the [Understanding Transport Types](using-in-cypher-cli#understanding-transport-types) section in the Using MCP in Cypher CLI guide.
