...
description: "How to configure and use MCP servers in the Cypher CLI"
---

# Using MCP in the CLI

The Cypher CLI supports both local and remote MCP servers. Once added, MCP tools are automatically available to the LLM alongside built-in tools.
...
|---|---|---|
| **Global** | `~/.config/cypher/cypher.json` | `cypher.jsonc`, `config.json` |
| **Project** | `./cypher.json` or `./.cypher/cypher.json` | `cypher.jsonc` |
...
Browse community-contributed MCP server configurations and agent skills in the [Cypher Marketplace](https://github.com/Cypher-Org/cypher-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
