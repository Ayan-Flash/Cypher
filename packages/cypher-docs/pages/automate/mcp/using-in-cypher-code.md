---
title: "Using MCP in Cypher CLI"
description: "How to use MCP servers in Cypher CLI"
---

# Using MCP in Cypher CLI

Model Context Protocol (MCP) extends Cypher CLI's capabilities by connecting to external tools and services. This guide covers everything you need to know about using MCP with Cypher CLI.
...
MCP server configurations are stored inside the main Cypher config file. There are two levels:

1. **Global Configuration**: `~/.config/cypher/cypher.jsonc` — applies to all projects.
2. **Project-level Configuration**: `cypher.jsonc` in your project root, or `.cypher/cypher.jsonc` for a cleaner setup.

**Precedence**: Project-level configuration takes precedence over global configuration.

### Editing MCP Settings

You can edit MCP settings from the Cypher CLI settings UI:
...
If the UI cannot add a server, edit a Cypher config file directly and add the server under the top-level `mcp` key. For project-specific servers, edit `./cypher.json` or `./cypher.jsonc` if your project already has one; otherwise use `./.cypher/cypher.json` or `./.cypher/cypher.jsonc` for a cleaner setup. For servers you want in every workspace, use `~/.config/cypher/cypher.json` or `~/.config/cypher/cypher.jsonc`.

### Config Format

MCP servers are configured under the `mcp` key in `cypher.jsonc`:
...
Remote servers support OAuth 2.0 authentication. If the server supports it, Cypher CLI will automatically start the OAuth flow when you connect. You can also disable OAuth with `"oauth": false`.

{% /tab %}
{% tab label="CLI" %}

The CLI accepts several config filenames. The recommended file is `cypher.json`:

| Scope | Recommended Path | Also supported |
|---|---|---|
| **Global** | `~/.config/cypher/cypher.json` | `cypher.jsonc`, `opencode.json`, `opencode.jsonc`, `config.json` |
| **Project** | `./cypher.json` or `./.cypher/cypher.json` | `cypher.jsonc`, `opencode.jsonc`, `opencode.json` |
...
- **Remote (HTTP/SSE)**: Servers hosted over HTTP/HTTPS. Cypher CLI tries `StreamableHTTP` first, then falls back to `SSE` automatically.
...
| Command | Description |
|---|---|
| `cypher mcp list` | List all configured MCP servers |
| `cypher mcp add` | Add an MCP server |
| `cypher mcp auth` | Authenticate with an MCP server |
| `cypher mcp logout` | Log out from an MCP server |
| `cypher mcp debug` | Debug an MCP server connection |
...
Run `cypher mcp list` to verify the server status.
...
**In your config file:** Add the tool name (or a wildcard pattern) to the `permission` key in `cypher.jsonc`:
...
Cypher CLI does not come with any pre-installed MCP servers. You'll need to find and install them separately.

- **Cypher Marketplace:** Browse community-contributed MCP server configurations and agent skills in the [Cypher Marketplace](https://github.com/Cypher-Org/cypher-marketplace). The marketplace includes ready-to-use configs for popular tools like Figma, Sentry, and more.
- **Community Repositories:** Check for community-maintained lists of MCP servers on GitHub
- **Ask Cypher CLI:** You can ask Cypher CLI to help you find or even create MCP servers
- **Build Your Own:** Create custom MCP servers using the SDK to extend Cypher CLI with your own tools

For full SDK documentation, visit the [MCP GitHub repository](https://github.com/modelcontextprotocol/).

## Using MCP Tools in Your Workflow

After configuring an MCP server, Cypher CLI will automatically detect available tools and resources. To use them:

1. Type your request in the Cypher CLI chat interface
2. Cypher CLI will identify when an MCP tool can help with your task
...
- **Server Not Responding:** Check if the server process is running. Use `cypher mcp debug <server-name>` to inspect the connection.
- **Permission Errors:** Ensure API keys and credentials are set in your `cypher.jsonc` config or via `{env:VARIABLE_NAME}` references.
