...
The sandbox adds an operating-system boundary around agent tools. It limits where tools can write and, by default, blocks outbound network access from model-originated commands. This boundary applies even when a tool passes Cypher's permission checks.
...
Sandboxing is not available on Windows. If the macOS or Linux sandbox backend is unavailable, Cypher reports the reason and runs tools without sandbox confinement. The sandbox does not fail closed.
{% /callout %}
...
1. Open Cypher CLI Settings using the gear icon ({% codicon name="gear" /%}).
2. Select **Sandboxing**.
3. Turn on **Sandbox**.
...
You can also configure the default in the global `cypher.jsonc` file:
...
|---|---|---|
| Permissions | Whether Cypher allows, asks about, or denies a matching tool invocation | Prompting for sensitive file reads, blocking specific commands or tools, reviewing consequential actions, and limiting MCP tool or subagent invocation |
...
- Cypher's runtime directories listed below
- Paths listed in `sandbox.writable_paths`

| Writable Cypher path | Purpose |
|---|---|
| `$XDG_DATA_HOME/cypher` (normally `~/.local/share/cypher`) | Session data, logs, and Cypher's managed repository cache under `repos/` |
| `$XDG_CACHE_HOME/cypher` (normally `~/.cache/cypher`) | Cached data and downloaded binaries |
| `$XDG_CONFIG_HOME/cypher` (normally `~/.config/cypher`) | Configuration and installed plugins |
| `$XDG_STATE_HOME/cypher` (normally `~/.local/state/cypher`) | Runtime state |
| `$TMPDIR/cypher` | Temporary files; on macOS this is commonly under `/var/folders/.../T/cypher` |
...
- Cypher's stored sandbox policy and preference files are read-only.
...
Shell commands and their child processes inherit the same restrictions. Cypher's file tools perform mutations through a sandboxed worker. Writable file handles are unavailable, so a tool that requires an open read-write handle may fail even for an allowed path.

Because Cypher's config directory is writable, a shell command can change configuration, permissions, plugins, or additional writable paths that affect future tool calls. Direct filesystem access inside trusted integrations is confined only when the integration uses Cypher's sandbox-aware filesystem service. Starting or restarting a process with the background-process tool is unavailable while sandboxing is active.
...
When network restriction is on, Cypher blocks:
...
- Remote MCP tool calls and custom or plugin tools that Cypher cannot prove will remain offline
...
This is not a system-wide firewall. It applies to the sandboxed tool execution boundary, not every Cypher, extension, or local process. Proxy environment variables are removed from sandboxed commands while network access is restricted.
...
| Linux | Bubblewrap (`bwrap`) | Uses system `/usr/bin/bwrap` or a bundled, SHA-256-verified binary. `CYPHER_BWRAP_PATH` can select another binary. Cypher probes filesystem and network namespace support before enabling confinement. Additional writable paths must already exist before Bubblewrap starts. |
