...
description: "Use your own API keys with Cypher Gateway while retaining platform features"
---

# Bring Your Own Key (BYOK)

Bring Your Own Key (BYOK) lets you use your own API keys when using the Cypher Gateway, while retaining Cypher platform features like Code Reviews and Cloud Agents.

A user or organization may want to use BYOK to:

- Utilize new models quickly, Cypher Gateway supports most new models in minutes
...
Cypher Gateway supports BYOK keys for these providers.
...
These providers offer coding-focused subscriptions or dedicated endpoints. Bring the API key issued by your plan to use its included models through the Cypher Gateway:
...
1. Log into the Cypher platform and select the account or organization you want to add the BYOK key to.
...
- When you use the **Cypher Gateway** provider, Cypher checks if there's a BYOK key for the selected model's provider.
- If a matching BYOK key exists, the request is routed using your key.
- If the key is invalid, the request fails. It does not fall back to using Cypher's keys.
...
- BYOK works with the Cypher Gateway provider. Users should ensure that is set as the active [provider](/docs/ai-providers).
- Cypher Gateway models that can use one of your enabled personal or organization BYOK providers display a `BYOK` badge in the model picker. The badge does not apply to models selected through other providers.
