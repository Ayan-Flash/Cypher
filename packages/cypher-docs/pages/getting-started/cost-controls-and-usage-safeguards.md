...
How much you spend with Cypher CLI is shaped by several factors working together:
...
This page covers the controls currently available in Cypher CLI. For a direct overview of Auto Model tiers and token optimization tips, see [Cost Efficiency & Model Selection](/docs/getting-started/rate-limits-and-costs).
...
When the agent enters a repeated failure cycle — attempting the same action multiple times without making progress — Cypher pauses and asks for permission before continuing. This is controlled by the `doom_loop` permission, which defaults to `ask`.

**Where to configure:** Settings → Auto Approve (VS Code) or the `permission.doom_loop` key in `cypher.jsonc` (CLI).
...
Every action Cypher takes — reading files, editing code, running shell commands, launching sub-agents — is governed by the permission system. Each tool can be set to `allow`, `ask`, or `deny`. When set to `ask`, Cypher pauses before executing and you can approve or reject that specific action.

**Where to configure:** Settings → Auto Approve (VS Code) or the `permission` section in `cypher.jsonc` (CLI). See [Auto-Approving Actions](/docs/getting-started/settings/auto-approving-actions) for the full list of available permissions.
...
A shield button in the prompt controls lets you toggle auto-approve on and off at runtime without opening Settings. When enabled, pending permission prompts are approved automatically. The state stays synced across the sidebar and open Cypher tabs.
...
**Where to configure:** Organization spending limits are managed in the organization dashboard at [app.cypher.ai](https://app.cypher.ai). Individual credit top-up is at Settings → Adding Credits.

### Free model rate limits

Requests to free models (`cypher-auto/free` and other free-tier models) are rate-limited to **200 requests per hour**. If you exceed this, requests return HTTP 429 and you must wait before continuing.
...
Cypher automatically skips a set of directories including `node_modules`, `dist`, `build`, `.git`, `__pycache__`, `.cache`, and `vendor`. You can add additional paths using **permission deny rules** in `cypher.jsonc` or using a `.cypherignore` file at your workspace root.
...
**Where to configure:** `cypher.jsonc` (VS Code / CLI). See [.cypherignore](/docs/customize/context/cypherignore) for full details.

### Compact long conversations

When a conversation grows long, use `/compact` in the chat (also searchable as `smol` or `condense`) to summarize the history and free up context space. Cypher replaces older conversation turns with an anchored summary that captures your goal, constraints, progress, and next steps.

Auto-compaction is **enabled by default** — Cypher automatically compacts when approaching the context window limit so you do not need to intervene manually.

**Where to configure:** Toggle auto-compaction in **Settings → Context** (VS Code) or set `compaction.auto` in `cypher.jsonc`. Configure the trigger threshold with `compaction.threshold_percent` (e.g. `80` to compact at 80% of the model's context window).
...
**Where to configure:** Model settings in the Cypher CLI UI, or the `limit.output` key in custom model configuration.
...
Cypher automatically applies prompt caching on supported providers. Repeated context — your system prompt, stable file contents, and tool definitions — is reused from cache at a discounted rate. No configuration is required to benefit from this.
...
Cypher has auto-models that can help you control costs; more information is available in [Auto Model](/docs/code-with-ai/agents/auto-model).

### Practical examples by task type

| Task type | Suggested approach |
|---|---|
| Quick questions, syntax lookups, simple formatting | `cypher-auto/efficient` or a lightweight model |
| Routine edits, test generation, straightforward refactors | `cypher-auto/efficient` or a mid-tier model |
| Complex debugging, tracing unexpected behavior | `cypher-auto/frontier` or a strong reasoning model; Debug mode |
| Architecture planning, design decisions | `cypher-auto/frontier`; Architect mode |
| Repository-wide analysis or search | A model with a large context window (256K+); Architect mode |
| Code review and summarization | `cypher-auto/efficient` or a cost-effective model |
| Automated background tasks (CI, scripting) | `cypher-auto/efficient` or `cypher-auto/free` |

### Manually selecting a model

Use the **model selector dropdown** in the Cypher CLI chat interface to switch models for the current session. In the CLI, pass the `--model` flag to `cypher run` or use the model picker in the TUI (`Ctrl+X m` or `/models`).
...
- **VS Code:** Settings → Models → Model per Mode, or edit `cypher.jsonc` directly.
- **CLI:** Set `agent.<name>.model` in `cypher.jsonc`.

```jsonc
{
  "agent": {
    "code": {
      "model": "cypher-auto/efficient"
    },
    "architect": {
      "model": "cypher-auto/frontier"
    }
  }
}
```
...
**Where to access:** [app.cypher.ai](https://app.cypher.ai) → Usage tab. 
...
- Use `cypher-auto/efficient` as the default model
- Switch to `cypher-auto/free` for low-stakes questions and exploration
...
- Assign `cypher-auto/efficient` to Code and Debug agents for everyday work
- Assign `cypher-auto/frontier` to Architect (or Plan) agent for planning tasks
- Set `cypher-auto/efficient` as the compaction model for all agents
- If on an Enterprise plan, use Providers & Models to block high-cost models that are not needed for your team's typical work

```jsonc
{
  "agent": {
    "code": { "model": "cypher-auto/efficient" },
    "debug": { "model": "cypher-auto/efficient" },
    "architect": { "model": "cypher-auto/frontier" },
...
The snippet below is a ready-to-copy `cypher.jsonc` that turns on every available cost-control knob at its most restrictive setting. Drop it into your project root (or your global `~/.config/cypher/cypher.jsonc`) and adjust individual values upward as you get comfortable with how each one behaves.

{% callout type="tip" %}
This configuration uses only `cypher-auto/efficient`.
{% /callout %}
...
```jsonc
{
  ...
  // ── Model selection ──────────────────────────────────────────────────────
  // Route all requests through the two lowest-cost Cypher Auto tiers.
  // cypher-auto/efficient: lowest-cost paid tier (classifies each request by
  //   difficulty and routes to the cheapest benchmark-proven model).
  "model": "cypher-auto/efficient",
  "subagent_model": "cypher-auto/efficient", // default model for Task-tool subagents
  ...
  "agent": {
    ...
    "compaction": { "model": "cypher-auto/free" },
    "title":      { "model": "cypher-auto/free" },
    "summary":    { "model": "cypher-auto/free" }
  },
  ...
  // "ask" means Cypher pauses and requires your approval before executing.
...
- **Check your usage dashboard** at [app.cypher.ai/usage](https://app.cypher.ai/usage) for a breakdown by day, model, and project
...
For further reading: [4 Levers to Take Control of Your AI Spend](https://blog.cypher.ai/p/4-spend-levers)
