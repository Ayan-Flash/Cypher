...
Codebase Indexing enables semantic code search across your entire project using AI embeddings. Instead of searching for exact text matches, it understands the _meaning_ of your queries, helping Cypher CLI find relevant code even when you don't know specific function names or file locations.
...
4. **Provides the [`semantic_search`](/docs/automate/tools/semantic-search) tool** to Cypher CLI for intelligent code discovery
...
- **Enhanced AI Understanding**: Cypher CLI can better comprehend and work with your codebase
...
1. Open Cypher CLI **Settings** → **Indexing**, or click the indexing indicator at the bottom of the prompt input panel.
...
You can also edit the `indexing` section in `cypher.jsonc` directly:
...
- **LanceDB** (default). Embedded and file-based, with no server to run. Stores data under your Cypher data directory by default.
...
Open a Cypher TUI session and run:
...
All changes are written to your `cypher.jsonc` config and take effect immediately.
...
- `lancedb` uses `{ directory? }` and is the default. It is embedded and file-based, with no server to run. Cypher uses its data directory when `directory` is omitted.
...
- Files matching `.gitignore` and [`.cypherignore`](/docs/customize/context/cypherignore) patterns
...
- **API Keys**: Stored in your `cypher.jsonc` config. Treat that file as a secret in shared environments.
...
- Check that `indexing.enabled` is `true` in your `cypher.jsonc`
...
Once indexed, Cypher CLI can use the [`semantic_search`](/docs/automate/tools/semantic-search) tool to find relevant code:
...
The tool provides Cypher CLI with:
...
- **Secure storage**: API keys are stored in your local `cypher.jsonc` configuration
- **Fully local option**: Use **Ollama + LanceDB** for completely local processing
- **Access control**: Respects existing file permissions and `.cypherignore`
