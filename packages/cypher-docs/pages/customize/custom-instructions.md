...
description: "Provide custom instructions to guide Cypher CLI"
---

# Custom Instructions

Custom Instructions allow you to personalize how Cypher CLI behaves, providing specific guidance that shapes responses, coding style, and decision-making processes. Both the **VSCode** and **CLI** versions support custom instructions, though the mechanisms differ.

## What Are Custom Instructions?

Custom Instructions define specific Extension behaviors, preferences, and constraints beyond Cypher's basic role definition. Examples include coding style, documentation standards, testing requirements, and workflow guidelines.
...
Cypher automatically discovers instruction files at your project root and in parent directories (via `findUp`). The following filenames are recognized:

- **`AGENTS.md`** — The primary instruction file for Cypher
...
- **Cypher:** `~/.config/cypher/AGENTS.md`
...
The `instructions` key in `cypher.jsonc` accepts an array of paths, globs, or URLs pointing to additional instruction files. You can manage these in **Settings → Agent Behaviour → Rules** subtab.
...
## Legacy `.cypherrules` Support

If your project contains `.cypherrules` files from the VSCode extension, these are still loaded via auto-migration. However, migrating to `AGENTS.md` is recommended for new projects.
...
Each agent can have its own custom prompt defined in its `.md` file (the markdown body) or via the `agent.<name>.prompt` key in `cypher.jsonc`:
...
Or as the markdown body in `.cypher/agents/code.md`:
...
The `instructions` key in `cypher.jsonc` accepts an array of paths, globs, or URLs pointing to additional instruction files. Configure these in your `cypher.jsonc`:
