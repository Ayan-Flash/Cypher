---
title: "Skills"
description: "Extend Cypher CLI capabilities with skills"
---

# Skills

Cypher CLI implements [Agent Skills](https://agentskills.io/home), a lightweight, open format for extending AI agent capabilities with specialized knowledge and workflows.
...
1. **Discovery**: Skills are scanned from designated directories when Cypher CLI initializes. Only the metadata (name, description, and file path) is read at this stage—not the full instructions.
...
Global skills are located in the `.cypher` directory within your Home directory:

- Mac and Linux: `~/.cypher/skills/`
- Windows: `\Users\<yourUser>\.cypher\skills\`
...
Located in `.cypher/skills/` within your project:
...
For interoperability with other tools, Cypher CLI also loads skills from:
...
You can configure extra skill locations and remote skill URLs in your `cypher.jsonc` config (project or global):
...
When multiple skills share the same name, project-level skills (`.cypher/skills/`) take precedence over global skills (`~/.cypher/skills/`). Skills from compatibility directories (`.claude/skills/`, `.agents/skills/`) and additional configured paths are loaded alongside project and global skills.
...
- In the **CLI**: Skills are loaded when you start a new session or run `cypher run`
...
In Cypher CLI, the `name` field **must match** the parent directory name:
...
1. Create the skill directory:
   ...
   ```bash
   mkdir -p ~/.cypher/skills/api-design
   ```
...
- **[Cypher Marketplace repository](https://github.com/Cypher-Org/cypher-marketplace)** — Browse community skills on GitHub and manually download them into your skills directory
...
3. **Check file location**: Ensure `SKILL.md` is directly inside the skill directory (e.g., `.cypher/skills/my-skill/SKILL.md`), not nested further.

4. **Check config paths**: If using `skills.paths` or `skills.urls`, verify the paths and URLs are correct in your `cypher.jsonc`.
...
Have you created a skill that others might find useful? Share it with the community by contributing to the [Cypher Marketplace](https://github.com/Cypher-Org/cypher-marketplace)!

{% tabs %}
{% tab label="VSCode" %}

While the new platform does not yet have a built-in marketplace UI, skills from the [Cypher Marketplace repository](https://github.com/Cypher-Org/cypher-marketplace) can be manually downloaded into your `.cypher/skills/` directory or loaded via `skills.urls` in config.
