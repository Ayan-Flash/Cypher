...
The Cypher CLI **memory bank** feature has been deprecated in favor of AGENTS.md.
...
1. Examine the contents in `.cypher/rules/memory-bank/`
2. Move that content into your project's `AGENTS.md` file (or ask Cypher to do it for you)
   {% /callout %}

## What is AGENTS.md?

AGENTS.md is an open standard for configuring AI agent behavior in software projects. It's a simple Markdown file placed at the root of your project that contains instructions for AI coding assistants. The standard is supported by multiple AI coding tools, including Cypher CLI, Cursor, and Windsurf.
...
Both `AGENTS.md` and `AGENT.md` are **write-protected files** in Cypher CLI. This means:
...
## How AGENTS.md Works in Cypher CLI

### Loading Behavior

When you start a task in Cypher CLI:

1. Cypher CLI checks for `AGENTS.md` or `AGENT.md` at the project root
...
| **[Instructions](/docs/customize/custom-rules)** | Project | `instructions` key in project `cypher.jsonc` | 2 |
| **AGENTS.md** | Project | `AGENTS.md` at project root | 3 |
| **[Instructions](/docs/customize/custom-rules)** | Global | `instructions` key in global `cypher.jsonc` | 4 |
| **[Skills](/docs/customize/skills)** | Both | `.cypher/skills/`, config `skills` key | Loaded on demand |
...
```bash
export CYPHER_DISABLE_EXTERNAL_SKILLS=true
```
...
- **[Custom Rules](/docs/customize/custom-rules)** - Cypher CLI-specific rules with more control
