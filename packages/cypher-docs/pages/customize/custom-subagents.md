...
Cypher CLI includes two built-in subagents:
...
Add agents to the `agent` section of your `cypher.jsonc` config file. Any key that doesn't match a built-in agent name creates a new custom agent.
...
- **Global**: `~/.config/cypher/agents/`
- **Project-specific**: `.cypher/agents/`
...
2. **Global config** (`~/.config/cypher/config.json`)
3. **Project config** (`cypher.jsonc` in the project root)
4. **Global agent markdown files** (`~/.config/cypher/agents/*.md`)
5. **Project agent markdown files** (`.cypher/agents/*.md`)
