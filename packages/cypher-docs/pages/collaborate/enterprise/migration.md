---
title: "Migration"
description: "Migrate your team to Cypher CLI Enterprise"
---

# Migration

Switch to **Cypher Teams** or **Cypher Enterprise** from other AI coding tools and experience transparent pricing, no vendor lock-in, and superior team management capabilities.

## Why Teams Switch to Cypher
...
**Cypher Teams** and **Cypher Enterprise** show you exactly what each AI request costs - no markup, no hidden fees, complete transparency.

### No Rate Limiting

**Other tools** slow you down with rate limits and model switching when you need AI most.

**Cypher Teams** and **Cypher Enterprise** never limit your usage - pay for what you use, use what you need.

### True Team Management

**Other solutions** offer basic user management with limited visibility.

**Cypher Teams** provides comprehensive team analytics, role-based permissions, and detailed usage insights, while **Cypher Enterprise** adds advanced governance, audit logging, and enterprise-level security controls.
...
### What You Gain with Cypher Teams or Cypher Enterprise
...
3. **Calculate current costs** (if visible) vs. Cypher pricing
4. **Plan migration timeline** to minimize disruption

**Step 2: Cypher Setup**

1. **Create organization** at [app.cypher.ai](https://app.cypher.ai)
2. **Subscribe to Teams ($15/user/month)** or **Enterprise ([Contact Sales](https://cypher.ai/contact-sales))**
3. **Configure team settings** and usage policies
4. **Purchase initial AI credits** based on usage estimates

**Step 3: Team Migration**

1. **Invite team members** to Cypher
2. **Install Cypher CLI extension** alongside Cursor initially
3. **Migrate projects gradually** starting with non-critical work
4. **Train team** on Cypher CLI features and workflows

**Step 4: Full Transition**

1. **Monitor usage patterns** in Cypher dashboard
...
| Cursor Feature | Cypher Equivalent |
|---|---|
...
2. **Install Cypher CLI** extension for team members
3. **Start with simple tasks** in Cypher CLI
4. **Compare results** and team satisfaction

**Phase 2: Gradual Transition (Week 3-4)**

1. **Use Cypher CLI** for new projects
...
Cypher CLI uses a compatible rules system that supports Cursor and Windsurf patterns. Migrating your custom rules and configurations is straightforward and typically takes 5-10 minutes per project.

**Quick Overview:**

- **Project rules**: `.cursor/rules/*.mdc` → `.cypher/rules/*.md` (remove YAML frontmatter, keep Markdown content)
- **Legacy rules**: `.cursorrules` → `.cypher/rules/legacy-rules.md`
- **AGENTS.md**: Works identically in Cypher CLI (no conversion needed)
- **Global rules**: Recreate in `~/.cypher/rules/*.md` directory

Cypher CLI also supports mode-specific rules (`.cypher/rules-{mode}/`), which Cursor and Windsurf don't have. This allows different rules for different workflows (e.g., Code mode vs Debug mode).
...
- Cypher CLI extension installation and setup
...
Ready to make the switch? Contact our migration team at migrations@cypher.ai to plan your transition to transparent AI coding.
