---
title: "API Reference"
description: "Complete API reference for the Cypher AI Gateway, including chat completions, FIM completions, and model listing endpoints."
---

# API Reference

The Cypher AI Gateway provides an OpenAI-compatible API. All endpoints use the base URL:

```
https://api.cypher.ai/api/gateway
```
...
```bash
curl -X POST "https://api.cypher.ai/api/fim/completions" \
  -H "Authorization: Bearer $CYPHER_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
