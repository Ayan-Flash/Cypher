...
description: "Learn about the AI models available through the Cypher AI Gateway, including model IDs and how to use them."
---

# Models & Providers

The Cypher AI Gateway provides access to hundreds of AI models through a single unified API. You can switch between models by changing the model ID string -- no code changes required.
...
const result = streamText({
  model: cypher.chat("anthropic/claude-sonnet-4.6"),
  prompt: "Hello!",
})
```
...
```
GET https://api.cypher.ai/api/gateway/models
```
...
Auto virtual models select an underlying model using tier-specific routing. Frontier uses the `x-cypher-mode` request header. Balanced uses the API interface, Free uses deterministic affinity across available candidates, and Small uses account balance.

{% callout type="info" title="Underlying models can change" %}
The mappings below reflect the current routing. The underlying models behind each `cypher-auto/*` tier are updated server-side as better options become available or as providers change pricing and availability — the tier IDs themselves remain stable.
{% /callout %}

### `cypher-auto/frontier`
...
curl -X POST "https://api.cypher.ai/api/gateway/chat/completions" \
  ...
  -H "x-cypher-mode: plan" \
  -H "Content-Type: application/json" \
  -d '{"model": "cypher-auto/balanced", "messages": [{"role": "user", "content": "Design a database schema"}]}'
```
