...
description: "Overview of the Cypher CLI platform architecture"
---

# Architecture Overview

This page maps Cypher CLI's repository-defined architecture. It introduces the local runtime, editor clients, cloud service boundaries, and hosted execution products before the subsystem pages add implementation detail.
...
| Repository | Contents |
|---|---|
| [Cypher&#8209;Org/cypher](https://github.com/Cypher-Org/cypher) | Cypher CLI runtime, local daemon, Cypher Console, VS Code extension, JetBrains plugin, JavaScript SDK, codebase indexing, Cypher Gateway client, telemetry, docs, and shared UI packages |
| [Cypher&#8209;Org/cloud](https://github.com/Cypher-Org/cloud) | Web control plane, Cypher Gateway routes, Cloud Agent session runtime, automation, generated-application preview and deployment services, CypherClaw, Gas Town, billing, and supporting Workers |

## Three architecture layers

| Layer | Responsibility | Typical boundaries |
|---|---|---|
| Local runtime and clients | Runs local coding sessions and connects editor surfaces to one local agent engine | Cypher CLI runtime, `cypher serve` server, local daemon, Cypher Console, VS Code extension, JetBrains plugin |
| Cypher Cloud shared services | Handles hosted identity, authorization, model routing, billing, orchestration, and shared product services | Web control plane, Cypher Gateway, Workers, queues, Durable Objects, persistence |
| Hosted product runtimes and automation | Runs scoped cloud work for coding, app generation, assistants, security analysis, and multi-agent orchestration | Cloud Agent, Automation Services, App Builder, Security Agent, CypherClaw, Gas Town, Wasteland |

Local execution and hosted execution are separate boundaries. Editor clients use a local `cypher serve` server. Hosted automation can launch Cloud Agent execution sessions when cloud coding work is required.

## Terms used throughout

| Term | Meaning |
|---|---|
| Cypher CLI | Umbrella product across local clients, Cypher CLI runtime, and Cypher Cloud services |
| Cypher CLI runtime | Local agent engine in `packages/opencode/`; owns tools, sessions, config, persistence, and provider routing |
...
| Web control plane | Hosted Cypher Cloud application layer for identity, organization authorization, billing, product configuration, and API orchestration |
| Cypher Gateway | First-party hosted model-routing boundary |
...
|---|---|---|---|
| Local coding | Cypher CLI, Cypher Console, VS Code, or JetBrains | Cypher CLI runtime on developer machine | Editor clients talk to local `cypher serve` server. Local runtime owns coding session and sends model requests directly or through Cypher Gateway. |
| Hosted work | Webhook, source-control event, command, schedule, or hosted product | Cypher Cloud services; Cloud Agent when coding is required | Cloud services coordinate work. Only flows that need repository changes launch Cloud Agent execution session. |
...
flowchart LR
  ...
  chat["Cypher Chat, events, and notifications"]
  claw["CypherClaw"]
  town["Gas Town"]
  wasteland["Wasteland"]
...
| CypherClaw | Coordinates owner-scoped hosted assistant runtimes | [Cloud Platform](/docs/contributing/architecture/cloud-platform#cypherclaw) | [CypherClaw ingress](/docs/contributing/architecture/cloud-security#cypherclaw-ingress) |
...
These local surfaces live in [`Cypher-Org/cypher`](https://github.com/Cypher-Org/cypher). Package paths below are relative to that repository root.

| Surface | Package in `Cypher-Org/cypher` | Runtime model |
|---|---|---|
| Cypher CLI TUI | `packages/opencode/` | Interactive local client with daemon attach and worker-backed fallback paths |
| `cypher run` | `packages/opencode/` | Headless prompt execution through explicit attach, daemon attach, or embedded fallback |
| `cypher serve` | `packages/opencode/` | Local HTTP + SSE server for local clients |
| Cypher Console | `packages/cypher-console/`{% linebreak /%}`packages/opencode/` | Browser UI served at `/console` by a started or reused local daemon |
| VS Code extension | `packages/cypher-vscode/` | Extension host starts one shared editor-owned `cypher serve` server and routes webviews through HTTP + global SSE; SDK directory selects local runtime instance |
| JetBrains plugin | `packages/cypher-jetbrains/` | Split-mode Swing plugin; backend module starts one editor-owned `cypher serve` server and caches workspace clients by directory |

## Cloud service families

Hosted service families live in [`Cypher-Org/cloud`](https://github.com/Cypher-Org/cloud). Paths below are relative to that repository root unless another repository is named.

| Boundary | Primary source paths | Role |
|---|---|---|
| Cypher Cloud | `apps/web/`{% linebreak /%}`services/` | Hosted platform repository for identity, billing, routing, product configuration, automation, and scoped execution services |
| Web control plane | `apps/web/` | Hosted application layer for authorization, configuration, and API orchestration |
| Cypher Gateway | `apps/web/src/app/api/gateway/`{% linebreak /%}`apps/web/src/lib/ai-gateway/`{% linebreak /%}Local integration: `Cypher-Org/cypher/packages/cypher-gateway/` | First-party model-routing boundary and local client integration |
...
| Adjacent hosted boundaries | `services/app-builder/`{% linebreak /%}`services/cypherclaw/`{% linebreak /%}`services/gastown/`{% linebreak /%}`services/wasteland/`{% linebreak /%}Supporting services | App Builder, CypherClaw, Gas Town, Wasteland, chat, notifications, and supporting services |

## Supporting packages

These supporting packages also live in [`Cypher-Org/cypher`](https://github.com/Cypher-Org/cypher). Package paths below are relative to that repository root.

| Package in `Cypher-Org/cypher` | Role |
|---|---|
| `packages/cypher-indexing/` | Per-directory asynchronous codebase indexing engine behind Cypher CLI bridge |
| `packages/sdk/js/` | Generated JavaScript client and handwritten wrapper for local server APIs |
| `packages/cypher-gateway/` | Local Cypher Gateway client integration used by Cypher CLI runtime |
| `packages/cypher-console/` | Browser UI served by local daemon at `/console` |

## Architecture pages

| Page | What it covers |
|---|---|
| [CLI Runtime](/docs/contributing/architecture/cli-runtime) | Local execution modes, daemon, server authentication, routing, persistence, snapshots, SDK, config, SSE, Cypher Console, and indexing |
| [VS Code Extension](/docs/contributing/architecture/vscode-extension) | Shared local `cypher serve` ownership, webview bridge, Agent Manager, PTYs, recovery, bundled resources, and build outputs |
| [JetBrains Plugin](/docs/contributing/architecture/jetbrains-plugin) | Split-mode modules, RPC, bundled local `cypher serve` lifecycle, Kotlin SDK, recovery, and remote-development constraints |
...
- [CypherClaw Overview](/docs/cypherclaw/overview) - customer-facing CypherClaw docs
