...
description: "Architecture of the Cypher JetBrains split-mode plugin"
---

# JetBrains Plugin Architecture

The JetBrains plugin (`packages/cypher-jetbrains/`) is a split-mode Swing client of [Cypher CLI runtime](/docs/contributing/architecture/cli-runtime). Frontend module renders IDE UI. Backend module owns project-local logic and one bundled `cypher serve` server. Shared module defines cross-process RPC contracts and serializable payloads.
...
flowchart LR
  ...
  subgraph backend ["JetBrains backend"]
    ...
    conn["CypherConnectionService"]
    workspaces["Directory workspace cache"]
    cli["Extracted cypher serve --port 0"]
  end
...
|---|---|---|
| `CypherAppRpcApi` | Application | Connect, state flow, health, retry, restart, reinstall, model state, profile, login, telemetry |
| `CypherWorkspaceRpcApi` | Directory | Resolve real backend project directory, workspace state flow, reload, file lookup, open file |
| `CypherSessionRpcApi` | Session and directory | Create/list sessions, prompt, stream events, permission and question replies, config update |
| `CypherMigrationRpcApi` | Legacy migration | Detect, run, and observe migration state |
...
```text
<PathManager.getSystemPath()>/cypher/bin/cypher
<PathManager.getSystemPath()>/cypher/bin/cypher.exe   # Windows
```

It chooses platform resource by OS and CPU architecture, reuses extracted binary when resource size matches, and can force re-extraction during reinstall flow. This editor-owned child is separate from detached local daemon managed by `cypher daemon`.

| Area | Behavior |
|---|---|
| Spawn | Runs extracted binary as `cypher serve --port 0` |
| Port | CLI server prefers `4096`, then asks OS for free port; backend reads listening line from stdout |
| Authentication | Generates random 32-byte hex password and passes `CYPHER_SERVER_PASSWORD`; username defaults to `cypher` |
...
This mirrors CLI `InstanceStore`: directory remains isolation key while one editor-owned `cypher serve` process serves multiple workspace contexts.
...
| Check | Command from `packages/cypher-jetbrains/` |
|---|---|
...
| Concern | Source path |
|---|---|
| Split modules | `packages/cypher-jetbrains/settings.gradle.kts` and module XML descriptors |
| Contributor constraints | `packages/cypher-jetbrains/AGENTS.md` |
| CLI lifecycle | `packages/cypher-jetbrains/backend/src/main/kotlin/ai/cypher/backend/cli/CypherBackendCliManager.kt` |
| Connection recovery | `packages/cypher-jetbrains/backend/src/main/kotlin/ai/cypher/backend/app/CypherBackendConnectionService.kt` |
| Workspace cache | `packages/cypher-jetbrains/backend/src/main/kotlin/ai/cypher/backend/workspace/CypherBackendWorkspaceManager.kt` |
| Kotlin client generation | `packages/cypher-jetbrains/backend/build.gradle.kts` |
| RPC contracts | `packages/cypher-jetbrains/shared/src/main/kotlin/ai/cypher/rpc/` |
