...
description: "Architecture of the Cypher CLI runtime, daemon, server, persistence, SDK, and indexing"
---

# CLI Runtime Architecture

The CLI (`packages/opencode/`) is Cypher CLI's local agent engine. It owns agent execution, tools, sessions, provider integration, configuration, local persistence, directory routing, and HTTP surfaces used by editor clients and Cypher Console.
...
| Term | Meaning |
|---|---|
| Cypher CLI runtime | Local agent engine in `packages/opencode/` |
| `cypher serve` server | Local HTTP and SSE process used by editor clients and Cypher Console; selected browser-oriented paths also use WebSocket |
| Local daemon | Detached reusable `cypher serve` server managed by `cypher daemon` commands |
| Directory context | Normalized local filesystem directory used to select local runtime state |
| Local runtime instance | Directory-keyed runtime context inside one Cypher CLI process |
...
| Process-shared state | Runtime service state shared by every directory context in one Cypher CLI process |
| Modes | Configurable agent presets for tools, prompts, restrictions, and behavior |
| MCP | Protocol for extending agent tools |

One `cypher serve` process can host several local runtime instances. Directory-keyed state stays isolated. Process-shared service state does not.

## Command entry points

| Entry point | Command or caller | Runtime model |
|---|---|---|
| Interactive TUI | `cypher` | Attaches to local daemon when available; otherwise starts Bun worker and sends SDK-shaped requests over RPC |
| Headless run | `cypher run` | Uses daemon attach when available, then embedded server fetch fallback |
| Attached run | `cypher run --attach <url>` | Targets explicit running `cypher serve` server |
| Explicit API server | `cypher serve` | Starts HTTP + SSE server for external local clients |
| Local daemon | `cypher daemon start` | Starts detached `cypher serve` child for reuse |
| Browser console | `cypher console` | Starts or reuses local daemon and opens daemon-served `/console` UI |
| Editor-spawned server | VS Code or JetBrains client | Starts bundled `cypher serve --port 0` child owned by editor client, not local daemon manager |

```mermaid
flowchart LR
  run["cypher run"]
  tui["cypher TUI"]
  daemon["Detached daemon: cypher serve"]
  worker["Bun worker"]
  rpc["RPC-backed fetch and global events"]
  embedded["Embedded Server.Default().app.fetch"]
  serve["Explicit cypher serve"]
  editors["VS Code or JetBrains"]
  editorServer["Editor-owned cypher serve --port 0"]
...
Each running editor host starts one editor-owned `cypher serve` server. That server can handle coding sessions for workspace root and additional worktree directories at same time. It does not start separate server process for each directory.

```mermaid
flowchart LR
  views["Editor views<br/>Workspace root and worktrees"]
  server["One editor-owned<br/>cypher serve server"]
  store["InstanceStore"]
...
|---|---|---|
| Local `cypher serve` access | HTTP, SSE, and selected WebSocket access to local server | Cypher CLI server and spawning local client |
| Outbound provider authentication | Model provider, Cypher Gateway, catalog, and indexing access | Cypher CLI provider router and auth stores |
| Remote MCP OAuth | Browser authorization and credentials for remote MCP server | Cypher CLI MCP runtime |

### Local `cypher serve` access

Server Basic Auth is optional. It becomes required when `CYPHER_SERVER_PASSWORD` is non-empty. Default username is `cypher`; `CYPHER_SERVER_USERNAME` can override it.
...
| PTY shell child | Removes `CYPHER_SERVER_PASSWORD` and `CYPHER_SERVER_USERNAME` from spawned user-shell environment |
...
Provider auth records use `api`, `oauth`, or `wellknown` variants in `${Global.Path.data}/auth.json`, written with mode `0600`. `CYPHER_AUTH_CONTENT` can supply process-local auth JSON. Separate v2 multi-account auth store also exists for account-oriented flows.

| Path | Behavior |
|---|---|
| Direct providers | Use provider-specific keys, OAuth records, environment values, and configured endpoints |
| Cypher Gateway | Resolves Cypher model access and model catalog through gateway client |
| Anonymous Cypher | If no Cypher key exists, provider loader sets API key value `anonymous`; gateway model catalog can fall back to public unauthenticated endpoint |
| Organization catalog | Cypher model fetch includes organization ID when resolved from config, auth, or environment |
...
| Indexing auth | Resolves indexing-specific Cypher config first, then provider config, auth record, provider options, and `CYPHER_API_KEY` / `CYPHER_ORG_ID` environment values |
...
2. `x-cypher-directory` request header.
3. Server process cwd.

Local routing workspace selection is separate. Session workspace, `workspace` query parameter, and `CYPHER_WORKSPACE_ID` can select workspace context. Configured `CYPHER_WORKSPACE_ID` keeps requests local to current workspace runtime. Other selected workspaces resolve through workspace-routing adapter to local directory or remote target.
...
| Tool registry | Loads built-in, Cypher-specific, MCP, and readiness-gated semantic search tools |
...
| Provider router | Resolves direct providers, Cypher Gateway, custom endpoints, and credentials |
| HTTP server | Publishes REST, WebSocket, and SSE surfaces |

## Daemon lifecycle

`cypher daemon start|status|stop|restart` manage a detached local `cypher serve` child, with bare `cypher daemon` equivalent to `cypher daemon start`. `cypher console` calls the same start path, so it reuses a healthy daemon instead of spawning a second process, while `cypher console stop` aliases `cypher daemon stop`.
...
| Child process | Detached `cypher serve --hostname <host> --port <port>` process |
...
| Opt-out | `CYPHER_NO_DAEMON` disables automatic attach by clients; explicit daemon commands still manage daemon |

Daemon credentials differ from editor-spawned server credentials. Current daemon source stores username `cypher`, password `cypher`, and base64 Basic token in `daemon.json`. File permissions protect this local credential record. Editor clients generate random passwords per spawned server.
...
| Area | Behavior |
|---|---|
| Default database | `${Global.Path.data}/cypher.db` |
| Override | `CYPHER_DB`; relative paths resolve under data directory; `:memory:` is accepted |
...
Snapshot implementation state is directory-keyed through `InstanceState`. One `Snapshot.Service` also owns process-shared slow-snapshot guard state outside directory cache. This distinction matters when multiple Agent Manager worktrees use same `cypher serve` process.
...
3. Cypher-specific API groups and handlers live under `packages/opencode/src/cypher/server/httpapi/` and enter shared API through narrow injection seams.
4. `packages/sdk/js/script/build.ts` generates TypeScript v2 client from CLI OpenAPI.
5. `packages/sdk/js/src/v2/client.ts` adds `createCypherClient()` wrapper for directory and workspace routing, Electron and Node fetch compatibility, and clearer empty-response errors.
...
| Order | Source |
|---|---|
| 1 | Legacy Cypher migrations |
| 2 | Organization modes |
| 3 | Auth-record `.well-known/opencode` remote config |
| 4 | Global config files |
| 5 | Explicit `CYPHER_CONFIG` file |
| 6 | Project `cypher.json[c]` and `opencode.json[c]` files plus discovered config directories |
| 7 | `CYPHER_CONFIG_DIR` directory |
| 8 | `CYPHER_CONFIG_CONTENT` |
| 9 | Active Cypher Cloud organization config |
...
Global config files load from `${Global.Path.config}`. Project updates prefer existing config files found in ancestor `.cypher` or legacy `.cypher` directories, then existing project root config files, then create `.cypher/cypher.json`. Global indexing settings can carry provider and storage defaults, but global `indexing.enabled` is stripped so project enablement remains local in effective instance config.
...
Runtime config loading is separate from editor-facing JSON Schema publication. Cloud-served schema improves validation and completion for `cypher.json` and `cypher.jsonc`; it does not load, apply, or override effective runtime config. When adding or changing config key, follow [CLI Config Schema](/docs/contributing/architecture/config-schema) so CLI source and cloud overlay stay aligned.
...
`cypher console` starts or reuses daemon, opens `/console`, and prints Console launch URL. Browser launch URL embeds daemon Basic credentials so initial request authenticates.

| Area | Behavior |
|---|---|
| Frontend | Solid/Vite app in `packages/cypher-console/` |
...
| SDK | Console calls generated JavaScript SDK through `createCypherClient()` |
...
`packages/cypher-indexing/` owns indexing engine. CLI bridge injects indexing plugin by default unless default plugins are disabled, then starts indexing asynchronously per normalized directory during instance bootstrap.

| Area | Behavior |
|---|---|
| Bootstrap | `CyphercodeBootstrap` forks indexing initialization so instance startup is not blocked |
...
| Worktrees | Agent Manager `.cypher/worktrees/` and legacy `.cypher/worktrees/` paths return disabled status |
| Empty VS Code window | Extension sets `CYPHER_DISABLE_CODEBASE_INDEXING=vscode-no-workspace`; bridge reports disabled status |
| Embeddings | Supports Cypher, OpenAI, Ollama, OpenAI-compatible, Gemini, Mistral, Vercel AI Gateway, Bedrock, OpenRouter, and Voyage configuration |
...
| Daemon | `packages/opencode/src/cypher/daemon/` |
...
| Snapshots | `packages/opencode/src/snapshot/index.ts`{% linebreak /%}`packages/opencode/src/cypher/snapshot/track.ts` |
| SDK | `packages/sdk/js/`{% linebreak /%}`script/generate.ts` |
| Console | `packages/cypher-console/`{% linebreak /%}`packages/opencode/src/cypher/console/` |
| Indexing | `packages/cypher-indexing/`{% linebreak /%}`packages/opencode/src/cypher/indexing.ts` |
