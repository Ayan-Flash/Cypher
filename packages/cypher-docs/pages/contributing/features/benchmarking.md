---
title: "Benchmarking"
description: "Current evaluation evidence and roadmap for benchmarking Cypher CLI"
---
...
1. How do models compare when used by same Cypher CLI agent?
2. How do agents or Cypher CLI versions compare when used with same model?
...
|---|---|---|
| Harbor-facing smoke eval | Current workflow | `.github/workflows/smoke-test.yml` checks out private `Cypher-Org/cypher-bench`, installs dependencies, and runs two smoke tasks through repository scripts |
...
| Private `cypher-bench` internals | Not verified here | Private repository scripts, adapter behavior, and supported local commands are outside inspected docs scope |
...
|---|---|---|
| Contributor-facing Harbor adapter | Unverified roadmap | Run Cypher CLI autonomously in controlled evaluation environments |
...
| Standard agent comparison workflow | Planned | Compare agents or Cypher CLI versions on same tasks |
...
|---|---|---|
| [Harbor](https://harborframework.com) | Evaluation harness and datasets | Confirm supported Cypher adapter and invocation contract |
...
| [Opik](https://www.comet.com/docs/opik) | Trace ingestion and analysis | Confirm Harbor integration setup and Cypher adapter support |
...
|---|---|---|---|
| Model comparison | Cypher CLI agent and task set | Model | Completion, cost, and wall-clock time |
| Agent comparison | Model and task set | Agent or Cypher CLI version | Completion, cost, and wall-clock time |
| Trace analysis | Evaluation task | Run trajectory | Tool choices, errors, and repeated steps |

## Command verification requirement

Do not document `opik harbor run -a cypher`, `cypher --auto`, or `cypher run --auto` as ready-to-run interfaces until adapter and autonomous CLI invocation are verified in relevant repository. Private `cypher-bench` workflow commands are implementation evidence, not public usage guarantees.
