...
{% callout type="info" %}
**New versions of the VS Code extension and CLI are being developed in [Cypher-Org/cypher](https://github.com/Cypher-Org/cypher)** (extension at `packages/cypher-vscode`, CLI at `packages/opencode`). For extension and CLI development, please head over to that repository.
{% /callout %}
...
4. **Java 21** - Required only when running JetBrains plugin checks or repo-level checks that include `@cypher/cypher-jetbrains`
...
1. **Fork and Clone the Repository**:
   - **Fork the Repository**:
     - Visit the [Cypher CLI GitHub repository](https://github.com/Cypher-Org/cypher)
...
- **`packages/opencode/`** - Cypher CLI, agent runtime, local HTTP server, session management, and TUI
- **`packages/cypher-vscode/`** - VS Code extension, webview UI, Agent Manager, and extension packaging
- **`packages/sdk/js/`** - Generated TypeScript SDK for the local server API
- **`packages/cypher-docs/`** - Documentation site
- **`packages/cypher-jetbrains/`** - JetBrains plugin
...
`bun dev` and `bun run dev` are equivalent. Both run the local source in `packages/opencode/`; they do not use a globally installed `cypher` binary.

### Backend/API Validation

For backend and API validation, use the root [TESTING.md](https://github.com/Cypher-Org/cypher/blob/main/TESTING.md) guide. It covers starting the local backend with:
...
When the Extension Development Host opens, check the Cypher CLI output channel and the Developer Tools console for startup or webview errors.
...
Cypher CLI uses several types of tests to ensure quality:
...
Use the root [TESTING.md](https://github.com/Cypher-Org/cypher/blob/main/TESTING.md) guide for backend/API checks that require `bun dev serve` and `curl`-based requests.
...
- When editing shared `packages/opencode/` files, keep changes small and mark Cypher-only edits with `// cypher_change` for a single line or `// cypher_change start` / `// cypher_change end` for a block. Do not add these markers inside `cypher`-named paths.
...
- Check the Output panel in VSCode (View > Output) and select "Cypher CLI" from the dropdown
