...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Add header
    sections.push("# Cypher CLI Documentation")
    sections.push("")
    sections.push(
      "This file contains the complete documentation for Cypher CLI, the leading open source agentic engineering platform.",
    )
    sections.push("")
    const protocol = req.headers["x-forwarded-proto"] || "https"
    const host = req.headers.host || "cypher.ai"
