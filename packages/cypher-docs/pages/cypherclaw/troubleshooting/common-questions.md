...
description: "Answers to common CypherClaw troubleshooting questions"
---
...
2. Click **OpenClaw Doctor** on your [dashboard](/docs/cypherclaw/dashboard)
...
1. Check that the channel token is configured in [Settings](/docs/cypherclaw/dashboard#channels)
...
You can directly access the files in `/root/.openclaw/` on the [CypherClaw Dashboard](https://app.cypher.ai/claw) using the file browser of the edit files dialog. This can be a useful way to examine or update the config files (especially `openclaw.json`) if you run into an issue. There may also be backups in the form of `openclaw.bak` files that you can manually restore from if needed.
...
3. If the issue persists, join the [Cypher Discord](https://cypher.ai/discord) and share details in the CypherClaw channel
