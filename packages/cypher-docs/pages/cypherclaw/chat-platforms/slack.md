---
title: "Slack"
description: "Using CypherClaw with Slack"
---

# Slack

This page covers everything you need to use CypherClaw with Slack: connecting your bot, controlling who can DM it, and adding it to channels.

## Connecting CypherClaw to Slack

{% youtube url="https://youtu.be/Q5bt-qH-_pY" title="Slack Setup Guide" caption="How to connect your CypherClaw agent to Slack" /%}

Create a Slack app from the OpenClaw manifest and link it to your CypherClaw dashboard.
...
4. Customize the manifest before creating:
   ...
   - Update the slash command if desired (e.g., `/cypherclaw`)
5. Click **Create**
...
### Step 3: Connect Slack to CypherClaw

1. In the [CypherClaw UI](https://app.cypher.ai/claw), find the Slack integration section (may show "not configured")
2. Enter both tokens:
   - The `xapp-` app-level token
   - The `xoxb-` bot user OAuth token
3. Click **Save**
4. Scroll to the top of the CypherClaw UI and click **Redeploy**. Wait for the instance to come back up

### Step 4: Pair Slack with CypherClaw

1. In Slack, DM the app and send any message — this triggers the pairing flow
2. The app will return a pairing code
3. Return to [app.cypher.ai/claw](https://app.cypher.ai/claw) and confirm the pairing code and approve
...
By default, CypherClaw can respond to any DMs and will not respond in Slack channels, even if added.

## Making CypherClaw DM-Only (from you)

By default, CypherClaw will respond to DMs from any user in Slack.
...
## Adding CypherClaw to a Slack Channel

By default, CypherClaw will not respond in Slack channels, even if added. To have CypherClaw participate in a Slack channel:
...
Tell your CypherClaw agent (via DM):
