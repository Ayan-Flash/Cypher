...
This guide walks you through setting up Mistral's Codestral model for free autocomplete in Cypher CLI. Mistral offers a free tier that's perfect for getting started with AI-powered code completions.

## Prerequisites

- A [Cypher CLI account](https://app.cypher.ai) (free to create)
...
## Step 4: Add Your Key via BYOK in Cypher

1. Log into the [Cypher platform](https://app.cypher.ai).
2. Navigate to the [Bring Your Own Key (BYOK) page](https://app.cypher.ai/byok), available in the sidebar under **Account**.
...
Once your BYOK key is saved, Cypher CLI's autocomplete will automatically use your Codestral key through the Cypher Gateway. No additional configuration is needed in the extension.

1. Open VS Code with the Cypher CLI extension installed.
...
The autocomplete status bar in VS Code shows the current provider ("Cypher Gateway") and tracks cumulative cost. With BYOK, requests are billed directly by Mistral at their rates (Codestral has a free tier) and show as $0.00 on your Cypher balance.
...
```
Your Editor → Cypher Gateway (with your key) → Mistral
```

- The Cypher Gateway detects your BYOK key and routes autocomplete requests using it.
- You are billed directly by Mistral — Cypher does not add any markup.
- If your BYOK key is invalid, the request will fail (it does not fall back to Cypher's keys).

## Troubleshooting

- **Autocomplete not appearing?** Check that autocomplete is enabled in Cypher CLI settings (it is on by default). Also verify you are signed into Cypher CLI in the extension.
...
- **Seeing charges on your Cypher balance?** If you haven't configured BYOK, autocomplete defaults to using your Cypher credits. Add your Codestral key via BYOK to route requests through your own Mistral account.
