---
title: "File Encoding"
description: "How Cypher handles text file encodings when reading and editing files"
---

# File Encoding

Cypher automatically detects the text encoding of each file it reads and preserves that encoding when writing changes back. You can work with source files in any supported encoding without worrying about Cypher corrupting them or showing the model garbled text.
...
New files Cypher creates are always UTF-8 without a BOM. Encoding detection only runs when Cypher reads or overwrites an existing file.
...
If Cypher displays a file as garbled text, or writes it back in a different encoding than it was saved in, please open an issue at [github.com/Cypher-Org/cypher/issues](https://github.com/Cypher-Org/cypher/issues) and include all of the following:
...
- **The model and provider** you were using when the issue occurred, for example `claude-sonnet-4.5` via Cypher Gateway.
- **The exact Cypher version** you are running. For the CLI, run `cypher --version`. For the VS Code extension, open the Extensions view and check the version next to "Cypher CLI".
