...
Instead of maintaining a static list that's perpetually behind, we built something better — a real-time leaderboard showing which models Cypher CLI users are actually having success with right now.

## Check the Live Models List

**[👉 See what's working today at cypher.ai/models](https://cypher.ai/models)**
...
- Set per-agent defaults and a global default in the **Settings** panel (Models tab), or directly in the `cypher.jsonc` config file.
...
- For non-interactive use, pass `--model` flag to `cypher run` (e.g., `cypher run --model claude-sonnet-4-20250514`).
- Set the global default with the `model` key in `cypher.jsonc`, or configure per-agent models in the `agent` section.
...
You don't need a paid API key to use Cypher CLI productively. For the lowest cost on paid work, [Auto Efficient](/docs/code-with-ai/agents/auto-model#tiers) (`cypher-auto/efficient`) routes each request to the cheapest model proven accurate enough for that task. The fastest way to start for free is [Auto Model Free](/docs/code-with-ai/agents/auto-model) (`cypher-auto/free`), which routes to the best available free models automatically. See [Using Cypher for Free](/docs/getting-started/using-cypher-for-free) for the full zero-cost setup.

If you prefer to pick models yourself, type `free` in the model picker to filter by free models, or browse the full list at [cypher.ai/models](https://cypher.ai/models).

{% callout type="info" %}
Free model availability changes as providers adjust promotional periods. Check [cypher.ai/models](https://cypher.ai/models) for the live list.
{% /callout %}
...
- **Via config file** — edit `cypher.jsonc`:
...
```
vscode://cypher.cypher-cli/cypher/switch?model=<modelID>
vscode://cypher.cypher-cli/cypher/switch?agent=<agentName>
vscode://cypher.cypher-cli/cypher/switch?model=<modelID>&agent=<agentName>
```

Replace `<modelID>` with a Cypher Gateway model ID such as `cypher-auto/free`. Replace `<agentName>` with a visible primary agent ID such as `code` or `plan`, rather than its display name.

### Example: Auto Free

To open Cypher CLI and switch to the [Auto Free](/docs/code-with-ai/agents/auto-model) tier (`cypher-auto/free`), use:

```
vscode://cypher.cypher-cli/cypher/switch?model=cypher-auto%2Ffree
```

To switch only to Plan and use its normal model selection, specify the agent without a model:

```
vscode://cypher.cypher-cli/cypher/switch?agent=plan
```

To select both at the same time, include both parameters:

```
vscode://cypher.cypher-cli/cypher/switch?model=cypher-auto%2Ffree&agent=plan
```
...
- **VS Code open**: the Cypher sidebar is focused and the linked selection is applied to the active session immediately.
- **VS Code closed**: VS Code launches, then applies the selection once the extension is ready.
- When `model` is provided, it must identify a model in the current Cypher Gateway catalog. Invalid or unavailable models cause the deep link to be ignored.
...
```html
<a href="vscode://cypher.cypher-cli/cypher/switch?model=cypher-auto%2Ffree&amp;agent=plan">
  Open Cypher CLI with Auto Free in Plan
</a>
```
...
The AI model space moves fast. Bookmark [cypher.ai/models](https://cypher.ai/models) and check back when you're evaluating options. What's best today might not be best next month — and that's actually exciting.
