...
description: "Learn how to use the Cypher CLI chat interface effectively"
---

# Chatting with Cypher CLI

{% callout type="tip" %}
**Bottom line:** Cypher CLI is an AI coding assistant. You chat with it in plain English, and it writes, edits, and explains code for you.
{% /callout %}
...
Click the Cypher CLI icon ({% cypherCodeIcon /%}) in VS Code's Primary Side Bar to open the sidebar chat. You can also pop it out into an editor tab for a larger workspace.

{% /tab %}
{% tab label="CLI" %}

Open your terminal and run `cypher` to launch the interactive terminal interface (TUI). You'll see a prompt where you can start typing requests immediately. The TUI is fully keyboard-driven — no mouse required.

{% /tab %}
{% /tabs %}

## How to Talk to Cypher CLI

**The key insight:** Just type what you want in normal English. No special commands needed.

{% image src="/docs/img/typing-your-requests/typing-your-requests.png" alt="Example of typing a request in Cypher CLI" width="800" caption="Example of typing a request in Cypher CLI" /%}
...
Cypher builds the export from the complete local session history, not only the messages currently loaded in the chat view.
...
| Assuming Cypher knows context | Use `@` to reference specific files |
...
**Why it matters:** Cypher CLI works best when you communicate like you're talking to a smart teammate who needs clear direction.

## Suggested Responses

When Cypher CLI needs more information to complete a task, it asks a follow-up question and often provides suggested answers to make responding faster.

**How it works:**

1. **Question Appears** - Cypher CLI asks a question using the `question` tool
...
- **Clarity** - Suggestions often clarify the type of information Cypher CLI needs
- **Flexibility** - Edit suggestions to provide precise, customized answers when needed

This feature streamlines the interaction when Cypher CLI requires clarification, allowing you to guide the task effectively with minimal effort.
...
{% callout type="tip" %}
**Move Cypher CLI to the Secondary Side Bar** for a better layout. Right-click on the Cypher CLI icon in the Activity Bar and select **Move To → Secondary Side Bar**. This lets you see the Explorer, Search, Source Control, etc. alongside Cypher CLI.

{% image src="/docs/img/move-to-secondary.png" alt="Move to Secondary Side Bar" width="600" caption="Move Cypher CLI to the Secondary Side Bar for better workspace organization" /%}
{% /callout %}
...
Ready to start coding? Start a session in Cypher CLI and describe what you want to build!
