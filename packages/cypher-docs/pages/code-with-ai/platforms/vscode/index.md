---
title: "Cypher CLI for VS Code: Free Open-Source AI Coding Extension"
description: "Using Cypher CLI in Visual Studio Code"
---

# Cypher CLI for VS Code

The Cypher CLI VS Code extension uses the shared Cypher agent runtime while integrating chat, agents, and automation into the editor.
...
The extension includes its own embedded runtime. No separate Cypher CLI installation is required.
...
- **[JSONC config files](/docs/getting-started/settings)** — Portable settings in `cypher.jsonc` instead of VS Code settings
- **[Granular permissions](/docs/getting-started/settings/auto-approving-actions)** — Per-tool permission rules with glob patterns
- **[Agents](/docs/code-with-ai/agents/using-agents)** — Customizable agents (`.cypher/agents/*.md`) replacing the modes system
...
Settings apply across extension surfaces, including the sidebar and Agent Manager. The standalone CLI uses the same `~/.config/cypher/cypher.jsonc` (global) and `./cypher.jsonc` (project) files when used directly.

## Proxy and Certificate Troubleshooting

Cypher CLI for VS Code starts its embedded runtime from the extension and applies the relevant VS Code network settings to that runtime. On managed networks, configure proxy and certificate trust in VS Code settings rather than in a separate CLI install.
...
- Leave `http.proxySupport` enabled unless you intentionally want VS Code and Cypher CLI to ignore proxy settings.
...
- If the operating system trust store is not enough, set `cypher-cli.new.extraCaCerts` to the absolute path of a PEM file that contains the additional certificate authority certificates.
...
```json
{
  ...
  "cypher-cli.new.extraCaCerts": "/absolute/path/to/corporate-ca.pem"
}
```
