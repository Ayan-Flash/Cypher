---
title: "Cypher CLI CLI: Run the AI Coding Agent from Your Terminal"
description: "Using Cypher CLI from the command line"
platform: new
---

{% callout type="warning" title="Version Notice" %}
This documentation applies only to Cypher version 1.0 and later. Users running versions below 1.0 should upgrade before proceeding.
{% /callout %}

# Cypher CLI CLI: AI Coding Agent in Your Terminal

Orchestrate agents from your terminal. Plan, debug, and code fast with keyboard-first navigation on the command line.

The Cypher CLI CLI uses the same underlying technology that powers the IDE extensions, so you can expect the same workflow to handle agentic coding tasks from start to finish.

**Source code & issues (Cypher CLI 1.0):** [Cypher-Org/cypher](https://github.com/Cypher-Org/cypher) · [Report an issue](https://github.com/Cypher-Org/cypher/issues)
...
Change directory to where you want to work and run cypher:
...
After installation, run `cypher` and use the `/connect` command to add your first provider credentials. This is the interactive way to configure API keys for model providers.

## Update

Upgrade the Cypher CLI:

`cypher upgrade`

Or use npm:

`npm update -g @cypher/cli`

## What you can do with Cypher CLI CLI
...
- **Switch between hundreds of LLMs without constraints.** Other CLI tools only work with one model or curate opinionated lists. With Cypher, you can switch models without booting up another tool.
...
#### Cypher Gateway Commands (when connected)

| Command | Aliases | Description |
|---|---|---|
| `/profile` | `/me`, `/whoami` | View your Cypher Gateway profile |
| `/teams` | `/team`, `/org`, `/orgs` | Switch between Cypher Gateway teams |
...
- Config files in **`~/.config/cypher/`**: use **`cypher.jsonc`** for provider, model, permission, and **MCP** settings. Restart the CLI after editing. See [Using MCP in Cypher CLI](/docs/automate/mcp/using-in-cypher-cli) for MCP config format.
- **`tui.jsonc`** for terminal UI settings such as notifications, sounds, themes, and keybindings
- `cypher auth` for credential management
...
- Run `cypher console`, open your project, then go to **Settings > CLI > Notifications**.
- Edit the TUI configuration directly. Use `~/.config/cypher/tui.jsonc` (or `tui.json`) for global settings, or `.cypher/tui.json` (or `tui.jsonc`) for project settings.
...
Supported sound names are `default`, `question`, `permission`, `error`, `done`, and `subagent_done`. Relative paths are resolved from the directory containing the TUI configuration file. If an override cannot be loaded, Cypher falls back to the active sound pack and then the built-in `opencode.default` pack.
...
There is no notification slash command or command-palette toggle. Use Cypher Console or `tui.json` / `tui.jsonc` so all attention behavior is controlled by the same configuration.
...
Cypher CLI uses the permission config to decide whether a given action should run automatically, prompt you, or be blocked.
...
Use `external_directory` to allow tool calls that touch paths outside the working directory where Cypher was started. This applies to any tool that takes a path as input (for example `read`, `edit`, `glob`, `grep`, and many bash commands).
...
The Cypher CLI is a fork of [OpenCode](https://opencode.ai) and supports the same configuration options. The CLI you install with `npm install -g @cypher/cli` (Cypher CLI 1.0) is built from [Cypher-Org/cypher](https://github.com/Cypher-Org/cypher). For comprehensive configuration documentation, see the [OpenCode Config documentation](https://opencode.ai/docs/config).

### Config File Location (Cypher CLI 1.0)

| Scope | Path |
|---|---|
| **Global** | `~/.config/cypher/cypher.json[c]` or legacy `opencode.json[c]` (Windows config dir may vary) |
| **Project** | `./cypher.json[c]`, legacy `./opencode.json[c]`, or config inside `./.cypher/` (legacy `./.cypher/` is also read) |

Project-level configuration takes precedence over global settings.

{% callout type="warning" %}
**Migrating from opencode?** Cypher no longer falls back to opencode configuration stored in `.opencode` directories (such as `~/.config/opencode` or a project `./.opencode/`). To keep using it, move your global config into `~/.config/cypher/` and any project config into `./.cypher/`.
{% /callout %}
...
Some terminals don't send modifier keys with Enter by default. Windows Terminal requires a one-time configuration to forward `Shift+Enter` as an escape sequence that Cypher can read.
...
Save the file and restart Windows Terminal or open a new tab. `Shift+Enter` will now insert a newline in the Cypher prompt instead of submitting the message.

### OpenTelemetry Export

Cypher telemetry is enabled by default and can be disabled with `experimental.openTelemetry = false`:
...
`{env:VAR}` (and `{file:...}`) references are resolved **only** in trusted config: your global config (`~/.config/cypher`), a config passed via `CYPHER_CONFIG` / `CYPHER_CONFIG_CONTENT`, or organization/MDM-managed config. A project-level `cypher.json` / `opencode.json` committed to a repository **cannot** use `{env:VAR}` — the reference is ignored and a warning is logged. This prevents a malicious repository from exfiltrating your secrets to an attacker-controlled `baseURL` simply by being opened. `{file:...}` still works in project config, but only for files that resolve inside the project root — references that leave it (absolute paths outside the root, `../` traversal, and symlink escapes) are rejected.
{% /callout %}
...
Interactive mode is the default mode when running Cypher CLI without the `--auto` flag, designed to work interactively with a user through the console.

In interactive mode Cypher CLI will request approval for operations which have not been auto-approved, allowing the user to review and approve operations before they are executed, and optionally add them to the auto-approval list.
...
Cypher only saves the pattern you select. Approving a specific command does not approve redirected variants or broader command patterns unless that broader option is shown and selected.

## Autonomous Mode (Non-Interactive)

Autonomous mode allows Cypher CLI to run in automated environments like CI/CD pipelines without requiring user interaction.

```bash
# Run in autonomous mode with a message
cypher run --auto "Implement feature X"
```
...
- name: Run Cypher CLI
  run: |
    cypher run "Implement the new feature" --auto
```
...
Remote Connections let you access your local CLI sessions from the Cloud Agents web interface. Requires [Cypher Gateway](/docs/gateway) connection.
...
Requires connection to Cypher Gateway. The `/remote` command appears only when authenticated.

**Enable by default:**

Add to `~/.config/cypher/config.json`:
...
Once enabled, start a CLI session and open [Cloud Agents](https://app.cypher.ai/cloud). Your local session appears in the dashboard. See [Cloud Agent Remote Connections](/docs/code-with-ai/platforms/cloud-agent#remote-connections) for details.

### Requirements

- Connection to Cypher Gateway
- Same Cypher account on CLI and Cloud Agent
- CLI must remain running with internet connection

{% callout type="warning" title="Security Warning" %}
Anyone with access to your Cypher account can send messages to your computer when remote mode is enabled.
{% /callout %}
...
- `CYPHER_PROVIDER`: Override the active provider ID
- For `cypher` provider: `CYPHER_<FIELD_NAME>` (e.g., `CYPHER_MODEL` → `cypherModel`)
- For other providers: `CYPHER_<FIELD_NAME>` (e.g., `CYPHER_API_KEY` → `apiKey`)

## Using the CLI in an Organization

If you belong to a Cypher organization (Team or Enterprise), you can route CLI requests through that organization. The process differs slightly between interactive and non-interactive usage.
...
### Non-Interactive Usage (`cypher run`)

There is no `--org` or `--team` flag on `cypher run`. Instead, the organization is determined from the following sources, in order of priority (highest first):

1. **`CYPHER_ORG_ID` environment variable** — Best for non-interactive and CI environments.
