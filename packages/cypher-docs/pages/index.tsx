...
const terminalContent = {
  installation: (
    <>
      <span className="terminal-comment"># Install Cypher CLI VS Code Extension</span>
      {"\n"}
      <span className="terminal-prompt">$</span> code --install-extension cypher.cypher-cli
      {"\n"}
      {"\n"}
      <span className="terminal-comment"># Or install via CLI</span>
      {"\n"}
      <span className="terminal-prompt">$</span> npm install -g @cypher/cli
    </>
  ),
  gateway: (
    <>
      <span className="terminal-comment"># Call Cypher Gateway with a quick curl script</span>
      {"\n"}
      <span className="terminal-prompt">$</span> export CYPHER_API_KEY="YOUR_API_KEY"
      {"\n"}
      <span className="terminal-prompt">$</span> curl https://api.cypher.ai/api/gateway/chat/completions \{"\n"}
      -H "Authorization: Bearer $CYPHER_API_KEY" \{"\n"}
      -H "Content-Type: application/json" \{"\n"}
      {`  -d '{"model":"anthropic/claude-sonnet-4.5","messages":[{"role":"user","content":"Say hi from Cypher Gateway"}]}'`}
    </>
  ),
  firstTask: (
    <>
      <span className="terminal-comment"># Start a new task with Cypher CLI</span>
      {"\n"}
      <span className="terminal-prompt">$</span> cypher "Create a React component for a user profile"
      {"\n"}
      {"\n"}
      <span className="terminal-comment">
        # Or for interactive sessions, just run the Cypher CLI in your project folder
      </span>
      {"\n"}
      <span className="terminal-prompt">$</span> cypher
      {"\n"}
      {"\n"}
      <span className="terminal-comment"># Run in architect mode for planning</span>
      {"\n"}
      <span className="terminal-prompt">$</span> cypher --mode architect "Design a REST API"
    </>
  ),
  customRules: (
    <>
      ...
      {"\n"}
      <span className="terminal-prompt">$</span> touch .cypher/rules.md
      {"\n"}
      {"\n"}
      <span className="terminal-comment"># Or use the CLI to add rules</span>
      {"\n"}
      <span className="terminal-prompt">$</span> cypher rules add "Always use TypeScript"
      {"\n"}
      <span className="terminal-prompt">$</span> cypher rules add "Follow React best practices"
      {"\n"}
      {"\n"}
      <span className="terminal-comment"># List all active rules</span>
      {"\n"}
      <span className="terminal-prompt">$</span> cypher rules list
    </>
  ),
  cypherclaw: (
    <>
      <span className="terminal-comment"># Chat with your CypherClaw agent from the CLI</span>
      {"\n"}
      <span className="terminal-prompt">$</span> cypher /claw
      {"\n"}
      {"\n"}
      ...
      {"\n"}
      <span className="terminal-prompt">$</span> curl -X POST https://your-instance.cypherclaw.ai/webhook \{"\n"}
      -H "Content-Type: application/json" \{"\n"}
...
const categories = [
  {
    title: "Get Started",
    description: "Install Cypher CLI and get up and running in minutes",
  ...
  {
    title: "Code with AI",
    description: "Learn how to use Cypher CLI to write, edit, and understand code",
    href: "/code-with-ai",
    icon: (
      <svg className="category-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
  ...
  },
  {
    ...
    links: [
      { title: "Overview", href: "/cypherclaw/overview" },
      { title: "Chat Platforms", href: "/cypherclaw/chat-platforms" },
      { title: "Development Tools", href: "/cypherclaw/development-tools" },
    ],
  },
  {
    title: "Collaborate",
    ...
    links: [
      ...
      { title: "Cypher for Teams", href: "/collaborate" },
  ...
  {
    title: "Contributing",
    description: "Help improve Cypher CLI and learn about its architecture",
    href: "/contributing",
    icon: (
      <svg className="category-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
        <path
...
export default function HomePage() {
  const [activeTab, setActiveTab] = useState<"installation" | "firstTask" | "customRules" | "gateway" | "cypherclaw">(
    "installation",
  )
  ...
  return (
    <div className="homepage">
      <Head>
        <title>Cypher CLI Docs: Setup, Models, MCP, Custom Modes & CLI</title>
      </Head>
      ...
      <section className="hero">
        <div className="hero-content">
          <h1 className="hero-title">Cypher Documentation</h1>
          <p className="hero-subtitle">
            Explore guides and examples for the Cypher platform — from coding agents and AI-powered development to hosted
            agentic infrastructure with CypherClaw.
          </p>
          <div className="hero-buttons">
            <Link href="/getting-started" className="btn btn-primary">
              Get started with Cypher CLI →
            </Link>
            <Link href="/cypherclaw/overview" className="btn btn-secondary">
              Explore CypherClaw
            </Link>
          </div>
        </div>
        ...
        <div className="quick-panel">
          <div className="quick-section">
            <h3 className="quick-title">CYPHER CLI</h3>
          ...
          <div className="quick-section">
            <h3 className="quick-title">CYPHERCLAW</h3>
            <div className="quick-links">
              <Link href="/cypherclaw/overview" className="quick-link">
                Overview
              </Link>
              <Link href="/cypherclaw/chat-platforms" className="quick-link">
                Chat Platforms
              </Link>
              <Link href="/cypherclaw/development-tools" className="quick-link">
                Development Tools
              </Link>
            </div>
          </div>
      ...
      <section className="terminal-section">
        <div className="terminal-intro">
          ...
          <p className="terminal-description">Get started quickly with Cypher CLI, CypherClaw, and Cypher Gateway</p>
        </div>
        <div className="terminal-container">
          <div className="terminal-tabs">
            ...
            <button
              className={`terminal-tab ${activeTab === "cypherclaw" ? "active" : ""}`}
              onClick={() => setActiveTab("cypherclaw")}
            >
              CypherClaw
            </button>
            <button
        ...
        <div className="footer-grid">
          <div className="footer-item">
            ...
            <div>
              ...
              <Link href="https://cypher.ai/discord" className="footer-link">
                Join our Discord
              </Link>
            </div>
          </div>
          <div className="footer-item">
            ...
            <div>
              ...
              <Link href="https://github.com/Cypher-Org/cypher/releases" className="footer-link">
                Changelog
              </Link>
            </div>
          </div>
          <div className="footer-item">
            ...
            <div>
              ...
              <Link href="https://github.com/Cypher-Org/cypher/issues" className="footer-link">
                Report an issue
              </Link>
            </div>
          </div>
