import { createCypherClient, type Config as EffectiveConfig } from "@cypher/sdk/v2/client"
import type {
  ...
  GlobalEvent,
  CypherEmbeddingModelCatalog,
  CypherProfileResponse,
  LspStatusResponse,
  McpStatusResponse,
  Pty as PtyInfo,
  PermissionRequest,
  Project as CypherProject,
  QuestionRequest,
  ProviderAuthAuthorization,
  ProviderAuthResponse,
  ProviderListResponse,
  Session as CypherSession,
  SessionStatus,
  ToolIdsResponse,
  ToolListResponse,
...
export type CypherProfileData = CypherProfileResponse

export type ProjectItem = CypherProject
...
const key = "cypher.config.server"
...
function server(input: string) {
  ...
  const user = decode(url.username) || "cypher"
  const pass = decode(url.password) || "cypher"
  url.username = ""
  url.password = ""
  url.search = ""
  url.hash = ""
...
function owner(items: ProjectItem[], session: Pick<CypherSession, "projectID" | "directory">) {
...
export async function loadEmbeddingModels(input: Query): Promise<CypherEmbeddingModelCatalog> {
  return demand("Cypher embedding models", await client(input).indexing.models())
}

export async function loadCypherProfile(input: ProjectQuery): Promise<CypherProfileData> {
  const sdk = client(input)
  const result = await sdk.cypher.profile(directory(input))
  return demand("Cypher profile", result)
}

export async function setCypherOrganization(input: ProjectQuery, organizationId: string | null) {
  const sdk = client(input)
  const result = await sdk.cypher.organization.set({ ...directory(input), organizationId })
  demand("Switch Cypher account", result)
  await sdk.global.dispose()
}

export async function logoutCypher(input: ProjectQuery) {
  const sdk = client(input)
  const result = await sdk.auth.remove({ providerID: "cypher" })
  demand("Log out of Cypher", result)
  await sdk.global.dispose()
}

export async function startCypherLogin(input: ProjectQuery): Promise<ProviderAuthAuthorization> {
  const sdk = client(input)
  const result = await sdk.provider.oauth.authorize({ ...directory(input), providerID: "cypher", method: 0 })
  return demand("Start Cypher login", result)
}

export async function completeCypherLogin(input: ProjectQuery, signal?: AbortSignal) {
  const sdk = client(input)
  const result = await sdk.provider.oauth.callback(
    { ...directory(input), providerID: "cypher", method: 0 },
    signal ? { signal } : undefined,
  )
  demand("Complete Cypher login", result)
  await sdk.global.dispose()
}
...
export async function loadProjectTerminals(input: ProjectQuery, dir: string): Promise<ProjectTerminalItem[]> {
  ...
  const sessions = new Map<string, CypherSession>()
...
export async function createProjectPty(input: Query, dir: string, title = "Cypher session"): Promise<ProjectPtyInfo> {
  const sdk = client({ url: input.url, dir })
  const result = await sdk.pty.create({ directory: dir, command: "cypher", cwd: dir, title })
