...
import type { IndexingConfig } from "@cypher/sdk/v2/client"
...
export function IndexingRoute() {
  ...
  const cypherModels = createMemo<SelectOption<string>[]>(() => {
    const models = catalog()?.models ?? []
    if (models.length === 0) return [{ value: "", label: "No Cypher embedding models available", disabled: true }]
    return models.map((model) => ({
      value: model.id,
      label: `${model.name} (${model.note ? `${model.note}, ` : ""}${model.dimension}d)`,
    }))
  })
  const cypherModel = createMemo(() => {
    const data = catalog()
    if (!data) return ""
        ...
        <section class="builder-form agent-builder-stack">
          ...
          <Card class="ui-card agent-builder-card" padding={0}>
            ...
            <div class="ui-form agent-builder-form">
              <FieldCard
                label="Provider"
                description="Automatic uses Cypher when signed in, otherwise the provider runtime default."
                actions={
                  <SourceBadge
              ...
              <FieldCard
                ...
                description={
                  provider() === "cypher"
                    ? "Select a Cypher-hosted embedding model."
              ...
              >
                <input
                  ...
                  disabled={Boolean(ctx.saving()) || provider() === "cypher"}
                  onInput={(event) => number("dimension", event.currentTarget.value)}
                />
              </FieldCard>

              <Show when={provider() === "cypher"}>
                <div class="indexing-note">
                  Cypher embeddings use the account currently signed in to this Cypher server. Model dimensions are supplied
                  by the catalog.
                </div>
              </Show>
                ...
                fallback={
                  ...
                  >
                    <input
                      ...
                      placeholder="Default Cypher state directory"
                      disabled={Boolean(ctx.saving())}
                      onInput={(event) =>
