...
import { Badge } from "@cypher/cypher-web-ui/badge"
import { Button } from "@cypher/cypher-web-ui/button"
import { Card } from "@cypher/cypher-web-ui/card"
import { Icon } from "@cypher/cypher-web-ui/icon"
import { ResizeHandle } from "@cypher/cypher-web-ui/resize-handle"
import { Spinner } from "@cypher/cypher-web-ui/spinner"
import { File } from "@cypher/cypher-web-ui/file"
import { FileComponentProvider } from "@cypher/cypher-web-ui/context/file"
import { SessionReview, type SessionReviewDiffStyle } from "@cypher/cypher-web-ui/session-review"
...
export function ProjectConsoleRoute() {
  ...
  const [selected, setSelected] = createSignal(window.localStorage.getItem(`cypher.console.${params.project}.dir`) ?? "")
  const [active, setActive] = createSignal(window.localStorage.getItem(`cypher.console.${params.project}.pty`) ?? "")
  ...
  function labelKey(dir: string) {
    return `cypher.console.${project()}.worktree.${encodeURIComponent(dir)}.label`
  }
  ...
  function remember(dir: string, pty?: string) {
    window.localStorage.setItem(`cypher.console.${project()}.dir`, dir)
    if (pty) {
      window.localStorage.setItem(`cypher.console.${project()}.pty`, pty)
      return
    }
    window.localStorage.removeItem(`cypher.console.${project()}.pty`)
  }
  ...
  function submitEditor() {
    ...
    run("Creating worktree", async () => {
      ...
      window.localStorage.setItem(`cypher.console.${project()}.dir`, next.directory)
    })
  }
  ...
  function addSession(item = current()) {
    ...
    const label = `Cypher ${terminalsFor(item.dir).length + 1}`
      ...
      <main class="project-console-main">
        ...
        <Show when={!terminal() && !snap.loading && !snap.error && !failure()}>
          <div class="project-terminal-empty">
            ...
            <span>Use + next to a worktree to start Cypher CLI.</span>
          </div>
        </Show>
      </main>
