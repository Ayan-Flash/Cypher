...
import { completeCypherLogin, loadCypherProfile, startCypherLogin, type ProjectQuery } from "../../client"
  ...
  function start(input: ProjectQuery | undefined = server.query()) {
    ...
    void startCypherLogin(input)
      .then((info) => {
        ...
        return completeCypherLogin(input, ctl.signal).then(() => true)
      })
      .then((ok) => {
        if (!ok || attempt() !== rev) return
        setAuth({ status: "success" })
        void loadCypherProfile(input)
    ...
    <section class="route-empty">
      <div class="profile-login-page">
        <header class="profile-login-header">
          <p class="eyebrow">Cypher Login</p>
          <h1>Login to Cypher</h1>
          <p>Authorize this Cypher Console through the same device auth flow used by the editor clients.</p>
        </header>
        ...
        <Show when={!server.query() && !server.discoverable()}>
          <Card class="profile-login-card" variant="warning">
            ...
            <p>Start a local Cypher server or pass a server URL with ?server=.</p>
