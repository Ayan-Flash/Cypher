...
import { loadCypherProfile, logoutCypher, setCypherOrganization, type CypherProfileData, type ProjectQuery } from "../../client"
...
type Org = NonNullable<CypherProfileData["profile"]["organizations"]>[number]
type State = { kind: "connected"; data: CypherProfileData } | { kind: "disconnected" }

async function load(input: ProjectQuery): Promise<State> {
  if (wasDisconnected()) return { kind: "disconnected" }
  try {
    const data = await loadCypherProfile(input)
...
export function ProfileRoute() {
  ...
  const login = () => page(params(), "/cypher/login")
  ...
  function choose(id: string | null) {
    ...
    setSaving(next)
    setError("")
    void setCypherOrganization(current, id)
        ...
        <div class="profile-page">
          <Show when={!disconnected()}>
            <header class="profile-header">
              <div>
                <p class="eyebrow">Cypher Account</p>
                <h1>Your Profile</h1>
                <p>Manage your Cypher identity, account context, credits, and billing shortcuts.</p>
              </div>
          ...
          <Show when={disconnected()}>
            <Card class="profile-connect-card">
              ...
              <div class="profile-connect-copy">
                <p class="eyebrow">Cypher Account</p>
                <h1>Connect your Cypher account</h1>
                <p>Sign in to view your credits, organizations, and account details in Cypher Console.</p>
              </div>
              <A
                class="profile-primary-link"
                data-component="button"
          ...
          <Show when={!server.query() && !server.discoverable()}>
            <Card class="profile-banner" variant="warning">
              <strong>Cypher server not found</strong>
              <span>Start Cypher Console from a running Cypher server or pass a server URL with ?server=.</span>
            </Card>
          </Show>
                  ...
                  <div class="profile-org-list">
                    ...
                    >
                      ...
                      <span class="profile-org-body">
                        ...
                        <span>Your personal Cypher credits and settings</span>
                      </span>
