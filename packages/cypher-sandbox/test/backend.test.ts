...
describe("sandbox launch preparation", () => {
  ...
  test("keeps the host network namespace in Linux allow mode", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "cypher-bubblewrap-policy-"))
    ...
    try {
      const result = generateBubblewrap(profile, { ...launch, cwd: root }, "/opt/cypher/bwrap")
      const writable = result.args.indexOf("--bind")
      const protectedPath = result.args.indexOf("--ro-bind", writable + 1)
      expect(result.command).toBe("/opt/cypher/bwrap")
  ...
  test("isolates the Linux network namespace in deny mode", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "cypher-bubblewrap-network-"))
  ...
  test("parses escaped mount points from Linux mountinfo", () => {
    const content = [
      ...
      String.raw`37 36 0:33 / /tmp/cypher\040root rw - tmpfs tmpfs rw`,
      String.raw`38 37 0:34 / /tmp/cypher\040root/nested\011mount rw - tmpfs tmpfs rw`,
      String.raw`39 36 0:35 / /tmp/back\134slash rw - tmpfs tmpfs rw`,
      "",
    ].join("\n")

    expect(parseMountinfo(content)).toEqual(["/", "/tmp/cypher root", "/tmp/cypher root/nested\tmount", "/tmp/back\\slash"])
  })

  test("allows a mounted writable root but rejects nested mount points", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "cypher-bubblewrap-mount-"))
    ...
    try {
      expect(() => generateBubblewrap(profile, launch, "/opt/cypher/bwrap", [root])).not.toThrow()
      expect(() => generateBubblewrap(profile, launch, "/opt/cypher/bwrap", [root, nested])).toThrow(
  ...
  test("rejects a Bubblewrap executable inside a writable root", () => {
    const root = mkdtempSync(path.join(os.tmpdir(), "cypher-bubblewrap-helper-"))
