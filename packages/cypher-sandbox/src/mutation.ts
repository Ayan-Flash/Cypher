...
declare const CYPHER_SANDBOX_MUTATION_WORKER_PATH: string

function worker() {
  if (typeof CYPHER_SANDBOX_MUTATION_WORKER_PATH === "undefined") {
    return { path: fileURLToPath(new URL("./cypher-sandbox-mutation-worker.ts", import.meta.url)), environment: {} }
  }
  const path = CYPHER_SANDBOX_MUTATION_WORKER_PATH.startsWith(".")
    ? fileURLToPath(new URL(CYPHER_SANDBOX_MUTATION_WORKER_PATH, import.meta.url))
    : resolve(dirname(process.execPath), CYPHER_SANDBOX_MUTATION_WORKER_PATH)
...
const CurrentRunner = Context.Reference<Runner>("@cypher/sandbox/CurrentMutationRunner", {
  defaultValue: () => mutate,
})
