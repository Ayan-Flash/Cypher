...
import { CypherMemory } from "../src/effect/index"
...
async function tmp() {
  const dir = await mkdtemp(path.join(os.tmpdir(), "cypher-memory-effect-"))
  return {
...
describe("MemoryCapture (fake ports)", () => {
  test("turn-close typed LLM saves environment memory and audit records", async () => {
    const t = await tmp()
    try {
      await CypherMemory.enable({ root: t.root })
      await CypherMemory.configure({ root: t.root, settings: { autoConsolidate: true } })
      ...
      const shown = await CypherMemory.show({ root: t.root })
  ...
  test("auto-consolidate off skips digest and typed model writes", async () => {
    ...
    try {
      await CypherMemory.enable({ root: t.root })
      await CypherMemory.configure({ root: t.root, settings: { autoConsolidate: false } })
...
describe("MemoryTimers signal ref-counting", () => {
  test("shares one controller per root and drops it once the last capture releases", () => {
    const root = "/cypher-memory/ref-count-root"
