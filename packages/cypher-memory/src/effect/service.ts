...
import { CypherMemory } from "./index"
...
type ConfigureInput = CypherMemory.Input & {
  settings: Partial<Pick<MemorySchema.State, "autoConsolidate">>
}

type ApplyInput = CypherMemory.Input & {
...
type RememberInput = CypherMemory.Input & {
  text: string
  key?: string
...
type CorrectInput = CypherMemory.Input & {
  text: string
  key?: string
}

type ForgetInput = CypherMemory.Input & {
  query: string
}

type RecallInput = CypherMemory.Input & {
  query: string
  sessionID?: string
}
...
type RecordInput = CypherMemory.Input & {
  sessionID: string
  topic?: string
  summary: string
  time?: number
...
export namespace MemoryService {
  ...
  export interface Interface {
    readonly prepare: (input: CypherMemory.Input) => Effect.Effect<string, Failure>
    readonly status: (input: CypherMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.status>>, Failure>
    readonly show: (input: CypherMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.show>>, Failure>
    readonly enable: (input: CypherMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.enable>>, Failure>
    readonly disable: (
      input: CypherMemory.Input,
    ) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.disable>>, Failure>
    readonly rebuild: (
      input: CypherMemory.Input,
    ) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.rebuild>>, Failure>
    readonly configure: (
      input: ConfigureInput,
    ) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.configure>>, Failure>
    readonly apply: (input: ApplyInput) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.apply>>, Failure>
    readonly remember: (input: RememberInput) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.remember>>, Failure>
    readonly correct: (input: CorrectInput) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.correct>>, Failure>
    readonly forget: (input: ForgetInput) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.forget>>, Failure>
    readonly purge: (input: CypherMemory.Input) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.purge>>, Failure>
    readonly recall: (input: RecallInput) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.recall>>, Failure>
    ...
    readonly recordSession: (
      input: RecordInput,
    ) => Effect.Effect<Awaited<ReturnType<typeof CypherMemory.recordSession>>, Failure>
  ...
  export class Service extends Context.Service<Service, Interface>()("@cypher/MemoryService") {}

  export function make() {
    const locks = new Map<SessionID, { sema: Semaphore.Semaphore; holders: number }>()
    let settle = IDLE_SETTLE_MS
    return Service.of({
      prepare: (input) => bridge(() => CypherMemory.prepare(input)),
      status: (input) => bridge(() => CypherMemory.status(input)),
      show: (input) => bridge(() => CypherMemory.show(input)),
      enable: (input) => bridge(() => CypherMemory.enable(input)),
      disable: (input) => bridge(() => CypherMemory.disable(input)),
      rebuild: (input) => bridge(() => CypherMemory.rebuild(input)),
      configure: (input) => bridge(() => CypherMemory.configure(input)),
      apply: (input) => bridge(() => CypherMemory.apply(input)),
      remember: (input) => bridge(() => CypherMemory.remember(input)),
      correct: (input) => bridge(() => CypherMemory.correct(input)),
      forget: (input) => bridge(() => CypherMemory.forget(input)),
      purge: (input) => bridge(() => CypherMemory.purge(input)),
      recall: (input) => bridge(() => CypherMemory.recall(input)),
      search: (input) => bridge(() => MemoryRecall.search(input)),
      recordSession: (input) => bridge(() => CypherMemory.recordSession(input)),
