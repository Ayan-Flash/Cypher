...
const reg = process.env.REGISTRY ?? "ghcr.io/cypher-org" // cypher_change
...
const images = ["base", "bun-node", "jetbrains", "rust", "tauri-linux", "publish"] // cypher_change

const setup = async () => {
  if (!push) return
  const list = await $`docker buildx ls`.text()
  // cypher_change start
  if (list.includes("cypher")) {
    await $`docker buildx use cypher`
    return
  }
  await $`docker buildx create --name cypher --use`
  // cypher_change end
}
