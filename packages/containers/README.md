...
- `jetbrains`: `bun-node` plus Java 21, JBR font libraries, and pre-cached Gradle and Bun packages <!-- cypher_change -->
...
```
REGISTRY=ghcr.io/cypher-org TAG=24.04 bun ./packages/containers/script/build.ts
REGISTRY=ghcr.io/cypher-org TAG=24.04 bun ./packages/containers/script/build.ts --push
```
...
jobs:
  build-cli:
    ...
    container:
      image: ghcr.io/cypher-org/build/bun-node:24.04
```
