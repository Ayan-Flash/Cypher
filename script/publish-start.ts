...
const skipNotes = process.env["CYPHER_SKIP_NOTES"] === "1" // cypher_change
if (skipNotes) console.log("changelog skipped: CYPHER_SKIP_NOTES=1") // cypher_change
...
console.log("\n=== opencode ===\n")
await import(`../packages/opencode/script/legacy-publish`)

console.log("\n=== sdk ===\n")
await import(`../packages/sdk/js/script/publish`)

console.log("\n=== plugin ===\n")
await import(`../packages/plugin/script/publish`)
...
if (!Script.preview) {
  ...
  // cypher_change start - skip draft flag when CYPHER_SKIP_NOTES=1 (used by publish-stable.yml which doesn't have a publish-complete step)
