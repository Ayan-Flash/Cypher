...
const OUTPUT = path.join(ROOT, "packages/cypher-docs/source-links.md")

const check = process.argv.includes("--check")

const DIRS = [
  path.join(ROOT, "packages/cypher-vscode/src"),
  path.join(ROOT, "packages/cypher-vscode/webview-ui"),
...
const EXCLUDE_PATTERNS = [
  ...
  /^https?:\/\/api\.cypher\.ai\/api\//,
  /^https?:\/\/supermassive-black-hole\.cypherapps\.io\/v1\/session-export\//, // cypher_change
  /^https?:\/\/ingest\.cyphersessions\.ai/,
  /^https?:\/\/api\.openai\.com/,
  /^https?:\/\/api\.github\.com/,
  /^https?:\/\/api\.githubcopilot\.com/,
  /^https?:\/\/[^/]+\.openai\.azure\.com\/openai/, // cypher_change
...
const SKIP_FILES = ["check-forbidden-strings.ts"] // cypher_change
...
function shouldSkipFile(filepath: string): boolean {
  if (filepath === "packages/opencode/src/cli/cmd/account.ts") return true // cypher_change - command is not registered in Cypher
  ...
  if (parts.includes("i18n") && path.basename(filepath) !== "en.ts") return true // cypher_change
...
if (check) {
  const committed = await Bun.file(OUTPUT)
    ...
    .catch(() => "")
  if (committed === output) {
    console.log("packages/cypher-docs/source-links.md is up to date.")
    process.exit(0)
  }
  console.error(
    [
      "ERROR: packages/cypher-docs/source-links.md is out of date.",
      "",
...
console.log(`Wrote ${sorted.length} unique URLs to packages/cypher-docs/source-links.md`)
