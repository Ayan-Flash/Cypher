#!/usr/bin/env bun
/**
 * Shared cypher_change marker helpers used by both the marker fixer and the
...
export const standalone = [
  /^\s*\/\/\s*cypher_change\b.*$/,
  /^\s*#\s*cypher_change\b.*$/,
  /^\s*\{?\s*\/\*\s*cypher_change\b.*\*\/\}?\s*$/,
]
export const start = /\bcypher_change\s+start\b/
export const end = /\bcypher_change\s+end\b/
export const freshmark = /\bcypher_change\s*-\s*new\s*file\b/
...
function strip(file: string, line: string): { line: string | null; mark?: string } {
  ...
  if (style(file) === "hash") return comment(line, [/^#\s*cypher_change\b/])
  return comment(line, [/^\{\/\*\s*cypher_change\b/, /^\/\*\s*cypher_change\b/, /^\/\/\s*cypher_change\b/])
}
...
function block(mode: Style, pad: string) {
  if (mode === "hash") return { start: `${pad}# cypher_change start`, end: `${pad}# cypher_change end` }
  if (mode === "jsx") return { start: `${pad}{/* cypher_change start */}`, end: `${pad}{/* cypher_change end */}` }
  if (mode === "block") return { start: `${pad}/* cypher_change start */`, end: `${pad}/* cypher_change end */` }
  return { start: `${pad}// cypher_change start`, end: `${pad}// cypher_change end` }
}

function note(mode: Style) {
  if (mode === "hash") return " # cypher_change"
  if (mode === "jsx") return " {/* cypher_change */}"
  if (mode === "block") return " /* cypher_change */"
  return " // cypher_change"
}
...
export function fresh(file: string, clean: Clean) {
  ...
  const line = clean.marks.file ?? (mode === "hash" ? "# cypher_change - new file" : "// cypher_change - new file")
...
export async function changed(base: Text, head: Text, opts?: { ignoreWhitespace?: boolean }): Promise<Diff> {
  const dir = await mkdtemp(path.join(tmpdir(), "cypher-markers-"))
