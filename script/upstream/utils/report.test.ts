...
test("recommends keep ours for Cypher directories", () => {
  expect(getRecommendation("packages/cypher-vscode/.prettierignore", [], []).recommendation).toBe("keep-ours")
  expect(getRecommendation("packages/cypher-i18n/tsconfig.json", [], []).recommendation).toBe("keep-ours")
})
