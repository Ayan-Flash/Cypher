...
function compatTag(message: string): string | null {
  const prefix = "refactor: cypher compat for "
...
export async function findLatestCompatCommit(base: string, target: string): Promise<CompatBase | null> {
  const grep = "^refactor: cypher compat for "
 ...
 * Used to clean build artifacts from Cypher-specific directories after checking
...
/**
 * Check if the "ours" version of a conflicted file contains cypher_change markers.
 ...
 */
export async function oursHasCyphercodeChanges(file: string): Promise<boolean> {
  const result = await $`git show :2:${file}`.quiet().nothrow()
  if (result.exitCode !== 0) return false
  return result.stdout.toString().includes("cypher_change")
}
...
export async function trainRerere(grep: string): Promise<number> {
  ...
  const dir = join(tmpdir(), `cypher-rerere-train-${randomUUID()}`)
