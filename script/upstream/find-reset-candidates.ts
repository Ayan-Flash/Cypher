 ...
 *   - markers-only        : only diff is cypher_change markers wrapping
 ...
 *   - upstream-missing    : file does not exist upstream (cypher-only, skipped)
...
const CYPHER_ONLY_PATHSPECS = [
  ":(exclude,glob)packages/cypher-*/**",
  ":(exclude,glob)**/cypher/**",
...
async function candidates(commit: string, scope: string | undefined, top: string): Promise<CandidateSet> {
  const pathspecs = [scope ?? ".", ...CYPHER_ONLY_PATHSPECS]
...
/**
 * Files the upstream merge config marks as "keep ours" (Cypher-specific preserved
 * versions) or "skip" (upstream-only, removed in Cypher) should never be touched
 * by the bulk resetter. They show up in `git diff` against raw upstream but
 * resetting them would undo deliberate Cypher decisions.
 */
