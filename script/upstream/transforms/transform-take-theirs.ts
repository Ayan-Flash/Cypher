#!/usr/bin/env bun
/**
 * Transform files by taking upstream version and applying Cypher branding
 *
 * This script handles files that have only branding differences (no logic changes).
 * It takes the upstream version and applies Cypher branding transforms.
 *
 * Use this for:
 * - UI components with OpenCode -> Cypher branding
 * - Config files with predictable patterns
 * - Files without cypher_change logic blocks
 */
...
export function applyBrandingTransforms(content: string, verbose = false): { result: string; replacements: number } {
  ...
  for (const line of lines) {
    // Skip lines with cypher_change marker (already customized)
    if (line.includes("// cypher_change")) {
