...
test("fixScripts preserves Cypher-only root scripts from base", () => {
  const ours = {
    scripts: {
      "dev-setup": "cypher dev-setup",
      postinstall: "bun run --cwd packages/opencode fix-node-pty && bun run script/setup-git.ts",
      extension: "bun --cwd packages/cypher-vscode script/launch.ts",
    },
  }
...
test("fixMetadata preserves opencode publish metadata from base", () => {
  const ours = { keywords: ["cli", "cypher", "opencode"], private: false }
...
test("mergeWithNewestVersions preserves ours' key order so cypher-only deps don't relocate", () => {
  // Regression: when ours has a cypher-only dep in the middle (e.g. rotating-file-stream
...
test("selectBunPackageManager keeps the newer Bun version and prefers Cypher on ties", () => {
  ...
  expect(selectBunPackageManager("bun@1.3.14+cypher", "bun@1.3.14+upstream")).toBe("bun@1.3.14+cypher")
})
...
test("fixPackageManager prevents root Bun downgrades", () => {
  ...
  expect(changes).toEqual(["packageManager: bun@1.3.13 -> bun@1.3.14 (preserved Cypher pin)"])
})

test("fixPackageManager restores a valid Cypher pin over malformed upstream", () => {
  ...
  expect(pkg.packageManager).toBe("bun@1.3.14")
  expect(changes).toEqual(["packageManager: bun@latest -> bun@1.3.14 (preserved Cypher pin)"])
})
