#!/usr/bin/env bun
/**
 * Enhanced package.json transform with Cypher dependency injection
 *
 * This script handles package.json conflicts by:
 * 1. Taking upstream's version (to get new dependencies)
 * 2. Transforming package names (opencode -> cypher)
 * 3. Injecting Cypher-specific dependencies
 * 4. Preserving Cypher's version number
 * 5. Preserving overrides and patchedDependencies
 * 6. Preserving Cypher's repository configuration
...
export function fixPackageManager(
  ...
  changes.push(`packageManager: ${prior} -> ${next} (preserved Cypher pin)`)
 ...
 *
 * Key order preserves ours' order first (so cypher-only deps stay in their
  ...
  if (ours) {
    for (const [name, ourVersion] of Object.entries(ours)) {
      ...
      if (theirVersion === undefined) {
        ...
        changes.push(`${section}: preserved ${name}@${ourVersion} (cypher-only)`)
        continue
      }
      ...
      if (cmp === null) {
        ...
        result[name] = ourVersion
        changes.push(`${section}: ${name} ${theirVersion} -> ${ourVersion} (cypher newer)`)
      } else {
        result[name] = theirVersion
...
const PACKAGE_NAME_MAP: Record<string, string> = {
  "opencode-ai": "@cypher/cli",
  "@opencode-ai/cli": "@cypher/cli",
  "@opencode-ai/sdk": "@cypher/sdk",
  "@opencode-ai/plugin": "@cypher/plugin",
}

// Cypher-specific dependencies to inject into specific packages
// NOTE: When adding new Cypher-specific workspace dependencies (packages starting with @cypher/cypher-*),
// add them here to prevent them from being removed during upstream merges
const CYPHER_DEPENDENCIES: Record<string, Record<string, string>> = {
  // packages/opencode/package.json needs these
  "packages/opencode/package.json": {
    "@cypher/cypher-gateway": "workspace:*",
    "@cypher/cypher-telemetry": "workspace:*",
  },
}

// Cypher-specific bin entries to set on specific packages
const CYPHER_BIN: Record<string, Record<string, string>> = {
  "packages/opencode/package.json": {
    cypher: "./bin/cypher",
    cypher: "./bin/cypher",
  },
...
const TRANSFORM_PACKAGE_NAMES: Record<string, string> = {
  "package.json": "@cypher/cypher",
  "packages/opencode/package.json": "@cypher/cli",
  "packages/plugin/package.json": "@cypher/plugin",
  "packages/sdk/js/package.json": "@cypher/sdk",
}

// Cypher-specific scripts to preserve from the base branch per package.json.
...
// Cypher doesn't ship (desktop-electron, console/app, app) and would otherwise
...
// in by upstream features (e.g. desktop Sentry integration) that Cypher doesn't
...
/**
 * Re-apply Cypher-specific scripts on top of the upstream-shaped scripts block,
 * and prune upstream-only scripts that target packages Cypher doesn't ship.
 */
export function fixScripts(
  ...
  for (const name of DELETE_UPSTREAM_SCRIPTS[path] || []) {
    if (theirs[name]) {
      delete theirs[name]
      changes.push(`scripts.${name}: removed (upstream-only, no Cypher target)`)
    }
  }
...
/**
 * Prune upstream-only catalog entries that have no consumers in Cypher.
 */
export function fixCatalog(pkg: Record<string, unknown>, path: string, changes: string[]): void {
  ...
  for (const name of DELETE_UPSTREAM_CATALOG[path] || []) {
    if (cat[name]) {
      delete cat[name]
      changes.push(`workspaces.catalog.${name}: removed (upstream-only, no Cypher consumer)`)
    }
  }
}
...
export async function transformPackageJson(file: string, options: PackageJsonOptions = {}): Promise<PackageJsonResult> {
  ...
  if (await oursHasCyphercodeChanges(file)) {
  ...
  try {
    // Save Cypher's version BEFORE taking theirs
    ...
    // 2. Preserve Cypher version if requested
    if (options.preserveVersion !== false) {
      const cypherVersion = await getCurrentVersion()
      if (pkg.version !== cypherVersion) {
        changes.push(`version: ${pkg.version} -> ${cypherVersion}`)
        pkg.version = cypherVersion
      }
    }
    ...
    if (ourPkg) {
      ...
      // 5. Preserve patchedDependencies (Cypher-specific, upstream won't have these)
      ...
      // 6. Preserve repository (Cypher-specific, upstream doesn't have this)
      const ourRepo = ourPkg.repository
      if (ourRepo && JSON.stringify(pkg.repository) !== JSON.stringify(ourRepo)) {
        pkg.repository = ourRepo
        changes.push(`repository: preserved Cypher's repository configuration`)
      }

      fixMetadata(pkg, relativePath, ourPkg, changes)

      // 7. Handle workspaces for root package.json
      // Cypher has removed hosted platform packages (console/*, slack, etc.)
      // so we need to preserve Cypher's workspace configuration instead of taking upstream's
      ...
      if (relativePath === "package.json" && ourWorkspaces?.packages) {
        ...
        changes.push(`workspaces.packages: preserved Cypher's workspace configuration`)
      }
    ...
    // 7. Transform dependency names (opencode -> cypher)
    if (pkg.dependencies) {
    ...
    // 8. Inject Cypher-specific dependencies
    const cypherDeps = CYPHER_DEPENDENCIES[relativePath]
    if (cypherDeps) {
      pkg.dependencies = pkg.dependencies || {}
      for (const [name, version] of Object.entries(cypherDeps)) {
    ...
    // 9. Set Cypher-specific bin entries
    const cypherBin = CYPHER_BIN[relativePath]
    if (cypherBin) {
      pkg.bin = cypherBin
      changes.push(`bin: set Cypher bin entries`)
    }
...
/**
 * Get Cypher's package.json from the base branch (main) for comparison
 * Used during pre-merge to compare upstream versions against Cypher's versions
 */
async function getCypherPackageJson(path: string, baseBranch = "main"): Promise<Record<string, unknown> | null> {
  try {
    ...
    // File might not exist in Cypher
    return null
  }
}
 ...
 * This function merges Cypher's versions with upstream, using "newest wins" strategy
 */
export async function transformAllPackageJson(options: PackageJsonOptions = {}): Promise<PackageJsonResult[]> {
  ...
  for await (const path of glob.scan({ absolute: false })) {
    ...
    try {
      ...
      // Get Cypher's version from base branch for comparison
      const cypherPkg = await getCypherPackageJson(path)
      ...
      fixPackageManager(pkg, path, cypherPkg, changes)
      ...
      // 3. Merge dependencies with "newest wins" strategy (if Cypher has this file)
      if (cypherPkg) {
        pkg.dependencies = mergeWithNewestVersions(
          cypherPkg.dependencies as Record<string, string> | undefined,
          pkg.dependencies,
          changes,
          "dependencies",
        )
        ...
        pkg.devDependencies = mergeWithNewestVersions(
          cypherPkg.devDependencies as Record<string, string> | undefined,
          pkg.devDependencies,
          changes,
          "devDependencies",
        )
        ...
        pkg.peerDependencies = mergeWithNewestVersions(
          cypherPkg.peerDependencies as Record<string, string> | undefined,
          pkg.peerDependencies,
          changes,
          "peerDependencies",
        )
        ...
        // 4. Preserve/merge overrides
        const cypherOverrides = cypherPkg.overrides as Record<string, string> | undefined
        if (cypherOverrides || pkg.overrides) {
          pkg.overrides = mergeWithNewestVersions(cypherOverrides, pkg.overrides, changes, "overrides")
        }

        // 5. Preserve patchedDependencies (Cypher-specific, upstream won't have these)
        const cypherPatchedDeps = cypherPkg.patchedDependencies as Record<string, string> | undefined
        if (cypherPatchedDeps) {
          pkg.patchedDependencies = pkg.patchedDependencies || {}
          for (const [name, patch] of Object.entries(cypherPatchedDeps)) {
            if (!pkg.patchedDependencies[name]) {
              pkg.patchedDependencies[name] = patch
              changes.push(`patchedDependencies: preserved ${name}`)
            }
        ...
        // 6. Preserve repository (Cypher-specific, upstream doesn't have this)
        const cypherRepo = cypherPkg.repository
        if (cypherRepo && JSON.stringify(pkg.repository) !== JSON.stringify(cypherRepo)) {
          pkg.repository = cypherRepo
          changes.push(`repository: preserved Cypher's repository configuration`)
        }

        fixMetadata(pkg, path, cypherPkg, changes)

        // 7. Handle workspaces for root package.json
        // Cypher has removed hosted platform packages (console/*, slack, etc.)
        // so we need to preserve Cypher's workspace configuration instead of taking upstream's
        const cypherWorkspaces = cypherPkg.workspaces as
        ...
        if (path === "package.json" && cypherWorkspaces?.packages) {
          pkg.workspaces = pkg.workspaces || {}
          pkg.workspaces.packages = cypherWorkspaces.packages
          changes.push(`workspaces.packages: preserved Cypher's workspace configuration`)
        }

        fixScripts(pkg, path, cypherPkg, changes)

        // Merge catalog with "newest wins" strategy
        if (cypherWorkspaces?.catalog || upstreamWorkspaces?.catalog) {
          pkg.workspaces = pkg.workspaces || {}
          pkg.workspaces.catalog = mergeWithNewestVersions(
            cypherWorkspaces?.catalog,
      ...
      // 8. Inject Cypher-specific dependencies
      const cypherDeps = CYPHER_DEPENDENCIES[path]
      if (cypherDeps) {
        pkg.dependencies = pkg.dependencies || {}
        for (const [name, version] of Object.entries(cypherDeps)) {
          if (!pkg.dependencies[name]) {
      ...
      // 9. Set Cypher-specific bin entries
      const cypherBin = CYPHER_BIN[path]
      if (cypherBin) {
        pkg.bin = cypherBin
        changes.push(`bin: set Cypher bin entries`)
      }
 ...
 * cypher_change markers (manual review needed).
 */
export async function reconcilePackageJsonFromRefs(
  ...
  if (await oursHasCyphercodeChanges(file)) {
    warn(`${file} has cypher_change markers — skipping reconcile, needs manual resolution`)
...
export async function reconcileAllPackageJson(options: ReconcileOptions): Promise<PackageJsonResult[]> {
  ...
  // upstream-only and cypher-only files alike.
