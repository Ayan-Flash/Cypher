#!/usr/bin/env bun
/**
 * Keep Cypher's version of specific files during merge
 *
 * This script handles files that should always keep Cypher's version
...
export function shouldKeepOurs(filePath: string, patterns: string[]): boolean {
  const dirs = defaultConfig.cypherDirectories.map((dir) => `${dir}/**`)
  return matches(filePath, [...patterns, ...dirs]) || patterns.some((pattern) => filePath.includes(pattern))
}

/**
 * Keep Cypher's version of conflicted files
 */
...
/**
 * Reset specific files to Cypher's version (even if not conflicted)
 */
export async function resetToOurs(files: string[], options: KeepOursOptions = {}): Promise<KeepOursResult[]> {
  ...
  for (const file of files) {
    if (options.dryRun) {
      ...
      try {
        // Get the file from HEAD (Cypher's version)
...
if (import.meta.main) {
  ...
  console.log()
  success(`Kept Cypher's version for ${kept.length} files`)
