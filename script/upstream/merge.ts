 ...
 *
 * Automates the process of merging upstream opencode changes into Cypher.
 *
 * Usage:
...
function logWorktrees(refs: worktree.RefInfo, input: worktree.RefInput, baseName: string): void {
  ...
  logger.info(`  cypher-main:  ${refs.main} (${baseName}, ${input.base.slice(0, 8)})`)
  ...
  logger.info(`  - Cypher base main: ${refs.main}`)
...
async function main() {
  ...
  logger.header("Cypher Upstream Merge Tool")
  ...
  if (keepOursCount > 0) {
    conflictReport.recommendations.push(`${keepOursCount} files will keep Cypher's version`)
  }
  if (codemodCount > 0) {
  ...
  const cypherVersion = await version.getCurrentCypherVersion()
  ...
  // Create Cypher merge branch
  const cypherBranch = `${author}/cypher-opencode-${targetVersion.tag}`
  const cypherBackup = await git.backupAndDeleteBranch(cypherBranch)
  if (cypherBackup) {
    logger.info(`Backed up existing branch to: ${cypherBackup}`)
  }
  await git.createBranch(cypherBranch)

  if (options.push) {
    await git.push(config.originRemote, cypherBranch, true)
  }
  logger.info(`Created Cypher branch: ${cypherBranch}`)
  ...
  // This reduces conflicts by transforming upstream code to Cypher conventions BEFORE merging
  logger.step(6, 8, "Applying transformations to opencode branch (pre-merge)...")

  logger.info("Removing files skipped in Cypher...")
  ...
  // 6a. Transform package names (opencode-ai -> @cypher/cli)
  ...
  // 6b. Preserve Cypher versions
  logger.info("Preserving Cypher versions...")
  const versionResults = await preserveAllVersions({
    dryRun: false,
    verbose: options.verbose,
    targetVersion: cypherVersion,
  })
  logger.success(`Preserved versions in ${versionResults.length} files`)

  // 6c. Transform i18n files (OpenCode -> Cypher branding)
  ...
  if (i18nPreCount > 0) {
    logger.success(`Transformed ${i18nPreCount} i18n files with Cypher branding`)
  }
  ...
  if (brandingCount > 0) {
    logger.success(`Transformed ${brandingCount} files with Cypher branding`)
  }

  // 6f. Transform package.json files (names, deps, Cypher injections)
  ...
  // 6j. Reset keep-ours files to Cypher's version
  logger.info("Resetting Cypher-specific files...")
  const keepOursResults = await resetToOurs(config.keepOurs, { dryRun: false, verbose: options.verbose })
  logger.success(`Reset ${keepOursResults.length} files to Cypher's version`)
  ...
  // Clean untracked build artifacts from Cypher-specific directories.
  ...
  logger.info("Cleaning Cypher-specific directory artifacts...")
  await git.cleanDirectories(config.cypherDirectories)

  // Commit all transformations
  await git.stageAll()
  const compatMessage = `refactor: cypher compat for ${targetVersion.tag}`
  if (prior) {
  ...
  // Step 7: Merge into Cypher branch
  logger.step(7, 8, "Merging into Cypher branch...")

  await git.checkout(cypherBranch)
  if (prior) {
    const linked = await git.recordAncestor(targetVersion.commit, `merge: record upstream ${targetVersion.tag}`)
    if (linked) logger.info(`Recorded upstream ${targetVersion.tag} as Cypher branch ancestry`)
  }
  ...
  if (!mergeResult.success) {
    ...
    // These are likely files with cypher_change markers or actual logic differences.

    // Step 7a: Skip files that shouldn't exist in Cypher
    logger.info("Removing files that shouldn't exist in Cypher...")
    ...
    logger.info("Keeping Cypher-specific files...")
    ...
    if (autoResolved.length > 0) {
      logger.success(`Auto-resolved ${autoResolved.length} conflicts (kept Cypher's version)`)
    }
    ...
    // Files with cypher_change markers are flagged for manual resolution instead.
    ...
    if (conflictedFiles.length > 0) {
      ...
      // cypher_change markers, plus JSON/YAML/TOML key merges and other
      ...
      if (i18nFlagged.length > 0) {
        logger.warn(`${i18nFlagged.length} i18n file(s) have cypher_change markers — flagged for manual resolution`)
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (takeFlagged.length > 0) {
          logger.warn(
            `${takeFlagged.length} branding file(s) have cypher_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (pkgFlagged.length > 0) {
          logger.warn(
            `${pkgFlagged.length} package.json file(s) have cypher_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (scriptFlagged.length > 0) {
          logger.warn(
            `${scriptFlagged.length} script file(s) have cypher_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (extFlagged.length > 0) {
          logger.warn(
            `${extFlagged.length} extension file(s) have cypher_change markers — flagged for manual resolution`,
          )
      ...
      if (conflictedFiles.length > 0) {
        ...
        if (webFlagged.length > 0) {
          logger.warn(
            `${webFlagged.length} web/docs file(s) have cypher_change markers — flagged for manual resolution`,
          )
    ...
    // Combine git-reported conflicts with files flagged due to cypher_change markers
    const allManual = [...new Set([...remaining, ...flaggedFiles])]
    if (allManual.length > 0) {
      if (flaggedFiles.length > 0) {
        logger.warn(`${flaggedFiles.length} file(s) were flagged because they contain cypher_change markers:`)
        logger.list(flaggedFiles)
        logger.info("  These files have intentional Cypher-specific changes. Keep our version or merge carefully.")
        logger.info("")
      }
      ...
      logger.info("")
      logger.info("These conflicts contain cypher_change markers or actual code differences.")
      ...
      conflictReport.mergeBranch = cypherBranch
      const reportPath = `upstream-merge-report-${targetVersion.version}.md`
      await report.saveReport(conflictReport, reportPath)
      logger.success(`Report saved to ${reportPath}`)
      ...
      logger.info(`  3. git push ${config.originRemote} ${cypherBranch}`)
      logger.info("  4. Create PR from " + cypherBranch + " to " + config.baseBranch)
      logger.info("")
  ...
  if (options.push) {
    await git.push(config.originRemote, cypherBranch)
    logger.success(`Pushed ${cypherBranch} to ${config.originRemote}`)
  }
  ...
  logger.info(`Cypher branch: ${cypherBranch}`)
  ...
  if (remainingConflicts.length > 0) {
    ...
    logger.info("  2. Create PR from " + cypherBranch + " to " + config.baseBranch)
  }
