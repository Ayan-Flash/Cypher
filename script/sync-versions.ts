...
// Sync every Cypher version string across the monorepo to a single target.
//
...
// `.cypher/command/upstream-manual-merge.md`). It's also handy mid-merge to
// rebase our version bumps onto any new Cypher main releases.
//
// Usage:
...
//   - the five Cypher-Org download URLs inside that toml
//
// Intentionally NOT touched:
//   - `packages/cypher-jetbrains/**` — the JetBrains plugin has its own release
...
for await (const rel of glob.scan({ cwd: root, onlyFiles: true })) {
  ...
  if (rel.startsWith("packages/cypher-jetbrains/")) continue
...
if (await Bun.file(zed).exists()) {
  ...
  const next = text
    ...
    .replace(
      /https:\/\/github\.com\/Cypher-Org\/cypher\/releases\/download\/v[^/]+\//g,
      `https://github.com/Cypher-Org/cypher/releases/download/v${target}/`,
    )
  if (next !== text) {
