...
const props = new URL("../packages/cypher-jetbrains/gradle.properties", import.meta.url).pathname
const log = new URL("../packages/cypher-jetbrains/CHANGELOG.md", import.meta.url).pathname
...
if (!(await pinned())) {
  throw new Error("packages/cypher-jetbrains/gradle.properties has cypher.cli.pinned=false; JetBrains releases require cypher.cli.pinned=true")
}
...
await $`git add packages/cypher-jetbrains/gradle.properties packages/cypher-jetbrains/CHANGELOG.md`
...
async function writeprops(ver: string) {
  ...
  const line = `cypher.jetbrains.version=${ver}`
  const next = current.match(/^cypher\.jetbrains\.version=/m)
    ? current.replace(/^cypher\.jetbrains\.version=.*$/m, line)
    : `${current.trim()}\n${line}\n`
...
- Review \`packages/cypher-jetbrains/gradle.properties\` and edit \`packages/cypher-jetbrains/CHANGELOG.md\` before merging.
