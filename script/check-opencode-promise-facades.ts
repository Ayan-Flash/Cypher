 ...
 * remaining Cypher Promise facades are migrated away. It also prevents tests
...
const testAllow: Record<string, { count: number; reason: string }> = {
  "cypher/config-resilience.test.ts": { count: 4, reason: "existing runtime integration test" },
  "cypher/config-validation.test.ts": { count: 2, reason: "existing runtime integration test" },
  "cypher/plan-followup.test.ts": { count: 4, reason: "existing runtime integration test" },
  "cypher/session/platform-attribution.test.ts": { count: 2, reason: "existing runtime integration test" },
  "cypher/session-prompt-queue.test.ts": { count: 5, reason: "prompt queue legacy instance bridge regression" },
  "server/experimental-session-list.test.ts": { count: 2, reason: "Cypher session list integration test" },
  "cypher/server/listener-runtime.test.ts": { count: 3, reason: "listener and AppRuntime integration test" },
  "tool/recall.test.ts": { count: 11, reason: "existing runtime integration test" },
}

const owned = (file: string) => file.startsWith("cypher/") || file.startsWith("cypher-sessions/")
