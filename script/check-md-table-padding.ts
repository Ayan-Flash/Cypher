 ...
 *
 * Enforcement scope (Cypher-owned paths only, to avoid upstream-sync churn):
 *   - Any top-level markdown file (TESTING.md, AGENTS.md, README.md, …)
 *   - Any path segment containing "cypher" or starting with "cypher-"
...
function skip(file: string) {
  ...
  if (parts.some((p) => p.includes("cypher") || p.startsWith("cypher-"))) return false
