...
// cypher_change start - keep JetBrains CLI pin reviewable outside CLI release commits
const jetbrainsPkg = fileURLToPath(new URL("../packages/cypher-jetbrains/package.json", import.meta.url))
const jetbrainsPin = await Bun.file(jetbrainsPkg).text()
// cypher_change end

// cypher_change start - consume changesets on the publish runner so changelog
...
{
  await $`bun install`
  const paths = ["packages/cypher-vscode/CHANGELOG.md", "packages/opencode/CHANGELOG.md"]
...
for (const file of pkgjsons) {
  // cypher_change start - create a follow-up PR for JetBrains CLI pin bumps
...
await $`bun install`
await import(`../packages/sdk/js/script/build.js`)

if (Script.release) {
  // cypher_change start - commit, tag, and push with rebase + retry to handle
  ...
  // cypher_change start - publish channel-aware GitHub release notes
  const { publishNotes } = await import("./cypher/release-notes.js")
  await publishNotes({
...
console.log("\n=== cli ===\n")
await import(`../packages/opencode/script/publish.js`)

console.log("\n=== sdk ===\n")
await import(`../packages/sdk/js/script/publish.js`)

console.log("\n=== plugin ===\n")
await import(`../packages/plugin/script/publish.js`)

// cypher_change start
console.log("\n=== vscode ===\n")
await import(`../packages/cypher-vscode/script/publish.js`)
// cypher_change end

// cypher_change start - Cypher does not ship the opencode desktop app
// if (Script.release) {
...
// cypher_change start - non-blocking JetBrains CLI pin bump PR after stable CLI release
...
async function createJetbrainsPinPr() {
  ...
  const result = await $`bun .cypher/skills/release-jetbrains/script/set-pin.ts --version ${Script.version} --pr`.nothrow()
