...
const repo = process.env.GH_REPO ?? "Cypher-Org/cypher"
...
const order = ["Core", "TUI", "SDK", "Extensions"] as const // cypher_change
const sections = {
  core: "Core",
  tui: "TUI",
  sdk: "SDK",
...
function section(areas: Set<string>) {
  const priority = ["core", "tui", "sdk", "plugin", "extensions/zed", "extensions/vscode", "github"] // cypher_change
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/opencode packages/sdk packages/plugin packages/extensions github`.text() // cypher_change
