...
/**
 * Verifies that every Cypher-specific change in shared upstream-owned source files
 * is annotated with a cypher_change marker.
 *
 * Usage:
 ...
 *   - contains a cypher_change marker comment           (inline annotation)
 *   - falls inside a cypher_change start/end block      (block annotation)
 ...
 *
 * Exempt paths (no markers needed — entirely Cypher-specific):
 *   - packages/opencode/src/cypher/**
 *   - packages/opencode/test/cypher/**
 *   - Any path containing "cypher" in directory or filename
 *   - Any path with a directory starting with "cypher-" (e.g. cypher-sessions/)
 *   - script/upstream/**
 *   - Cypher-specific annotation checker support files
 */
...
function isSource(file: string) {
  ...
  if (ext) return false
  return content(file).startsWith("#!") // cypher_change
}

// Parses the unified=0 diff for `file` against `base` and returns:
//   - added: every added line number on HEAD
//   - revert: true when the file's diff removes any cypher_change marker.
//     In that case the changes are reverting Cypher modifications back to the
...
//     Cypher construct (e.g. `unixSkip(` → `unix(`) often live in different
...
// Matches the start of a cypher_change marker in JS, JSX, YAML, TOML, and shell comments.
...
function coveredLines(text: string): { lines: string[]; covered: Set<number> } {
  ...
  // Whole-file annotation: first non-shebang non-empty line is a cypher_change - new file marker.
...
for (const file of files) {
  ...
  if (revert) continue // cypher_change - file is reverting Cypher modifications back to upstream

  const text = content(file) // cypher_change
...
if (violations.length === 0) {
  console.log("All shared upstream changes are annotated with cypher_change markers.")
  process.exit(0)
}

console.error(
  [
    "Unannotated Cypher changes found in shared upstream files:",
    "",
    ...violations,
    "",
    "Every Cypher-specific change in shared upstream source files must be annotated.",
    "",
    "Checked paths:",
    ...SCOPES.map((scope) => `  - ${scope}/**`),
    "",
    "Inline (single line):",
    "  const url = Flag.CYPHER_MODELS_URL || 'https://models.dev' // cypher_change",
    "",
    ...
    "  - packages/opencode/src/cypher/**",
    "  - packages/opencode/test/cypher/**",
    "  - Any path containing 'cypher' in the directory or filename",
    "  - Any directory starting with 'cypher-' (e.g. cypher-sessions/)",
    "  - script/upstream/**",
    "  - Cypher-specific annotation checker support files",
    "",
