...
if (prop !== ver)
  throw new Error(`packages/cypher-jetbrains/gradle.properties cypher.jetbrains.version is ${prop}, expected ${ver}`)
if (!(await pinned())) {
...
const changelog = await Bun.file("packages/cypher-jetbrains/CHANGELOG.md").text()
...
async function props() {
  const text = await Bun.file("packages/cypher-jetbrains/gradle.properties").text()
  const line = text.split(/\r?\n/).find((item) => item.startsWith("cypher.jetbrains.version="))
  const value = line?.split("=", 2)[1]?.trim()
  if (!value) throw new Error("packages/cypher-jetbrains/gradle.properties is missing cypher.jetbrains.version")
  return value
}
