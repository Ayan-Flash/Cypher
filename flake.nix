{
  description = "Cypher development flake";
    ...
    in
    {
      devShells = forEachSystem (pkgs: {
        default =
          let
            ...
            cypher-dev = pkgs.writeShellScriptBin "cypher-dev" ''
              set -euo pipefail
            ...
            cypher-install-bin = pkgs.writeShellScriptBin "cypher-install" ''
              set -euo pipefail

              CACHE_DIR="$HOME/.cache/cypher-nix"
              VERSION="''${1:-latest}"
              ...
              filename="cypher-$target$ext"

              if [ "$VERSION" = "latest" ]; then
                url="https://github.com/Cypher-Org/cypher/releases/latest/download/$filename"
                echo "Installing latest version of cypher..." >&2
              else
                # Strip leading 'v' if present
                VERSION="''${VERSION#v}"
                url="https://github.com/Cypher-Org/cypher/releases/download/v''${VERSION}/$filename"
                echo "Installing cypher version $VERSION..." >&2
              fi
              ...
              if ! ${pkgs.curl}/bin/curl -fsSL -o "$tmp_dir/$filename" "$url"; then
                echo "Error: Failed to download cypher from $url" >&2
                echo "Please check your internet connection or visit https://github.com/Cypher-Org/cypher/releases" >&2
                exit 1
              fi
              ...
              # Install the binary
              KILO_BIN="$CACHE_DIR/cypher"
              mv "$tmp_dir/cypher" "$KILO_BIN"
              chmod +x "$KILO_BIN"

              # Get the installed version
              installed_version=$("$KILO_BIN" --version 2>/dev/null || echo "unknown")
              echo "Successfully installed cypher $installed_version to $KILO_BIN" >&2
            '';

            cypher-bin = pkgs.writeShellScriptBin "cypher" ''
              set -euo pipefail
              ...
              if [ ! -f "$KILO_BIN" ]; then
                echo "Error: cypher is not installed in the cache." >&2
                echo "Please run 'cypher-install' first to download and install cypher." >&2
                echo "" >&2
                echo "Examples:" >&2
                echo "  cypher-install          # Install latest version" >&2
                echo "  cypher-install 1.0.180  # Install specific version" >&2
                exit 1
              fi
      ...
      packages = forEachSystem (
        ...
        let
          ...
          cypher = pkgs.callPackage ./nix/cypher.nix {
            inherit node_modules;
          };
        in
        {
