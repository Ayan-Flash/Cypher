...
How to spin up the **local main-branch** Cypher backend and test it with `curl` / `fetch`. Aimed at a running Cypher CLI agent iterating on backend fixes without rebuilding the VS Code extension or TUI.
...
CYPHER_SERVER_PASSWORD="$PASS" bun dev serve --port 0 >/tmp/cypher-serve.log 2>&1 &
echo $! >/tmp/cypher-serve.pid
while ! grep -q "cypher server listening" /tmp/cypher-serve.log 2>/dev/null; do sleep 0.1; done
PORT=$(grep -oE "listening on http://[^:]+:[0-9]+" /tmp/cypher-serve.log | grep -oE "[0-9]+$")
AUTH="Authorization: Basic $(printf 'cypher:%s' "$PASS" | base64 | tr -d '\n')"
BASE="http://127.0.0.1:$PORT"

# Call any endpoint
curl -sS -H "$AUTH" -H "x-cypher-directory: $PWD" "$BASE/global/health"

# Stop when done
kill "$(cat /tmp/cypher-serve.pid)" 2>/dev/null || true
rm -f /tmp/cypher-serve.pid /tmp/cypher-serve.log
```

## 1. What this doc is for

Testing local backend fixes against a real running server, talking to it over HTTP the same way the VS Code extension, TUI, and `cypher run --attach` do — but without any of those clients. Every request is a `curl` the agent can copy-paste.

For in-process tests (no socket, fastest loop) see `packages/opencode/test/cypher/server/permission-allow-everything.test.ts` for the `Server.Default().app.request(...)` pattern. That's the right tool inside the `packages/opencode/` test suite; this doc is for out-of-process HTTP testing.

## 2. `cypher serve` vs `bun dev serve` — important

| Command | What it runs |
|---|---|
| `cypher serve` | The npm-installed production CLI on `$PATH`. **Not the code in this repo.** |
...
Do **not** use `createCypherServer()` from `@cypher/sdk/v2` to test local code: it spawns the PATH `cypher` binary (`packages/sdk/js/src/v2/server.ts:38-136`), which is the wrong tool here.
...
```bash
CYPHER_SERVER_PASSWORD=$(openssl rand -hex 16) \
  bun dev serve --port 0 >/tmp/cypher-serve.log 2>&1 &
echo $! >/tmp/cypher-serve.pid
while ! grep -q "cypher server listening" /tmp/cypher-serve.log; do sleep 0.1; done
PORT=$(grep -oE "listening on http://[^:]+:[0-9]+" /tmp/cypher-serve.log | grep -oE "[0-9]+$")
```
...
```bash
CYPHER_SERVER_PASSWORD=secret \
  bun dev serve --port 4096 --hostname 127.0.0.1 \
  >/tmp/cypher-serve.log 2>&1 &
echo $! >/tmp/cypher-serve.pid
while ! grep -q "cypher server listening" /tmp/cypher-serve.log; do sleep 0.1; done
PORT=4096
```
...
Omit `CYPHER_SERVER_PASSWORD` entirely. The server prints `Warning: CYPHER_SERVER_PASSWORD is not set; server is unsecured.` and the auth middleware is bypassed — fine for throwaway local testing, never for anything else.
...
| `--mdns-domain` | `cypher.local` |  |
| `--cors` | `[]` | Extra allowed origins. |

## 4. The two mandatory request knobs

### Auth header (only if `CYPHER_SERVER_PASSWORD` was set)

```bash
AUTH="Authorization: Basic $(printf 'cypher:%s' "$CYPHER_SERVER_PASSWORD" | base64 | tr -d '\n')"
```

The username is literally `cypher` (same as the VS Code extension). Skip this whole block if you launched without a password.

### Directory header on every call

```bash
DIR_HEADER="x-cypher-directory: $PWD"
```
...
|---|---|
| `CYPHER_SERVER_PASSWORD` | Enable Basic auth. Omit for auth-bypassed local testing. |
| `CYPHER_DB=":memory:"` | Skip on-disk SQLite — hermetic runs. |
| `CYPHER_DISABLE_DEFAULT_PLUGINS=true` | Don't auto-load bundled plugins. |
| `CYPHER_WORKSPACE_ID=<id>` | Single-workspace mode; disables control-plane routes. |
| `CYPHER_TELEMETRY_LEVEL=off` | Disable PostHog during tests. |
| `CYPHER_CONFIG_CONTENT='{…}'` | Inline JSON config without writing a file. |
...
import { createCypherClient } from "@cypher/sdk/v2"

const port = process.env.PORT!
const pass = process.env.CYPHER_SERVER_PASSWORD
const headers = pass ? { Authorization: "Basic " + Buffer.from("cypher:" + pass).toString("base64") } : undefined
...
```bash
PORT="$PORT" CYPHER_SERVER_PASSWORD="$CYPHER_SERVER_PASSWORD" bun /tmp/probe.ts
rm /tmp/probe.ts
```

Reminder: this script **connects to** the server you launched in Section 3 — it does not start one. `createCypherServer()` from the SDK would spawn the PATH `cypher` binary (production CLI), which defeats the point of testing local code.

## 9. Pitfalls

- Running `cypher serve` instead of `bun dev serve` runs the installed prod binary, not your edits.
- Missing `x-cypher-directory` (or `?directory=`) returns `400` from `InstanceMiddleware`.
...
- `CYPHER_SERVER_PASSWORD` must be set **before** launch — changing it after doesn't rotate credentials.
- When sharing `/tmp/cypher-serve.log` / `.pid` across terminals, unique-suffix the paths to avoid clobbering parallel runs.
