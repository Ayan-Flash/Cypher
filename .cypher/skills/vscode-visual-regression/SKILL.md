...
description: Write Storybook stories and visual regression tests for the Cypher VS Code extension webview UI
---

Use this skill when the user asks you to add visual regression tests, screenshot tests, or Storybook stories for components in `packages/cypher-vscode/`.
...
Stories live in `packages/cypher-vscode/webview-ui/src/stories/`. Existing files and their scope:
...
Snapshots live at `packages/cypher-vscode/tests/visual-regression.spec.ts-snapshots/`:
...
Key settings in `packages/cypher-vscode/playwright.config.ts`:
...
- `packages/util/**`
- `packages/sdk/js/**`
- `packages/cypher-vscode/webview-ui/**`
- `packages/cypher-vscode/.storybook/**`
- `packages/cypher-vscode/tests/visual-regression*`
...
cypher-ui components are imported via deep subpaths:

```tsx
import { Part } from "@cypher/cypher-ui/message-part"
import { BasicTool } from "@cypher/cypher-ui/basic-tool"
...
```tsx
import type { AssistantMessage as SDKAssistantMessage, TextPart, ToolPart } from "@cypher/sdk/v2"
...
```
globals=colorScheme:dark;theme:cypher-vscode;vscodeTheme:dark-modern
```

The `.storybook/preview.tsx` applies these via a decorator that calls `applyVscodeTheme()` / `applyCypherTheme()` from cypher-ui. Stories do NOT need to handle theming — it happens automatically.
