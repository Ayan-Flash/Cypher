...
const path = "packages/cypher-jetbrains/CHANGELOG.md"
...
if (values.help) {
  console.log(
    `Usage: bun .cypher/skills/release-jetbrains/script/update-changelog.ts --version <version> --file <section.md>`,
  )
  process.exit(0)
}
...
async function temp(body: object) {
  const file = `/tmp/cypher-jetbrains-changelog-${Date.now()}.json`
