...
**Do not load the `cypher-merge-minimizer` skill.** That skill is for
authoring new Cypher changes against shared upstream files; during an upstream
merge it gives the wrong guidance (it nudges toward extracting Cypher logic out
...
- does a `cypher_change` marker in HEAD encode a bug fix, a feature, or a
  defensive check?
...
When HEAD includes a non-obvious Cypher-specific wrapper (e.g. a helper in
`packages/opencode/src/cypher/`), find out why it exists before deciding to
keep or bypass it:

```bash
git log --all --oneline -S "<symbol>" -- packages/opencode/src/cypher/
git log --all --oneline -- <cypher-file>
```
...
- one-sentence rationale (what Cypher behaviour is preserved, what upstream
...
3. State which Cypher behaviour must survive and why (reference PR numbers /
   `cypher_change` comments when possible).
...
- `.worktrees/opencode-merge/cypher-main` — Cypher base snapshot
- `.worktrees/opencode-merge/auto-merge` — automated merge snapshot (original
  conflict reference)

Apply in order:

- prefer upstream code and architecture whenever compatible with Cypher behaviour
- preserve Cypher-specific behaviour marked with `cypher_change`
- keep `cypher_change` markers around Cypher-specific code in shared opencode
  files
- when upstream refactors a region that HEAD had annotated with
  `cypher_change`, **check whether the marker encodes a bug fix or a feature
  ...
  `Effect.gen` block.
- when a `take-theirs` drops a line that was the target of a Cypher pre-filter,
  the upstream line may be actively wrong for Cypher — e.g. an inner `continue`
  filter whose condition collides with an outer filter Cypher added. Re-read the
  surrounding 20 lines before committing to `take-theirs`.
- if Cypher-specific code must be refactored to fit new upstream architecture,
  explain the refactor in the final summary
- if upstream moved the relevant logic to another file, port the Cypher behaviour
  there and list both paths in the final summary. Verify the new file already
  carries the Cypher-renamed symbols (e.g. `x-cypher-directory`) by diffing against
  pristine upstream.
- if upstream extracts shared policy into a helper, move Cypher-specific additions
...
- if upstream deleted a file, analyse whether the Cypher behaviour should be
...
**commenting it out with `cypher_change` markers** over deletion when the
surrounding structure (an `if`, a loop) still makes sense. That keeps the
intent visible to the next merger. Example:

```ts
} else if (input?.scope !== "project" && !Flag.CYPHER_EXPERIMENTAL_WORKSPACES) {
  // cypher_change start - directory filtering handled by CypherSession.filters above
  // if (input?.directory) {
...
shows up in `git diff --diff-filter=U`; the new sibling may need a Cypher feature
...
- `cypher_change` comments in the *auto-merge* snapshot that didn't end up in
...
- **Orphaned imports and references.** A rename upstream may leave a Cypher
...
- **Partial auto-merges.** Upstream may have refactored a region Cypher
  ...
  path, the file compiles upstream but breaks on Cypher.
...
- other CI guards that touched files imply (knip for `cypher-vscode/`,
  `check-cypher-change`, source-links, visual regression,
  `script/check-forbidden-strings.ts`)
- if you encounter a hardcoded upstream URL, repo path, or attribution string
  during conflict resolution that obviously shouldn't ship in Cypher (e.g. another
...
shared code, but the change cascades into Cypher-only files (cypher tests,
cypher-specific source, plugins) that still reference the old symbol. Those
...
### 9.6. Handle upstream-added tests that diverge from Cypher

Upstream sometimes adds tests that encode design contracts Cypher intentionally
breaks. These auto-merge cleanly and then fail. Three resolution patterns:

- **Rewrite the test** when the test is a contract assertion and Cypher has a
  different but equally valid contract. Invert or adjust the assertion with a
  `cypher_change` marker explaining the divergence.
- **Skip the test** when the test relies on patterns that Cypher has replaced
  ...
  Cypher's extensions break). Mark with `cypher_change` and a rationale
  explaining what would need to change for the test to run.
- **Delete the test** when it covers functionality Cypher deliberately removed
  (fallback paths, deprecated endpoints, products Cypher doesn't ship). Note
...
`packages/extensions/zed/extension.toml` (version field + 5 Cypher-Org download
...
(e.g. `1.14.30`), whose release tag does not exist on Cypher's pipeline, so the
...
(excluding `node_modules`, hidden dirs, and `packages/cypher-jetbrains/` which
tracks its own cadence), plus the Zed extension toml. It is idempotent — rerun
it any time to rebase the version back onto Cypher main (useful during
...
- **Non-trivial merge decisions**: a short section per file (or group of
  ...
  Focus on *what Cypher behaviour survived* and *what upstream features were
  adopted*. Link to Cypher PRs when a `cypher_change` encodes a specific fix.
...
- Upstream can reintroduce bugs a Cypher `cypher_change` had already fixed —
  during big refactors check every `cypher_change` the refactor touched.
- "Take-theirs" on an inner conditional is often wrong when Cypher added an outer
...
- Stricter DOM lib types (upstream TS upgrade) can surface latent casting
  issues around `WebSocket.send`, `Headers`, etc. — prefer narrowing the Cypher
...
- Cypher code may rely on ambient context (async-local storage, globally-set
  flags, process env) being populated at a lifecycle moment that upstream
  refactors away. If Cypher behaviour reads ambient state during init, forked
...
- Tests that intercept via process-global or module-global spies can become
  ...
  the spy.
- When Cypher extends a shared data shape with extra optional fields, different
  ...
  every Cypher addition — audit the encoding assumption before adding fields.
...
- Upstream-added tests can encode a design contract Cypher deliberately breaks.
  The test passes upstream because upstream doesn't share Cypher's requirement.
  Decide between refactoring Cypher to match the upstream contract or rewriting
  the test to assert Cypher's divergent contract — with a `cypher_change`
