...
## Affected Areas
- `packages/cypher-jetbrains/shared/src/main/kotlin/ai/cypher/rpc/dto/ProviderDto.kt`
- `packages/cypher-jetbrains/backend/src/main/kotlin/ai/cypher/backend/workspace/CypherWorkspaceState.kt`
- `packages/cypher-jetbrains/backend/src/main/kotlin/ai/cypher/backend/cli/CypherCliDataParser.kt`
- `packages/cypher-jetbrains/backend/src/main/kotlin/ai/cypher/backend/rpc/CypherWorkspaceDtoMapper.kt`
- `packages/cypher-jetbrains/frontend/src/main/kotlin/ai/cypher/client/session/model/SessionModel.kt`
- `packages/cypher-jetbrains/frontend/src/main/kotlin/ai/cypher/client/session/SessionUi.kt`
- `packages/cypher-jetbrains/frontend/src/main/kotlin/ai/cypher/client/settings/models/ModelsSettingsUi.kt`
- `packages/cypher-jetbrains/frontend/src/main/kotlin/ai/cypher/client/session/ui/model/ModelPicker*.kt`
- `packages/cypher-jetbrains/frontend/src/main/resources/messages/CypherBundle*.properties`
- Existing tests under `packages/cypher-jetbrains/frontend/src/test/...` and `backend/src/test/...`
...
2. Parse and map the extra metadata in the JetBrains backend.
   - Extend `ModelInfo` in `CypherWorkspaceState.kt` with matching nullable metadata fields.
   - Update `CypherCliDataParser.parseModel` and `parseModelDto` to read the fields currently used by VS Code: `inputPrice`, `outputPrice`, `contextLength`, `releaseDate`, `latest`, `cost`, `capabilities.input`, `options.description`, `autoRouting.models`, and `terminalBench`.
   ...
   - Update `CypherWorkspaceDtoMapper.model` to copy all new fields to `ModelDto`.
...
5. Rework popup layout and size behavior.
   ...
   - Persist state under a key such as `cypher.model.picker.expanded` using `PropertiesComponent.getInstance().getBoolean(key, false)` and `setValue(key, value.toString())`.
...
7. Add localization strings.
   - Add base `CypherBundle.properties` entries for maximize/minimize tooltips, detail labels, pricing labels, context, capabilities, terminal-bench, description, auto-routing choices, and unavailable values.
...
## Validation
- From `packages/cypher-jetbrains/`: run `./gradlew typecheck` or `bun run typecheck`.
- From `packages/cypher-jetbrains/`: run targeted tests for backend parser/serialization and frontend model picker tests, or `./gradlew test` if targeted Gradle filters are not straightforward.
