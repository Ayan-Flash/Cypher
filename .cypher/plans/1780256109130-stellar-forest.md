...
- GitHub issues: `https://github.com/Cypher-Org/cypher/issues/new/choose`
- Discord: `https://cypher.ai/discord`
- Customer support: `https://cypher.ai/support`

The popup must hide when clicking outside or pressing Escape.

## Findings

- The JetBrains empty state is `packages/cypher-jetbrains/frontend/src/main/kotlin/ai/cypher/client/session/ui/EmptySessionPanel.kt`.
...
- The VS Code implementation is:
  - Button: `packages/cypher-vscode/webview-ui/src/components/chat/MessageList.tsx`
  - Popup content: `packages/cypher-vscode/webview-ui/src/components/chat/FeedbackDialog.tsx`
  - Styling: `packages/cypher-vscode/webview-ui/src/styles/welcome.css`
...
- IntelliJ platform icons found in `AllIcons`:
  ...
  - Discord: no platform icon found. Add a Discord icon by borrowing the existing VS Code/cypher-ui Discord SVG artwork and adapting it into the JetBrains plugin resources with IntelliJ-compatible SVG colors and dark variant if needed.

## Implementation Steps

1. Add localized JetBrains bundle keys.
   - Add base English strings to `frontend/src/main/resources/messages/CypherBundle.properties`:
   ...
   - Add matching keys to localized `CypherBundle_*.properties` files. If accurate translations are not already available from the VS Code i18n files, copy the VS Code translations for matching locales where practical and leave English fallback only if the JetBrains bundle mechanism supports it cleanly.
...
3. Implement the feedback popup in frontend Swing code.
   - Add a small popup builder method/class inside `EmptySessionPanel.kt` or a new nearby UI file if the file would become too large.
    - Content should mirror VS Code:
      - Cypher logo at top using existing `/icons/cypher-content.svg`
      - message text
   ...
   - Load the Discord icon with `IconLoader.getIcon("/icons/discord.svg", EmptySessionPanel::class.java)` or a small Cypher-owned icon object if multiple call sites need it.
...
4. Add test coverage in `EmptySessionPanelTest.kt`.
   - Assert feedback button text uses `CypherBundle.message("feedback.button")`.
...
6. Verify.
   - Run the focused frontend/JetBrains tests first, preferably from `packages/cypher-jetbrains/`:
     - `./gradlew :frontend:test --tests "ai.cypher.client.session.ui.EmptySessionPanelTest"`
   - Run JetBrains typecheck from `packages/cypher-jetbrains/`:
...
- Do not modify shared opencode files; this work is entirely under `packages/cypher-jetbrains/` plus a changeset.
