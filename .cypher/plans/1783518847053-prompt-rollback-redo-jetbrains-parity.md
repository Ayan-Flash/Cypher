...
- CLI already exposes everything needed; no CLI release or SDK regen required. Pinned CLI version is `7.4.1` (`packages/cypher-jetbrains/package.json`).
...
- JetBrains split-mode packages differ from AGENTS.md's stated `ai.cypher.jetbrains`; actual source roots are:
  - shared → `packages/cypher-jetbrains/shared/src/main/kotlin/ai/cypher/rpc/`
  - backend → `packages/cypher-jetbrains/backend/src/main/kotlin/ai/cypher/backend/`
  - frontend → `packages/cypher-jetbrains/frontend/src/main/kotlin/ai/cypher/client/`
- All target files confirmed to exist: `SessionDto.kt`, `CypherSessionRpcApi.kt`, `CypherBackendChatManager.kt`, `CypherSessionRpcApiImpl.kt`, `CypherSessionService.kt`, `SessionController.kt`, `SessionModel.kt`, `SessionModelEvent.kt`, `SessionView.kt`, `MessageView.kt`, `MessageToolbar.kt`, `PromptPanel.kt`, `PromptEditorTextField.kt`, plus reuse targets `DiffStatBadge.kt`, `HoverIcon.kt`, `PermissionDiffView.kt`.
...
File: `packages/cypher-vscode/webview-ui/src/i18n/en.ts:138`
...
- In the 19 non-English locale files under `packages/cypher-vscode/webview-ui/src/i18n/` (ar, br, bs, da, de, es, fr, it, ja, ko, nl, no, pl, ru, th, tr, uk, zh, zht), overwrite `revert.banner.hint` with the identical new English string; leave a note that these need re-translation.
...
- `shared/.../rpc/CypherSessionRpcApi.kt`: add
...
- `backend/.../app/CypherBackendChatManager.kt`: add `revert(id, dir, messageID, partID?)` → `POST /session/$id/revert?directory=…` with `{messageID, partID?}` body, and `unrevert(id, dir)` → `POST /session/$id/unrevert?directory=…`. Mirror the existing `compact()` OkHttp pattern; add JSON body builders to `CypherCliDataParser`.
- `backend/.../rpc/CypherSessionRpcApiImpl.kt`: implement both, resolve worktree dir via `sessions.getDirectory(id, directory)`, wrap in `ready { }` like `compact`/`abort`.

### 3. Frontend service

- `frontend/.../app/CypherSessionService.kt`: add `suspend revert(...)` / `unrevert(...)` wrappers over the RPC, using `durable {}` per split-mode rules.
...
- New `session/ui/RevertBanner.kt` Swing view, built once and mutated via `update()` per retained-Swing rules. Listens to `RevertChanged` + `DiffUpdated`. Contents:
  ...
  - per-file diff rows reusing `ai.cypher.client.ui.DiffStatBadge` fed from `model.diff` (`DiffFileDto.additions/deletions`),
...
### 7. i18n (`frontend/src/main/resources/messages/CypherBundle*.properties`)

- Add to base `CypherBundle.properties`: `revert.banner.count.one`, `revert.banner.count.other`, `revert.banner.redo`, `revert.banner.redo.all`, `revert.banner.hint` (new wording), `revert.disabled.agentBusy`.
- Other locale bundles fall back to base until translated; do not add stale translations.

### 8. Tests

- Backend: extend `CypherSessionRpcApiImplTest` / a `CypherBackendChatManager` test against `MockCliServer` asserting the exact HTTP path + JSON body for `revert`/`unrevert`, and that a session carrying `revert` deserializes into `SessionRevertDto`.
...
| Shared | `SessionDto.kt` (+`SessionRevertDto`), `CypherSessionRpcApi.kt` |
| Backend | `CypherBackendChatManager.kt`, `CypherCliDataParser`, `CypherSessionRpcApiImpl.kt` |
| Frontend service/controller/model | `CypherSessionService.kt`, `SessionController.kt`, `SessionModel.kt`, `SessionModelEvent.kt` |
| Frontend views | new `RevertBanner.kt` (reusing `DiffStatBadge`), `MessageView.kt`, `MessageToolbar.kt`, `SessionView.kt`, `PromptPanel.kt` |
| i18n | `CypherBundle.properties` |
| Tests | backend RPC/chat-manager, frontend controller + view tests, `FakeSessionRpcApi` |

## Validation

- VS Code string change: `bun run typecheck` from repo root (or from `packages/cypher-vscode/`).
- JetBrains: `./gradlew typecheck` and `./gradlew test` from `packages/cypher-jetbrains/` (Java 21).
- Manual: `./gradlew runIde` (or split mode) to exercise rollback → redo → redo-all and reverted-transcript hiding.

## Risks / Notes

- Confirm no `cypher_change` markers are needed for the VS Code i18n edits (Cypher-owned webview path — none required).
