...
All work is inside `packages/cypher-jetbrains/`. No shared `packages/opencode/` files change,
so no `cypher_change` markers are needed.
...
- Test harnesses already exist: frontend `SessionControllerTestBase` + `FakeSessionRpcApi`
  ...
  `CypherBackendChatManagerTest`. Banner tests instantiate `RevertBanner(model, redo, redoAll)`
...
- `frontend/src/main/resources/messages/CypherBundle.properties` (+ translated `CypherBundle_*.properties`)
- `backend/.../app/CypherBackendChatManager.kt` (`revert`/`unrevert`), `backend/.../cli/CypherCliDataParser.kt` (`buildRevertJson`)
- `backend/.../app/CypherBackendSessionManager.kt` (`revertDto` duplication)
- Tests: `frontend/.../session/controller/TurnLifecycleTest.kt` (or new `RevertFlowTest.kt`),
  ...
  `backend/.../testing/MockCliServer.kt`, `backend/.../app/CypherBackendChatManagerTest.kt`,
  `backend/.../cli/CypherCliDataParserTest.kt`
...
1. Add bundle key in `CypherBundle.properties` (place near the other `revert.banner.*` keys):
   - `revert.banner.filesNotRestored=Snapshots are off — only the conversation was reverted; your files were not changed.`
   - Add the same key to every translated `CypherBundle_*.properties` file. If translations are
...
2. In `RevertBanner.kt`:
   ...
   - In `update()`, after computing `revert`, set
     ...
     `CypherBundle.message("revert.banner.filesNotRestored")`.
...
4. In `CypherBackendSessionManager.kt`, collapse the two identical `revertDto(...)` overloads
   ...
   a pure refactor covered by existing `CypherBackendSessionManagerTest` parsing assertions.
...
9. `MockCliServer.kt` — add routing + capture fields (the current generic `/session/ses_*`
   ...
   - Cases (place beside the `summarize`/`prompt_async` cases):
     ...
     - `bare.matches(Regex("/session/ses_[^/]+/unrevert")) && method == "POST"` → store path+body,
       `respond(output, 200, sessionCreate)`.
10. `CypherBackendChatManagerTest.kt` — mirror the existing `compact posts summarize request` test:
...
11. `CypherCliDataParserTest.kt` — add `buildRevertJson` unit tests:
...
12. Remove the unused bundle key `revert.disabled.agentBusy` from `CypherBundle.properties` and all
...
Run from `packages/cypher-jetbrains/`:
...
- Targeted while iterating: run the touched test classes (`TurnLifecycleTest`/`RevertFlowTest`,
  `SessionMessageListPanelTest`, `SessionModelTest`, `CypherBackendChatManagerTest`,
  `CypherCliDataParserTest`).
...
Add `.changeset/<slug>.md` (`"@cypher/cypher-jetbrains": patch`) describing the user-facing
