...
## Findings
- Runtime colorization exists only in `frontend/src/main/kotlin/ai/cypher/client/ui/SvgIconColorizer.kt`.
- The only production caller is `frontend/src/main/kotlin/ai/cypher/client/session/scroll/ScrollButtonIcon.kt`.
...
- Other used frontend icons already have light/dark pairs:
  ...
  - `cypher.svg` / `cypher_dark.svg`
  - `cypher-content.svg` / `cypher-content_dark.svg`
  - `plus.svg` / `plus_dark.svg` exists, though no current direct source reference was found.
  - `cypher@20x20.svg` / `cypher@20x20_dark.svg` exists, though no current direct source reference was found.
...
2. Delete `frontend/src/main/kotlin/ai/cypher/client/ui/SvgIconColorizer.kt` after confirming no callers remain.
...
6. Run the smallest relevant verification from `packages/cypher-jetbrains/`:
   - `bun run typecheck` or `./gradlew typecheck`.

## Files Expected To Change
- `packages/cypher-jetbrains/frontend/src/main/kotlin/ai/cypher/client/session/scroll/ScrollButtonIcon.kt`
- `packages/cypher-jetbrains/frontend/src/main/kotlin/ai/cypher/client/session/ui/style/SessionUiStyle.kt`
- Delete `packages/cypher-jetbrains/frontend/src/main/kotlin/ai/cypher/client/ui/SvgIconColorizer.kt`
- Add `packages/cypher-jetbrains/src/main/resources/META-INF/pluginIcon_dark.svg`
