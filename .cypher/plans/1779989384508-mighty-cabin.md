...
5. Verification.
   - Run the targeted JetBrains frontend session scroll tests from `packages/cypher-jetbrains/` using Gradle test filtering for `SessionScrollTest` if supported.
   - Run `./gradlew typecheck` from `packages/cypher-jetbrains/` if the targeted tests pass or if Gradle filtering is unavailable.
...
- `packages/cypher-jetbrains/frontend/src/test/kotlin/ai/cypher/client/session/SessionScrollTest.kt`
