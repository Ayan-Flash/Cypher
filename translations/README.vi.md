...
Cài trực tiếp [tiện ích Cypher CLI](vscode:extension/cypher.cypher-cli), hoặc tải từ [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=cypher.Cypher-Code). Tạo tài khoản và bạn sẽ có quyền truy cập hơn 500 mô hình, bao gồm GPT-5.5, Claude Opus 4.7, Claude Sonnet 4.6 và Gemini 3.1 Pro Preview, tất cả theo giá của nhà cung cấp.
...
Sau đó chạy `cypher` trong bất kỳ thư mục dự án nào để bắt đầu.
...
Cài [plugin Cypher CLI](https://plugins.jetbrains.com/plugin/28350-cypher-cli) từ JetBrains Marketplace, hoặc tìm "Cypher CLI" trong `Settings → Plugins` bên trong bất kỳ JetBrains IDE nào.
...
Chạy Cypher từ web, không cần máy cục bộ, tại [app.cypher.ai/cloud](https://app.cypher.ai/cloud).
...
Thiết lập review code tự động bằng AI cho pull request của bạn tại [app.cypher.ai/code-reviews](https://app.cypher.ai/code-reviews).
...
Khởi chạy AI agent luôn hoạt động của bạn tại [app.cypher.ai/claw](https://app.cypher.ai/claw).
...
Tải binary mới nhất từ [trang Releases](https://github.com/Cypher-Org/cypher/releases).

| Nền tảng | Asset |
|---|---|
| Windows (hầu hết PC) | `cypher-windows-x64.zip` |
...
Ghi chú: `x64-baseline` là build tương thích cho CPU cũ không có AVX. `musl` là build liên kết tĩnh cho Alpine hoặc image Docker tối giản không có glibc. `cypher-vscode-*.vsix` là gói tiện ích VS Code, không phải CLI. Các archive `Source code` dùng để build từ mã nguồn.

</details>

### Agents

Cypher đi kèm các agents chuyên biệt để bạn chuyển đổi tùy theo tác vụ. Bạn cũng có thể tạo agents tùy chỉnh của riêng mình.
...
Tìm hiểu thêm về [agents và agents tùy chỉnh](https://cypher.ai/docs/code-with-ai/agents/using-agents).
...
Chạy `cypher run` với `--auto` để hoạt động hoàn toàn tự động không có prompts, dành cho pipeline CI/CD:
...
Về cấu hình và mọi thứ khác, hãy xem [tài liệu](https://cypher.ai/docs).

### Đóng góp

Chúng tôi chào đón đóng góp từ developer, writer và tất cả mọi người. Bắt đầu với [Contributing Guide](/CONTRIBUTING.md) để thiết lập môi trường, tiêu chuẩn code và cách mở pull request. Xem [RELEASING.md](../RELEASING.md) cho quy trình release tiện ích VS Code và CLI, và [packages/cypher-jetbrains/RELEASING.md](../packages/cypher-jetbrains/RELEASING.md) cho plugin JetBrains.
...
<details>
<summary>Cypher CLI đến từ đâu?</summary>

Cypher CLI là một fork của [OpenCode](https://github.com/anomalyco/opencode), được cải tiến để hoạt động trong nền tảng Cypher agentic engineering.

</details>

---

**Tham gia cộng đồng** [Discord](https://cypher.ai/discord) | [X](https://x.com/cypher) | [Reddit](https://www.reddit.com/r/cypher/)
