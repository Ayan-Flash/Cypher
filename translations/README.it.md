...
Installa direttamente l'[estensione Cypher CLI](vscode:extension/cypher.cypher-cli), oppure scaricala dal [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=cypher.Cypher-Code). Crea un account e avrai accesso a oltre 500 modelli, inclusi GPT-5.5, Claude Opus 4.7, Claude Sonnet 4.6 e Gemini 3.1 Pro Preview, tutti al prezzo del provider.
...
Poi esegui `cypher` in qualsiasi directory di progetto per iniziare.
...
Installa il [plugin Cypher CLI](https://plugins.jetbrains.com/plugin/28350-cypher-cli) dal JetBrains Marketplace, oppure cerca "Cypher CLI" in `Settings → Plugins` dentro qualsiasi IDE JetBrains.
...
Esegui Cypher dal web, senza una macchina locale, su [app.cypher.ai/cloud](https://app.cypher.ai/cloud).
...
Configura revisioni automatiche del codice con IA sulle tue pull request su [app.cypher.ai/code-reviews](https://app.cypher.ai/code-reviews).
...
Avvia il tuo agente IA sempre attivo su [app.cypher.ai/claw](https://app.cypher.ai/claw).
...
Scarica il binario più recente dalla [pagina Releases](https://github.com/Cypher-Org/cypher/releases).

| Piattaforma | Asset |
|---|---|
| Windows (la maggior parte dei PC) | `cypher-windows-x64.zip` |
...
Note: `x64-baseline` è una build di compatibilità per CPU più vecchie senza AVX. `musl` è la build collegata staticamente per Alpine o immagini Docker minimali senza glibc. `cypher-vscode-*.vsix` è il pacchetto dell'estensione VS Code, non la CLI. Gli archivi `Source code` servono per compilare dai sorgenti.

</details>

### Agents

Cypher include agents specializzati tra cui puoi passare in base all'attività. Puoi anche creare agents personalizzati.
...
Scopri di più su [agents e agents personalizzati](https://cypher.ai/docs/code-with-ai/agents/using-agents).
...
Esegui `cypher run` con `--auto` per un funzionamento completamente autonomo senza prompt, pensato per pipeline CI/CD:
...
<details>
<summary>Da dove viene Cypher CLI?</summary>

Cypher CLI è un fork di [OpenCode](https://github.com/anomalyco/opencode), migliorato per funzionare nella piattaforma di ingegneria agentica Cypher.

</details>

---

**Unisciti alla community** [Discord](https://cypher.ai/discord) | [X](https://x.com/cypher) | [Reddit](https://www.reddit.com/r/cypher/)
