...
Cypher CLI er en AI-kodeagent, der møder dig overalt, hvor du arbejder: [VS Code](https://cypher.ai/landing/vs-code), [JetBrains](https://cypher.ai/features/jetbrains-native) og [CLI](https://cypher.ai/cli). Den er open source med åben prissætning. Du vælger mellem mere end 500 modeller, skifter mellem dem midt i en opgave og betaler modeludbyderens pris uden tillæg. Ingen API-nøgler kræves for at komme i gang.

### Installation

Vælg, hvor du vil køre Cypher.
...
Installer [Cypher CLI-udvidelsen](vscode:extension/cypher.cypher-cli) direkte, eller hent den fra [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=cypher.Cypher-Code). Opret en konto, og du får adgang til mere end 500 modeller, herunder GPT-5.5, Claude Opus 4.7, Claude Sonnet 4.6 og Gemini 3.1 Pro Preview, alle til udbyderpris.
...
Kør derefter `cypher` i en vilkårlig projektmappe for at starte.
...
Installer [Cypher CLI-pluginet](https://plugins.jetbrains.com/plugin/28350-cypher-cli) fra JetBrains Marketplace, eller søg efter "Cypher CLI" i `Settings → Plugins` i en JetBrains IDE.
...
Kør Cypher fra webben, uden lokal maskine, på [app.cypher.ai/cloud](https://app.cypher.ai/cloud).
...
Opsæt automatiske AI-kodegennemgange på dine pull requests på [app.cypher.ai/code-reviews](https://app.cypher.ai/code-reviews).
...
Start din altid aktive AI-agent på [app.cypher.ai/claw](https://app.cypher.ai/claw).
...
Download den nyeste binære fil fra [Releases-siden](https://github.com/Cypher-Org/cypher/releases).
...
Bemærk: `x64-baseline` er en kompatibilitetsbuild til ældre CPU'er uden AVX. `musl` er den statisk linkede build til Alpine eller minimale Docker-images uden glibc. `cypher-vscode-*.vsix` er VS Code-udvidelsespakken, ikke CLI'en. `Source code`-arkiver er til at bygge fra kildekode.

</details>

### Agents

Cypher leveres med specialiserede agents, som du kan skifte mellem afhængigt af opgaven. Du kan også bygge dine egne brugerdefinerede agents.
...
Læs mere om [agents og brugerdefinerede agents](https://cypher.ai/docs/code-with-ai/agents/using-agents).
...
Kør `cypher run` med `--auto` for fuldt autonom drift uden prompts, bygget til CI/CD-pipelines:
...
For konfiguration og alt andet, se [dokumentationen](https://cypher.ai/docs).
...
Cypher CLI er en fork af [OpenCode](https://github.com/anomalyco/opencode), forbedret til at fungere i Cypher agentic engineering-platformen.

</details>

---

**Deltag i fællesskabet** [Discord](https://cypher.ai/discord) | [X](https://x.com/cypher) | [Reddit](https://www.reddit.com/r/cypher/)
