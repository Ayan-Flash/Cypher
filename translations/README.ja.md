...
Cypher CLI は、[VS Code](https://cypher.ai/landing/vs-code)、[JetBrains](https://cypher.ai/features/jetbrains-native)、[CLI](https://cypher.ai/cli) など、あなたが作業する場所で使える AI コーディングエージェントです。オープンソースで、透明な価格体系を採用しています。500 以上のモデルから選択し、タスクの途中で切り替え、追加料金なしでモデルプロバイダーの料金を支払います。開始に API キーは不要です。

### インストール

Cypher を実行する場所を選んでください。
...
[Cypher CLI 拡張機能](vscode:extension/cypher.cypher-cli)を直接インストールするか、[VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=cypher.Cypher-Code) から入手してください。アカウントを作成すると、GPT-5.5、Claude Opus 4.7、Claude Sonnet 4.6、Gemini 3.1 Pro Preview を含む 500 以上のモデルを、すべてプロバイダー価格で利用できます。
...
その後、任意のプロジェクトディレクトリで `cypher` を実行して開始します。
...
JetBrains Marketplace から [Cypher CLI プラグイン](https://plugins.jetbrains.com/plugin/28350-cypher-cli)をインストールするか、任意の JetBrains IDE の `Settings → Plugins` で "Cypher CLI" を検索してください。
...
ローカルマシンなしで、Web から [app.cypher.ai/cloud](https://app.cypher.ai/cloud) で Cypher を実行できます。
...
[app.cypher.ai/code-reviews](https://app.cypher.ai/code-reviews) で pull request に自動 AI コードレビューを設定できます。
...
[app.cypher.ai/claw](https://app.cypher.ai/claw) で常時稼働する AI エージェントを起動できます。
...
[Releases ページ](https://github.com/Cypher-Org/cypher/releases)から最新のバイナリをダウンロードしてください。

| プラットフォーム | アセット |
|---|---|
| Windows（ほとんどの PC） | `cypher-windows-x64.zip` |
...
注: `x64-baseline` は AVX のない古い CPU 向けの互換ビルドです。`musl` は Alpine や glibc のない最小 Docker イメージ向けの静的リンクビルドです。`cypher-vscode-*.vsix` は VS Code 拡張機能パッケージであり、CLI ではありません。`Source code` アーカイブはソースからビルドするためのものです。

</details>

### Agents

Cypher には、タスクに応じて切り替えられる特化型 agents が含まれています。独自のカスタム agents も作成できます。
...
[agents とカスタム agents](https://cypher.ai/docs/code-with-ai/agents/using-agents) について詳しく学べます。
...
CI/CD パイプライン向けに、プロンプトなしで完全自律動作させるには `cypher run` に `--auto` を指定します。
...
設定やその他の内容については、[ドキュメント](https://cypher.ai/docs)をご覧ください。

### コントリビューション

開発者、ライター、その他すべての方からのコントリビューションを歓迎します。環境設定、コーディング標準、pull request の作成方法については [Contributing Guide](/CONTRIBUTING.md) から始めてください。VS Code 拡張機能と CLI のリリース手順は [RELEASING.md](../RELEASING.md)、JetBrains プラグインについては [packages/cypher-jetbrains/RELEASING.md](../packages/cypher-jetbrains/RELEASING.md) を参照してください。
...
<details>
<summary>Cypher CLI はどこから来たのですか？</summary>

Cypher CLI は [OpenCode](https://github.com/anomalyco/opencode) の fork であり、Cypher agentic engineering プラットフォーム内で動作するように強化されています。

</details>

---

**コミュニティに参加** [Discord](https://cypher.ai/discord) | [X](https://x.com/cypher) | [Reddit](https://www.reddit.com/r/cypher/)
