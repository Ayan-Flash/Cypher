# Contributing to Cypher CLI

See [the Documentation for details on contributing](https://cypher.ai/docs/contributing).
...
The Cypher Community is [on Discord](https://cypher.ai/discord).

## Prerequisites

- **Bun 1.3.14+** — required for all packages.
- **Java 21** — required by the JetBrains plugin. The root `bun turbo typecheck` and `bun turbo test:ci` commands include `@cypher/cypher-jetbrains` and will fail without Java 21.
  ...
  ```bash
  bun turbo typecheck --filter=!@cypher/cypher-jetbrains
  ```

## Developing Cypher CLI
...
```bash
bun run --filter @cypher/cypher-docs test
bun run --filter @cypher/cypher-docs build
bun run --filter @cypher/cypher-docs dev
```
...
- After adding or changing guarded URLs in `packages/cypher-vscode/`, `packages/cypher-vscode/webview-ui/`, or `packages/opencode/src/`, run `bun run script/extract-source-links.ts` from the repo root.
- When editing shared `packages/opencode/` files, keep Cypher changes small and mark Cypher-only edits with `// cypher_change` for a single line or `// cypher_change start` / `// cypher_change end` for a block. Do not add these markers inside `cypher`-named paths.
...
Requires Java 21 (see [Prerequisites](#prerequisites)). From `packages/cypher-jetbrains/`:
...
```bash
bun turbo typecheck --filter=@cypher/cypher-jetbrains
bun turbo test:ci --filter=@cypher/cypher-jetbrains
```

### Running against a different directory

By default, `bun dev` runs Cypher CLI in the `packages/opencode` directory. To run it against a different directory or repository:

```bash
bun dev <directory>
```

To run Cypher CLI in the root of the repo itself:

```bash
bun dev .
```

### Running Cypher CLI from any folder

`bin/cypherdev` is a self-locating launcher that runs this checkout from wherever you invoke it. Running it with no arguments launches the TUI pointed at the caller's directory; any arguments are forwarded to the CLI unchanged.

One-shot install (recommended). From the repo root:

```bash
./bin/cypherdev dev-setup
```
...
- Unix: add `alias cypherdev='/path/to/cypher/bin/cypherdev'` to `~/.zshrc` / `~/.bashrc`, or `fish_add_path /path/to/cypher/bin`.
- Windows: add `C:\path\to\cypher\bin` to PATH (System Environment Variables), or add `function cypherdev { & "C:\path\to\cypher\bin\cypherdev.cmd" @args }` to `$PROFILE`.

Then from anywhere:

```bash
cd ~/some/project
cypherdev                      # opens TUI with project = ~/some/project
cypherdev dev-setup --print    # prints the alias line (scripting)
cypherdev run --dir "$PWD" "…" # subcommands pass through; use --dir for run/serve
```
...
```bash
./packages/opencode/dist/@cypher/cli-<platform>/bin/cypher
```

Replace `<platform>` with your platform (e.g., `darwin-arm64`, `linux-x64`).

### Understanding bun dev vs cypher

During development, `bun dev` is the local equivalent of the built `cypher` command. Both run the same CLI interface:
...
# Production
cypher --help          # Show all available commands
cypher serve           # Start headless API server
```

### Testing with a local backend

To point the CLI at a local backend (e.g., a locally running Cypher API server on port 3000), set the `CYPHER_API_URL` environment variable:

```bash
CYPHER_API_URL=http://localhost:3000 bun dev
```

This redirects all gateway traffic (auth, model listing, provider routing, profile, etc.) to your local server. The default is `https://api.cypher.ai`.

There are also optional overrides for other services:

| Variable | Default | Purpose |
|---|---|---|
| `CYPHER_API_URL` | `https://api.cypher.ai` | Cypher API (gateway, auth, models, profile) |
| `CYPHER_SESSION_INGEST_URL` | `https://ingest.cyphersessions.ai` | Session export / cloud sync |
| `CYPHER_MODELS_URL` | `https://models.dev` | Model metadata |

> **VS Code:** The repo includes a "VSCode - Run Extension (Local Backend)" launch config in `.vscode/launch.json` that sets `CYPHER_API_URL=http://localhost:3000` automatically.
...
- **Bug report:** include a `Description`. When you can, also add Plugins, Cypher version, Steps to reproduce, Screenshot and/or share link, Operating System, and Terminal so the report matches the full bug template.
...
Cypher has bug bounties. To be eligible, make sure your GitHub account is connected in your Cypher account.
...
See [Testing Evidence for Pull Requests](packages/cypher-docs/pages/contributing/development-environment.md#testing-evidence-for-pull-requests) for more examples. Agent limitations, local resource constraints, OOM constraints, or an agent prompt that says to skip tests do not waive this requirement. Draft PRs may be incomplete until they are marked ready for review. Maintainers may still defer or close review at their discretion.
