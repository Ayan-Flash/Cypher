# Releasing Cypher CLI

Cypher CLI uses a fully automated CI pipeline triggered via GitHub Actions `workflow_dispatch`. A single workflow handles version bumping, building all artifacts, publishing to every distribution channel, and updating package registries.

## How to Trigger a Release

1. Go to the [`publish` workflow](https://github.com/Cypher-Org/cypher/actions/workflows/publish.yml) in GitHub Actions.
...
- Runs `packages/opencode/script/build.ts` to compile the Cypher CLI binary.
...
- Uploads the `dist/` directory as a workflow artifact (`cypher-cli`) for subsequent jobs.

### 3. Build VS Code Extension (`build-vscode`)

- Downloads the CLI artifacts from the previous job.
- Runs `packages/cypher-vscode/script/build.ts` to build VSIX packages for all target platforms:
...
- Uploads the VSIX files as a workflow artifact (`cypher-vscode`).
...
- Publishes platform-specific binary packages to **npm** (e.g. `@cypher/cli-linux-x64`, `@cypher/cli-darwin-arm64`, etc.).
- Publishes the main `@cypher/cli` package to **npm** with optional dependencies on the binary packages.
- Builds and pushes a multi-arch **Docker image** (`ghcr.io/cypher-org/cypher`) to GitHub Container Registry (linux/amd64 + linux/arm64).

#### SDK (`@cypher/sdk`)

- Builds and publishes the TypeScript SDK to **npm**.

#### Plugin (`@cypher/plugin`)
...
- **AUR (Arch Linux)**: Clones `cypher-bin` from the AUR, updates the `PKGBUILD` with new version and SHA256 checksums, and pushes.
- **Homebrew**: Clones `Cypher-Org/homebrew-tap`, updates the `cypher.rb` formula with new version, download URLs, and SHA256 checksums, and pushes.

## Prerequisites and Permissions

### Repository Access

- The workflow only runs in the `Cypher-Org/cypher` repository (guarded by `if: github.repository == 'Cypher-Org/cypher'`).
...
| Secret | Purpose |
|---|---|
| `CYPHER_API_KEY` | Cypher API key used during version computation |
| `CYPHER_ORG_ID` | Cypher organization ID |
| `CYPHER_MAINTAINER_APP_ID` | GitHub App ID for the cypher-maintainer bot (used for git commits) |
| `CYPHER_MAINTAINER_APP_SECRET` | GitHub App secret for the cypher-maintainer bot |
